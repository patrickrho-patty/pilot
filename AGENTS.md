...
- `packages/skills-catalog/`: app-shipped skills catalog (`@pilotai/skills-catalog`)
- `packages/teams-catalog/`: app-shipped teams catalog (`@pilotai/teams-catalog`)
...
When your task produces a user-inspectable deliverable file, follow the Pilot skill's "Generated Artifacts and Work Products" workflow before final disposition. In this repo, prefer the self-contained skill helper at `skills/pilot/scripts/pilot-upload-artifact.sh` so the file is available through the Pilot API, create/update an artifact work product when the file is the deliverable, link the uploaded artifact in the final issue comment, and then set status. Do not rely on local filesystem paths as the only access path. If an important file intentionally remains workspace-only, create/update a work product with `metadata.resourceRef.kind: "workspace_file"` and a workspace-relative path, then name that work product and path in the final comment. Treat browse/search as a fallback for recovering workspace files, not the preferred deliverable path. See `doc/AGENT-ARTIFACTS.md` for details and `.mp4`/`.webm` examples.
