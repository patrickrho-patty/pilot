...
pilot_home="$(expand_home "${PILOT_HOME:-$HOME/.pilot}")"
...
if [[ "${PILOT_KILL_WORKSPACES_ONLY_CURRENT:-}" != "1" ]]; then
  for dir in \
    "$HOME"/.pilot/instances/*/runtime-services \
    "$HOME"/.pilot-worktrees/instances/*/runtime-services \
    "$REPO_ROOT"/.pilot/instances/*/runtime-services \
    "$REPO_ROOT"/.pilot/runtime-services/instances/*/runtime-services
  do
    append_runtime_dir "$dir"
  done

  for sibling_root in "$REPO_PARENT"/pilot*; do
    [[ -d "$sibling_root" ]] || continue
    for dir in \
      "$sibling_root"/.pilot/instances/*/runtime-services \
      "$sibling_root"/.pilot/runtime-services/instances/*/runtime-services
    do
      append_runtime_dir "$dir"
    done
  done
