...
const SCOPE_RE = /^@pilotai\/[a-z0-9][a-z0-9._-]*$/;
...
function usage() {
  process.stderr.write(
    [
      ...
      "",
      "Examples:",
      "  node scripts/bootstrap-npm-package.mjs @pilotai/new-package",
      "  node scripts/bootstrap-npm-package.mjs @pilotai/new-package --publish",
      "",
    ].join("\n"),
  );
}
  ...
  if (!SCOPE_RE.test(packageName)) {
    throw new Error(
      `refusing to publish a placeholder for ${JSON.stringify(packageName)}: ` +
        "the name must be a lowercase package inside the @pilotai scope " +
...
export function buildPlaceholderFiles(packageName) {
  const deprecationNote =
    ...
    "supersedes it. See https://github.com/patty-io/pilot";
  ...
  const packageJson = {
    ...
    repository: {
      type: "git",
      url: "git+https://github.com/patty-io/pilot.git",
    },
    homepage: "https://github.com/patty-io/pilot",
    publishConfig: {
      access: "public",
    },
  };
  ...
  const readme = [
    ...
    "[patty-io/pilot](https://github.com/patty-io/pilot).",
    "",
  ].join("\n");
  ...
  if (/\bE401\b|401 Unauthorized/i.test(output)) {
    throw new Error(
      [
        ...
        "Run `npm logout --registry=https://registry.npmjs.org/` and then `npm login` or `npm adduser` on this maintainer machine with an npm account that can publish to the @pilotai scope, then rerun with --publish.",
...
function printNextSteps(packageName) {
  process.stdout.write(
    [
      ...
      "3. Add repository patty-io/pilot",
