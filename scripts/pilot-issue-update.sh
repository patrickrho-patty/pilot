...
usage() {
  cat <<'EOF'
Usage:
  scripts/pilot-issue-update.sh [--issue-id ID] [--status STATUS] [--comment TEXT] [--dry-run]
...
Examples:
  scripts/pilot-issue-update.sh --issue-id "$PILOT_TASK_ID" --status in_progress <<'MD'
  ...
  scripts/pilot-issue-update.sh --issue-id "$PILOT_TASK_ID" --status done --dry-run <<'MD'
  Done
...
if [[ -z "$issue_id" ]]; then
  printf 'Missing issue id. Pass --issue-id or set PILOT_TASK_ID.\n' >&2
  exit 1
fi
...
curl -sS -X PATCH \
  "$PILOT_API_URL/api/issues/$issue_id" \
