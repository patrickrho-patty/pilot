...
if (worktreeEnvBootstrap.missingEnv) {
  console.error(
    `[pilot] linked git worktree at ${repoRoot} is missing ${path.relative(repoRoot, worktreeEnvBootstrap.envPath)}. Run \`pilotai worktree init\` in this worktree before \`pnpm dev\`.`,
  );
  process.exit(1);
}
...
const devServerStatusFilePath = path.join(repoRoot, ".pilot", "dev-server-status.json");
const devServerRestartRequestFilePath = path.join(repoRoot, ".pilot", "dev-server-restart-request.json");
const devServerStatusToken = mode === "dev" ? randomUUID() : null;
const devServerStatusTokenHeader = "x-pilot-dev-server-status-token";
...
const ignoredRelativePaths = new Set([
  ".pilot/dev-server-restart-request.json",
  ".pilot/dev-server-status.json",
]);
...
const managedRuntimeExposure = process.env.PILOT_MANAGED_RUNTIME_EXPOSURE === "tailscale_https";
...
for (let index = 0; index < cliArgs.length; index += 1) {
  ...
  if (arg === "--bind") {
    ...
    if (!value || value.startsWith("--") || !BIND_MODES.includes(value as BindMode)) {
      console.error(`[pilot] invalid --bind value. Use one of: ${BIND_MODES.join(", ")}`);
      process.exit(1);
    }
  ...
  if (arg === "--bind-host") {
    ...
    if (!value || value.startsWith("--")) {
      console.error("[pilot] --bind-host requires a value");
      process.exit(1);
    }
    bindHost = value;
    index += 1;
...
if (bindMode === "custom" && !bindHost) {
  console.error("[pilot] --bind custom requires --bind-host <host>");
  process.exit(1);
}
...
if (tailscaleAuth || bindMode) {
  ...
  if (tailscaleAuth) {
    console.log("[pilot] note: --tailscale-auth/--authenticated-private are legacy aliases for --bind lan");
  }
  env.PILOT_BIND = effectiveBind;
  if (bindHost) {
    env.PILOT_BIND_HOST = bindHost;
  } else {
    delete env.PILOT_BIND_HOST;
  }
  if (effectiveBind === "loopback" && !tailscaleAuth) {
    delete env.PILOT_DEPLOYMENT_MODE;
    delete env.PILOT_DEPLOYMENT_EXPOSURE;
    delete env.PILOT_AUTH_BASE_URL_MODE;
    console.log("[pilot] dev mode: local_trusted (bind=loopback)");
  } else {
    env.PILOT_DEPLOYMENT_MODE = "authenticated";
    env.PILOT_DEPLOYMENT_EXPOSURE = "private";
    env.PILOT_AUTH_BASE_URL_MODE = managedRuntimeExposure ? "explicit" : "auto";
    console.log(
      `[pilot] dev mode: authenticated/private (bind=${effectiveBind}${bindHost ? `:${bindHost}` : ""})`,
    );
  }
} else {
  delete env.PILOT_BIND;
  delete env.PILOT_BIND_HOST;
  delete env.PILOT_DEPLOYMENT_MODE;
  delete env.PILOT_DEPLOYMENT_EXPOSURE;
  delete env.PILOT_AUTH_BASE_URL_MODE;
...
if (existingRunner) {
  console.log(
    `[pilot] ${devService.serviceName} already running (pid ${existingRunner.pid}${typeof existingRunner.metadata?.childPid === "number" ? `, child ${existingRunner.metadata.childPid}` : ""})`,
  );
  process.exit(0);
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@pilotai/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[pilot] Command failed with code ${status.code}: pnpm --filter @pilotai/db exec tsx src/migration-status.ts --json\n`,
    );
...
async function maybePreflightMigrations(options: { interactive?: boolean; autoApply?: boolean; exitOnDecline?: boolean } = {}) {
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[pilot] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). Refusing to start watch mode against a stale schema.\n`,
      );
      process.exit(1);
    }
    return;
...
async function buildPluginSdk() {
  ...
  const result = await runPnpm(
    ["--filter", "@pilotai/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@pilotai/server", serverScript, ...forwardedArgs],
