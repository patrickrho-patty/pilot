...
const DEFAULT_QUERY = "\"Co-Authored-By: Pilot <noreply@pilot.test>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/pilot-commit-metrics-cache.json");
...
const PILOT_EMAIL = "noreply@pilot.test";
...
function printHelp() {
  console.log(`Usage: tsx scripts/pilot-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === PILOT_EMAIL) || (displayName && displayName.toLowerCase() === PILOT_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("Pilot commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "pilot-commit-metrics",
