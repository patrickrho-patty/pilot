...
// execution-workspace binding (PIL-17617). For each explicit issue=branch
...
//
// Usage:
//   node scripts/repair-pr-prep-workspace-attachment.mjs PIL-16555=PIL-14380-salvage-pil-9514 [more ISSUE=BRANCH ...] [--apply]
//
...
const apiUrl = process.env.PILOT_API_URL;
...
const headers = {
  ...
  ...(process.env.PILOT_RUN_ID ? { "x-pilot-Run-Id": process.env.PILOT_RUN_ID } : {}),
};
...
for (const { issue, branch } of pairs) {
  ...
  const response = await fetch(`${apiBase}/api/issues/${issue}`, {
    ...
    body: JSON.stringify({
      ...
      comment:
        `Repair for PIL-17617/PIL-17618: pinned this PR-preparation task to an isolated git worktree on the exact ` +
