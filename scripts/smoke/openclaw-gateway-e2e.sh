...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${OPENCLAW_TMP_DIR}/openclaw-pilot-smoke}"
...
PILOT_API_URL_FOR_OPENCLAW="${PILOT_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set PILOT_COOKIE or PILOT_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg pilotApiUrl "$PILOT_API_URL_FOR_OPENCLAW" \
...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/pilot"
  local claimed_file="${workspace_dir}/pilot-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/pilot-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/pilot-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/pilot-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/pilot"
    assert_status "200"
    {
      echo "PILOT_API_URL=${PILOT_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set PILOT_API_KEY=${AGENT_API_KEY} in your run context before running Pilot heartbeat steps."
...
main() {
  ...
  wait_http_ready "${PILOT_API_URL%/}/api/health" 15 || fail "Pilot API health endpoint not reachable"
