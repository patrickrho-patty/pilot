...
PILOT_HOST_PORT="${PILOT_HOST_PORT:-3100}"
PILOT_HOST_FROM_CONTAINER="${PILOT_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.pilot-openclaw.override.yml"
...
detect_pilot_base_url() {
  ...
  for candidate in "$PILOT_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${PILOT_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${PILOT_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
pilot_base_url="$(detect_pilot_base_url || true)"
...
Pilot URL for OpenClaw container:
EOF
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${PILOT_HOST_PORT}
  (Do not use http://127.0.0.1:${PILOT_HOST_PORT} inside the container.)
  If Pilot rejects the host, run on host machine:
    pnpm pilotai allowed-hostname host.docker.internal
  Then restart Pilot and re-run this script.
EOF
  fi
  cat <<EOF
