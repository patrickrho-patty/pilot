...
publish_package_to_npm() {
  ...
  publish_log="$(mktemp "${TMPDIR:-/tmp}/pilot-npm-publish.XXXXXX")"
...
verify_npm_installable() {
  ...
  install_dir="$(mktemp -d "${TMPDIR:-/tmp}/pilot-release-install.XXXXXX")"
  ...
  installed_version="$(node -e "console.log(require(process.argv[1]).version)" "$install_dir/node_modules/pilotai/package.json")"
  rm -rf "$install_dir"
