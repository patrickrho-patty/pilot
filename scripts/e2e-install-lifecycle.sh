...
#
# Env knobs:
#   E2E_REPO          GitHub repo to install from (default: pilotai/pilot)
...
# bootstrap simulates what `npx pilotai@<channel> install` will run post-release:
...
BOOTSTRAP_CLI="$HOME/e2e-bootstrap-cli/node_modules/pilotai/dist/index.js"
...
mkdir -p "$HOME/.pilot" && touch "$HOME/.pilot/e2e-user-data-marker"
...
[ -f "$HOME/.pilot/e2e-user-data-marker" ] && pass "10d user data under ~/.pilot preserved" || fail_ "10d user data preserved"
