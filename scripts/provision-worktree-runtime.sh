...
if [[ -L "$base_cwd/.pilot" && ! -d "$base_cwd/.pilot" ]]; then
  echo "Registered base project workspace .pilot is a broken symlink: $base_cwd/.pilot" >&2
  exit 1
fi
source_config_args=()
if [[ ! -e "$base_cwd/.pilot/config.json" && ! -L "$base_cwd/.pilot/config.json" ]]; then
...
run_ensure_seeded() {
  ...
  if command -v pnpm >/dev/null 2>&1 && pnpm pilotai --help >/dev/null 2>&1; then
    (
      cd "$worktree_cwd" &&
        pnpm pilotai worktree ensure-seeded --config "$worktree_config_path" ${source_config_args[@]+"${source_config_args[@]}"}
    )
    return
  fi
