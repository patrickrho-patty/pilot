...
Usage:
  ...
  curl -fsSL https://pilot.test/install.sh | bash -s -- --no-prompt [options]
...
if [ -n "$REF" ] || [ -n "$REPO" ]; then
  fail "git-ref installs are not supported by install.sh; run 'npx pilotai install --ref <ref>' instead"
fi
...
ensure_temp_dir() {
  if [ -z "$TEMP_DIR" ]; then
    TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/pilot-install.XXXXXX")"
  fi
}
