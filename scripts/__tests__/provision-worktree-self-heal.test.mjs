...
if (cliArgs[0] === "worktree" && cliArgs[1] === "init") {
  ...
  fs.writeFileSync(".pilot/config.json", JSON.stringify({ $meta: { source: "fake-cli" } }));
  fs.writeFileSync(".pilot/.env", "PILOT_IN_WORKTREE=true\\n");
  process.exit(0);
}
if (cliArgs[0] === "worktree" && cliArgs[1] === "ensure-seeded") {
  ...
  fs.rmSync(".pilot/seed-pending", { force: true });
  fs.rmSync(".pilot/seed-complete", { force: true });
  fs.writeFileSync(".pilot/seed-manifest.json", JSON.stringify({
    version: 2,
    source: { instanceId: "base-source", configPath: ${JSON.stringify(path.join(baseCwd, ".pilot", "config.json"))} },
...
function readWorktreeConfig(worktreeCwd) {
  const configPath = path.join(worktreeCwd, ".pilot", "config.json");
...
test("uses the base CLI when its import graph boots", () => {
  ...
  assert.equal(
    JSON.parse(fs.readFileSync(path.join(worktreeCwd, ".pilot", "seed-manifest.json"), "utf8")).state,
    "pending",
  );
...
test("rejects a dangling base workspace .pilot symlink instead of falling back", () => {
  const baseCwd = makeBaseWorkspace({ helpExit: 0, initExit: 0 });
  // `-e`/`-L` on the config resolve `.pilot` first, so the config reads as absent
  ...
  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /\.pilot is a broken symlink/);
});
...
test("falls back to an isolated config when the base CLI cannot boot", () => {
  ...
  const env = fs.readFileSync(path.join(worktreeCwd, ".pilot", ".env"), "utf8");
...
test("reconciles deployment mode from the registered source when reusing a guest config", () => {
  ...
  fs.mkdirSync(path.join(baseCwd, ".pilot"), { recursive: true });
  fs.writeFileSync(
    path.join(baseCwd, ".pilot", "config.json"),
    `${JSON.stringify({
      server: {
...
test("a failed CLI init fails provisioning instead of being masked as success", () => {
  ...
  assert.ok(!fs.existsSync(path.join(worktreeCwd, ".pilot", "config.json")));
});
...
test("runtime provisioning invokes ensure-seeded once and fast-exits after success", () => {
  ...
  fs.mkdirSync(path.join(worktreeCwd, ".pilot"), { recursive: true });
  fs.writeFileSync(path.join(worktreeCwd, ".pilot", "config.json"), "{}\n");
  fs.writeFileSync(path.join(worktreeCwd, ".pilot", "seed-pending"), "{}\n");
  ...
  assert.ok(!fs.existsSync(path.join(worktreeCwd, ".pilot", "seed-pending")));
...
test("runtime provisioning omits the source override when the base config exists", () => {
  const baseCwd = makeBaseWorkspace({ helpExit: 0, initExit: 0 });
  fs.mkdirSync(path.join(baseCwd, ".pilot"), { recursive: true });
  fs.writeFileSync(path.join(baseCwd, ".pilot", "config.json"), "{}\n");
...
test("runtime provisioning bootstraps and seeds an empty .pilot directory", () => {
  const baseCwd = makeBaseWorkspace({ helpExit: 0, initExit: 0 });
...
test("runtime provisioning leaves seed-pending in place when ensure-seeded fails", () => {
  ...
  assert.ok(fs.existsSync(path.join(worktreeCwd, ".pilot", "seed-pending")));
  assert.ok(!fs.existsSync(path.join(worktreeCwd, ".pilot", "seed-complete")));
});
...
test("runtime provisioning does not trust a truncated verified manifest", () => {
  ...
  fs.writeFileSync(path.join(worktreeCwd, ".pilot", "config.json"), "{}\n");
  fs.writeFileSync(
    path.join(worktreeCwd, ".pilot", "seed-manifest.json"),
