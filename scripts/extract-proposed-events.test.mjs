...
export function trackSkillStudioCreated(
  ...
  client.track(
    // @ts-expect-error -- proposed-telemetry(PIL-2411): measure Skill Studio create completion
...
export function trackWorkspaceOpened(
  ...
  telemetry.track(
    // @ts-expect-error -- proposed-telemetry(PIL-2463): exercise alternate telemetry client parameter names
    "workspace.opened",
    dims,
  );
}
...
export const trackWorkspaceArrow = (
  ...
  telemetry.track(
    // @ts-expect-error -- proposed-telemetry(PIL-2463): exercise arrow wrapper extraction
...
export const trackWorkspaceFunctionExpression = function (
  ...
  tc.track(
    // @ts-expect-error -- proposed-telemetry(PIL-2463): exercise function-expression wrapper extraction
...
test("extractor rejects invalid rationale issue references when present", () => {
  assert.throws(
    ...
    /rationale issue must be PIL-<digits>/,
  );
});
...
client.track(
  // @ts-expect-error -- proposed-telemetry(PIL-2411): TS2578 expiry fixture
  "skill_studio.skill_created",
  { sharing_scope: "team" },
);
`;
