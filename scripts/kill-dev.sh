...
while IFS= read -r line; do
  ...
  node_pids+=("$pid")
  node_lines+=("$line")
done < <(ps aux | grep -E '/pilot(-[^/]+)?/' | grep node | grep -v grep || true)
...
candidate_pidfiles=()
candidate_pidfiles+=(
  "$HOME"/.pilot/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.pilot/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.pilot/runtime-services/instances/*/db/postmaster.pid
)

for sibling_root in "$REPO_PARENT"/pilot*; do
  [[ -d "$sibling_root" ]] || continue
  candidate_pidfiles+=(
    "$sibling_root"/.pilot/instances/*/db/postmaster.pid
    "$sibling_root"/.pilot/runtime-services/instances/*/db/postmaster.pid
  )
done
