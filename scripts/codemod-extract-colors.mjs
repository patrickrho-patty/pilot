...
const SITES = [
  ...
  },
  {
    ...
    replaceAll: [
      ['// Liveness blue (#2563EB) for the human\'s own messages (PIL-95 rev 5).', '// Liveness blue (--status-task-in_progress) for the human\'s own messages (PIL-95 rev 5).'],
...
const ALLOWLIST_COMMENTS = [
  ...
  },
  {
    ...
    anchor: 'isHighlighted && "bg-[var(--pilot-code-highlight-bg,rgba(250,204,21,0.12))]",',
    commentLine: "                // token-extraction: allowlisted — half-migrated var(--x, fallback) pattern; --pilot-code-highlight-bg/-border don't exist in index.css yet. Needs human decision (see TOKEN-AUDIT.md 2) before minting, since defining the var changes a fallback-CSS-var-expression, not a plain literal swap.",
  },
  {
    file: "pages/InviteUxLab.tsx",
