#!/usr/bin/env node
/**
 * Screenshot utility for Pilot UI.
 *
 * Reads the board token from ~/.pilot/auth.json and injects it as a
...
const rawUrl = args[0];
const outPath = args[1] || "/tmp/pilot-screenshot.png";
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".pilot/auth.json");
  try {
...
if (!cred) {
  console.error("No board token found in ~/.pilot/auth.json");
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    // Scope the auth header to the Pilot origin only
