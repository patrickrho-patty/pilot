...
test("linkSdkInto creates the plugin-sdk symlink and is idempotent", () => {
  ...
  const link = join(pkg, "node_modules", "@pilotai", "plugin-sdk");
...
test("linkSdkInto leaves a real (non-symlink) install in place", () => {
  ...
  const scopeDir = join(pkg, "node_modules", "@pilotai");
...
test("linkSdkInto replaces a stale symlink that resolves to a directory", () => {
  const pkg = makePackage(join(workDir, "stale-directory-link"));
  const staleTarget = makePackage(join(workDir, "old-plugin-sdk"));
  const scopeDir = join(pkg, "node_modules", "@pilotai");
  mkdirSync(scopeDir, { recursive: true });
  symlinkSync(staleTarget, join(scopeDir, "plugin-sdk"), "dir");

  assert.equal(linkSdkInto(pkg), true);
  assert.notEqual(readlinkSync(join(scopeDir, "plugin-sdk")), staleTarget);
});
