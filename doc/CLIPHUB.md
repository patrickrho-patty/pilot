...
ClipHub is the public registry where people share, discover, and download Pilot company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Pilot what a package registry is to a programming language. Pilot already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Pilot — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Pilot instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Pilot spec. It contains:
...
**Install (fresh start):**
```
pilot install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Pilot instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Pilot, export your company as a template, then publish:

```
pilot export --template my-company
pilot publish cliphub my-company
```
...
ClipHub is a **separate service** from Pilot itself. Pilot is self-hosted; ClipHub is a hosted registry that Pilot instances talk to.
...
| **Pilot CLI** | `pilotai install`, `pilotai publish`, `pilotai cliphub sync` — built into Pilot |
| **Pilot UI** | "Browse ClipHub" panel in the Pilot web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Pilot) |
| Backend | TypeScript + Hono (consistent with Pilot) |
| Database | PostgreSQL |
...
4. Run `pilotai install cliphub:acme/lean-saas-shop`
5. Pilot creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Pilot until it works well
2. Export: `pilotai export --template my-agency`
3. Publish: `pilotai publish cliphub my-agency`
...
3. Install just that agent: `pilotai install cliphub:acme/senior-python-eng --agent`
...
ClipHub is **not required** to use Pilot. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Pilot what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `pilotai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Pilot web UI
- [ ] `pilotai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Pilot's own philosophy)
