...
**Ticket:** PIL-13350 (plan §D4 item 1). **Companion script:** [`tests/e2e/smoke-lab-browser-runner.mts`](../../tests/e2e/smoke-lab-browser-runner.mts).
...
**The shared dev worktree service works as-is now.** The Pilot-managed dev
service (`pnpm dev`, `PILOT_DEPLOYMENT_MODE=authenticated` + `NODE_ENV=production`,
e.g. `http://pilot-dev:45439`) is private, so it can run the Smoke Lab directly —
just turn the flag on. (Historically it was blocked because the gate required
`local_trusted` + non-`production`; that restriction was removed in PIL-13351.) The
...
export NODE_ENV=test PORT=3211 \
  PILOT_HOME=/tmp/pil-smoke-home \
  PILOT_INSTANCE_ID=pil-smoke \
  PILOT_CONFIG=/tmp/pil-smoke-home/instances/pil-smoke/config.json \
  PILOT_BIND=loopback \
  PILOT_DEPLOYMENT_MODE=local_trusted \
  PILOT_DEPLOYMENT_EXPOSURE=private
pnpm pilotai onboard --yes --run    # serves http://127.0.0.1:3211, own embedded PG
...
#   SMOKE_SHOT_DIR=/tmp/pil13350-shots
```

The runner launches real Chromium via the ARM64 wrapper
(`.pilot/browser-runtime/chromium-arm64/bin/chromium-agent-browser`) — see
