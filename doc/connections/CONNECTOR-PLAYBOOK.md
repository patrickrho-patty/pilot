...
This playbook is the repeatable template for adding a vendor to the Apps catalog as data, not as a plugin. It follows the accepted connections framework in [PIL-13211](/PIL/issues/PIL-13211), the first-30 rollout matrix in [PIL-2432](/PIL/issues/PIL-2432), and the production validation scope in [PIL-12373](/PIL/issues/PIL-12373).
...
- A smoke checklist aligned to [PIL-12373](/PIL/issues/PIL-12373): connect, discover catalog, allowed read call, ask-first write call, denied/quarantined call, revoke, and audit evidence.
...
Classify the vendor before writing metadata. Use the [PIL-2432](/PIL/issues/PIL-2432) matrix terms so rollout planning, security review, and QA can compare providers consistently.
...
[PIL-12373](/PIL/issues/PIL-12373) owns real-vendor gallery smoke evidence and connector validation. Do not duplicate that issue's screenshot/evidence matrix in this playbook. A connector proposal should instead state exactly how it will be validated there:
...
instance-local (ratified in the PIL-14828 connector-service spec, section 10
...
the redirect URI. `id.pilot.test` authenticates operators only and never
holds resource tokens; `connect.pilot.test` is a fallback only for
...
This dry run applies the template to Linear, one of the [PIL-2432](/PIL/issues/PIL-2432) Batch A providers.
...
Linear's real-vendor evidence belongs in [PIL-12373](/PIL/issues/PIL-12373). The smoke pass should prove:

- OAuth connect succeeds with Pilot-owned Linear app registration once [PIL-12372](/PIL/issues/PIL-12372) provides credentials.
...
ship with RFC 7591 dynamic client registration (PIL-16637; server
implementation PIL-16649, PR #11009). Unlike the Linear appendix, every
...
(PIL-14828 spec section 10 item 8.4); **cloud-hosted and self-hosted use the
...
sequenceDiagram
    ...
    participant UI as Pilot UI<br/>/PIL/apps/connect?source=notion
...
### Dry-Run Request Log (PIL-16649, 2026-08-06/07)
...
| --- | --- |
| `http://pilot-dev:3100/api/tools/oauth/callback` | 400 `invalid_redirect_uri` — "Redirect URI must use HTTPS unless it is a loopback HTTP URI" |
| `https://pilot-dev:3100/api/tools/oauth/callback` | Accepted — private host is fine over HTTPS |
...
- Instance prerequisites: the instance base URL must be HTTPS on any host or
  ...
  pilot-dev did). The `enableApps` experimental setting must be on for
  `/apps/*` routes. The connecting user must be allowed to install
  integrations in their Notion workspace.
- How to verify: visit `/PIL/apps/connect?source=notion`, complete the Notion
...
classes below; the full catalog review with per-tool defaults is PIL-16652
...
1. Operator opens `/PIL/apps/connect?source=notion` (or the Notion gallery
...
End-to-end evidence belongs to PIL-16654 (P6) and the PIL-12373 matrix:

- Zero-setup OAuth connect succeeds on
  `https://pilot-dev.tail29c1aa.ts.net/PIL/apps/connect?source=notion`
