...
### Manual first publish for `@pilotai/ui`
...
npm view @pilotai/ui version

# make sure the dist payload is fresh
pnpm --filter @pilotai/ui build
...
- If `npm view @pilotai/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@pilotai/ui`, the `pilotai` npm organization must exist and the publisher must be a member with permission to publish to that scope.
...
- every published internal `@pilotai/*` dependency referenced by that manifest exists on npm
...
pnpm run release:bootstrap-package -- @pilotai/new-package
...
pnpm run release:bootstrap-package -- @pilotai/new-package --publish
```

The helper script:

- refuses names outside the `@pilotai/` scope
...
1. open `https://www.npmjs.com/package/@pilotai/new-package`
2. go to `Settings` → `Trusted publishing`
3. add repository `pilotai/pilot`
