...
Full Pilot CLI/API parity smoke pass against a disposable local source-tree instance.

## Isolation Contract

- Repo: `/Users/aronprins/Documents/PilotAI/pilot`
- Scratch root: `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity`
- `PILOT_HOME`: `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/home`
- `PILOT_INSTANCE_ID`: `cli-api-parity`
- `PILOT_CONFIG`: `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/home/instances/cli-api-parity/config.json`
- `PILOT_CONTEXT`: `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/home/context.json`
- `PILOT_AUTH_STORE`: `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/home/auth.json`
- `PILOT_API_URL`: `http://127.0.0.1:3197`
- `PILOT_SERVER_PORT`: `3197`
- `PORT`: `3197`
- `CODEX_HOME`: `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/codex-home`
- `CLAUDE_HOME`: `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/claude-home`
- `DATABASE_URL`: unset
...
- Command: `sed -n ... pilot-localdev-runbook.md`, `doc/DEVELOPING.md`, `doc/CLI.md`, `doc/DATABASE.md`, `doc/GOAL.md`, `doc/PRODUCT.md`, `doc/SPEC-implementation.md`, `doc/DEPLOYMENT-MODES.md`, `doc/plans/2026-05-23-cli-api-parity-openapi-reference.ts`
...
- Actual result: Runbook requires explicit scratch paths, port `3197`, unset database env vars, `pnpm pilotai onboard --yes --run --bind loopback`, and pre-test isolation checks.
- Status: PASS.
...
- Prerequisites/IDs used: none.
- Expected result: All Pilot/Codex/Claude paths point under `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity`; `DATABASE_URL` and `DATABASE_MIGRATION_URL` are unset; port `3197` has no listener.
...
- Status: PASS.
- Output summary: No references to `~/.pilot`, `~/.codex`, `~/.claude`, or `localhost:3100`.
- Follow-up: Start Pilot with the runbook command.

### 2026-05-24T11:06:45+02:00 - Start isolated instance

- Command: `env -u DATABASE_URL -u DATABASE_MIGRATION_URL ... pnpm pilotai onboard --yes --run --bind loopback`
- Purpose: Create and start the disposable source-tree Pilot instance.
...
- Actual result: Server started in `local_trusted/private` mode on `127.0.0.1:3197`, with embedded PostgreSQL data at `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/home/instances/cli-api-parity/db`.
- Status: PASS.
...
- Command: `pnpm pilotai env`; `pnpm pilotai context show --json`; `curl -sS http://127.0.0.1:3197/api/health`; `find tmp/cli-api-parity/home -maxdepth 4 -type d`
...
- Actual result: Config path, context path, storage path, secrets key path, and DB directory all resolve under `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity`; health returned `status: ok`, version `0.3.1`, `deploymentMode: local_trusted`, `companyDeletionEnabled: true`.
- Status: PASS.
- Output summary: `pilotai env` redacted here because it prints the generated `PILOT_AGENT_JWT_SECRET`. Context existed at the scratch path with an empty `default` profile.
- Follow-up: Set API context and begin CLI parity checks.

### 2026-05-24T11:08:20+02:00 - Basic context/auth/connectivity

- Command: `pnpm pilotai context set --api-base http://127.0.0.1:3197 --use --json`; `pnpm pilotai whoami --json`; `pnpm pilotai company list --json`; `pnpm pilotai access whoami --json`
...
- Command: `pnpm pilotai company create --payload-json '{"name":"CLI API Parity Test","description":"Disposable company for CLI API parity testing","goal":"Exercise the CLI API surface end to end"}' --json`; `pnpm pilotai context set --company-id 12e9db4b-f66c-459b-959e-d645002240fb --use --json`; `pnpm pilotai company get 12e9db4b-f66c-459b-959e-d645002240fb --json`; `pnpm pilotai company update 12e9db4b-f66c-459b-959e-d645002240fb --payload-json '{"description":"Updated by CLI API parity test","budgetMonthlyCents":12345}' --json`
...
- Command: edited `cli/src/commands/client/context.ts`, `cli/src/client/context.ts`, and `cli/src/__tests__/context.test.ts`; `pnpm exec vitest run cli/src/__tests__/context.test.ts`; `pnpm --dir cli typecheck`; `pnpm pilotai context set --api-base http://127.0.0.1:3197 --company-id 12e9db4b-f66c-459b-959e-d645002240fb --use --json`; `pnpm pilotai context show --json`
...
- Output summary: The assigned `process` adapter agent ran automatically and generated heartbeat runs. The issue later moved to `blocked` via recovery handling because the smoke process printed output without a concrete Pilot disposition.
...
- Command: `token board create --company-id ... --name cli-parity-board --never-expires --json`; `token board list --json`; `whoami --api-key <board-token> --json`; `token agent create --company-id ... --agent ... --name cli-parity-agent --json`; `token agent list --company-id ... --agent ... --json`; `context set --profile cli-agent --persona agent ... --api-key-env-var-name PILOT_API_KEY --json`; `agent me --profile cli-agent --json`; `agent inbox --profile cli-agent --json`; `issue list --profile cli-agent --company-id ... --json`; `company list --profile cli-agent --json`; `token agent revoke ...`; `token board revoke ...`
...
- Command: `kill <pilot pid on 3197>`; `pnpm pilotai onboard --yes --run --bind loopback`; `curl http://127.0.0.1:3197/api/health`; `issue tree-hold:get <issue-id> null --json`
...
- Status: PASS.
- Output summary: No real `~/.pilot`, `~/.codex`, or `~/.claude` paths were used. The server session is currently running under the isolated environment.
...
- Command: `curl -sf http://127.0.0.1:3197/api/health`; `pnpm pilotai health --json`; `pnpm pilotai adapter list/get/config-schema/ui-parser/models/model-profiles/detect-model/test-environment ... --json`; `pnpm pilotai environment list/capabilities/probe-config/create ... --json`; `pnpm pilotai project-workspace create/list/update/delete ... --json`; `pnpm pilotai workspace list --company-id ... --json`; `pnpm pilotai plugin list/examples/ui-contributions/tools/init ... --json`
...
- Command: `pnpm pilotai asset image:upload --company-id <company-id> --file doc/assets/avatars/zinc.png --namespace cli-parity --alt ... --title ... --json`; `pnpm pilotai asset content <asset-id> --out tmp/cli-api-parity/artifacts/asset-download.png --json`; `pnpm pilotai asset logo:upload --company-id <company-id> --file ui/public/favicon-32x32.png --json`; `pnpm pilotai skill list/create/get/file/file:update/update-status/install-update/delete/scan-projects ... --json`
...
- Command: `env -u DATABASE_URL -u DATABASE_MIGRATION_URL ... pnpm pilotai environment create --company-id 12e9db4b-f66c-459b-959e-d645002240fb --payload-json '{"name":"CLI parity local env","description":"Disposable CLI parity environment","driver":"local","config":{"cwd":"/Users/aronprins/Documents/PilotAI/pilot"}}' --json`
- Purpose: Verify `BUG-004` against the restarted isolated source-tree server.
- Prerequisites/IDs used: Same scratch env; server restarted with `pnpm pilotai onboard --yes --run --bind loopback`; company `12e9db4b-f66c-459b-959e-d645002240fb`.
...
- Command: `pnpm pilotai environment create/get/leases/probe/update/delete ... --json`; `pnpm pilotai plugin install/list/inspect/health/config/jobs/local-folders/ui-contributions/disable/enable/uninstall ... --json`; `pnpm pilotai secrets list/create/link/declarations/migrate-inline-env ... --json`
- Purpose: Add positive non-local environment coverage, plugin lifecycle coverage, and deeper secrets coverage.
- Prerequisites/IDs used: Company `12e9db4b-f66c-459b-959e-d645002240fb`; bundled plugin path `/Users/aronprins/Documents/PilotAI/pilot/packages/plugins/plugin-workspace-diff`; temporary SSH environment `cc5ae311-13f5-42b8-8044-11065b4e1af0`; temporary plugin install `e8421ed5-c103-4950-afb7-1463a0fbb9c5`; temporary secret `20c74546-7bec-4766-80cd-0b6c57545f7d`.
...
- Command: `pnpm pilotai secrets update 20c74546-7bec-4766-80cd-0b6c57545f7d --payload-json ... --json`; `pnpm pilotai secrets rotate 20c74546-7bec-4766-80cd-0b6c57545f7d --value ... --json`; `pnpm pilotai secrets usage 20c74546-7bec-4766-80cd-0b6c57545f7d --json`; `pnpm pilotai secrets access-events 20c74546-7bec-4766-80cd-0b6c57545f7d --json`; `pnpm pilotai secrets delete 20c74546-7bec-4766-80cd-0b6c57545f7d --yes --confirm 20c74546-7bec-4766-80cd-0b6c57545f7d --json`; `pnpm pilotai secrets list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --json`
...
- Expected result: `pilotai health` exists; `pilotai access whoami` works; `invite test-resolution` has a URL option; `join list --status pending` maps to `pending_approval`; issue help text no longer overstates valid cancel/recovery inputs.
...
- Command: `pnpm pilotai health --json`; `pnpm pilotai access whoami --json`; `pnpm pilotai join list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --status pending --request-type agent --json`; `pnpm pilotai invite create --company-id 12e9db4b-f66c-459b-959e-d645002240fb --payload-json '{}' --json`; `pnpm pilotai invite test-resolution <token> --url https://example.com/invite/<token> --json`; `pnpm pilotai invite revoke <invite-id> --json`; `pnpm pilotai issue recovery:resolve --help`; `pnpm pilotai issue interaction:cancel --help`
...
- Prerequisites/IDs used: Isolated `CLAUDE_HOME=/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/claude-home`.
...
- Status: PASS.
- Output summary: Mounted `llmRoutes` under `/api` in addition to the existing root mount. Updated available-skill discovery to read `CLAUDE_HOME/skills` when configured, include built-in Pilot repo skills in `available-skill list`, and allow `available-skill get` for safe listed/built-in skill names.
...
- Command: `pnpm pilotai llm agent-configuration --json`; `pnpm pilotai llm agent-icons --json`; `pnpm pilotai llm agent-configuration:adapter process --json`; `pnpm pilotai available-skill list --json`; `pnpm pilotai available-skill get pilot --json`; `pnpm pilotai available-skill get cmux --json`; `pnpm pilotai openapi --json`
...
- Expected result: LLM docs commands pass; built-in Pilot skills are listed and fetchable; real-user `~/.claude` skills are not listed; `openapi` still documents the unresolved gap if no route exists.
- Actual result: LLM docs commands passed. `available-skill list` returned Pilot repo skills such as `diagnose-why-work-stopped`; `available-skill get pilot` returned markdown. `available-skill get cmux` now returns 404 because `cmux` is no longer listed from the real Claude home. `openapi` still returns `404: API route not found`.
- Status: MIXED.
...
- Command: environment echo; `pnpm pilotai health --json`; `pnpm pilotai token board list --json`; `pnpm pilotai token board revoke <redacted-board-token-id> --json`; `pnpm pilotai token agent list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --agent 1dd601a1-031a-4225-b005-419427fd059f --json`; `pnpm pilotai plugin list --json`; `pnpm pilotai secrets list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --json`; `pnpm pilotai environment list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --json`; `pnpm pilotai project-workspace list d32032ce-d95e-4c4e-a942-dd98498025fb --json`; `pnpm pilotai openapi --json`
...
- Command: `pnpm pilotai doctor --config <scratch-config>`; `pnpm pilotai doctor --config <scratch-config> --repair --yes`; `pnpm pilotai env --config <scratch-config>`; `pnpm pilotai db:backup --config <scratch-config> --dir tmp/cli-api-parity/artifacts/root-setup/backups --retention-days 1 --filename-prefix cli-parity --json`; `pnpm pilotai allowed-hostname cli-parity.test --config <scratch-config>`; `pnpm pilotai auth bootstrap-ceo --config <scratch-config> --force --base-url http://127.0.0.1:3197`; `pnpm pilotai auth whoami --json`; `pnpm pilotai routines disable-all --config <scratch-config> --company-id 12e9db4b-f66c-459b-959e-d645002240fb --json`; `pnpm pilotai env-lab doctor --instance cli-api-parity --json`; `pnpm pilotai env-lab status --instance cli-api-parity --json`
...
- Prerequisites/IDs used: Scratch config `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/home/instances/cli-api-parity/config.json`; company `12e9db4b-f66c-459b-959e-d645002240fb`.
...
- Command: `PILOT_WORKTREES_DIR=tmp/cli-api-parity/worktrees-home pnpm pilotai worktree:list --json`; `pnpm pilotai worktree env --config <scratch-config> --json`; `pnpm pilotai worktree:merge-history --from current --to current --company CLI --dry`; `pnpm pilotai cloud push --company 12e9db4b-f66c-459b-959e-d645002240fb --dry-run --json`
...
- Prerequisites/IDs used: Scratch config `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/home/instances/cli-api-parity/config.json`; scratch worktree root `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/worktrees-home`; company `12e9db4b-f66c-459b-959e-d645002240fb`.
...
- Actual result: `worktree:list` passed and showed the current repo branch `improvement/cli-api-parity` with no Pilot worktree config. `worktree env --config <scratch-config> --json` passed and printed the scratch `PILOT_CONFIG` plus generated env values; the generated JWT secret is intentionally not copied into this log. `worktree:merge-history --from current --to current --company CLI --dry` failed as expected with `Source and target Pilot configs are the same. Choose different --from/--to worktrees.` `cloud push --dry-run` failed as expected with `Cloud sync is disabled. Enable the cloud sync experimental setting before running pilotai cloud push.`
...
- Command: `HOME=tmp/cli-api-parity/shell-home PILOT_WORKTREES_DIR=tmp/cli-api-parity/worktree-instances pnpm pilotai worktree:make cli-parity-wt --home <scratch-worktree-home> --from-config <scratch-config> --server-port 3198 --db-port 54331 --seed-mode minimal`; `pkill` only for the runaway scratch install attempt; edited `cli/src/commands/worktree.ts` and `cli/src/__tests__/worktree.test.ts`; `pnpm exec vitest run cli/src/__tests__/worktree.test.ts`; `pnpm --dir cli typecheck`; `pilotai worktree:cleanup cli-parity-wt --home <scratch-worktree-home> --force`; reran `pilotai worktree:make ...`; `pilotai worktree:list --json`; `pilotai worktree env --config <scratch-worktree-config> --json`; `pilotai worktree:merge-history --from pilot-cli-parity-wt --to current --company CLI --dry`
...
- Prerequisites/IDs used: Scratch `HOME` `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/shell-home`; scratch worktree instance home `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/worktree-instances`; source config `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/home/instances/cli-api-parity/config.json`; worktree branch/path `pilot-cli-parity-wt`.
- Expected result: `worktree:make` creates `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/shell-home/pilot-cli-parity-wt`, installs dependencies once, writes repo-local `.pilot/config.json` and `.pilot/.env`, seeds a minimal isolated DB on ports `3198`/`54331`, and leaves normal `worktree:list`, `worktree env`, and `worktree:merge-history --dry` usable.
- Actual result: The first live attempt exposed BUG-007: dependency installation recursively invoked the user pnpm shim when `HOME` was overridden. After the fix, focused worktree tests and CLI typecheck passed. `worktree:cleanup --force` removed the partial scratch branch/worktree. The rerun completed successfully: dependencies installed, minimal DB seed succeeded, repo config/env were written under the scratch worktree, instance data was written under scratch worktree home, `worktree:list` showed the new worktree with `hasPilotConfig: true`, `worktree env --json` printed the scratch worktree env, and `worktree:merge-history --dry` previewed zero inserts with existing company history already present. The generated worktree JWT secret is intentionally not copied here.
- Status: PASS after BUG-007 fix.
- Output summary: One disposable worktree remains for manual continuation at `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/shell-home/pilot-cli-parity-wt`; its isolated config is `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/shell-home/pilot-cli-parity-wt/.pilot/config.json`.
...
- Purpose: Exercise cloud connect/push without a real external Pilot Cloud stack, finish run subcommands that were not covered by earlier run list/get/events/log checks, and verify `connect` behavior in the non-interactive test runner.
...
- Expected result: Cloud sync can be enabled only in the scratch instance; `cloud connect` stores a fake upstream connection under scratch `PILOT_HOME`; `cloud push --dry-run` exports local company data and posts a preview bundle to the fake upstream; run read/control commands return structured results or controlled 404 for a nonexistent workspace operation; `connect` refuses non-interactive execution with guidance.
...
- Expected result: Pilot remains running on `127.0.0.1:3197`; fake cloud port `3199` is stopped; no active board or agent tokens from the test remain; plugin list is empty; disposable routine is archived; `openapi` remains the one documented unresolved API route gap.
...
- Expected result: `/api/openapi.json` and `pilotai openapi --json` return a valid OpenAPI 3.0 document with the reference route inventory, including representative CLI/API parity paths such as `/api/companies/{companyId}/agents` and `/api/agents/{id}/keys`.
- Actual result: Focused test and `server` typecheck passed. After restart, direct curl returned `{"openapi":"3.0.0","pathCount":247}`. The CLI command returned `openapi: "3.0.0"`, title `Pilot API`, `247` paths, `/api/openapi.json` summary `Get the generated OpenAPI document`, and `/api/agents/{id}/keys` POST summary `Create an agent API key`.
...
- Command: Edited `cli/src/commands/client/agent.ts`; ran `pnpm exec vitest run cli/src/__tests__/agent-lifecycle.test.ts`; `pnpm --dir cli typecheck`; `pnpm --silent pilotai agent instructions-path:update --help`.
...
- Actual result: Install/list/inspect/health/logs/config/jobs/job:runs/job:trigger/webhook/dashboard/bridge:data/data/action/upgrade/disable/enable/uninstall passed. Config commands initially failed until payloads were corrected to `{"configJson":{...}}`; bridge action initially failed until payload used `key` instead of `action`. `bridge:stream` returned expected `Plugin stream bridge is not enabled`. Local-folder calls returned expected validation because the kitchen-sink manifest declares no local folders. `plugin tools` listed `pilot-kitchen-sink-example:echo`, but `plugin tool:execute` returned `502: worker for plugin "pilot-kitchen-sink-example" is not running` even though bridge calls to the same plugin worker succeeded.
...
- Command: Edited `server/src/services/plugin-tool-dispatcher.ts`, `server/src/services/plugin-loader.ts`, and `server/src/__tests__/plugin-database.test.ts`; ran `pnpm exec vitest run server/src/__tests__/plugin-database.test.ts`; `pnpm --dir server typecheck`; restarted the isolated server; installed kitchen-sink plugin; `plugin tools`; `plugin tool:execute --payload-json '{"tool":"pilot-kitchen-sink-example:echo",...}'`; cleanup `plugin uninstall --force`.
...
- Prerequisites/IDs used: Same scratch server and kitchen-sink plugin; tool `pilot-kitchen-sink-example:echo`.
- Expected result: The listed echo tool dispatches to the running kitchen-sink worker and returns the echo result.
- Actual result: Focused test and server typecheck passed. After restart, `plugin tools` listed `pilot-kitchen-sink-example:echo`; `plugin tool:execute` returned `content: "CLI parity tool after fix"` and the expected run context. The plugin was uninstalled and `plugin list` returned `[]`.
...
- Expected result: Deleting a webhook routine trigger removes the generated pilot-managed secret and binding. No active secrets remain from parity cleanup.
...
- Actual result: Final handoff stated that the full isolated CLI/API parity pass completed and was committed, the scratch Pilot server was still running on `http://127.0.0.1:3197`, and goal usage was 2,625,855 tokens over about 2h 23m.
- Status: PASS.
- Output summary:
...
- Final paths:
  - Scratch home: `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/home`
  - Config: `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/home/instances/cli-api-parity/config.json`
  - Context: `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/home/context.json`
  - Auth store: `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/home/auth.json`
  - Embedded DB: `/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/home/instances/cli-api-parity/db`, active on pg port `54330`
...
```sh
export PILOT_HOME=/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/home
export PILOT_INSTANCE_ID=cli-api-parity
export PILOT_CONFIG=/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/home/instances/cli-api-parity/config.json
export PILOT_CONTEXT=/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/home/context.json
export PILOT_AUTH_STORE=/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/home/auth.json
export PILOT_API_URL=http://127.0.0.1:3197
export PILOT_SERVER_PORT=3197
export PORT=3197
export CODEX_HOME=/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/codex-home
export CLAUDE_HOME=/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/claude-home
unset DATABASE_URL DATABASE_MIGRATION_URL
pnpm pilotai health --json
```
...
- Prerequisites/IDs used: Isolated scratch server restarted on `127.0.0.1:3197`; `DATABASE_URL` and `DATABASE_MIGRATION_URL` unset; `PILOT_HOME`, `PILOT_CONFIG`, `PILOT_CONTEXT`, `PILOT_AUTH_STORE`, `CODEX_HOME`, and `CLAUDE_HOME` all under `tmp/cli-api-parity`.
- Expected result: `/api/openapi.json` and `pilotai openapi --json` return OpenAPI 3.0 with schema-backed request bodies, security schemes, public-operation security overrides, and create-operation `201` responses.
...
- Command: `pnpm pilotai health --json`; `pnpm pilotai openapi --json`; `lsof -nP -iTCP:3197 -sTCP:LISTEN`; `git status --short --branch`, all with the scratch `PILOT_*`, `CODEX_HOME`, and `CLAUDE_HOME` environment and with `DATABASE_URL`/`DATABASE_MIGRATION_URL` unset.
...
- Command: Stopped the foreground scratch server process; started the same runbook command in detached screen session `pilot-cli-parity`; verified `pnpm pilotai health --json`; checked `lsof -nP -iTCP:3197 -sTCP:LISTEN`; checked `screen -ls`.
- Purpose: Leave the disposable instance running without tying it to the active tool session.
- Prerequisites/IDs used: Same scratch env and unset database variables; detached screen session `91568.pilot-cli-parity`.
- Expected result: Server continues running on non-default port `3197` with the same isolated home/config/context/auth paths.
- Actual result: Health returned `status:"ok"` with deployment mode `local_trusted`; `node` PID `91583` is listening on `127.0.0.1:3197`; `screen -ls` shows detached session `91568.pilot-cli-parity`.
- Status: PASS.
- Output summary: Detached server log is `tmp/cli-api-parity/artifacts/caveat-followup/server-screen.log`.
- Follow-up: Manual continuation can use the same env block and `screen -r pilot-cli-parity` to inspect the server session.
...
- Expected result: The webhook trigger's generated pilot-managed secret and binding are removed when the trigger is deleted.
...
- Reproduction command: Install `packages/plugins/examples/plugin-kitchen-sink-example`, then run `pnpm pilotai plugin tool:execute --payload-json '{"tool":"pilot-kitchen-sink-example:echo","parameters":{"message":"CLI parity tool"},"runContext":{"companyId":"<company-id>","projectId":"<project-id>","agentId":"<agent-id>","runId":"<run-id>"}}' --json`.
- Expected result: The listed tool dispatches to the running kitchen-sink worker and returns a `ToolResult`.
- Actual result: `plugin tools` listed `pilot-kitchen-sink-example:echo`, and bridge data/action calls to the same plugin worker succeeded, but `tool:execute` returned `502: Cannot execute tool ... worker for plugin "pilot-kitchen-sink-example" is not running`.
...
- Reproduction command: `pnpm pilotai token agent list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --agent 1dd601a1-031a-4225-b005-419427fd059f --json`.
...
- Suspected cause: The CLI did not reuse the pnpm executable that launched the current Pilot command, so dependency installation was subject to PATH/shim behavior under an overridden `HOME`.
...
- Status: Fixed.
- Severity: High for isolated CLI testing; a non-default `apiBase` can be silently removed and later commands may fall back to `http://localhost:3100` if `PILOT_API_URL` is absent.
- Reproduction command: `pnpm pilotai context set --api-base http://127.0.0.1:3197 --use --json`; then `pnpm pilotai context set --company-id <company-id> --use --json`; then `pnpm pilotai context show --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/context.test.ts`; `pnpm --dir cli typecheck`; `pnpm pilotai context show --json`.
...
- Reproduction command: `pnpm pilotai access whoami --json`.
...
- Fix summary: Added `pilotai access whoami` as an alias for the existing top-level `whoami` command.
- Verification command: `pnpm exec vitest run cli/src/__tests__/access-parity.test.ts`; `pnpm --dir cli typecheck`; `pnpm pilotai access whoami --json`.
- Remaining risk: Low.
...
- Reproduction command: `pnpm pilotai issue interaction:accept <issue-id> <request-confirmation-interaction-id> --json`.
...
- Reproduction command: `pnpm pilotai issue tree-hold:get <issue-id> null --json` or `pnpm pilotai issue tree-hold:release <issue-id> null --json`.
...
- Reproduction command: `pnpm pilotai issue interaction:cancel <issue-id> <request-confirmation-interaction-id> --reason "..." --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/issue-subresources.test.ts`; `pnpm --dir cli typecheck`; `pnpm pilotai issue interaction:cancel --help`.
...
- Status: Fixed help text.
- Severity: Low command UX drift.
- Reproduction command: `pnpm pilotai issue recovery:resolve <issue-id> --action-id <action-id> --outcome restored --source-issue-status blocked --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/issue-subresources.test.ts`; `pnpm --dir cli typecheck`; `pnpm pilotai issue recovery:resolve --help`.
...
- Severity: Low command UX drift.
- Reproduction command: `pnpm pilotai agent instructions-path:update <process-agent-id> --payload-json '{"path":"docs/cli-parity.md"}' --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/agent-lifecycle.test.ts`; `pnpm --dir cli typecheck`; `pnpm pilotai agent instructions-path:update --help`.
...
- Reproduction command: `pnpm pilotai invite test-resolution <invite-token> --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/access-parity.test.ts`; `pnpm --dir cli typecheck`; `pnpm pilotai invite test-resolution <token> --url https://example.com/invite/<token> --json`.
...
- Status: Fixed and live-verified.
- Severity: Low command UX drift.
- Reproduction command: `pnpm pilotai join list --company-id <company-id> --status pending --request-type agent --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/access-parity.test.ts`; `pnpm --dir cli typecheck`; `pnpm pilotai join list --company-id <company-id> --status pending --request-type agent --json`.
- Remaining risk: Low.
...
- Reproduction command: `pnpm pilotai openapi --json`; `pnpm pilotai available-skill get cmux --json`; `pnpm pilotai llm agent-configuration --json`; `pnpm pilotai llm agent-icons --json`; `pnpm pilotai llm agent-configuration:adapter process --json`.
...
- Fix summary: Mounted LLM docs routes under `/api`; made available-skill discovery honor `CLAUDE_HOME`, include built-in Pilot repo skills, and fetch safe skill markdown consistently; added `/api/openapi.json`, then upgraded it from the initial path inventory to the schema-backed `OpenAPIRegistry`/`OpenApiGeneratorV3` implementation from the parity reference.
- Verification command: `pnpm exec vitest run server/src/__tests__/llms-routes.test.ts cli/src/__tests__/access-parity.test.ts`; `pnpm --dir server typecheck`; `pnpm --dir cli typecheck`; live `llm` and `available-skill` commands after restart; `pnpm exec vitest run server/src/__tests__/openapi-routes.test.ts`; live `curl http://127.0.0.1:3197/api/openapi.json`; live `pnpm pilotai openapi --json`; follow-up live schema-backed `pilotai openapi --json`.
...
- Reproduction command: `CLAUDE_HOME=/Users/aronprins/Documents/PilotAI/pilot/tmp/cli-api-parity/claude-home pnpm pilotai available-skill list --json`.
- Expected result: Skill discovery uses the isolated Claude home or built-in repo skills only.
- Actual result: Before the fix, the list included `cmux` from the real user Claude skills home, and `available-skill get cmux` failed because only a hardcoded Pilot subset was fetchable.
- Suspected cause: Server code read `HOME/.claude/skills` directly and did not add built-in Pilot skills unless they were present in Claude's skills directory.
- Files changed: `server/src/routes/access.ts`, `doc/bugs/2026-05-24-cli-api-parity-e2e-log.md`.
- Fix summary: Use `CLAUDE_HOME/skills` when `CLAUDE_HOME` is set, include built-in Pilot skills in catalog output, and resolve safe skill markdown from both Claude and Pilot skills directories.
- Verification command: live `available-skill list`, `available-skill get pilot`, and `available-skill get cmux` after restarting the isolated server.
- Remaining risk: Low; this is runtime environment-sensitive and covered by live isolated verification.

### MISMATCH-008 - `pilotai health` is not registered

- Status: Fixed and live-verified.
- Severity: Low command/API parity gap.
- Reproduction command: `pnpm pilotai health --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/access-parity.test.ts`; `pnpm --dir cli typecheck`; `pnpm pilotai health --json`.
- Remaining risk: Low.
...
- Reproduction command: `pnpm pilotai environment create --company-id 12e9db4b-f66c-459b-959e-d645002240fb --payload-json '{"name":"CLI parity local env","description":"Disposable CLI parity environment","driver":"local","config":{"cwd":"/Users/aronprins/Documents/PilotAI/pilot"}}' --json`.
...
- Status: Fixed and live-verified.
- Severity: Medium CLI/API parity gap.
- Reproduction command: `pnpm pilotai secrets --help` did not expose commands for `PATCH /api/secrets/:id`, `POST /api/secrets/:id/rotate`, `GET /api/secrets/:id/usage`, `GET /api/secrets/:id/access-events`, or `DELETE /api/secrets/:id`.
