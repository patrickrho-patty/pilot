...
For `claude_local` and `codex_local`, Pilot assumes the CLI is already installed and authenticated on the host machine.
...
Pilot stores resumable session state per `(agent, taskKey, adapterType)`.
