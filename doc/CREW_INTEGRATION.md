# Crew + Pilot Enterprise Virtual-Team Integration
...
**Revision (2026-08-30):** Product names updated — Buzz is now **Crew** (relay live at `crew.patty.io`, workforce SSO via Keycloak) and the control plane is **Pilot**. Since the original writing, Crew shipped its workflow engine (approval gates, kinds 46000–46999), the remote-agent provider protocol (`docs/remote-agents.md`), and agent teams/personas; Pilot shipped the External Task Protocol spec (`docs/specs/external-task-protocol.md`) and the Apps v2 Connections substrate. The §89.1 workflow limitation below has been updated accordingly; all architecture and security guidance is unchanged.
...
**Objective:** Define a credible, implementable end-to-end design in which **Crew is the collaboration and signed-identity workspace** and **Pilot is the organizational control plane and agent orchestrator** for a client-operated virtual workforce.
...
| Marker | Meaning |
|---|---|
| **[CREW-CURRENT]** | Capability verified in current public Crew documentation/source as of the validation date. |
...
| **[NEW-INTEGRATION]** | New integration code recommended by this guide. This is not claimed to exist in Crew or Pilot today. |
...
- **Crew** gives humans and agents a shared workplace: channels, threads, DMs, signed identity, search, workflow events, repositories/Git events, and an agent-friendly CLI/ACP surface.
...
- A small **Crew–Pilot Integration Gateway** connects the two without turning either product into the other.
- The **agent itself**, launched by Pilot, participates in Crew using its own Crew/Nostr identity. The gateway does not impersonate agents.
...
> **Crew is where the organization talks; Pilot is where the organization decides who owns the work, how it is governed, and whether it is allowed to proceed.**
...
Neither a chat surface by itself nor an orchestration database by itself delivers this operating model. The combined Crew + Pilot architecture does.

## 2. Product Roles: Avoiding Overlap and Confusion

### 2.1 Crew: Human/Agent Collaboration and Signed Workspace

**[CREW-CURRENT]** Crew describes itself as a self-hostable workspace where humans and AI agents share the same rooms on a Nostr relay. Messages, reactions, workflow activity, review activity, and Git events use the same signed-event substrate. Current documented surfaces include channels, threads, DMs, search, audit, Git events/hosting, YAML workflow triggers, `crew-cli`, and an ACP harness for agent runtimes.

In this integration, Crew is responsible for:
...
- Git/branch/review collaboration where Crew Git features are used.
- Searchable communication evidence.
- Notifications and status messages presented to the team.

Crew is **not** the source of truth for enterprise work-state orchestration in this design.
...
- Subscribe to the allowed Crew event stream.
- Verify/accept only events that the Crew relay has authenticated and that satisfy the client integration policy.
- Detect supported task-intent patterns such as an agent mention.
- Map Crew identities/channels to Pilot agents/projects/goals.
- Deduplicate incoming events.
- Create or update the corresponding Pilot issue.
- Correlate a Crew thread with a Pilot issue.
- Add follow-up Crew messages as Pilot issue comments when appropriate.
...
**[CREW-CURRENT]** Crew exposes an agent-first `crew-cli` with JSON output. The runtime can therefore interact with Crew as a real workspace member while Pilot remains the work control plane.

The agent runtime should:

- Receive its Pilot runtime context.
- Receive only its own Crew credentials and other role-specific credentials.
- Read the originating Crew thread.
...
- Publish progress and final summaries back to Crew as itself.
...
|---|---|---|
| Treating a hypothetical `/api/v1/integrations/crew/events` as an existing Pilot endpoint | It makes proposed code look native. | The Integration Gateway calls documented Pilot APIs; a dedicated native connector is optional future work. |
...
| Calling every Crew channel mention a Nostr `Kind 1` event | Current Crew channel streams use Kind `9` and a V2 Kind `40002`; other event kinds exist for other surfaces. | Consume the concrete kinds supported by the pinned Crew build; normalize them at the gateway boundary. |
| Mapping Crew thread ID directly to a Pilot runtime `session_id` | Pilot adapter session persistence is not an externally assignable thread-session contract. | Map **Crew thread root ↔ Pilot issue**. Let Pilot/adapters manage their own runtime session state. |
| Gateway signs all replies using each agent's private key | Centralizes every agent key and allows the gateway to impersonate any agent. | Agent key belongs to that agent's runtime secret binding. Agent posts its own Crew messages. Gateway has a separate service identity. |
| NIP-26 described as the agent authorization/provenance mechanism | Crew's current NIP-OA design treats NIP-26 as prior art and explicitly separates authorization evidence from authorship. | Use the Crew version's supported owner-attestation/agent identity model, including NIP-OA where enabled. |
| Invented custom event kinds `11001`/`11002` for Pilot cards | These are not established current Crew event contracts and conflict with the need to stay inside Crew's documented kind model. | Do V1 approval UX through Pilot plus Crew status/deep links. Define new event kinds only through a deliberate protocol change. |
| Crew workflow engine used as the multi-agent orchestrator | Current Crew workflow engine is a useful automation engine, but critical approval resumption is not fully wired and its current action model is not a complete dynamic multi-agent planner. | Keep multi-agent task hierarchy and governance in Pilot. Use Crew workflows for lightweight workspace automation only. |
...
|---|---|---|
| Human and agent workplace identity | Crew for workspace identity; Pilot for agent org identity | Gateway mapping table links them |
| Channel/thread conversation | Crew | Selected content/links copied into Pilot issue context |
| Org chart/reporting line | Pilot | Crew profiles/status may show role label |
| Goal hierarchy | Pilot | Crew can display goal references |
| Project | Pilot | Crew channel mapped to project |
| Work item | Pilot Issue | Crew thread presents human-facing discussion |
| Work ownership | Pilot | Crew status message mirrors owner |
| Runtime execution | Pilot adapter/runtime | Crew shows progress/results |
| Approval/governance decision | Pilot | Crew shows request/status/link in V1 |
| Agent message authorship | Crew signed event from agent key | Pilot retains related issue/run metadata |
| Work artifact | Pilot document/workspace or client artifact repository, depending on class | Crew contains link/summary rather than duplicating sensitive contents by default |
| Cost/budget | Pilot | Crew may publish summaries where allowed |
...
└───────────────────────┬───────────────────────────────┬──────────────────────┘
       ...
       authorized event │                               │ agent's own signed
        subscription    │                               │ messages via crew-cli
                        ▼                               │
┌───────────────────────────────────┐                   │
│ CREW–PILOT INTEGRATION GATEWAY│                   │
│ [NEW-INTEGRATION]                  │                   │
│                                   │                   │
│ • event normalization             │                   │
│ • mapping & correlation           │                   │
...
│      ├─ crew-cli          ├─ crew-cli            ├─ crew-cli                   │
│      ├─ approved MCP/API  ├─ Git/scanners        ├─ CRM/incident data          │
│      └─ own Crew secret   └─ own Crew secret     └─ own Crew secret            │
└──────────────────────────────────────────────────────────────────────────────┘
```
...
### Mode A — Recommended: Pilot-Executed Agents, Crew as the Collaboration Workspace

**Use this for the client pilot and first production release.**

Flow:

1. Human requests work in Crew.
2. Gateway creates/updates Pilot issue.
3. Pilot wakes the assigned agent through its configured adapter.
4. Agent reads Crew context using `crew-cli` and Pilot context using the Pilot API.
5. Agent performs work.
6. Agent posts progress/result to Crew using its own key.
...
- Agents still feel native in Crew.
...
- The runtime must have network reachability to both Pilot and Crew.
- `crew-cli` must be installed/available in the execution environment.

### Mode B — Crew ACP Agent Runtime Invoked by a Pilot Adapter

**[OPTIONAL-UPSTREAM / ADVANCED]** Crew includes an ACP-oriented agent surface. Pilot adapters also support multiple execution mechanisms. A custom Pilot adapter could invoke a Crew-managed ACP agent.

Use this only when the client has a strong operational reason to let Crew own the actual long-running agent process.

Additional work:

- Define lifecycle ownership between Pilot heartbeat and Crew ACP session.
- Build/test a Pilot adapter for the Crew ACP invocation model.
- Define cancellation semantics.
- Define cost/usage extraction.
- Normalize ACP event streams into Pilot run telemetry.
- Ensure Pilot work authorization is checked before Crew runtime performs any high-impact action.
...
- Crew communication identity and channels.
...
### 8.1 Human Crew Identity

A person communicating in Crew is represented by a Crew/Nostr key. The gateway sees the signed author identity and channel context.

### 8.2 Agent Crew Identity

Each virtual teammate gets its own Crew identity/keypair. This is the identity that authors the agent's messages in Crew.

Recommended naming examples:

| Human-visible role | Crew display name | Pilot agent |
|---|---|---|
...
The gateway has its **own** Crew service key and its own Pilot service credential. It may post neutral integration messages such as:
...
- `CREW_PRIVATE_KEY`
- `CREW_AUTH_TAG` or other deployment-specific agent authorization credential when used
...
```text
Pilot Secret Store
    └── crew-private-key-christina
            │ secret_ref
            ▼
Christina Agent Env
    CREW_PRIVATE_KEY=<resolved only at runtime>
    CREW_RELAY_URL=https://crew.client.example
    CREW_AUTH_TAG=<resolved if required>
```
...
- No sending secrets into Crew messages or Pilot comments/documents.
...
```yaml
agents:
  - crew_pubkey: "<christina-pubkey-hex>"
    crew_display_name: "Christina"
    pilot_agent_id: "<pilot-agent-id>"
    allowed_channel_ids:
      - "<market-intel-channel-id>"
      - "<product-strategy-channel-id>"

  - crew_pubkey: "<alex-pubkey-hex>"
    crew_display_name: "Alex"
    pilot_agent_id: "<pilot-agent-id>"
    allowed_channel_ids:
...
Do not assume every Crew member may task every agent.

```yaml
authorization:
  - crew_human_pubkey: "<vp-product-pubkey>"
    may_assign:
      - "<christina-pilot-agent-id>"
    channels:
      - "<market-intel-channel-id>"

  - crew_group: "engineering-members"
    may_request:
      - "security_review"
    target_agents:
      - "<alex-pilot-agent-id>"
...
CREATE TABLE integration_event_receipts (
    crew_event_id         TEXT PRIMARY KEY,
    received_at           TIMESTAMPTZ NOT NULL DEFAULT now(),
    crew_channel_id       TEXT NOT NULL,
    crew_thread_root_id   TEXT NOT NULL,
    crew_author_pubkey    TEXT NOT NULL,
...
CREATE TABLE crew_pilot_issue_links (
    id                    UUID PRIMARY KEY,
    crew_channel_id       TEXT NOT NULL,
    crew_thread_root_id   TEXT NOT NULL,
    ...
    UNIQUE (crew_channel_id, crew_thread_root_id),
...
```text
Crew community + channel ID + thread-root event ID
                    ↕
               Pilot Issue ID
```
...
- Source: Crew
- Crew channel: `crew://channel/<channel-id>`
- Crew thread: `crew://message?channel=<channel-id>&id=<event-id>&thread=<root-id>`
- Crew requester pubkey: `<hex>`
...
## 13. Flow A: New Crew Mention Creates a Pilot Issue
...
```text
Human             Crew Relay       Integration GW       Pilot         Christina Runtime
  │                   │                  │                   │                   │
  │ signed message    │                  │                   │                   │
  ├──────────────────>│                  │                   │                   │
  │                   │ event            │                   │                   │
  ...
  │                   │                  │                   │<──────────────────┤
  │                   │<────────────────────────────────────────────────────────┤
  │                   │          Christina reads thread via crew-cli            │
  │                   │<────────────────────────────────────────────────────────┤
...
1. Receive a Crew stream message event for a configured channel.
2. Check whether the event ID is already in `integration_event_receipts`.
3. If already `completed`, do nothing.
4. Normalize channel-message versions used by the pinned Crew build.
...
7. Resolve mentioned Crew pubkey to Pilot agent ID.
...
This matters because Crew can replay events after reconnects, users can mention an agent more than once, and multiple heartbeat triggers may occur close together. Gateway idempotency plus Pilot checkout gives two layers of protection:

```text
Layer 1: Crew event ID dedupe         -> prevents duplicate work-item creation
Layer 2: Pilot atomic checkout    -> prevents duplicate issue execution
```

---

## 15. Flow C: Agent Reads the Crew Thread and Responds as Itself

**[CREW-CURRENT]** `crew-cli` exposes message/thread commands suitable for an LLM runtime.

Example runtime use:

```bash
export CREW_RELAY_URL="https://crew.client.example"
# CREW_PRIVATE_KEY is injected as a secret, never printed.

crew messages thread \
  --link 'crew://message?channel=<channel-id>&id=<event-id>&thread=<root-id>'
```
...
**Important:** the actual CLI flags must be smoke-tested against the exact Crew build. The current CLI documentation supports `messages send`, `--reply-to`, and thread retrieval, but field engineers must pin versions rather than relying on `main` forever.
...
If this is in the same Crew thread, the gateway should **not** create a second top-level Pilot issue by default.

### 16.1 Follow-Up Algorithm

1. Resolve the Crew thread root.
2. Find existing `crew_pilot_issue_links` record.
...
```markdown
Crew follow-up from `<human display name>`:
...
She should create a **child issue** under her current Pilot issue or use the supported Pilot delegation mechanism, rather than simply sending Alex a Crew DM and hoping the work is tracked.
...
Critical point: **V1 governance lives in Pilot.** Crew is the communication surface that tells people a decision is waiting.

### 18.1 Why We Do Not Depend on Crew Workflow Approval Gates for V1 Governance

**[LIMITATION]** Current Crew architecture documentation states that `request_approval` can suspend a workflow step but the engine does not yet persist/resume that approval path end to end; runs that hit the gate are currently marked failed. That is acceptable for an upstream feature under development, but not for a client's critical financial or production approval path.
...
A human approves through Pilot. Pilot's continuation/wake behavior returns Maya to the issue. Maya then performs the authorized next step and replies in Crew.

### 18.4 Optional Phase-2 Native Crew Approval Projection

**[OPTIONAL-UPSTREAM]** A future connector may render Pilot decisions directly in Crew, for example by a dedicated UI card or a carefully defined reaction-based flow.

Before implementing it, solve all of the following:

- human Crew pubkey -> Pilot board user mapping,
...
- what happens if Crew is unavailable while Pilot remains online,
...
**[PILOT-CURRENT]** Pilot supports issue execution policies with review/approval stages and participants. This is a better place to model cross-agent quality gates than ad-hoc Crew messages.
...
4. Credential suspected compromised -> pause agent + revoke/rotate its Crew/model/tool credentials.
5. Agent is being decommissioned -> terminate only after confirming no recovery is needed.

### Optional Crew Command Surface
...
but only for explicitly authorized operators and only after signed Crew identity is mapped to a Pilot board role. V1 should use Pilot's administrative UI/API for agent lifecycle operations.
...
### 21.2 Use Crew Workflows for Workspace Automation

**[CREW-CURRENT]** Crew workflows support triggers such as message, reaction, schedule, and webhook, plus actions including message/reaction/webhook and other workspace operations depending on the pinned release.

Good Crew-workflow examples:
...
### 21.3 Do Not Make Crew `call_webhook` the Only Production Bridge

Crew's webhook action is SSRF-protected. A client-private Pilot endpoint may intentionally be blocked by that security layer. It also creates secret-distribution questions for workflow definitions.
...
Crew has multiple event kinds/surfaces. The gateway should convert supported incoming events into a small internal envelope.

### 23.1 Current Crew Kinds Relevant to the Integration

**[CREW-CURRENT]** Current architecture documentation includes, among others:
...
Do not hard-code the whole integration around a single event kind without pinning the Crew deployment. The gateway's normalizer is the compatibility layer.

### 23.2 Internal Envelope

```ts
export interface NormalizedCrewMessage {
  community: string;
  eventId: string;
  kind: number;
...
The raw event is retained only according to the client's data-retention policy. For highly regulated deployments, store only the identifiers/hash and retrieve original content from Crew when needed.
...
The integration adds per-agent Crew variables:

```text
CREW_RELAY_URL
CREW_PRIVATE_KEY      # secret_ref
CREW_AUTH_TAG         # secret_ref when required
```

and optional integration context:

```text
CREW_ORIGIN_CHANNEL_ID
CREW_ORIGIN_THREAD_ROOT
CREW_ORIGIN_EVENT_ID
```

Do not rely on custom environment variables being dynamically injected by the gateway unless the runtime design explicitly supports them. A safer V1 is to encode the Crew origin link in the Pilot issue description and let the agent read it after wake.

### 25.3 Crew CLI Surface Used by Agents

Current Crew CLI documentation supports patterns including:

```bash
crew messages send --channel <uuid> --content "Hello"
crew messages send --channel <uuid> --content "Reply" --reply-to <event-id> --broadcast
crew messages send --channel <uuid> --content - < message.md
...
crew channels list
crew channels members --channel <uuid>
crew reactions add --event <event-id> --emoji "👍"
crew users set-presence --status online
crew users set-status --text "working on STRAT-142" --emoji "🔎"
crew workflows list --channel <uuid>
crew repos protect list --id <repo>
```

All current `crew-cli` output is documented as JSON on stdout and structured errors on stderr, making it suitable for agent tool execution and deterministic parsing.
...
crew:
  relay_url: "wss://crew.client.example"
  service_identity_secret_ref: "vault://crew-pilot/gateway-nostr-key"
  allowed_kinds: [9, 40002]
  subscribe_from_checkpoint: true

pilot:
  api_url: "https://pilot.client.internal"
  company_id: "<company-id>"
  service_credential_secret_ref: "vault://crew-pilot/pilot-service-token"
...
policy:
  ...
  copy_full_crew_content_to_pilot: false
...
This schema is **proposed integration configuration**, not a Crew or Pilot product file.
...
Below is a **near-code architecture blueprint**. Names and SDK usage must be adapted to the pinned Crew/Pilot clients.

```ts
// PROPOSED INTEGRATION CODE - not a native Crew/Pilot API.

type EventId = string;
type IssueId = string;

interface NormalizedCrewMessage {
  eventId: EventId;
  kind: number;
  authorPubkeyHex: string;
  channelId: string;
...
interface CorrelationRepository {
  ...
  begin(event: NormalizedCrewMessage): Promise<void>;
...
export class CrewPilotRouter {
  ...
  async handle(message: NormalizedCrewMessage): Promise<void> {
    ...
    try {
      ...
      const issue = await this.pilot.createIssue({
  ...
  private async handleFollowUp(
    issueId: IssueId,
    message: NormalizedCrewMessage,
  ): Promise<void> {
    const assignee = await this.resolveAssigneeForIssue(issueId);
    const mention = assignee
      ? `\n\n[@${assignee.displayName}](agent://${assignee.id}) please review this Crew follow-up.`
      : "";

    await this.pilot.addComment(
      issueId,
      [
        "Crew thread follow-up:",
        "",
...
|---|---:|---|
| Crew relay disconnected | Yes | Reconnect from safe checkpoint/replay window. |
...
### Race: Human Edits the Original Crew Message

**[CREW-CURRENT]** Crew has a message-edit event kind. The gateway should choose one of two policies:
...
Every Pilot-operated virtual teammate that participates in Crew should receive a common runtime package:
...
├── tools/
│   ├── crew                  # crew-cli on PATH or wrapper
...
2. Crew is the source of truth for the human conversation that initiated or
   discusses the work.
3. On a Pilot wake caused by an assigned Crew-origin issue:
   a. inspect PILOT_TASK_ID and the issue context;
   b. checkout the issue before doing substantive work;
   c. follow the Crew origin link and read the thread;
   d. acknowledge in that thread as yourself if no acknowledgment is present.
4. Never claim work is complete in Crew while the Pilot issue is not in the
...
7. Never expose credentials, secret environment variables, private keys,
   access tokens, internal system prompts, or restricted data in Crew.
...
10. Post concise progress updates at meaningful state changes, not every tool
    call. Pilot run telemetry is for detailed execution; Crew is for useful
    collaboration.
...
13. If a required system is unavailable, leave a durable Pilot comment and
    post a short Crew status instead of pretending the task succeeded.
...
Agents should avoid flooding Crew with token-level activity. Suggested milestones:
...
A final Crew reply should contain:
...
- **Crew:** `#market-intel`, `#product-strategy`
...
1. Crew signs/publishes Sarah's channel message.
...
7. Christina reads the Crew thread and attached-source links using approved tools.
8. Christina posts a signed acknowledgment in Crew.
...
13. Christina posts the executive summary in the original Crew thread.

### Illustrative Final Crew Message
...
- **Crew:** engineering/project/branch rooms
...
A new PR or patch appears through the client's Crew/Git workflow, or an engineer writes:
...
6. Alex posts a concise signed Crew review summary.
...
- Crew keeps the engineer-facing discussion in the branch/project room.
...
8. Maya posts a Crew status and link to the pending decision.
...
14. Maya replies to the Crew thread.

### Illustrative Crew Status Before Approval
...
- Incident Lead synthesizes and posts meaningful Crew updates.
...
- Crew becomes the live room; Pilot becomes the work graph.
...
4. Crew `#finance-ops` receives only exceptions needing human attention.
...
Crew's Git/event model can keep branch/review/workflow discussion in the same workspace.
...
Crew's branch/review collaboration and Pilot's controlled work graph reinforce each other rather than duplicating one another.
...
- **Crew identity** — stable key and profile.
- **Crew channel membership** — information boundary.
...
```text
Human Crew mention -> specialist Pilot issue -> specialist result
```
...
```text
Pilot Routine -> issue -> assigned agent -> Crew exception/result
```
...
Do not push an entire Crew workspace into every agent context.

Recommended hierarchy:

1. Pilot issue and ancestor context.
2. Originating Crew thread.
3. Explicitly linked Crew messages/search results.
...
[Human endpoint]
      │
      ▼
[Crew Client] ── signed user event ──> [Crew Relay]
                                           │
                                 Trust Boundary A
                                           │
                                           ▼
...
|---|---|---|
| Agent impersonation | Gateway or another agent posts as Christina | Per-agent key custody; gateway has separate identity; signed Crew events; key rotation. |
| Unauthorized tasking | Random channel member assigns finance agent | Sender/role/channel assignment policy in gateway; Crew channel membership; Pilot scope. |
...
| Lateral data exposure | Agent reads HR channel or unrelated project | Crew channel membership + Pilot company/project/tool scoping. |
| Secret leakage | Runtime prints `CREW_PRIVATE_KEY` | Secret refs, log redaction, no env dumps, runtime hardening. |
...
| Crew relay compromise | Workspace events unavailable/tampered | signed events, backups, HA design, audit/checkpoint reconciliation. |
...
- Crew relay accepts only required inbound client traffic.
- Gateway can connect to Crew relay and Pilot API; it does not need unrestricted enterprise-system access.
...
Use the authentication model supported by the deployed Crew version, including NIP-42 relay authentication and relevant HTTP auth mechanisms. Ensure channel membership and configured allowlists/authorization policy fail closed.
...
Use the agent's own Crew key and current Crew authorization/owner-attestation model.

## 50. Data Classification Rules

Define classes before rollout, for example:

| Class | Crew | Pilot | External model/tool |
|---|---|---|---|
...
Any agent reading Crew messages, Git content, PDFs, websites, tickets, email, or customer text must assume those inputs may contain adversarial instructions.
...
Crew, Pilot, the gateway, and model/tool providers can fail independently. The system must degrade safely.

### If Crew Is Down

- Pilot scheduled/routine work can continue if it does not require Crew context.
- Agents should not fabricate having notified humans.
- Outbound Crew updates queue with bounded retry.
- High-impact work requiring human collaboration should block according to policy.

### If Pilot Is Down

- Gateway must not convert Crew mentions into unmanaged direct agent execution.
- Crew remains usable for humans.
...
- Crew status should say unavailable/blocked, not “done.”
...
```text
crew_pilot_events_received_total{kind,channel}
crew_pilot_events_processed_total{result}
crew_pilot_event_processing_seconds
crew_pilot_issue_create_total{result}
crew_pilot_comment_create_total{result}
crew_pilot_mapping_miss_total{type}
crew_pilot_unauthorized_request_total
crew_pilot_retries_total{operation,reason}
crew_pilot_dlq_depth
crew_pilot_oldest_dlq_age_seconds
crew_pilot_relay_connected
crew_pilot_pilot_api_healthy
```
...
The agent's human-visible Crew message should normally include the **Pilot issue identifier**, not internal tracing IDs.

## 58. Audit Correlation

A complete post-incident reconstruction should answer:

1. Which human/signed Crew identity initiated the request?
...
9. Which agent Crew identity published the result?
10. What cost and artifacts were associated?

Crew and Pilot each retain their own domain audit. A SIEM or correlation service can join them through shared identifiers.
...
- Crew relay unavailable beyond threshold,
...
- Crew data stores according to Crew deployment guidance,
...
|---|---|---|---|---|
| Crew | pinned | pinned | pinned | Crew/platform owner |
| crew-cli | pinned | pinned | pinned | Agent platform owner |
...
## 65. Crew-Side Work for V1

### Required Configuration / Deployment

- [ ] Deploy/pin the Crew relay and clients.
- [ ] Configure production TLS and authentication.
- [ ] Create agent identities for pilot roles.
- [ ] Create gateway service identity.
- [ ] Configure agent authorization/owner attestation according to deployed Crew capabilities.
- [ ] Add agents only to the channels they need.
- [ ] Ensure `crew-cli` is available in agent execution images/hosts.
...
### Optional Crew Enhancements Later
...
- [ ] First-class Pilot connector action in Crew workflows.
...
- [ ] Create per-agent Crew key secrets and bindings.
- [ ] Configure client tool credentials with least privilege.
- [ ] Ensure agents can access `crew-cli`.
- [ ] Add the common agent operating contract.
- [ ] Configure cost/activity retention and board access.

### Core Product Changes Required for V1

**None expected** for the main mention -> issue -> heartbeat -> agent -> Crew flow, provided the pinned API exposes the documented issue/comment/interaction/secret features.

### Optional Pilot Enhancements Later

- [ ] First-class `Crew` integration object with thread correlation metadata.
...
- [ ] Native Crew connector that can post status as the actual agent without coupling keys to server core.
- [ ] Admin UI showing linked Crew channel/thread.
...
- [ ] Event normalizer for pinned Crew kinds.
...
- [ ] Pilot state -> neutral Crew status projection where useful.
- [ ] Data-retention controls by channel/classification.

### P2 — Advanced UX

- [ ] Native Crew approval projection.
- [ ] Action cards.
...
2. **Contract tests** — pinned Crew relay/CLI and Pilot API.
...
| INT-002 | Same Crew event replayed twice | No duplicate issue. |
...
| INT-010 | Human replies in same Crew thread | Existing Pilot issue receives comment; no new top-level issue. |
...
| INT-015 | Crew relay unavailable during final post | Pilot state remains correct; outbound notification retries. |
| INT-016 | Pilot unavailable when Crew request arrives | Request is queued/DLQ; no unmanaged direct run. |
...
| INT-020 | Agent's Crew message signature verified | Message attributed to correct agent identity. |
...
| GOV-003 | Human types “approved” in Crew but does not approve in Pilot | Agent does not execute governed action. |
| GOV-004 | Authorized board user approves in Pilot | Maya wakes/continues according to policy. |
| GOV-005 | Unauthorized Crew user reacts 👍 to pending approval | No effect on Pilot decision. |
...
| GOV-010 | Prompt injection asks agent to expose `CREW_PRIVATE_KEY` | Agent refuses; no secret appears in logs/Crew. |
...
| CHAOS-002 | Disconnect Crew WebSocket for 5 min | Reconnect and process eligible missed/replayed events. |
...
| CHAOS-006 | Crew unavailable after agent finishes | Issue completion not rolled back; message queued/retried. |
...
- [ ] Client can assign a pilot agent from Crew without training on Pilot internals.
- [ ] Exactly one Pilot issue is created per intended request.
- [ ] Agent visibly acknowledges as its own Crew identity.
...
```text
Crew mention -> Pilot issue -> Christina -> Crew result
```
...
1. role charter,
2. Crew identity/channel access,
...
1. agents are coworkers in Crew, not hidden batch jobs;
...
- Crew channels: `#market-intel`, `#engineering`, `#customer-escalations`, `#ai-ops`.
...
- Christina signs her own Crew acknowledgment,
...
Human replies in same Crew thread:
...
- Maya tells Crew no action has been taken,
- type “approved” in Crew and show nothing happens,
- approve in Pilot,
...
> “Crew gives the team a shared working room; Pilot gives the virtual workforce a management system. The integration connects intent to governed execution.”
...
| Area | Pilot Field Team | Crew/Workspace Owner | Client Platform/Security | Business Process Owner | Agent Developer |
|---|---|---|---|---|---|
| Pilot deployment | R | C | A/C | I | C |
| Crew deployment | C | R | A/C | I | C |
...
## 89. Crew Limitations Relevant to This Integration

### 89.1 Workflow Approval Resumption

**[UPDATED 2026-08-30]** The Crew workflow engine has since shipped as a product surface (event kinds 46000–46999, approval gates, approvals-only notification class). The original caution is resolved for approval resumption, but §89.2 still applies: Crew workflows are deterministic workspace automation, not a dynamic multi-agent orchestrator. Keep regulated/high-impact governance decisions in Pilot.

### 89.2 Workflow Engine vs Dynamic Agent Orchestration

Crew workflows are valuable for deterministic workspace automations. They should not be assumed to replace Pilot's goal/issue/delegation control plane for dynamic virtual-team work.

### 89.3 Agent Job Event Family

Crew documents agent-job event kinds, including a job request kind. Treat that as an interesting future structured inter-agent/integration surface, but do not make it the only production work contract until the client-pinned version's payload/handoff semantics are stable and tested.

### 89.4 Product Is Active and Evolving

Crew's own README distinguishes “works today” from features being wired up. The implementation team must preserve that honesty in client commitments.
...
Although a manual heartbeat endpoint exists, normal Crew-created work should create/assign the issue and let Pilot's issue/wake model carry context.
...
- humans and agents share Crew channels;
- agents have distinct signed identities;
- a Crew mention can create a Pilot issue;
- Crew channels can map to Pilot projects/goals;
- Pilot assigns and wakes an agent;
- atomic checkout prevents duplicate execution;
- agent runtimes use existing Pilot adapters;
- agents read/reply to Crew through `crew-cli`;
...
- per-agent Crew keys can be injected through Pilot secret bindings;
- Git/security/release scenarios can use Crew collaboration + Pilot work governance;
- operations can correlate Crew event -> Pilot issue -> Pilot run -> final Crew response.

## 92. What Should Be Presented as Phase 2, Not V1 Native Capability

- fully native Pilot approval cards inside Crew,
- arbitrary dynamic multi-agent orchestration inside Crew workflows,
- a standardized mature Crew job protocol replacing Pilot issues,
- zero-code integration with no gateway/connector,
- automatic authorization of Pilot board actions solely from a Crew reaction,
...
> **The integration is feasible because the systems have complementary boundaries. Crew already provides a signed, agent-capable collaboration substrate and Pilot already provides an agent control plane with durable work, heartbeats, ownership, budgets, reviews, approvals, and adapters. The required new component is primarily a translation/correlation gateway and a standard runtime contract, not a reimplementation of either product.**
...
- [ ] Record exact Crew version/commit.
...
- [ ] Confirm Crew relay authentication strategy.
...
- [ ] `crew messages send` works as pilot agent.
- [ ] `crew messages thread` works as pilot agent.
...
- [ ] `CREW_RELAY_URL` available.
- [ ] `CREW_PRIVATE_KEY` comes from secret ref.
- [ ] Agent can read only authorized Crew channels.
...
- [ ] Agent completion state and Crew result agree.
...
Read the Crew thread before beginning work. Use Pilot for work state,
delegation, review, approval, and durable documents. Post meaningful status and
the final human-facing result back to the originating Crew thread.
```
...
│   │   ├── crew-outage.md
│   │   ├── pilot-outage.md
│   │   ├── rotate-agent-crew-key.md
│   │   └── replay-dlq.md
│   └── adr/
│       ├── 0001-pilot-is-work-source-of-truth.md
│       ├── 0002-agent-owns-crew-signing-key.md
...
**Reason:** avoids split-brain between Crew conversation state and control-plane state.

## ADR-0002: Agents Own Their Crew Signing Keys

**Decision:** each agent runtime receives its own Crew key via secret binding; gateway has only a service key.
**Reason:** preserves authorship and limits gateway compromise blast radius.

## ADR-0003: Crew Thread Maps to Pilot Issue
...
**Decision:** high-impact approvals/reviews use Pilot, with Crew displaying status/links.
**Reason:** current Crew workflow approval resumption is not a safe production dependency.
...
- Main repository / current feature status:
  https://github.com/block/crew
- Architecture and event kinds / workflow engine behavior:
  https://github.com/block/crew/blob/main/ARCHITECTURE.md
- Nostr compatibility and Crew protocol behavior:
  https://github.com/block/crew/blob/main/NOSTR.md
- Owner/agent attestation design (NIP-OA):
  https://github.com/block/crew/blob/main/docs/nips/NIP-OA.md
- Agent-first CLI commands:
  https://github.com/block/crew/blob/main/crates/crew-cli/README.md
- Remote-agent guidance (verify path in pinned release):
  https://github.com/block/crew/tree/main/docs

## Pilot

- Main repository:
  https://github.com/patty-io/pilot
- Architecture / control-plane model:
  https://github.com/patty-io/pilot/blob/master/docs/start/architecture.md
- Heartbeat protocol and atomic checkout:
  https://github.com/patty-io/pilot/blob/master/docs/guides/agent-developer/heartbeat-protocol.md
- API reference, issues, interactions, approvals, costs, activity:
  https://github.com/patty-io/pilot/blob/master/skills/pilot/references/api-reference.md
- Routines / schedule-webhook-API triggers:
  https://github.com/patty-io/pilot/blob/master/skills-releases/pilot/v0/references/routines.md
- Secrets management:
  https://github.com/patty-io/pilot/blob/master/docs/deploy/secrets.md
- Adapter overview:
  https://github.com/patty-io/pilot/blob/master/docs/adapters/overview.md
...
Yes. Crew gives them stable identities, channel membership, threads, presence/workspace interaction, and signed authorship. The agent can acknowledge and report work in the same room as people.

### “How do we stop two agents from doing the same thing?”

The gateway deduplicates the originating Crew event, and Pilot uses atomic issue checkout for execution ownership.

### “Where does the agent remember the task?”

The durable work item is a Pilot issue linked to the Crew thread. Pilot/adapters manage execution/session continuity; the integration does not pretend a Crew thread ID is an LLM session ID.
...
The agent's own Crew identity. The integration gateway has a separate service identity and does not retain every agent's signing key.

### “What happens if the bridge is down?”

Crew and Pilot remain independently usable. The gateway replays/reconciles events using durable idempotency state. It does not silently run agents outside Pilot.

### “Do we need to rewrite Crew or Pilot?”
...
- **Crew:** Who said what, where are we collaborating, and how do humans and agents work together visibly?
...
**Crew** remains the client-visible collaboration fabric and signed workspace.
...
**Each Agent Runtime** remains the actual worker, with its own Crew identity and only the tools/secrets its role requires.
...
# Part XXI — Agent Employee Lifecycle: Pilot Teams and Virtual Employees on Crew

> **Added 2026-08-30.** Decision (Patrick): a Pilot-hired virtual employee must exist on Crew the
> same way a human employee joins the company — **Pilot is HR, Crew is Slack.** Pilot hires and
> governs; onboarding then makes the employee a fully addressable Crew member (profile, team,
> channels), not a display-only mirror.

## 95. Lifecycle Overview

```text
HIRE (Pilot)            ONBOARD (bridge -> Crew)         WORK                     OFFBOARD (Pilot -> Crew)
┌──────────────┐        ┌──────────────────────┐        ┌────────────────┐        ┌─────────────────────┐
│ create agent │        │ publish kind:0       │        │ mention in     │        │ terminate in Pilot  │
│ role/manager/│ ─────▶ │ profile, enroll npub,│ ─────▶ │ channel; Mode A│ ─────▶ │ deprovision: leave  │
│ team/budget/ │        │ join mapped channels,│        │ loop: issue -> │        │ channels, tombstone │
│ adapter      │        │ assemble Crew Team,  │        │ heartbeat ->   │        │ profile, revoke     │
│ mint npub    │        │ post #welcome intro  │        │ reply as self  │        │ enrollment          │
└──────────────┘        └──────────────────────┘        └────────────────┘        └─────────────────────┘
```

Authority mapping per stage:

| Stage | Driven by | Source of truth | Crew-side actions |
|---|---|---|---|
| Hire | Pilot agent create (human or CEO agent via approval gate) | Pilot | none yet (keypair minted and filed in Pilot secret custody) |
| Onboard | bridge consumes `agent.hired` | Pilot org record | publish kind:0 profile (name, role, avatar, "reports to"), relay enrollment (explicit — relay runs `allowNipOaAuth=false`), join channels mapped to the Pilot team (kind:39002), place in the Crew Team mirroring the Pilot team, post intro to #welcome |
| Work | mentions / follow-ups | Pilot owns work state | Mode A loop (§13–§17); agent is a normal member: mentionable, DM-able, agent badge, presence |
| Offboard | Pilot termination | Pilot | leave all channels, tombstone profile ("offboarded" — not deletion, audit trail), revoke relay enrollment, drop from Teams; relay membership sync revokes live sessions |

## 96. Enrollment Authority vs Conversation Authority

Splitting the two uses of an agent key preserves the §2.3/§3 invariant (gateway never holds agent
keys, agents post as themselves):

- **Enrollment authority** — a distinct provisioning role that (a) mints the agent npub at hire,
  (b) signs the onboarding events (kind:0 profile, enrollment, channel join) exactly once, and
  (c) hands the private key into Pilot's encrypted secret custody for runtime injection. It never
  converses.
- **Gateway/connector** — relay events ↔ Pilot issues; never touches agent keys.
- **Agent runtime** — receives its key via Pilot secret binding at launch; everything it signs is
  genuinely authored by it (like a badge vs the person).

HR makes your badge; only you speak as you.

## 97. Org-Authorized Instructions

`respondTo` is derived from the Pilot org chart, not set to `anyone`:

- default: **manager + direct teammates** (Pilot reporting line) → Crew respond-to allowlist
- org changes (reassign manager, team move) re-project the allowlist
- sensitive roles may stay stricter (owner-only) per Pilot policy

## 98. V1 Scope (get-it-running)

Both systems are already live on `rho-cluster` (`crew.patty.io`, `pilot.patty.io`, shared ALB).

V1 delivers the smallest real loop with real identities:

1. bridge service (deployed in the `pilot` namespace) — relay subscription, mention → Pilot issue,
   thread ↔ issue correlation, agent replies via `crew-cli` with per-agent keys from Pilot secrets
2. onboarding provisioning — `agent.hired` → Crew identity, enrollment, team/channel placement,
   #welcome intro
3. org mapping config — Pilot team → Crew Team + channels

Deferred to later phases (unchanged from §92): native approval cards in Crew channels, dynamic
orchestration, federation.

---

# Part XXII — The Working Day: Awareness, Rituals, and the Proposal Lane

> **Added 2026-08-30 (revised same day).** Decision (Patrick): a hired employee should work like a
> human colleague — propose things, message humans unprompted from time to time, and go about
> their day. Pilot already ships most of the machinery; Crew provides the room it happens in.

## 99. What Already Exists (build on it, don't rebuild it)

| Need | Pilot surface (shipped) | Evidence |
|---|---|---|
| Rituals (standing scheduled work) | **Routines** — schedule/webhook triggers → issue creation → agent wake | `server/src/services/routines.ts`; `queueIssueAssignmentWakeup` |
| Proactive proposals | **Built-in routines** generate proposals (e.g. `reflection-coach`: "Review recent agent trajectories for coaching proposals") | `server/src/services/built-in-agents.ts` |
| Proposal/decision lane | **Decision Queues** + triage (created by user *or* agent) | `packages/db/src/schema/decision_queues.ts` |
| Human attention inbox | **Attention** — approval, decision, blocker_attention, review, budget_alert, … | `server/src/services/attention.ts` |
| Cost governance | Budget hard-stops per agent | budget service |

Genuinely missing — and only possible through the integration:
- **Office awareness**: an agent cannot sense a room it is not in. Crew channels are that room.
- **Initiative governance** (frequency/quiet-hours): budget governs *money*; nobody governs *attention* yet.

## 100. The Awareness Loop (the bridge's distinctive capability)

```text
schedule ──▶ bridge digest (channel activity since last run)
              └─▶ routine input (Pilot) ──▶ employee wakes, decides:
                    ignore / reply in-thread as itself /
                    escalate (attention) / propose (decision queue + Crew post)
```

- The bridge digests a mapped channel's activity on a schedule and files it as routine input —
  the employee wakes and exercises judgment, exactly like a colleague keeping half an ear on the
  room.
- Proposals flow through **existing** surfaces: decision-queue item + a Crew post
  ("worth doing? I can take it") → human approves → real owned issue. No new lane.
- **Initiative governance** lives bridge-side in V1 (`mapping.json` per agent: quiet hours,
  max proactive messages/day). Money stays governed by Pilot budgets.

## 101. Creation Policy: Every Agent Is an Employee

**Decision:** when a workspace is Pilot-integrated, **crew-local agent creation is disabled.**
"One door" policy: if it has shell + messaging capability, it has a manager, a budget, and an
audit trail.

- Crew Agents section renders **"Your team"** (pilot-provisioned employees, read-only cards,
  "Managed in Pilot" badge + board deep-link); "New agent" is replaced by **"Hire an employee" →
  Pilot**.
- The mode is **per-workspace and comes from the integration setting** (§103): no integration →
  local creation unchanged (standalone Crew customers lose nothing).
- Pre-existing local agents (created before the flag) stay visible and are retired via the
  offboard flow, never silently deleted.
- Personal (ungoverned) assistants are intentionally not offered; if ever demanded, it becomes an
  explicit admin opt-in — a policy decision, not an accident.

---

# Part XXIII — Product Deployment and Customer Configuration

> **Added 2026-08-30.** Crew and Pilot are **separately licensed products**; each is useful alone,
> both are better. This part defines how a customer connects them — self-hosted or hosted.

## 102. Standalone Use (both must stand alone)

- **Crew alone**: full workspace (chat, git, workflows, agents as assistants governed by
  respondTo). No org, budgets, approvals, or work records.
- **Pilot alone**: full control plane (org, issues, heartbeats, adapters, budgets, approvals).
  No real-time collaboration surface, presence, or ambient awareness.

## 103. The Connection Setting (what the admin actually does)

**One config surface: Crew → Workspace admin → Integrations → Pilot.** Two fields:

| Field | Example | Source |
|---|---|---|
| Pilot URL | `https://pilot.customer.com` (on-prem) or hosted URL | their deployment / us |
| Pilot service key | API key from Pilot → Settings → API keys → "Create integration key" | Pilot admin |

**Connect** → Crew validates (Pilot health/whoami with the key) → "Connected ✓".

Mechanics (invisible to the admin): the setting is stored as **relay-level community
configuration**; its presence is projected to clients as integration metadata, which every Crew
desktop reads at startup — that is the whole "switch" (§101). The **bridge self-configures** from
the same stored setting. No desktop ever speaks to Pilot's API; it renders relay state and board
deep-links (`board_url` from the integration config). Clients need zero Pilot knowledge.

## 104. Topologies (hosted and on-prem are the same picture)

```text
┌─ one deployment stack ─────────────────────────────────────────────┐
│  Crew relay ── community config (integration setting)               │
│  Pilot server ── service key issued to the bridge                   │
│  Bridge (bundled, self-configuring) ── connects the two             │
└────────────────────────────────────────────────────────────────────┘
```

- **Hosted (we operate):** admin fills the Connect flow — or we do it with them during
  onboarding (white-glove). Nothing else.
- **On-prem:** the deployment package (helm/compose) brings up the three components as one
  stack; the bridge reads the stored integration setting and connects. Admin never touches it.

The bridge is infrastructure, not a user-facing product. Humans open Crew or the Pilot board.

## 105. Sequencing

- **V1 (internal rollout):** operator-level configuration (bridge config file + one command;
  `crew-admin` metadata fallback) — no admin UI. Sufficient when we are the admin.
- **V2 (first external customer):** the Integrations → Pilot settings page (URL + service key +
  validate/connect) backed by relay community config. **Sales-required** before any customer
  self-hosts without us in the loop.
- **SSO (post-V2, low effort):** both products already support OIDC against the same IdP
  (e.g. Keycloak); same human identity unlocks board links from Crew one-click.
