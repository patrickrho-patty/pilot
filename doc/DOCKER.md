...
| File | Purpose |
|------|---------|
| `docker/quadlet/pilot.pod` | Pod definition — groups containers into a shared network namespace |
| `docker/quadlet/pilot.container` | Pilot server — joins the pod, connects to Postgres at `127.0.0.1` |
| `docker/quadlet/pilot-db.container` | PostgreSQL 17 — joins the pod, health-checked |
...
3. Create a secrets env file (keep out of version control):
   ...
   ```sh
   cat > ~/.config/containers/systemd/pilot.env <<EOL
   BETTER_AUTH_SECRET=$(openssl rand -hex 32)
   PILOT_TOOL_ACTION_SIGNING_SECRET=$(openssl rand -hex 32)
   POSTGRES_USER=pilot
   POSTGRES_PASSWORD=pilot
   POSTGRES_DB=pilot
   DATABASE_URL=postgres://pilot:pilot@127.0.0.1:5432/pilot
   # OPENAI_API_KEY=sk-...
...
- Pilot data persists at `~/.local/share/pilot`.
...
SMOKE_DETACH=true SMOKE_METADATA_FILE=/tmp/pilot-smoke.env PILOTAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```
...
- Pilot data persists via Docker volumes/bind mounts (compose) or at `~/.local/share/pilot` (quadlet).
