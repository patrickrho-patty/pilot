...
Pilot should not auto-load arbitrary code from a workspace repo like `.paperclip/plugins` or project directories.
...
A `@paperclipai/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Pilot instance.
...
- `@paperclipai/plugin-test-harness` and `create-pilot-plugin` starter template
...
8. Ship `@paperclipai/plugin-test-harness` and `create-pilot-plugin` starter template.
