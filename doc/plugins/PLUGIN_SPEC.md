...
- The current runtime ships a small host-provided plugin UI component kit through `@pilotai/plugin-sdk/ui`, but does not support plugin asset uploads/reads yet. Treat plugin asset APIs as future-scope ideas, not current implementation promises.
...
4. Depend on project-local plugin folders such as `.pilot/plugins`.
...
- `~/.pilot/instances/default/plugins/package.json`
- `~/.pilot/instances/default/plugins/node_modules/`
- `~/.pilot/instances/default/plugins/.cache/`
- `~/.pilot/instances/default/data/plugins/<plugin-id>/`
...
- `npx pilotai plugin install <package[@version]>`
- `npx pilotai plugin uninstall <plugin-id>`
- `npx pilotai plugin upgrade <plugin-id> [version]`
- `npx pilotai plugin doctor <plugin-id>`
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@pilotai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@pilotai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@pilotai/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@pilotai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@pilotai/plugin-sdk/ui`
...
The `@pilotai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
6. The operator may force-purge immediately via CLI: `npx pilotai plugin purge <plugin-id>`.
...
### 27.1 `@pilotai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@pilotai/plugin-test-harness";
...
- The operator installs the plugin from a local path: `npx pilotai plugin install ./path/to/plugin`
...
- `@pilotai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@pilotai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@pilotai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@pilotai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@pilotai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@pilotai/plugin-sdk/ui`
