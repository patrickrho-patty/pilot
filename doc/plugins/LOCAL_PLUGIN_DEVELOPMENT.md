...
This is the short happy-path guide for developing a Pilot plugin from a folder on your machine. You will scaffold a plugin, run it in watch mode, install it into a running Pilot instance from an absolute local path, and edit code with the plugin worker reloading after each rebuild.
...
- A local Pilot checkout you can run from source. Local plugin installs read source from disk, so the running server must be able to see the path you give it.

## The five steps

```bash
# 1. Start Pilot locally
pnpm pilotai run

# 2. Scaffold a plugin outside the Pilot repo
pilotai plugin init @acme/hello-plugin --output ~/dev/pilot-plugins

# 3. Install dependencies and start the watch build
cd ~/dev/pilot-plugins/hello-plugin
pnpm install
pnpm dev

# 4. In another terminal, install the plugin from its absolute path
pilotai plugin install ~/dev/pilot-plugins/hello-plugin

# 5. Confirm it loaded
pilotai plugin list
pilotai plugin inspect acme.hello-plugin
```
...
Pilot listens on `http://127.0.0.1:3100` by default. The CLI talks to that server, so leave it running.
...
This creates `~/dev/pilot-plugins/hello-plugin/` with `src/manifest.ts`, `src/worker.ts`, `src/ui/index.tsx`, an esbuild watch config, a Vitest config, and a snapshot of `@pilotai/plugin-sdk` from your local Pilot checkout. You can run the package and tests without publishing anything to npm.
...
If your plugin has UI and you want a browser-side dev server with hot module replacement during local UI iteration, run `pnpm dev:ui` in a second terminal. It serves `dist/ui/` on `http://127.0.0.1:4177`. This is optional; Pilot can load the built UI directly from `dist/ui/` without it.
...
Before it installs, the CLI probes `GET /api/health` on the instance it is configured to talk to and prints the **target diagnostics** so you can confirm *which* Pilot you are installing into. You will see a confirmation like:

```
Target Pilot: http://127.0.0.1:3100
  health: status=ok  version=0.1.0  mode=local_trusted  exposure=private
Installing plugin from local path: /Users/you/dev/pilot-plugins/hello-plugin
✓ Installed acme.hello-plugin v0.1.0 (ready)
Local plugin installs run trusted local code from your machine.
Keep `pnpm dev` running in /Users/you/dev/pilot-plugins/hello-plugin;
Pilot watches rebuilt dist output and reloads the plugin worker.
```

Read that first line. If the API URL, version, or mode is not the instance you expect, stop and re-point the CLI (see [Targeting a branch / issue-workspace runtime](#targeting-a-branch--issue-workspace-runtime)) before trusting the result. Pass `--no-verify-target` to skip the probe, or run `pilotai plugin target` to see the same diagnostics without installing anything.

Relative paths are resolved against the current working directory, so `pilotai plugin install .` from inside the plugin folder works too.
...
The five-step loop above assumes one Pilot on `http://127.0.0.1:3100`. That breaks down the moment your plugin depends on **server code that only exists on a branch**. Examples:
...
2. `PILOT_API_URL` environment variable,
3. the active CLI context profile's `apiBase`,
4. inferred default `http://<PILOT_SERVER_HOST|localhost>:<PILOT_SERVER_PORT|config.server.port|3100>`.

So the API URL is explicit and overridable — the gap was never that you *couldn't* point at a branch server, it was that nothing told you which server you ended up on. `pilotai plugin target` and the pre-install probe close that gap.
...
PILOT_SERVER_PORT=3120 pnpm dev          # or: pnpm pilotai run

# 2. Confirm the CLI will talk to that exact branch service before installing.
pilotai plugin target --api-base http://127.0.0.1:3120
# Target Pilot: http://127.0.0.1:3120
...
export PILOT_API_URL=http://127.0.0.1:3120
pilotai plugin target
pilotai plugin install ~/dev/pilot-plugins/hello-plugin
```
...
curl -s "http://127.0.0.1:3120/api/companies/<companyId>/<branch-route>" \
  -H "Authorization: Bearer $PILOT_API_KEY" | head
```
...
Pilot watches the on-disk plugin package after a local install. The watcher targets the runtime entrypoints declared in the package's `pilotPlugin` field (`dist/manifest.js`, `dist/worker.js`, `dist/ui/`).

What that means in practice:

- **Worker code:** save a `.ts` file → esbuild rewrites `dist/worker.js` → Pilot debounces ~500ms and restarts the plugin worker. The next worker call uses the new code. There is no in-process hot module replacement for worker code; it is a worker restart.
- **Manifest:** save `src/manifest.ts` → `dist/manifest.js` rewrites → the worker restarts and the host re-reads the manifest.
- **Plugin UI:** save a `.tsx` file → esbuild rewrites `dist/ui/` → Pilot reloads the UI bundle on its next mount. To get HMR during UI iteration, run `pnpm dev:ui` and point at the dev server with `devUiUrl` in your manifest while developing.
- **Without `pnpm dev`:** the watcher only fires on `dist/*` changes. If you stop the watch build, source edits do not reach Pilot. Restart `pnpm dev` (or run `pnpm build` once) before expecting changes.
- **`node_modules`, `.git`, `.pilot-sdk`, and other dotfolders are ignored.** Adding a dependency requires the new code to actually be imported and rebuilt before the worker sees it.

The package's own build scripts still own compilation. Pilot does not compile arbitrary local-path plugins for you. The exceptions are bundled plugins inside the Pilot repo under `packages/plugins/`: workspace packages auto-build once with `pnpm --filter <package> build`, and standalone sandbox-provider packages under `packages/plugins/sandbox-providers/` first bootstrap package-local dependencies with `pnpm install --ignore-workspace ...` and then run `pnpm build` in place. Set `PILOT_DISABLE_PLUGIN_AUTOBUILD=1` in the server environment to disable those fallbacks.

## Local path plugins vs npm packages

Both go through the same install endpoint, but they mean different things:

- **Local path plugins are trusted local code.** Pilot executes worker code from disk under the same trust boundary as the rest of the running instance. This is meant for developing or operating a plugin against a checkout you control. There is no signature check, no sandboxing of worker code, and no provenance metadata beyond the path. Do not install local-path plugins you did not write.
- **npm packages are the deployable artifact.** `pilotai plugin install @acme/plugin-foo` (optionally `--version 1.2.3`) installs from your configured npm registry, version-pins, and produces an install record that other operators can reproduce. Ship plugins this way.
...
- **Restart cleanly:** `pilotai plugin disable <key>` pauses the plugin without removing it. `pilotai plugin enable <key>` brings it back. `pilotai plugin uninstall <key>` removes the install record; add `--force` to also purge plugin state and settings.
- **Browse examples:** `pilotai plugin examples` lists the bundled example plugins that ship with the repo, each with a ready-to-run `pilotai plugin install <path>` line.
- **Go deeper:** [`PLUGIN_AUTHORING_GUIDE.md`](./PLUGIN_AUTHORING_GUIDE.md) covers worker capabilities, managed agents/projects/routines/skills, plugin database namespaces, scoped API routes, and the shared UI components in `@pilotai/plugin-sdk/ui`. [`PLUGIN_SPEC.md`](./PLUGIN_SPEC.md) is the longer-form specification, including future ideas that are not yet implemented.
...
- **`Plugin install returned no plugin record` or `error` status.** Run `pilotai plugin inspect <key>` for the last error. The most common causes are (1) the plugin has not built yet — run `pnpm dev` or `pnpm build` first, (2) the `pilotPlugin` entries in `package.json` point at files that do not exist on disk, or (3) the manifest failed validation. Bundled repo plugins may auto-build once during install, but external local-path plugins still require you to build them yourself. The Pilot server log has the full validation error.
- **Edits do not seem to reload.** Confirm `pnpm dev` is still running and writing to `dist/`. If you renamed entry files, update the `pilotPlugin.manifest` / `pilotPlugin.worker` / `pilotPlugin.ui` fields in `package.json` so the watcher targets them.
...
- **Plugin behaves as if a route or field is missing (e.g. `API route not found`, empty data, or a fallback path triggering unexpectedly).** You are probably installed into a Pilot instance that does not run your branch code. Run `pilotai plugin target` and compare the reported API URL and `version` against the branch service you meant to test. See [Targeting a branch / issue-workspace runtime](#targeting-a-branch--issue-workspace-runtime) to run the branch server and point the CLI at it explicitly.
