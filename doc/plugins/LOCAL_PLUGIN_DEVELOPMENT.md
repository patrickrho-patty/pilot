...
pilotai plugin init @acme/hello-plugin --output ~/dev/paperclip-plugins
...
pilotai plugin install ~/dev/paperclip-plugins/hello-plugin
...
This creates `~/dev/paperclip-plugins/hello-plugin/` with `src/manifest.ts`, `src/worker.ts`, `src/ui/index.tsx`, an esbuild watch config, a Vitest config, and a snapshot of `@paperclipai/plugin-sdk` from your local Pilot checkout. You can run the package and tests without publishing anything to npm.
