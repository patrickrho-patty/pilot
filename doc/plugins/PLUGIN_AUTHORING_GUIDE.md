...
- The host provides a small shared React component kit through
  `@pilotai/plugin-sdk/ui`; use it for common Pilot controls before
...
Inside this monorepo, the scaffold uses `workspace:*` for `@pilotai/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@pilotai/plugin-sdk` from the local Pilot checkout into a `.pilot-sdk/` tarball so you can build and test a plugin without publishing anything to npm first. Pass `--sdk-path /absolute/path/to/pilot/packages/plugins/sdk` if you have more than one Pilot checkout.
...
```ts
import { definePlugin } from "@pilotai/plugin-sdk";
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@pilotai/plugin-sdk/ui`
...
Use shared components from `@pilotai/plugin-sdk/ui` when the plugin needs a
...
Use `FileTree` from `@pilotai/plugin-sdk/ui` whenever the plugin only needs
