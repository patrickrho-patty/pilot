...
```bash
cd ~/Projects/DEV/pilot
git fetch origin && git worktree add ../pilot-run3 -b design/component-convergence origin/master
cd ../pilot-run3 && pnpm install
```
...
```bash
cd ~/Projects/DEV/pilot-design-simplify
...
4. pnpm check:token-gates 3/3 CLEAN; pnpm typecheck green;
   pnpm --filter @pilotai/ui build exit 0.
