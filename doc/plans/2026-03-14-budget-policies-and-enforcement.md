...
Pilot already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model Pilot should implement next.

## Product Goals

Pilot should let operators:
...
For the next implementation pass, Pilot should enforce these defaults:
...
Pilot should separate subscription-included usage from billed spend:
...
Pilot should have two threshold classes:
...
Pilot must know that a project is budget-paused and enforce it.
...
Pilot must also block execution before new work starts.
