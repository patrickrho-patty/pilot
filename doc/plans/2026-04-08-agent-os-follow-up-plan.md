# PIL-1229 Agent OS Follow-up Plan

Date: 2026-04-08
Related issue: `PIL-1229`
Companion analysis: `doc/plans/2026-04-08-agent-os-technical-report.md`

## Goal

Turn the `agent-os` research into a low-risk Pilot execution plan that preserves Pilot's control-plane model while testing the few runtime ideas that appear worth adopting.

## Decision summary

Pilot should not absorb `agent-os` as a product model or orchestration layer.

Pilot should evaluate `agent-os` in three narrow areas:
...
This work is successful when Pilot has:
...
- a proposed runtime capability model that fits current Pilot adapters
...
- replace Pilot heartbeats, issues, comments, approvals, or budgets with `agent-os` primitives
- introduce Rust/sidecar requirements for all local execution paths
- migrate all adapters at once
- add runtime workflow/queue abstractions to Pilot core

## Existing Pilot integration points
...
- prove that Pilot can drive one local agent through an `agent-os` runtime without breaking heartbeat semantics
...
- keep Pilot's existing heartbeat, issue, workspace, and comment flow authoritative
...
- introduce a Pilot-native capability vocabulary without coupling the product to `agent-os`
...
- determine whether a layered/snapshotted root model improves some Pilot workloads
...
- identify where Pilot should prefer explicit typed tools over ambient shell access
...
- one concrete proposal for typed-tool adoption in Pilot
...
- runtime experimentation could blur the boundary between Pilot as control plane and Pilot as execution platform
