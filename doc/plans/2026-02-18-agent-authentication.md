...
Agents need API keys to authenticate with Pilot. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Pilot
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Pilot generates a token and passes it directly to the agent
...
`PILOT_API_URL`, `PILOT_AGENT_ID`, `PILOT_COMPANY_ID`. We need to
add a `PILOT_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Pilot CLI
...
managed by the Pilot server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Pilot.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Pilot SKILL.md (or a link to it)
   - What information Pilot needs from the agent (e.g. webhook URL, adapter
...
5. Pilot stores the pending registration.
...
7. On approval, Pilot provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Pilot issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "requiredFields": {
      ...
      "adapterType": "How Pilot should send you heartbeats",
...
1. Generate a Pilot invite link for the company.
...
4. A Pilot company member approves the new agent.
5. Pilot begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `PILOT_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `pilotai auth login` for developer-managed remote agents.                                      |
