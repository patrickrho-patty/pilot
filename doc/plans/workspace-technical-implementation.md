...
This repo already uses `PILOT_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `PILOT_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Pilot may create host-local worktrees, processes, and paths
- `cloud`
  - Pilot should assume no durable host-local execution workspace management
...
- use `PILOT_EXECUTION_TOPOLOGY` as a defaulting guardrail
