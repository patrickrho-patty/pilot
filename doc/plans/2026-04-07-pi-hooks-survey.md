...
We were asked to find the hook surfaces exposed by `pi` and `pi-mono`, then decide which ideas transfer cleanly into Pilot.
...
- current Pilot plugin and adapter surfaces in this repo
...
Pilot already has several hook-like surfaces, but they are much narrower and safer:
...
## What transfers well to Pilot

These ideas from `pi-mono` fit Pilot with little conceptual risk:

### 1. Read-only run lifecycle subscriptions

Pilot should continue exposing run and transcript events to plugins, for example:
...
This matches Pilot's control-plane posture: observe, react, automate.

### 2. Plugin-to-plugin events

Pilot already has this. It is worth keeping and extending.

This is the clean replacement for many ad hoc hook chains.

### 3. Explicit worker lifecycle hooks

Pilot already has `setup()` and `onHealth()`. That is the right shape.

If more lifecycle is needed, it should stay explicit and host-controlled.

### 4. Trusted adapter-level prompt/runtime middleware

Some `pi-mono` ideas do belong in Pilot, but only inside trusted adapter/runtime code:
...
These `pi-mono` capabilities are a bad fit for Pilot core:

### 1. Arbitrary mutation hooks on control-plane decisions

Pilot should not let general plugins rewrite:
...
Pilot should keep plugin tools namespaced and non-shadowing.

### 3. Project-local plugin loading

Pilot is an operator-controlled control plane. Repo-local plugin auto-loading would make behavior too implicit and too hard to govern.
...
They do not map directly to Pilot's board-and-issues model.
...
If the question is "what works for Pilot too?":
