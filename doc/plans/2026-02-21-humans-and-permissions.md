...
2. CLI command `pnpm pilotai auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm pilotai onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm pilotai onboard`).
...
- expose key instance settings in API + CLI (`pilotai configure` / `pilotai onboard`)
...
7. `pnpm pilotai onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
