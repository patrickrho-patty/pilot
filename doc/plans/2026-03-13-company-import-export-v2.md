...
This document defines the next-stage plan for Pilot company import/export.

The core shift is:

- move from a Pilot-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Pilot.
...
Pilot already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Pilot
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `pilot.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.paperclip.yaml` sidecar for high-fidelity Pilot-specific details
...
11. adapter-aware skill sync surfaces so Pilot can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Pilot at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Pilot sidecar.
...
Pilot must not redefine `SKILL.md`.
...
- Pilot-specific fidelity lives in `.paperclip.yaml`
- Pilot may resolve and install `SKILL.md` packages, but it must not require a Pilot-only skill format
...
### 5.4 Base Package Vs Pilot Extension
...
- base package:
  ...
  - no Pilot-only runtime fields by default
- Pilot extension:
  - `.paperclip.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Pilot tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`pilot.manifest.json` is not part of the future package direction.
...
Pilot import/export should support these entity kinds:
...
- Pilot can treat “this set of agents came from team package X” as the imported-team model
...
Pilot should support source references in package metadata with:
...
Pilot should define a new adapter surface area around skills:
...
Planned Pilot behavior:

- if an adapter supports read, Pilot should show current skills in the UI
- if an adapter supports write, Pilot should let the user enable/disable imported skills
- if an adapter supports sync, Pilot should compute desired vs actual state and offer reconcile actions
...
Pilot should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `pilotai company export <company-id> --out <path>`
- `pilotai company import <path-or-url> --dry-run`
- `pilotai company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Pilot storage, or only referenced at import time?
...
3. Should Pilot support direct local folder selection in the web UI, or keep that CLI-only initially?
