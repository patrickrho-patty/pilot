...
This document defines the rollout plan for adapter-wide skill support in Pilot.
...
- Pilot stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

Pilot currently has these adapters:
...
1. Desired skills live in Pilot for every adapter.
...
These adapters have a stable local skills directory that Pilot can read and manage.
...
These adapters do not have a meaningful Pilot-owned persistent install state.
...
- desired skills are stored in Pilot
...
- desired skills stored in Pilot
...
- Pilot reconciles desired skills into `~/.cursor/skills`
...
- Pilot reconciles desired skills into `~/.gemini/skills`
...
- Pilot reconciles desired skills into `~/.pi/agent/skills`
...
- Pilot can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects Pilot skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- Pilot may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed Pilot skills that are clearly marked as Pilot-managed
...
- Pilot stores desired skills only
...
- avoid destructive wording unless Pilot can prove a skill is Pilot-managed
