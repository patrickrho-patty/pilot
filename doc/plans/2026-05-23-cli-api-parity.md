...
The most important requirement is credential ergonomics. External integrations need a reliable "way in" to Pilot:
...
- `agent local-cli` creates an agent API key through board access, installs local skills, and prints `PILOT_API_URL`, `PILOT_COMPANY_ID`, `PILOT_AGENT_ID`, and `PILOT_API_KEY`.
...
The CLI must always know which Pilot API it is operating against. This is especially important for fork/local development, where Pilot may run on `3101+` rather than the upstream default `3100`.
...
- `pilotai connect` must show the resolved API base before any auth or mutation and allow the user to override it.
...
```json
{
  ...
  "profiles": {
    "default": {
      ...
      "persona": "agent",
      "agentId": "agent-id",
      "apiKeyEnvVarName": "PILOT_API_KEY"
    }
  }
}
```
...
```sh
pilotai token board create --company-id <company-id> --name "external-admin"
pilotai token board list
pilotai token board revoke <key-id>
```
...
```sh
pilotai token agent create --company-id <company-id> --agent <agent-id-or-name> --name "external-worker"
pilotai token agent list --company-id <company-id> --agent <agent-id-or-name>
pilotai token agent revoke --agent <agent-id-or-name> <key-id>
```
...
```sh
pilotai agent-prompt <agent-name-or-id> <agent-api-key> "Prompt here"
```

Recommended safer variants:

```sh
pilotai agent prompt --agent <agent-name-or-id> --api-key-env PILOT_API_KEY "Prompt here"
pilotai agent prompt --profile my-agent "Prompt here"
pilotai board prompt --agent <agent-name-or-id> "Prompt here"
```
...
- Default prompt target should be `issue create + assign + wake`, because Pilot's communication model is tasks/comments, not chat.
...
```sh
pilotai agent wake <agent>
pilotai run list --company-id <company-id>
pilotai run get <run-id>
pilotai run log <run-id>
pilotai run cancel <run-id>
pilotai issue runs <issue-id>
```
...
```sh
pilotai project list|get|create|update|delete
pilotai goal list|get|create|update|delete
```
...
```sh
pilotai issue child create <issue-id>
pilotai issue document list|get|put|delete|lock|unlock|revisions|restore
pilotai issue work-product list|create|update|delete
pilotai issue interaction list|create|accept|reject|respond|cancel
pilotai issue attachment list|upload|download|delete
pilotai issue force-release <issue-id>
pilotai issue label list|create|delete
pilotai issue read|unread|archive|unarchive
```
...
```sh
pilotai agent create|update|pause|resume|approve|terminate|delete
pilotai agent org
pilotai agent config get|revisions|rollback
pilotai agent instructions get|set|file
pilotai adapter list|models|profiles|detect|test|install|enable|disable|reload
```
...
```sh
pilotai cost summary|by-agent|by-project|by-provider|issue
pilotai cost event create
pilotai finance event create|list|summary
pilotai budget overview|set-company|set-agent|policy-create|incident-resolve
```
...
```sh
pilotai invite create|list|revoke|show|onboarding
pilotai join list|approve|reject|claim-key
pilotai member list|update|archive|permissions
pilotai admin user list|promote|demote|company-access
```
...
```sh
pilotai routine list|create|get|update|run|runs|trigger|revision
pilotai environment list|create|get|update|delete|probe|leases
pilotai workspace list|get|update|operations|runtime
pilotai project workspace list|create|update|delete|runtime
```
...
```text
pilotai connect
pilotai token board|agent create|list|revoke
pilotai whoami
pilotai prompt ...
pilotai board ...
pilotai agent ...
...
- Add aliases only for high-frequency flows, for example `pilotai ask` as an alias for `pilotai prompt`.
...
- Destructive tests must require an explicit test opt-in such as an env var or a dedicated test command; normal unit tests must not mutate a real running Pilot instance.
...
- A new user can run `pilotai connect`, confirm or override the API base, select board or agent, and get a saved working profile tied to that API base.
...
```sh
pilotai agent-prompt AgentName "$AGENT_API_KEY" "Prompt here"
```

- The one-liner creates or updates Pilot work, does not require a browser, and fails with a clear company/agent mismatch error when the token does not belong to the requested agent.
