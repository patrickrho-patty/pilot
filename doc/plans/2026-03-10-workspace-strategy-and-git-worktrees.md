...
`PAP-447` asks how Pilot should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Pilot instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Pilot:

- some operators use Pilot to manage Pilot and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Pilot
- Claude and Codex expose different built-in affordances, so Pilot should not overfit to one tool

## Core Product Decision

Pilot should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Pilot
...
- Pilot should not assume that every adapter can see or use a host filesystem path directly
...
If Pilot makes worktrees agent-first, it will blur:
...
- Pilot core resolves and records execution workspace state
...
That gives Pilot room to support:
...
- Pilot handles workspace runtime services under the hood using project/workspace policy
...
Once Pilot is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Pilot cannot infer
...
- when issue moves to done, should Pilot auto-commit?
- should it auto-push?
...
Pilot already has the right foundation for a project-first model.
...
For server-spawned local adapters, Pilot already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Pilot should mirror directly

Implication:

- for `codex_local`, Pilot should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Pilot should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Pilot should separate:
...
Pilot should be able to express intentions such as:
...
These are not specific to Pilot. They are a common property of working in a dev workspace, whether local or remote.
...
Do not model this as a Pilot-specific `devServer` flag.
...
Pilot should distinguish between:
...
Pilot should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Pilot should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Pilot
...
For runtime services specifically, Pilot should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Pilot-only dev-server flags
...
3. Inject resolved execution workspace details into `context.pilotWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Pilot instance can be reused through env/config injection when appropriate
...
- Pilot can decide whether to reuse or start a fresh service deterministically
...
3. Test against Pilot-on-Pilot workflows first.
...
7. Workspace runtime services are modeled generically, not as Pilot-specific dev-server toggles.
