...
Today release smoke testing for published Pilot packages is manual and shell-driven:
...
- runs `pilotai@${PILOTAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@pilot.local`
- password: `pilot-smoke-password`
...
- the suite passes locally against both:
  - `PILOTAI_VERSION=canary`
  - `PILOTAI_VERSION=latest`
...
1. A published `pilotai@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `pilotai@latest` Docker install can be smoke-tested by Playwright in CI.
