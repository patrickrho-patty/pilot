...
- Team of 3 can use one shared Pilot instance.
...
Pilot should manage the **issue → workspace → preview/PR → review handoff** lifecycle, but leave diffs/review/merge to external tools.
...
execution:
  ...
  preview:
    command: "pnpm dev --port $PILOT_PREVIEW_PORT"
    healthPath: "/"
    ttlMinutes: 120
  vcs:
    provider: github
...
- A chat plugin can converse without requiring chat to become the core Pilot product.
...
- **Keep** Pilot as the board-level control plane.
