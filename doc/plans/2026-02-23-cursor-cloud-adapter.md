...
This document defines the V1 design for a Pilot adapter that integrates with
...
4. synthesized stdout events for Pilot UI/CLI
...
3. **Agent callback auth to Pilot** uses a bootstrap exchange flow (no long-lived Pilot key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Pilot endpoints, not full SKILL.md prompt injection.
...
- `pilotPublicUrl` (optional override; else `PILOT_PUBLIC_URL` env)
...
## Pilot Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `PILOT_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.pilotPublicUrl`
2. `process.env.PILOT_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Pilot credentials in prompt text.

Flow:

1. Before launch/follow-up, Pilot mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Pilot validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Pilot API calls.
...
1. Prompt includes a compact instruction to fetch skills from Pilot.
...
- include webhook object when enabled:
  - `url: <pilotPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Pilot run context.
...
5. `pilotPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Pilot skill endpoints |
| Pilot auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Pilot context.
3. Add richer PR metadata surfacing in Pilot UI.
