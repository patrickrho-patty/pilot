# PIL-1231 Agent Browser Process Cleanup Plan

Status: Proposed
Date: 2026-04-08
Related issue: `PIL-1231`
...
Yes, there is a likely root cause in Pilot's local execution model.
...
If Codex, Claude, Cursor, or a skill launched through them starts Chrome or Chromium helpers, Pilot can lose ownership of those descendants even when it still believes it handled the run correctly.
...
This is the clearest internal comparison point: Pilot already has one local-process subsystem that treats process-group ownership as the right abstraction.
...
- Pilot may think it cancelled the run because the parent process is gone
...
This work is successful when Pilot can:
...
- require every skill author to add bespoke browser teardown logic before Pilot can clean up correctly
...
- make the leak measurable from Pilot's side before changing execution semantics
...
- add a control-plane cleanup action or CLI utility for stale local run processes owned by Pilot
...
Treat this as a Pilot executor ownership bug, not an `agent-browser` bug.
