# Agent OS Technical Report for Pilot
...
`agent-os` is not a competitor to Pilot's core product. It is an execution substrate: an embedded, VM-like runtime for agents, tools, filesystems, and session orchestration. Pilot is a control plane: company scoping, task hierarchy, approvals, budgets, activity logs, workspaces, and governance.

The strongest takeaway is not "copy agent-os wholesale." The strongest takeaway is that Pilot could selectively use its runtime ideas to improve local agent execution safety, reproducibility, and portability while keeping all company/task/governance logic in Pilot.

My recommendation is:

1. Do not merge agent-os concepts into the Pilot core product model.
...
4. Do not import its workflow/cron/queue abstractions into Pilot core until they are reconciled with Pilot's issue/comment/governance model.
...
That gives `agent-os` a much stronger isolation story than Pilot's current "launch a host CLI in a workspace" local adapter approach.
...
This is directly relevant to Pilot because our adapter layer still normalizes each CLI agent in a fairly bespoke way.

## Pilot anchor points

The most relevant current Pilot surfaces for any future `agent-os` integration are:
...
## What Pilot can learn from it

### 1. A safer local execution substrate

Pilot's local adapters currently run host CLIs in managed workspaces and rely on adapter-specific behavior plus process-level controls. That is pragmatic, but weakly isolated.
...
This fits Pilot because it improves execution safety without changing the control-plane model.

### 2. Snapshotted execution roots instead of only mutable workspaces

Pilot already has strong execution-workspace concepts, but they are repo/worktree-centric. `agent-os` adds a stronger "start from known lower layers, write into a disposable upper layer" model.
...
Pilot has governance at the company/task level:
...
It has less structure at the runtime capability level. `agent-os` offers a clear vocabulary that Pilot could adopt even without adopting the runtime itself:
...
Pilot's plugin system and adapters already have the beginnings of a controlled extension surface. `agent-os` reinforces the value of exposing capabilities as typed tools rather than raw shell access.
...
This aligns well with Pilot's governance story.

### 5. Better adapter normalization around sessions and capabilities

Pilot's adapter contract already supports execution results, session params, environment tests, skill syncing, quota windows, and optional `detectModel()`. But much of the per-agent behavior is still adapter-specific.
...
Pilot does not need ACP everywhere, but it would benefit from a more formal internal session capability model inspired by this.
...
Pilot can adopt that philosophy directly:
...
## What does not fit Pilot well

### 1. Its built-in orchestration primitives overlap the wrong layer

`agent-os` includes cron/session/workflow style primitives inside the runtime package. Pilot already has higher-level orchestration concepts:
...
If Pilot copied `agent-os` cron/workflow/queue ideas directly into core, we would likely duplicate orchestration across two layers. That would blur ownership and make debugging harder.

Pilot should keep orchestration authoritative at the control-plane layer.
...
Those are Pilot's differentiators. They should not be displaced by runtime abstractions.
...
The permission model is good, but it is low-level. Pilot would still need to answer:
...
- keep Pilot heartbeat/task/workspace/budget logic unchanged
...
- Pilot's Codex adapter is already important and carries repo-specific behavior
...
- Pilot still records logs, summaries, cost metadata, and issue comments normally
...
Even without using `agent-os`, Pilot should consider standardizing adapter/runtime permissions around a vocabulary like:
...
Pilot should evaluate whether some execution workspaces can be backed by:
...
Pilot plugins and adapters should continue moving toward explicit typed tools over ad hoc shell access. `agent-os` confirms that this is the right direction.
...
### Recommendation E: do not import runtime-level workflows into Pilot core

Pilot should not copy `agent-os` cron/workflow/queue concepts into core orchestration yet.
...
- moving Pilot orchestration into agent-os workflows
...
`agent-os` is useful to Pilot as an execution technology reference, not as a product model.

Pilot should treat it the same way it treats sandboxes or agent CLIs:
