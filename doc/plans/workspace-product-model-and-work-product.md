...
Pilot needs to support two very different but equally valid ways of working:
...
Today, Pilot already has the beginnings of this model:
...
- making Pilot feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Pilot users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Pilot deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Pilot into a full code review product
...
This object must be recorded explicitly so that Pilot can:
...
Pilot should treat PRs as a type of work product linked back to:
...
Pilot already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Pilot must be a first-class deployment mode

Pilot cannot assume that it is running on the same machine as the code.

In cloud deployments, Pilot may:
...
- `Runtime service`: a process or service Pilot owns or tracks for a workspace
...
It also lets cloud-hosted Pilot deployments point at codebases and remotes without pretending the Pilot host has direct filesystem access.
...
Without an explicit `execution workspace` record, Pilot has nowhere stable to attach:
...
Pilot needs a single place to show:
...
- Pilot-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Pilot owning the execution host
...
- `Codebase: Web App`
- `Workspace: Shared`
- `Workspace: PIL-447-add-worktree-support`
- `PR: Open`
- `Preview: Healthy`
...
## 1. Pilot host and execution host must be decoupled
...
without the Pilot host itself running local git or local preview processes.
...
Pilot should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Pilot.
...
Pilot should keep workspace automation centered on a small set of hooks:
...
Pilot should provide the equivalent visibility in the project `Code` surface:
...
Pilot should treat execution workspaces as the natural home for:
...
Pilot should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Pilot should continue treating cleanup policy as part of the core workspace model:
...
Pilot should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Pilot host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Pilot should borrow Worktrunk's good execution patterns while keeping the broader Pilot model:
...
That keeps Pilot focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Pilot deployments remain first-class

That is a better fit for Pilot than either extreme:
