...
This document defines the product and UI plan for skill management in Pilot.
...
- Pilot company import/export can include skills as package content
...
3. The UI must always tell the truth about what Pilot knows:
   - desired state in Pilot
...
Pilot should treat skills at two scopes:
...
- created locally inside Pilot later
...
7. “Understand what Pilot knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Pilot-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Pilot did not install.

These should be clearly marked:

- external
- not managed by Pilot
...
- `desired_only`: Pilot wants it, adapter does not show it yet
- `external`: adapter has it but Pilot does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Pilot only tracks desired state
...
- Pilot can still track desired skills
...
- `.paperclip.yaml` may add Pilot-specific fidelity, but should not replace the base shortname association model
...
- `.paperclip.yaml` may add Pilot-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Pilot should have a company-scoped skill model or managed package model representing:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Pilot-specific.
...
That gives Pilot one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
