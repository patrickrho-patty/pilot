...
Add a single storage subsystem for Pilot that supports:
...
- API serves attachment bytes through authenticated Pilot endpoints.
...
- `pilotai onboard` writes a valid `storage` config block by default.
- `pilotai configure --section storage` can switch between local and s3 modes.
...
- `pilotai doctor` reports actionable storage status.
