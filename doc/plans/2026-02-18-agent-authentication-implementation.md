...
- New config/env settings:
  ...
  - `PILOT_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `PILOT_AGENT_JWT_ISSUER` (default: `pilot`)
  - `PILOT_AGENT_JWT_AUDIENCE` (default: `pilot-api`)
...
1. In:
   ...
   inject `PILOT_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `PILOT_API_KEY`, do not overwrite it.
- Continue injecting:
  - `PILOT_AGENT_ID`
  - `PILOT_COMPANY_ID`
  - `PILOT_API_URL`
...
- Local adapters authenticate without manual `PILOT_API_KEY` config.
