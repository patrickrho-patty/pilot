...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Pilot plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Pilot.
...
- plugin id: `pilot.kitchen-sink-example` or `pilot-kitchen-sink-example`

Recommendation: use `pilot-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Pilot event such as `issue.created`
...
Recommendation: default to a Pilot-local endpoint or a stable public echo endpoint to avoid flaky docs.
