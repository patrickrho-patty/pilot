...
`@pilotai/hermes-pilot-adapter` package to override or shadow a built-in
...
- `GET /api/skills/pilot`

For CLI-driven setup, create and inspect the invite directly:

```sh
npx pilotai invite create --company-id <company-id> --payload-json '{"requestType":"agent"}'
...
- CLI invite flow: `npx pilotai invite create`, `invite show`,
...
- Adapter development override: Adapter manager can install
  `@pilotai/hermes-pilot-adapter` as an external override, but normal
