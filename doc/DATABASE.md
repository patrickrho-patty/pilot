...
1. Creates a `~/.pilot/instances/default/db/` directory for storage
2. Ensures the `pilot` database exists
3. Runs migrations automatically for empty databases
4. Starts serving requests

Data persists across restarts in `~/.pilot/instances/default/db/`. To reset local dev data, delete that directory.
...
Docker note: the Docker quickstart image also uses embedded PostgreSQL by default. Persist `/pilot` to keep DB state across container restarts (see `doc/DOCKER.md`).
...
# DATABASE_URL=postgres://pilot:pilot@localhost:5432/pilot
```
...
| `DATABASE_URL` | Mode |
|---|---|
| Not set | Embedded PostgreSQL (`~/.pilot/instances/default/db/`) |
...
- Default key file: `~/.pilot/instances/default/secrets/master.key` (auto-created if missing).
- CLI config location: `~/.pilot/instances/default/config.json` under `secrets.localEncrypted.keyFilePath`.
