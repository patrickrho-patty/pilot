...
- **Execution policies** — Issues can carry a review/approval execution policy with multi-stage signoff workflows. Reviewers and approvers are selected per-stage, and Pilot routes the issue through each stage automatically. ([#3222](https://github.com/paperclipai/paperclip/pull/3222))
...
- **Standalone MCP server** — New `@paperclipai/mcp-server` package exposing the Pilot API as an MCP tool server, including approval creation. ([#2435](https://github.com/paperclipai/paperclip/pull/2435))
...
- **Agent auth JWT** — Fixed agent auth to fall back to `BETTER_AUTH_SECRET` when `PILOT_AGENT_JWT_SECRET` is absent. ([#2866](https://github.com/paperclipai/paperclip/pull/2866), @ergonaworks)
