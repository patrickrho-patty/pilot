...
- **Telemetry** — Anonymized App-side telemetry. Disable with `DO_NOT_TRACK=1` or `PILOT_TELEMETRY_DISABLED=1` ([#2527](https://github.com/paperclipai/paperclip/pull/2527))
...
- **Routines skill** — New `pilot-routines` skill with documentation moved into Pilot references. Routine runs now support workspace awareness and variables. ([#2414](https://github.com/paperclipai/paperclip/pull/2414), @aronprins)
...
- **Commit metrics** — New Pilot commit metrics script with filtered exports and edge case handling.
...
- **Hermes adapter upgrade** — Upgraded hermes-pilot-adapter with UI adapter and skills support, plus detectModel improvements.
