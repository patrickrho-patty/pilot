...
- Scaffold the plugin into a directory outside the Pilot checkout (e.g. `~/dev/pilot-plugins/<name>`).
...
```bash
pilotai plugin init @acme/my-plugin --output ~/dev/pilot-plugins
```
...
- `--sdk-path <path>` — snapshot the local SDK from a Pilot checkout into `.pilot-sdk/` (useful when developing against an unreleased SDK).
...
```bash
pnpm --filter @pilotai/create-pilot-plugin build
node packages/plugins/create-pilot-plugin/dist/index.js @acme/my-plugin \
