...
function runtime(options, { requireApi = true } = {}) {
  const apiUrl = options.api_url ?? process.env.PILOT_API_URL;
  const apiKey = options.api_key ?? process.env.PILOT_API_KEY;
  return {
    ...
    runId: options.run_id ?? process.env.PILOT_RUN_ID ?? null,
  };
}
...
async function confirm(options) {
  ...
  const drivingIssueId = options.issue_id ?? process.env.PILOT_TASK_ID;
...
async function applyAccepted(options) {
  const data = candidateFile(options);
  const drivingIssueId = options.issue_id ?? process.env.PILOT_TASK_ID ?? null;
