...
test("unselected candidates start unchecked and are labelled as previously declined", () => {
  ...
  const candidates = [
    { issueId: "issue-1", identifier: "PIL-1", title: "Kept before", bucket: "B", lastActivityAt: "2026-05-01T00:00:00.000Z", reason: { message: "Stale." } },
    { issueId: "issue-2", identifier: "PIL-2", title: "New candidate", bucket: "B", lastActivityAt: "2026-05-01T00:00:00.000Z", reason: { message: "Stale." } },
  ];
...
test("decodes the responsible user and normalizes API URLs locally", () => {
  ...
  assert.equal(normalizeApiBase("https://pilot.example/api/"), "https://pilot.example");
});
