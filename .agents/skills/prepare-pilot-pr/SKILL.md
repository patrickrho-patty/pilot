...
green pull request against `pilotai/pilot` master. Apply it once per
...
* Do all PR work in a **git worktree** on a dedicated branch. The main
  `~/pilot` checkout typically runs the live Pilot server — never
...
* Confirm which remote/ref you are targeting (normally `master` on the
  `pilotai/pilot` repo; the task may name a specific remote such as
  `origin` or `public-gh`).
...
* Commit messages must end with exactly:
  `Co-Authored-By: Pilot <noreply@pilot.test>`
...
* Follow `CONTRIBUTING.md` (repo root,
  https://github.com/patty-io/pilot/blob/master/CONTRIBUTING.md) for
