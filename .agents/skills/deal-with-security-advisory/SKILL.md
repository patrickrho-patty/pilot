...
```
gh api repos/pilotai/pilot/security-advisories/{{ghsaId}}
...
```
gh api --method POST \
  repos/pilotai/pilot/security-advisories/{{ghsaId}}/forks
...
```
# Clone the private fork somewhere outside ~/pilot
...
**Do not edit `~/pilot`** — the dev server is running off the `~/pilot` master branch and we don't want to touch it. All work happens in the private fork clone.
...
```
gh api --method POST \
  repos/pilotai/pilot/security-advisories/{{ghsaId}}/cve
...
```
gh api repos/pilotai/pilot/security-advisories/{{ghsaId}} --jq '.credits'
...
```
gh api --method PATCH \
  repos/pilotai/pilot/security-advisories/{{ghsaId}} \
  --input - << 'EOF'
{
  "credits": [
    {
...
### Advisory
https://github.com/patty-io/pilot/security/advisories/{{ghsaId}}
...
gh release view v{{patchedVersion}} --repo pilotai/pilot
...
* The published advisory URL: `https://github.com/patty-io/pilot/security/advisories/{{ghsaId}}`
* The release URL
