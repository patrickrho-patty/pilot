...
Real run: PIL-15684 → PR #10410 (`feat(skills-catalog): add optional
...
key: pilotai/optional/content/simplified-english
recommendedForRoles:
  - engineer
  - product
  - writer
...
- `pnpm --filter @pilotai/skills-catalog build:manifest` → regenerates
  `generated/catalog.json` with the new entry.
- Add `"pilotai/optional/content/simplified-english"` to
  `EXPECTED_OPTIONAL_KEYS` in `src/shipped-catalog.test.ts` (alphabetical).
- `pnpm --filter @pilotai/skills-catalog test` → green.
