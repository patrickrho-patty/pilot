---
name: pilot-dev-workspace-run-verify-fix
description: >
...
worktree service such as `pilot-dev`.
...
- The master checkout `/srv/pilot/home/pilotai/pilot` (the primary
  ...
  under `<master>/.pilot/worktrees/`, and never edit its `.env` while
  repairing a workspace. Managed workspaces run in their own folders, for
  example `~/.pilot-worktrees/instances/<slug>/…`.
- Never trust a `running` / `healthy` runtime row or a passing `/api/health`
  alone. The `service:pilot-dev` port is pinned in service config, so a
...
- runtime workspace command id, usually `service:pilot-dev`
- expected service URL, for example `http://pilot-dev:40631`
...
curl -sS -X POST \
  ...
  --data-binary '{"workspaceCommandId":"service:pilot-dev"}'
```
...
`service:pilot-dev` pins an explicit port in service config, so every
...
```sh
npx pilotai worktree reseed --from-instance default --seed-mode full --yes
```
...
- matching workspace command id, usually `service:pilot-dev`
- `status: "running"`
...
Symptom: the main app sees `pilot-dev` running, but the cloned app copied a
...
- Worktree instance logs live at
  `~/.pilot-worktrees/instances/<slug>/logs`. Read them before guessing at
  login or startup failures.
- Probing the service through a LAN or tailscale IP can return `403` while
  `127.0.0.1` (or the `pilot-dev` hostname) works. Probe loopback first
...
- Browser QA that lands on a blank company path (for example `/FOR/...`)
  ...
  `/PIL/...`) before concluding the reseed failed.
