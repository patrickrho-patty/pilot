...
description: >
  Diagnose stalled, looping, or over-recovered Pilot issue trees and propose
...
1. **Productive work continues.** Agents that have a clear next action must keep working without needing the user to wake them. ([PIL-2674](/PIL/issues/PIL-2674), [PIL-2708](/PIL/issues/PIL-2708))
2. **Only real blockers stop work.** Stops happen when something genuinely cannot proceed (missing approval, missing dependency, human owner). Pseudo-stops (in_review with no action path, cancelled leaves, malformed metadata) must be detected and routed, not left silent. ([PIL-2335](/PIL/issues/PIL-2335), [PIL-2674](/PIL/issues/PIL-2674))
3. **No infinite loops.** Stranded-work recovery and continuation loops must be bounded and distinguishable from genuinely productive continuation. ([PIL-2602](/PIL/issues/PIL-2602), [PIL-2486](/PIL/issues/PIL-2486))
...
- Walk the tree node-by-node and find the exact issue + state combination that stops the world. Common shapes seen in the company so far:
  - `in_review` with no typed execution participant, no active run, no pending interaction, no recovery issue ([PIL-2335](/PIL/issues/PIL-2335), [PIL-2674](/PIL/issues/PIL-2674)).
  - `in_progress` after a successful run with no future action path queued ([PIL-2674](/PIL/issues/PIL-2674)).
  - Blocker chain whose leaf is `cancelled` / malformed / cross-company-inaccessible ([PIL-2602](/PIL/issues/PIL-2602)).
  - `issue.continuation_recovery` waking the same issue >N times after successful runs ([PIL-2602](/PIL/issues/PIL-2602)).
  - Stranded-work recovery treating its own recovery issues as more recoverable source work ([PIL-2486](/PIL/issues/PIL-2486)).
- Quote the evidence: run ids, comment timestamps, status transitions. "Inferred" is acceptable only when an API boundary blocks direct evidence — say so explicitly and mark the claim provisional ([PIL-2631](/PIL/issues/PIL-2631)).

Respect the API boundary. If the linked issue is in another company and your agent token returns 403, do not bypass scoping. Either request a board-approved diagnostic path or proceed from inferred PIL-side evidence and label it.

### 2. Survey recent related work

Before proposing a new product rule, read what already shipped this week in the same area. The user has explicitly called this out: ([PIL-2602](/PIL/issues/PIL-2602)) "review our recent work on liveness that we shipped in the last couple of days." A new rule that contradicts code merged 48 hours ago is rework, not improvement.
...
This is the table the user has asked for repeatedly ([PIL-2335](/PIL/issues/PIL-2335)). Without it the plan is abstract.

### 4. Frame as a general product rule

The user does not want a one-off patch on the named tree. They want the rule. Two checks:

- The rule is **stated as a contract**, not as an if/else patch. Example contract: "every agent-owned non-terminal issue must finish each heartbeat with a terminal state, an explicit waiting path, or an explicit live path" ([PIL-2674](/PIL/issues/PIL-2674)).
...
- Wait for board/CTO acceptance. If the user posts a new comment that supersedes the plan, the prior confirmation is invalidated — open a fresh confirmation tied to the new revision ([PIL-2602](/PIL/issues/PIL-2602) cycled three revisions; that is fine).
...
- Move stalled `in_review` leaves with no participant to `todo` with a precise next action and named owner ([PIL-2335](/PIL/issues/PIL-2335)).
...
- **Recursive recovery.** Stranded-work recovery that recovers its own recovery issues is the canonical infinite loop ([PIL-2486](/PIL/issues/PIL-2486)). Detect it and refuse to deepen.
