...
description: >
  Add a new Pilot issue-thread interaction kind end-to-end. Use when repo
...
Developer/maintainer skill. Do NOT install this on production Pilot agents.

This skill walks a Pilot contributor through introducing a new issue-thread
...
human or coding agent making code changes inside `pilotai/pilot`, not
the operational agents that run inside a deployed Pilot company.

Do NOT install this on production Pilot agents. This guide is for repository contributors changing Pilot itself.
...
- Changing how Pilot agents *call* interactions. Update `skills/pilot`
...
| Helpers/Docs | CLI command, MCP tool, plugin SDK type+host+testing path, `skills/pilot` guidance.            |
...
[PIL-10415](/PIL/issues/PIL-10415#document-plan). Use it as the template for
your own plan document if you are running this work through Pilot itself.
...
- `skills/pilot/SKILL.md` — add a row to the interaction-kinds table:
  *when to use*, *when not to use*, plus a copyable payload example.
- `skills/pilot/references/api-reference.md` — full payload and result
...
- [ ] CLI, MCP, and plugin SDK helpers all accept the new payload shape and
      have test coverage.
- [ ] `skills/pilot/SKILL.md` and `references/api-reference.md` updated.
