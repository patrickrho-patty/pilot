...
test("extracts only pull requests from the requested repository", () => {
  assert.equal(extractPullRequestNumber("https://github.com/pilotai/pilot/pull/9507", "pilotai/pilot"), 9507);
  assert.equal(extractPullRequestNumber("github.com/pilotai/pilot/pull/9507", "pilotai/pilot"), 9507);
  assert.equal(extractPullRequestNumber("https://github.com/other/repo/pull/9507", "pilotai/pilot"), null);
});
...
test("origin selection prioritizes work products then comment mentions", () => {
  const issues = [
    ...
    },
    {
      ...
      workProducts: [{ type: "pull_request", url: "http://github.com/pilotai/pilot/pull/9507?source=pilot#review" }],
    },
  ];
  assert.equal(chooseOriginatingIssue(issues, "https://github.com/pilotai/pilot/pull/9507").issueId, "origin");
});
        ...
        results: [
          {
            ...
            matches: [
              { value: "https://github.com/pilotai/pilot/pull/1", field: "comment", label: "Comment", source: { type: "comment", commentId: "c1" } },
              { value: "https://github.com/pilotai/pilot/pull/1", field: "document_body", label: "Document", source: { type: "document", documentId: "d1", documentKey: "plan" } },
              { value: "https://github.com/pilotai/pilot/pull/2", field: "description", label: "Description", source: { type: "issue", issueId: "issue-1" } },
              { value: "https://github.com/pilotai/pilot/pull/3", field: "description", label: "Description", source: { type: "issue", issueId: "issue-1" } },
              { value: "https://github.com/pilotai/pilot/pull/4", field: "comment", label: "Comment", source: { type: "comment", commentId: "c2" } },
            ],
          },
        ],
      };
    }
    return [{ type: "pull_request", url: "https://github.com/pilotai/pilot/pull/1/" }];
  };
  const ghJson = (args) => {
    ...
    return {
      number,
      url: `https://github.com/pilotai/pilot/pull/${number}`,
      title: `PR ${number}`,
...
test("candidate discovery deduplicates mentions, drops closed PRs, and excludes community authors by default", async () => {
  ...
  const query = new URL(`http://pilot.test${fixture.extractPath()}`).searchParams;
...
test("capped match sets are recorded instead of aborting discovery", async () => {
  ...
  assert.deepEqual(result.source.truncatedIssues.map((issue) => issue.identifier), ["PIL-1"]);
});
...
test("drafts are report-only and missing Greptile blocks normal PRs", () => {
  const base = {
    ...
    behindBy: 0,
    originatingIssue: { status: "done", identifier: "PIL-1" },
  };
...
test("renders scope, purpose, confidence groups, and immutable guardrail", () => {
  const entry = {
    number: 1,
    url: "https://github.com/pilotai/pilot/pull/1",
    title: "Example",
    author: "cryppadotta",
    ...
    confidence: "high",
    headSha: "abc",
    originatingIssue: { identifier: "PIL-1", status: "done" },
    checks: { checks: [{}], pending: [], failing: [] },
...
test("escapes contributor-controlled Markdown in report titles and purposes", () => {
  const report = renderReport({
    ...
    pullRequests: [
      {
        number: 2,
        url: "https://github.com/pilotai/pilot/pull/2",
