...
`pilot-page` publishes static page directories to a Pilot-controlled S3
bucket served through CloudFront. It is the durable Pilot-owned replacement
...
- Public content only.
- Dedicated uploader IAM identity, separate from Pilot attachment storage.
...
```bash
.agents/skills/pilot-page/scripts/publish.sh ./site --slug demo --dry-run
```

Publish:

```bash
.agents/skills/pilot-page/scripts/publish.sh ./site --slug demo
```

Update an existing page from the same source directory:

```bash
.agents/skills/pilot-page/scripts/publish.sh ./site --slug demo --update
```
...
- `.pilot-page/state.json` is allowed and excluded from uploads.
...
export PILOT_PAGE_BUCKET=pilot-pages-prod
export PILOT_PAGE_BASE_URL=https://pages.pilot.test
export PILOT_PAGE_AWS_ACCESS_KEY_ID=...
export PILOT_PAGE_AWS_SECRET_ACCESS_KEY=...
```

Optional:

```bash
export PILOT_PAGE_DEFAULT_PREFIX=""
export PILOT_PAGE_AWS_PROFILE=pilot-page-uploader
export PILOT_PAGE_AWS_SESSION_TOKEN=...  # only with the namespaced key pair
```

Credential resolution order inside `publish.sh`:

1. `PILOT_PAGE_AWS_ACCESS_KEY_ID` + `PILOT_PAGE_AWS_SECRET_ACCESS_KEY`
   (scoped to the helper's `aws` calls; the surrounding process identity is
   untouched)
2. `PILOT_PAGE_AWS_PROFILE`, passed to `aws` as `--profile` (ambient
   `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` / `AWS_SESSION_TOKEN` /
   `AWS_PROFILE` are stripped from the helper's `aws` calls so the named
   profile always wins)
3. The ambient AWS credential chain

Setting both the namespaced key pair and `PILOT_PAGE_AWS_PROFILE` is an
error.

Recommended Pilot secret names:

- `pilot-page-aws-access-key-id`
- `pilot-page-aws-secret-access-key`

Bind those secrets into publisher agents as `PILOT_PAGE_AWS_ACCESS_KEY_ID`
and `PILOT_PAGE_AWS_SECRET_ACCESS_KEY`. Never bind them as the global
...
use, other buckets). Do not reuse Pilot's internal S3 attachment/object
storage credentials.
...
```bash
export AWS_PROFILE=pilot-admin
export AWS_REGION=us-east-1
export BUCKET=pilot-pages-prod
export DOMAIN=pages.pilot.test
export UPLOADER_USER=pilot-page-uploader
export CLOUDFRONT_COMMENT="Pilot pages"
...
```bash
mkdir -p /tmp/pilot-pages-bootstrap
printf '<!doctype html><title>Not found</title><h1>Not found</h1>\n' \
  > /tmp/pilot-pages-bootstrap/404.html

aws s3 cp /tmp/pilot-pages-bootstrap/404.html "s3://$BUCKET/404.html" \
  --profile "$AWS_PROFILE" \
...
aws acm request-certificate \
  ...
  --idempotency-token pilotpages \
  > /tmp/pilot-pages-acm.json

export CERT_ARN="$(jq -r '.CertificateArn' /tmp/pilot-pages-acm.json)"
...
aws cloudfront create-origin-access-control \
  ...
  --origin-access-control-config "{
    \"Name\":\"pilot-pages-oac\",
    \"Description\":\"OAC for $BUCKET\",
    \"SigningProtocol\":\"sigv4\",
    \"SigningBehavior\":\"always\",
    \"OriginAccessControlOriginType\":\"s3\"
  }" \
  > /tmp/pilot-pages-oac.json

export OAC_ID="$(jq -r '.OriginAccessControl.Id' /tmp/pilot-pages-oac.json)"
```
...
```bash
cat > pilot-pages-index-router.js <<'EOF'
...
aws cloudfront create-function \
  --profile "$AWS_PROFILE" \
  --name pilot-pages-index-router \
  --function-config 'Comment=Rewrite clean page URLs to index.html,Runtime=cloudfront-js-2.0' \
  --function-code fileb://pilot-pages-index-router.js \
  > /tmp/pilot-pages-function.json

export FUNCTION_ETAG="$(jq -r '.ETag' /tmp/pilot-pages-function.json)"

aws cloudfront publish-function \
  --profile "$AWS_PROFILE" \
  --name pilot-pages-index-router \
  --if-match "$FUNCTION_ETAG" \
  > /tmp/pilot-pages-function-live.json

export FUNCTION_ARN="$(jq -r '.FunctionSummary.FunctionMetadata.FunctionARN' /tmp/pilot-pages-function-live.json)"
```

Create `cloudfront-config.json`:

```bash
export CALLER_REFERENCE="pilot-pages-$(date +%s)"
...
aws cloudfront create-distribution \
  ...
  > /tmp/pilot-pages-cloudfront.json

export DISTRIBUTION_ID="$(jq -r '.Distribution.Id' /tmp/pilot-pages-cloudfront.json)"
export DISTRIBUTION_DOMAIN="$(jq -r '.Distribution.DomainName' /tmp/pilot-pages-cloudfront.json)"
```
...
Create `pilot-page-uploader-policy.json`. This policy supports collision
...
jq -n \
  ...
  }' > pilot-page-uploader-policy.json
```
...
aws iam put-user-policy \
  --profile "$AWS_PROFILE" \
  --user-name "$UPLOADER_USER" \
  --policy-name PilotPagePublisher \
  --policy-document file://pilot-page-uploader-policy.json
```
...
aws iam create-access-key \
  --profile "$AWS_PROFILE" \
  --user-name "$UPLOADER_USER" \
  > /tmp/pilot-page-uploader-key.json
chmod 600 /tmp/pilot-page-uploader-key.json
```
...
```bash
export CF_ZONE_ID=<pilot.test-zone-id>
...
```bash
export PILOT_PAGE_AWS_ACCESS_KEY_ID="$(jq -r '.AccessKey.AccessKeyId' /tmp/pilot-page-uploader-key.json)"
export PILOT_PAGE_AWS_SECRET_ACCESS_KEY="$(jq -r '.AccessKey.SecretAccessKey' /tmp/pilot-page-uploader-key.json)"

npx pilotai secrets create \
  --company-id <company-id> \
  --name pilot-page-aws-access-key-id \
  --value-env PILOT_PAGE_AWS_ACCESS_KEY_ID

npx pilotai secrets create \
  --company-id <company-id> \
  --name pilot-page-aws-secret-access-key \
  --value-env PILOT_PAGE_AWS_SECRET_ACCESS_KEY
```
...
```json
{
  "PILOT_PAGE_AWS_ACCESS_KEY_ID": {
    "type": "secret_ref",
    "secretId": "<access-key-secret-id>",
    "version": "latest"
  },
  "PILOT_PAGE_AWS_SECRET_ACCESS_KEY": {
    "type": "secret_ref",
    "secretId": "<secret-key-secret-id>",
    "version": "latest"
  },
  "AWS_REGION": { "type": "plain", "value": "us-east-1" },
  "PILOT_PAGE_BUCKET": { "type": "plain", "value": "pilot-pages-prod" },
  "PILOT_PAGE_BASE_URL": { "type": "plain", "value": "https://pages.pilot.test" },
  "PILOT_PAGE_DEFAULT_PREFIX": { "type": "plain", "value": "" }
}
```
...
```bash
npx pilotai skills create \
  --company-id <company-id> \
  --name "Pilot Page" \
  --slug pilot-page \
  --description "Publish static pages to the Pilot pages host" \
  --body-file .agents/skills/pilot-page/SKILL.md
```

Attach it to an agent:

```bash
npx pilotai skills agent sync <agent-id-or-shortname> \
  --company-id <company-id> \
  --skill pilot-page
```
...
aws iam create-access-key \
  --profile "$AWS_PROFILE" \
  --user-name "$UPLOADER_USER" \
  > /tmp/pilot-page-uploader-key-rotation.json
chmod 600 /tmp/pilot-page-uploader-key-rotation.json
```

2. Store new secret versions in Pilot Secrets.
