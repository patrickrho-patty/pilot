...
Publishes a static directory with a root index.html to the configured Pilot
pages bucket and prints the public URL and S3 prefix.

Required environment for live publish:
  PILOT_PAGE_BUCKET, PILOT_PAGE_BASE_URL, AWS_REGION, AWS credentials

Optional environment:
  PILOT_PAGE_DEFAULT_PREFIX, PILOT_PAGE_AWS_PROFILE
  PILOT_PAGE_AWS_ACCESS_KEY_ID, PILOT_PAGE_AWS_SECRET_ACCESS_KEY,
  PILOT_PAGE_AWS_SESSION_TOKEN
...
Credential resolution for aws calls made by this helper:
  ...
  2. PILOT_PAGE_AWS_PROFILE (passed as --profile; ambient AWS_* identity
...
normalize_base_url() {
  local value="$1"
  value="${value%/}"
  [[ "$value" == https://* ]] || die "PILOT_PAGE_BASE_URL must be an https URL"
  [[ ! "$value" =~ [[:space:]] ]] || die "PILOT_PAGE_BASE_URL cannot contain whitespace"
  printf '%s\n' "$value"
}
...
normalize_default_prefix() {
  ...
  raw="${raw#/}"
  raw="${raw%/}"
  [[ "$raw" != *"//"* ]] || die "PILOT_PAGE_DEFAULT_PREFIX cannot contain empty path segments"
  if [[ -z "$raw" ]]; then
    printf '\n'
    return
  fi
...
assert_safe_source_tree() {
  ...
  found="$(
    cd "$source_dir"
    find . -mindepth 1 \
      \( -path './.pilot-page' -o -path './.pilot-page/*' \) -prune -o \
...
assert_update_ownership() {
  ...
  local bucket="$2"
  local prefix="$3"
  local state_file="$source_dir/.pilot-page/state.json"
  local state_bucket
  local state_prefix
...
compute_source_hash() {
  ...
  (
    cd "$source_dir"
    find . -type f ! -path './.pilot-page/*' -print0 |
      LC_ALL=C sort -z |
...
write_state() {
  ...
  local base_url="$6"
  local source_hash="$7"
  local state_dir="$source_dir/.pilot-page"
...
bucket="${PILOT_PAGE_BUCKET:-}"
base_url="${PILOT_PAGE_BASE_URL:-}"
region="${AWS_REGION:-}"
default_prefix="$(normalize_default_prefix "${PILOT_PAGE_DEFAULT_PREFIX:-}")"

[[ -n "$bucket" ]] || die "PILOT_PAGE_BUCKET is required"
[[ "$bucket" =~ ^[a-z0-9][a-z0-9.-]{1,61}[a-z0-9]$ ]] || die "PILOT_PAGE_BUCKET does not look like a valid S3 bucket name"
[[ -n "$base_url" ]] || die "PILOT_PAGE_BASE_URL is required"
base_url="$(normalize_base_url "$base_url")"

page_access_key_id="${PILOT_PAGE_AWS_ACCESS_KEY_ID:-}"
page_secret_access_key="${PILOT_PAGE_AWS_SECRET_ACCESS_KEY:-}"
if [[ -n "$page_access_key_id" || -n "$page_secret_access_key" ]]; then
  [[ -n "$page_access_key_id" && -n "$page_secret_access_key" ]] ||
    die "PILOT_PAGE_AWS_ACCESS_KEY_ID and PILOT_PAGE_AWS_SECRET_ACCESS_KEY must be set together"
  [[ -z "${PILOT_PAGE_AWS_PROFILE:-}" ]] ||
    die "set PILOT_PAGE_AWS_PROFILE or the PILOT_PAGE_AWS_* key pair, not both"
fi
if [[ -n "${PILOT_PAGE_AWS_SESSION_TOKEN:-}" && -z "$page_access_key_id" ]]; then
  die "PILOT_PAGE_AWS_SESSION_TOKEN requires the PILOT_PAGE_AWS_* key pair"
fi
...
if [[ "$dry_run" == "0" ]]; then
  ...
  if [[ -n "$page_access_key_id" ]]; then
    ...
    if [[ -n "${PILOT_PAGE_AWS_SESSION_TOKEN:-}" ]]; then
      aws_env_overrides+=(AWS_SESSION_TOKEN="$PILOT_PAGE_AWS_SESSION_TOKEN")
    fi
  elif [[ -n "${PILOT_PAGE_AWS_PROFILE:-}" ]]; then
    aws_base_args+=(--profile "$PILOT_PAGE_AWS_PROFILE")
...
cat <<EOF
pilot-page published
mode: $mode
url: $url
bucket: $bucket
prefix: $prefix
state: $source_dir/.pilot-page/state.json
EOF
