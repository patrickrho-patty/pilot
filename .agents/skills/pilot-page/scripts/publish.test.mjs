...
function createSite(name = "pilot-page-test") {
  const siteDir = mkdtempSync(join(tmpdir(), `${name}-`));
  tempDirs.add(siteDir);
  writeFileSync(join(siteDir, "index.html"), "<!doctype html><title>Pilot</title>\n");
  return siteDir;
}
...
function writeState(siteDir, state) {
  mkdirSync(join(siteDir, ".pilot-page"), { recursive: true });
  writeFileSync(join(siteDir, ".pilot-page", "state.json"), `${JSON.stringify(state)}\n`);
}
  ...
  try {
    return {
      output: execFileSync("bash", [scriptPath, ...args], {
        ...
        env: {
          ...process.env,
          PILOT_PAGE_BUCKET: "pilot-pages-test",
          PILOT_PAGE_BASE_URL: "https://pages.example.test/",
          ...env,
        },
...
test("dry run validates and prints the planned target without requiring AWS", () => {
  ...
  assert.match(result.output, /^pilot-page dry run$/m);
  assert.match(result.output, /^mode: publish$/m);
  assert.match(result.output, /^bucket: pilot-pages-test$/m);
...
test("dry run normalizes a safe default prefix", () => {
  const result = runPublish(
    ...
    { PILOT_PAGE_DEFAULT_PREFIX: "/reports/launches/" },
  );
...
test("namespaced page keys require both halves of the pair", () => {
  const result = runPublish(
    [createSite(), "--slug", "demo-page", "--dry-run"],
    { PILOT_PAGE_AWS_ACCESS_KEY_ID: "AKIAPAGEUPLOADER" },
  );
...
test("namespaced session token requires the namespaced key pair", () => {
  const result = runPublish(
    [createSite(), "--slug", "demo-page", "--dry-run"],
    { PILOT_PAGE_AWS_SESSION_TOKEN: "page-session-token" },
  );

  assert.notEqual(result.status, 0);
  assert.match(result.output, /requires the PILOT_PAGE_AWS_\* key pair/);
});

test("namespaced page keys conflict with PILOT_PAGE_AWS_PROFILE", () => {
  const result = runPublish(
    [createSite(), "--slug", "demo-page", "--dry-run"],
    {
      PILOT_PAGE_AWS_ACCESS_KEY_ID: "AKIAPAGEUPLOADER",
      PILOT_PAGE_AWS_SECRET_ACCESS_KEY: "page-secret",
      PILOT_PAGE_AWS_PROFILE: "pilot-page-uploader",
    },
  );
...
test("namespaced page keys are scoped to the helper's aws calls", () => {
  const siteDir = createSite();
  const binDir = mkdtempSync(join(tmpdir(), "pilot-page-bin-"));
  tempDirs.add(binDir);
...
`,
  ...
  const result = runPublish([siteDir, "--slug", "demo-page"], {
    ...
    PATH: `${binDir}:${process.env.PATH}`,
    PILOT_PAGE_AWS_ACCESS_KEY_ID: "AKIAPAGEUPLOADER",
    PILOT_PAGE_AWS_SECRET_ACCESS_KEY: "page-secret-argv-canary",
    PILOT_PAGE_AWS_SESSION_TOKEN: "page-session-argv-canary",
  });
...
test("PILOT_PAGE_AWS_PROFILE strips ambient static credentials", () => {
  const siteDir = createSite();
  const binDir = mkdtempSync(join(tmpdir(), "pilot-page-bin-"));
  tempDirs.add(binDir);
  const envDump = join(binDir, "aws-env.txt");
...
`,
  ...
  const seen = readFileSync(envDump, "utf8");
  assert.match(seen, /^PROFILE_ARG=pilot-page-uploader$/m);
...
`,
  ...
  assert.equal(existsSync(join(siteDir, ".pilot-page", "state.json")), true);

  const state = JSON.parse(readFileSync(join(siteDir, ".pilot-page", "state.json"), "utf8"));
  assert.equal(state.bucket, "pilot-pages-test");
