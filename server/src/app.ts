...
import { derivePilotViteHmrPort, type DeploymentExposure, type DeploymentMode } from "@pilotai/shared";
...
import { createHostClientHandlers } from "@pilotai/plugin-sdk";
...
export function resolveViteHmrProtocol(value: string | undefined): "ws" | "wss" | undefined {
  ...
  throw new Error("PILOT_VITE_HMR_PROTOCOL must be ws or wss");
}
...
export async function createApp(
  ...
  opts: {
    ...
    /**
     * `plugins.autoInstall` from the managed config (PILOT_MANAGED_CONFIG).
  ...
  if (opts.uiMode === "static") {
    ...
    if (uiDist) {
      ...
      console.warn("[pilot] UI dist not found; running in API-only mode");
    }
  }
  ...
  //
  // Managed instances (`plugins.autoInstall` from PILOT_MANAGED_CONFIG)
  ...
  app.locals.pilotShutdown = shutdownAppServices;
