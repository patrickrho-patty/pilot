...
import { normalizeAgentUrlKey } from "@pilotai/shared";
import { parseFrontmatterMarkdown } from "@pilotai/shared/frontmatter";
...
async function statCatalogManifest() {
  ...
  throw new Error(
    `Teams catalog manifest not found. Checked: ${catalogPackageRootCandidates.map((root) => path.join(root, "generated/catalog.json")).join(", ")}. Run pnpm --filter @pilotai/teams-catalog build:manifest.`,
  );
}
...
/**
 * Extract `metadata.pilot.catalogTeam` provenance written by the team
...
export function readCatalogTeamProvenance(
  ...
  const pilot = isPlainRecord(metadata.pilot) ? metadata.pilot : null;
...
export function teamsCatalogService(db: Db) {
  ...
  async function prepareCatalogTeamSource(
    ...
    const existingExtension =
      typeof files[".pilot.yaml"] === "string"
        ? parseYamlDocument(files[".pilot.yaml"])
        : {};
    const generatedExtension = parseYamlDocument(await renderCatalogProvenanceYaml(team, targetManager));
    files[".pilot.yaml"] = renderYamlFile(mergePlainRecords(existingExtension, generatedExtension));
  ...
  async function installCatalogTeam(
    ...
    const warnings = [
      ...
      ...(defaultedAdapterSlugs.length > 0
        ? [
            `Catalog agents without explicit overrides (${defaultedAdapterSlugs.join(", ")}) default to ${defaultAdapterType}. Pass adapterOverrides or PILOT_TEAMS_CATALOG_DEFAULT_ADAPTER_TYPE to use a different supported adapter.`,
          ]
        : []),
    ];
