 ...
 * `environments` section of `PILOT_MANAGED_CONFIG` (parsed fail-closed in
...
export async function applyManagedEnvironments(
  ...
  // The forced-execution-mode bootstrap (`PILOT_EXECUTION_MODE=kubernetes`)
  ...
  if (parseExecutionPolicyBootstrapEnv(env)) {
    throw new Error(
      `PILOT_EXECUTION_MODE and the PILOT_MANAGED_CONFIG "environments" section are mutually exclusive: both manage the single instance sandbox environment`,
    );
  }
