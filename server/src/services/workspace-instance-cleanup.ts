...
export function readWorktreeInstanceId(workspacePath: string): string | null {
  const envPath = path.join(path.resolve(workspacePath), ".pilot", ".env");
  let contents: string;
  try {
  ...
  const instanceId = (envEntries.PILOT_INSTANCE_ID ?? envEntries.PILOT_INSTANCE_ID)?.trim();
...
export async function readWorktreeInstancePointer(workspacePath: string): Promise<WorktreeInstancePointer | null> {
  const envPath = path.join(workspacePath, ".pilot", ".env");
  try {
    return {
      envPath,
...
function resolveConfiguredInstanceRoot(pointer: WorktreeInstancePointer, expectedInstanceId?: string):
  ...
  // Env files written before the brand rename carry PILOT_* keys; accept
  // both spellings through the alias window.
  const configuredHome = (env.PILOT_HOME ?? env.PILOT_HOME)?.trim();
  const instanceId = (env.PILOT_INSTANCE_ID ?? env.PILOT_INSTANCE_ID)?.trim();
  ...
  if (!INSTANCE_ID_RE.test(instanceId)) {
    return {
      instanceRoot: null,
      warning: `Refusing worktree instance cleanup from ${pointer.envPath}: PILOT_INSTANCE_ID is not a safe path segment.`,
  ...
  if (!path.isAbsolute(expandedHome)) {
    return {
      instanceRoot: null,
      warning: `Refusing worktree instance cleanup from ${pointer.envPath}: PILOT_HOME is not absolute.`,
  ...
  if (expectedInstanceId && instanceId !== expectedInstanceId) {
    return {
      instanceRoot,
      warning: `Refusing worktree instance cleanup from ${pointer.envPath}: PILOT_INSTANCE_ID "${instanceId}" does not match the expected workspace instance "${expectedInstanceId}".`,
...
function resolveManagedInstancesDir(worktreesDir?: string): string {
  const managedWorktreesDir = path.resolve(
    expandHomePrefix(
      ...
      || path.join(os.homedir(), ".pilot-worktrees"),
    ),
  );
