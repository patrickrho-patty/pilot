 ...
 *    (`~/.pilot/plugins/`) and `node_modules` for packages matching
 *    the `pilot-plugin-*` naming convention. Aggregates results with
...
import { PLUGIN_RPC_ERROR_CODES } from "@pilotai/plugin-sdk";
import type {
...
const STANDALONE_BUNDLED_PLUGIN_SDK_PACKAGE = "@pilotai/plugin-sdk";
...
/**
 * Naming convention for npm-published Pilot plugins.
 * Packages matching this pattern are considered Pilot plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "pilot-plugin-";
 ...
 * `@pilotai/plugin-kubernetes`) are allowed to read from the
 ...
 * runs in-cluster (e.g. `@pilotai/plugin-kubernetes` with inCluster=true)
 ...
 * (`packagePath` null — the `@pilotai` scope is project-controlled at
...
> = {
  "@pilotai/plugin-daytona": { driverKey: "daytona", envVars: ["DAYTONA_API_KEY"] },
  "@pilotai/plugin-e2b": { driverKey: "e2b", envVars: ["E2B_API_KEY"] },
  "@pilotai/plugin-exe-dev": { driverKey: "exe-dev", envVars: ["EXE_API_KEY"] },
  "@pilotai/plugin-novita-sandbox": { driverKey: "novita", envVars: ["NOVITA_API_KEY"] },
};
...
export type PluginSource =
  | "local-filesystem"  // ~/.pilot/plugins/ local directory
  | "npm"               // npm packages matching pilot-plugin-* convention
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the pilot-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "pilot-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginLoader {
  ...
  /**
   * Discover Pilot plugins installed as npm packages in the current
   * Node.js environment matching the "pilot-plugin-*" naming convention.
   *
   ...
   * the "pilotPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Pilot plugin.
   * Throws if the package is a Pilot plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
...
/**
 * Check whether a package name matches the Pilot plugin naming convention.
 * Accepts both the "pilot-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @pilotai/plugin-*
  if (name.includes("/")) {
...
export function resolveDeclaredPluginEntrypoints(
  ...
  const pilotPlugin = pkgJson["pilotPlugin"];
  if (
    pilotPlugin === null
...
function formatLocalPluginManualBuildHint(
  ...
  const autoBuildDisabled = (options.processEnv ?? process.env)["PILOT_DISABLE_PLUGIN_AUTOBUILD"] === "1"
    ? " Auto-build is disabled by PILOT_DISABLE_PLUGIN_AUTOBUILD=1."
    : "";
 ...
 *
 * The spec defines a "pilotPlugin" key in package.json with a "manifest"
...
function getMinimumHostVersion(manifest: PilotPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumPilotVersion;
}
 ...
 *   packageName: "pilot-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      const manualBuildHint = localPath
        ...
        : "";
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Pilot plugin (no manifest found).${manualBuildHint}`,
      );
    }
  ...
  async function refreshPluginManifestFromPackage(
    ...
    if (!manifest) {
      throw new Error(`Plugin package ${plugin.packageName} no longer exposes a Pilot manifest`);
    }
   ...
   * if the package is not a Pilot plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasPilotPlugin = "pilotPlugin" in pkgJson;
  ...
  async function activatePlugin(
    ...
    try {
      ...
      // (for example @pilotai/shared exports). Run those workers through
