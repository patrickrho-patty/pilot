...
function wakeRequestTargetsIssue(issueId: string) {
  return sql`(
    ...
    or ${agentWakeupRequests.payload} -> '_pilotWakeContext' ->> 'issueId' = ${issueId}
    or ${agentWakeupRequests.payload} -> '_pilotWakeContext' ->> 'taskId' = ${issueId}
  )`;
}
...
const LEGACY_PLUGIN_OPERATION_ORIGIN_KINDS = [
  "plugin:pilotai.content-machine:case",
  "plugin:pilotai.content-machine:evaluation",
  "plugin:pilotai.content-machine:source-sync",
] as const;
    ...
    dbOrTx
      .select({
        ...
        issueId: sql<string | null>`coalesce(
          ...
          ${agentWakeupRequests.payload} -> '_pilotWakeContext' ->> 'issueId',
          ${agentWakeupRequests.payload} -> '_pilotWakeContext' ->> 'taskId'
        )`,
...
export function issueService(db: Db) {
  ...
  async function addStopRelayCommentIfNeeded(
    ...
    const childPrefix = childIdentifier.split("-")[0] || "PIL";
      ...
      if (nodeIds.length > 0) {
        ...
        const wakeTargetIssueIdSql = sql<string>`
          coalesce(
            ...
            wake.payload ->> 'taskId',
            wake.payload -> '_pilotWakeContext' ->> 'issueId',
            wake.payload -> '_pilotWakeContext' ->> 'taskId'
          )
        `;
