...
import { toolPolicyConditionsSchema } from "@pilotai/shared";
...
function evaluatePolicyConditions(
  ...
  if (conditions.trustBoundary) {
    ...
    if (boolCondition(boundary.pilotSelfOnly) === true && ctx.providerType !== "pilot_self") {
      return conditionGroupFail("trustBoundary", "Policy condition requires a Pilot self tool.");
    }
