import type { IssueCommentMetadata, IssueCommentPresentation } from "@pilotai/shared";
import {
  agentLinkRow,
  keyValueRow,
  runLinkRow,
...
export const DEFAULT_STRANDED_RECOVERY_NOTICE_BODY =
  "Pilot could not restore a live execution path for this issue automatically. " +
...
export function buildImmediateExecutionPathRecoveryNoticeSeed(input: {
  ...
  const retryDescription = input.status === "todo"
    ? "Pilot automatically retried dispatch for this assigned `todo` issue during terminal run recovery"
    : "Pilot automatically retried continuation for this assigned `in_progress` issue during terminal run recovery";
  return {
    body:
...
export function buildWorkspaceValidationRecoveryNoticeSeed(): StrandedRecoveryNoticeSeed {
  return {
    body:
      "Pilot stopped before launching the local adapter because the issue workspace failed validation. " +
...
export function buildConfigurationIncompleteRecoveryNoticeSeed(): StrandedRecoveryNoticeSeed {
  return {
    body:
      "Pilot stopped before dispatching the adapter because required secret/env bindings are missing. " +
...
export function buildExecutionReviewParticipantRecoveryNoticeSeed(): StrandedRecoveryNoticeSeed {
  return {
    body:
      "Pilot retried the pending execution-review participant once, but the review stage still has no " +
...
export function buildExecutionReviewParticipantUnavailableNoticeSeed(): StrandedRecoveryNoticeSeed {
  return {
    body:
      "Pilot cannot continue the pending execution-review participant because the participant is not " +
...
export function buildStrandedRecoveryEscalationNotice(input: {
  ...
  const recoveryRows: NoticeMetadataRow[] = [
    ...
    input.recoveryOwner
      ...
      : keyValueRow(
          "Recovery owner",
          "Board escalation - Pilot could not find an invokable manager, creator, or executive owner with budget available",
        ),
    keyValueRow(
      "Next action",
      input.recoveryOwner
