...
import { UNMANAGED_BACKGROUND_TASK_LIVENESS_REASON } from "@pilotai/adapter-utils/server-utils";
...
describe("successful run handoff decision", () => {
  it("queues one normal-model corrective wake to the original agent when a successful run has no disposition", () => {
    ...
    expect(decision.instruction).toContain("You are assigned PIL-1: Finish backend handoff.");
  ...
  it("strips control characters and collapses the issue title to a single line", () => {
    ...
    expect(instruction).toContain("You are assigned PIL-1: Finish backend handoff[31m now.");
  ...
  it("does not queue when a plugin owns the issue's lifecycle", () => {
    expect(decide({ issue: { ...issue, originKind: "plugin:pilot.workflow-engine" } as any })).toEqual({
      kind: "skip",
      reason: "issue lifecycle is owned by a plugin",
    });
    expect(decide({ issue: { ...issue, originKind: "plugin:pilot.workflow-engine:advance" } as any })).toEqual({
      kind: "skip",
      reason: "issue lifecycle is owned by a plugin",
    });
  });
  ...
  describe("isPluginManagedIssueLifecycle", () => {
    it("is true for any plugin: prefixed origin kind", () => {
      expect(isPluginManagedIssueLifecycle({ originKind: "plugin:pilot.workflow-engine" })).toBe(true);
      expect(isPluginManagedIssueLifecycle({ originKind: "plugin:pilot.workflow-engine:advance" })).toBe(true);
    });
    ...
    expect(notice.metadata.sections).toEqual(expect.arrayContaining([
      expect.objectContaining({
        ...
        rows: expect.arrayContaining([
          expect.objectContaining({ type: "issue_link", identifier: "PIL-1" }),
  ...
  it("builds the exhausted system notice with recovery metadata", () => {
    const notice = buildSuccessfulRunHandoffExhaustedNotice({
      ...
      recoveryIssue: {
        ...
        identifier: "PIL-2",
        title: "Recover missing next step PIL-1",
        status: "todo",
      } as any,
