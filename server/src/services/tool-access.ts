...
import { CLASS3_STATIC_LEASE_ALLOWLIST, credentialConfigPath, getAvailableConnectionMethod, getConnectableAppDefinition, isToolConnectionAttentionHealth, recommendedDefaultsForApp } from "@pilotai/shared";
...
const APPROVED_STDIO_TEMPLATES: Record<string, {
  ...
  "pilot.echo-calculator-time": {
    name: "Pilot Echo / Calculator / Time fixture",
    tools: [
      {
        name: "echo",
  ...
  "pilot.synthetic-todo-kv": {
    name: "Pilot Synthetic Todo / KV fixture",
    tools: [
...
const GOOGLE_SHEETS_TEMPLATE_ID = "pilot.google-sheets";
...
const TOOL_EXAMPLES: ToolExampleDefinition[] = [
  {
    ...
    applicationName: "Pilot example: Safe read-only Todo / KV",
    applicationDescription: "Deterministic MCP fixture for first-run tool governance checks.",
    connectionName: "Pilot example: Safe read-only Todo / KV",
    templateId: "pilot.synthetic-todo-kv",
    profileKey: "pilot.examples.safe-read-only-todo-kv.profile",
    profileName: "Example safe read-only tools",
    profileDescription: "Allows only the read-only tools from the Pilot Todo / KV example fixture.",
  },
];
...
/** Activity-log actions that map to a connection lifecycle event on the Activity tab (PIL-11284). */
...
function sanitizeHttpFailure(error: unknown): { status: ToolConnectionHealthStatus; message: string; code: string } {
  if (error instanceof HttpError) {
    ...
    return { status: "error", message: error.message, code: "pilot_error" };
  }
...
export function toolAccessService(db: Db, options: ToolAccessServiceOptions = {}) {
  ...
  async function loadBrokerRunContext(input: { companyId: string; agentId: string; runId: string }) {
    ...
    const pilotIssue = asRecord(snapshot.pilotIssue);
    return {
      run,
  ...
  function buildRuntimeAlerts(input: {
    ...
    return [
      ...
      runtimeAlert({
        ...
        firstResponderAction: "Check upstream MCP health, Pilot runtime capacity, and recent gateway audit failures before retrying workloads.",
        runbookSection,
      }),
      runtimeAlert({
  ...
  async function upsertExampleApplication(
    ...
    const metadata = { ...(existing?.metadata ?? {}), source: "pilot_example", exampleId: definition.id, safeDefault: true };
    if (existing) {
      const [updated] = await db
  ...
  async function upsertExampleConnection(
    ...
    const [created] = await db.insert(toolConnections).values({
      ...
      applicationId,
      name: definition.connectionName,
      uid: connectionUid("pilot", definition.connectionName, connectionId),
  ...
  async function syncExampleProfileEntries(
    ...
    const rows = await db.insert(toolProfileEntries).values(readEntries.map((entry) => ({
      ...
      conditions: { source: "pilot_example" },
    }))).returning();
  ...
  async function upsertExampleProfileBinding(
    ...
    const metadata = { ...(existing?.metadata ?? {}), source: "pilot_example", safeDefault: true };
    if (existing) {
      const [updated] = await db
  ...
  function sampleArguments(toolName: string): Record<string, unknown> {
    ...
    if (toolName === "set_value") return { key: "project", value: "pilot" };
   ...
   * check — the "Replace key" / reconnect flow (M7, PIL-10859). Rotates the
  ...
  /**
   * Build the connection lifecycle timeline for the Activity tab (PIL-11284) by
      ...
      const drafts = Object.entries(mcpServers).map(([name, rawServer]) => {
        ...
        if (typeof server.url === "string" || typeof server.endpoint === "string") {
          ...
          const credentialFields = Object.keys(headers).sort().map((key) => {
            warnings.push(`Header ${key} will be stored as a Pilot secret before activation.`);
            return {
        ...
        if (typeof server.command === "string") {
          warnings.push("Imported stdio commands stay draft-only unless mapped to an approved Pilot template.");
          return {
            name,
