...
import { deriveProjectUrlKey } from "@pilotai/shared";
import type {
...
type IssueSummaryRow = {
  ...
  status: string;
  priority: string;
  /** Who may give the `in_review` verdict; `null`/absent ≡ "anyone" (PIL-16506). */
 ...
 * (PIL-17287). A collapsed queue row offers Accept/Reject long before anything
 ...
 * "PIL-23 — Blocked by PIL-23". The UI renders that as a real dependency, which
...
export function attentionService(db: Db, serviceOptions: AttentionServiceOptions = {}) {
  ...
  return {
    list: async (companyId: string, options: AttentionListOptions = {}): Promise<AttentionFeed> => {
      ...
      // creator-excluding row pulls the roster in too (PIL-17287).
      ...
      for (const review of reviewRows) {
        ...
        // it is resolved in-row with the three review verbs (PIL-16080 §4.4).
