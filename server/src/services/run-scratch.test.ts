...
describe("heartbeat run scratch cleanup", () => {
  ...
  it("preserves pilot-named directories without the ownership marker", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-run-unmarked-"));
    cleanupDirs.add(dir);
  ...
  it("builds explicit scratch env without clobbering configured temp dirs", async () => {
    ...
    expect(result.env.PILOT_RUN_SCRATCH_DIR).toBe(scratch.dir);
    expect(result.env.PILOT_TASK_SCRATCH_DIR).toBe(scratch.dir);
    expect(result.env.PILOT_SCRATCH_DIR).toBe(scratch.dir);
    expect(result.env.PILOT_TMPDIR).toBe(scratch.dir);
