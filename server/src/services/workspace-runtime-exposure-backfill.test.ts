/**
 * Backfill decision coverage for PIL-17158.
 *
...
/** A persisted, running, pre-feature Pilot App dev runtime. */
...
/**
 * PIL-17285 regression coverage for the global startup exposure sweep.
 *
...
describe("decideStaleExposureReclaim", () => {
  ...
  it("reclaims only when the broker still attributes a listener to that exact runtime", () => {
    // Genuine orphan GC — the behaviour PIL-17207 row 7 requires — is preserved.
    expect(
      decideStaleExposureReclaim({
        runtimeId: RUNTIME,
