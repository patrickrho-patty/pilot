...
const DEFAULT_LOCAL_ENVIRONMENT_DESCRIPTION =
  "Default execution environment for Pilot runs on this machine.";
...
export interface KubernetesEnvironmentConfigInput {
   ...
   */
  timeoutMs?: number;
  adapters?: import("@pilotai/shared").AdapterRegistryEntry[];
  [key: string]: unknown;
}
...
const MANAGED_ENVIRONMENT_ARCHIVE_TOKEN_METADATA_KEY = "_pilotManagedArchiveToken";
const MANAGED_ENVIRONMENT_ARCHIVE_TOKEN_DEFAULTS_KEY = "_pilotManagedArchiveToken";
...
export function environmentService(db: Db) {
    ...
    const reconciliation = await db.transaction(
      async (tx): Promise<ManagedSandboxEnvironmentReconcileResult> => {
        ...
        let row = sandboxRows.find(
          (candidate) => (candidate.metadata as Record<string, unknown> | null)?.managedByPilot === true,
  ...
  const archiveManagedSandboxEnvironment = async (
    ...
    const archived = await db.transaction(async (tx) => {
      const existing = await tx
        ...
        .then(
          (rows) => rows.find(
            (row) => (row.metadata as Record<string, unknown> | null)?.managedByPilot === true,
          ) ?? null,
        );
  ...
  return {
    ...
    ensureLocalEnvironment: async (_companyId?: string): Promise<Environment> => {
      ...
      if (isCloudManagedInstance() && existingMetadata.managedByPilot !== true) {
        const adopted = await db
          .update(environments)
          .set({
            metadata: { ...existingMetadata, managedByPilot: true },
            updatedAt: new Date(),
          })
