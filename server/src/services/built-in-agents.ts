...
import { agents, builtInManagedResources, companies, issueThreadInteractions, issues, routines, routineTriggers } from "@pilotai/db";
import { syncRoutineVariablesWithTemplate } from "@pilotai/shared";
import type { Agent, Approval, CompanySkill, PermissionKey, Routine, RoutineTrigger, RoutineVariable } from "@pilotai/shared";
...
const FALLBACK_REFLECTION_COACH_INSTRUCTIONS = [
  "# Reflection Coach",
  "",
  "You are Pilot's built-in Reflection Coach.",
  ...
  "Do not apply changes in the same run. Present a reviewable diff and wait for the required Pilot issue-thread approval before any follow-up applies it.",
  "",
].join("\n");
...
const FALLBACK_REFLECTION_COACH_SKILL = [
  ...
  "key: pilotai/bundled/pilot-operations/reflection-coach",
  "---",
  "",
  "# Reflection Coach",
  "",
...
const FALLBACK_SUMMARIZER_INSTRUCTIONS = [
  "You are Summarizer, a built-in reporting agent at Pilot.",
  "",
  "Turn the current state of a Pilot scope (project, workspaces overview, project workspace, or execution workspace) into a short, honest, human-readable Markdown summary and write it back to that scope's summary slot as a new revision. Use the `summarize-status` skill as your operating procedure.",
  "",
...
export function readBuiltInTextWithFallback(
    ...
    try {
      ...
      if (code !== "ENOENT") {
        ...
        if (!warnedBuiltInTextReadErrors.has(warningKey)) {
          ...
          console.warn(
            "[pilot] Built-in agent asset " + label + " read error on " + candidatePath + ": " + codeLabel,
          );
        }
      }
  ...
  if (!warnedBuiltInTextFallbacks.has(label)) {
    ...
    console.warn(
      `[pilot] Built-in agent asset ${label} was not readable; using bundled fallback text. `
...
const skillsCatalogRoot = resolvePackageRoot("@pilotai/skills-catalog");
...
const REFLECTION_COACH_SKILL = readBuiltInTextWithFallback(
  ...
  [
    path.resolve(
      moduleDir,
      "../../../packages/skills-catalog/catalog/bundled/pilot-operations/reflection-coach/SKILL.md",
    ),
    ...(skillsCatalogRoot
      ? [path.join(skillsCatalogRoot, "catalog/bundled/pilot-operations/reflection-coach/SKILL.md")]
      : []),
  ],
...
const SUMMARIZER_SKILL = readBuiltInTextWithFallback(
  ...
  [
    path.resolve(
      moduleDir,
      "../../../packages/skills-catalog/catalog/bundled/pilot-operations/summarize-status/SKILL.md",
    ),
    ...(skillsCatalogRoot
      ? [path.join(skillsCatalogRoot, "catalog/bundled/pilot-operations/summarize-status/SKILL.md")]
      : []),
  ],
...
const DEFINITIONS = validateBuiltInAgentDefinitions([
  {
    ...
    defaultInstructions:
      "You are Pilot's built-in Briefs agent. Produce concise, sourced operational briefs that help the board understand current company work, risks, and next actions.",
    defaultRole: "general",
  ...
  },
  {
    ...
    bundle: {
      ...
      skill: {
        ...
        canonicalKey: "pilotai/bundled/pilot-operations/reflection-coach",
        files: {
  ...
  },
  {
    ...
    bundle: {
      ...
      skill: {
        ...
        canonicalKey: "pilotai/bundled/pilot-operations/summarize-status",
        files: {
