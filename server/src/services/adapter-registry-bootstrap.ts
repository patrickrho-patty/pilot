 ...
 *
 * One source (`PILOT_ADAPTERS` inline JSON, or `PILOT_ADAPTERS_FILE` a
...
import { adapterRegistrySchema, type AdapterRegistryEntryParsed } from "@pilotai/shared";
...
export function parseAdapterRegistryEnv(
  ...
  const inline = env.PILOT_ADAPTERS?.trim();
  const filePath = env.PILOT_ADAPTERS_FILE?.trim();
  ...
  if (inline) {
    ...
    try {
      ...
      throw new Error(
        `PILOT_ADAPTERS_FILE could not be read at "${filePath}": ${(err as Error).message}`,
      );
    }
  }
  ...
  try {
    ...
    throw new Error(`PILOT_ADAPTERS must be valid JSON: ${(err as Error).message}`);
  }

  const result = adapterRegistrySchema.safeParse(parsed);
  if (!result.success) {
    throw new Error(
      `PILOT_ADAPTERS failed validation: ${result.error.issues
...
export function reconcileAdapterAvailability(
  ...
  if (missing.length > 0) {
    throw new Error(
      `PILOT_ADAPTERS declares adapter type(s) with no installed adapter: ${missing.join(", ")}`,
    );
  }
  ...
  logger.info({ enabled, disabled }, "reconciled adapter availability from PILOT_ADAPTERS");
