/**
 * Protected workspace readiness (PIL-17572).
 *
...
import { authUsers, companies, companyMemberships, issues } from "@pilotai/db";
import type {
  WorkspaceReadiness,
 ...
 *
 * `PILOT_CONFIG` is the authoritative pointer a seeded worktree is started
...
export function resolveWorkspaceSeedMarkerDir(env: NodeJS.ProcessEnv = process.env): string {
  ...
  return path.resolve(process.cwd(), ".pilot");
}
