 ...
 */
import { pluginManifestV1Schema } from "@pilotai/shared";
import type { PilotPluginManifestV1 } from "@pilotai/shared";
import { PLUGIN_API_VERSION } from "@pilotai/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): PilotPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as PilotPluginManifestV1,
        };
      }
