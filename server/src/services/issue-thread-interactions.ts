...
import { trackInteractionCreated, trackInteractionResolved } from "@pilotai/shared/telemetry";
import type {
...
function parseStoredInteractionResult<S extends z.ZodTypeAny>(
  ...
  console.warn(
    `[pilot] Dropping unparseable ${row.kind} interaction result for interaction ${row.id}`,
    parsed.error.issues,
  );
  return null;
}
...
// the real ask, so the stale siblings auto-expire (PIL-437). Mirrors the
  ...
  try {
    ...
    if (!args?.creatorRoleByAgentId) {
      try {
        ...
        console.error("[pilot] Failed to load interaction.resolved creator role", error);
      }
    }
    ...
    console.error("[pilot] Failed to emit interaction.resolved telemetry", error);
  }
}
...
function emitInteractionCreatedTelemetry(args: {
  ...
  try {
    ...
    console.error("[pilot] Failed to emit interaction.created telemetry", error);
  }
}
...
async function emitResolvedInteractionsTelemetry(
  ...
  try {
    ...
    console.error("[pilot] Failed to load interaction.resolved creator roles", error);
  }
    ...
    create: async (
      ...
      try {
        ...
        const result = await db.transaction(async (tx) => {
          ...
          // request_confirmation drafts and ask_user_questions (PIL-437: probe
