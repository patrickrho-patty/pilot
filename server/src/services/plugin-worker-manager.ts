...
import { getActiveStepContext } from "@pilotai/adapter-utils/acpx-engine/startup-timing";
import { CLAUDE_SETUP_TOKEN_COMMAND } from "@pilotai/adapter-claude-local/server";
...
export function createPluginWorkerHandle(
  ...
  function routeExecuteLogNotification(notification: JsonRpcNotification): void {
    const invocationId = readNonEmptyString(
      (notification as { pilotInvocationId?: unknown }).pilotInvocationId,
    );
  ...
  function contextForWorkerMessage(message: JsonRpcRequest | JsonRpcNotification): WorkerHostCallContext {
    const invocationId = readNonEmptyString(
      (message as { pilotInvocationId?: unknown }).pilotInvocationId,
    );
    if (!invocationId) {
