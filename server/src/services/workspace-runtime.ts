...
import type { AdapterRuntimeServiceReport } from "@pilotai/adapter-utils";
import type { Db } from "@pilotai/db";
import { executionWorkspaces, issueComments, issues, projectWorkspaces, workspaceRuntimeServices } from "@pilotai/db";
import {
...
const DEFAULT_TAILSCALE_BROKER_SOCKET = "/run/pilot-tailscale-broker/broker.sock";
...
/**
 * Deployment-level switch for the automatic default (PIL-17158).
 *
...
export type ResolvedRuntimeServiceExposure = {
   ...
   * `default` — the server applied the automatic default (PIL-17158).
   *
...
function discoverWorkspacePackagePaths(rootDir: string): Map<string, string> {
  ...
  const ignoredDirNames = new Set([".git", ".pilot", "dist", "node_modules"]);
...
export function sanitizeRuntimeServiceBaseEnv(baseEnv: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  for (const key of Object.keys(env)) {
    if (key.startsWith("PILOT_") || key.startsWith("PILOT_")) {
      delete env[key];
    }
  }
...
function buildDirtyQuarantineRescueBranch(sourceIssue: ExecutionWorkspaceIssueRef | null) {
  ...
  return sanitizeBranchName(`pilot/rescue/${issueComponent}/${formatUtcBranchTimestamp()}`);
}
...
function explainGitWorktreeBranchIncoherence(input: {
  ...
  if (!input.expectedHeadSha || !input.actualHeadSha) {
    return `Pilot could not determine branch ancestry because the recorded branch "${input.expectedBranchName}" or checked-out branch "${actualBranch}" is missing a resolvable HEAD commit.`;
  }
  if (input.sameHead) {
  ...
  if (input.ancestryVerdict === "diverged") {
    return `The recorded branch "${input.expectedBranchName}" is not an ancestor of the checked-out branch "${actualBranch}", so Pilot cannot prove a forward-only reconciliation.`;
  }
  return `Pilot could not determine whether the checked-out branch "${actualBranch}" is forward of the recorded branch "${input.expectedBranchName}".`;
}
...
export async function ensureGitWorktreeBranchCoherent(input: {
  ...
  if (
    ...
    const reason = evidence.provenance.expectedBranchExists
      ...
      : "Automatic forward reconciliation: the recorded branch no longer exists, so Pilot adopted the clean checked-out branch.";
  ...
  if (
    ...
    return {
      ...
      reconciledForward: false,
      warnings: [
        `${warningPrefix} The checked-out branch contains the recorded branch plus newer commits, so Pilot adopted it for subsequent runs.`,
      ],
    };
  }
  ...
  if (
    ...
    return {
      ...
      reconciledForward: false,
      warnings: [
        `${warningPrefix} The detached HEAD contained the recorded branch plus newer commits, so Pilot moved the recorded branch to that HEAD.`,
      ],
    };
  }
...
function buildWorkspaceCommandEnv(input: {
  ...
  // be arbitrarily old), so emit a legacy PILOT_ twin for every injected
  // PILOT_* key through the alias window.
  for (const key of Object.keys(env)) {
    if (key.startsWith("PILOT_")) {
      const legacy = "PILOT_" + key.slice("PILOT_".length);
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".pilot", "worktrees");
...
async function deleteGitBranchAtVerifiedTip(input: {
  ...
  const detachedGitDir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-branch-delete-"));
 ...
 * pair until the workspace itself is released (PIL-17419).
 */
 ...
 * the host or still mapped to someone else (PIL-17419 regression #3).
 *
...
async function allocateAndReserveExposure(input: {
  ...
  // the range and undo "keep existing runtime ports when safe" (PIL-17158).
...
async function allocateIsolatedWorkspacePort(input: {
  ...
  for (const port of candidates) {
    ...
    // otherwise pick the same port (PIL-17249).
...
type RuntimeServiceHealthProbeInput = {
   ...
   * clone from masquerading as healthy (PIL-17572).
   */
  cwd?: string | null;
  executionWorkspaceId?: string | null;
...
function readWorkspaceSeedOperationEvidence(worktreePath: string): {
  ...
  const manifestPath = path.join(worktreePath, ".pilot", "seed-manifest.json");
  try {
...
export function resolveRuntimeProvisionCommand(input: {
  ...
  const stateDir = path.join(input.workspace.cwd, ".pilot");
...
async function spawnLocalRuntimeService(input: StartLocalRuntimeServiceInput): Promise<LocalRuntimeServiceStart> {
  ...
  // overwrites PILOT_BIND from its own `--bind` argv and deletes
  // PILOT_BIND_HOST — which is exactly how a branch pinned at plain master
  // bound 0.0.0.0 and failed every start (PIL-17256). argv is honoured by every
  ...
  const reservedExposure = exposureConfig
    ...
    : null;
  // Loopback port this start claimed in-process for its own duration (PIL-17249). A bare
  ...
  // both needed for a plain-HTTP loopback workspace too (PIL-17572).
  ...
  if (exposureConfig) {
    ...
    // can be arbitrarily old (PIL-17256): the `--bind loopback` argv
  ...
  const ownershipCheckedPorts = port
    ? exposureConfig
      ? [
          port,
          ...(exposureConfig.includePilotViteHmr ? [deriveViteHmrPort(port)] : []),
        ]
  ...
  const readinessPromise = Promise.race([
    Promise.all([
      waitForRuntimeServiceReadiness({
        ...
        // wrongly bound to the wildcard (PIL-17256).
    ...
    if (record.exposureConfig && record.exposureHandle && record.port) {
      ...
      if (provisioned.status.state !== "ready" || !record.url) {
        ...
        // in the operation log is what made PIL-17254 undiagnosable (PIL-17256).
    ...
    // before it may be published as running/healthy (PIL-17572).
    if (managedWorkspaceIdentity) {
    ...
    const exposureAssignedPorts: number[] =
      exposureConfig && port
        ? [port, ...(exposureConfig.includePilotViteHmr ? [deriveViteHmrPort(port)] : [])]
        : [];
...
async function cleanupRecordExposure(
  ...
  // still protects the pair from another workspace (PIL-17419) — this only
...
export async function reconcilePersistedRuntimeServicesOnStartup(db: Db) {
  ...
  // matters most to PIL-17419 is exactly the one with `exposure.state ===
  ...
  for (const row of rows) {
    // PIL-17419: this row's reserved pair is live, or Serve-mapped, under an
    ...
    if (row.status === "stopped" && row.exposure && row.exposure.state !== "removed") {
      ...
      // (PIL-17285). Corroborate against the broker's live ownership view, which
    ...
    // leave `http://pilot-dev:<port>` as the canonical URL forever. Stopping
  ...
  for (const row of projectWorkspaceRows) {
    ...
    try {
      const refs = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "Pilot", companyId: row.companyId },
        issue: null,
        workspace: {
          baseCwd: row.cwd,
