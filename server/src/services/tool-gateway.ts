...
import type { ToolRunContext } from "@pilotai/plugin-sdk";
import type {
...
const BUILTIN_LOCAL_STDIO_RUNTIME_TEMPLATES: Record<string, Omit<LocalStdioRuntimeTemplate, "templateId">> = {
  "pilot.google-sheets": {
    command: "pilot-google-sheets-mcp-server",
    args: [],
    envKeys: [
  ...
  "pilot.echo-calculator-time": {
    command: null,
    args: [],
    envKeys: [],
  },
  "pilot.synthetic-todo-kv": {
    command: null,
    args: [],
    envKeys: [],
  },
...
const sensitivePassthroughHeaderNames = new Set([
  ...
  "cookie",
  "set-cookie",
  "x-pilot-tool-gateway-token",
]);

function isSensitivePassthroughHeader(name: string) {
  return name.startsWith("x-pilot-")
...
function generateGatewayToken(sessionId: string) {
  return `pilotgt_${sessionId}.${randomBytes(32).toString("base64url")}`;
}

function generateNamedGatewayToken(tokenId: string) {
  return `pilotgw_${tokenId}.${randomBytes(32).toString("base64url")}`;
}
...
function sessionIdFromGatewayToken(token: string) {
  const match = token.match(/^pilotgt_([0-9a-fA-F-]{36})\.[A-Za-z0-9_-]+$/);
  return match?.[1] ?? null;
}

function namedGatewayTokenId(token: string) {
  const match = token.match(/^pilotgw_([0-9a-fA-F-]{36})\.[A-Za-z0-9_-]+$/);
  return match?.[1] ?? null;
}
...
function tokenPrefixFromNamedBearer(token: string) {
  ...
  if (tokenId) return `pilotgw_${tokenId.slice(0, 8)}`;
  return token.startsWith("pilotgw_") ? "pilotgw_malformed" : "unknown";
}
...
function safeClientMetadata(headers: Record<string, string | string[] | undefined> | undefined) {
  const clientName = safeHeaderValue(headers, "x-pilot-client-name", 120)
...
const BUILTIN_TOOLS: ToolGatewayDescriptor[] = [
  ...
  {
    name: "pilot-self:list_my_issues",
    displayName: "List my Pilot issues",
    description: "Pilot self-MCP read fixture that lists the authenticated agent's current issues.",
    parametersSchema: {
      type: "object",
    ...
    pluginId: "pilot-self",
    providerType: "pilot_self",
    risk: "read",
  },
  {
    name: "pilot-self:get_issue_context",
    displayName: "Get issue context",
    description: "Pilot self-MCP read fixture that returns scoped issue context and plan document metadata.",
    parametersSchema: {
      type: "object",
...
const VIRTUAL_SEARCH_TOOLS: ToolGatewayDescriptor = {
  ...
  description: "Search the tools available through this Pilot gateway without loading every target tool into the tool list.",
  parametersSchema: {
    type: "object",
    properties: {
      query: { type: "string" },
  ...
  pluginId: "pilot-gateway",
  providerType: "pilot_virtual",
  risk: "read",
};
...
const VIRTUAL_RUN_TOOL: ToolGatewayDescriptor = {
  ...
  description: "Run a target tool by name after Pilot applies the target tool's profile, policy, approval, and rate-limit checks.",
  parametersSchema: {
    type: "object",
    properties: {
      tool: { type: "string" },
...
export function createToolGatewayService(
  ...
  function pluginTools(): ToolGatewayDescriptor[] {
    return (pluginToolDispatcher?.listToolsForAgent() ?? []).map((tool) => ({
      ...tool,
      providerType: "pilot_plugin" as const,
  ...
  async function requestApprovalForRecordedToolCall(input: {
    ...
    const interaction = await interactions.create(
      { id: input.session.issueId, companyId: input.session.companyId },
      {
        ...
        summary: `${input.tool.name} requires approval before Pilot will execute it.`,
  ...
  async function findToolForSession(session: ToolGatewaySession, toolName: string): Promise<ToolGatewayDescriptor> {
    ...
    const tool = [...allTools(), ...connectedTools, ...virtualTools]
      .filter((candidate) => session.agentId || (candidate.providerType !== "pilot_self" && candidate.providerType !== "pilot_plugin"))
  ...
  async function listToolsForContext(session: ToolGatewaySession): Promise<ToolGatewayDescriptor[]> {
    ...
    const tools = [...allTools(), ...allConnectedTools.filter((tool) => !isOnDemandRemoteTool(tool))].filter(
      (tool) => session.agentId || (tool.providerType !== "pilot_self" && tool.providerType !== "pilot_plugin"),
    );
    const decisions = await Promise.all(tools.map(async (tool) => {
  ...
  async function executeBuiltinTool(session: ToolGatewaySession, tool: ToolGatewayDescriptor, parameters: unknown) {
    ...
    if (tool.name === "pilot-self:list_my_issues") {
      if (!session.agentId) {
        throw new ToolGatewayHttpError(403, "Pilot self tools require an agent-scoped gateway session", "agent_context_required");
      }
      const limit = Math.max(1, Math.min(50, Number(params.limit ?? 10) || 10));
      const rows = await db
        .select({
    ...
    if (tool.name === "pilot-self:get_issue_context") {
      if (!session.agentId) {
        throw new ToolGatewayHttpError(403, "Pilot self tools require an agent-scoped gateway session", "agent_context_required");
      }
  ...
  function metadataHeadersForSession(session: ToolGatewaySession, policy: HeaderPolicyConfig): Record<string, string> {
    ...
    for (const key of policy.metadataHeaders) {
      ...
      if (value) headers[`x-pilot-${key.replace(/_/g, "-")}`] = value;
    }
    return headers;
  }
  ...
  async function callLocalStdioMcp(input: {
    ...
    try {
      await request("initialize", {
        ...
        capabilities: {},
        clientInfo: { name: "pilot-tool-gateway", version: "0.3.1" },
      });
  ...
  async function requestElicitationForRecordedToolCall(input: {
    ...
    await db
      ...
      .set({
        ...
        errorMessage: "Remote MCP tool requested elicitation; Pilot created an issue interaction for the response.",
        updatedAt: now,
      })
  ...
  async function executeRemoteHttpTool(
    ...
    const requestId = `pilot-tool-${randomUUID()}`;
  ...
  async function writeProtocolRateLimitAudit(input: {
    ...
    await writeAudit({
      ...
      details: {
        ...
        gatewayPublicId: input.session.gatewayPublicId ?? null,
        gatewayTokenId: input.session.gatewayTokenId ?? null,
        tokenPrefix: typeof input.session.gatewayTokenId === "string" ? `pilotgw_${input.session.gatewayTokenId.slice(0, 8)}` : null,
    ...
    try {
      ...
      const result = tool.providerType === "mcp_remote_http"
        ...
        : tool.providerType === "mcp_local_stdio"
          ...
          : tool.providerType !== "pilot_plugin"
  ...
  return {
    ...
    async createNamedGatewayToken(input: {
      ...
      const tokenPrefix = `pilotgw_${tokenId.slice(0, 8)}`;
      const now = new Date();
      const [row] = await db
    ...
    async executeTool(input: ExecuteGatewayToolInput) {
      ...
      if (tool.name === "search_tools" && tool.providerType === "pilot_virtual") {
        const argumentValidation = validateToolContent({
          value: requestedParameters,
          direction: "arguments",
          sensitiveMode: "redact",
      ...
      if (tool.name === "run_tool" && tool.providerType === "pilot_virtual") {
      ...
      try {
        ...
        if (tool.providerType === "pilot_plugin" && (!session.agentId || !session.runId)) {
    ...
    async executePluginTool(input: ExecutePluginToolInput) {
      ...
      if (tool.providerType !== "pilot_plugin") {
