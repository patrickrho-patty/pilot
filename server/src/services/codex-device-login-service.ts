...
import { toPublicAdapterAuthSessionStatus } from "@pilotai/shared";
import {
...
export function adapterLoginPromotionLockKey(
  ...
  return `pilot:adapter-login-promotion:${companyId}:${startedByUserId}:${adapterType}`;
}
...
export function sessionCodexHomePath(sessionId: string): string {
  return `/tmp/pilot-adapter-login/${sessionId}`;
}
