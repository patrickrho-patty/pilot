...
import type { IssueWatchdog, IssueWatchdogSummary } from "@pilotai/shared";
...
function issueIdFromWakePayload(payload: unknown) {
  ...
  const nested = parseObject(parsed._pilotWakeContext);
      ...
      db
        ...
        .where(and(
          ...
          or(
            ...
            inArray(sql`${agentWakeupRequests.payload}->'_pilotWakeContext'->>'issueId'`, subtreeIssueIds),
            inArray(sql`${agentWakeupRequests.payload}->'_pilotWakeContext'->>'taskId'`, subtreeIssueIds),
          ),
        )),
      db
        .select({
