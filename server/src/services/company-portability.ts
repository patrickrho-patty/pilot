...
import { sha256HexOfBytes } from "@pilotai/shared/portability-hash";
import {
  readPilotSkillSyncPreference,
  writePilotSkillSyncPreference,
} from "@pilotai/adapter-utils/server-utils";
import { requireOpenCodeModelId } from "@pilotai/adapter-opencode-local/server";
...
// their schemaVersion in the .pilot.yaml extension at 6; undeclared
...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".pilot.yaml" || normalized === ".pilot.yml") return "extension";
...
function readSkillKey(frontmatter: Record<string, unknown>) {
  ...
  const pilot = isPlainRecord(metadata?.pilot) ? metadata?.pilot as Record<string, unknown> : null;
  return normalizeSkillKey(
    asString(frontmatter.key)
    ?? asString(frontmatter.skillKey)
    ?? asString(metadata?.skillKey)
    ?? asString(metadata?.canonicalKey)
    ?? asString(metadata?.pilotSkillKey)
...
function deriveManifestSkillKey(
  ...
  if (sourceKind === "pilot_bundled") {
    return `pilotai/pilot/${slug}`;
  }
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .pilot.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .pilot.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .pilot.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .pilot.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; Pilot will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .pilot.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .pilot.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .pilot.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .pilot.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .pilot.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .pilot.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .pilot.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .pilot.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .pilot.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function findPilotExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".pilot.yaml"] === "string") return ".pilot.yaml";
  if (typeof files[".pilot.yml"] === "string") return ".pilot.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.pilot.yaml") || entry.endsWith("/.pilot.yml")) ?? null;
}
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "pilot_bundled") {
    ...
    return {
      ...
      commit,
      trackingRef: "master",
      url: `https://github.com/pilotai/pilot/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  if (asString(metadata?.sourceKind) === "pilot_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.pilotSkillKey = skill.key;
  metadata.pilot = {
    ...(isPlainRecord(metadata.pilot) ? metadata.pilot : {}),
    skillKey: skill.key,
    slug: skill.slug,
...
function buildManifestFromPackageFiles(
  ...
  if (bundleSchemaVersion > BUNDLE_SCHEMA_VERSION) {
    throw unprocessable(`Company package declares schemaVersion ${bundleSchemaVersion}, which was produced by a newer Pilot; this board reads up to schemaVersion ${BUNDLE_SCHEMA_VERSION}.`);
  }
  ...
  for (const skillPath of skillPaths) {
    ...
    if (sourceKind === "github-dir" || sourceKind === "github-file") {
      ...
      const normalizedSourceKind = owner === "pilotai"
        && repoName === "pilot"
        && canonicalKey?.startsWith("pilotai/pilot/")
        ? "pilot_bundled"
        : "github";
      sourceType = "github";
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
  ...
  async function exportBundle(
    ...
    const pilotExtensionPath = ".pilot.yaml";
    ...
    return {
      ...
      pilotExtensionPath: pilotExtensionPath,
    };
  }
