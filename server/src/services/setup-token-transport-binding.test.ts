...
import type { SetupTokenPtySessionOpener } from "@pilotai/adapter-utils/setup-token-transport";
import {
...
describe("worker-bound live pseudo-terminal opener", () => {
  it("resolves the lease to its provider lease id and drives the worker route gate", async () => {
    ...
    const getLeaseById = vi.fn(async () => ({
      ...
      metadata: { pluginId: "pilot.daytona", provider: "daytona" },
    }));
    ...
    // none.
    expect(openSetupTokenPtySession).toHaveBeenCalledWith("pilot.daytona", {
      driverKey: "daytona",
      companyId: "company-1",
      environmentId: "env-1",
      providerLeaseId: "provider-lease-9",
...
const DIAGNOSTIC_ALLOWLIST = new Set<string>([
  "[pilot] Setup-token login: the process stop step errored.",
  "[pilot] Setup-token login: the driver dispose step errored.",
  "[pilot] Setup-token login: the code input step errored.",
  "[pilot] Setup-token login: sent the browser code to the prompt.",
  "[pilot] Setup-token login: delivered the credential to the sink.",
  "[pilot] Setup-token login: the credential delivery step errored.",
  "[pilot] Setup-token login cancelled before start.",
  "[pilot] Setup-token login timed out; stopping the process.",
  "[pilot] Setup-token login cancelled; stopping the process.",
  "[pilot] Setup-token login command ended with a non-zero exit code.",
  "[pilot] Setup-token login: the credential did not land; treating the run as a failure.",
  "[pilot] Setup-token login: surfaced the sign-in prompt.",
  "[pilot] Setup-token login command ended successfully.",
]);
...
describe("setup-token production transport binding diagnostics", () => {
  it("forwards only allowlisted, non-secret diagnostics on a success path", async () => {
    ...
    expect(captured).toContain("[pilot] Setup-token login: surfaced the sign-in prompt.");
    expect(captured).toContain(
      "[pilot] Setup-token login: delivered the credential to the sink.",
    );
    expect(captured).toContain("[pilot] Setup-token login command ended successfully.");
    // The runner sent the browser code, so the code-input path ran. The code
    // still never reaches a log line.
    expect(captured).toContain("[pilot] Setup-token login: sent the browser code to the prompt.");
