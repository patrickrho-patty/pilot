...
describe("parseExecutionPolicyBootstrapEnv", () => {
  ...
  it("leaves adapters undefined when PILOT_ADAPTERS is absent", () => {
  ...
  it("reads PILOT_K8S_RPC_TIMEOUT_MS into kubernetesConfig.timeoutMs", () => {
    const parsed = parseExecutionPolicyBootstrapEnv(
      env({
        PILOT_EXECUTION_MODE: "kubernetes",
  ...
  it("omits timeoutMs when PILOT_K8S_RPC_TIMEOUT_MS is absent", () => {
    const parsed = parseExecutionPolicyBootstrapEnv(env({ PILOT_EXECUTION_MODE: "kubernetes" }));
    expect(parsed?.kubernetesConfig.timeoutMs).toBeUndefined();
  });

  it("throws when PILOT_K8S_RPC_TIMEOUT_MS is not a positive integer", () => {
    expect(() =>
      parseExecutionPolicyBootstrapEnv(
        env({ PILOT_EXECUTION_MODE: "kubernetes", PILOT_K8S_RPC_TIMEOUT_MS: "0" }),
      ),
    ).toThrow(/PILOT_K8S_RPC_TIMEOUT_MS/);
    expect(() =>
      parseExecutionPolicyBootstrapEnv(
