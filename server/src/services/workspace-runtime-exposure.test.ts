...
function serviceCommand() {
  ...
  // publication requires semantic health, not just a 200 (PIL-17572).
...
/**
 * Fake guest dev runners written to disk (PIL-17256).
 *
 ...
 * argv and **ignores `PILOT_BIND` / `PILOT_MANAGED_RUNTIME_EXPOSURE`
...
beforeAll(async () => {
  guestDir = await fs.mkdtemp(path.join(os.tmpdir(), "pil-17256-guest-"));
 ...
 * range it never actually checked (PIL-17419; PIL-17255 started this with
...
const LEGACY_HTTP_EXPOSE = {
  type: "url",
  urlTemplate: "http://pilot-dev:{{port}}",
} as const;
...
function startInput(options?: {
  ...
  return {
    ...
    actor: { id: null, name: "Pilot", companyId: "aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee" },
    issue: null,
    workspace: {
      baseCwd: process.cwd(),
...
describe.skipIf(process.platform !== "linux")("automatic tailscale_https default for managed worktree runtimes", () => { // Guest binds and diagnoses rely on Linux /proc listener introspection.
  it("defaults a legacy pilot-dev service with no exposure block, relocating its pinned port", async () => {
    const { broker, calls } = createBroker();
    installDeps({ broker });
  ...
  it("preserves a deliberate opt-out and leaves the service on plain HTTP", async () => {
    ...
    expect(runtime.url).toBe(`http://pilot-dev:${runtime.port}`);
  }, 15_000);
  ...
  it("still honors an explicit opt-in when the broker socket is missing, failing loudly", async () => {
    ...
    await expect(
      startRuntimeServicesForWorkspaceControl(startInput({ serviceName: "pilot-dev" })),
    ).rejects.toThrow();
...
/**
 * PIL-17256: the deployed regression. A fresh managed lane on service index 0
 ...
 */
describe.skipIf(process.platform !== "linux")("loopback bind is forced on the guest, not merely requested (PIL-17256)", () => {
  ...
  it("explains rather than only coding the failure, so the next operator can act", async () => {
    ...
    // The bare code alone is what cost PIL-17254 three diagnostic cycles.
  ...
  it("leaves a non-Pilot service's --bind argument alone", async () => {
  ...
  it("does not rewrite a Pilot dev command when the service is not exposed", async () => {
    const { broker, calls } = createBroker();
    installDeps({ broker });
...
describe("readiness probes loopback for an exposed runtime (PIL-17256)", () => {
...
describe.skipIf(process.platform !== "linux")("the deployed failure shape: loopback app port, wildcard HMR (PIL-17256)", () => {
...
describe("recovers when a guest loses its assigned exposure port during startup (PIL-17256)", () => {
