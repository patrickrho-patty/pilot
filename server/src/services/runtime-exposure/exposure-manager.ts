 ...
 * lifecycle (PIL-17049 plan, PIL-17050 verdict). A backend can be running and
 ...
 *  - reconcile touches only Pilot-owned mappings the broker reports; it can
 ...
 */
import type { RuntimeExposureConfigInput, RuntimeExposureStatus } from "@pilotai/shared";
import {
...
export interface ExposureManagerDeps {
   ...
   * returning null when nothing is provably wrong (PIL-17256).
   *
 ...
 * ever returns Pilot-owned listeners, so unknown/manual mappings and the
 ...
 * `listener_ownership_mismatch` unactionable in PIL-17254.
 */
