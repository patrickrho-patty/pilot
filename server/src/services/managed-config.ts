 ...
 * variable, `PILOT_MANAGED_CONFIG`, holding a single JSON document:
 *
 *   {
 *     "v": 1,
 *     "mode": "cloud",
...
/**
 * Parse `PILOT_MANAGED_CONFIG` from a raw env map. Returns null only when
...
export function parseManagedConfigEnv(env: ManagedConfigEnv): ManagedInstanceConfig | null {
  ...
  if (doc.environments !== undefined) {
    ...
    if (doc.environments.length > 1) {
      fail(
        `"environments" supports at most one entry: each entry provisions the single Pilot-managed sandbox environment (DB invariant environments_managed_sandbox_idx)`,
      );
    }
