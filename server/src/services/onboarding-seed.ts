...
import { agents, companyOnboardingSeeds, goals, issues, projects } from "@pilotai/db";
import type { ApplyOnboardingSeed } from "@pilotai/shared";
...
export function onboardingSeedService(db: Db) {
  ...
  async function applyWithin(
    ...
    //
    //    No-first-task contract (PIL-67 r17.4): on the Cloud walk this branch
  ...
  async function apply(
    ...
    const result = await db.transaction(async (tx) => {
      await tx.execute(
        sql`select pg_advisory_xact_lock(hashtextextended(${`pilot:onboarding-seed:${companyId}`}, 0))`,
      );
