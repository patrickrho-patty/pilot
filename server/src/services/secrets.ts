...
import { CLASS3_STATIC_LEASE_ALLOWLIST,
  ...
  updateSecretProviderConfigSchema, MANAGED_MODE_SENTINEL_LEGACY, MANAGED_MODE_SENTINEL_PILOT } from "@pilotai/shared";
...
async function throwProviderWriteOrReservedRowRollbackError(input: {
  ...
  try {
    ...
    throw new HttpError(500, "Secret create failed and Pilot could not roll back the local secret reservation.", {
...
function throwProviderCleanupFailedAfterCreateRollback(input: {
  ...
  const providerConfigId = providerConfigIdentifier(input);
  throw new HttpError(500, "Secret create failed and Pilot could not clean up the remote provider secret.", {
...
function safeRemoteProviderErrorDetails(
  ...
  if (
    ...
    details.credentialPath = "Pilot server runtime/provider credential path";
...
export function secretService(db: Db) {
  ...
  async function resolveSecretValueForAgentAccess(
    ...
    if (context.bindingId) {
      ...
      const manifest = (asRecord(runContext.pilotSecrets) ?? {}).manifest;
    ...
    const effectiveIssueId = context.issueId ?? (
      typeof runContext.issueId === "string"
        ? runContext.issueId
        : typeof (asRecord(runContext.pilotIssue) ?? {}).id === "string"
          ? String((asRecord(runContext.pilotIssue) ?? {}).id)
          : null
    );
