...
import { readPilotSkillSyncPreference } from "@pilotai/adapter-utils/server-utils";
import { claudeConfigDir, parseClaudeStreamJson } from "@pilotai/adapter-claude-local/server";
import { codexHomeDir, parseCodexJsonl } from "@pilotai/adapter-codex-local/server";
import { parseOpenCodeJsonl } from "@pilotai/adapter-opencode-local/server";
import {
...
const FEEDBACK_SCHEMA_VERSION = "pilot-feedback-envelope-v2";
const FEEDBACK_BUNDLE_VERSION = "pilot-feedback-bundle-v2";
const FEEDBACK_PAYLOAD_VERSION = "pilot-feedback-v1";
const FEEDBACK_DESTINATION = "pilot_labs_feedback_v1";
...
function captureStatusFromFiles(files: FeedbackTraceBundleFile[]): FeedbackTraceBundleCaptureStatus {
  ...
  const hasAdapterFiles = files.some((file) =>
    file.source !== "pilot_run" &&
    file.source !== "pilot_run_events" &&
    file.source !== "pilot_run_log",
  );
...
async function buildOpenCodeTraceFiles(input: {
  ...
  const opencodeRoot = resolveHomeAwarePath(
    process.env.PILOT_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode",
  );
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.companyId !== row.companyId) {
      ...
      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.pilotRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "pilot/run-events.json",
        contentType: "application/json",
        source: "pilot_run_events",
        contents: `${JSON.stringify(sanitizedEvents, null, 2)}\n`,
      }));

      if (logText) {
        files.push(makeBundleFile({
          path: "pilot/run-log.ndjson",
          contentType: "application/x-ndjson",
          source: "pilot_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.pilotRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {
