 ...
 * calls. On startup we:
 *   1. Parse `PILOT_EXECUTION_MODE` (+ `PILOT_K8S_*`) from the env.
...
import type { InstanceExecutionMode } from "@pilotai/shared";
...
function parsePositiveIntMs(value: string | undefined): number | undefined {
  ...
  if (!Number.isFinite(parsed) || !Number.isInteger(parsed) || parsed <= 0) {
    throw new Error(
      `PILOT_K8S_RPC_TIMEOUT_MS must be a positive integer of milliseconds (got "${value}").`,
    );
  }
  return parsed;
}
 ...
 * unrestricted (no env, or `PILOT_EXECUTION_MODE=any`). Throws on an
...
export function parseExecutionPolicyBootstrapEnv(
  ...
  if (backend) {
    if (backend !== "job" && backend !== "sandbox-cr") {
      throw new Error(
        `PILOT_K8S_BACKEND must be "job" or "sandbox-cr" (got "${backend}").`,
      );
    }
  ...
  if (egressMode) {
    if (egressMode !== "cilium" && egressMode !== "standard") {
      throw new Error(
        `PILOT_K8S_EGRESS_MODE must be "cilium" or "standard" (got "${egressMode}").`,
      );
    }
