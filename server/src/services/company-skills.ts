...
import { readPilotSkillSyncPreference, writePilotSkillSyncPreference } from "@pilotai/adapter-utils/server-utils";
import type { PilotDesiredSkillEntry, PilotSkillEntry } from "@pilotai/adapter-utils/server-utils";
import type {
...
function assertImportedSkillKeyAllowed(skill: ImportedSkill) {
  if (!skill.key.startsWith("pilotai/pilot/")) return;
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  const sourceKind = asString(metadata?.sourceKind);
  if (sourceKind === "pilot_bundled") return;
  throw unprocessable(
    `Reserved Pilot skill key "${skill.key}" cannot be imported from unbundled sources.`,
    {
      skillKey: skill.key,
...
function resolveBundledSkillReleasesRoot() {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  return [
    path.resolve(moduleDir, "../../skills-releases/pilot"),
    path.resolve(process.cwd(), "skills-releases/pilot"),
    path.resolve(moduleDir, "../../../skills-releases/pilot"),
  ];
}
...
function deriveImportedSkillSource(
  ...
  if (kind === "github-dir" || kind === "github-file") {
    ...
    if (repo && owner && repoName) {
      const sourceKind = owner === "pilotai"
        && repoName === "pilot"
...
function isPilotBundledSkillKey(key: string) {
  return key.startsWith("pilotai/pilot/");
}

function pilotBundledFolderCategory(key: string, metadata?: unknown) {
  const keyParts = key.split("/");
  if (keyParts[0] === "pilotai" && keyParts[1] === "bundled" && keyParts[2]) {
    return keyParts[2];
  }
  if (isPilotBundledSkillKey(key)) return "pilot-core";
  if (isPlainRecord(metadata) && asString(metadata.sourceKind) === "pilot_bundled") {
    return "pilot-core";
  }
  return null;
}
...
function stripDerivedPilotBundledMetadata(key: string, metadata: unknown): unknown {
  ...
  if (out.sourceKind === "pilot_bundled") delete out.sourceKind;
...
function importedSkillMetadataEqual(existing: CompanySkill, values: ImportedSkillPersistValues) {
  ...
  if (isPilotBundledSkillKey(values.key) && asString(incomingMetadata?.sourceKind) === "pilot_bundled") {
...
const BUILT_IN_SKILL_TEST_RUN_TEMPLATE_BODY = [
  ...
  "",
  "Invoke and use the selected skill under test: `{{skillInvocation}}`. Use the pinned skill revision supplied by Pilot as the source of truth, regardless of any other runtime skills.",
  "",
...
function builtInSkillTestRunTemplate(companyId: string): CompanySkillTestRunTemplate {
  return {
    ...
    description: "Pilot's read-only default harness instructions for Skills Studio runs.",
...
function buildMissingRuntimeSourceDetail(skill: Pick<CompanySkill, "name" | "sourceLocator" | "metadata">) {
  ...
  if (sourcePath) {
    return `Company skill "${skill.name}" is in the library, but Pilot cannot find its local source at ${sourcePath}.`;
  }
  return `Company skill "${skill.name}" is in the library, but Pilot cannot find a valid local runtime source for it.`;
}
...
function deriveSkillSourceInfo(skill: SkillSourceInfoTarget): {
  ...
  if (metadata.sourceKind === "pilot_bundled") {
    return {
      editable: false,
      editableReason: "Bundled Pilot skills are read-only.",
      sourceLabel: "Pilot bundled",
      sourceBadge: "pilot",
      sourcePath: null,
    };
  }
...
export function companySkillService(db: Db) {
  ...
  async function assertLocalImportSourceAllowed(companyId: string, source: string) {
    ...
    if (!allowed) {
      throw forbidden("Local skill source is outside approved company workspace roots", {
        ...
        remediation: "Import from a configured Pilot workspace or the company managed-skill directory.",
      });
    }
  }
  ...
  async function ensureBundledSkillReleases(companyId: string, bundledSkills: CompanySkill[]) {
    const pilotSkill = bundledSkills.find((skill) => skill.key === "pilotai/pilot/pilot");
  ...
  async function reconcileLocalPathSkillSources(companyId: string) {
    ...
    for (const skill of skills) {
      if (skill.sourceType !== "local_path") continue;
      if (isPilotBundledSkillKey(skill.key) || asString(skill.metadata?.sourceKind) === "pilot_bundled") continue;
  ...
  async function renameSkill(
    ...
    if (!isPilotManagedRenameTarget(skill)) {
      throw unprocessable(
        "Only Pilot-managed skills can be renamed. Catalog, external, project-scanned, and unmanaged local skills are read-only.",
    ...
    for (const target of scanTargets) {
      ...
      for (const directory of directories) {
        ...
        const existingBundledBySlug = acceptedSkills.find((skill) => (
          ...
          && (isPilotBundledSkillKey(skill.key) || asString(skill.metadata?.sourceKind) === "pilot_bundled")
        )) ?? null;
  ...
  async function installFromCatalog(
    ...
    const values = {
      ...
      authorName: storeMetadata.authorName ?? existingByKey?.authorName ?? "Pilot",
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "pilot_bundled"
        && incomingKind === "github"
        && incomingOwner === "pilotai"
        && incomingRepo === "pilot"
      ) {
        out.push(existing);
        continue;
  ...
  function snapshotAgentConfig(agent: Awaited<ReturnType<typeof agents.getById>>) {
    ...
    return {
      ...
      adapterConfig,
      runtimeConfig,
      assignedSkills: isPlainRecord(adapterConfig.pilotSkillSync)
        ? adapterConfig.pilotSkillSync
        : null,
      instructionsRef:
