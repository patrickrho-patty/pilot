...
export const HEARTBEAT_RUN_SCRATCH_MARKER = ".pilot-run-scratch.json";
...
export async function prepareHeartbeatRunScratch(input: {
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), `pilot-run-${issueSegment}-${runSegment}-`));
...
export async function cleanupHeartbeatRunScratch(input: {
  ...
  if (!isPathInside(tmpRoot, dir) || !path.basename(dir).startsWith("pilot-run-")) {
