...
import { isUuidLike, normalizeAgentApiKeyScope, type DeploymentMode } from "@pilotai/shared";
...
export function actorMiddleware(db: Db, opts: ActorMiddlewareOptions): RequestHandler {
  ...
  return async (req, _res, next) => {
    ...
    const runIdHeader = req.header("x-pilot-run-id");
    ...
    if (!key) {
      ...
      if (normalizedRunIdHeader && normalizedRunIdHeader !== claims.run_id) {
        ...
        next(
          unprocessable("x-pilot-Run-Id does not match signed agent JWT run_id", {
...
export async function resolveCloudTenantActor(
  ...
  const token = req.header("x-pilot-cloud-tenant-token")?.trim();
  if (!token || !constantTimeStringEqual(token, expectedToken)) return null;

  const userId = requiredCloudHeader(req, "x-pilot-cloud-user-id");
  const userEmail = requiredCloudHeader(req, "x-pilot-cloud-user-email").toLowerCase();
  const stackId = requiredCloudHeader(req, "x-pilot-cloud-stack-id");
  const stackRole = stackMembershipRole(req.header("x-pilot-cloud-stack-role"));
  const userName = req.header("x-pilot-cloud-user-name")?.trim() || userEmail;
  const pilotCompanyId = req.header("x-pilot-cloud-pilot-company-id")?.trim();
  const pilotCompanyName = req
    .header("x-pilot-cloud-pilot-company-name")
    ?.trim();
  ...
  if (shouldSync) await db
    ...
    .values({
      id: companyId,
      name: companyName,
      description: `Provisioned by Pilot Cloud for stack ${stackId}.`,
      status: "active",
  ...
  if (shouldSync && pilotCompanyName) {
    await repairCloudTenantCompanyName(db, {
      ...
      pilotCompanyName: pilotCompanyName,
      now,
    });
  }
...
function cloudTenantCompanyId(stackId: string): string {
  const bytes = createHash("sha256").update(`pilot-cloud-tenant-company:${stackId}`).digest();
...
export function isKnownBadCloudCompanyName(
  name: string,
  ids: { companyId: string; pilotCompanyId?: string },
): boolean {
  const normalized = name.trim();
  return (
    /^pilot-stack-.+/i.test(normalized) ||
    /^stack-.+\s+pilot$/i.test(normalized) ||
    normalized === ids.companyId ||
    (ids.pilotCompanyId !== undefined &&
      normalized === ids.pilotCompanyId)
  );
}
  ...
  try {
    ...
    if (
      ...
      !isKnownBadCloudCompanyName(existing.name, {
        companyId: input.companyId,
        pilotCompanyId: input.pilotCompanyId,
      })
    ) {
      return;
    }
    await db.transaction(async (tx) => {
      const [updated] = await tx
        .update(companies)
        .set({ name: input.pilotCompanyName, updatedAt: input.now })
        .where(
          and(
