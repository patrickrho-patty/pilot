...
import { authUsers, companies, companyMemberships, instanceSettings, instanceUserRoles } from "@pilotai/db";
...
const VALID_HEADERS = {
  "x-pilot-cloud-tenant-token": "test-server-token",
  "x-pilot-cloud-user-id": "user-123",
  "x-pilot-cloud-user-email": "Owner@Example.com",
  "x-pilot-cloud-stack-id": "stack-abc",
  "x-pilot-cloud-stack-role": "owner",
};
...
describe("resolveCloudTenantActor (shared-pool hardening)", () => {
  ...
  it("resyncs an A to B to A context transition inside the debounce window", async () => {
    ...
    const contextB = { ...VALID_HEADERS, "x-pilot-cloud-stack-role": "member" };
  ...
  it("resolves identically from a raw upgrade-request header map via the shim", async () => {
    ...
    rawHeaders["x-pilot-cloud-user-name"] = ["Cloud Owner", "ignored-duplicate"];
  ...
  it("maps a non-owner stack role through to the membership without elevating", async () => {
    ...
    const actor = await resolveCloudTenantActor(
      db,
      fakeReq({ ...VALID_HEADERS, "x-pilot-cloud-stack-role": "member" }),
    );
    expect(actor!.isInstanceAdmin).toBe(false);
  ...
  describe("owner instance-admin elevation (enableOwnerInstanceAdmin)", () => {
    ...
    it.each(["member", "admin", "support"] as const)(
      ...
      async (stackRole) => {
        ...
        const actor = await resolveCloudTenantActor(
          db,
          fakeReq({ ...VALID_HEADERS, "x-pilot-cloud-stack-role": stackRole }),
        );
        expect(actor).not.toBeNull();
        expect(actor!.isInstanceAdmin).toBe(false);
      },
