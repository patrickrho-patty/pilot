...
import { trackErrorHandlerCrash } from "@pilotai/shared/telemetry";
...
function getPilotDb(req: Request): Db | null {
  const locals = req.app?.locals as { pilotDb?: Db; db?: Db } | undefined;
  return locals?.pilotDb ?? locals?.db ?? null;
}
