...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Pilot Agent Onboarding");
    ...
    expect(text).toContain("/api/invites/token-123/skills/pilot");
    expect(text).toContain("Suggested Pilot base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("pilotApiUrl");
    ...
    expect(text).toContain("agentDefaultsPayload.pilotApiUrl");
    expect(text).toContain("hermes_local");
    expect(text).toContain("Hermes-originated Pilot API usage");
    ...
    expect(text).toContain("Do not rotate or invent a Pilot key manually");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.pilotApiUrl");
    ...
    expect(text).toContain("Decide which Pilot adapter type matches your runtime.");
  });
  ...
  it("includes LAN candidates when the advertised host is tailnet-only", () => {
    ...
    try {
      const req = buildReq("pilot.example.test:3103");
      const invite = {
        id: "invite-4",
        companyId: "company-1",
        inviteType: "company_join",
      ...
      const text = buildInviteOnboardingTextDocument(req, "token-999", invite as any, {
        ...
        deploymentExposure: "private",
        bindHost: "0.0.0.0",
        allowedHostnames: ["pilot.example.test", "203.0.113.42"],
      });
