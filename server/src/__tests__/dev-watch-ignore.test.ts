...
describe("resolveServerDevWatchIgnorePaths", () => {
  it("includes both the worktree UI paths and their real shared targets", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-dev-watch-"));
    const sharedUiRoot = path.join(tempRoot, "shared-ui");
    const worktreeRoot = path.join(tempRoot, "repo", ".pilot", "worktrees", "PIL-884");
    ...
    const sharedWorktreesRoot = path.join(tempRoot, "repo", ".pilot", "worktrees");
