...
describeEmbeddedPostgres("summary slot service", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-summary-slots-");
  ...
  async function seedProject(companyId: string) {
    ...
    await db.insert(projects).values({ id: projectId, companyId, name: "Pilot App" });
    return projectId;
  }
  ...
  describe("generate", () => {
    ...
    it("creates a summarizer task, links it, and marks the slot generating", async () => {
      ...
      expect(issueRow.description).not.toContain("/PIL/issues/");
