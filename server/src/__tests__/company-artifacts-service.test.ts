...
import { ISSUE_CONTINUATION_SUMMARY_DOCUMENT_KEY } from "@pilotai/shared";
import {
...
describeEmbeddedPostgres("companyArtifactsService", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-company-artifacts-");
  ...
  async function seedArtifacts() {
    ...
    await db.insert(companies).values([
      { id: companyId, name: "Pilot", issuePrefix: "PIL", requireBoardApprovalForNewAgents: false },
  ...
  it("groups artifacts by task after applying media, project, and search filters", async () => {
    ...
    expect(grouped.groups?.map((group) => ({
    ...
    }))).toEqual([
      {
        ...
        count: 1,
        mediaKinds: ["document"],
        href: "/PIL/artifacts?groupBy=task&groupIssueId=77777777-7777-4777-8777-777777777777",
      },
      {
        issue: "PIL-1",
        count: 3,
        mediaKinds: ["video", "text"],
        href: "/PIL/artifacts?groupBy=task&groupIssueId=66666666-6666-4666-8666-666666666666",
      },
    ]);
    ...
    expect(projectVideos.groups?.map((group) => ({
    ...
    }))).toEqual([
      {
        ...
        count: 2,
        href:
          "/PIL/artifacts?groupBy=task&groupIssueId=66666666-6666-4666-8666-666666666666&kind=video&projectId=55555555-5555-4555-8555-555555555555",
      },
    ]);
  ...
  it("paginates grouped task lists with the active group cursor", async () => {
    ...
    expect(firstPage.groups?.map((group) => group.issue.identifier)).toEqual(["PIL-2"]);
    expect(firstPage.nextCursor).toEqual(expect.any(String));
    ...
    expect(secondPage.groups?.map((group) => group.issue.identifier)).toEqual(["PIL-1"]);
