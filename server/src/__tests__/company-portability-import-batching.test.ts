...
// The `.pilot.yaml` extension is consumed by the importer's own hand-rolled
...
function renderPilotYaml(value: Record<string, unknown>): string {
...
function buildSyntheticBundle(options: SyntheticBundleOptions): {
  ...
  files[".pilot.yaml"] = renderPilotYaml({ schemaVersion: 7, tasks });
...
describeEmbeddedPostgres("company import batches inserts", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-import-batching-");
  ...
  it("preserves bundle timestamps and parent hierarchy through a real import", async () => {
    ...
    files[".pilot.yaml"] = renderPilotYaml({
      schemaVersion: 7,
      tasks: {
