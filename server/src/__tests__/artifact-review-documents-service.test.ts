...
describeEmbeddedPostgres("artifactReviewDocumentService", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-artifact-review-docs-");
  ...
  it("rejects work products that are not attachment-backed Pilot artifacts", async () => {
    const fixture = await seedFixture();
    await expect(
