...
import { buildSkillMentionHref } from "@pilotai/shared";
import {
...
describe("resolveExecutionRunAdapterConfig", () => {
  ...
  it("drops PILOT_API_KEY bindings but forwards other PILOT_-named env to resolution", async () => {
...
describe("resolveExecutionRunAdapterConfig codex_local credential pre-dispatch gate", () => {
  ...
  async function stubManagedCodexEnv(options: { seedSharedAuth: boolean }) {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-gate-"));
