...
describeEmbeddedPostgres("workspace operation reconciliation", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-workspace-operation-reconcile-");
    db = createDb(tempDb.connectionString);
    logRoot = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-workspace-operation-logs-"));
  ...
  it("terminalizes an orphaned runtime start with an inspectable reconciliation log", async () => {
    ...
    // Recovery is bounded and cannot steal a live operation (PIL-17249): a `running` control
