...
import { parseFrontmatterMarkdown } from "@pilotai/shared";
import {
...
describeEmbeddedPostgres("companySkillService.list", () => {
  ...
  async function createManagedSkillDir(companyId: string, prefix: string) {
    if (!pilotHome) throw new Error("Expected Pilot test home");
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-company-skills-service-");
    oldPilotHome = process.env.PILOT_HOME;
    oldPilotInstanceId = process.env.PILOT_INSTANCE_ID;
    pilotHome = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-company-skills-home-"));
  ...
  it("lists skills without exposing markdown content", async () => {
    ...
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-heavy-skill-"));
    cleanupDirs.add(skillDir);
  ...
  it("optionally enriches list items with latest version editor identities", async () => {
    ...
    async function writeTrackedSkillDir(slug: string, name: string) {
      const dir = await fs.mkdtemp(path.join(os.tmpdir(), `pilot-${slug}-`));
      cleanupDirs.add(dir);
  ...
  it("does not retouch unchanged bundled skills during list refresh", async () => {
    ...
    const bundledSkill = initialList.find((skill) => skill.key.startsWith("pilotai/pilot/"));
    expect(bundledSkill).toBeDefined();
    if (!bundledSkill) throw new Error("Expected bundled Pilot skills fixture");
  ...
  it("seeds bundled skill releases idempotently and materializes the frozen champion snapshot", async () => {
    ...
    const pilotSkill = initialList.find((skill) => skill.key === "pilotai/pilot/pilot");
    expect(pilotSkill).toBeDefined();
    if (!pilotSkill) throw new Error("Expected bundled Pilot skill");
    ...
    expect(championHashes).toMatchObject({
      "SKILL.md": "950102ec38be3d029d4a8143941dc6164fac3dd168458e6c56baba94da2a50c7",
      "references/cases.md": "d1756450c671efe4d501796f3a93dbdfa362df6531359d343dbc4a8b8cea87d5",
      "references/workflows.md": "b939c21f871f856b6006dd8ad8fbe043da71bde96b97680a8a9e6bb62512e29c",
    });
  ...
  it("repairs a squatted bundled root during bundled-skill list refresh", async () => {
    ...
    expect(listed.some((skill) => skill.key.startsWith("pilotai/pilot/"))).toBe(true);
  ...
  it("filters store list results by category and creates version snapshots", async () => {
    const companyId = randomUUID();
    const skillId = randomUUID();
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-versioned-skill-"));
    cleanupDirs.add(skillDir);
  ...
  it("resolves detail by unique skill slug for Studio deep links", async () => {
    ...
    const skill = await svc.createLocalSkill(companyId, {
      name: "Pilot Blog Cover Image",
      slug: "pilot-blog-cover-image",
      markdown: "# Pilot Blog Cover Image\n",
    });

    await expect(svc.detail(companyId, "pilot-blog-cover-image")).resolves.toMatchObject({
      id: skill.id,
      slug: "pilot-blog-cover-image",
      name: "Pilot Blog Cover Image",
    });
  ...
  it("creates a fork from the creation flow with copied files and lineage", async () => {
    ...
    const sourceSkillDir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-source-fork-skill-"));
  ...
  it("prechecks existing forks and reassigns selected agents when forking", async () => {
    const companyId = randomUUID();
    const sourceSkillId = randomUUID();
    const sourceSkillDir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-reassign-source-"));
    cleanupDirs.add(sourceSkillDir);
    ...
    expect(reassignConfig.pilotSkillSync.desiredSkills).toEqual([`company/${companyId}/source-skill-fork`]);
    expect(keepConfig.pilotSkillSync.desiredSkills).toEqual([`company/${companyId}/source-skill`]);
  ...
  it("validates version-aware desired skill selections", async () => {
    ...
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-pinned-skill-"));
    cleanupDirs.add(skillDir);
  ...
  it("rejects unknown desired keys by default but preserves them when tolerating (PIL-13222)", async () => {
    const companyId = randomUUID();
    const skillId = randomUUID();
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-tolerant-skill-"));
    cleanupDirs.add(skillDir);
  ...
  it("preserves missing local-path skills that active agents still desire", async () => {
    ...
    const missingSkillDir = path.join(await fs.mkdtemp(path.join(os.tmpdir(), "pilot-missing-used-skill-")), "gone");
  ...
  it("continues pruning missing local-path skills that no active agent desires", async () => {
    const companyId = randomUUID();
    const skillId = randomUUID();
    const missingSkillDir = path.join(await fs.mkdtemp(path.join(os.tmpdir(), "pilot-missing-unused-skill-")), "gone");
    cleanupDirs.add(path.dirname(missingSkillDir));
  ...
  it("refreshes stale local-path file inventory from disk", async () => {
    const companyId = randomUUID();
    const skillId = randomUUID();
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-stale-inventory-skill-"));
    cleanupDirs.add(skillDir);
  ...
  it("rejects unbundled package imports that claim reserved Pilot skill keys", async () => {
    const companyId = randomUUID();
    const skillId = randomUUID();
    const bundledSkillDir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-bundled-skill-"));
    ...
    await db.insert(companySkills).values({
      ...
      compatibility: "compatible",
      fileInventory: [{ path: "SKILL.md", kind: "skill" }],
      metadata: { sourceKind: "pilot_bundled" },
    });
    ...
    await expect(svc.importPackageFiles(companyId, {
      ...
      status: 422,
      message: 'Reserved Pilot skill key "pilotai/pilot/pilot" cannot be imported from unbundled sources.',
    });
    ...
    expect(stored?.name).not.toBe("Trojan Pilot");
    expect(stored?.markdown).not.toContain("Trojan Pilot");
  });

  it("clears the missing-source marker when a local-path skill source returns", async () => {
    const companyId = randomUUID();
    const skillId = randomUUID();
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-restored-skill-"));
    cleanupDirs.add(skillDir);
  ...
  it("marks source-missing company skills as unavailable during read-only runtime listing", async () => {
    ...
    const skillId = randomUUID();
    const skillKey = `company/${companyId}/reflection-coach`;
    const missingSkillDir = path.join(await fs.mkdtemp(path.join(os.tmpdir(), "pilot-readonly-missing-skill-")), "gone");
    cleanupDirs.add(path.dirname(missingSkillDir));
  ...
  it("materializes source-missing company skills from the stored markdown during runtime listing", async () => {
    ...
    const missingSkillDir = path.join(await fs.mkdtemp(path.join(os.tmpdir(), "pilot-runtime-missing-skill-")), "gone");
    cleanupDirs.add(path.dirname(missingSkillDir));
  ...
  it("falls back to stored markdown when reading SKILL.md from a missing local source", async () => {
    ...
    const missingSkillDir = path.join(await fs.mkdtemp(path.join(os.tmpdir(), "pilot-missing-read-skill-")), "gone");
    cleanupDirs.add(path.dirname(missingSkillDir));
  ...
  it("browses project folders and imports a selected non-standard skill", async () => {
    ...
    const workspaceDir = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "pilot-skill-browse-")));
    ...
    // Probe for case-insensitive filesystems (macOS default): lowercase skill.md is
    // indistinguishable from SKILL.md there, so remove it and skip the case-sensitivity assertion.
    const caseSensitiveFs = !(await fs
      .stat(path.join(workspaceDir, "content", "SKILL.md"))
      .then(() => true)
      .catch(() => false));
    if (!caseSensitiveFs) {
      await fs.rm(path.join(workspaceDir, "content", "skill.md"));
    }
    ...
    if (caseSensitiveFs) {
      contentEntries.push(expect.objectContaining({ name: "skill.md", kind: "file", isSkill: false }));
    }
    expect(content.entries).toEqual(contentEntries);
  ...
  it("previews project workspace skill candidates without importing them", async () => {
    ...
    const projectId = randomUUID();
    const workspaceId = randomUUID();
    const workspaceDir = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "pilot-skill-preview-")));
    cleanupDirs.add(workspaceDir);
  ...
  it("reports a project skill as already installed when the source path matches", async () => {
    ...
    const projectId = randomUUID();
    const workspaceId = randomUUID();
    const workspaceDir = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "pilot-skill-same-path-")));
    cleanupDirs.add(workspaceDir);
  ...
  it("reports project skills that duplicate built-in slugs as already available", async () => {
    ...
    const workspaceDir = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "pilot-skill-built-in-")));
    const bundledSkillDir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-skill-bundled-source-"));
    cleanupDirs.add(workspaceDir);
    cleanupDirs.add(bundledSkillDir);
    ...
    await db.insert(companySkills).values({
      id: bundledSkillId,
      companyId,
      key: "pilotai/pilot/built-in-review",
  ...
  it("imports a conflicting project skill under a selected replacement slug", async () => {
    ...
    const workspaceDir = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "pilot-skill-rename-")));
    const existingSkillDir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-skill-existing-"));
    cleanupDirs.add(workspaceDir);
  ...
  it("imports only selections rediscovered inside project workspaces", async () => {
    ...
    const projectId = randomUUID();
    const workspaceId = randomUUID();
    const workspaceDir = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "pilot-skill-selective-")));
    cleanupDirs.add(workspaceDir);
    ...
    const outsideSkillFile = path.join(
      await fs.mkdtemp(path.join(os.tmpdir(), "pilot-skill-selective-outside-")),
      "SKILL.md",
    );
  ...
  it("treats out-of-scope workspace selections as unmatched without leaking workspace metadata", async () => {
    ...
    const projectId = randomUUID();
    const workspaceId = randomUUID();
    const workspaceDir = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "pilot-skill-scope-")));
    const otherCompanyId = randomUUID();
    const otherProjectId = randomUUID();
    const otherWorkspaceId = randomUUID();
    const otherWorkspaceDir = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "pilot-skill-scope-")));
    cleanupDirs.add(workspaceDir);
  ...
  it("skips a selected project skill whose SKILL.md is a symlink outside the workspace", async () => {
    ...
    const projectId = randomUUID();
    const workspaceId = randomUUID();
    const workspaceDir = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "pilot-skill-symlink-")));
    const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-skill-outside-"));
    cleanupDirs.add(workspaceDir);
  ...
  it("files new project imports without moving them back on re-import", async () => {
    ...
    const workspaceId = randomUUID();
    const folderSvc = folderService(db);
    const workspaceDir = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "pilot-skill-project-folder-")));
    cleanupDirs.add(workspaceDir);
  ...
  function runtimeSkillName(key: string, slug: string) {
    if (key.startsWith("pilotai/pilot/")) return slug;
    return `${slug}--${createHash("sha256").update(key).digest("hex").slice(0, 10)}`;
  }

  it("renames a Pilot-managed skill, moving the directory and rewriting SKILL.md frontmatter", async () => {
    const companyId = randomUUID();
  ...
  it("rejects a rename whose derived key conflicts with another skill", async () => {
    ...
    const squatterDir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-key-squatter-"));
  ...
  it("rejects renaming non Pilot-managed skill sources with 422", async () => {
    const companyId = randomUUID();
    await seedCompany(companyId);

    const unmanagedDir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-unmanaged-skill-"));
  ...
  it("rewrites agent desired-skill keys on rename while preserving version pins", async () => {
    ...
    await db.insert(agents).values([
      ...
      },
      {
        ...
        adapterType: "codex_local",
        adapterConfig: {
          pilotSkillSync: { desiredSkills: [`company/${companyId}/shared`] },
        },
      },
      {
        id: otherAgentId,
        ...
        adapterType: "codex_local",
        adapterConfig: {
          pilotSkillSync: { desiredSkills: [`company/${companyId}/unrelated`] },
        },
      },
    ]);
    ...
    expect(pinned.pilotSkillSync.desiredSkills).toEqual([
      { key: `company/${companyId}/shared-renamed`, versionId: pinnedVersionId },
    ]);
    expect(loose.pilotSkillSync.desiredSkills).toEqual([`company/${companyId}/shared-renamed`]);
    expect(other.pilotSkillSync.desiredSkills).toEqual([`company/${companyId}/unrelated`]);
  });
