...
describe("resolveBundledCatalogRoot", () => {
  ...
  it("honors PILOT_BUNDLED_PLUGIN_ROOT", () => {
...
function makeManifest(pluginKey: string, version: string) {
  return { id: pluginKey, apiVersion: 1, version } as unknown as import("@pilotai/shared").PilotPluginManifestV1;
}
...
describe("ensureBundledPlugins", () => {
  ...
  it("logs and continues past a failing install, still processing later entries", async () => {
    ...
    expect(deps.lifecycle.load).toHaveBeenCalledWith("id-pilot.daytona-sandbox-provider");
  });
