...
describeEmbeddedPostgres("plugin orchestration APIs", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-plugin-orchestration-");
  ...
  async function makeLocalRoot() {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-plugin-host-folder-"));
    tempRoots.push(root);
    return root;
  }
  ...
  it("returns plugin-safe execution workspace metadata scoped to the company", async () => {
    ...
    await db.insert(executionWorkspaces).values({
      ...
      repoUrl: "https://example.com/pilot.git",
      baseRef: "main",
      branchName: "feature/workspace",
      providerType: "git_worktree",
      providerRef: "/tmp/pilot-feature",
      metadata: {
    ...
    const services = buildHostServices(db, "plugin-record-id", "pilot.workspace", createEventBusStub());
  ...
  it("creates plugin-origin issues with full orchestration fields and audit activity", async () => {
    ...
    const services = buildHostServices(db, "plugin-record-id", "pilot.missions", createEventBusStub());
    ...
    expect(stored?.originKind).toBe("plugin:pilot.missions");
      ...
      expect.arrayContaining([
        expect.objectContaining({
          ...
          details: expect.objectContaining({
            ...
            sourcePluginKey: "pilot.missions",
  ...
  it("enforces plugin origin namespaces", async () => {
    ...
    expect(featureIssue.originKind).toBe("plugin:pilot.missions:feature");
    ...
    ).rejects.toThrow("Plugin may only use originKind values under plugin:pilot.missions");
  ...
  it("creates plugin operation issues with the generic operation origin", async () => {
    ...
    expect(issue.originKind).toBe("plugin:pilot.missions:operation");
  ...
  it("lets bootstrap-style actions initialize required local folders from an empty root", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "pilotai.plugin-llm-wiki",
      packageName: "@pilotai/plugin-llm-wiki",
      version: "0.1.0",
      manifestJson: {
        id: "pilotai.plugin-llm-wiki",
        apiVersion: 1,
        version: "0.1.0",
        ...
        localFolders: [
          {
            ...
            access: "readWrite",
            requiredDirectories: ["raw", "wiki", "wiki/concepts", ".pilot"],
  ...
  it("rejects worker local-folder access for undeclared manifest keys", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "pilot.local-folders",
      packageName: "@pilot/plugin-local-folders",
      version: "0.1.0",
      manifestJson: {
        id: "pilot.local-folders",
        apiVersion: 1,
        version: "0.1.0",
  ...
  it("resolves plugin-managed projects by stable key without overwriting user edits", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "pilot.missions",
      packageName: "@pilot/plugin-missions",
      version: "0.1.0",
      apiVersion: 1,
      categories: ["automation"],
      status: "ready",
    ...
    const services = buildHostServices(db, pluginId, "pilot.missions", createEventBusStub());
  ...
  it("rejects a human-attributed plugin comment when actorUserId is not an active company member", async () => {
    ...
    const services = buildHostServices(db, "plugin-record-id", "pilot.gateway", createEventBusStub());
    await expect(
