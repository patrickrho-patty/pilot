  ...
  it("skips git metadata probing for packaged installs under node_modules", () => {
    ...
    expect(
      resolveServerVersion({
        ...
        debugLog,
        packageRoot: "/tmp/npm/_npx/example/node_modules/@pilotai/server",
      }),
  ...
  it("uses git metadata for source checkouts whose path contains node_modules", () => {
    ...
    expect(
      resolveServerVersion({
        ...
        debugLog,
        gitDescribeCommand,
        packageRoot: "/tmp/node_modules/source/pilot/server",
        pathExists: (path) => path === "/tmp/node_modules/source/pilot/.git",
        realpath: (path) => path,
      }),
    ).toBe("2026.626.0+58.git.518fc71ce");
