...
import { sessionCodec as claudeSessionCodec } from "@pilotai/adapter-claude-local/server";
import { sessionCodec as codexSessionCodec, isCodexUnknownSessionError } from "@pilotai/adapter-codex-local/server";
import {
...
import { sessionCodec as acpxSessionCodec } from "@pilotai/adapter-utils/acpx-engine/session-codec";
...
describe("adapter session codecs", () => {
  ...
  it("preserves claude ACP session params for ACP lane resumes", () => {
    const parsed = claudeSessionCodec.deserialize({
      sessionKey: "pilot:company:agent:task:fingerprint",
