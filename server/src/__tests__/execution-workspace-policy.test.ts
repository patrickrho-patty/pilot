...
describe("execution workspace policy helpers", () => {
  ...
  it("accepts an existing-branch pin only with isolated mode and a git_worktree strategy", () => {
    expect(issueExecutionWorkspaceSettingsSchema.parse({
      ...
      workspaceStrategy: {
        type: "git_worktree",
        existingBranch: "PIL-14380-salvage-pil-9514",
      },
    }).workspaceStrategy?.existingBranch).toBe("PIL-14380-salvage-pil-9514");
  ...
  it("carries the existing-branch pin through issue settings parsing", () => {
    expect(
      parseIssueExecutionWorkspaceSettings({
        mode: "isolated_workspace",
        workspaceStrategy: { type: "git_worktree", existingBranch: " PIL-14754-run-redaction " },
      })?.workspaceStrategy,
    ).toEqual({ type: "git_worktree", existingBranch: "PIL-14754-run-redaction" });
  });
