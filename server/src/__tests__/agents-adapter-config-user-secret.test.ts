...
vi.mock("@pilotai/adapter-claude-local/server", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@pilotai/adapter-claude-local/server")>();
  return {
    ...actual,
...
describeEmbeddedPostgres("agents adapter-config user-secret resolution routes", () => {
  ...
  const secretsTmpDir = path.join(os.tmpdir(), `pilot-adapter-user-secret-${randomUUID()}`);
