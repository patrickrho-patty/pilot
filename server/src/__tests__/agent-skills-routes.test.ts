...
vi.mock("@pilotai/shared/telemetry", () => ({
...
function registerModuleMocks() {
  vi.doMock("@pilotai/shared/telemetry", () => ({
    trackAgentCreated: mockTrackAgentCreated,
    trackErrorHandlerCrash: vi.fn(),
  }));
...
describe.sequential("agent skill routes", () => {
  beforeEach(() => {
    ...
    mockCompanySkillService.resolveRequestedSkillEntries.mockImplementation(
      async (_companyId: string, requested: Array<{ key: string; versionId?: string | null }>) => ({
        resolved: requested.map((entry) => ({
          key: entry.key === "pilot" ? "pilotai/pilot/pilot" : entry.key,
    ...
    mockAdapter.listSkills.mockResolvedValue({
      ...
      supported: true,
      mode: "ephemeral",
      desiredSkills: ["pilotai/pilot/pilot"],
      entries: [],
      warnings: [],
    });
    ...
    expect(mockAdapter.listSkills).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        config: expect.objectContaining({
          pilotRuntimeSkills: expect.any(Array),
        }),
      }),
    );
  }, 10_000);
  ...
  it("threads a non-undefined actor secret context into resolveAdapterConfigForRuntime on both skills routes (audit fidelity, skipUserSecrets preserved)", async () => {
    ...
    const syncRes = await requestApp(
      ...
      (baseUrl) => request(baseUrl)
        ...
        .send({ desiredSkills: ["pilot"], mode: "replace" }),
    );
    ...
    expect(mockAgentService.update).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        adapterConfig: expect.objectContaining({
          agent: "codex",
          pilotSkillSync: expect.objectContaining({
            desiredSkills: ["pilotai/pilot/pilot"],
          }),
        }),
      }),
  ...
  it("adds only named desired skills while preserving existing assignments", async () => {
    mockAgentService.getById.mockResolvedValue({
      ...makeAgent("claude_local"),
      adapterConfig: {
        pilotSkillSync: { desiredSkills: ["company-1/keep"] },
      },
    });

    const res = await requestApp(await createApp(), (baseUrl) => request(baseUrl)
      .post("/api/agents/11111111-1111-4111-8111-111111111111/skills/sync?companyId=company-1")
      .send({ desiredSkills: ["pilot"], mode: "add" }));
    ...
    expect(mockAgentService.update).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        adapterConfig: expect.objectContaining({
          pilotSkillSync: {
            desiredSkills: ["company-1/keep", "pilotai/pilot/pilot"],
          },
        }),
      }),
      expect.any(Object),
  ...
  it("removes only named desired skills while preserving other assignments", async () => {
    ...
    const res = await requestApp(await createApp(), (baseUrl) => request(baseUrl)
      .post("/api/agents/11111111-1111-4111-8111-111111111111/skills/sync?companyId=company-1")
      .send({ desiredSkills: ["pilot"], mode: "remove" }));
  ...
  it("accepts version pins while beta skills are enabled", async () => {
    ...
    const res = await requestApp(await createApp(), (baseUrl) => request(baseUrl)
      ...
      .send({
        mode: "replace",
        desiredSkills: [{ key: "pilotai/pilot/pilot", versionId }],
      }));
  ...
  it("preserves stale desired keys instead of 422-ing when syncing (PIL-13222)", async () => {
    ...
    const res = await requestApp(await createApp(), (baseUrl) => request(baseUrl)
      .post("/api/agents/11111111-1111-4111-8111-111111111111/skills/sync?companyId=company-1")
      .send({ desiredSkills: ["pilot", "stale/removed/skill"], mode: "replace" }));
    ...
    expect(mockAgentService.update).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        adapterConfig: expect.objectContaining({
          pilotSkillSync: expect.objectContaining({
            desiredSkills: ["pilotai/pilot/pilot", "stale/removed/skill"],
          }),
        }),
      }),
      expect.any(Object),
  ...
  it("skips runtime materialization when syncing Claude skills", async () => {
    ...
    const res = await requestApp(await createApp(), (baseUrl) => request(baseUrl)
      .post("/api/agents/11111111-1111-4111-8111-111111111111/skills/sync?companyId=company-1")
      .send({ desiredSkills: ["pilotai/pilot/pilot"], mode: "replace" }));
  ...
  it("materializes the bundled default instruction set for non-CEO agents with no prompt template", async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(mockAgentInstructionsService.materializeManagedBundle).toHaveBeenCalledWith(
        expect.any(Object),
        expect.objectContaining({
          "AGENTS.md": expect.stringContaining("skills/pilot/scripts/pilot-upload-artifact.sh"),
        }),
        expect.any(Object),
      );
    });
