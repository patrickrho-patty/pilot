...
import type { CompanyPortabilityFileEntry } from "@pilotai/shared";
...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "pilot",
        name: "pilot",
        description: "Pilot coordination skill",
        markdown: "---\nname: pilot\ndescription: Pilot coordination skill\n---\n\n# Pilot\n",
        sourceType: "github",
        sourceLocator: "https://github.com/pilotai/pilot/tree/master/skills/pilot",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/pilotai/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      hostname: "github.com",
      owner: "pilotai",
      repo: "companies",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/pilotai/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      hostname: "github.com",
      owner: "pilotai",
  ...
  it("exports referenced skills as stubs by default with sanitized Pilot extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Pilot"');
    ...
    expect(asTextFile(exported.files["skills/pilotai/pilot/pilot/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/pilotai/pilot/pilot/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/pilotai/pilot/pilot/references/api.md"]).toBeUndefined();
    expect(asTextFile(exported.files["skills/company/PIL/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(asTextFile(exported.files["skills/company/PIL/company-playbook/references/checklist.md"])).toContain("# Checklist");

    const extension = asTextFile(exported.files[".pilot.yaml"]);
    expect(extension).toContain('schema: "pilot/v1"');
    ...
    expect(extension).not.toContain("pilotSkillSync");
  ...
  it("exports agent permission grants through the Pilot extension and manifest", async () => {
    const db = {
  ...
  it("exports hire approval policy only when approval is required", async () => {
    ...
    expect(asTextFile(exported.files[".pilot.yaml"])).toContain("requireBoardApprovalForNewAgents: true");
  });
  ...
  it("exports default sidebar order into the Pilot extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".pilot.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/pilotai/pilot/pilot/SKILL.md"])).toContain("# Pilot");
    expect(asTextFile(exported.files["skills/pilotai/pilot/pilot/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/pilotai/pilot/pilot/references/api.md"])).toContain("# API");
  });

  it("exports catalog skill provenance in portable Pilot frontmatter", async () => {
    const portability = companyPortabilityService({} as any);
    const catalogKey = "pilotai/bundled/software-development/review";
    ...
    const catalogSkill = {
      ...
      markdown: "---\nname: review\ndescription: Catalog review skill\n---\n\n# Review\n",
      sourceType: "catalog",
      sourceLocator: "/tmp/pilot/catalog/review",
    ...
    const skillMarkdown = asTextFile(exported.files["skills/pilotai/bundled/software-development/review/SKILL.md"]);
    expect(skillMarkdown).toContain("pilot:");
    expect(skillMarkdown).toContain("catalog:");
    expect(skillMarkdown).toContain(`sourceRef: "${originHash}"`);
    expect(skillMarkdown).toContain('catalogId: "pilotai:bundled:software-development:review"');
    ...
    expect(skillMarkdown).toContain('packageName: "@pilotai/skills-catalog"');
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(exported.files["skills/company/PIL/company-playbook/SKILL.md"]).toBeDefined();
    expect(asTextFile(exported.files["skills/company/PIL/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/pilotai/pilot/pilot/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PIL/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/pilotai/pilot/pilot/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .pilot.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".pilot.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-pilot",
        companyId: "company-1",
        key: "pilotai/pilot/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/pilotai/pilot/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
        metadata: {
          sourceKind: "pilot_bundled",
          owner: "pilotai",
          repo: "pilot",
          ref: "0123456789abcdef0123456789abcdef01234567",
          trackingRef: "master",
    ...
    expect(asTextFile(exported.files["skills/pilotai/pilot/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/pilotai/pilot/release-changelog/SKILL.md"])).toContain("pilotai/pilot/release-changelog");
  });
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/pilot",
            repoUrl: "https://github.com/pilotai/pilot.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .pilot-tmp",
            remoteProvider: null,
  ...
  it("normalizes invalid imported project icon names to null", async () => {
    ...
    await portability.importBundle({
      source: { type: "inline", rootPath: "pilot-demo", files },
      include: { company: true, agents: false, projects: true, issues: false },
      target: { mode: "new_company", newCompanyName: "Imported Pilot" },
      collisionStrategy: "rename",
    }, "user-1");
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/pilotai/pilot.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    const extension = asTextFile(exported.files[".pilot.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/pilotai/pilot.git"');
    expect(extension).toContain('projectWorkspaceKey: "pilot"');
  ...
  it("collapses repeated task workspace warnings into one summary per missing workspace", async () => {
    ...
    expect(exported.warnings).toContain("Project launch workspace Local Scratch was omitted from export because it does not have a portable repoUrl.");
    expect(exported.warnings).toContain("Tasks pil-1, pil-2, pil-3 reference workspace workspace-1, but that workspace could not be exported portably.");
  ...
  it("reads env inputs back from .pilot.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("reads project env inputs back from .pilot.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with Pilot routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("pauses imported agents and routines when pauseAutomations is requested", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported Pilot"', "---", ""].join("\n"),
  ...
  it("imports recurring tasks without a project or assignee as paused routines", async () => {
    ...
    const request = {
      source: { type: "inline" as const, rootPath: "pilot-demo", files },
      include: { company: true, agents: false, projects: false, issues: true, skills: false },
      target: { mode: "new_company" as const, newCompanyName: "Imported Pilot" },
  ...
  it("imports a vendor-neutral package without .pilot.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Pilot");
  ...
  it("imports packaged skills and restores desired skill refs on agents", async () => {
    ...
    companySkillSvc.importPackageFiles.mockResolvedValueOnce([{
      ...
      originalSlug: "pilot",
      requestedRefs: ["pilot"],
      reason: "Existing skill matched; renamed to pilot-2.",
    }]);
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".pilot.yaml"] = `${exported.files[".pilot.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
  ...
  function mockTaskFidelityExportSources() {
    ...
    issueSvc.getRelationSummaries.mockImplementation(async (issueId: string) => {
      if (issueId === "issue-1") {
        return {
          blockedBy: [relationSummary("issue-outside", "PIL-9", "Outside task")],
          blocks: [relationSummary("issue-2", "PIL-2", "Beta task")],
        };
      }
      if (issueId === "issue-2") {
        return {
          blockedBy: [relationSummary("issue-1", "PIL-1", "Alpha task")],
          blocks: [],
        };
      }
  ...
  it("carries blockers, documents, work products, and monitors through export and import", async () => {
    ...
    expect(asTextFile(exported.files["tasks/pil-1/documents/spec.md"])).toBe("# Spec\n\nDetails.");
    const extension = asTextFile(exported.files[".pilot.yaml"]);
    expect(extension).toContain("blockedBy:");
    expect(extension).toContain('"pil-1"');
    ...
    const alphaEntry = exported.manifest.issues.find((issue) => issue.slug === "pil-1");
    const betaEntry = exported.manifest.issues.find((issue) => issue.slug === "pil-2");
    expect(alphaEntry?.documents).toEqual([
      { key: "spec", title: "Spec", format: "markdown", path: "tasks/pil-1/documents/spec.md" },
    ]);
    ...
    expect(betaEntry?.blockedBy).toEqual(["pil-1"]);
  ...
  it("skips blockers and documents of tasks excluded from the import selection", async () => {
    ...
    const result = await portability.importBundle({
      ...
      agents: "all",
      collisionStrategy: "rename",
      selectedFiles: ["COMPANY.md", ".pilot.yaml", "tasks/pil-2/TASK.md"],
    }, "user-1");
    ...
    expect(insertedRelationValues).toEqual([]);
    expect(result.warnings).toContain(
      "Task pil-2 blocker pil-1 was skipped because that task was not imported.",
    );
  });
  ...
  it("carries task timestamps and parent links through export and import", async () => {
    ...
    const extension = asTextFile(exported.files[".pilot.yaml"]);
    expect(extension).toContain("schemaVersion: 7");
    expect(extension).toContain('parent: "pil-1"');
    ...
    const alphaEntry = exported.manifest.issues.find((issue) => issue.slug === "pil-1");
    const betaEntry = exported.manifest.issues.find((issue) => issue.slug === "pil-2");
    const gammaEntry = exported.manifest.issues.find((issue) => issue.slug === "pil-3");
    const deltaEntry = exported.manifest.issues.find((issue) => issue.slug === "pil-4");
    ...
    expect(betaEntry?.parentSlug).toBe("pil-1");
    expect(gammaEntry?.parentSlug).toBe("pil-2");
  ...
  it("carries issue attachments as content-addressed blobs through export and import", async () => {
    ...
    const taskEntry = exported.manifest.issues.find((issue) => issue.slug === "pil-1");
    ...
    expect(exported.warnings).toContain(
      "Attachment notes.bin on task pil-1 was exported under its recomputed content hash because the stored hash did not match.",
    );
  ...
  it("skips attachment export with a per-task warning when storage is unavailable", async () => {
    ...
    expect(exported.warnings).toContain(
      "Skipped 2 attachments on task pil-1 because storage is unavailable.",
    );
    expect(Object.keys(exported.files).some((filePath) => filePath.startsWith("blobs/"))).toBe(false);
    expect(asTextFile(exported.files[".pilot.yaml"])).not.toContain("attachments:");
  });
  ...
  it("skips oversized and missing-blob attachments with warnings instead of failing", async () => {
    ...
    expect(issueSvc.addImportedAttachments).not.toHaveBeenCalled();
    expect(result.warnings).toContain(
      `Task pil-1 attachment notes.bin was skipped because its blob is missing from the package: blobs/${sha}`,
    );
    expect(result.warnings).toContain(
      `Task pil-1 attachment screenshot.png was skipped because its blob is missing from the package: blobs/${sha}`,
    );
    expect(result.warnings).toContain(
      "Task pil-1 attachment big.bin was skipped because it exceeds this board's attachment size limit of 10 bytes.",
    );
  });
  ...
  it("skips embedded image references that are foreign or dangling with one aggregate warning", async () => {
    ...
    expect(asTextFile(exported.files[".pilot.yaml"])).not.toContain("embeddedAssets:");
    expect(exported.manifest.embeddedAssets).toEqual([]);
    expect(exported.warnings).toContain(
  ...
  it("prunes embedded asset entries and blobs when the referencing files are excluded from the export selection", async () => {
    ...
    const kept = await portability.exportBundle("company-1", {
      include: { company: true, agents: false, projects: false, issues: true },
      selectedFiles: ["COMPANY.md", ".pilot.yaml", "tasks/pil-1/TASK.md", "tasks/pil-1/documents/spec.md", `blobs/${sha}`],
    });
    expect(asTextFile(kept.files[".pilot.yaml"])).toContain("embeddedAssets:");
    expect(kept.files[`blobs/${sha}`]).toBeDefined();

    const pruned = await portability.exportBundle("company-1", {
      include: { company: true, agents: false, projects: false, issues: true },
      selectedFiles: ["COMPANY.md", ".pilot.yaml"],
    });
    expect(Object.keys(pruned.files).some((filePath) => filePath.startsWith("blobs/"))).toBe(false);
    const prunedYaml = asTextFile(pruned.files[".pilot.yaml"]);
  ...
  it("rejects packages produced by a newer Pilot", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    await expect(portability.importBundle({
      ...
      agents: "all",
      collisionStrategy: "rename",
    }, "user-1")).rejects.toThrow(/newer Pilot/);
    expect(issueSvc.importIssues).not.toHaveBeenCalled();
  });
    ...
    issueSvc.listComments.mockResolvedValue([
      {
        ...
        authorAgentId: null,
        authorUserId: null,
        body: "Pilot needs a disposition before this issue can continue.",
        presentation,
        metadata,
  ...
  it("downgrades user-authored imported comments to system when no importing user exists", async () => {
    ...
    expect(result.warnings).toContain(
      "Comment on task pil-1 was imported as a system comment because no importing user was available.",
    );
  });
      ...
      source: {
        ...
        files: {
          ...
          ".pilot.yaml": [
            ...
            "      default:",
            "        name: App",
            "        repoUrl: https://github.com/pilotai/pilot",
            "        setupCommand: pnpm install",
            "",
          ].join("\n"),
        },
  ...
  it("resolves issue assignee to existing agent when agent is skipped", async () => {
    ...
    issueSvc.list.mockResolvedValue([{
      id: "issue-1", companyId: "company-1", title: "Test task", identifier: "PIL-1",
    ...
    issueSvc.create.mockResolvedValue({ id: "issue-new", identifier: "PIL-100" });
