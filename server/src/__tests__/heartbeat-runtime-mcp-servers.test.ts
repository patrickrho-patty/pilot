...
describeEmbeddedPostgres("heartbeat runtime MCP servers", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-heartbeat-runtime-mcp-");
  ...
  it("provisions one gateway per installed connection and mints short-lived run tokens", async () => {
    process.env.PILOT_API_URL = "https://pilot.example.test";
    ...
    expect(first[0]).toMatchObject({
      name: "Installed MCP",
      connectionId: installedConnection!.id,
      url: expect.stringMatching(/^https:\/\/pilot\.example\.test\/api\/tool-gateway\/gateways\/.+\/mcp$/),
