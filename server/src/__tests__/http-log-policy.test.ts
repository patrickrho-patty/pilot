...
describe("shouldSilenceHttpSuccessLog", () => {
  it("silences cached 304 responses", () => {
    expect(shouldSilenceHttpSuccessLog("GET", "/api/issues/PIL-1383", 304)).toBe(true);
  });
  ...
  it("silences successful static asset requests", () => {
    ...
    expect(shouldSilenceHttpSuccessLog("GET", "/@fs/Users/dotta/pilot/ui/src/main.tsx", 200)).toBe(true);
  ...
  it("keeps normal successful application requests", () => {
    expect(shouldSilenceHttpSuccessLog("GET", "/api/issues/PIL-1383", 200)).toBe(false);
    expect(shouldSilenceHttpSuccessLog("PATCH", "/api/issues/PIL-1383", 200)).toBe(false);
  });

  it("keeps failing requests visible", () => {
    expect(shouldSilenceHttpSuccessLog("GET", "/api/health", 500)).toBe(false);
    expect(shouldSilenceHttpSuccessLog("GET", "/@fs/Users/dotta/pilot/ui/src/main.tsx", 404)).toBe(false);
  });
});
