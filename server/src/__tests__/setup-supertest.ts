...
type SupertestTestConstructor = {
  prototype: {
    ...
    __pilotLoopbackPatched?: boolean;
  };
};
...
if (!process.env.CODEX_HOME) {
  const codexHome = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-vitest-codex-home-"));
  fs.writeFileSync(path.join(codexHome, "auth.json"), '{"OPENAI_API_KEY":"sk-vitest"}\n', { mode: 0o600 });
  process.env.CODEX_HOME = codexHome;
}

// The automatic Tailscale HTTPS default (PIL-17158) probes for a real host
...
if (!SupertestTest.prototype.__pilotLoopbackPatched) {
  ...
  SupertestTest.prototype.__pilotLoopbackPatched = true;
}
