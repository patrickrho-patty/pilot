...
import { companySearchQuerySchema } from "@pilotai/shared";
import {
...
describeEmbeddedPostgres("deleted issue comment redaction", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-comment-redaction-");
  ...
  it("serializes comment timestamps as ISO strings through the redacted comments route (PIL-16607)", async () => {
    const { companyId, issueId } = await seedIssue();
    const commentId = randomUUID();
    await db.insert(issueComments).values({
      id: commentId,
