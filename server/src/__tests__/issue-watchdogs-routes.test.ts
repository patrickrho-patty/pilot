...
describeEmbeddedPostgres("issue watchdog routes", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-issue-watchdogs-routes-");
  ...
  async function seedCompany(name = "Pilot") {
  ...
  it("routes watchdog-discovered product bugs outside the watched source tree with evidence links", async () => {
    ...
    const res = await request(app)
      ...
      .send({
        ...
        description: "Watchdog found a Pilot follow-up routing bug.",
    ...
    expect(res.body.description).toContain("Watched source issue: [PIL-100](/PIL/issues/PIL-100)");
    expect(res.body.description).toContain("Watchdog issue: [PIL-102](/PIL/issues/PIL-102)");
    expect(res.body.referencedIssueIdentifiers).toEqual(expect.arrayContaining(["PIL-100", "PIL-102"]));
