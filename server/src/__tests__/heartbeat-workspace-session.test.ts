...
import type { agents } from "@pilotai/db";
import { sessionCodec as codexSessionCodec } from "@pilotai/adapter-codex-local/server";
...
async function createGitCheckout(options: { withRemote: boolean }) {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-push-preflight-"));
...
describe("assertGitSensitiveAdapterWorkspaceValid", () => {
  ...
  it("rejects a git worktree persisted workspace when the checked-out branch differs from the recorded branch", async () => {
    ...
    const worktreeParent = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-branch-worktree-"));
    const worktreePath = path.join(worktreeParent, "workspace");
    const recordedBranch = "PIL-1-recorded-branch";
    const actualBranch = "PIL-1-push-pr-head";
    try {
  ...
  it("rejects a workspace-linked issue when adapter cwd has no git metadata", async () => {
    const input = buildWorkspaceValidationInput();
    const cwd = "/tmp/pilot-workspace-without-git-metadata";
...
describe("assertGitWorktreeBaseWorkspaceReady", () => {
  ...
  it("rejects isolated git worktrees when the resolved base is not a git checkout", async () => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-non-git-workspace-"));
    try {
      await expect(assertGitWorktreeBaseWorkspaceReady({
        requestedExecutionWorkspaceMode: "isolated_workspace",
        config: { workspaceStrategy: { type: "git_worktree" } },
  ...
  it("keeps the not-a-git-checkout reason for a fallback with no failed materialization attempt", async () => {
    ...
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-unavailable-path-fallback-"));
    try {
      await expect(assertGitWorktreeBaseWorkspaceReady({
        requestedExecutionWorkspaceMode: "isolated_workspace",
        config: { workspaceStrategy: { type: "git_worktree" } },
  ...
  it("does not require git for shared project-primary workspaces", async () => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-shared-workspace-"));
    try {
      await expect(assertGitWorktreeBaseWorkspaceReady({
...
describe("buildAnchorFallbackWorkspaceNotes", () => {
  it("reports materialization failures ahead of the generic no-cwd note", () => {
    expect(buildAnchorFallbackWorkspaceNotes({
      fallbackCwd: "/pilot/workspaces/agent-1",
      ...
      'Failed to prepare the project workspace checkout: fatal: could not read Username. Using fallback workspace "/pilot/workspaces/agent-1" for this run.',
    ]);
  });
...
describe("requiresPushCapabilityPreflight", () => {
  it("only enables the guard when the issue explicitly mentions the GitHub PR workflow skill", () => {
    expect(requiresPushCapabilityPreflight({
      adapterType: "codex_local",
      issueId: "issue-1",
      explicitRunScopedSkillKeys: ["pilotai/bundled/software-development/github-pr-workflow"],
    })).toBe(true);
...
function buildWorkspaceConfigMetadata(
  ...
  return buildEffectiveRunWorkspaceConfigMetadata({
    ...
    workspaceStrategy: {
      ...
      baseRef: "origin/main",
      branchTemplate: "{{issue.identifier}}-{{slug}}",
      worktreeParentDir: ".pilot/worktrees",
    },
    repoUrl: "https://github.com/example/repo.git",
    repoRef: "origin/main",
    configSnapshot: {
...
describe("effective run execution workspace config freshness", () => {
  ...
  it("fails loudly when explicit reuse restore errors", async () => {
    ...
    await expect(provisionExecutionWorkspaceForFreshnessDecision({
      ...
      issueRef: { id: "issue-1", identifier: "PIL-42" },
      runId: "run-1",
      workspaceConfigFreshness: decision,
  ...
  it.each([
    { name: "a different branch", branchName: "PIL-9001-derived-child-branch" },
    ...
    async ({ branchName }) => {
      const reuseRequest = resolveExecutionWorkspaceReuseRequestForIssue({
        ...
        requestedExistingBranch: "PIL-14380-salvage-pil-9514",
  ...
  it("keeps reusing an inherited workspace whose branch matches the pinned existing branch", () => {
    expect(resolveExecutionWorkspaceReuseRequestForIssue({
      ...
      existingExecutionWorkspaceStatus: "active",
      requestedExistingBranch: "PIL-14380-salvage-pil-9514",
      existingExecutionWorkspaceBranchName: "PIL-14380-salvage-pil-9514",
    })).toEqual({
      requestedExecutionWorkspaceId: "workspace-old",
      requestedShouldReuseExisting: true,
  ...
  it("resets when configured model differs from persisted session model", () => {
    expect(
      shouldResetTaskSessionForModelChange({
        ...
        taskSessionParams: {
          sessionId: "thread-1",
          __pilotConfiguredModel: "opencode/mimo-v2-pro-free",
        },
      }),
    ).toBe(true);
  });
  ...
  it("does not reset when models match", () => {
    expect(
      shouldResetTaskSessionForModelChange({
        ...
        taskSessionParams: {
          sessionId: "thread-1",
          __pilotConfiguredModel: "gpt-5.4-mini",
        },
      }),
    ).toBe(false);
  });
    ...
    runtimeSkills: [
      {
        key: "pilot",
        runtimeName: "pilot",
        source: "/tmp/pilot/runtime-skills/pilot",
        versionId: null,
...
function sessionParamsWithConfigMetadata(
  ...
  return {
    sessionId: "thread-1",
    __pilotConfiguredModel: configuredModel,
    __pilotConfigFingerprint: metadata.fingerprint,
    __pilotConfigFingerprintVersion: metadata.version,
    __pilotConfigCategories: metadata.categories,
    __pilotConfigCategoryFingerprints: metadata.categoryFingerprints,
  };
}
...
describe("effective run session config freshness", () => {
  ...
  it("detects instructions content drift without storing the contents", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-session-fingerprint-"));
...
describe("stripConfiguredModelFromSessionParams", () => {
  ...
  it("returns a copy without mutating the input", () => {
    const input = { sessionId: "thread-1", __pilotConfiguredModel: "gpt-5.4-mini" };
    const result = stripConfiguredModelFromSessionParams(input);
    expect(result).not.toBe(input);
    expect(input.__pilotConfiguredModel).toBe("gpt-5.4-mini");
  });
...
describe("stripPilotSessionMetadataFromSessionParams", () => {
  it("removes all internal Pilot session metadata before adapter invocation", () => {
    expect(
      stripPilotSessionMetadataFromSessionParams({
        sessionId: "thread-1",
        cwd: "/tmp/project",
        __pilotConfiguredModel: "gpt-5.4-mini",
        __pilotConfigFingerprint: "v1:sha256:abc",
        __pilotConfigFingerprintVersion: 1,
        __pilotConfigCategories: ["adapterConfig"],
        __pilotConfigCategoryFingerprints: { adapterConfig: "v1:sha256:def" },
      }),
    ).toEqual({
      sessionId: "thread-1",
      cwd: "/tmp/project",
...
describe("comment wake batching", () => {
  it("preserves ordered wake comment ids when coalescing queued follow-up wakes", () => {
    ...
    expect(merged.pilotWake).toBeUndefined();
  });
...
describe("formatRuntimeWorkspaceWarningLog", () => {
  it("emits informational workspace warnings on stdout", () => {
    expect(formatRuntimeWorkspaceWarningLog("Using fallback workspace")).toEqual({
      stream: "stdout",
      chunk: "[pilot] Using fallback workspace\n",
    });
  });
});
...
describe("parseSessionCompactionPolicy", () => {
  it("disables Pilot-managed rotation by default for codex and claude local", () => {
