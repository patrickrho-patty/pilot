...
describeEmbeddedPostgres("tool gateway acceptance", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-tool-gateway-");
  ...
  it("exposes a named gateway with scoped bearer-token auth and revocation", async () => {
    ...
    try {
      ...
      expect(token.tokenPrefix).toMatch(/^pilotgw_[a-f0-9]{8}$/);
  ...
  it("throttles named gateway bearer auth failures without leaking bearer material", async () => {
    ...
    const app = createGatewayRouteApp(db, gateway);
    const badToken = `pilotgw_${randomUUID()}.not-a-real-secret`;

    const first = await request(app)
      .post(`/mcp/gateways/${created.gatewayPublicId}`)
      .set("authorization", `Bearer ${badToken}`)
      .set("x-pilot-client-name", "Noisy client")
    ...
    expect(audits[0]!.details).toMatchObject({
      ...
      tokenPrefix: badToken.slice(0, "pilotgw_".length + 8),
    });
  ...
  it("prunes expired persisted public gateway auth limiter counters", async () => {
    ...
    await request(app)
      .post(endpoint)
      .set("authorization", `Bearer pilotgw_${randomUUID()}.bad-secret`)
      .send({ jsonrpc: "2.0", id: 1, method: "tools/list" })
      .expect(401);
  ...
  it("shares public gateway auth limiter counters across service instances", async () => {
    ...
    const throttled = await request(createGatewayRouteApp(db, serviceB))
      .post(`/mcp/gateways/${created.gatewayPublicId}`)
      .set("authorization", `Bearer ${badToken}`)
      .set("x-pilot-client-name", "Shared counter client")
  ...
  it("passes only approved env values to local stdio MCP processes", async () => {
    ...
    process.env.DATABASE_URL = "postgres://server-secret.example/pilot";
    try {
      const company = await createCompany(db);
      const agent = await createAgent(db, company.id);
      const { run } = await createIssueAndRun(db, company.id, agent.id);
...
`,
  ...
  it("executes a connected remote HTTP MCP tool with stored credentials and redacted audit state", async () => {
    ...
    try {
      ...
      // Streamable HTTP requires advertising both JSON and SSE on the call (PIL-11096).
  ...
  it("keeps managed credentials authoritative even when legacy override flags are set", async () => {
    ...
    const fake = await startFakeRemoteMcpServer((fakeRequest) => {
      ...
      expect(fakeRequest.headers["x-pilot-agent-id"]).toBe(agent.id);
      expect(fakeRequest.headers["x-pilot-issue-id"]).toBe(issue.id);
      expect(fakeRequest.headers["x-pilot-tool-gateway-token"]).toBeUndefined();
    ...
    try {
        ...
        .set({
          config: {
            ...
            headerPolicy: {
              ...
              passthrough: {
                allowedHeaders: ["x-client-request-id", "authorization", "x-pilot-tool-gateway-token"],
                allowManagedCredentialOverride: true,
              },
      ...
      await gateway.executeTool({
        ...
        callerHeaders: {
          ...
          "x-pilot-tool-gateway-token": "caller-session-token",
      ...
      expect(activity.details).toMatchObject({
        headerSummary: {
          ...
          metadataHeaderNames: ["x-pilot-agent-id", "x-pilot-issue-id"],
          collisionRules: expect.arrayContaining([
            { header: "authorization", source: "caller", action: "kept_managed_credential" },
            { header: "x-pilot-tool-gateway-token", source: "caller", action: "dropped_sensitive_header" },
          ]),
        },
      });
      const persisted = JSON.stringify({
  ...
  it("drops auth-bearing and Pilot session headers from passthrough allowlists", async () => {
    const company = await createCompany(db);
    const agent = await createAgent(db, company.id);
    const { run } = await createIssueAndRun(db, company.id, agent.id);
    const fake = await startFakeRemoteMcpServer((fakeRequest) => {
    ...
    try {
      ...
      await gateway.executeTool({
        ...
        callerHeaders: {
          ...
          "x-pilot-tool-gateway-token": "drop-gateway-token",
        },
      });
  ...
  it("decodes an SSE-framed tools/call response from a spec-compliant Streamable HTTP server", async () => {
    ...
    // types, and replies with an SSE-framed body (PIL-11096).
    const fake = await startFakeRemoteMcpServer((fakeRequest) => {
      ...
      expect(approvedCompletion?.details).toMatchObject({
        ...
        execution: {
          ...
          request: {
            ...
            requestId: expect.stringMatching(/^pilot-tool-/),
            upstreamToolName: "kv_set",
            dispatched: true,
          },
          response: {
  ...
  it("rejects gateway session tokens passed through query strings", async () => {
    ...
    const listWithHeaderToken = await request(app)
      .get("/api/tool-gateway/tools")
      .set("x-pilot-tool-gateway-token", session.token);
  ...
  it("scopes agent gateway session revocation to the authenticated run", async () => {
    ...
    const otherRunStillActive = await request(app)
      .get("/api/tool-gateway/tools")
      .set("x-pilot-tool-gateway-token", otherRunSession.token);
  ...
  it("lazy-starts, reuses, and idles down the local stdio fixture slot", async () => {
    ...
    expect(idleSlot).toMatchObject({
      status: "idle",
      commandTemplateKey: "pilot.slow-stateful-stdio",
      healthStatus: "ok",
    });
