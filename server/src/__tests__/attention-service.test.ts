...
describeEmbeddedPostgres("attention service", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-attention-service-");
  ...
  it("returns ranked decision-only items for every active source and excludes non-human or transient rows", async () => {
    ...
    const blockerLeafId = await insertIssue({
      ...
      // PIL-16506: the /decisions review card states who may give the verdict,
    ...
    await db.insert(joinRequests).values({
      ...
      requestIp: "127.0.0.1",
      requestEmailSnapshot: "new@pilot.test",
    ...
    expect(feed.items.find((item) =>
      ...
      // A stalled review resolves in-row on the /decisions card (PIL-16080 §4.4).
  ...
  // PIL-17287: a collapsed queue row offers Accept/Reject before anything fetches
  ...
  // claimed the task was blocked by itself ("PIL-23 — Blocked by PIL-23").
