...
/**
 * Regression coverage for PIL-17249. The adverse report on PIL-17207 left an execution
...
describeEmbeddedPostgres("managed runtime-control operation recovery", () => {
  ...
  beforeAll(async () => {
    const started = await startEmbeddedPostgresTestDatabase("pilot-runtime-control-recovery-");
    stopDb = started.cleanup;
    db = createDb(started.connectionString);
    logRoot = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-runtime-control-logs-"));
