...
describeEmbeddedPostgres("issueReferenceService", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-issue-refs-");
  ...
  it("tracks outbound and inbound references across issue fields, comments, and documents", async () => {
    ...
    await db.insert(issues).values([
      {
        id: sourceIssueId,
        companyId,
        title: "Coordinate PIL-2",
        description: "Review /issues/pil-3 and ignore PIL-1 self references.",
        status: "todo",
        priority: "medium",
        identifier: "PIL-1",
      },
      ...
      },
      {
        ...
        description: "This one depends on PIL-1.",
        status: "in_progress",
        priority: "high",
        identifier: "PIL-4",
      },
    ...
    await db.insert(issueComments).values({
      ...
      body: "Follow up in https://pilot.test/issues/pil-2 after the document lands.",
    });
    ...
    await db.insert(documents).values({
      ...
      title: "Plan",
      format: "markdown",
      latestBody: "Spec note: /PIL/issues/PIL-3",
    ...
    expect(summary.outbound.map((item) => item.issue.identifier)).toEqual(["PIL-2", "PIL-3"]);
    ...
    expect(summary.inbound.map((item) => item.issue.identifier)).toEqual(["PIL-4"]);

    await refs.deleteDocumentSource(documentId);

    const withoutDocument = await refs.listIssueReferenceSummary(sourceIssueId);
    const pil3 = withoutDocument.outbound.find((item) => item.issue.identifier === "PIL-3");

    expect(pil3?.mentionCount).toBe(1);
    expect(pil3?.sources.map((source) => source.label)).toEqual(["description"]);
  });
  ...
  it("backfills existing references for a company without requiring write-time sync", async () => {
    ...
    await db.insert(issueComments).values({
      ...
      companyId,
      issueId: sourceIssueId,
      body: "Legacy comment points at PIL-20.",
    });
    ...
    await db.insert(documents).values({
      ...
      title: "Legacy plan",
      format: "markdown",
      latestBody: "Legacy plan also links /issues/PIL-20.",
      latestRevisionNumber: 1,
    });
    await db.insert(issueDocuments).values({
      id: issueDocumentId,
    ...
    expect(summary.outbound[0]?.issue.identifier).toBe("PIL-20");
    expect(summary.outbound[0]?.mentionCount).toBe(2);
