...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "pilot", LOGNAME: "pilot", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "pilot" }),
      },
    );

    expect(tokens).toEqual(["pilot", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "pilot" }, {
        userInfo: () => ({ username: "pilot" }),
      });

      expect(tokens).toEqual(["pilot", "custom-token"]);
    } finally {
