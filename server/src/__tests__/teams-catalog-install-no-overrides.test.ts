...
import { agents, companies, createDb } from "@pilotai/db";
import {
...
describeEmbeddedPostgres("teams catalog install with no caller adapter overrides", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-teams-catalog-no-overrides-"));
    process.env.PILOT_HOME = tempHome;
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-teams-catalog-no-overrides-");
