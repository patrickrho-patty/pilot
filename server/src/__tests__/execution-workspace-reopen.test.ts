...
describeEmbeddedPostgres("reopen archived isolated execution workspace", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-reopen-");
  ...
  async function makeExistingDir(): Promise<string> {
    const dir = await mkdtemp(join(tmpdir(), "pilot-reopen-cwd-"));
    tempDirs.push(dir);
    return dir;
  }
  ...
  async function seedCompanyProject() {
    ...
    await db.insert(companies).values({
      id: companyId,
      name: "Pilot",
      issuePrefix: `PIL-${companyId.slice(0, 8)}`,
  ...
  async function seedIssue(input: {
    ...
    await db.insert(issues).values({
      ...
      companyId: input.companyId,
      projectId: input.projectId,
      identifier: `PIL-${input.issueNumber}`,
  ...
  it("fails closed and keeps the row closed when the rebuild fails", async () => {
    ...
    const missingDir = join(tmpdir(), `pilot-reopen-missing-${randomUUID()}`);
  ...
  it("resolves the managed base checkout for a git_worktree row when the project workspace cwd is null", async () => {
    ...
    const tempHome = await mkdtemp(join(tmpdir(), "pilot-reopen-home-"));
