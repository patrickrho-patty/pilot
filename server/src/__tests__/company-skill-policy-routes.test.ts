...
import { activityLog, agents, companies, companySkillPolicies, createDb } from "@pilotai/db";
...
describeEmbeddedPostgres("company skill policy routes", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-skill-policy-routes-");
