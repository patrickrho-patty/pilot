...
import { COMPANY_IMPORT_TRANSFERS_ROUTE_PATH } from "@pilotai/shared/company-import-transfer";
...
describe("openapi routes", () => {
  it("serves the generated OpenAPI document", async () => {
    ...
    expect(res.body.info.title).toBe("Pilot API");
  ...
  it("documents auth and reviewed response-code invariants", () => {
    ...
    expect(spec.paths["/api/plugins/install"].post["x-pilot-authorization"]).toEqual({
      actor: "board",
      instanceAdmin: true,
    });
    ...
    expect(spec.paths["/api/execution-workspaces/{id}/reconcile-branch"].post["x-pilot-authorization"]).toEqual({
      actor: "board",
    });
