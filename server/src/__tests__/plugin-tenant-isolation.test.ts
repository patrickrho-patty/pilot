...
describeEmbeddedPostgres("plugin tenant isolation (company_id FK)", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-plugin-tenant-isolation-");
  ...
  async function seedPlugin() {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "pilot.tenant-isolation-test",
      packageName: "@pilotai/plugin-tenant-isolation-test",
      version: "0.0.1",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
        id: "pilot.tenant-isolation-test",
        apiVersion: 1,
        version: "0.0.1",
