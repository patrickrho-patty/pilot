...
import type { PilotSkillEntry } from "@pilotai/adapter-utils/server-utils";
import {
...
describeEmbeddedPostgres("heartbeat runtime skill version pins", () => {
  ...
  beforeAll(async () => {
    ...
    pilotHome = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-runtime-skills-home-"));
    process.env.PILOT_HOME = pilotHome;
    // The server normalizes PILOT_API_URL into its own env at boot
    ...
    registerServerAdapter({
      ...
      execute: async (ctx) => {
        ...
        capturedRuns.push({
          ...
          skills: (ctx.config.pilotRuntimeSkills ?? []) as PilotSkillEntry[],
  ...
  it("materializes different pinned skill versions for different agents at runtime", async () => {
    ...
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-versioned-runtime-skill-"));
  ...
  it("delivers installed connections without exposing gateway bearers in adapter config or logs", async () => {
    ...
    expect(captured?.mcpServers[0]).toMatchObject({
      connectionId: installed!.id,
      name: installed!.name,
      token: expect.stringMatching(/^pilotgw_/),
    ...
    expect(bearer).toMatch(/^pilotgw_/);
    if (!bearer) throw new Error("Expected runtime MCP bearer");
    expect(captured?.config).not.toHaveProperty("pilotRuntimeMcpServers");
    ...
    expect(log.content).not.toContain("pilotgw_");
  });
});
