...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-execution-workspace-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.name", "Pilot Test"]);
  await runGit(repoRoot, ["config", "user.email", "test@pilot.local"]);
...
describeEmbeddedPostgres("executionWorkspaceService.getCloseReadiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-execution-workspaces-service-");
  ...
  async function seedTerminalWorkspace(options: {
    ...
    const worktreePath = path.join(path.dirname(repoRoot), `pilot-terminal-${randomUUID()}`);
    tempDirs.add(repoRoot);
    tempDirs.add(worktreePath);
    await runGit(repoRoot, ["branch", "PIL-16015-delivery"]);
    await runGit(repoRoot, ["worktree", "add", worktreePath, "PIL-16015-delivery"]);
    ...
    if (options.mergedPr) {
      await db.insert(issueWorkProducts).values({
        ...
        provider: "github",
        title: "Delivered PR",
        url: "https://github.com/pilotai/pilot/pull/10623",
        status: "merged",
      });
    }
    pullRequestDetailsByKey.set(`${companyId}:10623`, {
      state: "merged",
      headRef: "PIL-16015-delivery",
      headSha,
    });
  ...
  it("reports a squash cross-branch delivery as merged_via_pr and suppresses the ancestry warning", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `pilot-delivery-${randomUUID()}`);
    tempDirs.add(worktreePath);
    await runGit(repoRoot, ["branch", "PIL-16015-delivery"]);
    await runGit(repoRoot, ["worktree", "add", worktreePath, "PIL-16015-delivery"]);
    await fs.writeFile(path.join(worktreePath, "delivered.txt"), "delivered\n", "utf8");
  ...
  async function seedAncestryTerminalWorkspace(overrides: { updatedAt?: Date } = {}) {
    ...
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `pilot-ancestry-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("fails closed before archive when git status inspection is unavailable", async () => {
    ...
    try {
      ...
      expect(readiness).toMatchObject({
        ...
        blockingReasons: [
          "Pilot could not verify the workspace git status. Retry before destructive cleanup.",
        ],
      });
  ...
  it("does not treat an unrelated inbound issue mention as delivery evidence", async () => {
    ...
    await db.insert(issueWorkProducts).values({
      ...
      url: "https://github.com/pilotai/pilot/pull/10624",
      status: "merged",
    });
  ...
  it("does not trust directly linked merged products for a different repository or branch", async () => {
    ...
    await db.insert(issueWorkProducts).values([
      ...
      },
      {
        ...
        url: "https://github.com/unrelated/pilot/pull/10623",
        status: "merged",
      },
    ]);
  ...
  it("holds Git index and ref locks across terminal cleanup", async () => {
    ...
    expect(refUpdateFailure).toMatch(/HEAD\.lock|refs\/heads\/PIL-16015-delivery\.lock/);
    expect(sweep).toMatchObject({ archived: 1, cleanupFailed: 0 });
    expect(workspace).toMatchObject({ status: "archived", cleanupReason: "issue_terminal" });
    await expect(fs.access(seeded.worktreePath)).rejects.toThrow();
    expect(await readGit(seeded.repoRoot, ["branch", "--list", "PIL-16015-delivery"])).toBeNull();
  });
  ...
  it("does not treat a descendant PR on the shared workspace as parent delivery evidence", async () => {
    ...
    await db.insert(issueWorkProducts).values({
      ...
      url: "https://github.com/pilotai/pilot/pull/10625",
      status: "merged",
    });
  ...
  it("reconciles a forward branch record, comments on the source issue, and resolves matching workspace recovery", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `pilot-reconcile-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("reconciles forward when the recorded branch has no resolvable commit and the worktree is clean", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `pilot-missing-recorded-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("keeps forward reconciliation fail-closed when the recorded branch is missing but the worktree is dirty", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `pilot-missing-recorded-dirty-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("keeps forward reconciliation fail-closed when the checked-out branch ref does not resolve either", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `pilot-missing-both-refs-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("quarantine_restore rescues dirty live-branch work, resolves recovery, and returns the source issue to todo", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `pilot-quarantine-restore-${randomUUID()}`);
    tempDirs.add(worktreePath);
    ...
    expect(result.rescueRef).toMatchObject({
      branchName: expect.stringMatching(/^pilot\/rescue\/PIL-124\/\d{8}T\d{6}Z$/),
      fileCount: 2,
    });
  ...
  it("quarantine_restore rejects active runtime services before creating a rescue branch", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `pilot-quarantine-running-${randomUUID()}`);
    tempDirs.add(worktreePath);
    ...
    await expect(readGit(
      repoRoot,
      ["for-each-ref", "--format=%(refname:short)", "refs/heads/pilot/rescue"],
    )).resolves.toBeNull();
  ...
  it.each(["review", "approval"] as const)(
    ...
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `pilot-quarantine-${stageType}-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it.each([
    {
      ...
      claimantIssueIdentifier: "PIL-126",
      claimantHasActiveRun: true,
      expectedReason: "active run",
    },
    {
      claimantLabel: "idle",
      claimantIssueIdentifier: "PIL-127",
    ...
    async ({ claimantIssueIdentifier, claimantHasActiveRun, expectedReason }) => {
      const repoRoot = await createTempRepo();
      tempDirs.add(repoRoot);
      const worktreePath = path.join(path.dirname(repoRoot), `pilot-quarantine-claimant-${randomUUID()}`);
      tempDirs.add(worktreePath);
      ...
      const claimantWorkspacePath = path.join(path.dirname(repoRoot), `pilot-claimant-${randomUUID()}`);
      const now = new Date();
  ...
  it("rejects branch reconciliation when the worktree is dirty", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `pilot-dirty-reconcile-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("rejects branch reconciliation while the workspace lifecycle is active", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `pilot-active-reconcile-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("rejects branch reconciliation if the workspace becomes active before the branch record update", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `pilot-race-reconcile-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("rejects branch reconciliation while runtime services are active", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `pilot-running-reconcile-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("rejects branch reconciliation when a runtime service starts before the locked update", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `pilot-raced-service-reconcile-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("rejects branch reconciliation when runtime service activation is already spawning", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `pilot-spawning-service-reconcile-${randomUUID()}`);
    tempDirs.add(worktreePath);
    ...
    const issueId = randomUUID();
    const executionWorkspaceId = randomUUID();
    const runtimeStartedMarker = path.join(os.tmpdir(), `pilot-runtime-started-${randomUUID()}.marker`);
  ...
  it("rejects forward branch reconciliation for diverged branches", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `pilot-diverged-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("serializes the verified HTTPS URL as canonical and never falls back to the HTTP backend", async () => {
    // PIL-17158: the UI's workspace/project/issue launch links read `url` off the
    ...
    const hostname = "pilot-dev.tail29c1aa.ts.net";
    ...
    await db.insert(projectWorkspaces).values({
      ...
      metadata: {
        runtimeConfig: {
          workspaceRuntime: { services: [{ name: "pilot-dev", command: "pnpm dev" }] },
  ...
  it("warns about dirty and unmerged git worktrees and reports cleanup actions", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `pilot-worktree-${randomUUID()}`);
    tempDirs.add(worktreePath);

    await runGit(repoRoot, ["branch", "pilot-close-check"]);
    await runGit(repoRoot, ["worktree", "add", worktreePath, "pilot-close-check"]);
