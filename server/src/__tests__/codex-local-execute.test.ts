...
import { runChildProcess } from "@pilotai/adapter-utils/server-utils";
import { execute } from "@pilotai/adapter-codex-local/server";
...
const payload = {
  ...
  codexConfigContents: process.env.CODEX_HOME && fs.existsSync(process.env.CODEX_HOME + "/config.toml")
    ...
    : null,
  pilotWakePayloadJson: process.env.PILOT_WAKE_PAYLOAD_JSON || null,
  pilotApiUrl: process.env.PILOT_API_URL || null,
  pilotApiKey: process.env.PILOT_API_KEY || null,
  pilotApiBridgeMode: process.env.PILOT_API_BRIDGE_MODE || null,
  pilotEnvKeys: Object.keys(process.env)
    .filter((key) => key.startsWith("PILOT_") || key.startsWith("PILOT_"))
    .sort(),
};
if (capturePath) {
...
type CapturePayload = {
  ...
  pilotWakePayloadJson: string | null;
  pilotApiUrl?: string | null;
  pilotApiKey?: string | null;
  pilotApiBridgeMode?: string | null;
  pilotEnvKeys: string[];
};
...
describe("codex execute", () => {
  it("uses a Pilot-managed CODEX_HOME outside worktree mode while preserving shared auth and config", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-execute-default-"));
    ...
    const previousPilotInWorktree = process.env.PILOT_IN_WORKTREE;
    ...
    try {
      ...
      const result = await execute({
        ...
        config: {
          ...
          promptTemplate: "Follow the pilot heartbeat.",
        },
        context: {},
      ...
      expect(logs).toContainEqual(
        expect.objectContaining({
          stream: "stdout",
          chunk: expect.stringContaining("Using Pilot-managed Codex home"),
        }),
      );
    } finally {
      ...
      if (previousPilotInWorktree === undefined) delete process.env.PILOT_IN_WORKTREE;
      else process.env.PILOT_IN_WORKTREE = previousPilotInWorktree;
  ...
  it("writes managed MCP gateways into Codex config and warns on overlapping direct entries without logging tokens", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-managed-mcp-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    ...
    const previousHome = process.env.HOME;
    const previousPilotHome = process.env.PILOT_HOME;
    const previousPilotApiUrl = process.env.PILOT_API_URL;
    const previousPilotRuntimeApiUrl = process.env.PILOT_RUNTIME_API_URL;
    const previousCodexHome = process.env.CODEX_HOME;
    process.env.HOME = root;
    process.env.PILOT_HOME = pilotHome;
    process.env.PILOT_API_URL = "http://pilot.local:3100";
    process.env.PILOT_RUNTIME_API_URL = "http://pilot.local:3100";
    process.env.CODEX_HOME = sharedCodexHome;
    ...
    try {
      ...
      const result = await execute({
        ...
        runtimeMcp: {
          getServers: () => [
            {
              name: "github",
              url: "http://pilot.local:3100/api/tool-gateway/gateways/gateway-1/mcp",
              token: "pilotgw_secret-managed-token",
      ...
      expect(configText).toContain("[mcp_servers.\"pilot-github\"]");
      expect(configText).toContain('url = "http://pilot.local:3100/api/tool-gateway/gateways/gateway-1/mcp"');
      expect(configText).toContain('Authorization = "Bearer pilotgw_secret-managed-token"');
      expect(logs).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            stream: "stderr",
            chunk: expect.stringContaining("Pilot cannot enforce policies for that direct entry"),
          }),
        ]),
      );
      expect(JSON.stringify(logs)).not.toContain("pilotgw_secret-managed-token");
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
      if (previousPilotHome === undefined) delete process.env.PILOT_HOME;
      else process.env.PILOT_HOME = previousPilotHome;
      if (previousPilotApiUrl === undefined) delete process.env.PILOT_API_URL;
      else process.env.PILOT_API_URL = previousPilotApiUrl;
      if (previousPilotRuntimeApiUrl === undefined) delete process.env.PILOT_RUNTIME_API_URL;
      else process.env.PILOT_RUNTIME_API_URL = previousPilotRuntimeApiUrl;
      if (previousCodexHome === undefined) delete process.env.CODEX_HOME;
      else process.env.CODEX_HOME = previousCodexHome;
      await fs.rm(root, { recursive: true, force: true });
    }
  ...
  it("emits a command note that Codex auto-applies repo-scoped AGENTS.md files", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-execute-notes-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      expect(commandNotes).toContain(
        "Codex exec automatically applies repo-scoped AGENTS.md instructions from the current workspace; Pilot does not currently suppress that discovery.",
      );
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
  ...
  it("logs HOME and the resolved executable path in invocation metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-execute-meta-"));
    const workspace = path.join(root, "workspace");
  ...
  it("injects bridge env into sandbox-managed remote runs", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-execute-sandbox-"));
    ...
    try {
      ...
      const capture = JSON.parse(await fs.readFile(capturePath, "utf8")) as CapturePayload;
      expect(capture.codexHome).toBe(path.join(remoteWorkspace, ".pilot-runtime", "codex", "home"));
      expect(capture.pilotApiUrl).toMatch(/^http:\/\/127\.0\.0\.1:\d+$/);
      expect(capture.pilotApiKey).not.toBe("run-jwt-token");
      expect(capture.pilotApiBridgeMode).toBe("queue_v1");
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
      if (previousPath === undefined) delete process.env.PATH;
  ...
  it("injects structured Pilot wake payloads into env and prompt", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-execute-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      const capture = JSON.parse(await fs.readFile(capturePath, "utf8")) as CapturePayload;
      expect(capture.pilotEnvKeys).toContain("PILOT_WAKE_PAYLOAD_JSON");
      expect(capture.pilotWakePayloadJson).not.toBeNull();
      expect(JSON.parse(capture.pilotWakePayloadJson ?? "{}")).toMatchObject({
        reason: "issue_commented",
        latestCommentId: "comment-2",
        commentIds: ["comment-1", "comment-2"],
      });
      expect(capture.prompt).toContain("## Pilot Wake Payload");
  ...
  it("classifies remote-compaction high-demand failures as retryable transient upstream errors", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-execute-transient-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("classifies mid-turn harness crashes as retryable transient upstream errors", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-execute-harness-crash-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    await fs.mkdir(workspace, { recursive: true });
...
`;
  ...
  it("persists retry-not-before metadata for codex provider quota failures", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-execute-usage-limit-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingCodexCommand(
  ...
  it("classifies Codex refresh-token auth failures without credential telemetry", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-execute-refresh-token-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("uses safer invocation settings and a fresh-session handoff for codex transient fallback retries", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-execute-fallback-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      expect(capture.prompt).toContain("Pilot session handoff:");
  ...
  it("renders execution-stage wake instructions for reviewer and executor roles", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-execute-stage-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
        ...
        context: {
          ...
          pilotWake: {
            ...
            issue: {
              id: "issue-1",
              identifier: "PIL-1207",
              title: "implement the plan of PIL-1200",
              status: "in_review",
              priority: "medium",
            },
            executionStage: {
      ...
      const executorResult = await execute({
        ...
        config: {
          ...
          cwd: workspace,
          env: {
            PILOT_TEST_CAPTURE_PATH: executorCapturePath,
          },
          promptTemplate: "Follow the pilot heartbeat.",
        },
        context: {
  ...
  it("renders an issue-scoped wake prompt even when the wake has no comments yet", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-execute-issue-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      expect(capture.prompt).toContain("## Pilot Wake Payload");
      expect(capture.prompt).toContain("Do not switch to another issue until you have handled this wake.");
      expect(capture.prompt).toContain("- issue: PIL-1201 Fix gallery opening for inline images");
  ...
  it("uses a compact wake delta instead of the full heartbeat prompt when resuming a session", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-execute-resume-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    ...
    try {
      ...
      expect(capture.prompt).toContain("## Pilot Resume Delta");
      expect(capture.prompt).toContain("Do not switch to another issue until you have handled this wake.");
      expect(capture.prompt).toContain("Second comment");
      expect(capture.prompt).not.toContain("Follow the pilot heartbeat.");
      expect(capture.prompt).not.toContain("You are managed instructions.");
      expect(invocationPrompt).toContain("## Pilot Resume Delta");
  ...
  it("uses a worktree-isolated CODEX_HOME while preserving shared auth and config", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-execute-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    ...
    const homeSkill = path.join(isolatedCodexHome, "skills", "pilot");
    await fs.mkdir(workspace, { recursive: true });
    await fs.mkdir(sharedCodexHome, { recursive: true });
    await fs.writeFile(path.join(sharedCodexHome, "auth.json"), `${fakeCodexAuthJson}\n`, "utf8");
    await fs.writeFile(path.join(sharedCodexHome, "config.toml"), 'model = "codex-mini-latest"\n', "utf8");
    ...
    process.env.HOME = root;
    process.env.PILOT_HOME = pilotHome;
    process.env.PILOT_INSTANCE_ID = "worktree-1";
    ...
    try {
      ...
      expect(capture.argv).toEqual(expect.arrayContaining(["exec", "--json", "-"]));
      expect(capture.prompt).toContain("Follow the pilot heartbeat.");
      expect(capture.pilotEnvKeys).toEqual(
        expect.arrayContaining([
          "PILOT_AGENT_ID",
          "PILOT_API_KEY",
          "PILOT_API_URL",
      ...
      expect(logs).toContainEqual(
        expect.objectContaining({
          stream: "stdout",
          chunk: expect.stringContaining('Injected Codex skill "pilot"'),
        }),
      );
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
  ...
  it("respects an explicit CODEX_HOME config override even in worktree mode", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-execute-explicit-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    ...
    try {
      ...
      expect((await fs.lstat(path.join(explicitCodexHome, "skills", "pilot"))).isSymbolicLink()).toBe(true);
      await expect(fs.lstat(path.join(pilotHome, "instances", "worktree-1", "codex-home"))).rejects.toThrow();
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
      if (previousPilotHome === undefined) delete process.env.PILOT_HOME;
