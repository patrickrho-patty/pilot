...
describe("ui branding", () => {
  it("detects worktree mode from PILOT_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ PILOT_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ PILOT_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ PILOT_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      PILOT_IN_WORKTREE: "true",
      PILOT_WORKTREE_NAME: "pilot-pr-432",
      PILOT_WORKTREE_COLOR: "#4f86f7",
    });

    expect(branding.enabled).toBe(true);
    expect(branding.name).toBe("pilot-pr-432");
  ...
  it("renders runtime branding metadata for the ui", () => {
    ...
    expect(meta).toContain('name="pilot-worktree-name"');
    expect(meta).toContain('content="pilot-pr-432"');
    expect(meta).toContain('name="pilot-worktree-color"');
  });
  ...
  it("omits the instance-id meta when the runtime id is unset", () => {
    ...
    expect(renderRuntimeBrandingMeta(branding)).not.toContain('name="pilot-instance-id"');
  });
  ...
  it("rewrites the favicon and runtime branding blocks for worktree instances only", () => {
    ...
    expect(branded).toContain('name="pilot-worktree-name"');
