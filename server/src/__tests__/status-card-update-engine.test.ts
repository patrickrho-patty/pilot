...
import { statusCardRefreshPolicySchema } from "@pilotai/shared";
import {
...
describe("status card update engine", () => {
  ...
  it("retains non-terminal and terminal status transitions plus membership changes", () => {
    const changes = diffStatusCardFingerprint({
      churn: { status: "todo", updatedAt: "2026-07-23T10:00:00.000Z", identifier: "PIL-1", title: "Churn" },
      done: { status: "in_progress", updatedAt: "2026-07-23T10:00:00.000Z", identifier: "PIL-2", title: "Done" },
      removed: { status: "blocked", updatedAt: "2026-07-23T10:00:00.000Z", identifier: "PIL-3", title: "Removed" },
    }, {
      churn: { status: "in_progress", updatedAt: "2026-07-23T10:01:00.000Z", identifier: "PIL-1", title: "Churn" },
      done: { status: "done", updatedAt: "2026-07-23T10:01:00.000Z", identifier: "PIL-2", title: "Done" },
      added: { status: "todo", updatedAt: "2026-07-23T10:01:00.000Z", identifier: "PIL-4", title: "Added" },
    });
  ...
  it("tracks human comments independently from generic issue updates", () => {
    const previous = {
      issue: { status: "in_progress", updatedAt: "2026-07-23T10:00:00.000Z", latestHumanCommentAt: null, identifier: "PIL-5", title: "Commented" },
    };
    const current = {
      issue: { status: "done", updatedAt: "2026-07-23T10:01:00.000Z", latestHumanCommentAt: "2026-07-23T10:01:00.000Z", identifier: "PIL-5", title: "Commented" },
    };
    ...
    expect(filterStatusCardChanges(changes, commentOnlyPolicy)).toMatchObject([{ identifier: "PIL-5", changeKind: "human_comment" }]);
  });

  it("changes the pending signature when equal-sized change sets are replaced", () => {
    const first = [{ issueId: "one", identifier: "PIL-1", title: "One", from: "todo", to: "done", changeKind: "status" as const }];
    const second = [{ issueId: "two", identifier: "PIL-2", title: "Two", from: "todo", to: "done", changeKind: "status" as const }];
  ...
  it("extracts identifier and issue-link mentions from summary markdown", () => {
    const markdown = [
      "**Decide:** [PIL-15357](/issues/PIL-15357) is blocked; PIL-15357 and pil-99 (lowercase) plus SC2-4 moved.",
