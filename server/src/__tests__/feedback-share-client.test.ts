...
describe("feedback trace share client", () => {
  ...
  it("defaults to telemetry.pilot.test when no backend url is configured", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "https://telemetry.pilot.test/feedback-traces",
  ...
  it("wraps the feedback trace payload as gzip+base64 json before upload", async () => {
    const client = createFeedbackTraceShareClientFromConfig({
      feedbackExportBackendUrl: "https://telemetry.pilot.test",
    ...
    expect(call?.[0]).toBe("https://telemetry.pilot.test/feedback-traces");
