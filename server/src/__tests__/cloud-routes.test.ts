...
describe("GET /api/cloud/stacks", () => {
  it("returns the actor's portfolio without forwarding client-supplied identity", async () => {
    ...
    const res = await request(app)
      ...
      .set("x-pilot-cloud-user-id", "spoofed-header-user")
    ...
    expect(init?.headers).toEqual({
      ...
      "x-pilot-cloud-user-id": "actor-user",
      "x-pilot-cloud-stack-id": "stack-current",
    });
  ...
  it("keeps cache entries isolated by the server-derived actor user id", async () => {
    const fetchImpl = vi.fn<typeof fetch>().mockImplementation(async (_url, init) => {
      ...
      return jsonResponse({ userId: headers.get("x-pilot-cloud-user-id") });
    });
    const app = express();
    app.use((req, _res, next) => {
