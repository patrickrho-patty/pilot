...
describe("resolveEnvironmentExecutionTarget", () => {
  ...
  it("keeps sandbox targets on bridge mode even when lease metadata includes a Pilot API URL", async () => {
    mockResolveEnvironmentDriverConfigForRuntime.mockResolvedValue({
      driver: "sandbox",
      config: {
        provider: "fake-plugin",
    ...
    const target = await resolveEnvironmentExecutionTarget({
      ...
      leaseId: "lease-1",
      leaseMetadata: {
        pilotApiUrl: "https://pilot.example.test",
      },
      lease: null,
      environmentRuntime: null,
    });
    ...
    expect(target).not.toHaveProperty("pilotApiUrl");
    expect(target).not.toHaveProperty("pilotTransport");
  });
  ...
  it("keeps sandbox targets on callback bridge execution even when lease metadata advertises SSH access", async () => {
    ...
    const target = await resolveEnvironmentExecutionTarget({
      ...
      leaseId: "lease-1",
      leaseMetadata: {
        remoteCwd: "/home/sandbox/pilot-workspace",
        sshAccess: {
          type: "ssh",
  ...
  // span ended. Each bridge step (`bridge.pilot`, `bridge.process-session`)
  ...
  it("opens an unparented exec span for a pilot bridge continuation", async () => {
    const execSpan = await runContinuationExec("bridge.pilot", { wrap: true });
    expect(execSpan).toBeTruthy();
    expect(execSpan!.parent).toBeNull();
  });
