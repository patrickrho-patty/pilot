...
import { deriveViteHmrPort, type Environment, type EnvironmentLease } from "@pilotai/shared";
import { resolvePilotConfigPath } from "../paths.ts";
import type { WorkspaceOperation } from "@pilotai/shared";
...
async function writeRegisteredSourceConfig(baseCwd: string, instanceId = "source-instance") {
  const configDir = path.join(baseCwd, ".pilot");
  await fs.mkdir(configDir, { recursive: true });
  await fs.writeFile(path.join(configDir, "config.json"), "{}\n", "utf8");
  await fs.writeFile(
    path.join(configDir, ".env"),
    `PILOT_INSTANCE_ID=${instanceId}\n`,
    "utf8",
  );
}

async function createTempRepo(defaultBranch = "main") {
  const repoRoot = await mktemp("pilot-worktree-repo-");
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.email", "pilot@example.com"]);
  await runGit(repoRoot, ["config", "user.name", "Pilot Test"]);
...
async function createClonedRepoWithRemote() {
  ...
  const remoteDir = await mktemp("pilot-worktree-remote-");
  const remotePath = path.join(remoteDir, "pilot.git");
  await execFileAsync("git", ["clone", "--bare", sourceRepo, remotePath]);

  const cloneRoot = await mktemp("pilot-worktree-clone-");
  const repoRoot = path.join(cloneRoot, "pilot");
...
describe("sanitizeRuntimeServiceBaseEnv", () => {
  it("removes inherited Pilot and pnpm auth flags before spawning runtime services", () => {
    const sanitized = sanitizeRuntimeServiceBaseEnv({
      PATH: process.env.PATH,
      DATABASE_URL: "postgres://example.test/pilot",
...
describe("resolveRuntimeProvisionCommand", () => {
  it("backfills deferred seeding for legacy managed git worktrees", async () => {
    const baseCwd = await mktemp("pilot-runtime-provision-");
    ...
    try {
      ...
      await fs.mkdir(path.join(cwd, ".pilot"), { recursive: true });
      await fs.writeFile(path.join(cwd, ".pilot", "config.json"), "{}\n");
      expect(resolveRuntimeProvisionCommand({ config: {}, workspace })).toBe(
        "bash ./scripts/provision-worktree-runtime.sh",
      );

      await fs.writeFile(path.join(cwd, ".pilot", "seed-pending"), "{}\n");
      expect(resolveRuntimeProvisionCommand({ config: {}, workspace })).toBe(
        "bash ./scripts/provision-worktree-runtime.sh",
      );
      ...
      await fs.writeFile(path.join(cwd, ".pilot", "seed-complete"), "{}\n");
      expect(resolveRuntimeProvisionCommand({ config: {}, workspace })).toBe(
        "bash ./scripts/provision-worktree-runtime.sh",
      );

      await fs.writeFile(
        path.join(cwd, ".pilot", "seed-manifest.json"),
...
describe("refreshRemoteTrackingBaseRef git auth", () => {
  ...
  it("attributes a failed authenticated fetch to the credential that was used", async () => {
    ...
    await runGit(repoRoot, ["remote", "set-url", "origin", path.join(os.tmpdir(), "pilot-missing-remote", "repo.git")]);
...
describe("ensureServerWorkspaceLinksCurrent", () => {
  it("relinks stale server workspace dependencies inside the current repo root", async () => {
    const repoRoot = await mktemp("pilot-runtime-links-");
    const staleRoot = await mktemp("pilot-runtime-links-stale-");
    const serverNodeModulesScopeDir = path.join(repoRoot, "server", "node_modules", "@pilotai");
    ...
    await fs.writeFile(path.join(repoRoot, ".git"), "gitdir: /tmp/pilot-main/.git/worktrees/runtime-links\n", "utf8");
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "@pilotai/db" }),
      "utf8",
    );
    await fs.writeFile(
  ...
  it("skips relinking when server workspace dependencies already point at the repo", async () => {
    const repoRoot = await mktemp("pilot-runtime-links-current-");
    const serverNodeModulesScopeDir = path.join(repoRoot, "server", "node_modules", "@pilotai");
    const expectedPackageDir = path.join(repoRoot, "packages", "db");
    ...
    await fs.mkdir(expectedPackageDir, { recursive: true });
    await fs.mkdir(serverNodeModulesScopeDir, { recursive: true });
    await fs.writeFile(path.join(repoRoot, ".git"), "gitdir: /tmp/pilot-main/.git/worktrees/runtime-links-current\n", "utf8");
    await fs.writeFile(path.join(repoRoot, "pnpm-workspace.yaml"), "packages:\n  - packages/*\n  - server\n", "utf8");
    await fs.writeFile(
      path.join(repoRoot, "server", "package.json"),
      JSON.stringify({
  ...
  it("skips relinking outside linked git worktrees", async () => {
    const repoRoot = await mktemp("pilot-runtime-links-non-worktree-");
    const staleRoot = await mktemp("pilot-runtime-links-non-worktree-stale-");
    const serverNodeModulesScopeDir = path.join(repoRoot, "server", "node_modules", "@pilotai");
    const expectedPackageDir = path.join(repoRoot, "packages", "db");
    const stalePackageDir = path.join(staleRoot, "db");
...
describe("realizeExecutionWorkspace", () => {
  ...
  it("creates and reuses a git worktree for an issue-scoped branch", async () => {
    ...
    expect(first.branchName).toBe("PIL-447-add-worktree-support");
    expect(first.cwd).toContain(path.join(".pilot", "worktrees"));
  ...
  it("defaults the repo-provided worktree provisioner for git worktree strategies", async () => {
    ...
    await expect(
      fs.readFile(path.join(workspace.cwd, ".pilot", "default-provision-ran"), "utf8"),
  ...
  it("stops before git worktree add when the base ref is absent on origin", async () => {
    ...
    await expect(
      fs.stat(path.join(repoRoot, ".pilot", "worktrees", "PIL-447-add-worktree-support")),
    ).rejects.toThrow();
  });
  ...
  it("surfaces an authenticated fetch failure as an unresolved base ref, not a crash", async () => {
    ...
    await runGit(repoRoot, ["remote", "set-url", "origin", path.join(os.tmpdir(), "pilot-missing-remote.git")]);
  ...
  it("rejects reusing an empty directory that only looks like a worktree because it sits inside the repo", async () => {
    const repoRoot = await createTempRepo();
    const branchName = "PIL-447-add-worktree-support";
    const poisonedPath = path.join(repoRoot, ".pilot", "worktrees", branchName);
  ...
  it("reuses the current linked worktree instead of nesting another worktree inside it", async () => {
    const repoRoot = await createTempRepo();
    const branchName = "PIL-1355-worktree-reuse";
    const currentWorktree = path.join(repoRoot, ".pilot", "worktrees", branchName);
  ...
  it("repairs a clean linked worktree whose branch drifted from the expected issue branch", async () => {
    ...
    await expect(readGit(initial.cwd, ["branch", "--show-current"])).resolves.toBe("PIL-447-add-worktree-support");
    expect(operations).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          phase: "worktree_prepare",
          command: "git checkout PIL-447-add-worktree-support",
          metadata: expect.objectContaining({
            branchIncoherenceRepair: true,
            expectedBranchName: "PIL-447-add-worktree-support",
  ...
  it("reuses an already checked out branch from git worktree metadata even when the target path differs", async () => {
    const repoRoot = await createTempRepo();
    const branchName = "PIL-1355-worktree-reuse";
    const existingWorktree = path.join(repoRoot, ".pilot", "worktrees", branchName);
    const { recorder, operations } = createWorkspaceOperationRecorderDouble();
    ...
    const realized = await realizeExecutionWorkspace({
      ...
      config: {
        workspaceStrategy: {
          type: "git_worktree",
          branchTemplate: "{{issue.identifier}}-{{slug}}",
          worktreeParentDir: ".pilot/other-worktrees",
        },
      },
      issue: {
        id: "issue-1",
  ...
  it("slugifies unsafe issue titles for branch names and worktree folders", async () => {
    ...
    expect(realized.branchName).toBe(
      "PIL-991-there-should-be-a-setting-for-the-allowance-of-thumbs-up-thumbs-down-data-rm-rf",
    );
  ...
  it("preserves intentional slashes and dots from the branch template", async () => {
    ...
    expect(realized.branchName).toBe("release/PIL-992.hotfix-april-1");
    expect(path.basename(realized.cwd)).toBe("PIL-992.hotfix-april-1");
  });
  ...
  it("runs a configured provision command inside the derived worktree", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$PILOT_WORKSPACE_BRANCH\" > .pilot-provision-branch",
        "printf '%s\\n' \"$PILOT_WORKSPACE_BASE_CWD\" > .pilot-provision-base",
        "printf '%s\\n' \"$PILOT_WORKSPACE_CREATED\" > .pilot-provision-created",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".pilot-provision-branch"), "utf8")).resolves.toBe(
      "PIL-448-run-provision-command\n",
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".pilot-provision-base"), "utf8")).resolves.toBe(
      `${repoRoot}\n`,
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".pilot-provision-created"), "utf8")).resolves.toBe(
      "true\n",
    );
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".pilot-provision-created"), "utf8")).resolves.toBe("false\n");
  });
  ...
  it("uses the latest repo-managed provision script when reusing an existing worktree", async () => {
    ...
    await expect(fs.readFile(path.join(initial.cwd, ".pilot-provision-version"), "utf8")).resolves.toBe("v1\n");
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".pilot-provision-version"), "utf8")).resolves.toBe("v2\n");
  }, 30_000);

  it("writes an isolated repo-local Pilot config and worktree branding when provisioning", async () => {
    const repoRoot = await createTempRepo();
    ...
    const pilotHome = await mktemp("pilot-worktree-home-");
    const isolatedWorktreeHome = await mktemp("pilot-worktrees-");
    const isolatedBin = await mktemp("pilot-worktree-bin-");
    ...
    await fs.writeFile(sharedEnvPath, 'DATABASE_URL="postgres://worktree:test@db.example.com:6543/pilot"\n', "utf8");
    ...
    try {
      ...
      const configPath = path.join(workspace.cwd, ".pilot", "config.json");
      const envPath = path.join(workspace.cwd, ".pilot", ".env");
      ...
      expect(envVars.PILOT_WORKTREE_NAME).toBe("PIL-885-show-worktree-banner");
      ...
      await fs.writeFile(envPath, `${envContents}PILOT_WORKTREE_COLOR="#112233"\n`, "utf8");
      ...
      expect(reusedEnvContents).toContain('PILOT_WORKTREE_COLOR="#112233"');
    } finally {
  ...
  it("provisions successfully when install is needed but there are no symlinked node_modules to move", async () => {
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".pilot", "config.json"), "utf8")).resolves.toContain(
      "\"database\"",
    );
  }, 30_000);

  it("reinstalls worktree-local pnpm dependencies when package metadata changes", async () => {
    const tempRoot = await mktemp("pilot-worktree-stale-deps-");
  ...
  it("fails instead of writing an unseeded fallback config when worktree init errors after CLI detection succeeds", async () => {
    const tempRoot = await mktemp("pilot-worktree-provision-fail-");
    const baseRoot = path.join(tempRoot, "base");
    const worktreeRoot = path.join(tempRoot, "worktree");
    const fakeBin = path.join(tempRoot, "bin");
    const fakePnpmPath = path.join(fakeBin, "pnpm");
    ...
    try {
      ...
      await fs.writeFile(
        ...
        [
          ...
          "  exit 0",
          "fi",
          "if [ \"$1\" = \"pilotai\" ] && [ \"$2\" = \"worktree\" ] && [ \"$3\" = \"init\" ]; then",
      ...
      await expect(fs.stat(path.join(worktreeRoot, ".pilot", "config.json"))).rejects.toThrow();
      await expect(fs.stat(path.join(worktreeRoot, ".pilot", ".env"))).rejects.toThrow();
    } finally {
      await fs.rm(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("regenerates stale worktree config that points at another host", async () => {
    const tempRoot = await mktemp("pilot-worktree-stale-config-");
    const baseRoot = path.join(tempRoot, "base");
    const worktreeRoot = path.join(tempRoot, "worktree");
    const fakeBin = path.join(tempRoot, "bin");
    const fakePnpmPath = path.join(fakeBin, "pnpm");
    ...
    try {
      ...
      await fs.writeFile(
        ...
        JSON.stringify({
          database: {
            mode: "embedded-postgres",
            embeddedPostgresDataDir: "/Users/example/.pilot-worktrees/instances/stale/db",
          },
          logging: {
            mode: "file",
            logDir: "/Users/example/.pilot-worktrees/instances/stale/logs",
          },
          storage: {
            provider: "local_disk",
            localDisk: {
              baseDir: "/Users/example/.pilot-worktrees/instances/stale/data/storage",
            },
          },
          secrets: {
            provider: "local_encrypted",
            localEncrypted: {
              keyFilePath: "/Users/example/.pilot-worktrees/instances/stale/secrets/master.key",
            },
          },
        }),
        "utf8",
      ...
      await fs.writeFile(
        ...
        [
          "PILOT_HOME=/Users/example/.pilot-worktrees",
          "PILOT_INSTANCE_ID=stale",
          `PILOT_CONFIG=/Users/example/pilot/${path.basename(worktreeRoot)}/.pilot/config.json`,
          "",
        ].join("\n"),
        "utf8",
      );
      await fs.writeFile(
        ...
        [
          ...
          "if [ \"$1\" = \"pilotai\" ] && [ \"$2\" = \"worktree\" ] && [ \"$3\" = \"init\" ]; then",
          "  mkdir -p \"$PWD/.pilot\"",
          "  printf '%s\\n' '{\"database\":{\"embeddedPostgresDataDir\":\"'$PWD'/.pilot/runtime/db\"}}' > \"$PWD/.pilot/config.json\"",
          "  printf '%s\\n' \"PILOT_HOME=$PWD/.pilot/runtime\" \"PILOT_INSTANCE_ID=healthy\" \"PILOT_CONFIG=$PWD/.pilot/config.json\" > \"$PWD/.pilot/.env\"",
          "  exit 0",
          "fi",
          "exit 0",
          "",
      ...
      expect(result.stderr).toContain("Existing isolated Pilot worktree config is stale for this host; regenerating.");
      await expect(fs.readFile(path.join(pilotDir, ".env"), "utf8")).resolves.toContain(
        `PILOT_CONFIG=${worktreeRoot}/.pilot/config.json`,
      );
  ...
  it("retries worktree-local pnpm install without a frozen lockfile when the lockfile is outdated", async () => {
    const tempRoot = await mktemp("pilot-worktree-outdated-lockfile-");
    const baseRoot = path.join(tempRoot, "base");
    const worktreeRoot = path.join(tempRoot, "worktree");
    const fakeBin = path.join(tempRoot, "bin");
    const fakePnpmPath = path.join(fakeBin, "pnpm");
    ...
    try {
      ...
      await expect(fs.readFile(path.join(worktreeRoot, ".pilot", "config.json"), "utf8")).resolves.toContain(
        "\"database\"",
      );
    } finally {
      await fs.rm(tempRoot, { recursive: true, force: true });
  ...
  it("records worktree setup and provision operations when a recorder is provided", async () => {
    ...
    expect(operations[0]?.metadata).toMatchObject({
      branchName: "PIL-540-record-workspace-operations",
      created: true,
    });
  ...
  it("reuses an existing branch without resetting it when recreating a missing worktree", async () => {
    const repoRoot = await createTempRepo();
    const branchName = "PIL-450-recreate-missing-worktree";
  ...
  it("reattaches a missing persisted git worktree before manual control starts it", async () => {
    const repoRoot = await createTempRepo();
    const branchName = "PIL-451-restore-persisted-worktree";
    await fs.mkdir(path.join(repoRoot, "scripts"), { recursive: true });
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$PILOT_WORKSPACE_BRANCH\" > .pilot-restored-branch",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(initial.cwd, ".pilot-restored-branch"), "utf8")).resolves.toBe(`${branchName}\n`);
  ...
  it("repairs a clean persisted git worktree branch mismatch when both branches point at the same commit", async () => {
    const repoRoot = await createTempRepo();
    const expectedBranch = "PIL-454-repair-clean-branch-mismatch";
    const actualBranch = "PIL-454-publish-head";
    const realWorktreeRoot = path.join(repoRoot, ".pilot", "real-worktrees");
    const symlinkedWorktreeRoot = path.join(repoRoot, ".pilot", "worktrees");
    ...
    expect(operations).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          phase: "worktree_prepare",
          command: "git checkout PIL-454-repair-clean-branch-mismatch",
          metadata: expect.objectContaining({
            branchIncoherenceRepair: true,
  ...
  it("reattaches a clean forward detached HEAD to the recorded persisted git worktree branch", async () => {
    const repoRoot = await createTempRepo();
    const branchName = "PIL-454-reattach-detached-head";
    const worktreePath = path.join(repoRoot, ".pilot", "worktrees", branchName);
  ...
  it("rejects dirty persisted git worktree branch incoherence with bounded recovery evidence", async () => {
    const repoRoot = await createTempRepo();
    const expectedBranch = "PIL-455-reject-dirty-branch-mismatch";
    const actualBranch = "PIL-455-publish-head";
    const worktreePath = path.join(repoRoot, ".pilot", "worktrees", expectedBranch);
    await fs.mkdir(path.dirname(worktreePath), { recursive: true });
    await runGit(repoRoot, ["branch", expectedBranch]);
  ...
  it("routes non-reusable persisted git worktrees through workspace validation recovery", async () => {
    const repoRoot = await createTempRepo();
    const expectedBranch = "PIL-455-not-registered-worktree";
    const detachedWorktreePath = path.join(repoRoot, ".pilot", "worktrees", expectedBranch);
  ...
  it("adopts an existing persisted git worktree when the checked-out branch is forward of the recorded branch", async () => {
    ...
    const actualBranch = "PIL-456-push-pr-head";
  ...
  it("classifies persisted git worktree branch incoherence as diverged when the checked-out branch is not forward", async () => {
    const repoRoot = await createTempRepo();
    const expectedBranch = "PIL-457-recorded-work";
    const actualBranch = "PIL-457-sibling-work";
    const worktreePath = path.join(repoRoot, ".pilot", "worktrees", expectedBranch);
  ...
  it("routes a deleted recorded branch with a clean worktree to forward adoption when reconcile-forward is enabled", async () => {
    const repoRoot = await createTempRepo();
    const expectedBranch = "PIL-458-deleted-recorded-branch";
    const actualBranch = "PIL-458-actual-work";
    const worktreePath = path.join(repoRoot, ".pilot", "worktrees", expectedBranch);
  ...
  it("keeps a deleted recorded branch fail-closed when reconcile-forward is disabled", async () => {
    const repoRoot = await createTempRepo();
    const expectedBranch = "PIL-458-deleted-recorded-branch-flag-off";
    const actualBranch = "PIL-458-actual-work-flag-off";
    const worktreePath = path.join(repoRoot, ".pilot", "worktrees", expectedBranch);
  ...
  it("keeps forward reconciliation fail-closed for same-content rewritten history", async () => {
    const repoRoot = await createTempRepo();
    const expectedBranch = "PIL-459-recorded-content";
    const actualBranch = "PIL-459-rewritten-content";
    const worktreePath = path.join(repoRoot, ".pilot", "worktrees", expectedBranch);
  ...
  it("keeps forward reconciliation fail-closed for an unrelated task branch", async () => {
    const repoRoot = await createTempRepo();
    const expectedBranch = "PIL-459-recorded-task";
    const actualBranch = "PIL-999-unrelated-task";
    const worktreePath = path.join(repoRoot, ".pilot", "worktrees", expectedBranch);
  ...
  it("keeps forward reconciliation fail-closed when the live branch is behind the recorded branch", async () => {
    const repoRoot = await createTempRepo();
    const expectedBranch = "PIL-459-recorded-ahead";
    const actualBranch = "PIL-459-live-behind";
    const worktreePath = path.join(repoRoot, ".pilot", "worktrees", expectedBranch);
  ...
  it("does not reuse a missing persisted local filesystem workspace", async () => {
    const baseCwd = await mktemp("pilot-workspace-base-");
  ...
  it("reprovisions an existing persisted git worktree before manual control starts it", async () => {
    ...
    await fs.rm(path.join(initial.cwd, ".pilot-restored-state"), { force: true });
    ...
    await expect(fs.readFile(path.join(initial.cwd, ".pilot-restored-state"), "utf8")).resolves.toBe("reprovisioned\n");
  }, 15_000);
  ...
  it("auto-detects the default branch when baseRef is not configured", async () => {
    ...
    const bareRemote = await mktemp("pilot-worktree-bare-");
  ...
  it("auto-detects the default branch via symbolic-ref when origin/HEAD is set", async () => {
    ...
    const bareRemote = await mktemp("pilot-worktree-bare-symref-");
    await runGit(bareRemote, ["init", "--bare"]);
    await runGit(repoRoot, ["remote", "add", "origin", bareRemote]);
  ...
  it("records teardown and cleanup operations when a recorder is provided", async () => {
    ...
    const worktreesDir = await mktemp("pilot-cleanup-instances-");
    ...
    await fs.mkdir(path.join(workspace.cwd, ".pilot"), { recursive: true });
    await fs.writeFile(
      path.join(workspace.cwd, ".pilot", ".env"),
      `PILOT_HOME=${JSON.stringify(worktreesDir)}\nPILOT_INSTANCE_ID=${JSON.stringify(instanceId)}\n`,
      "utf8",
    );
...
describe("ensureRuntimeServicesForRun", () => {
  ...
  it("runs runtime provisioning once when service starts race for the same workspace", async () => {
    const workspaceRoot = await mktemp("pilot-runtime-provision-race-");
  ...
  it("logs runtime provisioning failure and retries it on the next service start", async () => {
    const workspaceRoot = await mktemp("pilot-runtime-provision-retry-");
  ...
  it("records the built-in deferred seed as failed when its manifest is not verified", async () => {
    const workspaceRoot = await mktemp("pilot-workspace-seed-operation-");
    ...
    const markerDir = path.join(workspaceRoot, ".pilot");
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        `printf '%s\\n' '${JSON.stringify({ version: 2, state: "failed", phase: "source_validation" })}' > .pilot/seed-manifest.json`,
      ].join("\n"),
      "utf8",
    );
    const config = runtimeProvisionTestConfig({});
  ...
  it("keeps an explicit command matching the built-in seed command as runtime provisioning", async () => {
    const workspaceRoot = await mktemp("pilot-explicit-runtime-provision-");
  ...
  it("does not create a runtime provision operation when the command is absent", async () => {
    const workspaceRoot = await mktemp("pilot-runtime-provision-noop-");
  ...
  it("leaves manual runtime services untouched during agent runs", async () => {
    const workspaceRoot = await mktemp("pilot-runtime-manual-");
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("requires Pilot dev runtime services to pass /api/health readiness", async () => {
    const workspaceRoot = await mktemp("pilot-runtime-health-");
    const workspace = buildWorkspace(workspaceRoot);
    const runId = "run-pilot-health";
  ...
  it("replaces a reused Pilot dev runtime whose 2xx health payload is unhealthy", async () => {
    const workspaceRoot = await mktemp("pilot-runtime-misreported-health-");
    const workspace = buildWorkspace(workspaceRoot);
    const serviceCommand =
  ...
  it("reuses a shared Pilot dev runtime after one transient unhealthy response", async () => {
    const workspaceRoot = await mktemp("pilot-runtime-transient-health-");
    const workspace = buildWorkspace(workspaceRoot);
    const serviceCommand =
  ...
  it("uses explicit readiness URL when exposed URL is not the local probe address", async () => {
    const workspaceRoot = await mktemp("pilot-runtime-explicit-readiness-");
    const workspace = buildWorkspace(workspaceRoot);
    const runId = "run-pilot-explicit-readiness";
    const serviceCommand =
    ...
    try {
      ...
      expect(services).toHaveLength(1);
      expect(services[0]?.url).toMatch(/^http:\/\/not-a-real-pilot-host\.invalid:\d+$/);
    } finally {
      await releaseRuntimeServicesForRun(runId);
    }
  });
  ...
  it("reuses shared runtime services across runs and starts a new service after release", async () => {
    const workspaceRoot = await mktemp("pilot-runtime-workspace-");
    const workspace = buildWorkspace(workspaceRoot);
    const serviceCommand =
  ...
  it("does not reuse project-scoped shared services across different workspace launch contexts", async () => {
    const primaryWorkspaceRoot = await mktemp("pilot-runtime-primary-");
    const worktreeWorkspaceRoot = path.join(primaryWorkspaceRoot, ".pilot", "worktrees", "PIL-874-chat-speed-issues");
    ...
    const executionWorkspace: RealizedExecutionWorkspace = {
      ...
      branchName: "PIL-874-chat-speed-issues",
    ...
    const serviceCommand =
      "node -e \"require('node:http').createServer((req,res)=>{if(req.url==='/api/health'){res.setHeader('content-type','application/json');res.end(JSON.stringify({status:'ok'}));return;}res.end(process.env.PILOT_HOME)}).listen(Number(process.env.PORT), '127.0.0.1')\"";
    const config = {
      workspaceRuntime: {
        services: [
          {
            ...
            cwd: ".",
            env: {
              PILOT_HOME: "{{workspace.cwd}}/.pilot/runtime-services",
            },
            port: { type: "auto" },
            readiness: {
              type: "http",
    ...
    expect(await primaryResponse.text()).toBe(path.join(primaryWorkspaceRoot, ".pilot", "runtime-services"));

    const executionResponse = await fetch(executionServices[0]!.url!);
    expect(await executionResponse.text()).toBe(path.join(worktreeWorkspaceRoot, ".pilot", "runtime-services"));
  });

  it("does not leak parent Pilot instance env into runtime service commands", async () => {
    const workspaceRoot = await mktemp("pilot-runtime-env-");
    const workspace = buildWorkspace(workspaceRoot);
    ...
    const serviceCommand = [
      ...
      JSON.stringify(
        [
          ...
          "pilotConfig: process.env.PILOT_CONFIG ?? process.env.PILOT_CONFIG ?? null,",
          "pilotHome: process.env.PILOT_HOME ?? process.env.PILOT_HOME ?? null,",
          "pilotInstanceId: process.env.PILOT_INSTANCE_ID ?? process.env.PILOT_INSTANCE_ID ?? null,",
    ...
    process.env.PILOT_CONFIG = "/tmp/base-pilot-config.json";
    process.env.PILOT_HOME = "/tmp/base-pilot-home";
    process.env.PILOT_INSTANCE_ID = "base-instance";
    process.env.DATABASE_URL = "postgres://shared-db.example.com/pilot";
    ...
    expect(captured.pilotConfig).toBeNull();
    expect(captured.pilotHome).toBeNull();
    expect(captured.pilotInstanceId).toBeNull();
  ...
  it("stops execution workspace runtime services by executionWorkspaceId", async () => {
    const workspaceRoot = await mktemp("pilot-runtime-stop-");
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("does not stop services in sibling directories when matching by workspace cwd", async () => {
    const workspaceParent = await mktemp("pilot-runtime-sibling-");
  ...
  it("starts only the selected workspace-controlled runtime service", async () => {
    const workspaceRoot = await mktemp("pilot-runtime-control-start-");
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("stops only the selected execution workspace runtime service", async () => {
    const workspaceRoot = await mktemp("pilot-runtime-control-stop-");
    const workspace = buildWorkspace(workspaceRoot);
...
describe("readLocalServicePortOwner", () => {
  ...
  it("attributes a Windows listener to a descendant of the launched process", async () => {
    const fakeBin = await mktemp("pilot-runtime-windows-tools-");
    const previousPath = process.env.PATH;
    const port = 43_123;
  ...
  it("accepts service cwd nested within the requested workspace", async () => {
    const workspace = await mktemp("pilot-runtime-workspace-");
  ...
  it("preserves newlines and trailing whitespace from Darwin lsof cwd output", async () => {
    const fakeBin = await mktemp("pilot-runtime-lsof-tools-");
    const previousPath = process.env.PATH;
    const reportedCwd = path.join(os.tmpdir(), "pilot-runtime-line\nbreak ");
  ...
  it("returns null for invalid PIDs and a missing Darwin lsof binary", async () => {
    ...
    const fakeBin = await mktemp("pilot-runtime-missing-lsof-");
    const previousPath = process.env.PATH;
    Object.defineProperty(process, "platform", { value: "darwin" });
    process.env.PATH = fakeBin;
  ...
  it("keeps a live registry record adoptable when Darwin cwd inspection confirms it", async () => {
    ...
    const pilotHome = await mktemp("pilot-runtime-home-");
    process.env.PILOT_HOME = pilotHome;
  ...
  it("refuses to adopt a listener whose real cwd belongs to another workspace", async () => {
    ...
    const targetWorkspace = await mktemp("pilot-runtime-target-");
    const ownerWorkspace = await mktemp("pilot-runtime-owner-");
    const pilotHome = await mktemp("pilot-runtime-home-");
    process.env.PILOT_HOME = pilotHome;
  ...
  it("adopts a port owner running inside the workspace when the registry record is gone", async () => {
    ...
    const workspace = await mktemp("pilot-runtime-adopt-");
    const pilotHome = await mktemp("pilot-runtime-home-");
    process.env.PILOT_HOME = pilotHome;
  ...
  it("refuses to adopt a listener whose cwd differs only by trailing whitespace", async () => {
    ...
    const workspace = await mktemp("pilot-runtime-ws-");
...
describeEmbeddedPostgres("workspace dirty quarantine branch repair", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-workspace-dirty-quarantine-");
  ...
  async function createDirtyMismatchRepo(input: {
    ...
    const repoRoot = await createTempRepo();
    const worktreePath = path.join(repoRoot, ".pilot", "worktrees", input.expectedBranch);
    await fs.mkdir(path.dirname(worktreePath), { recursive: true });
  ...
  async function seedDirtyQuarantineRecords(input: {
    ...
    await db.insert(issues).values({
      ...
      identifier: input.sourceIdentifier ?? "PIL-455",
    });
    ...
    if (input.claimant && input.claimant !== "none") {
      ...
      const claimantIdentifier = "PIL-999";
      await db.insert(issues).values({
        id: claimantIssueId,
        companyId,
        projectId,
      ...
      await db.insert(executionWorkspaces).values({
        ...
        status: "active",
        cwd: path.join(input.repoRoot, ".pilot", "claimants", claimantWorkspaceId),
        providerRef: path.join(input.repoRoot, ".pilot", "claimants", claimantWorkspaceId),
        baseRef: "HEAD",
    ...
    return {
      ...
      runId,
      claimant,
      sourceIdentifier: input.sourceIdentifier ?? "PIL-455",
    };
  }
  ...
  it("quarantines dirty foreign-branch work into a rescue branch before restoring the recorded branch", async () => {
    const expectedBranch = "PIL-455-recorded";
    const actualBranch = "PIL-455-live";
    ...
    expect(rescueBranch).toMatch(/^pilot\/rescue\/PIL-455\/\d{8}T\d{6}Z$/);
  ...
  it("quarantines a worktree wedged mid-rebase and clears the interrupted rebase state", async () => {
    const expectedBranch = "PIL-456-recorded";
    const repoRoot = await createTempRepo("master");
    const worktreePath = path.join(repoRoot, ".pilot", "worktrees", expectedBranch);
    await fs.mkdir(path.dirname(worktreePath), { recursive: true });
    await runGit(repoRoot, ["branch", expectedBranch]);
    ...
    const ids = await seedDirtyQuarantineRecords({
      ...
      worktreePath,
      expectedBranch,
      actualBranch: "PIL-456-live",
      sourceIdentifier: "PIL-456",
      claimant: "none",
    });
    ...
    const rescueBranch = warning?.match(/"([^"]+)"/)?.[1] ?? "";
    expect(rescueBranch).toMatch(/^pilot\/rescue\/PIL-456\/\d{8}T\d{6}Z$/);
  ...
  it("refuses dirty quarantine repair when the live branch has an active claimant", async () => {
    const expectedBranch = "PIL-456-recorded";
    const actualBranch = "PIL-456-live";
  ...
  it("refuses dirty quarantine repair when the live branch has an idle claimant", async () => {
    const expectedBranch = "PIL-457-recorded";
    const actualBranch = "PIL-457-live";
    const { repoRoot, worktreePath } = await createDirtyMismatchRepo({ expectedBranch, actualBranch });
    const ids = await seedDirtyQuarantineRecords({
      repoRoot,
      worktreePath,
  ...
  it("refuses dirty quarantine repair while the execution workspace has an active runtime service", async () => {
    const expectedBranch = "PIL-458-recorded";
    const actualBranch = "PIL-458-live";
    const { repoRoot, worktreePath } = await createDirtyMismatchRepo({ expectedBranch, actualBranch });
    const ids = await seedDirtyQuarantineRecords({
      repoRoot,
      worktreePath,
    ...
    await db.insert(workspaceRuntimeServices).values({
      ...
      serviceName: "pilot-dev",
      status: "running",
      lifecycle: "shared",
      reuseKey: `execution_workspace:${ids.sourceWorkspaceId}:pilot-dev`,
      command: "pnpm dev",
      cwd: worktreePath,
      port: 49195,
  ...
  it("best-effort restores the recorded branch when the rescue commit fails", async () => {
    const expectedBranch = "PIL-460-recorded";
    const actualBranch = "PIL-460-live";
    const { repoRoot, worktreePath } = await createDirtyMismatchRepo({ expectedBranch, actualBranch });
    const ids = await seedDirtyQuarantineRecords({
      repoRoot,
      worktreePath,
...
describeEmbeddedPostgres("workspace runtime service control persistence", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-workspace-runtime-control-");
    db = createDb(tempDb.connectionString);
  }, 20_000);
  ...
  it("persists provisioning before starting and excludes provision time from readiness timeout", async () => {
    const workspaceRoot = await mktemp("pilot-runtime-slow-control-");
    const pilotHome = await mktemp("pilot-runtime-control-home-");
    const previousPilotHome = process.env.PILOT_HOME;
    const previousPilotInstanceId = process.env.PILOT_INSTANCE_ID;
    process.env.PILOT_HOME = pilotHome;
  ...
  async function createRuntimeFixture(input?: {
    ...
    const projectId = randomUUID();
    const projectWorkspaceId = randomUUID();
    const baseRoot = await mktemp("pilot-runtime-port-fixture-");
  ...
  async function createRuntimeHome() {
    const pilotHome = await mktemp("pilot-runtime-port-home-");
    const previousPilotHome = process.env.PILOT_HOME;
    const previousPilotInstanceId = process.env.PILOT_INSTANCE_ID;
    process.env.PILOT_HOME = pilotHome;
  ...
  it("does not accept an occupied allocated port when listener ownership is unavailable", async () => {
    ...
    const workspace = fixture.workspaces[0]!;
    const basePort = await findFreePort();
    const fakeBin = await mktemp("pilot-runtime-no-lsof-");
  ...
  it("keeps fixed ports strict for shared persisted workspaces and identity-less starts", async () => {
    ...
    try {
      ...
      const identitylessCwd = await mktemp("pilot-runtime-identityless-");
      try {
        await expect(startRuntimeServicesForWorkspaceControl({
...
describeEmbeddedPostgres("workspace runtime startup reconciliation", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-workspace-runtime-");
    db = createDb(tempDb.connectionString);
  }, 20_000);
  ...
  it("restores desired services when one row is stopped and a live registered service has no row", async () => {
    const workspaceRoot = await mktemp("pilot-runtime-desired-reconcile-");
    const pilotHome = await mktemp("pilot-runtime-home-");
    process.env.PILOT_HOME = pilotHome;
    process.env.PILOT_INSTANCE_ID = `runtime-desired-reconcile-${randomUUID()}`;

    const reservePort = async () => {
      // Deterministic scan avoids ephemeral-port exhaustion under load.
      for (let port = 55_534 - Math.floor(Math.random() * 2_000); port >= 50_000; port -= 2) {
        const probe = net.createServer();
      ...
      throw new Error("Failed to reserve runtime reconciliation test port");
    };
    ...
    const actor = { id: null, name: "Pilot", companyId };
    const workspace = {
      ...buildWorkspace(workspaceRoot),
      projectId,
      workspaceId: projectWorkspaceId,
  ...
  it("backfills a pre-existing HTTP-only managed worktree runtime to verified HTTPS in place", async () => {
    // PIL-17158: an eligible workspace created before the feature must come
    ...
    const workspaceRoot = await mktemp("pilot-runtime-https-backfill-");
    const pilotHome = await mktemp("pilot-runtime-home-");
    const previousPilotHome = process.env.PILOT_HOME;
    const previousPilotInstanceId = process.env.PILOT_INSTANCE_ID;
    ...
    const reservePort = async () => {
      // Scan explicit candidates: the OS ephemeral counter can sit above 55535
      // under load, making port-0 roulette exhaust its retry budget. Randomize the
      // start so parallel test workers do not fight over the same candidates.
      for (let port = 55_535 - Math.floor(Math.random() * 2_000); port >= 50_000; port -= 1) {
        if (port >= 42_000 && port <= 42_999) continue;
        const probe = net.createServer();
        const bound = await new Promise<boolean>((resolve) => {
          probe.once("error", () => resolve(false));
          probe.listen(port, "127.0.0.1", () => resolve(true));
  ...
  it("re-adopts a request-logging service on the same auto port after supervisor stdio closes", async () => {
    const workspaceRoot = await mktemp("pilot-runtime-reconcile-");
    const pilotHome = await mktemp("pilot-runtime-home-");
    process.env.PILOT_HOME = pilotHome;
  ...
  it("re-adopts a live service whose shell command differs from the surviving process argv", async () => {
    const workspaceRoot = await mktemp("pilot-runtime-pnpm-reconcile-");
    const pilotHome = await mktemp("pilot-runtime-home-");
    const previousPilotHome = process.env.PILOT_HOME;
    const previousInstanceId = process.env.PILOT_INSTANCE_ID;
    process.env.PILOT_HOME = pilotHome;
    ...
    const reservePort = async () => {
      // Deterministic scan (see HTTPS backfill reservePort note above).
      for (let candidate = 55_535 - Math.floor(Math.random() * 2_000); candidate >= 50_000; candidate -= 1) {
        if (candidate >= 42_000 && candidate <= 42_999) continue;
        const probe = net.createServer();
        const bound = await new Promise<boolean>((resolve) => {
          probe.once("error", () => resolve(false));
  ...
  it("does not reuse a stopped auto-port service port while another process owns it", async () => {
    const workspaceRoot = await mktemp("pilot-runtime-unhealthy-adopt-");
    const pilotHome = await mktemp("pilot-runtime-home-");
    process.env.PILOT_HOME = pilotHome;
  ...
  it("does not adopt a live registry process from another workspace with the same runtime service ID", async () => {
    ...
    await db.insert(workspaceRuntimeServices).values({
      ...
      status: "running",
      lifecycle: "shared",
      reuseKey: `project_workspace:${projectWorkspaceId}:pilot-dev`,
      command: "pnpm dev",
      cwd: "/tmp/pilot-primary",
      port: 49195,
      url: "http://127.0.0.1:49195",
    ...
    await writeLocalServiceRegistryRecord({
      version: 1,
      serviceKey: "workspace-runtime-pilot-dev-stale",
      profileKind: "workspace-runtime",
      serviceName: "pilot-dev",
      command: "pnpm dev",
      cwd: process.cwd(),
  ...
  it("adopts stopped persisted local services when a matching registry process is alive", async () => {
    ...
    const reuseKey = `project_workspace:${projectWorkspaceId}:pilot-dev`;
    ...
    await writeLocalServiceRegistryRecord({
      version: 1,
      serviceKey: "workspace-runtime-pilot-dev-live-stopped",
      profileKind: "workspace-runtime",
      serviceName: "pilot-dev",
      command: "node",
      cwd,
  ...
  it("persists controlled execution workspace stops as stopped", async () => {
    const workspaceRoot = await mktemp("pilot-runtime-stop-persisted-");
    const companyId = randomUUID();
    const agentId = randomUUID();
    const projectId = randomUUID();
    const runId = randomUUID();
  ...
  it("restarts a stopped auto-port service on the same port when rendered env changes", async () => {
    const workspaceRoot = await mktemp("pilot-runtime-port-reuse-env-");
    const companyId = randomUUID();
    const agentId = randomUUID();
    const projectId = randomUUID();
    const executionWorkspaceId = randomUUID();
  ...
  it("restarts a stopped auto-port service on the same port when it is available", async () => {
    const workspaceRoot = await mktemp("pilot-runtime-port-reuse-");
    const companyId = randomUUID();
    const agentId = randomUUID();
    const projectId = randomUUID();
    const executionWorkspaceId = randomUUID();
...
describe("realizeExecutionWorkspace with an exact existing branch", () => {
  ...
  it("attaches an isolated worktree checked out on exactly the requested branch", async () => {
    ...
    expect(workspace.cwd).toContain(path.join(".pilot", "worktrees"));
  ...
  it("fails closed when the requested branch does not exist and creates nothing", async () => {
    ...
    expect(
      existsSync(path.join(repoRoot, ".pilot", "worktrees", "feature", "never-created")),
    ).toBe(false);
  });
  ...
  it("fails closed instead of reconciling when the managed worktree path holds another branch", async () => {
    ...
    const managedPath = path.join(repoRoot, ".pilot", "worktrees", "feature/pinned");
