...
import { companies, createDb } from "@pilotai/db";
import {
...
describeEmbeddedPostgres("company import transfer routes", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-import-transfer-routes-");
    ...
    spoolRoot = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-import-transfer-spool-"));
    app = await createApp();
  });
