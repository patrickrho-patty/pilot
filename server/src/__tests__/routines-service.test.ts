...
describeEmbeddedPostgres("routine service live-execution coalescing", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-routines-service-");
  ...
  it("interpolates routine variables into the execution issue and stores resolved values", async () => {
    ...
    expect(storedIssue?.title).toBe("repo triage for pilot");
    expect(storedIssue?.description).toBe("Review pilot for high bugs");
  ...
  it("auto-populates workspaceBranch from a reused isolated workspace", async () => {
    ...
    await db.insert(executionWorkspaces).values({
      ...
      status: "active",
      providerType: "git_worktree",
      branchName: "pil-1634-routine-branch",
    });
    ...
    expect(storedIssue?.title).toBe("Review pil-1634-routine-branch");
    expect(storedIssue?.description).toBe("Use branch pil-1634-routine-branch");
    expect(storedRun?.triggerPayload).toEqual({
      variables: {
        workspaceBranch: "pil-1634-routine-branch",
      },
    });
  });
  ...
  it("rejects an HMAC webhook replay when automatic execution is suppressed", async () => {
    const runtimeEnv = { PILOT_IN_WORKTREE: "yes", PILOT_INSTANCE_ID: "worktree-routines-test" };
  ...
  it("dispatches only post-cutoff scheduled routines in an armed worktree", async () => {
    const runtimeEnv = { PILOT_IN_WORKTREE: "true", PILOT_INSTANCE_ID: "worktree-routines-test" };
