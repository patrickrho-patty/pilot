// QA validation for [PIL-9522](/PIL/issues/PIL-9522). Drives the routine-secret
...
describeEmbedded("PIL-9522 QA: routine secrets end-to-end", () => {
  let db!: ReturnType<typeof createDb>;
  let tempDb: Awaited<ReturnType<typeof startEmbeddedPostgresTestDatabase>> | null = null;
  const secretsTmpDir = path.join(os.tmpdir(), `pilot-qa-routine-secrets-${randomUUID()}`);
  ...
  beforeAll(async () => {
    ...
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-qa-routine-secrets-");
