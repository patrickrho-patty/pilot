/**
 * PIL-17419 end-to-end regression: a leased HTTPS app/HMR pair is never handed
 ...
 * exercised as they actually run — the layer where PIL-17251 failed.
 *
 ...
 * unrelated workspace `PIL-16986-add-posthog-mcp` was then handed the pair.
 *
 * ## Host safety
 *
 ...
 * `afterEach`. `PILOT_HOME` is redirected to a temp dir so the local-service
...
(embeddedPostgresSupport.supported ? describe : describe.skip)(
  "PIL-17419 leased exposure pair reservation",
  () => {
    let db: Db;
    ...
    beforeAll(async () => {
      tempDb = await startEmbeddedPostgresTestDatabase("pil17419-reservation-");
    ...
    beforeEach(async () => {
      workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "pil17419-workspace-"));
      pilotHome = await fs.mkdtemp(path.join(os.tmpdir(), "pil17419-home-"));
      ...
      process.env.PILOT_INSTANCE_ID = `pil17419-${randomUUID()}`;
    });
    ...
    async function seedIncident(options?: { leaseStatus?: string; leasedRowStatus?: string }) {
      ...
      await db.insert(projects).values({ id: projectId, companyId, name: "Pilot App", status: "in_progress" });
    ...
    function startInput(seed: Awaited<ReturnType<typeof seedIncident>>, executionWorkspaceId: string) {
      return {
        invocationId: "pil-17419-reservation",
        actor: { id: null, name: "Pilot", companyId: seed.companyId },
        issue: null,
        db,
        workspace: {
    ...
    it("regression 3: reconciliation surfaces a removed row whose pair is mapped elsewhere, and adopts nothing", async () => {
      ...
      // for someone else. Exactly the falsely-attributed state from PIL-17251.
