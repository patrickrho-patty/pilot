/**
 * Opt-in LIVE exercise of the PIL-17158 in-place HTTPS backfill.
 *
 ...
 *
 *   - `PILOT_LIVE_BROKER_EXERCISE=1`
 *   - the broker socket exists (`PILOT_TAILSCALE_BROKER_SOCKET` or the default)
 *
 * Run it on a broker-provisioned host with:
 *
 *   PILOT_LIVE_BROKER_EXERCISE=1 pnpm --filter @pilotai/server exec \
...
const DEFAULT_BROKER_SOCKET = "/run/pilot-tailscale-broker/broker.sock";
...
if (optedIn && !live) {
  console.warn(
    `[PIL-17158] live exercise opted in but skipped: broker socket at ${brokerSocketPath} `
      + `present=${await brokerSocketPresent()}, embeddedPostgres=${embeddedPostgresSupport.supported}`,
  );
}

(live ? describe : describe.skip)("PIL-17158 live HTTPS backfill exercise", () => {
  let db: Db;
  let tempDb: Awaited<ReturnType<typeof startEmbeddedPostgresTestDatabase>>;

  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pil17158-live");
  ...
  it("upgrades a pre-existing HTTP workspace in place to a browser-trusted HTTPS URL", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "pil17158-live-"));
    const pilotHome = await fs.mkdtemp(path.join(os.tmpdir(), "pil17158-live-home-"));
    ...
    process.env.PILOT_INSTANCE_ID = `pil17158-live-${randomUUID()}`;
    ...
    try {
      ...
      const before = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "Pilot", companyId },
        issue: null,
        workspace: {
      ...
      console.log("[PIL-17158 live exercise]", JSON.stringify(evidence, null, 2));
      // Deprovisions the broker lease as well as the backend process.
      await stopRuntimeServicesForProjectWorkspace({
        db,
        projectWorkspaceId,
        workspaceCwd: workspaceRoot,
      }).catch((error) => console.error("[PIL-17158] teardown failed", error));
      await resetRuntimeServicesForTests();
