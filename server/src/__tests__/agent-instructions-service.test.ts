...
describe("agent instructions service", () => {
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    ...
    const externalRoot = await makeTempDir("pilot-agent-instructions-external-");
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("pilot-agent-instructions-new-external-");
    cleanupDirs.add(pilotHome);
    cleanupDirs.add(externalRoot);
    process.env.PILOT_HOME = pilotHome;
    process.env.PILOT_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("pilot-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const pilotHome = await makeTempDir("pilot-agent-instructions-recover-");
    cleanupDirs.add(pilotHome);
    process.env.PILOT_HOME = pilotHome;
    process.env.PILOT_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const pilotHome = await makeTempDir("pilot-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("pilot-agent-instructions-stale-root-");
    cleanupDirs.add(pilotHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const pilotHome = await makeTempDir("pilot-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("pilot-agent-instructions-heal-write-stale-");
    cleanupDirs.add(pilotHome);
    cleanupDirs.add(staleRoot);
    process.env.PILOT_HOME = pilotHome;
    process.env.PILOT_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const pilotHome = await makeTempDir("pilot-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("pilot-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(pilotHome);
    cleanupDirs.add(staleRoot);
    process.env.PILOT_HOME = pilotHome;
    process.env.PILOT_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const pilotHome = await makeTempDir("pilot-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("pilot-agent-instructions-partial-root-");
    cleanupDirs.add(pilotHome);
    cleanupDirs.add(staleRoot);
    process.env.PILOT_HOME = pilotHome;
    process.env.PILOT_INSTANCE_ID = "test-instance";
