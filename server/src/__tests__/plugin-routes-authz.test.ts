...
describe.sequential("plugin install and upgrade authz", () => {
  ...
  it("lists bundled monorepo plugin packages", async () => {
    ...
    expect(packageNames).toContain("@pilotai/plugin-workspace-diff");
    expect(packageNames).toContain("@pilotai/plugin-llm-wiki");
    expect(packageNames).toContain("@pilotai/plugin-modal");
    expect(packageNames).toContain("@pilotai/plugin-authoring-smoke-example");
    expect(packageNames).not.toContain("@pilotai/plugin-sdk");
    expect(byPackageName.get("@pilotai/plugin-workspace-diff")?.experimental).toBe(true);
    expect(byPackageName.get("@pilotai/plugin-llm-wiki")?.experimental).toBe(true);
    expect(byPackageName.get("@pilotai/plugin-modal")?.experimental).toBe(true);
    expect(byPackageName.get("@pilotai/plugin-authoring-smoke-example")?.experimental).toBe(false);
    expect(typeof byPackageName.get("@pilotai/plugin-workspace-diff")?.hasBuiltEntrypoints).toBe("boolean");
  }, 20_000);
  ...
  it("rejects plugin installation for non-admin board users", async () => {
    ...
    const res = await request(app)
      ...
      .send({ packageName: "pilot-plugin-example" });
  ...
  it("allows instance admins to install plugins", async () => {
    ...
    mockRegistry.getByKey.mockResolvedValue({
      id: pluginId,
      pluginKey,
      packageName: "pilot-plugin-example",
      version: "1.0.0",
    });
    mockRegistry.getById.mockResolvedValue({
      id: pluginId,
  ...
  it("resolves plugin keys without probing the UUID id column for core plugin actions", async () => {
    const pluginKey = "pilotqa.hello-plugin";
    const plugin = {
      id: pluginId,
      pluginKey,
      version: "1.0.0",
...
describe.sequential("scoped plugin API routes", () => {
  ...
  it("dispatches manifest-declared scoped routes after company access checks", async () => {
    ...
    const res = await request(app)
      .get("/api/plugins/pilot.example/api/smoke")
    ...
    for (const [label, rows] of cases) {
      ...
      const { app } = await createApp(boardActor(), {}, {
        ...
        toolDeps: {
          toolDispatcher: {
            listToolsForAgent: vi.fn(),
            getTool: vi.fn(() => ({ name: "pilot.example:search" })),
            executeTool,
          },
        },
      });
    ...
    const { app } = await createApp(
      ...
      {},
      {
        ...
        toolDeps: {
          toolDispatcher: {
            listToolsForAgent: vi.fn(),
            getTool: vi.fn(() => ({ name: "pilot.example:search", pluginDbId: pluginId })),
            executeTool,
          },
        },
      },
...
describe.sequential("operator-hidden plugin management floor", () => {
  ...
  it("floors plugin lifecycle and config writes for instance admins", async () => {
    ...
    const attempts: Array<[string, request.Test]> = [
      ["install", request(app).post("/api/plugins/install").send({ packageName: "@pilotai/plugin-modal" })],
