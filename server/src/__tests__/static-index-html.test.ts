...
describe("static SPA fallback HTML", () => {
  ...
  it("serves the current index.html instead of reusing stale asset hashes", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-static-index-"));
    ...
    await expect(request(app).get("/PIL/issues/PIL-9939")).resolves.toMatchObject({
    ...
    const res = await request(app).get("/PIL/issues/PIL-9939");
