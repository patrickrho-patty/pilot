...
import { activityLog, companies, companyMemberships, instanceUserRoles } from "@pilotai/db";
import {
  actorMiddleware,
...
describe("actorMiddleware authenticated session profile", () => {
  ...
  it("trusts Cloud tenant identity headers and seeds board access", async () => {
    ...
    const res = await request(app)
      .get("/actor")
      .set("x-pilot-cloud-tenant-token", "tenant-token")
      .set("x-pilot-cloud-user-id", "global-user-1")
      .set("x-pilot-cloud-user-email", "owner@example.com")
      .set("x-pilot-cloud-user-name", "Stack Owner")
      .set("x-pilot-cloud-stack-id", "stack-alpha")
      .set("x-pilot-cloud-pilot-company-id", "pilot-stack-alpha")
      .set("x-pilot-cloud-pilot-company-name", "Purple Rain")
      .set("x-pilot-cloud-stack-role", "owner");
  ...
  it("lets the cloud tenant actor through assertCompanyAccess for a company it holds a membership row in", async () => {
    ...
    const cloudHeaders = {
      "x-pilot-cloud-tenant-token": "tenant-token",
      "x-pilot-cloud-user-id": "global-user-1",
      "x-pilot-cloud-user-email": "owner@example.com",
      "x-pilot-cloud-stack-id": "stack-alpha",
      "x-pilot-cloud-stack-role": "member",
    };
  ...
  it("repairs a legacy machine company name from the trusted human-name header", async () => {
    ...
    const res = await request(app)
      ...
      .set("x-pilot-cloud-user-id", "global-user-1")
      .set("x-pilot-cloud-user-email", "owner@example.com")
      .set("x-pilot-cloud-stack-id", "stack-purple-rain")
      .set(
        "x-pilot-cloud-pilot-company-id",
        "pilot-stack-purple-rain",
      )
      .set("x-pilot-cloud-pilot-company-name", "Purple Rain")
      .set("x-pilot-cloud-stack-role", "owner");
    ...
    expect(activities).toEqual([
      expect.objectContaining({
        ...
        details: expect.objectContaining({
          ...
          previousName: "pilot-stack-purple-rain",
          name: "Purple Rain",
        }),
      }),
    ]);
...
describe("Cloud tenant company naming", () => {
  const ids = {
    ...
    pilotCompanyId: "pilot-stack-purple-rain",
  };

  it.each([
    "pilot-stack-purple-rain",
    "stack-purple-rain Pilot",
    ids.companyId,
  ...
  it("humanizes the stack slug for old harnesses without a name header", () => {
    ...
    expect(humanizeCloudStackSlug("pilot-stack-purple-rain")).toBe(
      "Purple Rain",
    );
  });
});
