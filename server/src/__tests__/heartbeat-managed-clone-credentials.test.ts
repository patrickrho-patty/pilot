...
beforeAll(async () => {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-managed-clone-"));
...
async function createLocalSourceRepo() {
  const sourceRepo = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-clone-source-"));
  await execFile("git", ["init"], { cwd: sourceRepo });
  await execFile("git", ["config", "user.email", "pilot@example.com"], { cwd: sourceRepo });
  await execFile("git", ["config", "user.name", "Pilot Test"], { cwd: sourceRepo });
...
describe("ensureManagedProjectWorkspace clone credentials", () => {
  ...
  it("does not blame the credential when an authenticated clone fails for non-auth reasons", async () => {
    ...
    const missingRepo = path.join(os.tmpdir(), "pilot-definitely-missing", "repo.git");
  ...
  it("keeps the credential env alive through the sanitizer spread order", () => {
    ...
    // sanitizer strips every PILOT_* key, so the token env must be spread after it.
