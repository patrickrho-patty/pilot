...
async function createGitRepo() {
  ...
  const repoRoot = await realpath(await mkdtemp(path.join(os.tmpdir(), "pilot-branch-containment-repo-")));
...
function readAdapterWorkspace(input: unknown) {
  ...
  const wake = context.pilotWake as { executionWorkspace?: { branchName?: string } } | undefined;
...
async function seedBranchContainmentRun(
  ...
  await db.insert(executionWorkspaces).values([
    ...
    },
    {
      ...
      status: "active",
      cwd: path.join(repoRoot, ".pilot", "worktrees", "other-workspace"),
      repoUrl: null,
      baseRef: "HEAD",
      branchName: "other-workspace",
      providerType: "git_worktree",
      providerRef: path.join(repoRoot, ".pilot", "worktrees", "other-workspace"),
      lastUsedAt: now,
      openedAt: now,
      createdAt: now,
      updatedAt: now,
...
describeEmbeddedPostgres("heartbeat workspace branch containment", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-branch-containment-");
