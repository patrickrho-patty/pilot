...
import type { CatalogTeam } from "@pilotai/shared";
...
const CORE_EXEC_TEAM_ID = "pilotai:bundled:company-defaults:core-exec-team";
const CORE_EXEC_TEAM_HASH = "sha256:280853ff1c8412325a9fec66ff8ed649cb4e88ce64890462495ac55a68a96388";
    ...
    metadata: {
      pilot: {
        catalogTeam: {
          ...
          catalogKey: "pilotai/bundled/company-defaults/core-exec-team",
...
describe("teamsCatalogService", () => {
  beforeEach(() => {
    ...
    mockCompanyPortabilityService.importBundle.mockResolvedValue({
      company: { id: "company-1", name: "Pilot", action: "unchanged" },
      agents: [],
      projects: [],
      envInputs: [],
      warnings: [],
    ...
    mockCompanySkillService.installFromCatalog.mockResolvedValue({
      action: "created",
      skill: { key: "pilotai/bundled/pilot-operations/task-planning" },
      catalogSkill: { id: "pilotai:bundled:pilot-operations:task-planning" },
      warnings: [],
    });
  ...
  it("builds an inline portability source with catalog skill keys and target-manager reparenting", async () => {
    ...
    expect(prepared.source.files["agents/ceo/AGENTS.md"]).toEqual(expect.stringContaining("pilotai/bundled/pilot-operations/task-planning"));
    expect(prepared.source.files["agents/cto/AGENTS.md"]).toEqual(expect.stringContaining("pilotai/bundled/software-development/github-pr-workflow"));
    expect(prepared.source.files[".pilot.yaml"]).toEqual(expect.stringContaining("reportsToExistingAgentId: \"manager-1\""));
    expect(prepared.source.files[".pilot.yaml"]).toEqual(expect.stringContaining("reportsToExistingAgentSlug: \"engineering-manager\""));
  });
  ...
  it("resolves target-manager slug against same-company agents before rendering reparent metadata", async () => {
    ...
    expect(prepared.source.files[".pilot.yaml"]).toEqual(expect.stringContaining("reportsToExistingAgentId: \"manager-1\""));
    expect(prepared.source.files[".pilot.yaml"]).toEqual(expect.stringContaining("reportsToExistingAgentSlug: \"ceo\""));
  });

  it("preserves package-declared Pilot sidecar permissions while adding generated catalog provenance", async () => {
    const svc = teamsCatalogService({} as any);

    const prepared = await svc.prepareCatalogTeamSource("company-1", "product-engineering");

    expect(prepared.source.files[".pilot.yaml"]).toEqual(expect.stringContaining("permissions:"));
    expect(prepared.source.files[".pilot.yaml"]).toEqual(expect.stringContaining("canCreateAgents: true"));
    expect(prepared.source.files[".pilot.yaml"]).toEqual(expect.stringContaining("catalogTeam:"));
    expect(prepared.source.files[".pilot.yaml"]).toEqual(expect.stringContaining("catalogSlug: \"product-engineering\""));
  });
  ...
  describe("readCatalogTeamProvenance", () => {
    it("reads catalogTeam provenance from agent metadata", () => {
      expect(
        readCatalogTeamProvenance({
          pilot: { catalogTeam: { catalogId: "team-x", catalogKey: "k", originHash: "sha256:1" } },
        }),
    ...
    it("returns null when there is no catalogTeam provenance", () => {
      ...
      expect(readCatalogTeamProvenance({ pilot: { catalog: { skillKey: "s" } } })).toBeNull();
      expect(readCatalogTeamProvenance({ pilot: { catalogTeam: { originHash: "h" } } })).toBeNull();
    });
  });
      ...
      mockAgentService.list.mockResolvedValue([
        {
          id: "removed",
          companyId: "company-1",
          metadata: { pilot: { catalogTeam: { catalogId: "pilotai:bundled:gone:removed", originHash: "sha256:x" } } },
        },
      ]);
      const svc = teamsCatalogService({} as any);
  ...
  it("classifies unresolved and unsafe external skill requirements as blocked", () => {
    const fakeTeam: CatalogTeam = {
      id: "pilotai:optional:test:unsafe",
      key: "pilotai/optional/test/unsafe",
      kind: "optional",
      category: "test",
      slug: "unsafe",
      name: "Unsafe",
