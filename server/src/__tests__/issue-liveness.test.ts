...
describe("issue graph liveness classifier", () => {
  it("detects a PIL-1703-style blocked chain with an unassigned blocker and stable incident key", () => {
  ...
  it("detects blocker assignees under terminated org ancestors as uninvokable", () => {
    ...
    expect(findings[0]).toMatchObject({
      ...
      reason: "PIL-1703 is blocked by PIL-1704, but its assignee is in an invalid org chain.",
      recommendedOwnerAgentId: managerId,
    });
  });
  ...
  it("detects the PIL-2239-style blocked chain at the first stalled in_review leaf without duplicate findings", () => {
    ...
    const findings = classifyIssueGraphLiveness({
      ...
      relations: [
        { companyId, blockerIssueId: phaseIssueId, blockedIssueId: "pil-2239" },
    ...
    expect(findings[0]).toMatchObject({
      ...
      dependencyPath: [
        expect.objectContaining({ issueId: "pil-2239" }),
        expect.objectContaining({ issueId: phaseIssueId }),
        expect.objectContaining({ issueId: reviewLeafId }),
      ],
      incidentKey: `harness_liveness:${companyId}:pil-2239:in_review_without_action_path:${reviewLeafId}`,
    });
  });
