...
describeEmbeddedPostgres("pipelineService", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-pipelines-service-");
  ...
  it("posts upstream drift notices to active dependent work issues only", async () => {
    ...
    expect(workComments[0]!.body).toBe(
      `Upstream case [draft](/PIL/pipelines/${pipeline.id}/cases/${upstream.case.id}) changed (v1→v2).`,
    );
