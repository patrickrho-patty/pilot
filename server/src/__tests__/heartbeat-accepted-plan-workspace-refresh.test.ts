...
async function createGitRepo() {
  const repoRoot = await mkdtemp(path.join(os.tmpdir(), "pilot-accepted-plan-repo-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["checkout", "-B", "master"]);
  await runGit(repoRoot, ["config", "user.email", "pilot-test@example.com"]);
...
async function createGitRepoWithOrigin() {
  ...
  const originRoot = await mkdtemp(path.join(os.tmpdir(), "pilot-accepted-plan-origin-"));
...
describeEmbeddedPostgres("accepted plan workspace refresh", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-accepted-plan-workspace-");
  ...
  it("realizes an isolated workspace and drops stale shared task-session params before executing", async () => {
    ...
    expect(adapterInput.context.pilotWorkspace).toEqual(expect.objectContaining({
      mode: "isolated_workspace",
      strategy: "git_worktree",
    }));
    expect((adapterInput.context.pilotWorkspace as { cwd: string }).cwd).not.toBe(repoRoot);
  ...
  it("keeps accepted-plan children strategy-only until first realization after the base ref moves", async () => {
    ...
    adapterExecute.mockImplementationOnce(async (input) => {
      ...
      const workspace = context.pilotWorkspace as Record<string, unknown> | undefined;
  ...
  it("forces a fresh session and suppresses accepted-plan continuation when another issue owns the in-flight claim", async () => {
    ...
    expect(adapterInput.context.acceptedPlanWakeRouting).toEqual(expect.objectContaining({
      ...
      otherActiveClaimIdentifier: "PIL-9302",
    }));
    expect(adapterInput.context.pilotTaskMarkdown).toContain("Make the plan only.");
    expect(adapterInput.context.pilotTaskMarkdown).not.toContain("Create child issues from the approved plan only");
  }, 20_000);
  ...
  it("guards cross-issue accepted-plan retries even when the waking issue is standard work mode", async () => {
    ...
    expect(adapterInput.context.acceptedPlanWakeRouting).toEqual(expect.objectContaining({
      reason: "other_issue_claim_in_flight",
      otherActiveClaimIssueId: otherPlanningIssueId,
      otherActiveClaimIdentifier: "PIL-9402",
    }));
    expect(adapterInput.context.pilotTaskMarkdown).toContain("Issue: \"PIL-9401\"");
    expect(adapterInput.context.pilotTaskMarkdown).not.toContain("Create child issues from the approved plan only");
  }, 20_000);
  ...
  it("preserves accepted-plan continuation resume state when the wake issue owns the in-flight claim", async () => {
    ...
    expect(adapterInput.context.pilotTaskMarkdown).toContain("Create child issues from the approved plan only");
  }, 20_000);
});
