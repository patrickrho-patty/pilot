...
import { writePilotSkillSyncPreference } from "@pilotai/adapter-utils/server-utils";
import {
  agents,
  companies,
  companySkills,
...
describeEmbeddedPostgres("feedbackService.saveIssueVote", () => {
  ...
  beforeAll(async () => {
    const started = await startEmbeddedPostgresTestDatabase("pilot-feedback-service-");
  ...
  async function seedIssueWithRichAgentComment() {
    ...
    const instructionsDir = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-feedback-instructions-"));
    ...
    await db.insert(agents).values({
      ...
      adapterConfig: writePilotSkillSyncPreference(
        ...
        ["pilotai/pilot/pilot", "octo/research/public-skill"],
      ),
      runtimeConfig: {
        heartbeat: {
          enabled: true,
  ...
  it("builds a detailed sanitized shared bundle with issue and agent context", async () => {
    ...
    expect(trace?.schemaVersion).toBe("pilot-feedback-envelope-v2");
    expect(trace?.bundleVersion).toBe("pilot-feedback-bundle-v2");
  ...
  it("keeps earlier local votes local when a later vote enables sharing", async () => {
    ...
    expect(localTrace?.payloadVersion).toBe("pilot-feedback-v1");
    ...
    expect(sharedTrace?.payloadVersion).toBe("pilot-feedback-v1");
  });

  it("captures Claude project session artifacts as full traces", async () => {
    const claudeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-feedback-claude-"));
  ...
  it("captures OpenCode message and part files as full traces", async () => {
    const opencodeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-feedback-opencode-"));
  ...
  it("flushes pending shared traces into configured object storage and marks them sent", async () => {
    ...
    expect(uploadTraceBundle.mock.calls[0]?.[0]).toMatchObject({
      ...
      envelope: {
        destination: "pilot_labs_feedback_v1",
        exportId: traces[0]?.exportId,
      },
    });
  });
