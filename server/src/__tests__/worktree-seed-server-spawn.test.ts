...
const originalConfig = process.env.PILOT_CONFIG;
const originalWorktreesDir = process.env.PILOT_WORKTREES_DIR;
...
afterEach(async () => {
  if (originalConfig === undefined) delete process.env.PILOT_CONFIG;
  else process.env.PILOT_CONFIG = originalConfig;
  if (originalWorktreesDir === undefined) delete process.env.PILOT_WORKTREES_DIR;
  else process.env.PILOT_WORKTREES_DIR = originalWorktreesDir;
...
describe("managed worktree seed source through the server spawn path", () => {
  it("re-derives an ambient-instance manifest written before provisioning", async () => {
    const tempRoot = await mktemp("pilot-server-seed-source-");
    ...
    const registeredConfigPath = path.join(repoRoot, ".pilot", "config.json");
...
const stateDir = path.join(cwd, ".pilot");
...
const instanceRoot = path.join(process.env.PILOT_WORKTREES_DIR ?? process.env.PILOT_WORKTREES_DIR, "instances", instanceId);
...
fs.writeFileSync(path.join(stateDir, ".env"), [
  "PILOT_HOME=" + JSON.stringify(process.env.PILOT_WORKTREES_DIR),
  "PILOT_INSTANCE_ID=" + JSON.stringify(instanceId),
  "PILOT_CONFIG=" + JSON.stringify(targetConfigPath),
  "",
].join("\\n"));
fs.writeFileSync(path.join(stateDir, "seed-manifest.json"), JSON.stringify({
  version: 2,
  source: { instanceId: "ambient-instance", configPath: process.env.PILOT_CONFIG },
  snapshotAt: null,
  seedMode: "minimal",
...
`,
    ...
    process.env.PILOT_CONFIG = ambientConfigPath;
    process.env.PILOT_WORKTREES_DIR = worktreeHome;
