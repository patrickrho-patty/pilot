...
describeEmbeddedPostgres("heartbeat local environment lifecycle", () => {
  ...
  it("runs work through the default Local environment lease", async () => {
    ...
    expect(context.pilotEnvironment).toMatchObject({
      id: localRows[0]?.id,
      name: "Local",
      driver: "local",
      leaseId: leases[0]?.id,
  ...
  it("injects run-scoped Pilot env into process agents", async () => {
    const companyId = randomUUID();
    const agentId = randomUUID();
    const issuePrefix = `T${companyId.replace(/-/g, "").slice(0, 6).toUpperCase()}`;
    const tempDir = await mkdtemp(join(tmpdir(), "pilot-process-env-"));
      ...
      adapterConfig: {
        ...
        args: [
          ...
          [
            ...
            "agentId: process.env.PILOT_AGENT_ID ?? null,",
            "companyId: process.env.PILOT_COMPANY_ID ?? null,",
            "apiUrl: process.env.PILOT_API_URL ?? null,",
            "runId: process.env.PILOT_RUN_ID ?? null,",
            "apiKeyPresent: Boolean(process.env.PILOT_API_KEY),",
            "}));",
          ].join(" "),
        ],
      },
