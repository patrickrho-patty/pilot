...
import { companies, companySkills, createDb, folders } from "@pilotai/db";
import {
  getEmbeddedPostgresTestSupport,
  startEmbeddedPostgresTestDatabase,
} from "./helpers/embedded-postgres.js";
import type { CatalogSkill, CatalogSkillFile } from "@pilotai/shared";
...
const sampleCatalogSkill: CatalogSkill = {
  id: "pilotai:bundled:software-development:review",
  key: "pilotai/bundled/software-development/review",
  kind: "bundled",
...
describeEmbeddedPostgres("companySkillService.installFromCatalog", () => {
  ...
  beforeAll(async () => {
    ...
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-company-skills-catalog-");
    db = createDb(tempDb.connectionString);
    svc = await createService();
  }, 20_000);

  beforeEach(async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-catalog-home-"));
  ...
  it("repairs an existing unfiled Pilot catalog skill during inventory refresh", async () => {
    const companyId = await createCompany();
  ...
  it("restores portable catalog provenance when importing packaged skills", async () => {
    const companyId = await createCompany();
    const importedFiles = {
      "skills/pilotai/bundled/software-development/review/SKILL.md": [
        "---",
        `key: "${sampleCatalogSkill.key}"`,
        'slug: "review"',
        'name: "review"',
      ...
      "skills/pilotai/bundled/software-development/review/references/checklist.md": sampleReferenceMarkdown,
    };
  ...
  it("rejects duplicate slug conflicts", async () => {
    const companyId = await createCompany();
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-existing-skill-"));
