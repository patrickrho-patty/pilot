...
describe("merged pull-request confirmation extraction", () => {
  ...
  it("extracts and deduplicates full GitHub URLs and owner/repo#N shorthand", () => {
    expect(extractGitHubPullRequestReferences([
      "Merge https://github.com/PilotAI/pilot/pull/39.",
      "Also pilotai/pilot#40 and PILOTAI/pilot#40.",
    ])).toEqual([
      { host: "github.com", owner: "PilotAI", repo: "pilot", number: 39 },
      { host: "github.com", owner: "pilotai", repo: "pilot", number: 40 },
    ]);
  });
  ...
  it("finds references in merge-confirmation body and custom target fields", () => {
    expect(getMergeConfirmationPullRequestReferences({
      ...
      payload: {
        ...
        detailsMarkdown: "Primary: pilotai/pilot#39",
        target: {
          type: "custom",
          key: "github-pr-40",
          href: "https://github.com/pilotai/pilot/pull/40",
        },
      },
    })).toEqual([
      { host: "github.com", owner: "pilotai", repo: "pilot", number: 39 },
  ...
  it("denies governed actions mentioned in details or custom target metadata", () => {
    ...
    expect(getMergeConfirmationPullRequestReferences({
      ...
      payload: {
        version: 1,
        prompt: "Merge the linked pull request?",
        detailsMarkdown: "Deploy to production after pilotai/pilot#39 merges.",
      },
    })).toEqual([]);
    expect(getMergeConfirmationPullRequestReferences({
      ...base,
      payload: {
        ...
        target: {
          ...
          href: "https://github.com/pilotai/pilot/pull/39",
        },
      },
    })).toEqual([]);
  });
  ...
  it("fails closed when a merge confirmation includes an additional action clause", () => {
    expect(getMergeConfirmationPullRequestReferences({
      ...
      payload: {
        version: 1,
        prompt: "Merge pilotai/pilot#39?",
        detailsMarkdown: "Erase all customer records after pilotai/pilot#39 merges.",
      },
    })).toEqual([]);
  });
});
...
describeEmbeddedPostgres.sequential("merged pull-request confirmation sweep", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-merged-pr-confirmations-");
    ...
    await db.insert(issueThreadInteractions).values([
      {
        ...
        title: "Merge https://github.com/pilotai/pilot/pull/39?",
      ...
      },
      {
        ...
        title: "Ready to merge pilotai/pilot#39 and pilotai/pilot#41?",
