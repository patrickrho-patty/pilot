...
import { companies, createDb, pluginConfig, plugins } from "@pilotai/db";
...
describeEmbeddedPostgres("registry.listConfigs (startup config delivery)", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-plugin-config-delivery-");
  ...
  it("returns every company-scoped config row for a plugin", async () => {
    ...
    const pluginId = await seedPlugin("pilot.gateway-test", 1);
  ...
  it("only returns rows for the requested plugin (no cross-plugin bleed)", async () => {
    const registry = pluginRegistryService(db);
    const pluginId = await seedPlugin("pilot.gateway-test", 1);
    const otherPluginId = await seedPlugin("pilot.other-test", 2);
    const companyA = await seedCompany();
