...
function mockToolsList(tools: unknown[]) {
  return vi.spyOn(globalThis, "fetch").mockResolvedValue(
    mcpHttpResponse({ jsonrpc: "2.0", id: "pilot-catalog-refresh", result: { tools } }),
  );
}
...
async function createBrokerConnection(
  ...
  const [application] = await db.insert(toolApplications).values({
    companyId,
    applicationKey: "pilot-pages",
    name: `Pilot Pages ${randomUUID()}`,
    type: "mcp_http",
    status: "active",
  }).returning();
...
describeEmbeddedPostgres("tool access service", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-tool-access-service-");
  ...
  it("sends the MCP Streamable HTTP Accept header and decodes an SSE catalog response", async () => {
    ...
    // SSE-framed body in response. Regression guard for PIL-11096.
  ...
  it("registers an approved local stdio template and exposes its runtime slot", async () => {
    ...
    const connection = await service.createConnection(company.id, {
      ...
      config: { templateId: "pilot.echo-calculator-time" },
      enabled: true,
      status: "active",
    });
    const health = await service.checkHealth(connection.id);
  ...
  it("requires tools:admin to create, list, and disable stdio command templates", async () => {
    ...
    expect(listed.body.templates).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ templateId: "pilot.echo-calculator-time", source: "built_in" }),
  ...
  it("installs the safe example fixture idempotently and smokes allow, deny, and audit paths", async () => {
    ...
    expect(install.application).toMatchObject({
      applicationKey: "pilot.examples.safe-read-only-todo-kv",
      type: "mcp_stdio",
      status: "active",
    });
    expect(install.connection).toMatchObject({
      ...
      status: "active",
      enabled: true,
      config: expect.objectContaining({ templateId: "pilot.synthetic-todo-kv" }),
    });
    expect(install.profile).toMatchObject({
      profileKey: "pilot.examples.safe-read-only-todo-kv.profile",
      defaultAction: "deny",
      status: "active",
    });
  ...
  it("rejects raw Google Sheets connection patches that claim another company's spreadsheet", async () => {
    ...
    const res = await request(app)
      ...
      .send({
        config: {
          templateId: "pilot.google-sheets",
  ...
  it("creates and resolves an agent-initiated user authorization grant card", async () => {
    ...
    const workspaceStarted = await service.startOAuth(company.id, connected.connectionId, {
      redirectUri: "https://pilot.example/api/tools/oauth/callback",
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (url, init) => {
      ...
      if (href === "https://mcp.slack.com/mcp") {
        return mcpHttpResponse({ jsonrpc: "2.0", id: "pilot-catalog-refresh", result: { tools: [] } });
      }
  ...
  it("starts and completes OAuth app sign-in with PKCE state and secret-backed tokens", async () => {
    vi.stubEnv("PILOT_TOOL_OAUTH_SLACK_CLIENT_ID", "slack-client-id");
    vi.stubEnv("PILOT_TOOL_OAUTH_SLACK_CLIENT_SECRET", "slack-client-secret");
    vi.stubEnv("PILOT_PUBLIC_URL", "https://pilot-public.example");
    const company = await createCompany(db);
    const app = createRouteApp(db);
    ...
    expect(startUrl.searchParams.get("redirect_uri")).toBe("https://pilot-public.example/api/tools/oauth/callback");
    ...
    const fetchMock = vi.spyOn(globalThis, "fetch").mockImplementation(async (url, init) => {
      ...
      if (href === "https://slack.com/api/oauth.v2.access") {
        ...
        expect(body.get("redirect_uri")).toBe("https://pilot-public.example/api/tools/oauth/callback");
        return {
          ok: true,
          json: async () => ({
            ok: true,
  ...
  it("requires non-viewer board access to start OAuth for active app connections", async () => {
    vi.stubEnv("PILOT_TOOL_OAUTH_SLACK_CLIENT_ID", "slack-client-id");
    vi.stubEnv("PILOT_PUBLIC_URL", "http://pilot.test");
    const company = await createCompany(db);
    const service = toolAccessService(db);
  ...
  it("discovers Notion MCP OAuth metadata, registers one public client, and reuses it", async () => {
    ...
    const redirectUri = "https://pilot-dev.tail29c1aa.ts.net/api/tools/oauth/callback";
    ...
    expect(registrationBodies).toEqual([{
      client_name: "Pilot (pilot-dev.tail29c1aa.ts.net)",
      redirect_uris: [redirectUri],
      grant_types: ["authorization_code", "refresh_token"],
      response_types: ["code"],
      token_endpoint_auth_method: "none",
  ...
  it("fails fast when Notion DCR is attempted from a non-loopback HTTP origin", async () => {
    ...
    await expect(service.startOAuth(company.id, connected.connectionId, {
      redirectUri: "http://pilot-dev:3100/api/tools/oauth/callback",
      actor: { actorType: "user", actorId: "board" },
    })).rejects.toMatchObject({
      status: 422,
  ...
  it("starts OAuth only for the marked Smoke Lab HTTP fixture", async () => {
    ...
    const [application] = await db.insert(toolApplications).values({
      companyId: company.id,
      applicationKey: "pilot.smoke-lab.http-fixture",
    ...
    expect(`${authorizationUrl.origin}${authorizationUrl.pathname}`).toBe(
      `http://pilot.test/api/companies/${company.id}/smoke-lab/oauth/authorize`,
    );
    expect(authorizationUrl.searchParams.get("client_id")).toBe("pilot-smoke-lab");
    ...
    expect(fetchMock.mock.calls.map(([url]) => String(url))).toContain(
      `http://pilot.test/api/companies/${company.id}/smoke-lab/oauth/token`,
    );
  ...
  it("reconnects a gallery app by rotating the existing credential in place (PIL-10859)", async () => {
    const company = await createCompany(db);
    const service = toolAccessService(db);
    mockToolsList([
  ...
  it("summarizes runtime health and flags stale slots plus degraded connections", async () => {
    ...
    const [connection] = await db.insert(toolConnections).values({
      ...
      enabled: true,
      config: { templateId: "pilot.echo-calculator-time" },
      transportConfig: { templateId: "pilot.echo-calculator-time" },
    ...
    await db.insert(toolRuntimeSlots).values({
      ...
      applicationId: application.id,
      connectionId: connection.id,
      slotKey: `${connection.id}:pilot.echo-calculator-time`,
      ownerScopeType: "connection",
      ownerScopeId: connection.id,
      runtimeKind: "local_stdio",
      status: "running",
    ...
    expect(preview.drafts).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          warnings: [expect.stringContaining("Pilot secret")],
        }),
        expect.objectContaining({
          name: "local",
          transport: "local_stdio",
          ...
          warnings: [expect.stringContaining("approved Pilot template")],
        }),
      ]),
    );
  });
...
describe("classifyRisk", () => {
  ...
  it("classifies namespaced write verbs as write (PIL-10902)", () => {
