...
import { resetClaudeCliCapabilitiesCacheForTests, testEnvironment } from "@pilotai/adapter-claude-local/server";
...
const ORIGINAL_PILOT_HOME = process.env.PILOT_HOME;
const ORIGINAL_PILOT_INSTANCE_ID = process.env.PILOT_INSTANCE_ID;
...
afterEach(() => {
  ...
  if (ORIGINAL_PILOT_HOME === undefined) {
    delete process.env.PILOT_HOME;
  } else {
    process.env.PILOT_HOME = ORIGINAL_PILOT_HOME;
  }
  if (ORIGINAL_PILOT_INSTANCE_ID === undefined) {
    delete process.env.PILOT_INSTANCE_ID;
  } else {
    process.env.PILOT_INSTANCE_ID = ORIGINAL_PILOT_INSTANCE_ID;
  }
});
...
describe("claude_local environment diagnostics", () => {
  ...
  it("creates a missing working directory when cwd is absolute", async () => {
    const cwd = path.join(
      os.tmpdir(),
      `pilot-claude-local-cwd-${Date.now()}-${Math.random().toString(16).slice(2)}`,
      "workspace",
    );
    ...
    expect(
      result.checks.some(
        (check) =>
          ...
          check.message === "Working directory is valid: /srv/pilot/workspace",
      ),
    ).toBe(true);
  ...
  it("uses the managed Claude config seed for sandbox hello probes", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-claude-envtest-managed-config-"));
...
if (!configDir.includes(".pilot-runtime/claude/config")) {
...
`, "utf8");
  ...
  it("warns and omits --effort for sandbox probes when the installed Claude CLI does not advertise it", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-claude-envtest-sandbox-effort-"));
