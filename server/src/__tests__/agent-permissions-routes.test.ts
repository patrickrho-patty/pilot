...
function registerModuleMocks() {
  vi.doMock("@pilotai/adapter-opencode-local/server", async () => {
    const actual = await vi.importActual<typeof import("@pilotai/adapter-opencode-local/server")>("@pilotai/adapter-opencode-local/server");
    return {
      ...actual,
...
describe.sequential("agent permission routes", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.doUnmock("@pilotai/shared/telemetry");
    ...
    vi.doUnmock("@pilotai/adapter-opencode-local/server");
    registerModuleMocks();
    vi.resetAllMocks();
    ...
    const res = await requestApp(app, (baseUrl) => request(baseUrl)
      ...
      .send({
        adapterConfig: {
          workspaceStrategy: {
            type: "git_worktree",
            provisionCommand: "touch /tmp/pilot-rce",
          },
        },
      }));
