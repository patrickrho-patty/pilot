...
describeEmbeddedPostgres("createPluginSecretsHandler shared vault integration", () => {
  ...
  const secretsTmpDir = path.join(os.tmpdir(), `pilot-plugin-secrets-${randomUUID()}`);
  ...
  async function seedPlugin() {
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "pilot.plugin-secrets-test",
      packageName: "@pilotai/plugin-secrets-test",
      version: "0.0.1",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
        id: "pilot.plugin-secrets-test",
        apiVersion: 1,
        version: "0.0.1",
