...
} from "@pilotai/adapter-utils/acpx-engine/startup-timing";
...
describeEmbeddedPostgres("environmentRuntimeService", () => {
  ...
  it("acquires and releases an SSH run lease through the runtime seam", async () => {
    ...
    const fixtureRoot = await mkdtemp(path.join(os.tmpdir(), "pilot-environment-runtime-ssh-"));
  ...
  it("destroys the remote plugin sandbox when the lease insert rejects a foreign-company binding", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "pilot.fake-plugin-sandbox-provider",
      packageName: "@pilotai/plugin-fake-sandbox",
      version: "1.0.0",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
        id: "pilot.fake-plugin-sandbox-provider",
        apiVersion: 1,
        version: "1.0.0",
  ...
  it("reports the plugin cleanup worker not ready while its worker is down, then ready after it recovers", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "pilot.fake-plugin-ready-sandbox-provider",
      packageName: "@pilotai/plugin-fake-ready-sandbox",
      version: "1.0.0",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
        id: "pilot.fake-plugin-ready-sandbox-provider",
        apiVersion: 1,
        version: "1.0.0",
  ...
  it("reports the plugin cleanup provider not ready while the plugin is not ready, then ready after it recovers", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "pilot.fake-plugin-reload-sandbox-provider",
      packageName: "@pilotai/plugin-fake-reload-sandbox",
      version: "1.0.0",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
        id: "pilot.fake-plugin-reload-sandbox-provider",
        apiVersion: 1,
        version: "1.0.0",
  ...
  it("reports the plugin cleanup provider not ready while the plugin is missing, then ready after it returns", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "pilot.fake-plugin-missing-sandbox-provider",
      packageName: "@pilotai/plugin-fake-missing-sandbox",
      version: "1.0.0",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
        id: "pilot.fake-plugin-missing-sandbox-provider",
        apiVersion: 1,
        version: "1.0.0",
  ...
  const SECRET_REF_PLUGIN_KEY = "pilot.secret-plugin-sandbox-provider";
  ...
  async function registerSecretRefPluginProvider(): Promise<string> {
    ...
    await db.insert(plugins).values({
      ...
      packageName: "@pilotai/plugin-secret-sandbox",
      version: "1.0.0",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
  ...
  it("does not reuse a sandbox lease owned by a different agent for the same execution workspace", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "acme.environments",
      packageName: "@acme/pilot-environments",
      version: "1.0.0",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
