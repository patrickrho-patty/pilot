...
import { ensureCursorSkillsInjected } from "@pilotai/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing Pilot skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("pilot-cursor-skills-src-");
    const skillsHome = await makeTempDir("pilot-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "pilot");
    await createSkillDir(skillsDir, "pilot-create-agent");
    ...
    const injectedA = path.join(skillsHome, "pilot");
    const injectedB = path.join(skillsHome, "pilot-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "pilot")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "pilot-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "pilot"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "pilot-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("pilot-cursor-preserve-src-");
    const skillsHome = await makeTempDir("pilot-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "pilot");
    ...
    expect((await fs.lstat(path.join(skillsHome, "pilot-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("pilot-cursor-fail-src-");
    const skillsHome = await makeTempDir("pilot-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
