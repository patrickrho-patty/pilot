...
describe("GET /health", () => {
  ...
  it("exposes public stack metadata on cloud-simulated health", async () => {
    const app = createApp(undefined, testServerInfo, undefined, {
      ...
      PILOT_PRIMARY_HOST: "acme.pilot.app",
      PILOT_CLOUD_API_ORIGIN: "https://app.pilot.app",
    });
    ...
    expect(res.body.cloud).toEqual({
      ...
      stackSlug: "acme",
      cloudBaseUrl: "https://app.pilot.app",
    });
  });
  ...
  it("surfaces a stale database backup warning in full health details", async () => {
    const backupDir = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-health-backups-"));
    const backupFile = path.join(backupDir, "pilot-20260705-031702.sql.gz");
    ...
    expect(res.body.databaseBackup).toMatchObject({
      ...
      maxAgeHours: 26,
      latestBackup: {
        name: "pilot-20260705-031702.sql.gz",
        ageHours: 33.7,
      },
      warnings: [
        {
  ...
  it("surfaces database backup failure markers in full health details", async () => {
    const backupDir = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-health-backups-"));
    const backupFile = path.join(backupDir, "pilot-20260706-031702.sql.gz");
  ...
  it("finds conventional database backup failure markers without an explicit alert file", async () => {
    const backupRoot = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-health-backups-root-"));
  ...
  it("surfaces redacted database backup warnings for anonymous authenticated probes", async () => {
    const backupDir = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-health-redacted-backups-"));
    const backupFile = path.join(backupDir, "pilot-20260705-031702.sql.gz");
    fs.writeFileSync(backupFile, "backup");
    fs.utimesSync(
      backupFile,
