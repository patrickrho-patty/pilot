/**
 * HTTP-level coverage for the Better Auth exchange endpoint (PIL-17572).
 *
...
const INSTANCE_ID = "pil-17572-endpoint-1122334455";
...
describe("GET /api/auth/workspace-handoff/exchange", () => {
  it("redirects to the requested board path and sets exactly one session cookie", async () => {
    ...
    const response = await exchange(app, mintTicket({ next: "/PIL/issues/PIL-17572" }));

    expect(response.status).toBe(302);
    expect(response.headers.location).toBe(`${ORIGIN}/PIL/issues/PIL-17572`);
