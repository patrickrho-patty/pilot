...
import { companies, companySkills, createDb, projects, projectWorkspaces } from "@pilotai/db";
...
describeEmbeddedPostgres("company skill local import boundary", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-skill-import-boundary-");
  ...
  it("allows configured workspace imports and rejects out-of-tree and symlink escapes", async () => {
    ...
    const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-approved-workspace-"));
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-outside-skill-"));
