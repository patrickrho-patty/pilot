...
import { companies, createDb, projects as projectsTable } from "@pilotai/db";
import {
...
// Verifies the PIL-69 data layer: the 0094 migration applies cleanly (the
...
describeEmbeddedPostgres("project icon persistence", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-project-icon-");
  ...
  // PIL-71: new projects must NOT auto-assign a color — they stay neutral gray
