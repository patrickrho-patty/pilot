...
vi.mock("@pilotai/adapter-codex-local/server", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@pilotai/adapter-codex-local/server")>();
  return {
    ...actual,
  ...
  return {
    async acquireLoginLease(input) {
      ...
      const lease: LoginSessionLease = {
        ...
        authPath: `/tmp/pilot-adapter-login/${input.sessionId}/auth.json`,
        driver: {
