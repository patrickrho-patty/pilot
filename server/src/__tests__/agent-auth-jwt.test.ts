...
describe("agent local JWT", () => {
  ...
  it("falls back to BETTER_AUTH_SECRET when PILOT_AGENT_JWT_SECRET is absent", () => {
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "pilot";
    process.env[audienceEnv] = "pilot-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
  ...
  // --- Instance isolation (PIL-12899) ---------------------------------------
  // A worktree/fork control-plane instance runs under a distinct
  // PILOT_INSTANCE_ID but deliberately shares PILOT_AGENT_JWT_SECRET
  ...
  it("rejects a fork/worktree-minted token on the live control plane", () => {
    ...
    process.env[instanceIdEnv] = "pil-12899-worktree";
  ...
  it("defaults TTL to 48h when PILOT_AGENT_JWT_TTL_SECONDS is unset", () => {
  ...
  it("accepts master-secret-signed tokens when PILOT_AGENT_JWT_DISABLE_LEGACY_FALLBACK is unset", () => {
    vi.setSystemTime(new Date("2026-01-01T00:00:00.000Z"));
    delete process.env[disableLegacyFallbackEnv];
  ...
  it("rejects master-secret-signed tokens when PILOT_AGENT_JWT_DISABLE_LEGACY_FALLBACK is enabled", () => {
    vi.setSystemTime(new Date("2026-01-01T00:00:00.000Z"));
    process.env[disableLegacyFallbackEnv] = "true";
    const legacyToken = craftLegacyMasterSecretToken(process.env[secretEnv]!, "company-legacy");
    expect(verifyLocalAgentJwt(legacyToken)).toBeNull();
  ...
  it("still verifies per-company-signed tokens when PILOT_AGENT_JWT_DISABLE_LEGACY_FALLBACK is enabled", () => {
    vi.setSystemTime(new Date("2026-01-01T00:00:00.000Z"));
    process.env[disableLegacyFallbackEnv] = "true";
    const token = createLocalAgentJwt("agent-1", "company-1", "claude_local", "run-1");
