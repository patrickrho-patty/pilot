...
vi.mock("@pilotai/shared/telemetry", async () => {
  const actual = await vi.importActual<typeof import("@pilotai/shared/telemetry")>(
    "@pilotai/shared/telemetry",
  );
  return {
    ...actual,
...
describeEmbeddedPostgres("heartbeat orphaned process recovery", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-heartbeat-recovery-");
  ...
  async function withTempPilotHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-hot-restart-"));
  ...
  it("blocks failed recovery work in place during immediate terminal-run cleanup", async () => {
    ...
    await db
      .update(issues)
      .set({
        title: "Recover stalled issue PIL-1",
  ...
  it("schedules an infra retry for a setup failure caused by a transient sandbox provider worker restart", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "pilot.kubernetes-sandbox-provider",
      packageName: "@pilotai/kubernetes-sandbox-provider",
      version: "1.0.0",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
        id: "pilot.kubernetes-sandbox-provider",
        apiVersion: 1,
        version: "1.0.0",
  ...
  it("blocks a git-sensitive local adapter before launch when a project-workspace-linked issue is missing its project id", async () => {
    ...
    await db.insert(projectWorkspaces).values({
      ...
      cwd: `/tmp/pilot-missing-workspace-${randomUUID()}`,
      isPrimary: true,
    });
    await db
      .update(issues)
  ...
  it("repairs the PIL-16986 deliberate wait through the original owner when no target exists", async () => {
    const { companyId, agentId, issueId } = await seedStrandedIssueFixture({
      status: "in_progress",
      runStatus: "cancelled",
      retryReason: "issue_continuation_needed",
