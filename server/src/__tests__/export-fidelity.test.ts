...
import { companies, createDb, issueLabels, issueRelations, issues, labels } from "@pilotai/db";
...
describeEmbeddedPostgres("export fidelity counts", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-export-fidelity-");
  ...
  it("builds a report whose warnings reflect the collected counts", async () => {
    ...
    expect(report.schema).toBe("pilot-export-fidelity-v1");
