...
async function createGitRepo() {
  const repoRoot = await mkdtemp(path.join(os.tmpdir(), "pilot-finalize-branch-repo-"));
...
async function seedRunTarget(db: Db, repoRoot: string) {
  ...
  await db.insert(issues).values({
    ...
    identifier: `PIL-${issueId.replace(/-/g, "").slice(0, 6).toUpperCase()}`,
...
describeEmbeddedPostgres("heartbeat workspace finalization branch guard", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-finalize-branch-");
