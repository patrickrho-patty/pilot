...
import { buildAgentMentionHref, buildProjectMentionHref, MAX_ISSUE_REQUEST_DEPTH, type IssueWorkMode } from "@pilotai/shared";
...
describeEmbeddedPostgres("issueService.list participantAgentId", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-issues-service-");
  ...
  it("ranks comment matches ahead of description-only matches", async () => {
    ...
    await db.insert(issueComments).values({
      ...
      body: "Reference: https://github.com/pilotai/pilot/pull/3303",
    });
  ...
  it("hides plugin operation issues from default lists and inbox-style filters while preserving explicit retrieval", async () => {
    ...
    await db.insert(issues).values([
      ...
      },
      {
        ...
        priority: "medium",
        assigneeAgentId: agentId,
        originKind: "plugin:pilot.missions:operation",
      ...
      },
      {
        ...
        priority: "medium",
        assigneeAgentId: agentId,
        originKind: "plugin:pilot.missions:operation:evaluation",
      ...
      },
      {
        ...
        priority: "medium",
        assigneeAgentId: agentId,
        originKind: "plugin:pilotai.content-machine:evaluation",
    ...
    await expect(svc.list(companyId, { originKind: "plugin:pilot.missions:operation" }))
    ...
    await expect(svc.list(companyId, { originId: "mission-alpha:operation-1" }))
      .resolves.toEqual([expect.objectContaining({ id: operationIssueId })]);
    await expect(svc.list(companyId, { originKindPrefix: "plugin:pilot.missions:operation" }))
...
describeEmbeddedPostgres("issueService.findOpenAncestorCreatedByAgent", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-issues-ancestor-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
    await ensureIssueRelationsTable(db);
  }, 20_000);
...
describeEmbeddedPostgres("issueService.create workspace inheritance", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-issues-create-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
    await ensureIssueRelationsTable(db);
  }, 20_000);
...
describeEmbeddedPostgres("issueService blockers and dependency wake readiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-issues-blockers-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
    await ensureIssueRelationsTable(db);
  }, 20_000);
  ...
  it("adds terminal blockers to immediate blocked-by summaries", async () => {
    ...
    await db.insert(issues).values([
      { id: issueA, companyId, identifier: "PIL-1", title: "Issue A", status: "blocked", priority: "medium" },
      { id: issueB, companyId, identifier: "PIL-2", title: "Issue B", status: "blocked", priority: "medium" },
      { id: issueC, companyId, identifier: "PIL-3", title: "Issue C", status: "blocked", priority: "medium" },
      { id: issueD, companyId, identifier: "PIL-4", title: "Issue D", status: "todo", priority: "high" },
    ]);
  ...
  it("gates dependents on the blocker's own workspace-finalize barrier until sync-back succeeds", async () => {
    ...
    expect(pendingDependent?.blockerAttention).toMatchObject({
      ...
      pendingFinalizeBlockerIssueIds: [blockerId],
      sampleBlockerIdentifier: "PIL-15043",
    });
    await expect(
  ...
  it("rejects execution when unresolved blockers remain", async () => {
    ...
    const blockedId = randomUUID();
    await db.insert(issues).values([
      { id: blockerId, companyId, identifier: "PIL-1", title: "Blocker", status: "todo", priority: "medium" },
      {
        id: blockedId,
        companyId,
        title: "Blocked",
...
describeEmbeddedPostgres("issueService.findMentionedProjectIds", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-issues-mentioned-projects-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
    await ensureIssueRelationsTable(db);
  }, 20_000);
...
describeEmbeddedPostgres("issueService.clearExecutionRunIfTerminal", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-issues-execution-lock-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
  }, 20_000);
...
describeEmbeddedPostgres("accepted plan decomposition", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-accepted-plan-decomposition-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
  }, 20_000);
...
describeEmbeddedPostgres("issueService.assertCheckoutOwner stale checkout adoption", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-issues-checkout-owner-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
  }, 20_000);
...
describeEmbeddedPostgres("issueService.addComment createdByRunId", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-issues-comment-runid-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
  ...
  it("nulls out a non-UUID x-pilot-run-id instead of 500-ing", async () => {
