 ...
 *
 * - Cloud-managed instances (PILOT_MANAGED_CONFIG present) may only
...
import { createDb, plugins } from "@pilotai/db";
...
async function createBuiltPluginFixture(parentDir: string, nameSuffix: string): Promise<FixturePlugin> {
  ...
  const packageName = `@pilotai/${slug}`;
  const pluginKey = `pilot.${slug.replace(/^plugin-/, "").replace(/-/g, "_")}`;
...
describeEmbeddedPostgres("plugin install route security floor", () => {
  ...
  const originalManagedConfig = process.env["PILOT_MANAGED_CONFIG"];

  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-plugin-install-guard-");
  ...
  afterEach(async () => {
    ...
    if (originalManagedConfig === undefined) {
      ...
      process.env["PILOT_MANAGED_CONFIG"] = originalManagedConfig;
    }
  });
  ...
  describe("cloud-managed instances", () => {
    it("rejects npm installs outright", async () => {
      process.env["PILOT_MANAGED_CONFIG"] = CLOUD_MANAGED_CONFIG;
      const app = await createInstallApp(db);

      const res = await request(app)
        .post("/api/plugins/install")
        .send({ packageName: "pilot-plugin-anything", version: "1.0.0" });
    ...
    it("rejects npm installs even when the managed config document is corrupted", async () => {
      ...
      process.env["PILOT_MANAGED_CONFIG"] = CLOUD_MANAGED_CONFIG;
      const app = await createInstallApp(db);
      process.env["PILOT_MANAGED_CONFIG"] = "{definitely not json";

      const res = await request(app)
        .post("/api/plugins/install")
        .send({ packageName: "pilot-plugin-anything" });
  ...
  describe("self-hosted npm installs (behavior unchanged)", () => {
    ...
    it("does not apply the cloud floor when PILOT_MANAGED_CONFIG is absent", async () => {
      const app = await createInstallApp(db);
      ...
      const res = await request(app)
        .post("/api/plugins/install")
        .send({ packageName: `pilot-plugin-guard-missing-${randomUUID().slice(0, 8)}` });
