...
import { execute, testEnvironment } from "@pilotai/adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@pilotai/adapter-openclaw-gateway/ui";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
        ...
        buildContext(
          ...
          },
          {
            ...
            context: {
              ...
              issueIds: ["issue-123"],
              pilotWorkspace: {
                cwd: "/tmp/worktrees/pil-123",
                strategy: "git_worktree",
                branchName: "pil-123-test",
              },
              pilotWorkspaces: [
                {
                  id: "workspace-1",
      ...
      expect(payload?.sessionKey).toBe("pilot:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("PILOT_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("PILOT_TASK_ID=task-123");
      expect(String(payload?.message ?? "")).toContain("## Pilot Wake Payload");
      ...
      expect(payload?.pilot).toBeUndefined();
