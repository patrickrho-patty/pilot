...
describe("awsSecretsManagerProvider", () => {
  ...
  it("creates Pilot-managed AWS secrets without persisting plaintext in provider material", async () => {
    ...
    const provider = createAwsSecretsManagerProvider({
      ...
      gateway: {
        async createSecret(input) {
          ...
          return {
            ARN: "arn:aws:secretsmanager:us-east-1:123456789012:secret:pilot/prod-use1/company-1/openai-api-key",
    ...
    expect(calls).toEqual([
      expect.objectContaining({
        ...
        input: expect.objectContaining({
          Name: "pilot/prod-use1/company-1/openai-api-key",
          KmsKeyId: "arn:aws:kms:us-east-1:123456789012:key/test",
        }),
      }),
    ]);
    expect(JSON.stringify(prepared)).not.toContain("super-secret-value");
    expect(prepared.externalRef).toContain("pilot/prod-use1/company-1/openai-api-key");
      ...
      expect.objectContaining({
        ...
        input: expect.objectContaining({
          ...
          Tags: expect.arrayContaining([
            { Key: "pilot:provider-owner", Value: "platform" },
            { Key: "pilot:environment", Value: "production" },
          ]),
        }),
      }),
    ]);
  ...
  it("signs AWS Secrets Manager JSON requests with default runtime credentials", async () => {
    ...
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(
        JSON.stringify({
          ARN: "arn:aws:secretsmanager:us-east-1:123456789012:secret:pilot/prod/company-1/openai-api-key",
          VersionId: "aws-version-1",
        }),
        { status: 200 },
      ),
  ...
  it("rejects linked external references under the Pilot-managed namespace", async () => {
    const provider = createAwsSecretsManagerProvider({
      config: {
        region: "us-east-1",
        endpoint: "https://secretsmanager.us-east-1.amazonaws.com",
    ...
    await expect(
      provider.linkExternalSecret({
        externalRef:
          "arn:aws:secretsmanager:us-east-1:123456789012:secret:pilot/prod-use1/company-2/openai-api-key",
        providerVersionRef: "linked-version-7",
      }),
    ).rejects.toThrow(/Pilot-managed namespace/i);
  });
  ...
  it("writes new values through to externally referenced AWS secrets as AWSCURRENT", async () => {
    ...
    const prepared = await provider.updateExternalSecretValue!({
      ...
      context: {
        ...
        secretName: "pilot-cloud/prod/provider/neon/admin-api-key",
        version: 2,
      },
    });
  ...
  it("rejects external value writes under the Pilot-managed namespace", async () => {
    const provider = createAwsSecretsManagerProvider({
      config: {
        region: "us-east-1",
        endpoint: "https://secretsmanager.us-east-1.amazonaws.com",
      ...
      gateway: {
        ...
        async listSecrets(input) {
          ...
          return {
            ...
            SecretList: [
              {
                ARN: "arn:aws:secretsmanager:us-east-1:123456789012:secret:pilot/prod-use1/company-1/openai",
                Name: "pilot/prod-use1/company-1/openai",
                KmsKeyId: "arn:aws:kms:us-east-1:123456789012:key/prod",
                Tags: [
                  { Key: "pilot:managed-by", Value: "pilot" },
                  { Key: "pilot:deployment-id", Value: "prod-use1" },
                  { Key: "pilot:company-id", Value: "company-1" },
                  { Key: "pilot:environment", Value: "production" },
                  { Key: "pilot:provider-owner", Value: "platform" },
                ],
              },
              {
                ARN: "arn:aws:secretsmanager:us-east-1:123456789012:secret:pilot/prod-use1/company-2/stripe",
                Name: "pilot/prod-use1/company-2/stripe",
                Tags: [
                  { Key: "pilot:managed-by", Value: "pilot" },
                  { Key: "pilot:company-id", Value: "company-2" },
                ],
              },
            ],
          };
  ...
  it("redacts AWS provider exception text when remote listing fails", async () => {
    const rawProviderMessage =
      "AccessDeniedException: User: arn:aws:sts::123456789012:assumed-role/prod/Pilot is not authorized to perform secretsmanager:ListSecrets on arn:aws:secretsmanager:us-east-1:123456789012:secret:prod/openai";
    const provider = createAwsSecretsManagerProvider({
      config: {
        region: "us-east-1",
        endpoint: "https://secretsmanager.us-east-1.amazonaws.com",
  ...
  it("resolves AWS secret values by provider version reference", async () => {
    ...
    const resolved = await provider.resolveVersion({
      material: {
        scheme: "aws_secrets_manager_v1",
        secretId: "arn:aws:secretsmanager:us-east-1:123456789012:secret:pilot/prod-use1/company-1/openai-api-key",
        versionId: "aws-version-2",
        source: "managed",
      },
      externalRef:
  ...
  it("warns when AWS provider configuration is incomplete and blocks managed writes", async () => {
    ...
    expect(health.message).toContain("missing PILOT_SECRETS_AWS_REGION");
    ...
    ).rejects.toThrow(/PILOT_SECRETS_AWS_REGION|AWS_REGION/i);
  });

  it("deletes only Pilot-managed AWS secrets", async () => {
    const calls: Array<{ op: string; input: Record<string, unknown> }> = [];
    const provider = createAwsSecretsManagerProvider({
      config: {
        region: "us-east-1",
  ...
  it("archives pending Pilot-managed AWS versions without deleting the secret", async () => {
    const calls: Array<{ op: string; input: Record<string, unknown> }> = [];
    const provider = createAwsSecretsManagerProvider({
      config: {
        region: "us-east-1",
