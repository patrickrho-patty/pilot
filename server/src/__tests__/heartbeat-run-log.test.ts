...
describe("compactRunLogChunk", () => {
  ...
  it("truncates oversized chunks after sanitizing them", () => {
    ...
    expect(compacted).toContain("[pilot truncated run log chunk:");
    expect(compacted.endsWith("tail")).toBe(true);
  });

  it("redacts Pilot credential shapes before persisting run-log chunks", () => {
    const chunk = [
      "Authorization: Bearer live-bearer-token-value",
      `export PILOT_API_KEY='pilot-shell-secret'`,
      `auth {"refresh_token":"refresh-token-fixture-secret"}`,
      `payload {"PILOT_API_KEY":"pilot-json-secret"}`,
      "--pilot-api-key=pilot-flag-secret",
    ].join("\n");
    ...
    expect(compacted).not.toContain("pilot-shell-secret");
    expect(compacted).not.toContain("refresh-token-fixture-secret");
    expect(compacted).not.toContain("pilot-json-secret");
    expect(compacted).not.toContain("pilot-flag-secret");
  });
});
