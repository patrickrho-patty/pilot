...
import { canonicalizeExternalObjectUrl } from "@pilotai/shared/external-objects-server";
...
describeEmbeddedPostgres("externalObjectService", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-external-objects-");
  ...
  async function createIssue(companyId = randomUUID()) {
    ...
    await db.insert(issues).values({
      id: issueId,
      companyId,
      identifier: `PIL-${companyId.replace(/-/g, "").slice(0, 12).toUpperCase()}`,
      title: "External refs",
  ...
  it("uses a mock plugin provider to detect and resolve non-GitHub objects", async () => {
    ...
    const [plugin] = await db.insert(plugins).values({
      ...
      packageName: "@pilot/mock-object-provider",
