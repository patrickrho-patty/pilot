...
import { execute } from "@pilotai/adapter-gemini-local/server";
...
const payload = {
  ...
  pilotEnvKeys: Object.keys(process.env)
    .filter((key) => key.startsWith("PILOT_") || key.startsWith("PILOT_") || key.startsWith("PILOT_"))
    .sort(),
};
if (capturePath) {
...
describe("gemini execute", () => {
  it("passes prompt via --prompt and injects pilot env vars", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-gemini-execute-"));
    ...
    try {
      ...
      expect(promptArg).toContain("Follow the pilot heartbeat.");
      expect(promptArg).toContain("Pilot runtime note:");
      ...
      expect(invocationPrompt).toContain("Pilot runtime note:");
      expect(invocationPrompt).toContain("PILOT_API_URL");
      expect(invocationPrompt).toContain("Pilot API access note:");
      expect(invocationPrompt).toContain("x-pilot-Run-Id");
  ...
  it("always passes --approval-mode yolo", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-gemini-yolo-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("normalizes turn-limit exhaustion into scheduler stop metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-gemini-max-turns-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("normalizes Gemini exit code 53 as max-turn exhaustion", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-gemini-exit-53-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingGeminiCommand(commandPath, {
  ...
  it("does not normalize unstructured turn-limit text into scheduler stop metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-gemini-max-turn-text-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingGeminiCommand(commandPath, {
  ...
  it("uses a compact wake delta instead of the full heartbeat prompt when resuming a session", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-gemini-resume-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      expect(promptArg).toContain("## Pilot Resume Delta");
      ...
      expect(promptArg).not.toContain("Follow the pilot heartbeat.");
    } finally {
      if (previousHome === undefined) {
        delete process.env.HOME;
      } else {
