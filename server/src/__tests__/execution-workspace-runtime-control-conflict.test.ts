...
// The integrated control path also takes the durable runtime-control lease (PIL-17205). This
...
function createRegisteredRepairFixture(
  ...
  const configDir = path.join(workspaceCwd, ".pilot");
  const sourceConfigPath = path.join(baseCwd, ".pilot", "config.json");
  ...
  fs.writeFileSync(path.join(path.dirname(sourceConfigPath), ".env"), "PILOT_INSTANCE_ID=repair-source\n");
  fs.writeFileSync(path.join(configDir, "config.json"), "{}\n");
  fs.writeFileSync(path.join(configDir, ".env"), `PILOT_INSTANCE_ID=${targetInstanceId}\n`);
...
/**
 * Route-level guarantees for PIL-17249: the competing lane in the PIL-17207 report must get a
...
describe.sequential("execution workspace runtime control conflict and failure reconciliation", () => {
  ...
  it("returns 422 with a stable reason when the repair seed manifest is malformed", async () => {
    const workspaceCwd = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-route-repair-malformed-"));
    try {
      const configDir = path.join(workspaceCwd, ".pilot");
      fs.mkdirSync(configDir, { recursive: true });
      fs.writeFileSync(path.join(configDir, "config.json"), "{}\n");
  ...
  it.each(["sibling", "foreign_instance", "symlink", "instance_mismatch"] as const)(
    ...
    async (variant) => {
      const fixture = createRegisteredRepairFixture(`pilot-route-repair-${variant}-`);
      ...
      try {
        ...
        fs.writeFileSync(
          ...
          `PILOT_INSTANCE_ID=${variant === "foreign_instance" ? "foreign-source" : "repair-source"}\n`,
        );
  ...
  it("rejects repair when the execution workspace has no registered base workspace", async () => {
    const workspaceCwd = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-route-repair-no-source-"));
    try {
      const configDir = path.join(workspaceCwd, ".pilot");
      fs.mkdirSync(configDir, { recursive: true });
      fs.writeFileSync(path.join(configDir, "config.json"), "{}\n");
  ...
  it("rejects repair when the registered base workspace has no runnable Pilot CLI", async () => {
    const fixture = createRegisteredRepairFixture("pilot-route-repair-no-cli-", { withCli: false });
    try {
      const res = await request(await createApp())
        .post(`/api/execution-workspaces/${executionWorkspaceId}/runtime-commands/repair`)
        .send({});
  ...
  it("returns 422 with a stable reason when the verified manifest names another instance", async () => {
    const fixture = createRegisteredRepairFixture("pilot-route-repair-wrong-instance-");
    try {
  ...
  it("uses the recorded instance pointer consistently for readiness, handoff, and repair", async () => {
    ...
    const fixture = createRegisteredRepairFixture("pilot-route-repair-success-", {
  ...
  it("completes database-only repair when no managed runtime service is configured", async () => {
    const fixture = createRegisteredRepairFixture("pilot-route-repair-db-only-");
    try {
      mockExecutionWorkspaceService.getById.mockResolvedValue(buildExecutionWorkspace({
  ...
  it("fails repair at the exact seed phase and leaves managed services stopped", async () => {
    const fixture = createRegisteredRepairFixture("pilot-route-repair-failure-");
    try {
      mockEnsurePersistedExecutionWorkspaceAvailable.mockResolvedValue({ cwd: fixture.workspaceCwd });
      mockSpawn.mockImplementation(() => {
        const child = new EventEmitter() as EventEmitter & {
