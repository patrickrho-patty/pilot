...
const PLUGIN_KEY = "pilot.daytona-sandbox-provider";
...
describe("listReadyPluginEnvironmentDrivers worker recovery", () => {
  ...
  it("does not lazy-start ready plugins outside the managed bundled allowlist", async () => {
    ...
    const drivers = await listReadyPluginEnvironmentDrivers({
      ...
      workerManager: worker.workerManager,
      recoverMissingWorker: {
        pluginKeys: ["pilot.kubernetes-sandbox-provider"],
        startWorker,
      },
    });
  ...
  it("lets callers suppress shared error-state writes on activation failure", async () => {
    const plugin = {
      ...createPlugin("ready"),
      packageName: "@pilotai/missing-sandbox-provider",
      packagePath: null,
      version: "1.0.0",
    };
  ...
  it("tears down partially-activated runtime state when error writes are suppressed", async () => {
    ...
    try {
      fs.writeFileSync(
        ...
        JSON.stringify({
          name: "@pilotai/daytona-sandbox-provider",
          version: "1.0.0",
          pilotPlugin: { manifest: "manifest.mjs" },
        }),
      );
      fs.writeFileSync(
      ...
      const plugin = {
        ...createPlugin("ready"),
        packageName: "@pilotai/daytona-sandbox-provider",
  ...
  it("bounds slow managed bundled recovery attempts without serial waits", async () => {
    ...
    try {
      const plugins = [
        ...
        createPlugin("ready", {
          id: "plugin-modal",
          pluginKey: "pilot.modal-sandbox-provider",
        }),
      ];
      ...
      const driversPromise = listReadyPluginEnvironmentDrivers({
        ...
        workerManager: worker.workerManager,
        recoverMissingWorker: {
          pluginKeys: [PLUGIN_KEY, "pilot.modal-sandbox-provider"],
          startWorker,
          timeoutMs: 25,
        },
      });
