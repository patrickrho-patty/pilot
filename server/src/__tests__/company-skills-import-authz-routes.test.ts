...
describeEmbeddedPostgres("company skill import authorization routes", () => {
  ...
  beforeAll(async () => {
    ...
    pilotHome = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-company-skills-import-authz-home-"));
    process.env.PILOT_HOME = pilotHome;
    process.env.PILOT_INSTANCE_ID = "default";
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-company-skills-import-authz-");
  ...
  async function writeSkillFixture(companyId: string) {
    ...
    const skillDir = await fs.mkdtemp(path.join(managedRoot, "pilot-import-authz-skill-"));
