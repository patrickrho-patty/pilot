...
vi.hoisted(() => {
  process.env.PILOT_HOME = "/tmp/pilot-test-home";
  process.env.PILOT_INSTANCE_ID = "vitest";
  process.env.PILOT_LOG_DIR = "/tmp/pilot-test-home/logs";
...
describeEmbeddedPostgres("access routes permissions upgrade compatibility", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-access-routes-permissions-upgrade-");
