...
import type { PilotPluginManifestV1 } from "@pilotai/shared";
import {
...
const multiMigrationPluginKey = "pilot.dbfixture";
const llmWikiPluginKey = "pilotai.plugin-llm-wiki";
...
describe("buildPluginWorkerEnv", () => {
  ...
  it("passes a first-party sandbox provider's documented credential env var to its own worker", () => {
    const env = buildPluginWorkerEnv({
      ...
      packageName: "@pilotai/plugin-daytona",
      packagePath: null,
      instanceInfo,
      processEnv: {
  ...
  it("does not pass the credential to a local plugin that self-declares the first-party name", () => {
    const env = buildPluginWorkerEnv({
      ...
      packageName: "@pilotai/plugin-daytona",
      packagePath: "/home/operator/.pilot/plugins/fake-daytona",
      trustedLocalPluginRoots: ["/app/packages/plugins"],
      instanceInfo,
      processEnv: {
        DAYTONA_API_KEY: "daytona-token",
...
describeEmbeddedPostgres("plugin database namespaces", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-plugin-db-");
    db = createDb(tempDb.connectionString);
  }, 20_000);

  afterEach(async () => {
    for (const pluginKey of ["pilot.dbtest", "pilot.escape", "pilot.refresh", multiMigrationPluginKey, llmWikiPluginKey]) {
  ...
  async function createPluginPackage(manifest: PilotPluginManifestV1, migrationSql: string) {
    const packageRoot = await mkdtemp(path.join(os.tmpdir(), "pilot-plugin-package-"));
  ...
  async function createInstallablePluginPackage(
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        type: "module",
        pilotPlugin: { manifest: "./manifest.js" },
      }),
      "utf8",
    );
    await writeFile(
  ...
  function manifest(pluginKey = "pilot.dbtest"): PilotPluginManifestV1 {
    return {
      id: pluginKey,
      apiVersion: 1,
      version: "1.0.0",
  ...
  it("applies the bundled LLM Wiki migrations through the production validator", async () => {
    ...
    expect(constraints).not.toContain("pilot_distillation_cursor_company_id_wiki_id_source_sco_key");
    expect(constraints).not.toContain("pilot_distillation_work_i_company_id_wiki_id_idempotenc_key");
    expect(constraints).not.toContain("pilot_page_bindings_company_id_wiki_id_page_path_key");
    expect(uniqueColumnSets).not.toContain("wiki_pages:company_id,wiki_id,path");
    expect(uniqueColumnSets).not.toContain("pilot_distillation_cursors:company_id,wiki_id,source_scope,scope_key,source_kind");
    expect(uniqueColumnSets).not.toContain("pilot_distillation_work_items:company_id,wiki_id,idempotency_key");
    expect(uniqueColumnSets).not.toContain("pilot_page_bindings:company_id,wiki_id,page_path");
  });
  ...
  it("records a failed migration when SQL escapes the plugin namespace", async () => {
    const pluginManifest = manifest("pilot.escape");
    const packageRoot = await createPluginPackage(
      pluginManifest,
  ...
  it("refreshes persisted manifests from disk before activation", async () => {
    const staleManifest = manifest("pilot.refresh");
