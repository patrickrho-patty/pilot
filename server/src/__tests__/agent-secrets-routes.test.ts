...
import { LOW_TRUST_REVIEW_PRESET, type AgentApiKeyScope } from "@pilotai/shared";
...
describeEmbeddedPostgres("agent secret routes", () => {
  ...
  const secretsTmpDir = path.join(os.tmpdir(), `pilot-agent-secret-routes-${randomUUID()}`);
  ...
  it("deduplicates concurrent redaction registrations for the same run and value", async () => {
    ...
    expect(run.contextSnapshot).toMatchObject({
      pilotSecretRedactions: [expect.objectContaining({ fingerprintSha256: expect.any(String) })],
    });
    expect((run.contextSnapshot as { pilotSecretRedactions: unknown[] }).pilotSecretRedactions).toHaveLength(1);
  });
