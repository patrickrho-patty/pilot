...
import { activityLog, agents, companies, createDb, executionWorkspaces, goals, issues, projects, projectWorkspaces, type Db } from "@pilotai/db";
...
async function makeWorkspace() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-file-resources-"));
...
describeEmbeddedPostgres("workspace file resources", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-file-resources-");
  ...
  it("reads explicit cross-project git_repo workspaces with a local checkout", async () => {
    ...
    const readmePath = "content-os/cases/active/2026-06-06-pil-10199-bundled-skills/README.md";
...
describeEmbeddedPostgres("file resource route guards", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-file-resource-guards-");
    db = createDb(tempDb.connectionString);
  }, 60_000);
