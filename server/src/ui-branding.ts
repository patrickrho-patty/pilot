const FAVICON_BLOCK_START = "<!-- PILOT_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- PILOT_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- PILOT_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- PILOT_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.PILOT_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.PILOT_WORKTREE_NAME) ?? nonEmpty(env.PILOT_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.PILOT_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
  ...
  return {
    ...
    instanceId: nonEmpty(env.PILOT_INSTANCE_ID),
  };
}
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  const tags = [
    '<meta name="pilot-worktree-enabled" content="true" />',
    `<meta name="pilot-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="pilot-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="pilot-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ];
  if (branding.instanceId) {
    tags.push(`<meta name="pilot-instance-id" content="${escapeHtmlAttribute(branding.instanceId)}" />`);
  }
  return tags.join("\n");
}
