...
export function readConfigFile(): PilotConfig | null {
  ...
  try {
    ...
    throw new Error(`Invalid Pilot config at ${configPath}: failed to read or parse JSON: ${reason}`);
  }
  ...
  try {
    ...
    if (error instanceof ZodError) {
      throw new Error(`Invalid Pilot config at ${configPath}: ${formatConfigValidationError(error)}`);
    }
