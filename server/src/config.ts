...
import { resolvePilotEnvPath } from "./paths.js";
...
const PILOT_ENV_FILE_PATH = resolvePilotEnvPath();
if (existsSync(PILOT_ENV_FILE_PATH)) {
  loadDotenv({ path: PILOT_ENV_FILE_PATH, override: false, quiet: true });
}

const CWD_ENV_PATH = resolve(process.cwd(), ".env");
const isSameFile = existsSync(CWD_ENV_PATH) && existsSync(PILOT_ENV_FILE_PATH)
  ? realpathSync(CWD_ENV_PATH) === realpathSync(PILOT_ENV_FILE_PATH)
  : CWD_ENV_PATH === PILOT_ENV_FILE_PATH;
...
function detectTailnetBindHost(): string | undefined {
  const explicit = process.env.PILOT_TAILNET_BIND_HOST?.trim();
...
export function loadConfig(): Config {
  ...
  const providerFromEnvRaw = process.env.PILOT_SECRETS_PROVIDER;
  ...
  const storageProviderFromEnvRaw = process.env.PILOT_STORAGE_PROVIDER;
  ...
  const storageS3Bucket = process.env.PILOT_STORAGE_S3_BUCKET ?? fileStorage?.s3?.bucket ?? "pilot";
  const storageS3Region = process.env.PILOT_STORAGE_S3_REGION ?? fileStorage?.s3?.region ?? "us-east-1";
  const storageS3Endpoint = process.env.PILOT_STORAGE_S3_ENDPOINT ?? fileStorage?.s3?.endpoint ?? undefined;
  const storageS3Prefix = process.env.PILOT_STORAGE_S3_PREFIX ?? fileStorage?.s3?.prefix ?? "";
  const storageS3ForcePathStyle =
    process.env.PILOT_STORAGE_S3_FORCE_PATH_STYLE !== undefined
      ? process.env.PILOT_STORAGE_S3_FORCE_PATH_STYLE === "true"
      : (fileStorage?.s3?.forcePathStyle ?? false);
  const feedbackExportBackendUrl =
    process.env.PILOT_FEEDBACK_EXPORT_BACKEND_URL?.trim() ||
    process.env.PILOT_TELEMETRY_BACKEND_URL?.trim() ||
    undefined;
  const feedbackExportBackendToken =
    process.env.PILOT_FEEDBACK_EXPORT_BACKEND_TOKEN?.trim() ||
    process.env.PILOT_TELEMETRY_BACKEND_TOKEN?.trim() ||
    undefined;

  const deploymentModeFromEnvRaw = process.env.PILOT_DEPLOYMENT_MODE;
  const deploymentModeFromEnv =
    deploymentModeFromEnvRaw && DEPLOYMENT_MODES.includes(deploymentModeFromEnvRaw as DeploymentMode)
      ? (deploymentModeFromEnvRaw as DeploymentMode)
      : null;
  const deploymentMode: DeploymentMode = deploymentModeFromEnv ?? fileConfig?.server.deploymentMode ?? "local_trusted";
  const strictModeFromEnv = process.env.PILOT_SECRETS_STRICT_MODE;
  const secretsStrictMode =
    strictModeFromEnv !== undefined
      ? strictModeFromEnv === "true"
      : (fileSecrets?.strictMode ?? deploymentMode === "authenticated");
  const deploymentExposureFromEnvRaw = process.env.PILOT_DEPLOYMENT_EXPOSURE;
  ...
  const bindFromEnvRaw = process.env.PILOT_BIND;
  const bindFromEnv =
  ...
  const customBindHost = process.env.PILOT_BIND_HOST ?? fileConfig?.server.customBindHost;
  const authBaseUrlModeFromEnvRaw = process.env.PILOT_AUTH_BASE_URL_MODE;
  const authBaseUrlModeFromEnv =
    authBaseUrlModeFromEnvRaw &&
    AUTH_BASE_URL_MODES.includes(authBaseUrlModeFromEnvRaw as AuthBaseUrlMode)
      ? (authBaseUrlModeFromEnvRaw as AuthBaseUrlMode)
      : null;
  const publicUrlFromEnv = process.env.PILOT_PUBLIC_URL;
  ...
  const disableSignUpFromEnv = process.env.PILOT_AUTH_DISABLE_SIGN_UP;
  ...
  const companyDeletionEnvRaw = process.env.PILOT_ENABLE_COMPANY_DELETION;
  const companyDeletionEnabled =
    companyDeletionEnvRaw !== undefined
      ? companyDeletionEnvRaw === "true"
      : deploymentMode === "local_trusted";
  const databaseBackupEnabled =
    process.env.PILOT_DB_BACKUP_ENABLED !== undefined
      ? process.env.PILOT_DB_BACKUP_ENABLED === "true"
      : (fileDatabaseBackup?.enabled ?? true);
  const databaseBackupIntervalMinutes = Math.max(
    1,
    Number(process.env.PILOT_DB_BACKUP_INTERVAL_MINUTES) ||
      fileDatabaseBackup?.intervalMinutes ||
      60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    Number(process.env.PILOT_DB_BACKUP_RETENTION_DAYS) ||
      fileDatabaseBackup?.retentionDays ||
      7,
  );
  const databaseBackupDir = resolveHomeAwarePath(
    process.env.PILOT_DB_BACKUP_DIR ??
  ...
  // PILOT_DB_BACKUP_RETENTION_DAYS precedent above.
  const workspaceReaperCooldownDaysEnv =
    process.env.PILOT_WORKSPACE_REAPER_COOLDOWN_DAYS?.trim();
  ...
  return {
    ...
    uiDevMiddleware: process.env.PILOT_UI_DEV_MIDDLEWARE === "true",
    secretsProvider,
    secretsStrictMode,
