...
import type { BindMode, DeploymentExposure, DeploymentMode } from "@pilotai/shared";
...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.PILOT_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.PILOT_AGENT_JWT_SECRET === "string" ? parsed.PILOT_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `npx pilotai onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolvePilotEnvPath();
