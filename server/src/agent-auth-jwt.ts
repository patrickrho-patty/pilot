...
import { normalizeAgentApiKeyScope, type AgentApiKeyScope } from "@pilotai/shared";
...
function jwtConfig() {
  ...
  return {
    ...
    issuer: process.env.PILOT_AGENT_JWT_ISSUER ?? "pilot",
    audience: process.env.PILOT_AGENT_JWT_AUDIENCE ?? "pilot-api",
    ...
    // name) even though it deliberately shares PILOT_AGENT_JWT_SECRET with
 ...
 *    instanceId — never matches. See PIL-12896 for the incident this closes.
 *
 ...
 * have expired (set PILOT_AGENT_JWT_DISABLE_LEGACY_FALLBACK=true). Normal
...
export function verifyLocalAgentJwt(token: string): LocalAgentJwtClaims | null {
  ...
  // live plane (PIL-12896/PIL-12899). Fall back to the raw master secret so
  ...
  //
  // Operators should set `PILOT_AGENT_JWT_DISABLE_LEGACY_FALLBACK=true`
  // approximately one JWT TTL (~48h by default, see PILOT_AGENT_JWT_TTL_SECONDS)
