 ...
 */
export function getStartupTracer(name = "pilot.startup"): StartupTracerHandle {
  try {
    ...
    if (!tracerApiLoadFailed) {
      ...
      console.warn(
        "[pilot] @opentelemetry/api is not available; startup tracing uses a no-op tracer.",
        err,
      );
    }
    return NOOP_TRACER;
 ...
 */
export function getStartupTraceContext(name = "pilot.startup"): StartupTraceContextHandle {
  try {
    const require = createRequire(import.meta.url);
    const api = require("@opentelemetry/api") as {
      trace?: {
    ...
    if (!traceContextApiLoadFailed) {
      ...
      // eslint-disable-next-line no-console
      console.warn(
        "[pilot] @opentelemetry/api is not available; startup tracing uses a no-op trace context.",
        err,
      );
    }
    return NOOP_TRACE_CONTEXT;
...
export function recordProviderPluginSpan(input: {
  ...
  try {
    ...
    const tracer = trace.getTracer("pilot.startup");
...
export function shutdownInstrumentation(): Promise<void> {
  shutdownPromise ??= (async () => {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.error("[pilot] OpenTelemetry shutdown failed", err);
    }
  })();
...
export function resolveProtocol(): {
  ...
  switch (raw) {
    ...
    default:
      // eslint-disable-next-line no-console
      console.warn(
        `[pilot] Unknown OTEL_EXPORTER_OTLP_PROTOCOL=${raw}; falling back to grpc. ` +
 ...
 * `dist` in a built server; both sit inside the Pilot checkout. A server
 ...
 * is a Pilot-specific variable, not an OpenTelemetry SDK variable, so
 * Pilot controls this precedence.
 */
...
async function bootstrapOtel(endpoint: string): Promise<void> {
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.log(`[pilot] OpenTelemetry service.version=${serviceVersion}`);

    const sdk = new NodeSDK({
      resource: resourceFromAttributes({
        [ATTR_SERVICE_NAME]: process.env.OTEL_SERVICE_NAME || "pilot",
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.error(
        "[pilot] OpenTelemetry SDK failed to start; continuing without tracing",
        err,
      );
      return;
    }
    ...
    // eslint-disable-next-line no-console
    console.warn(
      "[pilot] OTEL_EXPORTER_OTLP_ENDPOINT is set but the @opentelemetry/* " +
