...
import { applyLegacyPilotEnvAliases } from "@pilotai/shared";
...
import { warnIfUnsupportedNodeVersion } from "@pilotai/shared/node-version";
...
export async function startServer(): Promise<StartedServer> {
  ...
  // Brand-rename alias window: map any legacy PILOT_* env onto unset
  // PILOT_* before any config read, so existing deployments (k8s secrets,
  // user shells, CI) keep working through the rename.
  const legacyEnvMapped = applyLegacyPilotEnvAliases();
  if (legacyEnvMapped.length > 0) {
    console.warn(`[pilot] ${legacyEnvMapped.length} PILOT_* env var(s) mapped to PILOT_* (legacy alias window)`);
  }
      ...
      if (!apply) {
        throw new Error(
          `${label} has pending migrations (${formatPendingMigrationSummary(state.pendingMigrations)}). ` +
            "Refusing to start against a stale schema. Run pnpm db:migrate or set PILOT_MIGRATION_AUTO_APPLY=true.",
        );
      }
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@pilot.local";
  ...
  if (config.databaseUrl) {
    ...
    if (runningPid) {
      ...
      const configuredAdminConnectionString = `postgres://pilot:pilot@127.0.0.1:${configuredPort}/postgres`;
      try {
        ...
        await ensurePostgresDatabase(configuredAdminConnectionString, "pilot");
        logger.warn(
    ...
    const embeddedAdminConnectionString = `postgres://pilot:pilot@127.0.0.1:${port}/postgres`;
    const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "pilot");
    if (dbStatus === "created") {
      logger.info("Created embedded PostgreSQL database: pilot");
    }
  
    const embeddedConnectionString = `postgres://pilot:pilot@127.0.0.1:${port}/pilot`;
  ...
  // Cloud managed-config contract (harness → app). Parse PILOT_MANAGED_CONFIG
  ...
  try {
    ...
    logger.error({ err }, "invalid PILOT_MANAGED_CONFIG; refusing to start (fail closed)");
    throw err;
  }
  ...
  setupLiveEventsWebSocketServer(server, db as any, {
    ...
    // self-gating: it returns null unless PILOT_CLOUD_TENANT_SERVER_TOKEN
        ...
        logger.warn(
          {
            ...
            // verified HTTPS origin (PIL-17158).
  ...
  // env (PILOT_EXECUTION_MODE=kubernetes). Runs BEFORE the heartbeat resumes
  // queued runs so the policy + managed k8s environments are in place. A bad
  // PILOT_EXECUTION_MODE / PILOT_K8S_* value throws and fails startup
  ...
  // a contradictory deployment that also forces PILOT_EXECUTION_MODE
  ...
  // set (PILOT_ADAPTERS). Must run after external adapters are loaded so the
  ...
  try {
    ...
    logger.error({ err }, "failed to reconcile adapter availability from PILOT_ADAPTERS");
    throw err;
  }
  ...
  await new Promise<void>((resolveListen, rejectListen) => {
    ...
    server.listen(listenPort, config.host, () => {
      ...
      void systemdNotify(["--ready", `--status=Listening on ${config.host}:${listenPort}`]).then((notified) => {
        if (notified) logger.info("Notified systemd that Pilot is ready");
      });
  ...
  
  {
    const shutdown = async (signal: "SIGINT" | "SIGTERM") => {
      ...
      const appShutdown = (app as { locals?: { pilotShutdown?: () => Promise<void> } }).locals
        ?.pilotShutdown;
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "Pilot server failed to start");
    process.exit(1);
  });
}
