...
interface IncomingMessageWithTerminalContext extends IncomingMessage {
  ...
  pilotTerminalUpgradeContext?: TerminalUpgradeContext;
}
...
export function setupEnvironmentCustomImageTerminalWebSocketServer(
  ...
  wss.on("connection", (socket: TerminalWsSocket, req: IncomingMessage) => {
    const upgradeContext = (req as IncomingMessageWithTerminalContext).pilotTerminalUpgradeContext;
    if (!upgradeContext) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    reqWithContext.pilotWebSocketHandled = true;
    ...
    reqWithContext.pilotTerminalUpgradeContext = {
