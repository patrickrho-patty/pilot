...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@pilotai/db";
import type { DeploymentMode } from "@pilotai/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  pilotWebSocketHandled?: boolean;
  pilotUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).pilotUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    if ((req as IncomingMessageWithContext).pilotWebSocketHandled) {
      return;
    }
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.pilotUpgradeContext = context;
