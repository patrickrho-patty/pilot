...
import type { DeploymentMode, SecretProviderConfigDiscoveryPreviewResult } from "@pilotai/shared";
...
const DEFAULT_PREFIX = "pilot";
const DEFAULT_OWNER_TAG = "pilot";
...
const AWS_RUNTIME_CREDENTIAL_WARNING =
  "AWS bootstrap credentials must be available to the Pilot server runtime through the AWS SDK default credential provider chain: IAM role/workload identity, AWS_PROFILE/SSO/shared credentials, web identity, container/instance metadata, or short-lived shell credentials.";
const AWS_CREDENTIAL_CUSTODY_WARNING =
  "Do not store AWS root credentials or long-lived IAM user access keys in Pilot company_secrets; the AWS provider bootstrap belongs in deployment infrastructure, the process environment, an AWS profile, or the orchestrator secret store.";
...
function readProviderVaultConfig(input: SecretProviderVaultRuntimeConfig): AwsSecretsManagerConfig {
  ...
  if (!Number.isFinite(recoveryWindow) || recoveryWindow < 7 || recoveryWindow > 30) {
    throw unprocessable(
      "PILOT_SECRETS_AWS_DELETE_RECOVERY_DAYS must be an integer between 7 and 30",
    );
  }
...
function getAwsConfigReadiness() {
  ...
  if (!region) {
    missingConfig.push("PILOT_SECRETS_AWS_REGION or AWS_REGION/AWS_DEFAULT_REGION");
  }
  if (!deploymentId) {
...
function loadAwsSecretsManagerConfig(): AwsSecretsManagerConfig {
  ...
  if (!region) {
    throw unprocessable(
      "AWS Secrets Manager provider requires PILOT_SECRETS_AWS_REGION or AWS_REGION",
    );
  }
  if (!deploymentId) {
    throw unprocessable(
      "AWS Secrets Manager provider requires PILOT_SECRETS_AWS_DEPLOYMENT_ID",
    );
  }
  if (!kmsKeyId) {
    throw unprocessable(
      "AWS Secrets Manager provider requires PILOT_SECRETS_AWS_KMS_KEY_ID",
    );
  }
...
function assertNotManagedNamespaceExternalRef(
  ...
  throw unprocessable(
    "AWS Pilot-managed namespace secrets cannot be imported as external references",
  );
}
...
function buildManagedSecretTags(
  ...
  if (!context) return [];
  return [
    { Key: "pilot:managed-by", Value: "pilot" },
    { Key: "pilot:provider-owner", Value: config.providerOwnerTag },
    { Key: "pilot:deployment-id", Value: config.deploymentId },
    { Key: "pilot:company-id", Value: context.companyId },
    { Key: "pilot:secret-key", Value: context.secretKey },
    { Key: "pilot:environment", Value: config.environmentTag },
  ];
}
...
function inferPathSignals(entry: AwsSecretsManagerListSecretEntry, tags: Map<string, string>) {
  ...
  const pilotDeploymentId = tagValue(tags, ["pilot:deployment-id"]);
  const pilotManaged = tagValue(tags, ["pilot:managed-by"])?.toLowerCase() === "pilot";
...
function discoverAwsProviderConfigCandidates(input: {
  ...
  for (const entry of input.entries) {
    ...
    const pilotManaged = tagValue(tags, ["pilot:managed-by"])?.toLowerCase() === "pilot";
    const pilotCompanyId = tagValue(tags, ["pilot:company-id"]);
    ...
    samples.push({
      ...
      environmentTag: tagValue(tags, ["pilot:environment", "environment", "env", "stage"]),
      ownerTag: tagValue(tags, ["pilot:provider-owner", "owner", "team", "service", "application"]),
      kmsKeyId: asOptionalNonEmptyString(entry.KmsKeyId),
      pilotManaged: pilotManaged,
      pilotCompanyId: pilotCompanyId,
    });
  }

  if (skippedForeignPilotSampleCount > 0) {
    skippedWarnings.push(
      `Skipped ${skippedForeignPilotSampleCount} Pilot-managed AWS secret sample(s) that were not tagged for this company.`,
    );
  }
  ...
  const candidates = [...groups.values()]
    ...
    .map((group) => {
      ...
      if (group.some((sample) => sample.pilotManaged && sample.pilotCompanyId === input.companyId)) {
        candidateWarnings.push("Sample includes Pilot-managed secrets for this company; do not import them as external references.");
      }
      ...
      return {
        ...
        signals: {
          ...
          sampleCount: group.length,
          pilotManagedSampleCount: group.filter((sample) => sample.pilotManaged).length,
          skippedForeignPilotSampleCount: skippedForeignPilotSampleCount,
        },
...
export function createAwsSecretsManagerProvider(
  ...
  async function validateConfig(
    ...
    if (!config.prefix) {
      warnings.push("PILOT_SECRETS_AWS_PREFIX should be set to a deployment-scoped prefix");
    }
  ...
  async function healthCheck(
    ...
    try {
      ...
      return {
        ...
        backupGuidance: [
          "Back up Pilot metadata separately from AWS-managed secrets.",
          "Restoring access requires the Pilot database plus the same AWS secret namespace and KMS permissions.",
        ],
      };
    } catch (error) {
      const readiness = getAwsConfigReadiness();
      ...
      return {
        ...
        details: {
          ...
          requiredProviderConfig: input?.providerConfig
            ? ["region"]
            : [
                "PILOT_SECRETS_AWS_REGION or AWS_REGION/AWS_DEFAULT_REGION",
    ...
    async createSecret(input) {
      ...
      try {
        const createInput = {
          ...
          Description: input.context ? `Pilot secret ${input.context.secretName}` : undefined,
