...
import { validateAdapterLoginCapability } from "@pilotai/adapter-utils";
...
function extractUiParserSource(
  ...
  const contractVersion = pkg.pilot?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no pilot.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }
