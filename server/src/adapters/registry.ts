...
import type { AdapterLoginCapability } from "@pilotai/adapter-utils";
import {
...
} from "@pilotai/adapter-claude-local";
import {
...
} from "@pilotai/adapter-codex-local";
import {
...
} from "@pilotai/adapter-cursor-local";
import {
...
} from "@pilotai/adapter-cursor-cloud/server";
import { agentConfigurationDoc as cursorCloudAgentConfigurationDoc } from "@pilotai/adapter-cursor-cloud";
import {
...
} from "@pilotai/adapter-gemini-local";
import {
...
} from "@pilotai/adapter-kimi-local";
import {
  createHermesGatewayServerAdapter,
  createHermesLocalServerAdapter,
} from "@pilotai/hermes-pilot-adapter";
import {
...
} from "@pilotai/adapter-opencode-local";
import {
  execute as openclawGatewayExecute,
  testEnvironment as openclawGatewayTestEnvironment,
} from "@pilotai/adapter-openclaw-gateway/server";
import {
  agentConfigurationDoc as openclawGatewayAgentConfigurationDoc,
  models as openclawGatewayModels,
} from "@pilotai/adapter-openclaw-gateway";
...
Pilot keeps this tombstone registered so stale acpx_local rows fail clearly instead of falling back to the process adapter.
`;
...
const externalAdaptersReady: Promise<void> = (async () => {
  try {
    ...
    for (const externalAdapter of externalAdapters) {
      ...
      if (overriding) {
        console.log(
          `[pilot] External adapter "${externalAdapter.type}" overrides built-in adapter`,
        );
    ...
    console.error("[pilot] Failed to load external adapters:", err);
  }
})();
...
/**
 * Memoized view of PILOT_ADAPTER_MODELS, keyed by the raw env string so
...
function getDeclaredAdapterModels(): ReturnType<typeof parseAdapterModelsEnv> {
  ...
  try {
    ...
    console.error(
      "[pilot] Invalid PILOT_ADAPTER_MODELS; ignoring declared model lists:",
      err,
    );
  }
...
export function setOverridePaused(type: string, paused: boolean): boolean {
  ...
  if (paused && !wasPaused) {
    ...
    console.log(`[pilot] Override paused for "${type}" — builtin adapter restored`);
    return true;
  }
  if (!paused && wasPaused) {
    pausedOverrides.delete(type);
    console.log(`[pilot] Override resumed for "${type}" — external adapter active`);
    return true;
  }
  return false;
}
