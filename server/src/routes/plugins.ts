...
import type { PluginPerformActionActorContext, ToolRunContext } from "@pilotai/plugin-sdk";
import { JsonRpcCallError, PLUGIN_RPC_ERROR_CODES } from "@pilotai/plugin-sdk";
import {
  assertAuthenticated,
  assertBoard,
  assertBoardOrAgent,
 ...
 * (`instance.plugins` in PILOT_HIDDEN_SETTINGS), plugin lifecycle and
    ...
    bundledPlugins.push({
      entry: {
        ...
        description: metadata.description
          ?? `Bundled Pilot plugin from ${path.relative(REPO_ROOT, packageRoot)}.`,
...
export function pluginRoutes(
   ...
   * `PILOT_MANAGED_CONFIG` environment variable) enforce a positive
