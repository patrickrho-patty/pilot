...
export function cloudRoutes(opts: {
  ...
  router.get("/stacks", async (req, res) => {
    ...
    if (!context.cloudOrigin || !context.stackId || !tenantToken) {
      throw new HttpError(503, "Pilot Cloud portfolio is not configured", {
    ...
    try {
      const upstream = await fetchImpl(portfolioUrl, {
        ...
        headers: {
          ...
          "x-pilot-cloud-user-id": actorUserId,
          "x-pilot-cloud-stack-id": context.stackId,
        },
      ...
      if (!upstream.ok) {
        logger.warn(
          ...
          "Pilot Cloud portfolio request failed",
        );
        throw new HttpError(502, "Pilot Cloud portfolio request failed", {
      ...
      try {
        ...
        throw new HttpError(502, "Pilot Cloud portfolio returned invalid JSON", {
