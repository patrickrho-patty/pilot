...
import { agents as agentsTable } from "@pilotai/db";
import type { CompanyPortabilityImportResult } from "@pilotai/shared";
import {
  MAX_ZIP_ENTRY_DECOMPRESSED_BYTES,
  MAX_ZIP_TOTAL_DECOMPRESSED_BYTES,
  readZipArchive,
} from "@pilotai/shared/portability-zip";
import {
...
// @pilotai/shared/company-import-transfer — the browser and CLI clients
...
function wantsAsyncImport(req: Request) {
  return req.query.async === "1" || req.header("x-pilot-cloud-async-import") === "1";
}
...
export function companyRoutes(db: Db, storage?: StorageService, options?: CompanyRoutesOptions) {
  ...
  router.post("/", (req, _res, next) => {
    ...
    if (isCloudManagedInstance()) {
      throw forbidden("Company creation is managed by Pilot Cloud", {
        code: "cloud_managed",
      });
    }
    next();
...
function cloudTenantRequestKey(req: Request) {
  return [
    req.actor.userId ?? "",
    req.header("x-pilot-cloud-stack-id")?.trim() ?? "",
    req.header("x-pilot-cloud-pilot-company-id")?.trim() ?? "",
  ].join(":");
}
