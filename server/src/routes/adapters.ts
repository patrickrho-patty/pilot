 ...
 * toggle server-side adapter code for the whole Pilot instance.
 *
 ...
 * (`instance.adapters` in PILOT_HIDDEN_SETTINGS), adapter management
...
interface AdapterInstallRequest {
  /** npm package name (e.g., "droid-pilot-adapter") or local path */
  packageName: string;
...
export function adapterRoutes() {
  ...
  router.post("/adapters/install", async (req, res) => {
    ...
    // e.g. "@henkey/hermes-pilot-adapter@0.3.0" → packageName + version
  ...
  // without modifying Pilot's source code.
  //
