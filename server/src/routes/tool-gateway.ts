...
import { activityLog, agents, toolApplications, toolConnections, toolInvocations } from "@pilotai/db";
import { humanizeConnectionDisplayName, type PermissionKey } from "@pilotai/shared";
import {
  createToolMcpGatewaySchema,
  createToolMcpGatewayTokenSchema,
  updateToolMcpGatewaySchema,
} from "@pilotai/shared/validators/tool-access";
...
function gatewayToken(req: { header(name: string): string | undefined }) {
  return req.header("x-pilot-tool-gateway-token")?.trim() || null;
}
    ...
    if (body.method === "initialize") {
      ...
      res.json({
        ...
        result: {
          ...
          capabilities: { tools: {} },
          serverInfo: { name: "Pilot MCP Gateway", version: "1.0.0" },
        },
      });
      return;
    }
