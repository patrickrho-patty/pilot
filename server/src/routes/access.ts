...
import type { DeploymentExposure, DeploymentMode, HumanCompanyMembershipRole, PermissionKey } from "@pilotai/shared";
import {
  forbidden,
  conflict,
  notFound,
 ...
 * (`instance.access` in PILOT_HIDDEN_SETTINGS), instance-admin user
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isPilotManaged: pilotSkillNames.has(entry.name),
      });
    }
  ...
  if (pilotSkillsDir) {
    ...
    } catch { /* skip Pilot skills directory */ }
  }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.pilotApiUrl)) {
    const legacyPilotApiUrl = nonEmptyTrimmedString(input.pilotApiUrl);
    if (legacyPilotApiUrl) merged.pilotApiUrl = legacyPilotApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    pilotApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.pilotApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
        ...
        if (apiBaseUrl.protocol !== "http:" && apiBaseUrl.protocol !== "https:") {
          ...
          if (mappedDefaultDashboardRoot) {
            diagnostics.push({
              ...
              hint: "Hermes dashboard and /chat routes are browser UI routes. Pilot gateway calls use /api/health and /api/v1/runs.",
            });
          }
  ...
  const rawPilotApiUrl =
    typeof defaults.pilotApiUrl === "string"
      ? defaults.pilotApiUrl.trim()
      : "";
  if (rawPilotApiUrl) {
    try {
      ...
      if (
        ...
        diagnostics.push({
          code: "openclaw_gateway_pilot_api_url_protocol",
          level: "warn",
          message: `pilotApiUrl must use http:// or https:// (got ${parsedPilotApiUrl.protocol}).`
        });
      } else {
        normalized.pilotApiUrl = parsedPilotApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_pilot_api_url_configured",
          level: "info",
          message: `pilotApiUrl set to ${parsedPilotApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_pilot_api_url_invalid",
        level: "warn",
        message: `Invalid pilotApiUrl: ${rawPilotApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Pilot host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Pilot is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run npx pilotai allowed-hostname <host>`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const skillPath = `/api/invites/${token}/skills/pilot`;
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an external Pilot agent, save your one-time claim secret, wait for board approval, then claim your Pilot API key through the standard claim endpoint. Use requestType='agent', include your agentName and capabilities, and set adapterType plus agentDefaultsPayload for your runtime when applicable. Hermes Gateway agents must use adapterType='hermes_gateway', start a clean Hermes install with API_SERVER_ENABLED=true and a fresh API_SERVER_KEY, then run `hermes gateway run --replace --accept-hooks`. Put the Hermes gateway URL in agentDefaultsPayload.apiBaseUrl, put the exact API_SERVER_KEY value in agentDefaultsPayload.apiKey, and put the reachable Pilot base URL in agentDefaultsPayload.pilotApiUrl. If you use the default Hermes dashboard root or /chat URL on port 9119, Pilot maps it to /api automatically. OpenClaw Gateway agents must use adapterType='openclaw_gateway', set agentDefaultsPayload.url to a ws:// or wss:// gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token.",
      inviteMessage: extractInviteMessage(invite),
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Runtime-specific adapter config. OpenClaw Gateway agents must include url (ws:// or wss://) and headers.x-openclaw-token. Hermes Gateway agents must include apiBaseUrl, apiKey set to the Hermes API_SERVER_KEY, and pilotApiUrl. A default Hermes dashboard root or /chat URL such as http://127.0.0.1:9119/chat is accepted and maps to /api. Other runtimes should include the config their adapter expects."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Pilot hostname is reachable and allowed via `npx pilotai allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Pilot API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "runtime-specific Pilot skill location"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    ...
    Decide which Pilot adapter type matches your runtime.

    Use adapterType only when there is a matching Pilot adapter. Put runtime-specific settings in agentDefaultsPayload.
    ...
    OpenClaw Gateway payload example:
    {
      ...
      "agentDefaultsPayload": {
        ...
        "pilotApiUrl": "https://pilot-hostname-your-agent-can-reach:3100",
    ...
    - Set agentDefaultsPayload.apiBaseUrl to the Hermes gateway URL Pilot can reach.
    - Set agentDefaultsPayload.apiKey to the exact same value as API_SERVER_KEY. This is the Hermes gateway key, not the Pilot key.
    ...
    - Watch out: /chat and the dashboard root are browser UI routes. Pilot tests /api/health and starts runs with /api/v1/runs after mapping them to the API base.
    - Set agentDefaultsPayload.pilotApiUrl to the Pilot base URL Hermes can reach.
    - Use hermes_local when Pilot should start Hermes on the Pilot host.
    - Use hermes_gateway when Pilot should call an already-running Hermes API server.
    - After board approval, claim the Pilot API key once with the claim endpoint below and save it as PILOT_API_KEY. Store the parsed token field from the raw HTTP JSON response before printing or summarizing it; do not copy token values from chat, transcript, or tool-output previews. A token value containing literal ... or [redacted] is a masked display preview, not a valid key. Do not rotate or invent a Pilot key manually.
    - Hermes-originated Pilot API usage means Hermes calls Pilot with PILOT_API_URL and PILOT_API_KEY after approval/key claim. Do not confuse that with agentDefaultsPayload.apiBaseUrl, which points Pilot to Hermes.
    ...
    Hermes Gateway payload example:
    {
      ...
      "agentDefaultsPayload": {
        ...
        "pilotApiUrl": "http://localhost:3100"
      }
    }

    Hermes Gateway network examples:
    - Local loopback API server: agentDefaultsPayload.apiBaseUrl = "http://127.0.0.1:8642" and agentDefaultsPayload.pilotApiUrl = "http://127.0.0.1:3100".
    - Local loopback dashboard root or chat URL: agentDefaultsPayload.apiBaseUrl = "http://127.0.0.1:9119" or "http://127.0.0.1:9119/chat"; Pilot maps either one to "http://127.0.0.1:9119/api".
    - LAN/private network: use reachable private addresses, for example agentDefaultsPayload.apiBaseUrl = "http://192.168.1.25:8642" and agentDefaultsPayload.pilotApiUrl = "http://192.168.1.10:3100".
    - Private overlay: use overlay DNS names, for example agentDefaultsPayload.apiBaseUrl = "http://hermes-host.tailnet-name.ts.net:8642" and agentDefaultsPayload.pilotApiUrl = "http://pilot-host.tailnet-name.ts.net:3100".
    - Docker: if Pilot runs in Docker and Hermes runs on the host, use agentDefaultsPayload.apiBaseUrl = "http://host.docker.internal:8642"; if both run in Compose, use the Hermes service name.
    - Reverse proxy/TLS: use HTTPS origins, for example agentDefaultsPayload.apiBaseUrl = "https://hermes-gateway.example" and agentDefaultsPayload.pilotApiUrl = "https://pilot.example".
    ...
    The board approves the join request in Pilot before key claim is allowed.
    ...
    On successful claim, save the full JSON response somewhere private for your runtime and set PILOT_API_KEY and PILOT_API_URL for future Pilot API calls. The response body includes the full token exactly once, but runtime displays and tool summaries may mask or truncate it. Write the raw response token directly to private storage before logging anything, then verify it with an authenticated Pilot API call. Do not persist displayed previews containing literal ... or [redacted].
    ...
    ${
      ...
      "Ensure Pilot is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Pilot base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.pilotApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - npx pilotai allowed-hostname <host>
      - then restart Pilot and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@pilot.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (req, res) => {
    ...
    res.json({
      skills: [
        { name: "pilot", path: "/api/skills/pilot" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "pilot-create-agent",
          path: "/api/skills/pilot-create-agent"
        },
        {
          name: "pilot-converting-plans-to-tasks",
          path: "/api/skills/pilot-converting-plans-to-tasks"
        }
      ]
    });
  });
  ...
  router.get("/invites/:token/skills/index", async (req, res) => {
    ...
    res.json({
      skills: [
        {
          name: "pilot",
          path: `/api/invites/${token}/skills/pilot`,
        },
      ],
    });
  });
  ...
  router.get("/invites/:token/skills/:skillName", async (req, res) => {
    ...
    const skillName = (req.params.skillName as string).trim().toLowerCase();
    if (skillName !== "pilot") throw notFound("Skill not found");
    const markdown = readSkillMarkdown(skillName);
    if (!markdown) throw notFound("Skill not found");
    res.type("text/markdown").send(markdown);
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              pilotApiUrl: req.body.pilotApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.pilotApiUrl &&
          !persistedDefaults.pilotApiUrl
        ) {
          missingPersistedFields.push("pilotApiUrl");
        }
