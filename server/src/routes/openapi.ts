...
function applyDocumentFixups(document: any): any {
  ...
  document.components.securitySchemes = {
    [BOARD_SESSION_AUTH_SCHEME]: {
      ...
      name: "pilot_session",
      description:
        "Board session cookie in authenticated mode. Pilot uses Better Auth; cookie transport may vary by deployment.",
    },
    ...
    [AGENT_BEARER_AUTH_SCHEME]: {
      ...
      description:
        "Agent API key or Pilot-issued local agent JWT presented in the Authorization bearer header.",
    },
  };
  ...
  for (const [path, pathItem] of Object.entries(document.paths ?? {})) {
    for (const [method, operation] of Object.entries(pathItem as Record<string, any>)) {
      ...
      operation["x-pilot-authorization"] =
  ...
  responses: {
    200: r.ok(z.object({
      ...
      cloud: z.object({
        ...
        managedBy: z.literal("pilot-cloud"),
...
registry.registerPath({
  ...
  tags: ["companies"],
  summary: "Apply the onboarding seed Pilot Cloud collected at signup",
  request: { params: z.object({ companyId: z.string() }) },
...
registry.registerPath({
  ...
  description:
    ...
    "`x-pilot-cloud-async-import: 1` header (browsers cannot — the Cloud harness proxy " +
    "strips inbound `x-pilot-cloud-*` headers), while board sessions use the proxy-safe " +
...
export function buildOpenApiDocument(): any {
  return applyDocumentFixups({
    ...
    info: {
      title: "Pilot API",
      version: "1.0.0",
      description: "REST API for the Pilot AI agent management platform",
    },
    servers: [{ url: "/" }],
