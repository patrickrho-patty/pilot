...
import { agents as agentsTable, companies, heartbeatRuns, issues as issuesTable, projects as projectsTable } from "@pilotai/db";
...
import { trackAgentCreated } from "@pilotai/shared/telemetry";
...
import { runClaudeLogin } from "@pilotai/adapter-claude-local/server";
...
import { SETUP_TOKEN_TRANSPORT_ADVISORY_CODE } from "@pilotai/shared";
import { DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX } from "@pilotai/adapter-codex-local";
import {
...
import type { AdapterAuthSessionOwnerResponse } from "@pilotai/shared";
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@pilotai/adapter-cursor-local";
import { DEFAULT_GEMINI_LOCAL_MODEL } from "@pilotai/adapter-gemini-local";
import { DEFAULT_KIMI_LOCAL_MODEL } from "@pilotai/adapter-kimi-local";
import { DEFAULT_OPENCODE_LOCAL_MODEL } from "@pilotai/adapter-opencode-local";
...
function readRunIssueId(context: Record<string, unknown> | null) {
  ...
  const pilotIssue = readObject(context?.pilotIssue);
...
export function agentRoutes(
   ...
   * declarative registry (PILOT_ADAPTERS) curated it out, which
