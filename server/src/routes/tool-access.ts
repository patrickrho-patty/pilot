...
import { agents, companies } from "@pilotai/db";
 ...
 * per-app Activity tab can humanize them (PIL-11284). A single update may
...
export function toolAccessRoutes(
  ...
  function oauthRedirectUri() {
    ...
    if (!configured) {
      throw unprocessable("OAuth connections require PILOT_PUBLIC_URL or an auth public base URL");
    }
  ...
  router.post("/agents/me/connections/:connectionId/token", validate(connectionTokenRequestSchema), async (req, res) => {
    ...
    const headerRunId = req.get("x-pilot-Run-Id")?.trim();
  ...
  router.patch("/tool-connections/:connectionId", validate(updateToolConnectionSchema), async (req, res) => {
    ...
    if (lifecycleChanges.length === 0) {
      ...
      // humanized row each (PIL-11284), e.g. a combined pause + allowlist edit.
