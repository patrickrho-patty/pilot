...
import { trackAgentTaskCompleted } from "@pilotai/shared/telemetry";
...
function requiresPilotAttachmentMetadata(input: {
  ...
  return type === "artifact" && provider === "pilot";
}
  ...
  if (existing) {
    ...
    if (
      ...
      const event: IssueListStormEvent = {
        ...
        visibilityHint: input.req.header("x-pilot-tab-visible") ?? null,
      };
...
export function issueRoutes(
  ...
  async function resolveRunIssueWorkspaceInheritanceSource(
    ...
    const pilotIssue = context.pilotIssue && typeof context.pilotIssue === "object"
      ? context.pilotIssue as Record<string, unknown>
      : null;
  ...
  async function deriveRecoveryCommentPresentation(
    ...
    const pilotWake = readObject(context.pilotWake);
  ...
  // Resolve issue identifiers (e.g. "PIL-39") to UUIDs for all /issues/:id routes
  ...
  // Resolve issue identifiers (e.g. "PIL-39") to UUIDs for company-scoped attachment routes.
  ...
  router.get("/companies/:companyId/issues", async (req, res) => {
    ...
    res.setHeader("X-Pilot-Request-Cache", coordinated.cacheStatus);
  ...
  router.get("/issues/:id", async (req, res) => {
    ...
    res.setHeader("Server-Timing", `pilot_issue;dur=${(performance.now() - requestStartedAt).toFixed(1)}`);
    res.json({
      ...issue,
      ...inboxArchiveFields,
      goalId: goal?.id ?? issue.goalId,
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      //
      // Reviewed and accepted as best-effort rather than transactional (PIL-16101):
