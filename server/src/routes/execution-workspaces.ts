...
import { issues, projects, projectWorkspaces } from "@pilotai/db";
import {
...
import type { WorkspaceRuntimeDesiredState, WorkspaceRuntimeServiceStateMap } from "@pilotai/shared";
import {
...
export function executionWorkspaceRoutes(db: Db, opts: { pluginWorkerManager?: PluginWorkerManager } = {}) {
  ...
  async function handleExecutionWorkspaceRuntimeCommand(req: Request, res: Response) {
    ...
    if (!workspaceCwd) {
      res.status(422).json({ error: "Execution workspace needs a local path before Pilot can run workspace commands" });
      return;
    }
    ...
    if (action === "repair") {
      const manifestPath = path.join(workspaceCwd, ".pilot", "seed-manifest.json");
      let manifest: {
        attemptId?: unknown;
      ...
      try {
        ...
        repairSeedSource = resolveCanonicalWorktreeSeedSource({
          ...
          targetConfigPath: path.join(workspaceCwd, ".pilot", "config.json"),
        ...
        repairCliArgs = isReadableFile(cliRunner) && isReadableFile(cliEntry)
          ...
          : isReadableFile(cliDist)
            ...
            : null;
        if (!repairCliArgs) {
          throw new Error("Workspace repair cannot find a runnable Pilot CLI in the base workspace.");
        }
      } catch (error) {
        throw unprocessable(
      ...
      run: async (reportProgress) => {
        ...
        if (action === "run") {
          ...
          if (!availableWorkspace) {
            throw new Error("Execution workspace needs a local path before Pilot can run workspace commands");
          }
        ...
        if (action === "repair") {
          ...
          try {
            ...
            if (repairRestartsRuntimeServices) {
              const availableWorkspace = await ensureWorkspaceAvailable();
              if (!availableWorkspace) {
                throw new Error("Execution workspace needs a local path before Pilot can restart it.");
              }
        ...
        if (action === "start" || action === "restart") {
          const availableWorkspace = await ensureWorkspaceAvailable();
          if (!availableWorkspace) {
            throw new Error("Execution workspace needs a local path before Pilot can manage local runtime services");
          }
          let startedServices;
          try {
            startedServices = await startRuntimeServicesForWorkspaceControl({
