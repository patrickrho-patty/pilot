...
export function isPlatformProvisionedEnvironment(environment: {
  ...
  return (
    environment.metadata?.managedByPilot === true ||
 ...
 * (`PILOT_EXECUTION_MODE=kubernetes`). Both adopt and refresh the
...
export function environmentRoutes(
  ...
  function environmentDeleteBlockMessage(impact: EnvironmentDeleteBlastRadius): string | null {
    ...
    if (impact.reusableSandboxLeaseCount > 0) {
      return "Cannot delete this environment while it has a reusable sandbox lease. Remove the associated execution workspace or issue so Pilot can destroy the sandbox, then retry.";
    }
    return null;
  }
