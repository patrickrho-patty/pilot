...
name: design-guide
description: >
  Pilot UI design system guide for building consistent, reusable frontend
...
Pilot's UI is a professional-grade control plane — dense, keyboard-driven, dark-themed by default. Every pixel earns its place.
...
| Mono identifier | `text-xs font-mono text-muted-foreground` | Issue keys (PIL-001), CSS vars |
...
2. **Custom composites** (`ui/src/components/`) — StatusBadge, EntityRow, MetricCard, etc. These capture Pilot-specific design language.
