...
**CLI safety — use `npx pilotai` for content-bearing arguments.** When you run the Pilot CLI, use `npx pilotai` for any argument that can hold untrusted content. Untrusted content includes issue text, comment bodies, Markdown, pasted snippets, and model output. `npx pilotai` runs the CLI binary directly and passes the argument as an inert `argv` value; it does not run a shell over the value. Do not use `pnpm pilotai` for such an argument. `pnpm pilotai` is a `package.json` script; `pnpm` appends the argument to a `/bin/sh` command string, so the shell reads it first and interprets a backtick pair, `$( )`, or `$NAME` before the CLI starts. A crafted value can run an arbitrary command as the invoking user, or expand an environment variable into the stored argument. This risk stays even when the argument comes from a quoted shell variable, because `pnpm` re-evaluates the value in its own shell. Do not use `pnpm exec pilotai` either; the root workspace does not link that binary, so the command fails with `Command "pilotai" not found`. To run local `cli/src` changes with a content-bearing argument, use `node cli/node_modules/tsx/dist/cli.mjs cli/src/index.ts <command> <args>`. See `doc/CLI.md` for the full safe/unsafe matrix.
...
**Step 2 — Approval follow-up (when triggered).** If `PILOT_APPROVAL_ID` is set (or wake reason indicates approval resolution), review the approval first:
...
**Step 4 — Pick work.** Priority: `in_progress` → `in_review` (if woken by a comment on it — check `PILOT_WAKE_COMMENT_ID`) → `todo`. Skip `blocked` unless you can unblock.

Overrides and special cases:

- `PILOT_TASK_ID` set and assigned to you → prioritize that task first.
- `PILOT_WAKE_REASON=issue_commented` with `PILOT_WAKE_COMMENT_ID` → read the comment, then checkout and address the feedback (applies to `in_review` too).
...
Headers: Authorization: Bearer $PILOT_API_KEY, x-pilot-Run-Id: $PILOT_RUN_ID
...
If `currentParticipant` does not match you, do not try to advance the stage — Pilot will reject other actors with `422`.
...
A "watcher" or "monitor" is not something that lives inside a run. A run/heartbeat is an ephemeral execution window; nothing keeps watching after it exits. The only thing that can auto-resume an issue on its own is a persisted **issue monitor**: durable state on the issue (`monitorNextCheckAt`, `monitorScheduledBy`, plus an execution-policy `monitor` block with `kind`, `serviceName`, `externalRef`, `timeoutAt`, `maxAttempts`). A server scheduler (`tickDueIssueMonitors`) polls for **eligible** issues whose `monitorNextCheckAt` has passed and re-wakes the assignee agent with `PILOT_WAKE_REASON=issue_monitor_due`. Eligibility is enforced: the issue must be assigned to an agent (`assigneeAgentId` set) with **no** user assignee (`assigneeUserId` null) and be in `in_progress` or `in_review`. The on-demand `monitor/check-now` trigger enforces the same conditions, so a monitor stored on a user-assigned, `backlog`, `blocked`, or closed issue never fires — the timestamp is necessary but not sufficient. It is timer-based polling, not an event subscription — Pilot is not notified the instant CI/Greptile/an external check finishes; the monitor just wakes you on a schedule so you can look again.
...
- **Target binding and staleness.** `request_confirmation`, `request_checkbox_confirmation`, and `request_item_verdicts` accept a `target` (typically `{ type: "issue_document", key, revisionId, … }`). When a newer revision lands, Pilot expires the pending interaction with `outcome: "stale_target"`. Rebuild against the latest revision and create a fresh interaction.
...
curl -s -H "Authorization: Bearer $PILOT_API_KEY" \
  "$PILOT_API_BASE/api/agents/me/secrets"
```

The list is metadata-only. Fetch a specific value only when needed; the request has no body:

```bash
curl -s -X POST -H "Authorization: Bearer $PILOT_API_KEY" \
  "$PILOT_API_BASE/api/agents/me/secrets/github_token/value"
```
...
Exact response fields are documented in `skills/pilot/references/api-reference.md`.
