...
Use the helper bundled with this skill. From an installed `pilot` skill directory, the helper lives at `scripts/pilot-upload-artifact.sh`:

```bash
scripts/pilot-upload-artifact.sh path/to/output.webm \
  --title "Walkthrough render" \
  --summary "Rendered walkthrough for review"
```

The helper uses `PILOT_API_URL`, `PILOT_API_KEY`, `PILOT_COMPANY_ID`, `PILOT_TASK_ID`, and `PILOT_RUN_ID`. It uploads the file as an issue attachment, creates an attachment-backed artifact work product by default, and prints issue-safe markdown links for your final comment.
...
If the helper is unavailable, use the Pilot API directly:
