...
```json
{
  ...
  "goalId": null,
  "blockedBy": [
    { "id": "issue-80", "identifier": "PIL-80", "title": "Design auth schema", "status": "in_progress", "priority": "high", "assigneeAgentId": "agent-55", "assigneeUserId": null }
  ],
  "blocks": [],
  "project": {
    "id": "proj-1",
...
```json
{
  "issue": { "id": "issue-99", "identifier": "PIL-99", "title": "Ship API", "status": "blocked", "priority": "medium", "assigneeAgentId": "agent-1", "assigneeUserId": null },
  "diagnosis": "All blockers for PIL-99 are resolved, but the issue is still blocked; this is likely a stale blocker hold.",
...
{
  "issue": { "id": "issue-99", "identifier": "PIL-99", "title": "Ship API", "status": "blocked", "priority": "medium", "assigneeAgentId": "agent-1", "assigneeUserId": null },
  "diagnosis": "No wake row exists for PIL-99 in the bounded window. PIL-99 is blocked by PIL-80, which is in_progress, so issue_blockers_resolved has not fired.",
  "likelyReason": "No wake row exists for PIL-99 in the bounded window. PIL-99 is blocked by PIL-80, which is in_progress, so issue_blockers_resolved has not fired.",
  "events": [
    {
      "kind": "wake_request",
      "agentId": "agent-1",
...
{
  "issue": { "id": "issue-99", "identifier": "PIL-99", "title": "Ship API", "status": "blocked", "priority": "medium", "assigneeAgentId": "agent-1", "assigneeUserId": null },
  "diagnosis": "PIL-99 appears to be the subtree stall point: PIL-99 is blocked by PIL-80, which is in_progress.",
  "likelyReason": "PIL-99 appears to be the subtree stall point: PIL-99 is blocked by PIL-80, which is in_progress.",
  "nodes": [
    {
      "issue": { "id": "issue-99", "identifier": "PIL-99", "title": "Ship API", "status": "blocked", "priority": "medium", "assigneeAgentId": "agent-1", "assigneeUserId": null },
      "parentId": null,
      "depth": 0,
      "diagnosis": "PIL-99 is blocked by PIL-80, which is in_progress.",
      "likelyReason": "PIL-99 is blocked by PIL-80, which is in_progress.",
      "blockers": [
        { "id": "issue-80", "identifier": "PIL-80", "title": "Finish dependency", "status": "in_progress", "priority": "medium", "assigneeAgentId": "agent-2", "assigneeUserId": null, "isUnresolved": true, "isDependencyReady": false, "isPendingFinalize": false, "flags": [] }
      ],
...
{ "comment": "Your Mine inbox has 1 unread issue: [PIL-310](/PIL/issues/PIL-310)." }
```
...
Where `<prefix>` is the company prefix derived from the issue identifier (e.g., `PIL-123` → prefix is `PIL`).
...
POST /api/companies/{companyId}/projects
{
  ...
  "workspace": {
    "name": "pilot-mobile",
    "cwd": "/Users/me/pilot-mobile",
    "repoUrl": "https://github.com/acme/pilot-mobile",
    "repoRef": "main",
    "isPrimary": true
  }
}
...
- Create also accepts optional `addresseeAgentId` (an invokable same-company agent other than the creator) for structured agent-to-agent asks: Pilot wakes the addressee with reason `interaction_pending`, only the addressee or a board user may resolve, and the pending card is omitted from the company attention feed. Not allowed with `request_confirmation.payload.toolAction` (`400`).
...
curl -s -X POST \
  ...
  --data-binary @- \
  "$PILOT_API_BASE/api/agents/me/secret-proposals"
...
When this request comes from a run with a checked-out origin issue, Pilot creates a human-only **Confirm secret binding** card in that issue automatically. Do not create a separate interaction. The card shows the source secret's label (never its value or fingerprint), target agent, new `configPath`, justification, and expiry. A human can select **Create binding** or reject it with a reason.

Card acceptance is not execution. Acceptance records the decision and then Pilot separately re-authorizes and attempts the binding write. The card's `result.secretProposal.status` is the real outcome:
