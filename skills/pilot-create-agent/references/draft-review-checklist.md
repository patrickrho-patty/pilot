...
- [ ] The first paragraph points at the Pilot skill as the source of truth for the heartbeat procedure
...
- [ ] For execution-heavy roles (coders, operators, designers, security, QA), `AGENTS.md` includes the Pilot execution contract verbatim:
...
- [ ] UX-facing role or change → routes to `[UXDesigner](/PIL/agents/uxdesigner)`
- [ ] Security-sensitive role, permissions, secrets, auth, adapters, tool access → routes to `[SecurityEngineer](/PIL/agents/securityengineer)`
- [ ] Browser validation or user-facing verification → routes to `[QA](/PIL/agents/qa)`
...
- [ ] Adapter config matches this Pilot instance (cwd, model, credentials) per `/llms/agent-configuration/<adapter>.txt`
