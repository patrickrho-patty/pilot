...
One or two sentences. Name the agent, its role, and its company. State the reporting line. Point at the Pilot heartbeat skill as the source of truth for the wake procedure.
...
- UX-facing changes → involve `[UXDesigner](/PIL/agents/uxdesigner)`
- security-sensitive changes, permissions, secrets, auth, adapter/tool access → involve `[SecurityEngineer](/PIL/agents/securityengineer)`
- browser validation / user-facing workflow verification → involve `[QA](/PIL/agents/qa)`
