...
description: >
  Create new agents in Pilot with governance-aware hiring. Use when you need
  to inspect adapter configuration options, compare existing agent configs,
  draft a new agent prompt/config, and submit a hire request.
---

# Pilot Create Agent Skill
...
```sh
curl -sS "$PILOT_API_URL/api/agents/me" \
  -H "Authorization: Bearer $PILOT_API_KEY"
```

### 2. Discover adapter configuration for this Pilot instance
...
`skills/pilot-create-agent/references/agent-instruction-templates.md`

Generic fallback for no-template hires:
`skills/pilot-create-agent/references/baseline-role-guide.md`
...
- for coding or execution agents, include the Pilot execution contract: start actionable work in the same heartbeat; do not stop at a plan unless planning was requested; leave durable progress with a clear next action; use child issues for long or parallel delegated work instead of polling; mark blocked work with owner/action; respect budget, pause/cancel, approval gates, and company boundaries
...
`skills/pilot-create-agent/references/draft-review-checklist.md`
...
- when the board approves, you will be woken with `PILOT_APPROVAL_ID`; read linked issues and close/comment follow-up

```sh
curl -sS "$PILOT_API_URL/api/approvals/<approval-id>" \
  -H "Authorization: Bearer $PILOT_API_KEY"

curl -sS -X POST "$PILOT_API_URL/api/approvals/<approval-id>/comments" \
  -H "Authorization: Bearer $PILOT_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS -X POST "$PILOT_API_URL/api/issues/<issue-id>/approvals" \
  -H "Authorization: Bearer $PILOT_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS "$PILOT_API_URL/api/approvals/$PILOT_APPROVAL_ID" \
  -H "Authorization: Bearer $PILOT_API_KEY"

curl -sS "$PILOT_API_URL/api/approvals/$PILOT_APPROVAL_ID/issues" \
  -H "Authorization: Bearer $PILOT_API_KEY"
```
...
- Template index and how to apply a template: `skills/pilot-create-agent/references/agent-instruction-templates.md`
- Individual role templates: `skills/pilot-create-agent/references/agents/`
- Generic baseline role guide (no-template fallback): `skills/pilot-create-agent/references/baseline-role-guide.md`
- Pre-submit draft-review checklist: `skills/pilot-create-agent/references/draft-review-checklist.md`
- Endpoint payload shapes and full examples: `skills/pilot-create-agent/references/api-reference.md`
