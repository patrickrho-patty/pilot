...
RUN usermod -u $USER_UID --non-unique node \
  ...
  && usermod -g $USER_GID -d /pilot node
...
COPY . .
RUN pnpm --filter @pilotai/ui build
RUN pnpm --filter @pilotai/plugin-sdk build
...
ARG PILOT_BUILD_COMMIT=""
RUN pnpm --filter @pilotai/server build
...
ARG PILOT_BUILD_VERSION=""
# The exact commit this image was built from, for the same reason: server-info
# falls back to PILOT_BUILD_COMMIT when git is unavailable, which feeds the
...
RUN echo "cli-tools-epoch: ${CLI_TOOLS_CACHE_EPOCH}" \
  && npm install --global --omit=dev @anthropic-ai/claude-code@latest @openai/codex@latest opencode-ai @google/gemini-cli@latest @moonshot-ai/kimi-code@latest @earendil-works/pi-coding-agent@0.74.0 \
  && apt-get update \
...
ENV NODE_ENV=production \
  ...
  USER_UID=${USER_UID} \
  USER_GID=${USER_GID} \
  PILOT_CONFIG=/pilot/instances/default/config.json \
...
# `plugins.autoInstall` key list through PILOT_MANAGED_CONFIG and
