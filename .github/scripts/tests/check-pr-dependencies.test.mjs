...
test('resolveBaseRef: returns the explicit base ref without making API calls', async () => {
  ...
  }, 'token', 'pilotai/pilot', 6469, 'release/1.2');
...
test('resolveBaseRef: falls back to the PR base ref', async () => {
  ...
  assert.deepEqual(seenPaths, ['/repos/pilotai/pilot/pulls/6469']);
});

test('checkDependencies: compares package files against the resolved base ref instead of main', async () => {
  const seenPaths = [];
  ...
  assert.deepEqual(seenPaths, [
    '/repos/pilotai/pilot/contents/packages/foo/package.json?ref=release%2F1.2',
    '/repos/pilotai/pilot/contents/packages/foo/package.json?ref=refs%2Fpull%2F6469%2Fhead',
  ]);
});
