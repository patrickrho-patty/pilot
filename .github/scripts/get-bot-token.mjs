...
/**
 * get-bot-token.mjs
 * Generates a short-lived GitHub installation token for the commitpilot app.
 * Reads COMMITPILOT_KEY env var (PEM content of private key).
...
export async function resolveInstallationId(fetchInstallation, token, repo, owner) {
  ...
  if (!installations.length) {
    throw new Error(
      'ERROR: No installations found for commitpilot. Install URL: https://github.com/apps/commitpilot/installations/new'
    );
  }
  ...
  throw new Error(
    'ERROR: Multiple commitpilot installations found. Set GH_REPO or GITHUB_REPOSITORY so the correct installation can be selected.'
  );
}

async function main() {
  const privateKey = process.env.COMMITPILOT_KEY;
  if (!privateKey) {
    console.error('ERROR: COMMITPILOT_KEY env var not set.');
    console.error('Add to ~/.bash_profile: export COMMITPILOT_KEY="$(cat ~/.config/commitpilot/private-key.pem)"');
    process.exit(1);
  }
