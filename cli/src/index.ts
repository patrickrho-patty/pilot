...
// Brand-rename alias window: map legacy PILOT_* env onto unset PILOT_*
...
const DATA_DIR_OPTION_HELP =
  "Pilot data directory root (isolates state from ~/.pilot)";

program.enablePositionalOptions();

program
  .name("pilotai")
  .description("Pilot CLI — setup, diagnose, and configure your instance")
  .version(cliVersion);

program
  .command("install")
  .description("Install Pilot into a managed per-user CLI store")
...
program
  .command("update")
  .alias("upgrade")
  .description("Check, update, or roll back the Pilot CLI")
...
program
  ...
  .option("--run", "Start Pilot immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your Pilot setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "pilot")
...
const run = program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run Pilot")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the Pilot server API")
