...
export async function databaseCheck(config: PilotConfig, configPath?: string): Promise<CheckResult> {
  if (config.database.mode === "postgres") {
    if (!config.database.connectionString) {
      return {
        ...
        canRepair: false,
        repairHint: "Run `pilotai configure --section database`",
      };
    }
  ...
  if (config.database.mode === "embedded-postgres") {
    ...
    // flag: this is what happens when PILOT_HOME / PILOT_IN_WORKTREE leak
    ...
    // ephemeral/CI instances that don't set PILOT_IN_WORKTREE are not flagged.)
    ...
    if (isInsideOsTmpDir(dataDir) && process.env.PILOT_IN_WORKTREE === "true") {
      return {
        ...
        message:
          ...
          "while running in worktree mode (PILOT_IN_WORKTREE=true). Data stored here is " +
          "ephemeral and will be lost on reboot or a temp cleanup. If this is your primary " +
          "instance, PILOT_HOME / PILOT_IN_WORKTREE likely leaked into its environment, " +
          "pointing it at a throwaway worktree home instead of your real data.",
        canRepair: false,
        repairHint:
          "If this is the primary instance, unset PILOT_HOME and PILOT_IN_WORKTREE " +
