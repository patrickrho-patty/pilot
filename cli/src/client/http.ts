...
export class PilotApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-pilot-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  lines.push(
    "",
    "This usually means the Pilot server is not running, the configured URL is wrong, or the request is being blocked before it reaches Pilot.",
    "",
    "Try:",
    "- Start Pilot with `pnpm dev` (from a source checkout) or `npx pilotai run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Pilot is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`PILOT_API_URL\`.`,
  );
  return lines.join("\n");
}
