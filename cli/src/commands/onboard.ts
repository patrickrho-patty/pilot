...
type OnboardDefaults = Pick<PilotConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const TAILNET_BIND_WARNING =
  "No Tailscale address was detected during setup. The saved config will stay on loopback until Tailscale is available or PILOT_TAILNET_BIND_HOST is set.";
...
function describeServerBinding(server: Pick<PilotConfig["server"], "bind" | "customBindHost" | "host" | "port">): string {
...
function canCreateBootstrapInviteImmediately(config: Pick<PilotConfig, "database" | "server">): boolean {
...
function printManagedInstallHint(): void {
  ...
  p.log.info(
    `This npx run is temporary. Use ${pc.cyan("pilotai install")} for atomic updates, rollback, and service support.`,
  );
}
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  printPilotCliBanner();
  p.intro(pc.bgCyan(pc.black(" pilotai onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolvePilotInstanceId());
  p.log.message(
    pc.dim(
  ...
  let existingConfig: PilotConfig | null = null;
  ...
  if (configExists(opts.config)) {
    ...
    try {
      ...
      for (const warning of findPilotConfigKeyWarnings(existingConfig)) {
  ...
  if (existingConfig) {
    p.log.message(
      pc.dim("Existing Pilot install detected; keeping the current configuration unchanged."),
    );
    p.log.message(pc.dim(`Use ${pc.cyan("pilotai configure")} if you want to change settings.`));
    ...
    p.note(
      [
        ...
        "Agent auth: PILOT_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    p.note(
      [
        `Run: ${pc.cyan("pilotai run")}`,
        `Reconfigure later: ${pc.cyan("pilotai configure")}`,
        `Diagnose setup: ${pc.cyan("pilotai doctor")}`,
      ].join("\n"),
      "Next commands",
    );
    ...
    p.outro("Existing Pilot setup is ready.");
    return;
  }
  ...
  let llm: PilotConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@pilotai/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `pilotai doctor`"));
      }
    }
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("pilotai run")}`,
        `Then: ${pc.cyan("pilotai auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
