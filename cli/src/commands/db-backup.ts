...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@pilotai/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://pilot:pilot@127.0.0.1:${port}/pilot`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  printPilotCliBanner();
  p.intro(pc.bgCyan(pc.black(" pilot db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolvePilotInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "pilot";
