...
} from "@pilotai/shared/worktree-seed-source";
import {
  readWorktreePortRegistry,
  withWorktreePortRegistryLock,
  writeWorktreePortRegistry,
} from "@pilotai/shared/worktree-port-registry";
import {
...
const WORKTREE_NAME_PREFIX = "pilot-";
...
function resolveRepoManagedWorktreesRoot(cwd: string): string | null {
  ...
  const marker = `${path.sep}.pilot${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".pilot", "worktrees");
}
...
function collectClaimedWorktreePorts(
  ...
  for (const configPath of registeredConfigPaths) {
    ...
    if (resolvedConfigPath !== path.resolve(cwd, ".pilot", "config.json") && existsSync(resolvedConfigPath)) {
  ...
  if (repoManagedWorktreesRoot && existsSync(repoManagedWorktreesRoot)) {
    for (const entry of readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const configPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".pilot", "config.json");
      if (existsSync(configPath)) {
        configPaths.add(configPath);
      }
    }
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.pilot"));
...
export function resolveWorktreeReseedTargetPaths(input: {
  ...
  if (!homeDir || !instanceId) {
    throw new Error(
      `Target config ${input.configPath} does not look like a worktree-local Pilot instance. Expected PILOT_HOME and PILOT_INSTANCE_ID in the adjacent .env.`,
    );
  }
...
function resolveExistingGitWorktree(selector: string, cwd: string): MergeSourceChoice | null {
  ...
  if (existsSync(directPath)) {
    return {
      ...
      hasPilotConfig: existsSync(path.resolve(directPath, ".pilot", "config.json")),
...
async function ensureRepairTargetWorktree(input: {
  ...
  const currentConfigPath = path.resolve(currentRoot, ".pilot", "config.json");
  ...
  if (existing) {
    return {
      ...
      configPath: path.resolve(existing.worktree, ".pilot", "config.json"),
  ...
  return {
    rootPath: targetPath,
    configPath: path.resolve(targetPath, ".pilot", "config.json"),
    label: branchName,
    branchName,
    created: true,
  };
...
function resolveSourceConnectionString(config: PilotConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://pilot:pilot@127.0.0.1:${port}/pilot`;
}
...
export function resolveWorktreeSeedMigrationRevision(
  ...
  if (
    ...
    throw new Error("Migration journal is not a prefix of this Pilot checkout's migration journal.");
  }
...
async function seedWorktreeDatabase(input: {
  ...
  try {
    if (input.sourceConfig.database.mode === "embedded-postgres") {
      ...
      const sourceAdminConnectionString = `postgres://pilot:pilot@127.0.0.1:${sourceHandle.port}/postgres`;
      await ensurePostgresDatabase(sourceAdminConnectionString, "pilot");
    }
    ...
    const adminConnectionString = `postgres://pilot:pilot@127.0.0.1:${targetHandle.port}/postgres`;
    await resetPostgresDatabase(adminConnectionString, "pilot");
    const targetConnectionString = `postgres://pilot:pilot@127.0.0.1:${targetHandle.port}/pilot`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  if (opts.force) {
    ...
    // <repo>/.pilot/worktrees/ holding every repo-managed worktree
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run Pilot inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}
...
export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  printPilotCliBanner();
  p.intro(pc.bgCyan(pc.black(" pilotai worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeEnsureSeededCommand(opts: WorktreeEnsureSeededOptions): Promise<void> {
  printPilotCliBanner();
  p.intro(pc.bgCyan(pc.black(" pilotai worktree ensure-seeded ")));
...
export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printPilotCliBanner();
  p.intro(pc.bgCyan(pc.black(" pilotai worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasPilotConfig: existsSync(path.resolve(worktreePath, ".pilot", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printPilotCliBanner();
  p.intro(pc.bgCyan(pc.black(" pilotai worktree:cleanup ")));
...
export function resolveCurrentWorktreeEndpoint(): ResolvedWorktreeEndpoint {
  ...
  const localConfigPath = path.join(rootPath, ".pilot", "config.json");
  return {
    rootPath,
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasPilotConfig)
      .map((choice) => path.resolve(choice.worktree, ".pilot", "config.json")),
  ];
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start Pilot once) before using worktree merge history.`,
      );
    }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasPilotConfig ? "pilot" : "no-pilot-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".pilot", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".pilot", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .pilot/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasPilotConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a Pilot worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentWorktreeEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasPilotConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No Pilot worktrees were found. Run `pilotai worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target Pilot configs are the same. Choose different --from/--to worktrees.");
  }
...
async function backupWorktreeReseedTarget(input: {
  ...
  try {
    const adminConnectionString = `postgres://pilot:pilot@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "pilot");
    const result = await runDatabaseBackup({
      connectionString: `postgres://pilot:pilot@127.0.0.1:${targetHandle.port}/pilot`,
...
async function runWorktreeReseed(opts: WorktreeReseedOptions): Promise<void> {
  ...
  if (path.resolve(source.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target Pilot configs are the same. Choose different --from/--to values.");
  }
  ...
  if (runningTargetPid && !opts.allowLiveTarget) {
    throw new Error(
      `Target worktree database appears to be running (pid ${runningTargetPid}). Stop Pilot in ${targetEndpoint.rootPath} before reseeding, or re-run with --allow-live-target if you want to override this guard.`,
    );
  }
  ...
  const confirmed = opts.yes
    ? true
    : await p.confirm({
      message: `Overwrite the isolated Pilot DB for ${targetEndpoint.label} from ${source.label} using ${seedMode} seed mode?`,
      initialValue: false,
    });
  if (p.isCancel(confirmed) || !confirmed) {
...
export async function worktreeRepairCommand(opts: WorktreeRepairOptions): Promise<void> {
  ...
  if (path.resolve(source.configPath) === path.resolve(target.configPath)) {
    throw new Error("Source and target Pilot configs are the same. Use --from-config/--from-instance to point repair at a different source.");
  }
  ...
  if (runningTargetPid && !opts.allowLiveTarget) {
    throw new Error(
      `Target worktree database appears to be running (pid ${runningTargetPid}). Stop Pilot in ${target.rootPath} before repairing, or re-run with --allow-live-target if you want to override this guard.`,
    );
  }
  if (runningTargetPid && opts.allowLiveTarget) {
    p.log.warning(`Proceeding even though the target embedded PostgreSQL appears to be running (pid ${runningTargetPid}).`);
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local Pilot instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated Pilot instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with pilot- if needed (created at ~/pilot-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: PILOT_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: PILOT_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source PILOT_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local Pilot instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like Pilot worktrees")
  ...
  worktree
    .command("reseed")
    .description("Re-seed an existing worktree-local instance from another Pilot instance or worktree")
    .option("--from <worktree>", "Source worktree path, directory name, branch name, or current")
    .option("--to <worktree>", "Target worktree path, directory name, branch name, or current (defaults to current)")
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source PILOT_HOME used when deriving the source config")
  ...
  worktree
    .command("repair")
    .description("Create or repair a linked worktree-local Pilot instance without touching the primary checkout")
    .option("--branch <name>", "Existing branch/worktree selector to repair, or a branch name to create under .pilot/worktrees")
    .option("--home <path>", `Home root for worktree instances (env: PILOT_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source PILOT_HOME used when deriving the source config")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with pilot- if needed")
