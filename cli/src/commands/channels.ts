...
export async function channelsCommand(
  ...
  console.log(pc.bold("Pilot release channels"));
  console.log("");
  for (const entry of state) {
    ...
    console.log(`  ${" ".repeat(8)} ${pc.dim(`npx pilotai@${entry.distTag} onboard`)}`);
    console.log("");
  }
  ...
  console.log(`Docker images use the same names: ghcr.io/patty-io/pilot:{latest,beta,nightly,canary}`);
}
