...
export async function configure(opts: {
  ...
  printPilotCliBanner();
  p.intro(pc.bgCyan(pc.black(" pilot configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `pilotai onboard` first.");
    p.outro("");
    process.exitCode = 1;
    return;
  }
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because PILOT_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
