...
async function openConfiguredDb(configPath: string): Promise<{
  ...
  try {
    if (config.database.mode === "embedded-postgres") {
      ...
      const adminConnectionString = `postgres://pilot:pilot@127.0.0.1:${embeddedHandle.port}/postgres`;
      await ensurePostgresDatabase(adminConnectionString, "pilot");
      const connectionString = `postgres://pilot:pilot@127.0.0.1:${embeddedHandle.port}/pilot`;
...
export async function disableAllRoutinesInConfig(
  ...
  const companyId =
    ...
    ?? null;
  if (!companyId) {
    throw new Error("Company ID is required. Pass --company-id or set PILOT_COMPANY_ID.");
  }
