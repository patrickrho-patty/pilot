...
function buildAgentEnvExports(input: {
  ...
  return [
    ...
    `export PILOT_COMPANY_ID='${escaped(input.companyId)}'`,
    `export PILOT_AGENT_ID='${escaped(input.agentId)}'`,
    `export PILOT_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Pilot skills for Codex/Claude, and print shell exports",
      )
      .argument("<agentRef>", "Agent ID or shortname/url-key")
      .requiredOption("-C, --company-id <id>", "Company ID")
      ...
      .option(
        "--no-install-skills",
        "Skip installing Pilot skills into ~/.codex/skills, ~/.claude/skills, and ~/.kimi-code/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            ...
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Pilot skills directory. Expected ./skills in the repo checkout.",
              );
            }
