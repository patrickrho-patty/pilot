...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Pilot config file")
    .option("-d, --data-dir <path>", "Pilot data directory root (isolates state from ~/.pilot)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Pilot API")
    .option("--api-key <token>", "Bearer token for agent-authenticated calls")
    .option("--run-id <id>", "Heartbeat run id for agent-authenticated mutations (checkout/release/interactions/in-progress update); falls back to $PILOT_RUN_ID")
...
export function resolveCommandContext(
  ...
  // PILOT_RUN_ID env the adapter/embodiment context already exports.
