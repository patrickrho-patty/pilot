...
export function registerTeamCommands(program: Command): void {
  ...
  addCommonClientOptions(
    teams
      ...
      .option("--approval-issue-id <id>", "Issue ID to link to the fallback approval request; defaults to PILOT_TASK_ID when set")
...
async function requestInstallApproval(
  ...
  const payload = {
    ...
    payload: {
      ...
      summary:
        `A Pilot CLI agent-run attempted to install catalog team "${trimmedRef}" into company "${ctx.companyId}", ` +
