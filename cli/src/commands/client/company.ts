...
} from "@pilotai/shared/company-import-transfer";
...
function shouldIncludePortableFile(filePath: string): boolean {
  ...
  const isPilotYaml = baseName === ".pilot.yaml" || baseName === ".pilot.yml";
...
function findPortableExtensionPath(files: Record<string, CompanyPortabilityFileEntry>): string | null {
  if (files[".pilot.yaml"] !== undefined) return ".pilot.yaml";
  if (files[".pilot.yml"] !== undefined) return ".pilot.yml";
...
async function promptForImportSelection(preview: CompanyPortabilityPreviewResult): Promise<string[]> {
  ...
  while (true) {
    const choice = await p.select<ImportSelectableGroup | "company" | "confirm">({
      message: "Select what Pilot should import",
      options: [
        {
          value: "company",
...
export function resolveCompanyForDeletion(
  ...
  throw new Error(
    `No company found for selector '${selector}'. Use company ID or issue prefix (for example PIL).`,
  );
}
...
export function registerCompanyCommands(program: Command): void {
          ...
          printOutput(
            {
              ...
              pilotExtensionPath: exported.pilotExtensionPath,
  ...
  addCommonClientOptions(
    company
      ...
      .option("--pilot-url <url>", "Alias for --api-base on this command")
      ...
      .action(async (fromPathOrUrl: string, opts: CompanyImportOptions) => {
        try {
          if (!opts.apiBase?.trim() && opts.pilotUrl?.trim()) {
            opts.apiBase = opts.pilotUrl.trim();
          }
          const ctx = resolveCommandContext(opts);
  ...
  addCommonClientOptions(
    company
      ...
      .argument("<selector>", "Company ID or issue prefix (for example PIL)")
      .option(
        "--by <mode>",
...
async function createCompanyForContext(ctx: {
  ...
  try {
    ...
    if (isBoardAccessRequiredError(error) || isInstanceAdminRequiredError(error)) {
      throw new Error(
        "Creating companies requires board/instance-admin authentication. Agent API keys are scoped to one company; use `pilotai company list --json` or `pilotai company current --json` to select the scoped company, or rerun create with a board token/login.",
      );
    }
    throw error;
  }
...
async function resolveCurrentCompanyId(ctx: { companyId?: string; api: { get<T>(path: string): Promise<T | null> } }): Promise<string> {
  ...
  try {
    ...
    if (error instanceof ApiRequestError && (error.status === 401 || error.status === 403)) {
      throw new Error(
        "Current company is not available. Pass --company-id, set PILOT_COMPANY_ID, set a context profile companyId, or authenticate with an agent API key.",
      );
    }
    throw error;
  }
