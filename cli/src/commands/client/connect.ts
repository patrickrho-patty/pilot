...
import type { Agent, Company } from "@pilotai/shared";
import { createAgentKeySchema, createBoardApiKeySchema } from "@pilotai/shared";
...
async function connectWizard(opts: ConnectOptions) {
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    throw new Error("`pilotai connect` is interactive. For scripts, pass --api-base/--api-key or use context set/token commands.");
  }

  p.intro(pc.bgCyan(pc.black(" pilotai connect ")));
...
function buildExports(input: {
  ...
  return [
    `export PILOT_API_URL='${escaped(input.apiBase)}'`,
    input.companyId ? `export PILOT_COMPANY_ID='${escaped(input.companyId)}'` : null,
    input.agentId ? `export PILOT_AGENT_ID='${escaped(input.agentId)}'` : null,
