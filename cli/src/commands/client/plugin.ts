...
} from "../../../../packages/plugins/create-pilot-plugin/src/index.js";
...
function requireCompanyId(ctx: { companyId?: string }): string {
  if (!ctx.companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set PILOT_COMPANY_ID, or set context profile companyId via `pilotai context set`.",
    );
  }
...
export function renderLocalPluginInstallHint(packagePath: string): string {
  return [
    ...
    pc.dim(`Keep ${pc.cyan("pnpm dev")} running in ${packagePath}; Pilot watches rebuilt dist output and reloads the plugin worker.`),
  ].join("\n");
}
...
export function formatTargetDiagnostics(diag: TargetDiagnostics): string {
  const lines = [pc.dim(`Target Pilot: ${pc.cyan(diag.apiBase)}`)];
...
export function buildPluginInitNextCommands(outputDir: string): string[] {
  ...
  return [
    ...
    "pnpm install",
    "pnpm dev",
    `pilotai plugin install ${quotedOutputDir}`,
  ];
}
...
export function registerPluginCommands(program: Command): void {
  ...
  addCommonClientOptions(
    plugin
      ...
      .description("Scaffold a local Pilot plugin project")
      ...
      .option("--sdk-path <path>", "Local @pilotai/plugin-sdk package path")
    ...
    plugin
      ...
      .description(
        "Install a plugin from a local path or npm package.\n" +
          "  Examples:\n" +
          "    pilotai plugin install ./my-plugin              # local path\n" +
          "    pilotai plugin install @acme/plugin-linear      # npm package\n" +
          "    pilotai plugin install @acme/plugin-linear@1.2  # pinned version",
      )
      ...
      .option(
        "--no-verify-target",
        "Skip the pre-install probe that reports which Pilot instance the plugin installs into",
      )
  ...
  addCommonClientOptions(
    plugin
      .command("target")
      .description(
        "Show which Pilot instance plugin commands will talk to.\n" +
          ...
          for (const ex of rows) {
            console.log(
              `${pc.bold(ex.displayName)}  ${pc.dim(ex.pluginKey)}\n` +
                `  ${ex.description}\n` +
                `  ${pc.cyan(`pilotai plugin install ${ex.localPath}`)}`,
            );
          }
        } catch (err) {
          handleCommandError(err);
