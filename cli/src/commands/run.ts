...
export async function runCommand(opts: RunOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" pilotai run ")));
  ...
  if (!configExists(configPath)) {
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
      ...
      p.log.message(`Run ${pc.cyan("pilotai onboard")} once, then retry ${pc.cyan("pilotai run")}.`);
      process.exit(1);
    }
  ...
  p.log.step("Starting Pilot server...");
...
function maybeEnableUiDevMiddleware(entrypoint: string): void {
  ...
  if (normalized.endsWith("/server/src/index.ts") || normalized.endsWith("@pilotai/server/src/index.ts")) {
...
function ensureDevWorkspaceBuildDeps(projectRoot: string): void {
  ...
  if (result.error) {
    throw new Error(
      `Failed to prepare workspace build artifacts before starting the Pilot dev server.\n${formatError(result.error)}`,
    );
  }
...
async function importServerEntry(): Promise<StartedServer> {
  ...
  // Production mode: import the published @pilotai/server package
  try {
    const mod = await import("@pilotai/server");
    return await startServerFromModule(mod, "@pilotai/server");
  } catch (err) {
    const missingSpecifier = getMissingModuleSpecifier(err);
    const missingServerEntrypoint = !missingSpecifier || missingSpecifier === "@pilotai/server";
    if (isModuleNotFoundError(err) && missingServerEntrypoint) {
      throw new Error(
        `Could not locate a Pilot server entrypoint.\n` +
          `Tried: ${devEntry}, @pilotai/server\n` +
          `${formatError(err)}`,
      );
    }
    throw new Error(
      `Pilot server failed to start.\n` +
        `${formatError(err)}`,
    );
  }
}
...
async function startServerFromModule(mod: unknown, label: string): Promise<StartedServer> {
  ...
  if (typeof startServer !== "function") {
    throw new Error(`Pilot server entrypoint did not export startServer(): ${label}`);
  }
