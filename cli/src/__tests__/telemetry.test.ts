...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.PILOT_HOME = path.join(root, "home");
    process.env.PILOT_INSTANCE_ID = "telemetry-test";
