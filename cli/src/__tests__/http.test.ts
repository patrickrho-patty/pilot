...
import { ApiConnectionError, ApiRequestError, PilotApiClient } from "../client/http.js";
...
describe("PilotApiClient", () => {
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new PilotApiClient({
    ...
    expect(headers["x-pilot-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new PilotApiClient({ apiBase: "http://localhost:3100" });
