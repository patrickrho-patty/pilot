...
beforeEach(() => {
  ...
  process.env.PILOT_HOME = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-service-restart-"));
});
...
function managerFixture(active = true) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-service-doctor-"));
  const definitionPath = path.join(root, "pilot.service");
  ...
  it("detects a foreground process on the configured port while the service is inactive", async () => {
    ...
    expect(results).toContainEqual(
      expect.objectContaining({
        ...
        status: "fail",
        message: expect.stringContaining("another Pilot process"),
      }),
    );
  });
});
