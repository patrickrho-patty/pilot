...
describe("managed install store", () => {
  ...
  beforeEach(() => {
    root = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-install-store-"));
    paths = resolveInstallStorePaths({
      homeDir: path.join(root, "home"),
      pilotHome: path.join(root, "home", ".pilot"),
    });
  });
  ...
  it("writes a stable shim with the validated runtime and custom store path", () => {
    ...
    expect(fs.readFileSync(rcPath, "utf8")).not.toContain("pilotai managed PATH");
  ...
  it("reports managed provenance only for the payload selected by current", () => {
    ...
    const executable = path.join(manifestPayload, "node_modules", "pilotai", "dist", "index.js");
