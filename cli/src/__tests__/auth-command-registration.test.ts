...
describe("client auth API commands", () => {
  beforeEach(() => {
    ...
    delete process.env.PILOT_TEST_CHALLENGE_TOKEN;
  ...
  it("wraps CLI auth challenge endpoints", async () => {
    ...
    process.env.PILOT_TEST_CHALLENGE_TOKEN = "env-secret";
    await run(["challenge", "approve", "challenge/2", "--token-env", "PILOT_TEST_CHALLENGE_TOKEN"]);
