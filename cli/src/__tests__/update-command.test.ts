...
function createPayload(payloadPath: string, version: string): string {
  const entrypoint = path.join(payloadPath, "node_modules", "pilotai", "dist", "index.js");
  fs.mkdirSync(path.dirname(entrypoint), { recursive: true });
  fs.writeFileSync(entrypoint, version);
  return entrypoint;
}
beforeEach(() => {
  root = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-update-"));
  previousHome = process.env.HOME;
  previousPilotHome = process.env.PILOT_HOME;
  process.env.HOME = path.join(root, "home");
  process.env.PILOT_HOME = path.join(root, "pilot");
});
afterEach(() => {
  vi.restoreAllMocks();
  vi.unstubAllEnvs();
...
describe("update command", () => {
  ...
  it("detects managed, global npm, npx, and source modes", () => {
    ...
    expect(detectInstallMode(path.join(root, "lib", "node_modules", "pilotai", "dist", "index.js"), paths)).toBe("global-npm");
    expect(detectInstallMode(path.join(root, ".npm", "_npx", "abc", "node_modules", "pilotai", "dist", "index.js"), paths)).toBe("npx");
  ...
  it("re-resolves a moving git branch and activates the new SHA payload", async () => {
    ...
    fs.writeFileSync(path.join(oldPayload, "node_modules", "pilotai", "package.json"), JSON.stringify({ version: "0.3.1" }));
    const newPayload = payloadPathFor(paths, "git", newSha.slice(0, 12));
    createPayload(newPayload, "0.3.1");
    fs.writeFileSync(path.join(newPayload, "node_modules", "pilotai", "package.json"), JSON.stringify({ version: "0.3.1" }));
    flipCurrentAtomic(oldPayload, paths);
    writeInstallManifestAtomic({ schemaVersion: 1, source: "git", version: "0.3.1", channel: "pinned", repo: "pilotai/pilot", ref: "master", sha: oldSha, payloadPath: oldPayload, installedAt: "2026-07-22T00:00:00.000Z", previous: [] }, paths);
  ...
  it("reports SHA git installs as pinned without resolving again", async () => {
    ...
    flipCurrentAtomic(payload, paths);
    writeInstallManifestAtomic({ schemaVersion: 1, source: "git", version: "0.3.1", channel: "pinned", repo: "pilotai/pilot", ref: sha.slice(0, 12), sha, payloadPath: payload, installedAt: "2026-07-22T00:00:00.000Z", previous: [] }, paths);
  ...
  it("requires explicit confirmation before a global npm downgrade", async () => {
    ...
    const executable = path.join(root, "lib", "node_modules", "pilotai", "dist", "index.js");
  ...
  it("isolates global npm updates from hostile registry configuration", async () => {
    ...
    const runCommand = vi.fn<CommandRunner>(async (_file, args, commandOptions) => {
      ...
      expect(args).toContain("--@pilotai:registry=https://registry.npmjs.org");
  ...
  it("explains how to recover when the pre-update database is unreachable", async () => {
    ...
    await expect(updateCommand({}, { paths, executablePath: executable, runCommand, backup, hasInstanceData: () => true })).rejects.toThrow(
      "Start the service with `pilotai service start` and retry, or skip the backup with `pilotai update --no-backup`.",
    );
    expect(backup).toHaveBeenCalledOnce();
  ...
  it("does not inherit a managed pin for global npm updates", async () => {
    ...
    expect(runCommand).toHaveBeenCalledWith("npm", expect.arrayContaining(["view", "pilotai@latest"]), expect.anything());
  });
