...
function writeBaseConfig(configPath: string) {
  const base: PilotConfig = {
    ...
    storage: {
      provider: "local_disk",
      localDisk: { baseDir: "/tmp/pilot-storage" },
      s3: {
        bucket: "pilot",
        region: "us-east-1",
        prefix: "",
    ...
    secrets: {
      ...
      strictMode: false,
      localEncrypted: { keyFilePath: "/tmp/pilot-secrets/master.key" },
    },
  };
...
describe("configure command", () => {
  it("sets a failing exit code for unknown sections", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-configure-"));
  ...
  it("sets a failing exit code when no config exists", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-configure-missing-"));
  ...
  it("backs up invalid config bytes and refuses non-interactive replacement", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-configure-invalid-"));
    const configPath = path.join(root, "config.json");
