...
function createExistingConfigFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-onboard-"));
...
function createFreshConfigPath() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-onboard-fresh-"));
  return path.join(root, ".pilot", "config.json");
}
...
describe("onboard", () => {
  ...
  it("creates instance-root config and data paths for a fresh PILOT_HOME", async () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-onboard-home-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-onboard-cwd-"));
    process.chdir(cwd);
