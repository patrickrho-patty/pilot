...
import { readZipArchive } from "@pilotai/shared/portability-zip";
import {
...
async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(path.join(os.tmpdir(), "pilot-company-import-transfer-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("resolveChunkedImportZip", () => {
  ...
  it("takes the chunked path for a small zip whose entries inflate past the threshold", async () => {
    ...
    const zipBytes = buildDeflateZip("dense-package/NOTES.md", "pilot agent docs\n".repeat(3_200_000));
...
describe("company import command over the chunked transfer path", () => {
  ...
  it("slices an oversized local zip into a transfer and applies it against the spool", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      "http://pilot.test/api/companies/import/transfers",
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      "http://pilot.test/api/companies/import/transfers/transfer-1/parts/0",
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      3,
      "http://pilot.test/api/companies/import/transfers/transfer-1/parts/1",
      expect.objectContaining({ method: "PUT" }),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      4,
      "http://pilot.test/api/companies/import/transfers/transfer-1/preview",
      expect.objectContaining({ method: "POST" }),
    );
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      5,
      "http://pilot.test/api/companies/import/transfers/transfer-1/apply",
      expect.objectContaining({ method: "POST" }),
    );
  ...
  it("keeps small local zips on the inline single-shot path", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://pilot.test/api/companies/import/preview",
      expect.objectContaining({ method: "POST" }),
    );
