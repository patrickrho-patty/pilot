...
function createBasePilotEnv(options: TestPilotEnv) {
  ...
  env.PILOT_HOME = options.pilotHome;
  env.PILOT_INSTANCE_ID = options.instanceId;
  env.PILOT_CONTEXT = path.join(options.pilotHome, "context.json");
  env.PILOT_AUTH_STORE = path.join(options.pilotHome, "auth.json");
...
function isPortableAgent(agent: { metadata?: Record<string, unknown> | null }) {
  const marker = agent.metadata?.pilotBuiltInAgent;
...
async function runCliJson<T>(
  ...
  const cliArgs = ["--silent", "pilotai", ...args];
  if (opts.apiBase) {
...
async function waitForServer(
  ...
  while (Date.now() - startedAt < 30_000) {
    if (child.exitCode !== null) {
      throw new Error(
        `pilotai run exited before healthcheck succeeded.\nstdout:\n${output.stdout.join("")}\nstderr:\n${output.stderr.join("")}`,
      );
    }
...
describeEmbeddedPostgres("pilotai company import/export e2e", () => {
  let tempRoot = "";
  let configPath = "";
  let exportDir = "";
  let apiBase = "";
  ...
  beforeAll(async () => {
    tempRoot = mkdtempSync(path.join(os.tmpdir(), "pilot-company-cli-e2e-"));
    configPath = path.join(tempRoot, "config", "config.json");
    exportDir = path.join(tempRoot, "exported-company");
    pilotHome = path.join(tempRoot, "pilot-home");
    ...
    tempDb = await startEmbeddedPostgresTestDatabase("pilot-company-cli-db-");
  ...
  it("exports a company package and imports it into new and existing companies", async () => {
    ...
    const leakedContextPath = path.join(cliShellHome, ".pilot", "context.json");
    ...
    expect(readFileSync(path.join(exportDir, ".pilot.yaml"), "utf8")).toContain('schema: "pilot/v1"');
    ...
    writeFileSync(zipPath, createStoredZipArchive(portableFiles, "pilot-demo"));
