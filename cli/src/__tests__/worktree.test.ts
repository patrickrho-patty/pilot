...
async function seedValidWorktreeSource(
  ...
  await db.insert(authUsers).values({
    id: userId,
    email: userId === "local-board" ? "local@pilot.local" : "existing@pilot.test",
...
describe("worktree helpers", () => {
  it("uses the repo-local config for the current worktree", () => {
    const targetRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-current-worktree-")));
    try {
      const localConfig = path.join(targetRoot, ".pilot", "config.json");
      fs.mkdirSync(path.dirname(localConfig), { recursive: true });
      fs.writeFileSync(localConfig, "{}\n");
      process.env.PILOT_CONFIG = "/tmp/ambient-pilot/config.json";
  ...
  it("uses the repository config from a nested working directory", () => {
    const targetRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-current-worktree-nested-")));
    try {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("pilot-pr-432")).toBe(
      path.resolve(os.homedir(), "pilot-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("pilot/pr-432")).toThrow(
  ...
  it("rewrites auth URLs only when they already include a port", () => {
    ...
    expect(rewriteLocalUrlPort("https://pilot.example", 3110)).toBe("https://pilot.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/pilot-feature",
      homeDir: "/tmp/pilot-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/pilot-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/pilot-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );
    ...
    expect(env.PILOT_HOME).toBe(path.resolve("/tmp/pilot-worktrees"));
    expect(env.PILOT_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.PILOT_IN_WORKTREE).toBe("true");
    expect(env.PILOT_DB_BACKUP_ENABLED).toBe("false");
    expect(env.PILOT_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.PILOT_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export PILOT_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  it("requires the seed process to own the target embedded Postgres lifecycle", async () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-live-target-")));
    try {
      fs.writeFileSync(
  ...
  it("rejects a source migration journal that diverges from the code journal", () => {
    expect(() => resolveWorktreeSeedMigrationRevision({
      ...
      journalEntryCount: 3,
    }, "sourcePrefix")).toThrow("Migration journal is not a prefix of this Pilot checkout");
  });
  ...
  itEmbeddedPostgres("recognizes positive legacy database schema evidence", async () => {
    const tempDb = await startEmbeddedPostgresTestDatabase("pilot-worktree-legacy-evidence-");
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-legacy-config-")));
    try {
  ...
  it("ensure-seeded seeds once and fast-exits on the verified manifest", async () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-ensure-seeded-")));
    try {
      const sourceConfigPath = path.join(tempRoot, "source", "config.json");
      const targetRoot = path.join(tempRoot, "worktree");
      const targetConfigPath = path.join(targetRoot, ".pilot", "config.json");
      ...
      fs.writeFileSync(path.join(path.dirname(sourceConfigPath), ".env"), "PILOT_INSTANCE_ID=source\n");
      fs.writeFileSync(targetConfigPath, `${JSON.stringify(targetConfig)}\n`);
      fs.writeFileSync(
        path.join(targetRoot, ".pilot", ".env"),
        `PILOT_HOME=${targetPaths.homeDir}\nPILOT_INSTANCE_ID=${targetPaths.instanceId}\n`,
      );
      ...
      expect(fs.existsSync(path.join(targetRoot, ".pilot", "seed-pending"))).toBe(false);
      expect(fs.existsSync(path.join(targetRoot, ".pilot", "seed-complete"))).toBe(false);
  ...
  it("treats an unregistered markerless config as a normal non-worktree boot", async () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-unregistered-markerless-")));
    try {
      const configPath = path.join(tempRoot, "config.json");
      fs.writeFileSync(configPath, `${JSON.stringify(buildSourceConfig())}\n`);
      delete process.env.PILOT_WORKSPACE_BASE_CWD;
      delete process.env.PILOT_PROJECT_WORKSPACE_ID;
      delete process.env.PILOT_SEED_EXPECTED_COMPANY_ID;
  ...
  it("honors a legacy complete marker without resolving a seed source", async () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-complete-marker-")));
    try {
      const configPath = path.join(tempRoot, "config.json");
      fs.writeFileSync(configPath, `${JSON.stringify(buildSourceConfig())}\n`);
  ...
  it("seeds a configured worktree with no seed markers when no legacy database is present", async () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-unmarked-empty-")));
    try {
      const sourceConfigPath = path.join(tempRoot, "source", "config.json");
      const targetRoot = path.join(tempRoot, "worktree");
      const targetConfigPath = path.join(targetRoot, ".pilot", "config.json");
  ...
  it("adopts a markerless legacy worktree only after validating its database schema", async () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-unmarked-legacy-")));
    try {
      const sourceConfigPath = path.join(tempRoot, "source", "config.json");
      const targetRoot = path.join(tempRoot, "worktree");
      const targetConfigPath = path.join(targetRoot, ".pilot", "config.json");
  ...
  it("managed ensure-seeded derives a valid source from the registered base workspace", async () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-managed-seed-")));
    try {
      const baseRoot = path.join(tempRoot, "base");
      const sourceConfigPath = path.join(baseRoot, ".pilot", "config.json");
      const targetRoot = path.join(tempRoot, "worktree");
      const targetConfigPath = path.join(targetRoot, ".pilot", "config.json");
      const targetPaths = resolveWorktreeLocalPaths({
        cwd: targetRoot,
      ...
      fs.mkdirSync(path.dirname(targetConfigPath), { recursive: true });
      fs.writeFileSync(sourceConfigPath, `${JSON.stringify(sourceConfig)}\n`);
      fs.writeFileSync(path.join(path.dirname(sourceConfigPath), ".env"), "PILOT_INSTANCE_ID=managed-source\n");
      fs.writeFileSync(targetConfigPath, `${JSON.stringify(targetConfig)}\n`);
      fs.writeFileSync(
        path.join(path.dirname(targetConfigPath), ".env"),
        `PILOT_HOME=${targetPaths.homeDir}\nPILOT_INSTANCE_ID=managed-target\n`,
      );
      markWorktreeSeedPending({ configPath: targetConfigPath, sourceConfigPath });
      const seedDatabase = vi.fn().mockResolvedValue(mockVerifiedSeedResult());
  ...
  it.each(["sibling", "foreign_instance", "symlink", "instance_mismatch"] as const)(
    ...
    async (variant) => {
      const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), `pilot-worktree-managed-${variant}-`)));
      try {
        const baseRoot = path.join(tempRoot, "base");
        const canonicalSource = path.join(baseRoot, ".pilot", "config.json");
        const targetRoot = path.join(tempRoot, "worktree");
        const targetConfigPath = path.join(targetRoot, ".pilot", "config.json");
        ...
        fs.writeFileSync(path.join(path.dirname(canonicalSource), ".env"), "PILOT_INSTANCE_ID=registered-source\n");
        fs.writeFileSync(targetConfigPath, `${JSON.stringify(buildSourceConfig())}\n`);
        fs.writeFileSync(
          path.join(path.dirname(targetConfigPath), ".env"),
          `PILOT_HOME=${path.join(tempRoot, "worktree-home")}\nPILOT_INSTANCE_ID=managed-target\n`,
        );
        fs.writeFileSync(attackerConfig, `${JSON.stringify(buildSourceConfig())}\n`);
        fs.writeFileSync(
          path.join(attackerRoot, ".env"),
          `PILOT_INSTANCE_ID=${variant === "foreign_instance" ? "foreign" : "registered-source"}\n`,
        );
        ...
        expect(fs.existsSync(path.join(targetRoot, ".pilot", "seed.lock"))).toBe(false);
      } finally {
        fs.rmSync(tempRoot, { recursive: true, force: true });
      }
    },
  ...
  it("ensure-seeded records a target shutdown diagnostic when restore fails", async () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-ensure-seeded-failure-")));
    try {
      const sourceConfigPath = path.join(tempRoot, "source", "config.json");
      const targetRoot = path.join(tempRoot, "worktree");
      const targetConfigPath = path.join(targetRoot, ".pilot", "config.json");
  ...
  it("serializes concurrent ensure-seeded calls across the seed marker lock", async () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-ensure-seeded-lock-")));
    try {
      const sourceConfigPath = path.join(tempRoot, "source", "config.json");
      const targetRoot = path.join(tempRoot, "worktree");
      const targetConfigPath = path.join(targetRoot, ".pilot", "config.json");
  ...
  it("records an interrupted phase before retrying to a verified terminal state", async () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-interrupted-seed-")));
    try {
      const sourceConfigPath = path.join(tempRoot, "source", "config.json");
      const targetRoot = path.join(tempRoot, "worktree");
      const targetConfigPath = path.join(targetRoot, ".pilot", "config.json");
      ...
      fs.writeFileSync(
        path.join(targetRoot, ".pilot", "seed-manifest.json"),
  ...
  it("fails closed instead of racing to reclaim a stale seed lock", async () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-ensure-seeded-stale-lock-")));
    try {
      const targetConfigPath = path.join(tempRoot, ".pilot", "config.json");
      const lockPath = path.join(tempRoot, ".pilot", "seed.lock");
      fs.mkdirSync(path.dirname(targetConfigPath), { recursive: true });
      fs.writeFileSync(
        lockPath,
        `${JSON.stringify({
  ...
  itEmbeddedPostgres("quarantines copied live execution state in seeded worktree databases", async () => {
    const tempDb = await startEmbeddedPostgresTestDatabase("pilot-worktree-quarantine-");
    ...
    try {
      ...
      await db.insert(projectWorkspaces).values({
        ...
        metadata: {
          ...
          runtimeConfig: {
            workspaceRuntime: { services: [{ name: "pilot-dev" }] },
      ...
      await db.insert(workspaceRuntimeServices).values({
        ...
        port: 42013,
        url: "https://pilot-dev.example.test:42013",
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-secrets-")));
    const originalInlineMasterKey = process.env.PILOT_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.PILOT_SECRETS_MASTER_KEY_FILE;
    try {
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.PILOT_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.PILOT_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.PILOT_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          PILOT_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-jwt-")));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    const originalJwtSecret = process.env.PILOT_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.PILOT_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".pilot-worktrees"),
      });

      const envPath = path.join(repoRoot, ".pilot", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("PILOT_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("PILOT_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/PILOT_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.PILOT_AGENT_JWT_SECRET;
      } else {
        process.env.PILOT_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves repo-managed worktree checkouts when --force re-runs from the source repo", async () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-force-preserve-")));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      const repoConfigDir = path.join(repoRoot, ".pilot");
      ...
      const checkoutDir = path.join(worktreesDir, "PIL-100-feature");
  ...
  itEmbeddedPostgres(
    ...
    async () => {
      const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-local-board-seed-")));
      const worktreeRoot = path.join(tempRoot, "PIL-17696-local-board-seed");
      ...
      const worktreeHome = path.join(tempRoot, ".pilot-worktrees");
      const originalCwd = process.cwd();
      const sourceDb = await startEmbeddedPostgresTestDatabase("pilot-worktree-local-board-source-");
      ...
      try {
        ...
        await worktreeInitCommand({
          name: "PIL-17696-local-board-seed",
          home: worktreeHome,
          fromConfig: sourceConfigPath,
          force: true,
        });

        const targetConfigPath = path.join(worktreeRoot, ".pilot", "config.json");
        ...
        try {
          const targetDb = createDb(
            `postgres://pilot:pilot@127.0.0.1:${targetConfig.database.embeddedPostgresPort}/pilot`,
          );
  ...
  it("reserves distinct ports for postgres-mode siblings under a custom worktree parent", async () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-custom-parent-")));
    const homeDir = path.join(tempRoot, ".pilot-worktrees");
    ...
    try {
      ...
      firstConfig.database = {
        ...firstConfig.database,
        mode: "postgres",
        connectionString: "postgres://pilot:pilot@127.0.0.1:54330/pilot",
      };
  ...
  it("defaults the seed source config to the current repo-local Pilot config", () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-source-config-")));
    const repoRoot = path.join(tempRoot, "repo");
    const localConfigPath = path.join(repoRoot, ".pilot", "config.json");
    const originalCwd = process.cwd();
    const originalPilotConfig = process.env.PILOT_CONFIG;
    ...
    try {
      ...
      if (originalPilotConfig === undefined) {
        ...
        process.env.PILOT_CONFIG = originalPilotConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-source-override-")));
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("derives worktree reseed target paths from the adjacent env file", () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-reseed-target-")));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".pilot", "config.json");
    const envPath = path.join(worktreeRoot, ".pilot", ".env");
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          ...
          "PILOT_INSTANCE_ID=pil-1132-chat",
        ].join("\n"),
        "utf8",
      );
      expect(
  ...
  it("rejects reseed targets without worktree env metadata", () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-reseed-target-missing-")));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".pilot", "config.json");
    ...
    try {
      fs.mkdirSync(path.dirname(configPath), { recursive: true });
      fs.writeFileSync(configPath, JSON.stringify(buildSourceConfig()), "utf8");
      fs.writeFileSync(path.join(worktreeRoot, ".pilot", ".env"), "", "utf8");

      expect(() =>
        resolveWorktreeReseedTargetPaths({
          configPath,
          rootPath: worktreeRoot,
        })).toThrow("does not look like a worktree-local Pilot instance");
    } finally {
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  itEmbeddedPostgres("reseed preserves the current worktree ports, instance id, and branding", async () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-reseed-")));
    const repoRoot = path.join(tempRoot, "repo");
    ...
    const sourcePaths = resolveWorktreeLocalPaths({
      cwd: sourceRoot,
      homeDir: path.join(tempRoot, ".pilot-source"),
      instanceId: "default",
    });
    const originalCwd = process.cwd();
    const originalPilotConfig = process.env.PILOT_CONFIG;
    ...
    const sourceDb = await startEmbeddedPostgresTestDatabase("pilot-worktree-reseed-source-");
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          `PILOT_HOME=${homeDir}`,
          `PILOT_INSTANCE_ID=${currentInstanceId}`,
          "PILOT_WORKTREE_NAME=existing-name",
          "PILOT_WORKTREE_COLOR=\"#112233\"",
        ].join("\n"),
        "utf8",
      );
      ...
      expect(rewrittenEnv).toContain(`PILOT_INSTANCE_ID=${currentInstanceId}`);
      expect(rewrittenEnv).toContain("PILOT_WORKTREE_NAME=existing-name");
      expect(rewrittenEnv).toContain("PILOT_WORKTREE_COLOR=\"#112233\"");
  ...
  it("restores the current worktree config and instance data if reseed fails", async () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-reseed-rollback-")));
    const repoRoot = path.join(tempRoot, "repo");
    const sourceRoot = path.join(tempRoot, "source");
    const homeDir = path.join(tempRoot, ".pilot-worktrees");
    ...
    try {
      ...
      fs.writeFileSync(currentPaths.configPath, JSON.stringify(currentConfig, null, 2), "utf8");
      fs.writeFileSync(currentPaths.envPath, `PILOT_HOME=${homeDir}\nPILOT_INSTANCE_ID=${currentInstanceId}\n`, "utf8");
      ...
      expect(restoredEnv).toContain(`PILOT_INSTANCE_ID=${currentInstanceId}`);
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/pilot",
        targetRepoRoot: "/Users/example/pilot-pr-432",
        workspaceCwd: "/Users/example/pilot",
      }),
    ).toBe("/Users/example/pilot-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/pilot",
        targetRepoRoot: "/Users/example/pilot-pr-432",
        workspaceCwd: "/Users/example/pilot/packages/db",
      }),
    ).toBe("/Users/example/pilot-pr-432/packages/db");
  });
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-hooks-")));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-make-")));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "pilot-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("pilot-make-test", {
        seed: false,
        home: path.join(tempRoot, ".pilot-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".pilot", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".pilot", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
  ...
  it("no-ops on the primary checkout unless --branch is provided", async () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-repair-primary-")));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      expect(fs.existsSync(path.join(repoRoot, ".pilot", "config.json"))).toBe(false);
      expect(fs.existsSync(path.join(repoRoot, ".pilot", "worktrees"))).toBe(false);
    } finally {
      process.chdir(originalCwd);
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  ...
  it("repairs the current linked worktree when Pilot metadata is missing", async () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-repair-current-")));
    const repoRoot = path.join(tempRoot, "repo");
    const worktreePath = path.join(repoRoot, ".pilot", "worktrees", "repair-me");
    const sourceConfigPath = path.join(tempRoot, "source-config.json");
    const worktreeHome = path.join(tempRoot, ".pilot-worktrees");
  ...
  it("creates and repairs a missing branch worktree when --branch is provided", async () => {
    const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "pilot-worktree-repair-branch-")));
    const repoRoot = path.join(tempRoot, "repo");
    const sourceConfigPath = path.join(tempRoot, "source-config.json");
    const worktreeHome = path.join(tempRoot, ".pilot-worktrees");
    const originalCwd = process.cwd();
    const expectedWorktreePath = path.join(repoRoot, ".pilot", "worktrees", "feature-repair-me");
    ...
    try {
      ...
      expect(fs.existsSync(path.join(expectedWorktreePath, ".pilot", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(expectedWorktreePath, ".pilot", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
...
describeEmbeddedPostgres("pauseSeededScheduledRoutines", () => {
  it("pauses only routines with enabled schedule triggers", async () => {
    const tempDb = await startEmbeddedPostgresTestDatabase("pilot-worktree-routines-");
    const db = createDb(tempDb.connectionString);
    const companyId = randomUUID();
    const projectId = randomUUID();
    const agentId = randomUUID();
