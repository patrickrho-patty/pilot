...
describe("resolveCompanyForDeletion", () => {
  ...
  it("resolves by ID in auto mode", () => {
    ...
    expect(result.issuePrefix).toBe("PIL");
  });

  it("resolves by prefix in auto mode", () => {
    const result = resolveCompanyForDeletion(companies, "pil", "auto");
  ...
  it("respects explicit id mode", () => {
    expect(() => resolveCompanyForDeletion(companies, "PIL", "id")).toThrow(/No company found by ID/);
  });
...
describe("assertDeleteConfirmation", () => {
  ...
  it("requires --yes", () => {
    expect(() => assertDeleteConfirmation(company, { confirm: "PIL" })).toThrow(/requires --yes/);
  });

  it("accepts matching prefix confirmation", () => {
    expect(() => assertDeleteConfirmation(company, { yes: true, confirm: "pil" })).not.toThrow();
  });
