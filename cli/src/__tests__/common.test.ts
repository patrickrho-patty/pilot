...
function createTempPath(name: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-cli-common-"));
...
describe("resolveCommandContext", () => {
  beforeEach(() => {
    ...
    delete process.env.PILOT_AUTH_STORE;
    delete process.env.PILOT_SERVER_PORT;
  });
  ...
  it("resolves api base by explicit, env, profile, then config/default precedence", () => {
    ...
    process.env.PILOT_API_URL = "http://env:3/";
  ...
  it("prefers explicit and env tokens over profile env and stored board auth", () => {
    ...
    process.env.PILOT_AUTH_STORE = authStorePath;
    ...
    process.env.PILOT_API_KEY = "env-token";
...
describe("apiPath", () => {
  it("encodes dynamic path segments", () => {
    expect(apiPath`/api/issues/${"PIL-1/child"}/comments/${"needs review?"}`)
      .toBe("/api/issues/PIL-1%2Fchild/comments/needs%20review%3F");
  });
