...
describe("env-lab cleanup command hint", () => {
  ...
  it("never restores the unsafe pnpm invocation forms", () => {
    ...
    expect(command).not.toContain("pnpm pilotai");
    expect(command).not.toContain("pnpm exec pilotai");
  });
...
describe("env-lab doctor cleanup hint instance", () => {
  ...
  it("pins the PILOT_INSTANCE_ID instance when opts.instance is absent", async () => {
    // The doctor diagnoses the instance that `PILOT_INSTANCE_ID` selects.
  ...
  it("pins the explicit instance over PILOT_INSTANCE_ID", async () => {
