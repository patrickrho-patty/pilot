...
describe("managed install commands", () => {
  ...
  beforeEach(() => {
    root = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-install-command-"));
    process.env = {
      ...ORIGINAL_ENV,
      HOME: path.join(root, "home"),
      PILOT_HOME: path.join(root, "home", ".pilot"),
      PATH: "/usr/bin:/bin",
      SHELL: "/bin/bash",
    };
  ...
  it("resolves branch, tag, full SHA, and short SHA refs through GitHub", async () => {
    ...
    for (const ref of ["master", "v1.2.3", sha, sha.slice(0, 12)]) {
      await expect(resolveGitHubRef("pilotai/pilot", ref, runCommand)).resolves.toBe(sha);
    }
    expect(runCommand.mock.calls.map((call) => call[1].at(-1))).toEqual([
      "https://api.github.com/repos/pilotai/pilot/commits/master",
      "https://api.github.com/repos/pilotai/pilot/commits/v1.2.3",
      `https://api.github.com/repos/pilotai/pilot/commits/${sha}`,
      `https://api.github.com/repos/pilotai/pilot/commits/${sha.slice(0, 12)}`,
    ]);
  });

  it("supports fork overrides and classifies SHA refs as pinned", () => {
    expect(resolveGitInstallRequest({ ref: "feature/test", repo: "HenkDz/pilot" })).toEqual({ repo: "HenkDz/pilot", ref: "feature/test", pinned: false });
    expect(resolveGitInstallRequest({ ref: "abcdef1" })).toEqual({ repo: "pilotai/pilot", ref: "abcdef1", pinned: true });
    expect(() => resolveGitInstallRequest({ repo: "HenkDz/pilot" })).toThrow("requires --ref");
  });

  it("requires explicit non-interactive consent before resolving git refs", async () => {
    const runCommand = vi.fn();

    await expect(installCommand({ ref: "master", repo: "HenkDz/pilot" }, { runCommand }))
  ...
  it("reuses a SHA-keyed git payload without downloading or rebuilding", async () => {
    ...
    const packageRoot = path.join(payloadPath, "node_modules", "pilotai");
    ...
    await expect(installGitPayload("pilotai/pilot", sha, runCommand, paths)).resolves.toEqual({ payloadPath, reused: true, version: "0.3.1" });
  ...
  const createGitCheckoutRunCommand = (sha: string) =>
    vi.fn(async (file: string, args: string[], _options?: Parameters<CommandRunner>[2]) => {
      ...
      if (file === "tar") {
        ...
        const packages = [
          { dir: "packages/shared", name: "@pilotai/shared", packageJson: { name: "@pilotai/shared", version: "0.3.1" } },
          { dir: "packages/db", name: "@pilotai/db", packageJson: { name: "@pilotai/db", version: "0.3.1", dependencies: { "@pilotai/shared": "workspace:*" }, bundleDependencies: ["embedded-postgres"] } },
          { dir: "server", name: "@pilotai/server", packageJson: { name: "@pilotai/server", version: "0.3.1", dependencies: { "@pilotai/db": "workspace:*" } } },
        ];
      ...
      if (file === "corepack") {
        if (args.includes("pack")) {
          ...
          const packageName = packageDir === "server" ? "pilotai-server" : "pilotai-shared";
      ...
      if (file === "npm" && args[0] === "pack") {
        const packageName = args[1]?.includes("workspace-package-") ? "pilotai-db" : "pilotai";
        fs.writeFileSync(path.join(args[args.indexOf("--pack-destination") + 1], `${packageName}-0.3.1.tgz`), "package");
        return { stdout: "", stderr: "" };
      }
      if (file === "npm" && args[0] === "install") { const prefix = args[args.indexOf("--prefix") + 1]; const packageRoot = path.join(prefix, "node_modules", "pilotai"); fs.mkdirSync(path.join(packageRoot, "dist"), { recursive: true }); fs.writeFileSync(path.join(packageRoot, "package.json"), JSON.stringify({ version: "0.3.1" })); fs.writeFileSync(path.join(packageRoot, "dist", "index.js"), "#!/usr/bin/env node\n"); return { stdout: "", stderr: "" }; }
      if (file === process.execPath && args[0]?.endsWith("prepare-bundled-package.mjs")) {
        fs.mkdirSync(args[2], { recursive: true });
        fs.writeFileSync(path.join(args[2], "package.json"), JSON.stringify({ name: "@pilotai/db", version: "0.3.1" }));
        return { stdout: "", stderr: "" };
      }
  ...
  it("installs a GitHub branch through codeload and reuses the resolved SHA", async () => {
    ...
    await installCommand({ ref: "master", repo: "HenkDz/pilot", yes: true }, { runCommand });
    await installCommand({ ref: "master", repo: "HenkDz/pilot", yes: true }, { runCommand });
    const manifest = readInstallManifest(resolveInstallStorePaths());
    expect(manifest).toMatchObject({ source: "git", repo: "HenkDz/pilot", ref: "master", sha });
  ...
  it("builds git checkouts with NODE_ENV cleared so ambient production mode keeps devDependencies", async () => {
    ...
    const runCommand = createGitCheckoutRunCommand(sha);
    await expect(installGitPayload("pilotai/pilot", sha, runCommand, resolveInstallStorePaths())).resolves.toMatchObject({ version: "0.3.1", reused: false });
  ...
  it("resolves the complete server workspace dependency closure in dependency order", () => {
    ...
    const packages = [
      { dir: "packages/shared", name: "@pilotai/shared", dependencies: {} },
      { dir: "packages/db", name: "@pilotai/db", dependencies: { "@pilotai/shared": "workspace:*" } },
      { dir: "server", name: "@pilotai/server", dependencies: { "@pilotai/db": "workspace:*" } },
    ];
    fs.mkdirSync(path.join(checkout, "scripts"), { recursive: true });
    fs.writeFileSync(path.join(checkout, "scripts", "release-package-manifest.json"), JSON.stringify(packages.map(({ dir, name }) => ({ dir, name }))));
    for (const workspacePackage of packages) {
  ...
  it("installs through the shim, reports provenance, and uninstalls without deleting user data", async () => {
    ...
    const runCommand = vi.fn(async (file: string, args: string[], _options?: unknown) => {
      ...
      if (file === "npm" && args[0] === "install") {
        ...
        const entrypoint = path.join(prefix, "node_modules", "pilotai", "dist", "index.js");
    ...
    expect(installCall?.[1]).toContain("--@pilotai:registry=https://registry.npmjs.org");
    ...
    const entrypoint = path.join(manifest!.payloadPath, "node_modules", "pilotai", "dist", "index.js");
  ...
  it("refuses a symlinked git payload root before downloading", async () => {
    ...
    await expect(installGitPayload("pilotai/pilot", "4".repeat(40), runCommand, paths)).rejects.toThrow("unsafe payload root");
    expect(runCommand).not.toHaveBeenCalled();
  });
