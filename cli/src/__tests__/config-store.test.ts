...
import { pilotConfigSchema, type PilotConfig } from "../config/schema.js";
...
function createConfigPath(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "pilot-config-store-"));
  roots.push(root);
