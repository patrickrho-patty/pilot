...
export const MANAGED_SHIM_MARKER = "pilotai managed install shim v1";
export const MANAGED_STORE_MARKER = "pilotai managed install store v1\n";
export const PATH_BLOCK_START = "# >>> pilotai managed PATH >>>";
export const PATH_BLOCK_END = "# <<< pilotai managed PATH <<<";
...
export function resolveInstallStorePaths(options: {
  ...
  const pilotHome = path.resolve(options.pilotHome ?? resolvePilotHomeDir());
  ...
  return {
    ...
    shimPath: path.join(homeDir, ".local", "bin", "pilotai"),
  };
}
...
export function writeManagedShim(paths = resolveInstallStorePaths()): void {
  ...
  assertManagedShimWritable(paths);
  const entrypoint = path.join(paths.currentPath, "node_modules", "pilotai", "dist", "index.js");
