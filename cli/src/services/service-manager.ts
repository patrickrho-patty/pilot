...
export function resolveServiceShimPath(homeDir = os.homedir()): string {
  return process.env.PILOT_SHIM_PATH?.trim() || path.join(homeDir, ".local", "bin", "pilotai");
}

export function systemdServiceName(instanceId: string): string {
  return instanceId === "default" ? "pilot.service" : `pilot-${instanceId}.service`;
}

export function launchdServiceName(instanceId: string): string {
  return instanceId === "default" ? "io.patty.pilot" : `io.patty.pilot.${instanceId}`;
}

export function renderSystemdUnit(input: { instanceId: string; shimPath: string; homeDir: string }): string {
  return `[Unit]
Description=Pilot AI (${escapeSystemd(input.instanceId)})
...
Environment="PILOT_SERVICE_MANAGED=1"
Environment="PILOT_INSTANCE_ID=${escapeSystemd(input.instanceId)}"
Environment="PILOT_HOME=${escapeSystemd(input.homeDir)}"
WorkingDirectory=%h
Restart=always
RestartSec=5
TimeoutStopSec=300
...
<dict>
  ...
  <dict>
    <key>PILOT_SERVICE_MANAGED</key><string>1</string>
    <key>PILOT_INSTANCE_ID</key><string>${escapeXml(input.instanceId)}</string>
    <key>PILOT_HOME</key><string>${escapeXml(input.homeDir)}</string>
  </dict>
...
export async function detectServiceManager(input: { instanceId?: string; platform?: NodeJS.Platform; runner?: CommandRunner } = {}): Promise<ServiceManagerDetection> {
  ...
  if (platform !== "linux") return { supported: false, reason: `Service management is not supported on ${platform}. Use pilotai run instead.` };
  try {
    await runner("systemctl", ["--user", "show-environment"]);
    return { supported: true, manager: new SystemdServiceManager(instanceId, runner) };
  } catch {
    return { supported: false, reason: "No usable systemd user manager was detected (common in containers and WSL1). Use pilotai run instead." };
  }
}
...
export async function assertForegroundRunAllowed(instanceId: string, force = false, detector: typeof detectServiceManager = detectServiceManager): Promise<void> {
  ...
  if (status.active) throw new Error(`Pilot instance '${instanceId}' is already running as ${status.serviceName}. Use 'pilotai service status --instance ${instanceId}' or pass --force to bypass this safety check.`);
}
