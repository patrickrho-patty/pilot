...
const defaultDependencies: OnboardServiceDependencies = {
  ...
  confirm: async () => {
    const answer = await p.confirm({
      message: "Install Pilot as a background service?",
      initialValue: true,
    });
  ...
  confirmLinger: async () => {
    const answer = await p.confirm({
      message: "Allow Pilot to keep running after logout? This may request system authorization.",
      initialValue: false,
    });
    return !p.isCancel(answer) && answer === true;
  },
...
export async function handleOnboardService(
  ...
  if (!explicitlyRequested && !canPrompt) {
    deps.info(
      "Background service not installed. Use `pilotai onboard --install-service` or `pilotai service install` to opt in.",
    );
    return false;
  }
