/**
 * esbuild configuration for building the pilotai CLI for npm.
 *
 * Bundles all workspace packages (@pilotai/*) into a single file.
...
// to npm and resolved at runtime (e.g. @pilotai/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@pilotai/") && !bundledCliNpmDependencies.has(name)) {
      externals.add(name);
    }
  }
