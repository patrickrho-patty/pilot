...
Example Pilot plugin that demonstrates:
...
```bash
pnpm --filter @pilotai/plugin-file-browser-example build
npx pilotai plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
npx pilotai plugin uninstall pilot-file-browser-example --force
```
...
- Optional: use `pilot-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
