# @pilotai/plugin-hello-world-example
...
- a minimal React widget rendered in the Pilot dashboard
...
```bash
pnpm --filter @pilotai/plugin-hello-world-example build
npx pilotai plugin install ./packages/plugins/examples/plugin-hello-world-example
```
...
- **Reinstall after pulling.** If you installed a plugin by local path before the server stored `package_path`, the plugin may show status **error** (worker not found). Uninstall and install again so the server persists the path and can activate the plugin:  
  `npx pilotai plugin uninstall pilot.hello-world-example --force` then  
  `npx pilotai plugin install ./packages/plugins/examples/plugin-hello-world-example`.
