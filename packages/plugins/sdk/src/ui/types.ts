 ...
 *
 * Plugin UI bundles import from `@pilotai/plugin-sdk/ui`.  This subpath
...
export interface HostNavigation {
   ...
   *
   * For example, in company `PIL`, `resolveHref("/wiki")` returns
   * `"/PIL/wiki"`, while `resolveHref("/PIL/wiki")` stays unchanged.
   */
