 ...
 * import { startWorkerRpcHost } from "@pilotai/plugin-sdk";
 *
 * startWorkerRpcHost({ plugin });
 * ```
 *
...
export function startWorkerRpcHost(options: WorkerRpcHostOptions): WorkerRpcHost {
  ...
  const toolHandlers = new Map<string, {
    declaration: Pick<import("@pilotai/shared").PluginToolDeclaration, "displayName" | "description" | "parametersSchema">;
    ...
    return new Promise<WorkerToHostMethods[M][1]>((resolve, reject) => {
      ...
      try {
        ...
        const request = {
          ...
          ...(activeInvocation ? { pilotInvocationId: activeInvocation.id } : {}),
        };
        sendMessage(request);
      } catch (err) {
  ...
  async function handleHostRequest(request: JsonRpcRequest): Promise<void> {
    ...
    try {
      ...
      const result = request.pilotInvocation
        ? await invocationContextStorage.run(request.pilotInvocation, invoke)
        : await invoke();
  ...
  function handleLine(line: string): void {
    ...
    if (isJsonRpcResponse(message)) {
      ...
      const runNotification = (fn: () => void | Promise<void>) => {
        if (notif.pilotInvocation) {
          return invocationContextStorage.run(notif.pilotInvocation, fn);
        }
        return fn();
      };
