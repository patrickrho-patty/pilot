...
- **Worker SDK:** `@pilotai/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@pilotai/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@pilotai/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@pilotai/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@pilotai/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@pilotai/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@pilotai/plugin-sdk/ui` | UI entry: hooks, host navigation, HTTP-safe clipboard copy, shared components, and slot prop types |
| `@pilotai/plugin-sdk/ui/hooks` | Hooks only |
| `@pilotai/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@pilotai/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@pilotai/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@pilotai/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@pilotai/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@pilotai/plugin-sdk/types` | Worker context and API types (advanced) |
...
- The host ships a small shared React component kit through `@pilotai/plugin-sdk/ui`. Use it for native Pilot controls; custom React and CSS are still supported.
...
```ts
import { definePlugin, runWorker } from "@pilotai/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@pilotai/plugin-sdk`.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@pilotai/plugin-sdk`.
...
```tsx
import { usePluginData, usePluginAction } from "@pilotai/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@pilotai/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@pilotai/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@pilotai/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@pilotai/plugin-sdk/ui";
...
```tsx
import { useHostNavigation } from "@pilotai/plugin-sdk/ui";
...
`linkProps("/wiki")` resolves against the active company prefix, so in company `PIL` it renders `href="/PIL/wiki"`. Already-prefixed paths such as `/PIL/wiki` are not prefixed again. For button-style commands, call `hostNavigation.navigate("/issues/PIL-123")`.
...
```tsx
import { usePluginStream } from "@pilotai/plugin-sdk/ui";
...
The host provides selected shared UI components through `@pilotai/plugin-sdk/ui`.
...
```tsx
import { copyTextToClipboard } from "@pilotai/plugin-sdk/ui";
...
```tsx
import { MarkdownBlock, MarkdownEditor } from "@pilotai/plugin-sdk/ui";
...
```tsx
<MarkdownBlock
  content={"See [[wiki/entities/pilot|Pilot]]."}
  enableWikiLinks
...
```tsx
import { FileTree, type FileTreeNode } from "@pilotai/plugin-sdk/ui";
...
```tsx
import { AssigneePicker, ProjectPicker } from "@pilotai/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@pilotai/plugin-sdk/ui`.
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@pilotai/plugin-sdk/ui";
...
```ts
import { createPluginBundlerPresets } from "@pilotai/plugin-sdk/bundlers";
...
```ts
import { startPluginDevServer } from "@pilotai/plugin-sdk/dev-server";
