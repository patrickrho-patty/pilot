...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".pilot-sdk");
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Pilot plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "pilot-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["pilot", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@pilotai/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@pilotai/plugin-sdk": sdkDependency,
...
`,
    ...
    writeFile(
      ...
      `import { usePluginData, type PluginWidgetProps } from "@pilotai/plugin-sdk/ui";
...
} from "@pilotai/plugin-sdk/testing";
...
`,
    ...
    writeFile(
      path.join(outputDir, "src", "ui", "index.tsx"),
      `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@pilotai/plugin-sdk/ui";
...
When this package is installed from a local path, Pilot watches that rebuilt
output and reloads the plugin worker. Local installs run trusted code from this
folder on your machine.

${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@pilotai/plugin-sdk\` and \`@pilotai/shared\` from a local Pilot checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.pilot-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}

## Install Into Pilot

\`\`\`bash
pilotai plugin install ${shellQuote(toPosixPath(outputDir))}
\`\`\`
