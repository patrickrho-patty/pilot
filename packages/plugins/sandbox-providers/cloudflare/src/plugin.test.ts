...
describe("Cloudflare sandbox provider plugin", () => {
  ...
  it("maps acquire lease responses from the bridge", async () => {
    fetchMock.mockResolvedValueOnce(
      jsonResponse({
        providerLeaseId: "pilot-run-1-abcd1234",
        metadata: {
          provider: "cloudflare",
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "https://bridge.example.workers.dev/api/pilot-sandbox/v1/leases/acquire",
      expect.objectContaining({
        method: "POST",
        headers: expect.any(Headers),
      }),
    );
    expect(requestHeadersAt().get("x-pilot-Run-Id")).toBe("run-1");
    expect(requestHeadersAt().get("x-pilot-Environment-Id")).toBe("env-1");
    expect(requestHeadersAt().get("x-pilot-Issue-Id")).toBe("issue-1");
  ...
  it("defaults the sleepAfter passed to the bridge to 1h so long runs don't idle out", async () => {
    fetchMock.mockResolvedValueOnce(
      jsonResponse({
        providerLeaseId: "pilot-run-1-abcd1234",
        metadata: { provider: "cloudflare", remoteCwd: "/workspace/pilot", resumedLease: false },
      }),
    );
  ...
  it("returns expired lease semantics when resume reports lost state", async () => {
    ...
    const lease = await plugin.definition.onEnvironmentResumeLease?.({
      ...
      companyId: "company-1",
      environmentId: "env-1",
      providerLeaseId: "pilot-env-env-1",
      leaseMetadata: { remoteCwd: "/workspace/pilot" },
      config: {
        bridgeBaseUrl: "https://bridge.example.workers.dev",
        bridgeAuthToken: "resolved-token",
      },
  ...
  it("passes bridge execute results through unchanged", async () => {
    ...
    const result = await plugin.definition.onEnvironmentExecute?.({
      ...
      companyId: "company-1",
      environmentId: "env-1",
      lease: { providerLeaseId: "pilot-run-1-abcd1234", metadata: {} },
      command: "pwd",
      args: [],
      cwd: "/workspace/pilot",
      config: {
  ...
  it("routes bridge-channel execute calls through a dedicated session", async () => {
    ...
    expect(requestBodyAt()).toMatchObject({
      ...
      sessionId: "pilot-bridge",
      env: {
        KEEP_ME: "visible",
      },
    });
    expect(requestBodyAt().env).not.toHaveProperty("PILOT_SANDBOX_EXEC_CHANNEL");
  ...
  it("wraps realizeWorkspace bridge failures and forwards the issue header", async () => {
    ...
    await expect(plugin.definition.onEnvironmentRealizeWorkspace?.({
      ...
      lease: {
        providerLeaseId: "pilot-run-1-abcd1234",
        metadata: { remoteCwd: "/workspace/pilot" },
      },
      workspace: {
    ...
    })).rejects.toThrow("Failed to prepare Cloudflare sandbox workspace at /workspace/pilot: mkdir: permission denied");
