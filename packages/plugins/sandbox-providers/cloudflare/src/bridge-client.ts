...
function buildHeaders(config: CloudflareDriverConfig, extra: BridgeClientHeaders = {}): Headers {
  ...
  if (extra.environmentId) headers.set("x-pilot-Environment-Id", extra.environmentId);
  if (extra.runId) headers.set("x-pilot-Run-Id", extra.runId);
  if (extra.issueId) headers.set("x-pilot-Issue-Id", extra.issueId);
  return headers;
}
...
export function createCloudflareBridgeClient(options: BridgeClientOptions) {
  ...
  const apiPrefix = "/api/pilot-sandbox/v1";
