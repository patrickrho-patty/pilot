...
This Worker is the operator-facing bridge used by `@pilotai/plugin-cloudflare-sandbox`.

It exposes a small authenticated JSON API under `/api/pilot-sandbox/v1` and translates Pilot lease and command requests into Cloudflare Sandbox SDK calls.
...
- clean up timed-out sessions so Pilot does not inherit wedged background processes
...
After deploy, configure Pilot with:
