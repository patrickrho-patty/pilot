...
const PLUGIN_ID = "pilot.exe-dev-sandbox-provider";
...
const manifest: PilotPluginManifestV1 = {
  ...
  description:
    "Sandbox provider plugin that provisions exe.dev VMs as Pilot execution environments.",
  author: "Pilot",
  ...
  environmentDrivers: [
    {
      ...
      description:
        "Provisions exe.dev VMs through the HTTPS API, then runs commands over direct SSH for long-lived Pilot workloads.",
      configSchema: {
        type: "object",
        properties: {
          ...
          apiKey: {
            ...
            format: "secret-ref",
            description:
              "Paste your exe.dev API token, or pick a saved Pilot secret. Create one at exe.dev → Settings → API tokens with `/exec` scope (`new`, `ls`, `rm`).",
          },
          sshPrivateKey: {
            type: "string",
            format: "secret-ref",
          ...
          sshUser: {
            ...
            "x-pilot-advanced": true,
            "x-pilot-group": "SSH access",
          },
          sshIdentityFile: {
            type: "string",
            description:
              "Absolute path to a private key on the Pilot host. Used only when SSH Private Key is empty.",
            "x-pilot-advanced": true,
            "x-pilot-group": "SSH access",
          },
          sshPort: {
