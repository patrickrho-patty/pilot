# `@pilotai/plugin-exe-dev`

Published exe.dev sandbox provider plugin for Pilot.

This package lives in the Pilot monorepo, but it is intentionally excluded from the root `pnpm` workspace and shaped to publish and install like a standalone npm package. That lets operators install it from the Plugins page by package name without introducing root lockfile churn.
...
- When you save an environment, Pilot stores pasted API keys and pasted SSH private keys as company secrets.
...
- SSH access from the Pilot host to the resulting `*.exe.xyz` VMs.
