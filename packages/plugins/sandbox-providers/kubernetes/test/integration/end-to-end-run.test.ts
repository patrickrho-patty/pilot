 ...
 *        docker tag alpine:3.20 localhost/pilot-agent:latest
 *        kind load docker-image localhost/pilot-agent:latest --name pilot
 ...
 *      And a tini-bearing image pre-loaded (e.g. the same localhost/pilot-agent:latest
...
const NAMESPACE = "pilot-spike-e2e";
...
describe("plugin-kubernetes end-to-end", () => {
  ...
  it.runIf(process.env.RUN_K8S_INTEGRATION_TESTS === "1")(
    ...
    async () => {
      ...
      expect(lease.providerLeaseId).toMatch(/^pilot-/);
      ...
      expect(all).toContain("serviceaccount/pilot-tenant-sa");
      expect(all).toContain("role.rbac.authorization.k8s.io/pilot-tenant-role");
      expect(all).toContain("rolebinding.rbac.authorization.k8s.io/pilot-tenant-rb");
      expect(all).toContain("resourcequota/pilot-quota");
      expect(all).toContain("limitrange/pilot-limits");
      expect(all).toContain("networkpolicy.networking.k8s.io/pilot-deny-all");
      expect(all).toContain("networkpolicy.networking.k8s.io/pilot-egress-allow");
