...
describe("buildNetworkPolicyManifests", () => {
  ...
  it("produces a deny-all + egress allow pair", () => {
    ...
    expect(manifests[0].metadata.name).toBe("pilot-deny-all");
    expect(manifests[1].metadata.name).toBe("pilot-egress-allow");
  });
  ...
  it("egress allow includes kube-dns and pilot-server callback", () => {
    ...
    const pilotRule = rules.find((r: { to: { namespaceSelector?: { matchLabels?: Record<string, string> } }[] }) =>
      r.to.some((t) => t.namespaceSelector?.matchLabels?.["kubernetes.io/metadata.name"] === "pilot"),
    );
  ...
  it("uses pilot-server pod label selector for callback ingress to pilot ns", () => {
    const [, egress] = buildNetworkPolicyManifests(baseInput);
    const callbackRule = egress.spec.egress.find((r: { to: { podSelector?: { matchLabels?: Record<string, string> } }[] }) =>
      r.to.some((t) => t.podSelector?.matchLabels?.app === "pilot-server"),
    );
  ...
  it("builds a task-scoped allow policy without namespace-wide base rules", () => {
    const [, egress] = buildNetworkPolicyManifests({
      ...baseInput,
      name: "pilot-run-egress",
      podSelector: { "pilot-run-id": "run-123" },
      includeBaseRules: false,
      egressAllowFqdns: ["github.com", "pypi.org"],
      ownerReferences: [{ apiVersion: "batch/v1", kind: "Job", name: "pilot-run", uid: "uid-1" }],
    });

    expect(egress.metadata.name).toBe("pilot-run-egress");
    expect(egress.metadata.ownerReferences).toHaveLength(1);
    expect(egress.spec.podSelector.matchLabels).toEqual({ "pilot-run-id": "run-123" });
