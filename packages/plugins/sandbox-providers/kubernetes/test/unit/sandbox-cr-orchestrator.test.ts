...
describe("createSandboxCr", () => {
  it("calls custom.createNamespacedCustomObject with the correct params", async () => {
    ...
    const manifest = {
      ...
      kind: "Sandbox",
      metadata: { name: "pilot-abc", namespace: "pilot-acme" },
    };
    const result = await createSandboxCr(clients as never, "pilot-acme", manifest);
    expect(create).toHaveBeenCalledWith({
      group: SANDBOX_GROUP,
      version: SANDBOX_VERSION,
      namespace: "pilot-acme",
...
describe("getSandboxCrStatus", () => {
  it("maps phase=Ready to SandboxStatus.phase=Running with active=1", async () => {
    ...
    const status = await getSandboxCrStatus(clients as never, "ns", "pilot-abc");
...
describe("findPodForSandbox", () => {
  it("returns status.podName from the Sandbox CR when set", async () => {
    const get = vi.fn().mockResolvedValue(makeCr("Ready", "pilot-abc-pod-xyz"));
    const clients = {
      custom: { getNamespacedCustomObject: get },
      core: { readNamespacedPod: vi.fn(), listNamespacedPod: vi.fn() },
    };
    const podName = await findPodForSandbox(clients as never, "ns", "pilot-abc");
    expect(podName).toBe("pilot-abc-pod-xyz");
  ...
  it("resolves the pod by EXACT NAME when the controller names it after the sandbox (v0.4.x: pods carry only agents.x-k8s.io/sandbox-name-hash, never the full-name label)", async () => {
    ...
    const read = vi.fn().mockResolvedValue({
      metadata: { name: "pilot-abc", labels: { "agents.x-k8s.io/sandbox-name-hash": "1a2b3c" } },
    ...
    const podName = await findPodForSandbox(clients as never, "ns", "pilot-abc");
    expect(read).toHaveBeenCalledWith({ namespace: "ns", name: "pilot-abc" });
    expect(podName).toBe("pilot-abc");
  ...
  it("falls back to pod listing scoped by the unique sandbox-name label", async () => {
    ...
    const list = vi.fn().mockResolvedValue({
      items: [
        {
          metadata: { name: "pilot-abc-001", labels: { "agents.x-k8s.io/sandbox-name": "pilot-abc" } },
          status: { phase: "Running" },
        },
      ],
    });
    ...
    expect(list).toHaveBeenCalledWith(
      expect.objectContaining({ labelSelector: "agents.x-k8s.io/sandbox-name=pilot-abc" }),
    );
    expect(podName).toBe("pilot-abc-001");
  });
      ...
      items: [
        {
          ...
          metadata: { name: "pilot-abc-zzz", labels: { "agents.x-k8s.io/sandbox-name": "pilot-abc-zzz" } },
          status: { phase: "Running" },
        },
      ],
    });
  ...
  it("propagates non-404 errors from the exact-name pod GET instead of falling through", async () => {
    ...
    await expect(findPodForSandbox(clients as never, "ns", "pilot-abc")).rejects.toMatchObject({
      code: 403,
    });
    expect(list).not.toHaveBeenCalled();
  });
...
describe("deleteSandboxCr", () => {
  it("calls custom.deleteNamespacedCustomObject with Foreground propagation", async () => {
    ...
    await deleteSandboxCr(clients as never, "ns", "pilot-abc");
...
describe("waitForSandboxReady", () => {
  ...
  it("throws SandboxCrTimeoutError when deadline is exceeded", async () => {
    ...
    const clients = { custom: { getNamespacedCustomObject: get } };
    await expect(
      waitForSandboxReady(clients as never, "ns", "pilot-abc", {
        timeoutMs: 50,
        pollMs: 10,
      }),
