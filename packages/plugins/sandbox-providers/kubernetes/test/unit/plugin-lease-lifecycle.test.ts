...
function leaseMetadata(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    ...
    jobName: "pilot-abc",
    podName: "pilot-abc-pod",
    secretName: "pilot-abc-env",
    phase: "Pending",
    backend: "sandbox-cr",
    ...overrides,
  };
...
describe("onEnvironmentResumeLease", () => {
  ...
  it("returns a valid lease handle for a live sandbox-cr lease", async () => {
    h.clients = {
      custom: {
        getNamespacedCustomObject: vi.fn().mockResolvedValue(readySandboxCr("pilot-abc-pod")),
      },
      core: {
    ...
    const lease = await plugin.definition.onEnvironmentResumeLease!({
      ...
      environmentId: "env-1",
      config: CONFIG,
      providerLeaseId: "pilot-abc",
      leaseMetadata: leaseMetadata(),
    });

    expect(lease.providerLeaseId).toBe("pilot-abc");
  ...
  it("flags a resumed job-backend lease as native-sync-unsupported so the server keeps the base64 fallback", async () => {
    h.clients = {
      ...
      core: {
        listNamespacedPod: vi.fn().mockResolvedValue({
          items: [{ metadata: { name: "pilot-job-pod" }, status: { phase: "Running" } }],
        }),
      },
    };
    ...
    const lease = await plugin.definition.onEnvironmentResumeLease!({
      ...
      providerLeaseId: "pilot-job",
      leaseMetadata: leaseMetadata({ jobName: "pilot-job", backend: "job", podName: "pilot-job-pod" }),
    });

    expect(lease.providerLeaseId).toBe("pilot-job");
    expect(lease.metadata).toEqual(
      expect.objectContaining({
        backend: "job",
...
describe("onEnvironmentDestroyLease", () => {
  it("deletes the Sandbox CR, pod, and per-run Secret", async () => {
    ...
    expect(deleteCr).toHaveBeenCalledWith(
      expect.objectContaining({ namespace: "pilot-acme", name: "pilot-abc" }),
    );
  ...
  it("deletes the Job for job-backend leases", async () => {
    ...
    await plugin.definition.onEnvironmentDestroyLease!({
      ...
      config: { inCluster: true, backend: "job" },
      providerLeaseId: "pilot-job",
      leaseMetadata: leaseMetadata({ jobName: "pilot-job", backend: "job", podName: "pilot-job-pod", secretName: "pilot-job-env" }),
    });

    expect(deleteJob).toHaveBeenCalledWith(
      expect.objectContaining({ namespace: "pilot-acme", name: "pilot-job" }),
    );
    expect(deleteCr).not.toHaveBeenCalled();
  });
});
