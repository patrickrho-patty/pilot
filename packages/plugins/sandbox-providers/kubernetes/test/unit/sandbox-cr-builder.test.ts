...
const baseInput = {
  ...
  sandboxName: "pilot-01h00000000000000000000000",
  adapterType: "claude_local",
  image: "ghcr.io/pilotai/agent-runtime-claude:v1",
  envSecretName: "pilot-01h00000000000000000000000-env",
...
describe("buildSandboxCrManifest", () => {
  ...
  it("applies provided labels to CR metadata and pod template labels (with role=agent added)", () => {
    const cr = buildSandboxCrManifest(baseInput);
    expect(cr.metadata.labels["pilot-run-id"]).toBe("r1");
    expect(
      cr.spec.podTemplate.metadata.labels["pilot-run-id"],
    ).toBe("r1");
    expect(cr.spec.podTemplate.metadata.labels["pilot-role"]).toBe(
      "agent",
    );
  });
