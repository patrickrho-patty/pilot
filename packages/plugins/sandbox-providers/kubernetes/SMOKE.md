# Manual smoke test — `@pilotai/plugin-kubernetes`

Manual sanity check that the plugin works end-to-end against a real
pilot-server instance and a real Kubernetes cluster (kind for local
...
- A running kind cluster:
  ...
  ```
- `kubectl --context kind-pilot get nodes` returns a node in `Ready` state.
...
### 2. Start pilot-server in dev mode

In a separate terminal:

```bash
cd /path/to/pilot
export PILOT_HOME=/tmp/pilot-smoke
export PILOT_INSTANCE_ID=smoke
export PILOT_DEPLOYMENT_MODE=local_trusted
pnpm --filter @pilotai/server dev
```
...
npx pilotai plugin install \
  --local /path/to/pilot/packages/plugins/sandbox-providers/kubernetes \
  --api-base http://127.0.0.1:3100
```

Expected: `✓ Installed pilot.kubernetes-sandbox-provider v0.1.0 (ready)`.
...
curl -s -X POST -H "Content-Type: application/json" \
  -d "{
    ...
    \"config\": {
      ...
      \"imageAllowList\": [\"ghcr.io/pilotai/agent-runtime-claude:v1\"]
    }
  }" \
...
(`pilot-smoke`). On first probe the namespace may not yet exist —
...
1. `ensureTenant` — provision the `pilot-smoke` namespace, SA, Role,
...
```bash
kubectl --context kind-pilot get namespace pilot-smoke
kubectl --context kind-pilot get all,networkpolicy,resourcequota,limitrange,sa,role,rolebinding -n pilot-smoke
```

Expected:

- Namespace `pilot-smoke` exists with PSS labels
  (`pod-security.kubernetes.io/enforce=restricted`)
- ServiceAccount `pilot-tenant-sa`
- Role `pilot-tenant-role`, RoleBinding `pilot-tenant-rb`
- ResourceQuota `pilot-quota`, LimitRange `pilot-limits`
- NetworkPolicies `pilot-deny-all` + `pilot-egress-allow`
- Job `pilot-{ulid}` and its child Pod
- Secret `pilot-{ulid}-env` with `ownerReferences` pointing at the Job

### 8. Tear down

```bash
kubectl --context kind-pilot delete namespace pilot-smoke
kill %1  # pilot-server
```
...
- Output of `kubectl get all -n pilot-smoke` after step 6
