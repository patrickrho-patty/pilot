...
// runtime's base64 fallback stages to `<path>.pilot-upload`; the native
...
async function withHostTempDir<T>(fn: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-k8s-sync-"));
  try {
    return await fn(dir);
  } finally {
 ...
 *
 * Also defines `_pilot_confine_ancestor <path>`: it walks up to the DEEPEST already-
 * existing ancestor of `<path>`, canonicalizes it, and confines it inside
 * `_pilot_root` (exit 42 on escape). `mkdir -p` follows an existing symlink ancestor,
...
function canonicalizerPreamble(quotedRoot: string): string[] {
  return [
    ...
    `_pilot_root=$(_pilot_resolve ${quotedRoot}) || { echo "cannot resolve root" >&2; exit 41; };`,
    '_pilot_confine_ancestor() {',
    ...
    '  _pilot_a=$1;',
    '  while [ ! -e "$_pilot_a" ] && [ ! -L "$_pilot_a" ]; do',
    '    _pilot_p=$(dirname "$_pilot_a");',
    '    [ "$_pilot_p" = "$_pilot_a" ] && break;',
    '    _pilot_a=$_pilot_p;',
    '  done;',
    '  _pilot_ar=$(_pilot_resolve "$_pilot_a") || return 42;',
    '  case "$_pilot_ar/" in "$_pilot_root"/*) return 0 ;; *) return 42 ;; esac;',
    '};',
  ];
}
...
async function syncInFileMappings(input: {
  ...
  return withHostTempDir(async (tmp) => {
    ...
    for (const dir of parentDirs) {
      script.push(
        `_pilot_confine_ancestor ${shQuote(dir)} || { echo "ESCAPE" >&2; exit 42; };`,
    ...
    for (const dir of parentDirs) {
      script.push(
        `_pilot_d=$(_pilot_resolve ${shQuote(dir)}) || { echo "ESCAPE" >&2; exit 42; };`,
        `case "$_pilot_d/" in "$_pilot_root"/*) : ;; *) echo "ESCAPE" >&2; exit 42 ;; esac;`,
      );
    }
    ...
    mappings.forEach((mapping, i) => {
      ...
      script.push(
        `_pilot_tgt_dir=$(_pilot_resolve ${shQuote(parentDir)}) || { echo "ESCAPE" >&2; exit 42; };`,
        `case "$_pilot_tgt_dir/" in "$_pilot_root"/*) : ;; *) echo "ESCAPE" >&2; exit 42 ;; esac;`,
        `exec 8<"$_pilot_tgt_dir" || { echo "open failed" >&2; exit 47; };`,
        `_pilot_fd_dir=$(_pilot_resolve /proc/self/fd/8) || { echo "ESCAPE" >&2; exit 42; };`,
        `case "$_pilot_fd_dir/" in "$_pilot_root"/*) : ;; *) echo "ESCAPE" >&2; exit 42 ;; esac;`,
...
async function syncInDirectoryMapping(input: {
  ...
  return withHostTempDir(async (tmp) => {
    ...
    const script = [
      ...
      `_pilot_confine_ancestor ${shQuote(target)} || { echo "ESCAPE" >&2; exit 42; };`,
      ...
      `_pilot_real=$(_pilot_resolve ${shQuote(target)}) || { echo "ESCAPE" >&2; exit 42; };`,
      `case "$_pilot_real/" in "$_pilot_root"/*) : ;; *) echo "ESCAPE" >&2; exit 42 ;; esac;`,
      `exec 9<"$_pilot_real" || { echo "open failed" >&2; exit 46; };`,
      `_pilot_fd_real=$(_pilot_resolve /proc/self/fd/9) || { echo "ESCAPE" >&2; exit 42; };`,
      `case "$_pilot_fd_real/" in "$_pilot_root"/*) : ;; *) echo "ESCAPE" >&2; exit 42 ;; esac;`,
...
async function runPostUploadCommands(input: {
  ...
  for (const command of commands) {
    ...
    const wrapper = [
      ...canonicalizerPreamble(shQuote(remoteDir)),
      `_pilot_real=$(_pilot_resolve "$1") || { echo "ESCAPE" >&2; exit 42; };`,
      `case "$_pilot_real/" in "$_pilot_root"/*) : ;; *) echo "ESCAPE" >&2; exit 42 ;; esac;`,
      `exec 9<"$_pilot_real" || { echo "open failed" >&2; exit 46; };`,
      `_pilot_fd_real=$(_pilot_resolve /proc/self/fd/9) || { echo "ESCAPE" >&2; exit 42; };`,
      `case "$_pilot_fd_real/" in "$_pilot_root"/*) : ;; *) echo "ESCAPE" >&2; exit 42 ;; esac;`,
      `cd /proc/self/fd/9 || { echo "cd failed" >&2; exit 46; };`,
      `exec /bin/sh -c "$2" pilot-post-upload;`,
    ].join("\n");
    const commandTimeoutMs = command.timeoutMs ?? timeoutMs;
    // Positional args: $0=pilot-post-upload, $1=cwd, $2=the verbatim command string.
    const result = await exec(["/bin/sh", "-c", wrapper, "pilot-post-upload", cwd, command.command], {
...
async function syncOutFileMappings(input: {
  ...
  return withHostTempDir(async (tmp) => {
    ...
    mappings.forEach((mapping, i) => {
      ...
      script.push(
        `_pilot_real=$(_pilot_resolve ${shQuote(mapping.sourcePath)}) || { echo "ESCAPE" >&2; exit 42; };`,
        `case "$_pilot_real/" in "$_pilot_root"/*) : ;; *) echo "ESCAPE" >&2; exit 42 ;; esac;`,
        ...
        // we opened — if the sandbox swapped `$_pilot_real` for a symlink between the
        ...
        `exec 7<"$_pilot_real" || { echo "REPLACED" >&2; exit 45; };`,
        `_pilot_fd_real=$(_pilot_resolve /proc/self/fd/7) || { echo "ESCAPE" >&2; exit 42; };`,
        `case "$_pilot_fd_real/" in "$_pilot_root"/*) : ;; *) echo "ESCAPE" >&2; exit 42 ;; esac;`,
