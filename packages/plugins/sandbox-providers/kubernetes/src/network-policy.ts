...
export function buildNetworkPolicyManifests(input: BuildNetworkPolicyInput): Record<string, unknown>[] {
  ...
  const egressAllow: Record<string, unknown> = {
    ...
    spec: {
      podSelector: { matchLabels: input.podSelector ?? { "pilot-role": "agent" } },
      policyTypes: ["Egress"],
      egress: [
        ...
        ...(input.includeBaseRules === false ? [] : [{
          to: [
            {
              namespaceSelector: { matchLabels: { "kubernetes.io/metadata.name": input.pilotServerNamespace } },
              podSelector: { matchLabels: { app: "pilot-server" } },
            },
          ],
  ...
  (egressAllow.metadata as Record<string, unknown>).name = input.name ?? "pilot-egress-allow";
