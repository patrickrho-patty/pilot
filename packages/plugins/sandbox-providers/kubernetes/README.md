# @pilotai/plugin-kubernetes (alpha)
...
```bash
pilotai plugin install @pilotai/plugin-kubernetes
```

Or, for local development:

```bash
pilotai plugin install --local /path/to/pilot/packages/plugins/sandbox-providers/kubernetes
```
...
```
Sandbox CR         pilot-{ulid}                       (agents.x-k8s.io/v1alpha1; explicit delete on release)
Pod                pilot-{ulid}-{podSuffix}           (managed by Sandbox controller; torn down on CR delete)
Secret             pilot-{ulid}-env                   (owned by Sandbox CR; cascade-deleted)
```

For each agent run (job backend):

```
Job                pilot-{ulid}                       (backoffLimit: 0, ttlSecondsAfterFinished from config)
Pod                pilot-{ulid}-{podSuffix}           (owned by Job; cascade-deleted)
Secret             pilot-{ulid}-env                   (owned by Job; cascade-deleted)
```
...
- `readOnlyRootFilesystem: true` with explicit `emptyDir` mounts for `/workspace`, `/home/pilot`, `/home/pilot/.cache`, `/tmp`
