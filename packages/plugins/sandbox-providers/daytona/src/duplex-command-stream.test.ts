...
const GATEWAY = ["node", "/pilot/gateway.mjs"];
...
const GATEWAY_QUOTED = "'node' '/pilot/gateway.mjs'";
...
describe("buildDuplexChannelLaunchWrapper", () => {
  ...
  it("quotes each command argument that holds shell metacharacters", () => {
    ...
    expect(wrapper).toContain(`exec 'node' '/pilot/gateway.mjs' '; rm -rf ~'`);
    expect(wrapper).not.toContain("; rm -rf ~;");
    expect(wrapper).not.toContain("exec node /pilot/gateway.mjs ; rm -rf ~");
  });
...
describe("openDaytonaDuplexChannelSession", () => {
  ...
  it("redirects diagnostics to the given file path", async () => {
    ...
    await openDaytonaDuplexChannelSession(process, GATEWAY, {
      diagnosticsPath: "/tmp/pilot-duplex-fixed.log",
    });

    expect(process.handle?.inputs[0]).toContain("2>'/tmp/pilot-duplex-fixed.log'");
  });
  ...
  it("defaults the diagnostics path under /tmp when the caller gives none", async () => {
    ...
    expect(process.handle?.inputs[0]).toMatch(/2>'\/tmp\/pilot-duplex-[0-9a-f-]+\.log'/);
  });
  ...
  it("passes the working directory to the pseudo-terminal", async () => {
    ...
    await openDaytonaDuplexChannelSession(process, GATEWAY, { cwd: "/pilot-workspace" });

    expect(process.createOptions?.cwd).toBe("/pilot-workspace");
  });
});
...
describe("createDaytonaDuplexChannelSessionOpener", () => {
  it("opens a session for the command on each call", async () => {
    const process = createFakeProcess();
    const opener = createDaytonaDuplexChannelSessionOpener(process, { cwd: "/pilot-workspace" });
