...
// interface in `@pilotai/adapter-utils`, so the transport factory
 ...
 * `SetupTokenPtySession` interface in `@pilotai/adapter-utils`, so the
...
export async function openDaytonaSetupTokenPtySession(
  ...
  const handle = await process.createPty({
    id: `pilot-setup-token-${randomUUID()}`,
 ...
 * `createSetupTokenPtyTransport` factory from `@pilotai/adapter-utils` and
