...
import type { PluginContext } from "@pilotai/plugin-sdk";
...
function createMockSandbox(overrides: {
  ...
  return {
    ...
    name: overrides.name ?? "pilot-sandbox",
...
describe("Daytona sandbox provider plugin", () => {
  ...
  it("opens a duplex channel, forwards a host write, and closes it on lease release", async () => {
    ...
    try {
      ...
      const open = await plugin.definition.onDuplexChannelOpen?.({
        ...
        command: ["node", "/pilot/gateway.mjs"],
      });
      ...
      expect(inputs[0]).toContain("exec 'node' '/pilot/gateway.mjs'");
      expect(inputs[0]).toMatch(/2>'\/tmp\/pilot-duplex-.+\.log'/);
  ...
  it("probes by creating and then deleting a sandbox", async () => {
    ...
    expect(sandbox.fs.createFolder).toHaveBeenCalledWith("/home/daytona/pilot-workspace", "755");
    ...
    expect(result).toMatchObject({
      ...
      metadata: {
        ...
        remoteCwd: "/home/daytona/pilot-workspace",
      },
    });
  });
  ...
  it("acquires a lease from a created sandbox", async () => {
    ...
    expect(lease).toMatchObject({
      ...
      metadata: {
        ...
        reuseLease: true,
        workspaceSentinel: {
          path: "/home/daytona/pilot-workspace/.pilot-runtime/reusable-sandbox-lease.json",
          result: "written",
        },
      },
    });
    expect(sandbox.fs.createFolder).toHaveBeenCalledWith(
      "/home/daytona/pilot-workspace/.pilot-runtime",
      "755",
    );
    expect(sandbox.fs.uploadFile).toHaveBeenCalledWith(
      expect.any(Buffer),
      "/home/daytona/pilot-workspace/.pilot-runtime/reusable-sandbox-lease.json",
      300,
    );
  });
  ...
  it("starts an interactive setup sandbox with redacted metadata and one-time SSH payload", async () => {
    ...
    expect(createParams).toMatchObject({
      ...
      labels: {
        "pilot-provider": "daytona",
        "pilot-setup-session-id": "setup-1",
        "pilot-purpose": "interactive_setup",
      },
    });
  ...
  it("starts interactive setup from an image source when the environment is image-backed", async () => {
    ...
    expect(createParams).toMatchObject({
      ...
      labels: {
        "pilot-provider": "daytona",
        "pilot-setup-session-id": "setup-image-1",
        "pilot-purpose": "interactive_setup",
      },
    });
  ...
  it("captures a Daytona snapshot from a live setup sandbox with redacted metadata", async () => {
    ...
    expect(sandbox._experimental_createSnapshot).toHaveBeenCalledWith("pilot-env-1", 120);
    expect(result).toMatchObject({
      templateKind: "snapshot",
      templateRef: "pilot-env-1",
      metadata: {
        provider: "daytona",
  ...
  describe("session model lifecycle (per-lease session store)", () => {
    ...
    it("creates one session on the first execute and reuses it on the next", async () => {
      ...
      expect(sessionId).toMatch(/^pilot-/);
    });
    ...
    it("emits a session.open span on create and a session.close span on delete", async () => {
      ...
      try {
        ...
        expect(setup!.attributes["pilot.sandbox.startup.provider"]).toBe("daytona");
        ...
        expect(teardown!.attributes["pilot.sandbox.startup.provider"]).toBe("daytona");
      } finally {
        restore();
      }
    });
  ...
  it("stages stdin in the sandbox filesystem when execution needs redirected input", async () => {
    ...
    expect(sandbox.fs.uploadFile).toHaveBeenCalledWith(
      ...
      expect.stringMatching(/^\/tmp\/pilot-stdin-/),
      1,
    );
    ...
    expect(command).not.toMatch(/nvm\.sh/);
    expect(command).toMatch(/&& cd '\/workspace'/);
    expect(command).toMatch(/env .* 'cat' < '\/tmp\/pilot-stdin-/);
    expect(command).not.toMatch(/(?:^|&& )exec /);
    expect(sandbox.fs.deleteFile).toHaveBeenCalledWith(expect.stringMatching(/^\/tmp\/pilot-stdin-/));
    expect(result).toMatchObject({
      exitCode: 0,
      timedOut: false,
    });
  ...
  it("runs the one-shot command plain with no bwrap or su wrapper", async () => {
    ...
    await plugin.definition.onEnvironmentExecute?.({
      ...
      config: { timeoutMs: 300000, reuseLease: false },
      bypassSession: true,
      lease: { providerLeaseId: "sandbox-123", metadata: { remoteCwd: "/home/daytona/pilot-workspace" } },
      command: "printf",
      args: ["hello"],
      cwd: "/workspace",
      timeoutMs: 1000,
  ...
  describe("started-sandbox handle cache", () => {
    ...
    it("waits for an in-flight syncIn before interactive cancel cleanup starts", async () => {
      ...
      const remoteDir = "/home/daytona/pilot-workspace";
    ...
    it("realizes the workspace from the acquire-seeded handle without a client.get", async () => {
      ...
      const realize = await plugin.definition.onEnvironmentRealizeWorkspace?.({
        ...
        workspace: { remotePath: "/home/daytona/pilot-workspace" },
        config,
      });
      ...
      expect(sandbox.fs.createFolder).toHaveBeenCalledWith("/home/daytona/pilot-workspace", "755");
      expect(realize?.cwd).toBe("/home/daytona/pilot-workspace");
    });
  });
});

describe("daytona native file-sync hooks", () => {
  const REMOTE_DIR = "/home/daytona/pilot-workspace";
  ...
  it("records the writablePath destination of a staging-tar rw mapping, not the staging parent", async () => {
          ...
          files: [
            {
              ...
              sourcePath: source,
              targetPath: `${REMOTE_DIR}/.pilot-runtime/workspace-upload.tar`,
              kind: "file" as const,
              access: "rw" as const,
    ...
    expect(recorded).not.toContain(`${REMOTE_DIR}/.pilot-runtime`);
  });
      ...
      operations: [
        {
          ...
          files: [
            {
              sourcePath: roSource,
              targetPath: `${REMOTE_DIR}/.pilot-runtime/project-proj-first`,
            ...
            },
            {
              ...
              targetPath: `${REMOTE_DIR}/.pilot-runtime/default-upload.tar`,
              kind: "file" as const,
            },
          ],
        },
  ...
  it("syncIn coalesces file mappings into one uploadFiles batch to reserved temp destinations, then one batched mv, applying secret mode via setFilePermissions before the rename", async () => {
    ...
    for (const upload of uploads) {
      expect(path.posix.basename(upload.destination)).toMatch(/^\.pilot-upload-/);
    ...
    expect(path.posix.basename(permPath)).toMatch(/^\.pilot-upload-/);
    ...
    expect(mvCommand).toContain("_pilot_resolve");
    ...
    expect(mvCommand).toContain('exec 8<"$_pilot_tgt_dir"');
    expect(mvCommand).toContain("_pilot_fd_dir=$(_pilot_resolve /proc/self/fd/8)");
  ...
  it("opens a transfer span with the guard round-trip count around the bulk file upload", async () => {
    ...
    expect(transfer!.attributes["pilot.sandbox.startup.transfer.guard.count"]).toBe(2);
    expect(transfer!.attributes["pilot.sandbox.startup.provider"]).toBe("daytona");
    expect(typeof transfer!.attributes["pilot.sandbox.startup.transfer.wall_ms"]).toBe("number");
  ...
  it("marks the inbound transfer span with the inbound direction attribute", async () => {
    ...
    const transfer = spans.find((span) => span.name === "transfer");
    expect(transfer).toBeDefined();
    expect(transfer!.attributes["pilot.sandbox.startup.transfer.direction"]).toBe("inbound");
  });
  ...
  it("marks the outbound transfer span with the outbound direction attribute", async () => {
    ...
    const transfer = spans.find((span) => span.name === "transfer");
    expect(transfer).toBeDefined();
    expect(transfer!.attributes["pilot.sandbox.startup.transfer.direction"]).toBe("outbound");
  });
  ...
  it("opens a pack span and a transfer span around a directory mapping sync", async () => {
        ...
        operations: [
          {
            ...
            files: [
              { sourcePath: sourceDir, targetPath: `${REMOTE_DIR}/.pilot-runtime/assets`, kind: "directory" },
            ],
          },
        ],
      });
    ...
    expect(typeof pack!.attributes["pilot.sandbox.startup.pack.wall_ms"]).toBe("number");
  ...
  it("opens ensureDirectory, checkSymlinkEscape, transfer, promote spans in call order for a file-mapping sync", async () => {
    ...
    for (const span of spans) {
      ...
      expect(span.attributes["pilot.sandbox.startup.provider"]).toBe("daytona");
      ...
      if (span.name !== "transfer") {
        expect(span.attributes["pilot.sandbox.startup.ensureDirectory.wall_ms"]).toBeUndefined();
        expect(span.attributes["pilot.sandbox.startup.promote.wall_ms"]).toBeUndefined();
        expect(span.attributes["pilot.sandbox.startup.checkSymlinkEscape.wall_ms"]).toBeUndefined();
      }
    }
  });
  ...
  it("records a pack span for a traced directory mapping", async () => {
    ...
    expect(pack).toBeDefined();
    expect(pack!.ended).toBe(true);
    expect(pack!.attributes["pilot.sandbox.startup.provider"]).toBe("daytona");
  });
  ...
  it("opens a checkSymlinkEscape span and a postUploadCommand span in call order for a post-upload command with a working directory", async () => {
    ...
    expect(provision!.attributes["pilot.sandbox.startup.provider"]).toBe("daytona");
  });
  ...
  it("syncIn tars a directory mapping host-side honoring excludes and the followSymlinks flag, then extracts it in-sandbox via a single quoted tar command", async () => {
          ...
          files: [
            {
              ...
              targetPath: `${REMOTE_DIR}/.pilot-runtime/assets`,
              kind: "directory",
              exclude: ["*.log"],
            },
          ],
    ...
    expect(path.posix.basename(uploads[0].destination)).toMatch(/^\.pilot-upload-.*\.tar$/);
    ...
    const mkdirCall = sandbox.process.executeCommand.mock.calls.find(
      ([cmd]) =>
        ...
        String(cmd).includes(`'${REMOTE_DIR}/.pilot-runtime/assets'`) &&
    ...
    const inboundGuardCall = sandbox.process.executeCommand.mock.calls.find(([cmd]) =>
      String(cmd).includes("_pilot_resolve"),
    );
    ...
    expect(extractCommand).toContain("_pilot_resolve");
    expect(extractCommand).toContain(".pilot-runtime/assets");
    expect(extractCommand).toContain("tar -xf");
    expect(extractCommand).toContain('exec 9<"$_pilot_real"');
    expect(extractCommand).toContain("_pilot_fd_real=$(_pilot_resolve /proc/self/fd/9)");
    expect(extractCommand).toContain("-C /proc/self/fd/9");
    expect(extractCommand).toMatch(/rm -f .*\.pilot-upload-.*\.tar/);
  });
  ...
  it("syncOut reads all file mappings via one downloadFiles batch, writes each to its host target, and returns per-operation counts", async () => {
    ...
    for (const req of requests) {
      expect(path.basename(req.destination)).toMatch(/^\.pilot-upload-/);
      ...
      expect(path.posix.basename(req.source)).toMatch(/^\.pilot-upload-/);
    }
  ...
  it("syncOut snapshot guard re-checks the resolved source is a non-symlink regular file immediately before copying (validation→copy TOCTOU)", async () => {
    ...
    const guardCall = sandbox.process.executeCommand.mock.calls.find(
      ([cmd]) => String(cmd).includes("_pilot_resolve") && String(cmd).includes("cp --"),
    );
    expect(guardCall).toBeDefined();
    const guardCommand = String(guardCall?.[0]);
    expect(guardCommand).toContain('[ -L "$_pilot_real" ]');
    expect(guardCommand).toContain('[ -f "$_pilot_real" ]');
    const noFollowIdx = guardCommand.indexOf('[ -L "$_pilot_real" ]');
    const copyIdx = guardCommand.indexOf('cp -- "$_pilot_real"');
  ...
  it("syncOut rejects an outbound source whose in-sandbox realpath escapes the workspace remote dir, before any download", async () => {
    ...
    sandbox.process.executeCommand.mockImplementation(async (command: string) => {
      if (command.includes("_pilot_resolve")) {
  ...
  it("syncIn rejects a file mapping whose in-sandbox target parent resolves outside the remote dir (symlinked-parent escape), before uploading", async () => {
    ...
    // report the escape exit (42) for the `_pilot_resolve` probe, green otherwise.
    sandbox.process.executeCommand.mockImplementation(async (command: string) => {
      if (command.includes("_pilot_resolve")) {
  ...
  it("syncIn sweeps staged temps when the batched rename fails mid-promotion", async () => {
    ...
    // `.pilot-upload-*` temps that the error path must sweep with `rm -f`.
    sandbox.process.executeCommand.mockImplementation(async (command: string) => {
    ...
    const cleanupCall = sandbox.process.executeCommand.mock.calls.find(
      ([cmd]) => String(cmd).includes("rm -f") && String(cmd).includes(".pilot-upload-"),
    );
  ...
  it("stages a merged git-workspace operation as one uploadFiles batch and one rename exec, both extracts in order", async () => {
    ...
    const runtimeDir = `${REMOTE_DIR}/.pilot-runtime/adapter`;
  ...
  it("rejects a post-upload command whose cwd resolves outside the root via a symlink (realpath guard, C2)", async () => {
    ...
    sandbox.process.executeCommand.mockImplementation(async (command: string) => {
      if (command.includes("_pilot_resolve") && command.includes(cwd)) {
  ...
  it("runs two concurrent inbound syncIn calls with separate reserved scratch names", async () => {
    ...
    for (const destination of destinations) {
      expect(path.posix.basename(destination)).toMatch(/^\.pilot-upload-/);
  ...
  it("runs two concurrent outbound syncOut calls that both reach downloadFiles before either opens", async () => {
    ...
    for (const source of sources) {
      ...
      expect(path.posix.basename(source)).toMatch(/^\.pilot-upload-/);
    }
  ...
  it("opens a transfer span with the guard round-trip count around the bulk file download", async () => {
    ...
    expect(transfer!.attributes["pilot.sandbox.startup.transfer.guard.count"]).toBe(1);
    expect(transfer!.attributes["pilot.sandbox.startup.provider"]).toBe("daytona");
    expect(typeof transfer!.attributes["pilot.sandbox.startup.transfer.wall_ms"]).toBe("number");
  });
