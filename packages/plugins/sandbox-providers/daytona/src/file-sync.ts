...
const SPAN_ATTR_PREFIX = "pilot.sandbox.startup.";
const SPAN_ATTR = {
...
// The runtime's base64 fallback stages to `<path>.pilot-upload`; the native
...
const SCRATCH_PREFIX = ".pilot-upload";
...
async function withHostTempDir<T>(fn: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-daytona-sync-"));
  try {
    return await fn(dir);
  } finally {
...
/**
 * POSIX-sh preamble defining a `_pilot_resolve` canonicalizer (prefer `realpath`,
 * fall back to `readlink -f`; fail closed with exit 40 if neither exists so the
 * host-side lexical check is never the only line of defense) and `_pilot_root` =
...
function canonicalizerPreamble(quotedRoot: string): string[] {
  return [
    'if command -v realpath >/dev/null 2>&1; then _pilot_resolve() { realpath -- "$1"; };',
    'elif command -v readlink >/dev/null 2>&1; then _pilot_resolve() { readlink -f -- "$1"; };',
    'else echo "no path canonicalizer available"; exit 40; fi;',
    `_pilot_root=$(_pilot_resolve ${quotedRoot}) || { echo "cannot resolve root"; exit 41; };`,
  ];
}
...
async function assertSandboxPathsConfined(input: {
  ...
  const script = [
    ...
    `for _pilot_p in ${quotedPaths}; do`,
    '  _pilot_real=$(_pilot_resolve "$_pilot_p") || { echo "ESCAPE:$_pilot_p"; exit 42; };',
    '  case "$_pilot_real/" in "$_pilot_root"/*) : ;; *) echo "ESCAPE:$_pilot_p"; exit 42 ;; esac;',
    "done",
  ].join("\n");
 ...
 *  - validation→copy: `_pilot_real` is a canonical path, so a `[ -L ]`/`[ -f ]`
...
async function snapshotOutboundFileSources(input: {
  ...
  sources.forEach((source, index) => {
    ...
    lines.push(
      `_pilot_real=$(_pilot_resolve ${quotedSource}) || { echo "ESCAPE"; exit 42; };`,
      `case "$_pilot_real/" in "$_pilot_root"/*) : ;; *) echo "ESCAPE"; exit 42 ;; esac;`,
      ...
      `[ -L "$_pilot_real" ] && { echo "REPLACED"; exit 44; };`,
      `[ -f "$_pilot_real" ] || { echo "NOTREG"; exit 45; };`,
      ...
      `cp -- "$_pilot_real" ${quotedSnapshot} || { echo "snapshot copy failed"; exit 43; };`,
    );
  });
 ...
 * so a failed transfer never accumulates `.pilot-upload-*` scratch in the
...
async function syncInFileMappings(input: {
  ...
  // retry never accumulates stale `.pilot-upload-*` scratch.
  try {
    ...
    // are closed:
    //  - check→open (ancestor swap): `mv "$_pilot_tgt_dir"/<base>` would re-walk the
    ...
    for (const rename of renames) {
      ...
      renameScript.push(
        `_pilot_tgt_dir=$(_pilot_resolve ${shellQuote(parentDir)}) || { echo "ESCAPE"; exit 42; };`,
        `case "$_pilot_tgt_dir/" in "$_pilot_root"/*) : ;; *) echo "ESCAPE"; exit 42 ;; esac;`,
        `exec 8<"$_pilot_tgt_dir" || { echo "open failed"; exit 47; };`,
        `_pilot_fd_dir=$(_pilot_resolve /proc/self/fd/8) || { echo "ESCAPE"; exit 42; };`,
        `case "$_pilot_fd_dir/" in "$_pilot_root"/*) : ;; *) echo "ESCAPE"; exit 42 ;; esac;`,
...
async function syncInDirectoryMapping(input: {
  ...
  return withHostTempDir(async (tmp) => {
    ...
    // `.pilot-upload-*` entry. The extract script below removes it only on
    // success. On an upload or extract failure the scratch tar can remain, and the
    // runtime workspace wipe preserves every `.pilot-upload-*` entry, so a
    ...
    try {
      ...
      // an OPEN directory inode rather than a path string. `exec 9<"$_pilot_real"` itself
      // walks every ancestor of `$_pilot_real` during the `open()` syscall, so a sandbox
      // process that swaps an ancestor component for a symlink AFTER `_pilot_resolve`
      ...
      // — the pinned inode's own path — and confirm it is still inside `$_pilot_root`
      ...
      // redirect the write. (The initial `case` on `$_pilot_real` still fails fast on a
      // pre-open escape; the fd re-verify is what makes the guarantee race-free.)
      const extractScript = [
        ...canonicalizerPreamble(shellQuote(remoteDir)),
        `_pilot_real=$(_pilot_resolve ${shellQuote(mapping.targetPath)}) || { echo "ESCAPE"; exit 42; };`,
        `case "$_pilot_real/" in "$_pilot_root"/*) : ;; *) echo "ESCAPE"; exit 42 ;; esac;`,
        `exec 9<"$_pilot_real" || { echo "open failed"; exit 46; };`,
        `_pilot_fd_real=$(_pilot_resolve /proc/self/fd/9) || { echo "ESCAPE"; exit 42; };`,
        `case "$_pilot_fd_real/" in "$_pilot_root"/*) : ;; *) echo "ESCAPE"; exit 42 ;; esac;`,
