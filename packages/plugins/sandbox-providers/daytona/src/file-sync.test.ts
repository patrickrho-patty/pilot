...
import type { PluginSyncOperation } from "@pilotai/plugin-sdk";
...
describe("daytona file-sync inbound scratch cleanup", () => {
  ...
  it("removes the reserved scratch tar when a directory extraction fails", async () => {
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-daytona-scratch-"));
    ...
    const targetPath = "/workspace/.pilot-runtime/test-adapter/project-abc";
    ...
    // root. Its name carries the reserved `.pilot-upload-` prefix.
    expect(uploadedDestinations).toHaveLength(1);
    const scratchTar = uploadedDestinations[0];
    expect(scratchTar).toContain(".pilot-upload-");
