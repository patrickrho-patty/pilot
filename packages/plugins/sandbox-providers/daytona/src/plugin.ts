...
import { definePlugin, NOOP_PLUGIN_TRACER } from "@pilotai/plugin-sdk";
import type {
  PluginContext,
  PluginTracer,
...
// `createSetupTokenPtyTransport` factory from `@pilotai/adapter-utils` to
...
const WORKSPACE_SENTINEL_RELATIVE_PATH = ".pilot-runtime/reusable-sandbox-lease.json";
...
function buildSandboxLabels(input: {
  ...
  return {
    ...
    "pilot-environment-id": input.environmentId,
    "pilot-reuse-lease": input.reuseLease ? "true" : "false",
    ...(input.runId ? { "pilot-run-id": input.runId } : {}),
    ...(input.setupSessionId ? { "pilot-setup-session-id": input.setupSessionId } : {}),
    ...(input.purpose ? { "pilot-purpose": input.purpose } : {}),
  };
}
...
async function resolveSandboxWorkingDirectory(sandbox: Sandbox): Promise<string> {
  const root = (await sandbox.getWorkDir())?.trim()
    ...
    || "/home/daytona";
  const remoteCwd = path.posix.join(root, "pilot-workspace");
...
async function executeOneShot(
  ...
  const stdinPath = params.stdin != null ? `/tmp/pilot-stdin-${randomUUID()}` : null;
...
const plugin = definePlugin({
  ...
  async onEnvironmentRealizeWorkspace(
    ...
    const remoteCwd =
      ...
      params.lease.metadata.remoteCwd.trim().length > 0
        ...
        : params.workspace.remotePath ?? params.workspace.localPath ?? "/pilot-workspace";
  ...
  async onEnvironmentCaptureTemplate(
    ...
    const templateRef = sanitizeSnapshotName(
      ...
      `pilot-${params.environmentId}-${randomUUID().slice(0, 8)}`,
    );
