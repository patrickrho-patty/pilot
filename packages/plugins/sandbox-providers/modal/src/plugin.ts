...
const DEFAULT_WORKDIR = "/workspace/pilot";
...
function warnIfUnsupportedNode(logger: { warn: (msg: string) => void } | undefined): void {
  ...
  if (Number.isFinite(major) && major < 24) {
    const message = `Modal sandbox provider is running on Node ${process.versions.node}; Pilot requires Node 24+. Upgrade the host runtime before using this plugin.`;
    logger?.warn(message);
  }
}
  ...
  async onEnvironmentProbe(
    ...
    try {
      ...
      try {
        ...
        const proc = await sandbox.exec(["sh", "-lc", "printf pilot-probe"]);
        const { stdout, exitCode } = await readProcessStreams(proc);
        if (exitCode !== 0 || stdout.trim() !== "pilot-probe") {
          return {
            ok: false,
  ...
  async onEnvironmentExecute(
    ...
    try {
      ...
      const stdinPath = params.stdin != null
        ? `/tmp/pilot-stdin-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`
        : null;
      try {
