...
describe("Novita sandbox provider plugin", () => {
  ...
  it("parses defaults", () => {
    expect(parseNovitaDriverConfig({})).toMatchObject({
      ...
      domain: null,
      template: null,
      requestedCwd: "/home/user/pilot-workspace",
      timeoutMs: 300_000,
  ...
  it("builds a quoted shell command with cwd, env, args, and stdin", () => {
    ...
    expect(command).toContain("printf '%s' 'input body' > '/tmp/.pilot-stdin-");
    expect(command).toMatch(/< '\/tmp\/\.pilot-stdin-[^']+'/);
    expect(command).toMatch(/rm -f '\/tmp\/\.pilot-stdin-[^']+'/);
  ...
  it("uses a unique stdin path for each command", () => {
    ...
    const firstPath = first.match(/\/tmp\/\.pilot-stdin-[^']+/)?.[0];
    const secondPath = second.match(/\/tmp\/\.pilot-stdin-[^']+/)?.[0];

    expect(firstPath).toBeTruthy();
    expect(secondPath).toBeTruthy();
    expect(firstPath).not.toBe(secondPath);
    expect(first).not.toContain("/tmp/.pilot-stdin <<");
    expect(second).not.toContain("/tmp/.pilot-stdin <<");
  });
