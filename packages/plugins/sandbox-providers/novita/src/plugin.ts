...
export function parseNovitaDriverConfig(raw: Record<string, unknown>): NovitaDriverConfig {
  return {
    ...
    requestedCwd: parseOptionalString(raw.requestedCwd) ?? "/home/user/pilot-workspace",
...
function buildStdinPath(): string {
  return `/tmp/.pilot-stdin-${randomUUID()}`;
}
...
async function createSandbox(params: PluginEnvironmentAcquireLeaseParams | PluginEnvironmentProbeParams, config: NovitaDriverConfig) {
  const metadata = {
    "pilot-provider": "novita",
    "pilot-company-id": params.companyId,
    "pilot-environment-id": params.environmentId,
    ...(params.issueId ? { "pilot-issue-id": params.issueId } : {}),
    ...("runId" in params ? { "pilot-run-id": params.runId } : {}),
  };
  const opts = {
    ...sandboxOpts(config),
    metadata,
