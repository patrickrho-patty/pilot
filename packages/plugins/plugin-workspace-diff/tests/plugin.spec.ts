...
async function createGitWorkspace() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-workspace-diff-plugin-"));
  tempRoots.push(root);
  await fs.mkdir(path.join(root, "src"), { recursive: true });
  await git(root, ["init"]);
  await git(root, ["config", "user.email", "pilot@example.com"]);
  await git(root, ["config", "user.name", "Pilot Test"]);
