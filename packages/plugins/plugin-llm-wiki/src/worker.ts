...
function buildManualDistillPrompt(input: { companyId: string; projectId?: string | null; rootIssueId?: string | null }) {
  ...
  return [
    ...
    "",
    "Prompt source: LLM Wiki plugin action `distill-pilot-now` (`packages/plugins/plugin-llm-wiki/src/worker.ts`).",
    ...
    !input.projectId && !input.rootIssueId
      ? "- Do not hardcode a single project. Find non-plugin Pilot issues/comments/documents that changed in any project after the last processed cursor and are old enough for the stale/debounce threshold."
      : null,
    "",
    "Process:",
    "1. Read the wiki root AGENTS.md, wiki/index.md, and recent wiki/log.md entries.",
    "2. Assemble bounded Pilot source bundles for every eligible project or root issue, excluding LLM Wiki plugin-operation issues.",
    "3. Turn durable signal into project standups, wiki-insightful project pages, decisions, history, index, and log updates per the pilot-distill skill.",
...
const plugin = definePlugin({
  async setup(ctx) {
    ...
    for (const eventName of PILOT_EVENT_INGESTION_EVENTS) {
      ctx.events.on(eventName, async (event) => {
        ...
        if (result.status === "recorded") {
          ctx.logger.info("LLM Wiki recorded Pilot event for cursor discovery", {
    ...
    ctx.data.register("pilot-ingestion-profile", async (params) => {
    ...
    ctx.data.register("pilot-ingestion-candidates", async (params) => {
    ...
    ctx.actions.register("update-pilot-ingestion-profile", async (params) => {
    ...
    ctx.actions.register("queue-pilot-ingestion-backfill", async (params) => {
      const companyId = readCompanyIdFromParams(params);
      ...
      for (const scope of scopes) {
        ...
        const workItem = await createPilotDistillationWorkItem(ctx, {
          ...
          metadata: { backfillStartAt, backfillEndAt, requestedFrom: "queue-pilot-ingestion-backfill" },
        });
        const operation = await createOperationIssue(ctx, {
          ...
          spaceSlug,
          operationType: "backfill",
          title: scope.rootIssueId ? "Backfill Pilot root issue wiki history" : "Backfill Pilot project wiki history",
          useCheapModelProfile: params.useCheapModelProfile === true,
          prompt: [
            "Backfill LLM Wiki distillation was queued from a per-space Pilot ingestion profile.",
    ...
    ctx.actions.register("assemble-pilot-source-bundle", async (params) => {
    ...
    ctx.actions.register("create-pilot-distillation-run", async (params) => {
    ...
    ctx.actions.register("record-pilot-distillation-outcome", async (params) => {
    ...
    ctx.actions.register("distill-pilot-project-page", async (params) => {
    ...
    ctx.actions.register("distill-pilot-now", async (params) => {
      const companyId = readCompanyIdFromParams(params);
      const spaceSlug = stringField(params.spaceSlug);
      const projectId = stringField(params.projectId);
      ...
      const workItem = await createPilotDistillationWorkItem(ctx, {
        ...
        metadata: { requestedFrom: "distill-pilot-now" },
      });
      const operation = await createOperationIssue(ctx, {
        companyId,
        wikiId: stringField(params.wikiId),
        ...
        operationType: "distill",
        title: rootIssueId
          ? "Distill Pilot root issue into wiki"
          : projectId
            ? "Distill Pilot project into wiki"
            : "Distill Pilot changes into wiki",
        useCheapModelProfile: params.useCheapModelProfile === true,
    ...
    ctx.actions.register("enable-pilot-distillation-active-projects", async (params) => {
    ...
    ctx.actions.register("backfill-pilot-distillation", async (params) => {
      const companyId = readCompanyIdFromParams(params);
      const spaceSlug = stringField(params.spaceSlug);
      const projectId = stringField(params.projectId);
      const rootIssueId = stringField(params.rootIssueId);
      ...
      const workItem = await createPilotDistillationWorkItem(ctx, {
        ...
        metadata: { backfillStartAt, backfillEndAt, requestedFrom: "backfill-pilot-distillation" },
      });
      const operation = await createOperationIssue(ctx, {
        companyId,
        wikiId: stringField(params.wikiId),
        spaceSlug,
        operationType: "backfill",
        title: rootIssueId ? "Backfill Pilot root issue wiki history" : "Backfill Pilot project wiki history",
        useCheapModelProfile: params.useCheapModelProfile === true,
        prompt: [
          "Backfill LLM Wiki distillation requested for a bounded Pilot source window.",
    ...
    ctx.actions.register("create-pilot-distillation-work-item", async (params) => {
