...
export const PLUGIN_ID = "pilotai.plugin-llm-wiki";
...
export const PILOT_DISTILL_SKILL_KEY = "pilot-distill";
...
function canonicalSkillKey(skillKey: string) {
  return `plugin/pilotai-plugin-llm-wiki/${skillKey}`;
}
...
const CURSOR_WINDOW_ROUTINE_DESCRIPTION = `Process bounded Pilot issue-history windows into the LLM Wiki.

Run procedure:
Target space: default (slug: default). Pilot-derived indexing currently writes only into the default space, so this routine never sweeps other spaces. Per-space Pilot ingestion profiles are a later phase; until they ship, treat any prompt to operate on a non-default space here as a bug and stop.
...
5. For ambiguous contradictions or major rewrites, leave the pages unchanged and create or comment a follow-up Pilot issue with the exact files and evidence.
...
const manifest: PilotPluginManifestV1 = {
  ...
  localFolders: [
    {
      ...
      description: "Company-scoped local folder that stores raw sources, wiki pages, Pilot project standups under wiki/projects/, AGENTS.md, IDEA.md, wiki/index.md, and wiki/log.md.",
      access: "readWrite",
      requiredDirectories: [
        "raw",
        "wiki",
  ...
  skills: [
    ...
    },
    {
      ...
      displayName: "Pilot Distill",
      slug: PILOT_DISTILL_SKILL_KEY,
      description: "Turn Pilot cursor-window, distill, or backfill source bundles into wiki-insightful project knowledge.",
