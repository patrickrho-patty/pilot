...
import { createTestHarness } from "@pilotai/plugin-sdk/testing";
import type { Agent, Issue, PluginManagedRoutineResolution, Project } from "@pilotai/plugin-sdk";
import manifest, {
...
beforeEach(() => {
  ...
  (globalThis as TestBridgeGlobal).__pilotPluginBridge__ = {
    sdkUi: {
      ...
      usePluginToast: () => () => null,
      useHostNavigation: () => ({
        resolveHref: (to: string) => `/PIL${to.startsWith("/") ? to : `/${to}`}`,
        navigate: (to: string) => { mockNavigatedTo = to; },
        linkProps: (to: string) => ({
          href: `/PIL${to.startsWith("/") ? to : `/${to}`}`,
          onClick: () => undefined,
        }),
      }),
      useHostLocation: () => ({
      ...
      ManagedRoutinesList: (props: {
        ...
        props.routines.map((routine) => {
          ...
          return createElement(
            ...
            routine.href ? createElement("a", { href: `/PIL${routine.href}` }, "Configure") : null,
...
afterEach(() => {
  ...
  delete (globalThis as TestBridgeGlobal).__pilotPluginBridge__;
});
...
function pilotIssue(overrides: Partial<Issue> = {}): Issue {
  ...
  return {
    ...
    description: "Decide which Pilot issues, comments, and documents can be ingested into the wiki.",
    status: "todo",
    workMode: "standard",
    priority: "medium",
    reviewPolicy: null,
...
describe("LLM Wiki plugin scaffold", () => {
  it("declares standalone plugin surfaces without core wiki coupling", () => {
    expect(manifest.id).toBe("pilotai.plugin-llm-wiki");
  ...
  it("renders a host-aligned sidebar link with an open-book icon", () => {
    const markup = renderToStaticMarkup(createElement(SidebarLink, {
      context: { companyId: COMPANY_ID, companyPrefix: "PIL" },
    } as never));

    expect(markup).toContain('href="/PIL/wiki"');
  ...
  it("ships Karpathy-pattern schema and workflow prompts by default", () => {
    ...
    expect(manifest.agents?.[0]?.adapterConfig?.pilotSkillSync).toEqual({
  ...
  it("renders the route-scoped Wiki sidebar with tool actions, page navigation, and a back link", () => {
    ...
    expect(markup).toContain('href="/PIL/dashboard"');
    expect(markup).toContain("PIL");
    expect(markup).toContain('href="/PIL/wiki/query"');
    expect(markup).not.toContain('href="/PIL/wiki/lint"');
    expect(markup).toContain('href="/PIL/wiki/history"');
    expect(markup).toContain('href="/PIL/wiki/settings"');
    expect(markup).not.toContain('href="/PIL/wiki/operations"');
    // Add Content reuses the legacy /wiki/ingest route so deep links still work.
    expect(markup).toContain('href="/PIL/wiki/ingest"');
  ...
  it("routes legacy Wiki operations URLs to the History issue table", () => {
    mockPathname = "/PIL/wiki/operations";
    const markup = renderToStaticMarkup(createElement(WikiPage, {
      context: { companyId: COMPANY_ID, companyPrefix: "PIL" },
    } as never));

    expect(markup).toContain("Issues table · project-1 · plugin:pilotai.plugin-llm-wiki:operation");
  ...
  it("loads AGENTS.md from the Wiki page and exposes an edit affordance", () => {
    mockPathname = "/PIL/wiki/page/templates/AGENTS.md";
    const markup = renderToStaticMarkup(createElement(WikiPage, {
      context: { companyId: COMPANY_ID, companyPrefix: "PIL" },
    } as never));
  ...
  it("does not render page footer sources and backlinks", () => {
    mockPathname = "/PIL/wiki/page/wiki/projects/control-plane/index.md";
    ...
    mockPageMetadataByPath["wiki/projects/control-plane/index.md"] = {
      ...
      sourceRefs: [{ kind: "issue", issueIdentifier: "PIL-FOOTER-1" }],
    };
    ...
    expect(markup).not.toContain("PIL-FOOTER-1");
  ...
  it("renders YAML frontmatter as foldable properties without duplicating title", () => {
    mockPathname = "/PIL/wiki/page/wiki/concepts/sidebar-navigation.md";
...
`;
  ...
  it("shows folder repair instead of reading pages when the wiki root is stale", () => {
    mockPathname = "/PIL/wiki/page/AGENTS.md";
    mockOverviewFolder = {
      configured: true,
  ...
  it("highlights Settings for legacy lint links after lint moved under Settings", () => {
    mockPathname = "/PIL/wiki/lint";
    const markup = renderToStaticMarkup(createElement(WikiRouteSidebar, {
      context: { companyId: COMPANY_ID, companyPrefix: "PIL" },
    } as never));

    expect(markup).not.toContain('href="/PIL/wiki/lint"');
    const settingsAnchor = markup.match(/<a[^>]*href="\/PIL\/wiki\/settings"[^>]*>/);
    ...
    const askAnchor = markup.match(/<a[^>]*href="\/PIL\/wiki\/query"[^>]*>/);
  ...
  it("renders the maintainer settings without plugin metadata or inline AGENTS.md editing", () => {
    mockPathname = "/PIL/wiki/settings/maintainer";
    const markup = renderToStaticMarkup(createElement(WikiPage, {
      context: { companyId: COMPANY_ID, companyPrefix: "PIL" },
    } as never));
  ...
  it("renders root settings as a compact health checklist with the shared path picker", () => {
    mockPathname = "/PIL/wiki/settings";
    const markup = renderToStaticMarkup(createElement(WikiPage, {
      context: { companyId: COMPANY_ID, companyPrefix: "PIL" },
    } as never));
  ...
  it("renders project settings as a project picker without managed-resource metadata", () => {
    mockPathname = "/PIL/wiki/settings/project";
    const markup = renderToStaticMarkup(createElement(WikiPage, {
      context: { companyId: COMPANY_ID, companyPrefix: "PIL" },
    } as never));
  ...
  it("renders space settings with health line styling and disabled access controls", () => {
    mockPathname = "/PIL/wiki/settings/spaces/default";
  ...
  it("renders distillation settings with assigned-agent model selection and cheap path without budget controls", () => {
    mockPathname = "/PIL/wiki/settings/distillation";
  ...
  it("renders managed routines as normal routine rows with run, toggle, and configure controls", () => {
    mockPathname = "/PIL/wiki/settings/routines";
    mockSettingsManagedRoutines = [{
      status: "resolved",
      routineId: "routine-1",
    ...
    expect(markup).toContain("href=\"/PIL/routines/routine-1\"");
  });
  ...
  it("includes local wiki files in browse data before metadata is indexed", async () => {
    ...
    harness.ctx.localFolders.list = async (_companyId, folderKey, options) => ({
      ...
      entries: options?.relativePath === "wiki"
        ? [
            ...
            { path: "wiki/entities/pilot.md", name: "pilot.md", kind: "file", size: 10, modifiedAt },
    ...
    expect(result.pages).toEqual([
      ...
      expect.objectContaining({ path: "wiki/entities/pilot.md", title: null }),
  ...
  it("does not ingest Pilot events until operator controls enable them", async () => {
    const harness = createTestHarness({ manifest });
  ...
  it("records enabled Pilot issue events as cursor observations without creating ingest operations", async () => {
    const harness = createTestHarness({ manifest });
    const issue = pilotIssue();
    harness.seed({ issues: [issue] });
    const writes: Array<{ path: string; contents: string }> = [];
    ...
    expect(harness.dbExecutes.some((execute) => execute.sql.includes("wiki_sources"))).toBe(false);
    expect(harness.dbExecutes.some((execute) => execute.sql.includes("wiki_operations"))).toBe(false);
    const cursorUpsert = harness.dbExecutes.find((execute) => execute.sql.includes("pilot_distillation_cursors"));
  ...
  it("preserves Pilot event ingestion sources when only enabled changes", async () => {
    const harness = createTestHarness({ manifest });
  ...
  it("keeps Pilot event cursor observations company scoped and ignores plugin-operation issues", async () => {
    const harness = createTestHarness({ manifest });
    ...
    const cursorWrites = harness.dbExecutes.filter((execute) => execute.sql.includes("pilot_distillation_cursors"));
  ...
  it("routes Pilot issue, comment, and document event cursors only to the default space", async () => {
    const harness = createTestHarness({ manifest });
  ...
  it("routes Pilot events into an explicitly enabled shared non-default space", async () => {
    const harness = createTestHarness({ manifest });
    ...
    await harness.performAction("update-pilot-ingestion-profile", {
      companyId: COMPANY_ID,
      spaceSlug: created.space.slug,
      profile: enabledProfile,
    });
    mockPersistedWikiSpace(harness, { ...(created.space as unknown as Record<string, unknown>), settings: { pilotIngestion: enabledProfile } });
  ...
  it("stops new non-default observations after the per-space profile is disabled", async () => {
    ...
    mockPersistedWikiSpace(harness, { ...(created.space as unknown as Record<string, unknown>), settings: { pilotIngestion: { ...enabledProfile, enabled: false } } });
    await harness.emit("issue.updated", {}, {
      companyId: COMPANY_ID,
      entityId: issue.id,
      entityType: "issue",
    ...
    const nonDefaultCursorWrites = harness.dbExecutes.filter((execute) =>
      execute.sql.includes("pilot_distillation_cursors") && execute.params?.[10] === created.space.id);
    expect(nonDefaultCursorWrites).toHaveLength(1);
  });

  it("assembles deterministic Pilot source bundles with issue, document, and comment provenance", async () => {
    const harness = createTestHarness({ manifest });
    ...
    }>("assemble-pilot-source-bundle", {
      companyId: COMPANY_ID,
      rootIssueId: root.id,
      maxCharacters: 20000,
    });
    const second = await harness.performAction<typeof first>("assemble-pilot-source-bundle", {
      companyId: COMPANY_ID,
      rootIssueId: root.id,
      maxCharacters: 20000,
    });
    ...
    expect(first.sourceRefs).toEqual(expect.arrayContaining([
      expect.objectContaining({ kind: "issue", issueIdentifier: "PIL-4001" }),
  ...
  it("suppresses secret-like comment and document bodies before storing distillation snapshots", async () => {
    ...
    const snapshotInsert = harness.dbExecutes.find((execute) => execute.sql.includes("pilot_source_snapshots"));
  ...
  it("creates source snapshots and only advances cursors after successful distillation outcomes", async () => {
    ...
    expect(harness.dbExecutes.some((execute) => execute.sql.includes("pilot_source_snapshots"))).toBe(true);

    const failed = await harness.performAction<{ cursorAdvanced: boolean }>("record-pilot-distillation-outcome", {
      companyId: COMPANY_ID,
      runId: run.runId,
    ...
    const cursorUpdatesAfterFailure = harness.dbExecutes.filter((execute) =>
      execute.sql.trim().startsWith("UPDATE") && execute.sql.includes("pilot_distillation_cursors"));
    expect(cursorUpdatesAfterFailure).toHaveLength(0);

    const succeeded = await harness.performAction<{ cursorAdvanced: boolean }>("record-pilot-distillation-outcome", {
      companyId: COMPANY_ID,
      runId: run.runId,
      cursorId: run.cursorId,
      status: "succeeded",
  ...
  it("uses the existing distillation cursor id after an upsert conflict", async () => {
    ...
    const originalQuery = harness.ctx.db.query.bind(harness.ctx.db);
    harness.ctx.db.query = async <T,>(sql: string, params?: unknown[]) => {
      if (sql.includes("SELECT id") && sql.includes("pilot_distillation_cursors")) {
    ...
    await plugin.definition.setup(harness.ctx);
    const result = await harness.performAction<{ cursorId: string }>("create-pilot-distillation-run", {
      companyId: COMPANY_ID,
      projectId: project.id,
    });
    ...
    const runInsert = harness.dbExecutes.find((execute) =>
      execute.sql.includes("pilot_distillation_runs") && execute.sql.includes("'source_ready'"));
  ...
  it("creates explicit distillation work items for manual, retry, and backfill lanes", async () => {
    ...
    await harness.performAction("create-pilot-distillation-work-item", {
      companyId: COMPANY_ID,
      kind: "manual",
      projectId: "77777777-7777-4777-8777-777777777777",
    ...
    const workItemWrites = harness.dbExecutes.filter((execute) => execute.sql.includes("pilot_distillation_work_items"));
    ...
    await expect(harness.performAction("create-pilot-distillation-work-item", {
      companyId: COMPANY_ID,
      kind: "backfill",
  ...
  it("records estimated Pilot distillation cost without refusing on legacy cost config", async () => {
    const harness = createTestHarness({
      manifest,
      config: {
        maxPilotRoutineRunCostCents: 1,
        maxPilotDistillationTaskCostCents: 1,
        maxPilotDistillationProjectCostCents: 1,
        pilotCostCentsPerThousandSourceCharacters: 100,
      },
    });
    const project = existingProject();
    const issue = pilotIssue({
  ...
  it("queues visible manual distill operation issues for a company-wide stale cursor scan", async () => {
    ...
    expect(result.operation.issue.description).toContain("Prompt source: LLM Wiki plugin action `distill-pilot-now`");
    ...
    const workItemInsert = harness.dbExecutes.find((execute) =>
      execute.sql.includes("pilot_distillation_work_items") && execute.params?.[3] === "manual");
    ...
    const runInsert = harness.dbExecutes.find((execute) =>
      execute.sql.includes("pilot_distillation_runs") && execute.sql.includes("operation_issue_id"));
  ...
  it("enables distillation cursors for the most recently active non-plugin projects", async () => {
    ...
    }>("enable-pilot-distillation-active-projects", {
      companyId: COMPANY_ID,
      limit: 2,
    });
  ...
  it("backfills only the selected Pilot project and date window", async () => {
    const harness = createTestHarness({ manifest });
    const project = existingProject();
    ...
    expect(projectPatch?.proposedContents).toContain("PIL-4106");
    expect(projectPatch?.proposedContents).not.toContain("PIL-4107");
    expect(projectPatch?.proposedContents).not.toContain("PIL-4108");
    const workItemInsert = harness.dbExecutes.find((execute) =>
      execute.sql.includes("pilot_distillation_work_items") && execute.params?.[3] === "backfill");
    expect(String(workItemInsert?.params?.[9])).toContain('"backfillStartAt":"2026-04-01T00:00:00Z"');
  });

  it("generates review-required Pilot project page patches with provenance, index, and log updates", async () => {
    const harness = createTestHarness({ manifest });
    const project = existingProject();
    const issue = pilotIssue({
    ...
    expect(projectPatch.proposedContents).toContain("PIL-4100");
    expect(projectPatch.sourceRefs).toEqual([expect.objectContaining({ issueIdentifier: "PIL-4100" })]);
    ...
    expect(result.patches[3].proposedContents).toContain("pilot-distill | proposed");
  ...
  it("auto-applies Pilot project page patches by default when policy allows and records page bindings", async () => {
    const harness = createTestHarness({ manifest });
    const project = existingProject();
    const issue = pilotIssue({
    ...
    expect(files.get("wiki/log.md")).toContain("pilot-distill | proposed");
    const bindingWrites = harness.dbExecutes.filter((execute) => execute.sql.includes("pilot_page_bindings"));
  ...
  it("refuses auto-apply Pilot project page patches in authenticated/public deployments", async () => {
  ...
  it("refuses stale project page hashes before writing generated Pilot pages", async () => {
    const harness = createTestHarness({ manifest, config: { autoApplyIngestPatches: true } });
    const project = existingProject();
    const issue = pilotIssue({
    ...
    await plugin.definition.setup(harness.ctx);
    await expect(harness.performAction("distill-pilot-project-page", {
      companyId: COMPANY_ID,
      projectId: project.id,
      autoApply: true,
  ...
  it("skips low-signal Pilot source windows without proposing wiki writes", async () => {
    const harness = createTestHarness({ manifest });
    const project = existingProject();
    const issue = pilotIssue({
    ...
    await plugin.definition.setup(harness.ctx);
    const result = await harness.performAction<{ status: string; reason: string; patches: unknown[] }>("distill-pilot-project-page", {
      companyId: COMPANY_ID,
      projectId: project.id,
      maxCharacters: 20000,
    });
  ...
  it("preserves manual source ingest for non-default spaces while refusing Pilot distillation there", async () => {
    const harness = createTestHarness({ manifest });
    const writes: Array<{ path: string; contents: string }> = [];
    harness.ctx.localFolders.writeTextAtomic = async (_companyId, _folderKey, relativePath, contents) => {
      writes.push({ path: relativePath, contents });
    ...
    await expect(harness.performAction("distill-pilot-now", {
      companyId: COMPANY_ID,
      spaceSlug: created.space.slug,
      projectId: existingProject().id,
    })).rejects.toThrow("Pilot ingestion policy denied queue");
  });

  it("fails closed for direct Pilot ingestion actions against restricted spaces", async () => {
    const harness = createTestHarness({ manifest });
    harness.seed({
      agents: [wikiMaintainerAgent()],
      projects: [existingProject()],
    ...
    expect(operations).toHaveLength(0);
    expect(harness.dbExecutes.some((execute) => execute.sql.includes("pilot_distillation_work_items"))).toBe(false);
  });

  it("re-checks Pilot ingestion policy at execution time for queued work", async () => {
    const harness = createTestHarness({ manifest });
    const project = existingProject();
    const issue = pilotIssue({
    ...
    await plugin.definition.setup(harness.ctx);
    const queued = await harness.performAction<{ status: string }>("distill-pilot-now", {
      companyId: COMPANY_ID,
      projectId: project.id,
    });
    ...
    await expect(harness.performAction("create-pilot-distillation-run", {
      companyId: COMPANY_ID,
      projectId: project.id,
    })).rejects.toThrow("personal spaces cannot ingest Pilot sources");
    expect(harness.dbExecutes.some((execute) =>
      execute.sql.includes("pilot_distillation_runs") && execute.sql.includes("'source_ready'"))).toBe(false);
  });

  it("queues Pilot ingestion backfills for every selected project scope", async () => {
    const harness = createTestHarness({ manifest });
    await plugin.definition.setup(harness.ctx);
    ...
    }>("queue-pilot-ingestion-backfill", {
      companyId: COMPANY_ID,
      sourceScope: {
    ...
    expect(harness.dbExecutes.filter((execute) => execute.sql.includes("pilot_distillation_work_items"))).toHaveLength(2);
  });

  it("rejects oversized Pilot ingestion profile and source-scope payloads", async () => {
    const harness = createTestHarness({ manifest });
    ...
    await expect(harness.performAction("update-event-ingestion-settings", {
      ...
      maxCharacters: 20001,
      sources: { issues: true },
    })).rejects.toThrow("maxCharacters exceeds the hard Pilot ingestion cap");

    await expect(harness.performAction("enable-pilot-distillation-active-projects", {
      companyId: COMPANY_ID,
      limit: 26,
    })).rejects.toThrow("fan-out exceeds the hard cap");

    await expect(harness.performAction("assemble-pilot-source-bundle", {
      companyId: COMPANY_ID,
      projectId: "77777777-7777-4777-8777-777777777777",
