---
name: pilot-distill
description: Use when an operation issue is a Pilot cursor-window, distill, or backfill. Turn source-bundled Pilot activity into wiki-insightful project standups, durable project pages, decisions, and history without asset dereferencing.
---

# Pilot Distill

Distill Pilot project, issue, comment, and document activity into durable wiki pages. The success criterion is **wiki-insightful, not procedural**: a reader who has never seen Pilot should learn what the project is, what was decided, what is at risk, and what the current state is — without scanning a list of `## [YYYY-MM-DD]` headers.

## When this skill is needed

- Cursor-window distillation: the routine fed you a bounded source bundle of recent Pilot activity for one project or root issue.
...
- Manual `distill-pilot-now` request from the UI.
...
In Phase 1, every Pilot distill, backfill, and cursor-window operation writes into the
...
write Pilot-derived pages into a non-default space.

This rule is destination-only. The Pilot source scope (which projects, root issues,
comments, documents are read) is set elsewhere in the operation issue and is independent of
the destination.

## Inputs

- A Pilot source bundle (issue list, comment refs, document refs, source hash, cursor window).
...
Do not treat Pilot assets/attachments or issue work products as source text for this skill.

- Allowed Pilot body text: issue descriptions, comment bodies, document bodies.
...
1. **Datestamp-as-section-header.** Lines like `## [2026-04-15] pilot-distill | proposed` belong in `wiki/log.md`, not in the project page. The project page is durable knowledge; the log is the audit trail.
2. **Procedural status lists.** `Issue mix: 3 todo, 5 in_progress, 2 done` tells the reader nothing they could not read off Pilot directly. State *what is happening and why it matters*, then cite the issues that constitute the evidence.
3. **One-line-per-issue dumps.** A page that is mostly `- PIL-1234: title (in_progress, updated 2026-...)` is an issue list, not a wiki page. Group issues by what they are *about* (a decision, a risk, a workstream) and cite multiple issues per bullet when they share a story.
4. **Mechanical "Current as of" timestamps everywhere.** One `current_as_of` in frontmatter is enough.
5. **No interpretation.** "Active issues: PIL-A, PIL-B, PIL-C" is bookkeeping. "The team is concentrating on the schema migration ([PIL-A], [PIL-B]) and has parked the index work pending capacity ([PIL-C])." is wiki-insightful.
...
4. **Write `wiki/projects/<slug>/standup.md` first.** Every Pilot project represented in the wiki must have this file. It is the executive standup: where the project stands today, what changed recently, what is blocked or risky, and what happens next. Use stable sections, in this order:
   ...
   - **Next Actions** — concrete next actions and owners inferred from Pilot issues, not vague aspirations.
   - **Links** — durable wiki project page and relevant Pilot project/issues/documents.
   Rewrite the standup to today's state. Do not append endless dated sections; the audit trail belongs in `wiki/log.md` and Pilot comments.
5. **Write `wiki/projects/<slug>/index.md`** with these stable sections, in this order:
   ...
   - **References** — readable links to the current standup and supporting Pilot tasks/documents. Keep hashes and cursor ids out of the narrative.
...
9. **Append `wiki/log.md`** entry — this is where the datestamp belongs:
   ```
   ## [YYYY-MM-DD] pilot-distill | <project name>
   - standup: wiki/projects/<slug>/standup.md
   - page: wiki/projects/<slug>/index.md
   - source hash: `<hash>`
   - cursor window: <start> → <end>
   - notes: <one line on what changed in this distill, e.g. "decisions section grew with PIL-X reversal", "low-signal window, no page changes">
   ```
...
- Cite Pilot source refs inline using their issue identifier (e.g. `PIL-3179`), not opaque UUIDs.
...
- Append a `pilot-distill | low-signal skip` log entry naming the cursor window.
...
- [ ] The project page reads as wiki content, not as a Pilot status report. A reader new to the company should understand what the project is.
...
`wiki_search`, `wiki_read_page`, `wiki_write_page`, `wiki_list_sources`, `wiki_read_source`. Always include the operation issue's `wikiId` and `spaceSlug`. The Pilot source bundle arrives as part of the operation context — you do not need to assemble it.
