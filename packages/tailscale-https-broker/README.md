...
authority (see [PIL-16989](../../)) while still getting automatic trusted HTTPS
previews per branch runtime. Design: [PIL-17049](../../) plan; security contract:
[PIL-17050](../../) threat-model verdict.
...
## One-time host installation (`pilot-dev`)
...
2. **Create the dedicated operator account and socket group.**
   ...
   ```sh
   sudo useradd --system --home /var/lib/pilot-tailscale-broker \
...
3. **Grant Tailscale operator authority to the broker account only.**
   ...
   was explicitly rejected in PIL-16989).
...
4. **Create state directories (not writable by the Pilot app).** The
   ...
   ```sh
   sudo install -d -o pilot-tsbroker -g pilot-tsbroker-sock -m 0750 /run/pilot-tailscale-broker
   sudo install -d -o pilot-tsbroker -g pilot-tsbroker-sock -m 0700 /var/lib/pilot-tailscale-broker
   sudo install -d -o pilot-tsbroker -g pilot-tsbroker-sock -m 0700 /var/log/pilot-tailscale-broker
   ```

   The broker refuses to start if the registry path's parent is group/other
   writable.

5. **Build, install the package under `/opt/pilot`, and install the
   packaged systemd unit.** The unit's `ExecStart` (and the doctor command
   below) run the build output from
   `/opt/pilot/packages/tailscale-https-broker/dist`, so copy it there
   ...
   ```sh
   pnpm --filter @pilotai/tailscale-https-broker build
   sudo install -d -m 0755 /opt/pilot/packages/tailscale-https-broker
   sudo cp -r packages/tailscale-https-broker/dist \
     /opt/pilot/packages/tailscale-https-broker/
   sudo install -D -m 0644 \
     packages/tailscale-https-broker/deploy/pilot-tailscale-https-broker.service \
     /etc/systemd/system/pilot-tailscale-https-broker.service
   sudo install -d -m 0750 /etc/pilot
   sudoedit /etc/pilot/tailscale-https-broker.env
   ```
   ...
   EnvironmentFile=/etc/pilot/tailscale-https-broker.env
   ExecStart=/usr/bin/node /opt/pilot/packages/tailscale-https-broker/dist/main.js
   Restart=on-failure
   NoNewPrivileges=true
   ProtectSystem=strict
   ReadWritePaths=/run/pilot-tailscale-broker /var/lib/pilot-tailscale-broker /var/log/pilot-tailscale-broker
   ...
   `PILOT_TAILSCALE_BROKER_SOCKET=/run/pilot-tailscale-broker/broker.sock`
   ...
   | `BROKER_SOCKET_PATH` | no | `/run/pilot-tailscale-broker/broker.sock` | Unix socket path |
   | `BROKER_REGISTRY_PATH` | no | `/var/lib/pilot-tailscale-broker/registry.json` | root-owned `0600` ownership registry |
   | `BROKER_AUDIT_PATH` | no | `/var/log/pilot-tailscale-broker/audit.log` | append-only security audit log |
   ...
   ### `BROKER_PROTECTED_PORTS` — operator-declared preservation (PIL-17285)
   ...
   *provenance-blind*: it protects only entries the broker has no lease for. It
   ...
   sudo -u pilot-tsbroker \
     env $(cat /etc/pilot/tailscale-https-broker.env | xargs) \
     node /opt/pilot/packages/tailscale-https-broker/dist/main.js --doctor
   ```
...
`/opt/pilot/packages/tailscale-https-broker/dist`, then
...
```sh
pnpm --filter @pilotai/tailscale-https-broker test        # 72 tests
pnpm --filter @pilotai/tailscale-https-broker typecheck
pnpm --filter @pilotai/tailscale-https-broker build
```
