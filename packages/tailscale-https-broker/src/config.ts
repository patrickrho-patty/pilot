...
export interface BrokerHostConfig {
   ...
   * `BROKER_PROTECTED_PORTS` (PIL-17285). Empty when unset.
   */
...
export function loadHostConfig(env: NodeJS.ProcessEnv): BrokerHostConfig {
  const config: BrokerHostConfig = {
    socketPath: env.BROKER_SOCKET_PATH ?? "/run/pilot-tailscale-broker/broker.sock",
    registryPath: env.BROKER_REGISTRY_PATH ?? "/var/lib/pilot-tailscale-broker/registry.json",
    auditPath: env.BROKER_AUDIT_PATH ?? "/var/log/pilot-tailscale-broker/audit.log",
    ...
    // starting up silently protecting nothing (PIL-17285).
