 ...
 * mutation implements every PIL-17050 verdict requirement and invariant:
 *
...
export interface BrokerCoreConfig {
   ...
   * (PIL-17285). Outranks the broker's own ownership record: a protected port is
...
export class BrokerCore {
   ...
   * mutates Serve (PIL-17285). Deliberately checked ahead of the lease/ownership
  ...
  private doReserve(
    ...
    // on one and no later lifecycle op can reach it (PIL-17285).
  ...
  private doExpose(
    ...
    for (const port of lease.ports) {
      ...
      if (entry && !isSameNumberLoopbackEntry(entry, port)) {
        denied("manual_mapping_present", `port ${port} already has a non-Pilot mapping`);
      }
    }
   ...
   * durable record (PIL-17285 requirement #3, and req #6 which mandates one
