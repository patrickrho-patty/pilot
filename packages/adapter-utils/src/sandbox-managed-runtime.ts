 ...
 * workspace's git-history, overlay, or `.pilot-runtime` preservation
...
function buildWorkspaceTarExtractCommand(input: {
  ...
  // tarball named `.pilot-upload-<uuid>.tar` there. Without this preserve
  ...
  const wipe = input.wipeExceptNames
    ? ` && find ${shellQuote(input.workspaceRemoteDir)} -mindepth 1 -maxdepth 1 ` +
      `${preserveFindArgs([...input.wipeExceptNames, ".pilot-upload-*"])} -exec rm -rf -- {} +`
    : "";
  return (
...
async function copyWorkspaceEntry(sourceRoot: string, targetRoot: string, relative: string): Promise<void> {
  ...
  const stagedTargetPath = buildUniqueStagingPath({ targetPath, suffix: ".pilot-copy" });
...
export async function prepareSandboxManagedRuntime(input: {
  ...
  const runtimeRootDir = path.posix.join(workspaceRemoteDir, ".pilot-runtime", input.adapterKey);
  ...
  await withTempDir("pilot-sandbox-sync-", async (tempDir) => {
      ...
      inboundTasks.push(() =>
        runStepSpan("stage.workspace", async () => {
          ...
          await runStepSpan("pack", async () => {
            ...
            //    `runtimeRootDir` (`.pilot-runtime/<adapterKey>`). The git extract
            //    wipes the target tree EXCEPT `.pilot-runtime`, so the overlay tar,
            //    which sits under `.pilot-runtime`, survives to run its own extract.
            if (gitSnapshot) {
    ...
    // their own roots. No workspace, git-history, or `.pilot-runtime`
      ...
      if (syncWorkspace) {
        outboundTasks.push(() =>
          runStepSpan("restore.workspace", async () => {
            await withTempDir("pilot-sandbox-restore-", async (tempDir) => {
