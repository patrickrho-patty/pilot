...
export function readPilotIssueWorkModeFromContext(value: unknown): string | null {
  ...
  const wake = normalizePilotWakePayload(context.paperclipWake);
...
export function selectPilotTaskMarkdown(
  ...
  const wake = normalizePilotWakePayload(context?.paperclipWake);
  if (!wake) return full;
  if (isAssignmentShapedPilotWakeReason(wake.reason) || isPilotRecoveryWakePayload(context?.paperclipWake)) {
    return full;
  }
...
export async function readPilotRuntimeSkillEntries(
  ...
  const configuredEntries = normalizeConfiguredPilotRuntimeSkills(config.paperclipRuntimeSkills);
