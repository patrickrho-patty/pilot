...
// (PILOT_BRIDGE_RESPONSE_TIMEOUT_MS), so the host loop fails fast and writes
...
const SANDBOX_CALLBACK_BRIDGE_ENTRYPOINT = "pilot-bridge-server.mjs";
...
// selects the mode from `PILOT_API_BRIDGE_MODE`.
...
// value stays configurable through `PILOT_BRIDGE_RESPONSE_TIMEOUT_MS`, the
...
// `skills/pilot/SKILL.md` and `references/api-reference.md`.
...
export async function createSandboxCallbackBridgeAsset(): Promise<SandboxCallbackBridgeAsset> {
  const localDir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-bridge-asset-"));
...
export function createFileSystemSandboxCallbackBridgeQueueClient(): SandboxCallbackBridgeQueueClient {
  return {
    ...
    writeTextFile: async (remotePath, body) => {
      ...
      const tempPath = `${remotePath}.pilot-upload.decoded`;
    ...
    writeResponseFile: async (responsePath, body, options = {}) => {
      ...
      const lockDir = `${responsePath}.pilot-write.lock`;
...
export function createCommandManagedSandboxCallbackBridgeQueueClient(input: {
  ...
  return {
    ...
    writeTextFile: async (remotePath, body) => {
      ...
      const tempPath = `${remotePath}.pilot-upload.b64`;
      const decodedPath = `${remotePath}.pilot-upload.decoded`;
      await runChecked(
...
export async function startSandboxCallbackBridgeWorker(input: {
  ...
  const processRequestFile = async (fileName: string) => {
    ...
    const finalize = async (response: SandboxCallbackBridgeResponse) => {
      ...
      for (let attempt = 1; attempt <= MAX_BACKSTOP_WRITE_ATTEMPTS; attempt += 1) {
        try {
          ...
          console.warn(
            `[pilot] sandbox callback bridge failed to write response for ${response.id} (attempt ${attempt}/${MAX_BACKSTOP_WRITE_ATTEMPTS}): ${lastWriteError}`,
          );
    ...
    try {
      ...
      try {
        ...
        console.warn(
          `[pilot] sandbox callback bridge handler failed for ${request.id}: ${error instanceof Error ? error.message : String(error)}`,
        );
  ...
  const writeAbortedHandlerBackstop = async (
    ...
    for (let attempt = 1; attempt <= MAX_BACKSTOP_WRITE_ATTEMPTS; attempt += 1) {
      try {
        ...
        console.warn(
          `[pilot] sandbox callback bridge failed to write 504 backstop for ${requestId} (attempt ${attempt}/${MAX_BACKSTOP_WRITE_ATTEMPTS}): ${error instanceof Error ? error.message : String(error)}`,
        );
        if (attempt < MAX_BACKSTOP_WRITE_ATTEMPTS) {
          await new Promise((resolve) => setTimeout(resolve, BACKSTOP_WRITE_RETRY_MS));
        }
  ...
  const failPendingRequests = async (
    ...
    for (const fileName of fileNames) {
      ...
      try {
        ...
        console.warn(
          `[pilot] sandbox callback bridge could not read pending request ${requestId}: ${error instanceof Error ? error.message : String(error)}`,
        );
        continue;
      }
      ...
      for (let attempt = 1; attempt <= MAX_BACKSTOP_WRITE_ATTEMPTS; attempt += 1) {
        try {
          ...
          lastWriteError = error instanceof Error ? error.message : String(error);
          console.warn(
            `[pilot] sandbox callback bridge failed to write recovery 503 for ${requestId} (attempt ${attempt}/${MAX_BACKSTOP_WRITE_ATTEMPTS}): ${lastWriteError}`,
          );
          if (attempt < MAX_BACKSTOP_WRITE_ATTEMPTS) {
            await new Promise((resolve) => setTimeout(resolve, BACKSTOP_WRITE_RETRY_MS));
          }
      ...
      if (wrote503) {
        ...
        console.warn(
          `[pilot] sandbox callback bridge kept queued request ${requestId} after every recovery 503 write failed: ${lastWriteError}`,
        );
      }
    }
  };
  ...
  const surfaceRunError = async (error: Error) => {
    ...
    console.warn(`[pilot] ${error.message}`);
  };
  ...
  const handleWatchdogTrip = async (idleMs: number) => {
    ...
    try {
      ...
      console.warn(
        `[pilot] sandbox callback bridge watchdog failed to abort queued requests: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  };
  ...
  // `bridge.pilot` step. The loop runs run-time execs for the whole run,
  ...
  const loop = runWithoutActiveStep(() => (async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          `[pilot] sandbox callback bridge failed to abort queued requests after worker failure: ${failPendingError instanceof Error ? failPendingError.message : String(failPendingError)}`,
        );
      }
    } finally {
...
export async function syncRemoteTextFileWithHashSkip(input: {
  ...
  const remoteUploadPath = `${input.remotePath}.pilot-upload.b64`;
...
export async function syncSandboxCallbackBridgeEntrypoint(input: {
  ...
  const { uploaded, sha256 } = await syncRemoteTextFileWithHashSkip({
    ...
    lockDir: path.posix.join(input.assetRemoteDir, ".pilot-bridge-upload.lock"),
...
const bridgeMode = process.env.PILOT_API_BRIDGE_MODE || "${SANDBOX_CALLBACK_BRIDGE_FILE_MODE}";
const queueDir = process.env.PILOT_BRIDGE_QUEUE_DIR ?? process.env.PILOT_BRIDGE_QUEUE_DIR;
const bridgeToken = process.env.PILOT_BRIDGE_TOKEN ?? process.env.PILOT_BRIDGE_TOKEN ?? process.env.PILOT_BRIDGE_TOKEN;
const host = process.env.PILOT_BRIDGE_HOST || "127.0.0.1";
const port = Number(process.env.PILOT_BRIDGE_PORT || "0");
const pollIntervalMs = Number(process.env.PILOT_BRIDGE_POLL_INTERVAL_MS || "100");
const responseTimeoutMs = Number(
  process.env.PILOT_BRIDGE_RESPONSE_TIMEOUT_MS ||
    (bridgeMode === "${SANDBOX_CALLBACK_BRIDGE_DUPLEX_MODE}"
      ? "${DEFAULT_DUPLEX_GATEWAY_WAIT_BUDGET_MS}"
      : "${DEFAULT_BRIDGE_RESPONSE_TIMEOUT_MS}"),
);
const maxQueueDepth = Number(process.env.PILOT_BRIDGE_MAX_QUEUE_DEPTH || "${DEFAULT_BRIDGE_MAX_QUEUE_DEPTH}");
const maxBodyBytes = Number(process.env.PILOT_BRIDGE_MAX_BODY_BYTES || "${DEFAULT_BRIDGE_MAX_BODY_BYTES}");
const heartbeatIntervalMs = Number(
  process.env.PILOT_BRIDGE_HEARTBEAT_INTERVAL_MS || "${DEFAULT_DUPLEX_GATEWAY_HEARTBEAT_INTERVAL_MS}",
);
const heartbeatTimeoutMs = Number(
  process.env.PILOT_BRIDGE_HEARTBEAT_TIMEOUT_MS || "${DEFAULT_DUPLEX_GATEWAY_HEARTBEAT_TIMEOUT_MS}",
);
const lossExitGraceMs = Number(
  process.env.PILOT_BRIDGE_LOSS_EXIT_GRACE_MS || "${DEFAULT_DUPLEX_GATEWAY_LOSS_EXIT_GRACE_MS}",
);
...
if (!bridgeToken) {
  throw new Error("PILOT_BRIDGE_TOKEN is required.");  // legacy message kept for grep-ability
}
if (bridgeMode !== "${SANDBOX_CALLBACK_BRIDGE_DUPLEX_MODE}" && !queueDir) {
  throw new Error("PILOT_BRIDGE_QUEUE_DIR and PILOT_BRIDGE_TOKEN are required.");
}
...
async function runFileGateway() {
  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      const bridgeOutcome = responseHeaders["x-pilot-bridge-outcome"];
...
function runDuplexGateway() {
  ...
  function diag(message) {
    ...
    process.stderr.write("[pilot-bridge] " + message + "\\n");
  }
