...
describe("sandbox callback bridge", () => {
  ...
  it("round-trips localhost bridge requests over the sandbox queue without forwarding the bridge token", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-bridge-runtime-"));
    ...
    const queueDir = path.posix.join(prepared.runtimeRootDir, "pilot-bridge");
    ...
    expect(seenRequests[0]?.headers["x-pilot-run-id"]).toBeUndefined();

  });

  it("denies non-allowlisted requests by default", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-bridge-default-policy-"));
    cleanupDirs.push(rootDir);
  ...
  it("drains already-queued requests on stop", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-bridge-drain-"));
    cleanupDirs.push(rootDir);
  ...
  it("writes fast 503 responses for queued requests that miss the drain deadline", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-bridge-drain-timeout-"));
    cleanupDirs.push(rootDir);
  ...
  it("handles SSH queue polling failures without emitting an unhandled rejection", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-bridge-ssh-failure-"));
    cleanupDirs.push(rootDir);
    ...
    try {
      const worker = await startSandboxCallbackBridgeWorker({
        client: {
          ...
          listJsonFiles: async () => {
            throw new Error(
              "list /remote/.pilot-runtime/gemini/pilot-bridge/queue/requests failed with exit code 255: kex_exchange_identification: read: Connection reset by peer",
            );
          },
          readTextFile: async () => {
  ...
  it("keeps the queue-directory setup on the startup step but resets the poll loop store", async () => {
    // The worker starts inside the measured `bridge.pilot` step. Its awaited
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-bridge-step-store-"));
    cleanupDirs.push(rootDir);
    const queueDir = path.posix.join(rootDir, "queue");
  ...
  it("test_pilot_loop_exec_parents_to_run_context", async () => {
    // The worker starts inside the measured `bridge.pilot` step. Its awaited
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-bridge-run-parent-"));
    cleanupDirs.push(rootDir);
    const queueDir = path.posix.join(rootDir, "queue");
  ...
  it("wraps each request in a sandbox.callbackBridge.relayRequest span", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-bridge-relay-span-"));
    cleanupDirs.push(rootDir);
    const queueDir = path.posix.join(rootDir, "queue");
  ...
  it("test_pilot_loop_exec_stays_unparented_without_getter", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-bridge-no-getter-"));
    cleanupDirs.push(rootDir);
    const queueDir = path.posix.join(rootDir, "queue");
  ...
  it("serializes remote response writes so stop does not recreate a late orphaned response", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-bridge-response-lock-"));
    cleanupDirs.push(rootDir);
    ...
    await expect(
      readdir(directories.responsesDir).then((entries) =>
        entries.filter((entry) => entry.endsWith(".tmp") || entry.includes(".pilot-write.lock")),
      ),
    ).resolves.toEqual([]);
  });

  it("rejects non-JSON request bodies and full queues at the bridge server", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-bridge-server-guards-"));
    cleanupDirs.push(rootDir);
  ...
  it("returns a 502 when the host response times out", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-bridge-timeout-"));
    cleanupDirs.push(rootDir);
  ...
  it("returns a 502 for malformed host response files", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-bridge-malformed-response-"));
    cleanupDirs.push(rootDir);
  ...
  it("reuses an already-uploaded bridge entrypoint when the remote file hash matches", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-bridge-sync-"));
    cleanupDirs.push(rootDir);
    ...
    await expect(readFile(path.posix.join(remoteAssetDir, "pilot-bridge-server.mjs"), "utf8")).resolves.toBe(expandedSource);
    await expect(
      readdir(remoteAssetDir).then((entries) =>
        entries.filter(
          (entry) =>
            entry.endsWith(".pilot-upload.b64") ||
            entry.endsWith(".partial") ||
            entry === ".pilot-bridge-upload.lock",
        ),
      ),
    ).resolves.toEqual([]);
  });
  ...
  it("rejects a corrupted bridge entrypoint upload without committing a torn remote file", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-bridge-sync-corrupt-"));
    cleanupDirs.push(rootDir);
    ...
    await expect(readFile(path.posix.join(remoteAssetDir, "pilot-bridge-server.mjs"), "utf8")).rejects.toThrow();
    await expect(
      readdir(remoteAssetDir).then((entries) =>
        entries.filter(
          (entry) =>
  ...
  it("test_process_session_script_skipped_when_remote_hash_matches: warm start with a matching remote hash writes 0 execs", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-hashskip-warm-"));
    cleanupDirs.push(rootDir);
    const remoteDir = path.join(rootDir, "runtime", "codex", "process-sessions");
    const remotePath = path.posix.join(remoteDir, "pilot-process-session-remote.mjs");
    const lockDir = path.posix.join(remoteDir, ".pilot-process-session-script.lock");
      ...
      readdir(remoteDir).then((entries) =>
        entries.filter(
          (entry) =>
            entry.endsWith(".pilot-upload.b64") ||
            entry.endsWith(".partial") ||
            entry === ".pilot-process-session-script.lock",
        ),
      ),
    ).resolves.toEqual([]);
  });
  ...
  it("test_process_session_script_rewritten_on_hash_mismatch: a mismatched remote hash still rewrites the script", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-hashskip-cold-"));
    cleanupDirs.push(rootDir);
    const remoteDir = path.join(rootDir, "runtime", "codex", "process-sessions");
    const remotePath = path.posix.join(remoteDir, "pilot-process-session-remote.mjs");
    const lockDir = path.posix.join(remoteDir, ".pilot-process-session-script.lock");
  ...
  it("fails loud when the hash-skip sync exec errors instead of silently re-uploading", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-hashskip-fail-"));
    cleanupDirs.push(rootDir);
    const remoteDir = path.join(rootDir, "runtime", "codex", "process-sessions");
    const remotePath = path.posix.join(remoteDir, "pilot-process-session-remote.mjs");
    const lockDir = path.posix.join(remoteDir, ".pilot-process-session-script.lock");
  ...
  it("marks command-managed bridge operations with the bridge execution channel", async () => {
    ...
    await client.makeDir("/workspace/.pilot-runtime/codex/pilot-bridge/queue");
  ...
  it("creates the bridge queue directories in one directory-creation exec", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-bridge-makedirs-"));
    cleanupDirs.push(rootDir);
  ...
  it("falls back to sequential makeDir when the queue client omits makeDirs", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-bridge-makedir-fallback-"));
    cleanupDirs.push(rootDir);
  ...
  it("times out a stalled poll, writes a 503, and surfaces a run-level error", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-bridge-hang-"));
    cleanupDirs.push(rootDir);
  ...
  it("finalizes an aborted handler that ignores the signal and never settles with a non-retryable 504 backstop", async () => {
    ...
    expect(parsed.headers?.["x-pilot-bridge-outcome"]).toBe("indeterminate");
    expect(responseWrites.some((write) => write.status === 503)).toBe(false);
    expect(responseWrites.some((write) => write.status === 502)).toBe(false);
  ...
  it("maps an indeterminate host outcome to a non-retryable 409 in the in-sandbox bridge server", () => {
    ...
    expect(source).toContain("x-pilot-bridge-outcome");
  ...
  it("trips the watchdog on a stalled poll, writes a 503, and surfaces a run-level error", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-bridge-watchdog-"));
    cleanupDirs.push(rootDir);
  ...
  it("processes a fast request with no false-positive timeout and no run-level error", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-bridge-fast-"));
    cleanupDirs.push(rootDir);
