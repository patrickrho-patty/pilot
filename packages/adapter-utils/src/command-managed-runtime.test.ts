...
describe("command managed runtime", () => {
  ...
  it("keeps the runtime overlay out of sandbox workspace sync by default", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-command-runtime-"));
    ...
    await writeFile(path.join(localWorkspaceDir, ".pilot-runtime", "state.json"), "{\"keep\":true}\n", "utf8");
    ...
    const runner = {
      execute: async (input: {
        ...
        if (
          ...
          args[1] = `printf '%s' \"$PILOT_TEST_STDIN\" | (${args[1]})`;
        }
        try {
    ...
    await expect(readFile(path.join(remoteWorkspaceDir, ".pilot-runtime", "state.json"), "utf8")).rejects
    ...
    await writeFile(path.join(remoteWorkspaceDir, ".pilot-runtime", "remote-state.json"), "{\"remote\":true}\n", "utf8");
    await prepared.restoreWorkspace();

    await expect(readFile(path.join(localWorkspaceDir, "README.md"), "utf8")).resolves.toBe("remote workspace\n");
    await expect(readFile(path.join(localWorkspaceDir, ".pilot-runtime", "state.json"), "utf8")).resolves
      .toBe("{\"keep\":true}\n");
    await expect(readFile(path.join(localWorkspaceDir, ".pilot-runtime", "remote-state.json"), "utf8")).rejects
      .toMatchObject({ code: "ENOENT" });
  ...
  it("stages runtime assets without replacing or restoring an in-place workspace", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-command-runtime-assets-only-"));
    cleanupDirs.push(rootDir);
    ...
    expect(prepared.assetDirs.home).toBe(path.join(remoteWorkspaceDir, ".pilot-runtime", "codex", "home"));
  ...
  it("stages each additional project into an isolated dir on the base64/tar transport, one failure skipped", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-command-runtime-additional-"));
    cleanupDirs.push(rootDir);
    ...
    const runtimeRootDir = path.posix.join(remoteWorkspaceDir, ".pilot-runtime", "claude");
  ...
  it("keeps adapter detection on the profile-backed shell path", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-command-runtime-detect-"));
    cleanupDirs.push(rootDir);
  ...
  it("runs setup commands from a stable root cwd when staging into a nested remote workspace dir", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-command-runtime-nested-"));
    cleanupDirs.push(rootDir);

    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteBaseDir = path.join(rootDir, "remote-base");
    const remoteWorkspaceDir = path.join(remoteBaseDir, ".pilot-runtime", "runs", "test", "workspace");
    await mkdir(localWorkspaceDir, { recursive: true });
  ...
  it("uploads a multi-MB payload in a single process and preserves exact bytes", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-command-write-"));
    cleanupDirs.push(rootDir);
  ...
  it("stages a single-file write to <path>.pilot-upload then atomically renames it (single-stream path)", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-command-atomic-"));
    cleanupDirs.push(rootDir);
    const remotePath = path.join(rootDir, "nested", "payload.bin");
    ...
    // stage-then-atomic-rename shape (temp .pilot-upload + `mv -f`).
    const script = (calls[0].args ?? []).join(" ");
    expect(script).toContain(`${remotePath}.pilot-upload`);
    expect(script).toContain(`trap cleanup EXIT`);
    expect(script).toContain(`mv -f`);
    expect(script.indexOf(".pilot-upload")).toBeLessThan(script.indexOf("mv -f"));
    expect(await readFile(remotePath, "utf8")).toBe("hello atomic\n");
  });

  it("cleans up a staged upload when rename fails", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-command-upload-cleanup-"));
    cleanupDirs.push(rootDir);
    const remotePath = path.join(rootDir, "nested", "payload.bin");
    ...
    runner.execute = async (input) => {
      ...
      if (script.includes("mv -f") && script.includes(".pilot-upload.")) {
    ...
    const uploadCall = calls.find((call) => (call.args ?? []).join(" ").includes(".pilot-upload."));
    expect(uploadCall).toBeDefined();
    const stagedPath = (uploadCall?.args ?? []).join(" ").match(/([/A-Za-z0-9_.-]+\.pilot-upload\.[A-Za-z0-9-]+)/)?.[1];
  ...
  it("stages a single-file write to a temp then renames it on the chunked fallback path too", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-command-atomic-fallback-"));
    cleanupDirs.push(rootDir);
    const remotePath = path.join(rootDir, "nested", "payload.bin");
    ...
    expect(scripts.some((script) => script.includes(`${remotePath}.pilot-upload`))).toBe(true);
  ...
  it("test_client_syncIn_present_even_without_native_runner_syncIn", () => {
    // Phase 2 (PIL-3222): `client.syncIn` is ALWAYS present so the caller can
  ...
  it("fallback syncIn tarballs+uploads a directory then runs post-upload commands in order", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-syncin-fallback-"));
    cleanupDirs.push(rootDir);
    ...
    const scripts = calls.map((call) => (call.args ?? []).join("\n"));
    const uploadIdx = scripts.findIndex((s) => s.includes(".pilot-syncin.tar") && s.includes("base64 -d"));
  ...
  it("fallback syncIn stages mode-constrained files before chmod and rename", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-syncin-mode-"));
    cleanupDirs.push(rootDir);
    ...
    expect(scripts[0]).toContain(targetFile + ".pilot-syncin.");
    expect(scripts[0]).toContain(".pilot-upload.");
    expect(scripts[1]).toContain("rm -rf");
    expect(scripts[1]).toContain(".pilot-upload.");
    expect(scripts[2]).toContain("chmod 640");
    expect(scripts[2]).toContain(targetFile + ".pilot-syncin.");
    expect(scripts[3]).toContain("mv -f");
    expect(scripts[3]).toContain(targetFile + ".pilot-syncin.");
    expect(scripts[3]).toContain(targetFile);
    expect(scripts[4]).toContain("rm -rf");
    expect(scripts[4]).toContain(targetFile + ".pilot-syncin.");
  });

  it("fallback syncIn cleans up a staged file when chmod fails before rename", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-syncin-cleanup-"));
    cleanupDirs.push(rootDir);
    const sourceFile = path.join(rootDir, "source.txt");
    const targetFile = path.join(rootDir, "target.txt");
    await writeFile(sourceFile, "payload\n", "utf8");
  ...
  it("test_single_stream_writeFile_collapses_roundtrips_under_96MiB", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-single-stream-collapse-"));
    cleanupDirs.push(rootDir);
  ...
  it("falls back to chunked upload progress when the runner cannot report mid-stream stdin progress", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-command-write-fallback-"));
    cleanupDirs.push(rootDir);
    const remotePath = path.join(rootDir, "nested", "payload.bin");
  ...
  it("falls back to bounded chunks when the runner does not explicitly opt in", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-command-write-fallback-no-progress-"));
    cleanupDirs.push(rootDir);
    const remotePath = path.join(rootDir, "nested", "payload.bin");
  ...
  it("downloads in bounded stdout chunks and reports monotonic byte progress to the total", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-command-read-"));
    cleanupDirs.push(rootDir);
