  ...
  if (detectCommand) {
    ...
    if (!recheck.timedOut && recheck.exitCode === 0) {
      if (input.onLog) {
        ...
        await input.onLog(
          "stderr",
          `[pilot] Install command ${reason} (${installCommand}) but ${detectCommand} is on PATH; continuing.\n`,
        );
      }
      return;
    }
...
export function runtimeAssetDir(
  ...
  return prepared.assetDirs[key] ?? path.posix.join(fallbackRemoteCwd, ".pilot-runtime", key);
}

function buildBridgeResponseHeaders(response: Response): Record<string, string> {
  const out: Record<string, string> = {};
  // Keep `x-pilot-bridge-outcome` in this list. The host marks a
  ...
  for (const key of ["content-type", "etag", "last-modified", "x-pilot-bridge-outcome"]) {
...
const PROCESS_SESSION_PROXY_SCRIPT = "pilot-process-session-proxy.mjs";
const PROCESS_SESSION_REMOTE_SCRIPT = "pilot-process-session-remote.mjs";
...
const PROCESS_SESSION_REMOTE_STREAM_SCRIPT = "pilot-process-session-remote-stream.mjs";
...
async function syncProcessSessionRemoteScript(input: {
  ...
  const { uploaded } = await syncRemoteTextFileWithHashSkip({
    ...
    lockDir: path.posix.join(input.remoteScriptDir, ".pilot-process-session-script.lock"),
...
export async function startAdapterExecutionTargetProcessSessionBridge(input: {
  ...
  const bridgeRuntimeDir = path.posix.join(
    input.runtimeRootDir?.trim() || path.posix.join(target.remoteCwd, ".pilot-runtime", input.adapterKey),
    "process-sessions",
  );
  ...
  if (!streamOutput) {
    await onLog("stdout", `[pilot] Starting ACP process session bridge in sandbox (${target.providerKey ?? "provider"}).\n`);
    const startResult = await runner.execute({
      command: shellCommand,
      args: shellCommandArgs(
        [
          `mkdir -p ${shellQuote(stdinDir)} ${shellQuote(eventsDir)}`,
          `PILOT_PROCESS_SESSION_DIR=${shellQuote(sessionDir)} ` +
            `PILOT_PROCESS_SESSION_COMMAND_B64=${shellQuote(commandPayload)} ` +
  ...
  const proxyDir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-process-session-proxy-"));
  ...
  const poll = async () => {
    ...
    try {
      ...
      await onLog("stderr", `[pilot] ACP process session bridge poll failed: ${message}\n`);
  ...
  if (streamOutput) {
    ...
    await onLog(
      "stdout",
      `[pilot] Starting streamed ACP process session bridge in sandbox (${target.providerKey ?? "provider"}).\n`,
    );
...
const stdinMaxParseRetries = (() => {
  const raw = Number.parseInt(process.env.PILOT_PROCESS_SESSION_STDIN_MAX_RETRIES || "", 10);
...
const sessionDir = process.env.PILOT_PROCESS_SESSION_DIR ?? process.env.PILOT_PROCESS_SESSION_DIR;
const commandPayload = process.env.PILOT_PROCESS_SESSION_COMMAND_B64 ?? process.env.PILOT_PROCESS_SESSION_COMMAND_B64;
...
export async function startAdapterExecutionTargetPilotBridge(input: {
  ...
  if (hostApiToken.length === 0) {
    throw new Error("Sandbox bridge mode requires a host-side Pilot API token.");
  }

  const runtimeRootDir =
    input.runtimeRootDir?.trim().length
      ? input.runtimeRootDir.trim()
      : path.posix.join(target.remoteCwd, ".pilot-runtime", input.adapterKey);
  const bridgeRuntimeDir = path.posix.join(runtimeRootDir, "pilot-bridge");
  ...
  // PILOT_RUNTIME_API_URL / PILOT_API_URL exports now prefer a
  ...
  // exports PILOT_LISTEN_HOST / PILOT_LISTEN_PORT before any run
  ...
  await onLog(
    "stdout",
    `[pilot] Starting sandbox callback bridge for ${input.adapterKey} in ${bridgeRuntimeDir}.\n`,
  );
  ...
  try {
    ...
    // PILOT_BRIDGE_DEBUG opts into verbose stdout logs of every bridge
    ...
    // setup on the active `bridge.pilot` step, and runs each request under
    ...
    worker = await startSandboxCallbackBridgeWorker({
      ...
      handleRequest: async (request, options) => {
        ...
        if (bridgeDebugEnabled) {
          await onLog(
            "stdout",
            `[pilot] Bridge proxy ${method} ${request.path}${request.query ? `?${request.query}` : ""}\n`,
          );
        }
        ...
        headers.set("x-pilot-run-id", input.runId);
        ...
        if (bridgeDebugEnabled) {
          await onLog(
            "stdout",
            `[pilot] Bridge proxy response ${response.status} for ${method} ${request.path}${request.query ? `?${request.query}` : ""}\n`,
          );
        }
  ...
  if (target.transport === "sandbox" && target.streamRunLogs !== false) {
    ...
    await onLog("stdout", "[pilot] Sandbox run log streaming enabled for this run.\n");
  }
