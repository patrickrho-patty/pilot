...
describe("git workspace sync", () => {
  ...
  it("delegates every host-side full-tree enumeration to the registered scheduler", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-git-scheduler-hook-"));
  ...
  async function createRepo(rootDir: string): Promise<string> {
    ...
    await git(repo, ["config", "user.name", "Pilot Test"]);
    await git(repo, ["config", "user.email", "test@pilot.dev"]);
  ...
  it("creates a shallow standalone clone from the local HEAD snapshot", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-git-sync-"));
    cleanupDirs.push(rootDir);
    const repo = await createRepo(rootDir);
  ...
  it("copies the workspace origin remote into the shallow clone", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-git-origin-"));
    cleanupDirs.push(rootDir);
    const repo = await createRepo(rootDir);
  ...
  it("scrubs credentials from the origin remote before copying it", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-git-origin-scrub-"));
    cleanupDirs.push(rootDir);
    const repo = await createRepo(rootDir);
  ...
  it("leaves the shallow clone remote-less when the workspace has no origin", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-git-no-origin-"));
    cleanupDirs.push(rootDir);
    const repo = await createRepo(rootDir);
  ...
  it("drops a filesystem-path origin instead of copying it into the shallow clone", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-git-path-origin-"));
    cleanupDirs.push(rootDir);
    const repo = await createRepo(rootDir);
  ...
  it("pushes new commits from the shallow clone to an origin that holds the base commit", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-git-shallow-push-"));
    cleanupDirs.push(rootDir);
    const repo = await createRepo(rootDir);
    ...
    await withShallowGitWorkspaceClone({
      ...
      await git(cloneDir, ["config", "user.name", "Pilot Sandbox"]);
      await git(cloneDir, ["config", "user.email", "sandbox@pilot.dev"]);
  ...
  it("builds thin git delta bundles relative to the imported base", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-git-delta-"));
    cleanupDirs.push(rootDir);
    const repo = await createRepo(rootDir);
    const baseHead = await git(repo, ["rev-parse", "HEAD"]);
    const snapshot = await readGitWorkspaceSnapshot(repo);
    ...
    await withShallowGitWorkspaceClone({
      ...
      await git(remoteDir, ["config", "user.name", "Pilot Remote"]);
      await git(remoteDir, ["config", "user.email", "remote@pilot.dev"]);
  ...
  it("imports a diverged sandbox HEAD even when the host no longer holds baseSha", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-git-diverge-"));
    cleanupDirs.push(rootDir);
    ...
    await git(sandbox, ["config", "user.name", "Pilot Remote"]);
    await git(sandbox, ["config", "user.email", "remote@pilot.dev"]);
  ...
  it("re-exports a full bundle that imports when the host holds neither baseSha nor the merge-base", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-git-ancestor-"));
    cleanupDirs.push(rootDir);
  ...
  it("falls back to a full self-contained bundle when the sandbox lacks baseSha", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-git-full-"));
    cleanupDirs.push(rootDir);
  ...
  it("creates the concurrent-history merge commit with a deterministic identity", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-git-merge-identity-"));
    cleanupDirs.push(rootDir);
    ...
    const setupIdentity = ["-c", "user.name=Setup", "-c", "user.email=setup@pilot.dev"];
    const repo = path.join(rootDir, "repo");
    await mkdir(repo, { recursive: true });
    await git(repo, ["init"]);
    await git(repo, ["checkout", "-b", "main"]);
    ...
    expect(await git(repo, ["log", "-1", "--format=%an|%ae|%cn|%ce"]))
      .toBe("Pilot|noreply@pilot.test|Pilot|noreply@pilot.test");
    expect(await git(repo, ["log", "-1", "--format=%s"]))
      .toBe(`Pilot remote git sync merge ${importedHead.slice(0, 12)}`);
  ...
  it("grafts an imported head onto the current head when histories share no ancestor", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-git-graft-"));
    cleanupDirs.push(rootDir);
    const setupIdentity = ["-c", "user.name=Setup", "-c", "user.email=setup@pilot.dev"];
    const repo = path.join(rootDir, "repo");
    await mkdir(repo, { recursive: true });
    ...
    expect(body).toContain(`Pilot remote git sync graft ${importedHead.slice(0, 12)}`);
  ...
  it("does not graft when merge-base fails for a reason other than missing ancestry", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-git-no-graft-"));
    cleanupDirs.push(rootDir);
    const setupIdentity = ["-c", "user.name=Setup", "-c", "user.email=setup@pilot.dev"];
    const repo = path.join(rootDir, "repo");
    await mkdir(repo, { recursive: true });
