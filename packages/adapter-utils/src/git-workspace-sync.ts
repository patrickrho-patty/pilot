...
export async function withShallowGitWorkspaceClone<T>(
  ...
  const cloneDir = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-git-workspace-"));
  const tempRef = `refs/pilot/git-sync/import/${randomUUID()}`;
  try {
...
export function createImportedGitRef(scope = "remote"): string {
  return `refs/pilot/git-sync/imported/${scope}/${randomUUID()}`;
}

export function createRemoteGitExportRef(scope = "remote"): string {
  return `refs/pilot/git-sync/export/${scope}/${randomUUID()}`;
}
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    ...
    if (noCommonAncestor) {
      ...
      const graftCommit = await createUnrelatedHistoryGraftCommit({
        ...
        syncLabel: "Pilot remote git sync",
      });
      try {
    ...
    const mergeCommit = await runLocalGit(
      ...
      [
        ...
        input.importedHead,
        "-m",
        `Pilot remote git sync merge ${input.importedHead.slice(0, 12)}`,
      ],
      {
        timeout: 60_000,
        maxBuffer: 64 * 1024,
