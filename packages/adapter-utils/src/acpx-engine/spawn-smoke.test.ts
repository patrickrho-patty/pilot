...
it("spawns a real Node ACP agent with per-session env on this platform", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-acpx-spawn-smoke-"));
  tempRoots.push(root);
  ...
  const result = await execute({
    ...
    config: {
      ...
      stateDir,
      cwd: repoRoot,
      env: { PILOT_ACPX_SPAWN_SMOKE: "spawn-ok" },
    },
    context: {},
  ...
  expect(stderr).toContain("pilot-acp-echo-agent started");
});
...
it("captures the Node error shape for a host-invalid spawn cwd", async () => {
  ...
  const missingCwd = path.join(os.tmpdir(), "pilot-acpx-missing-spawn-cwd", "nested", "does-not-exist");
