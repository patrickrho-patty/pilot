...
export interface SandboxRunSiteOptions {
  ...
  readonly measureBridgeStep: <T>(step: "bridge.pilot" | "bridge.process-session", run: () => Promise<T>) => Promise<T>;
  /**
  ...
  readonly onPilotBridgeLog: () => Promise<void>;
...
export function createSandboxRunSite(options: SandboxRunSiteOptions): SandboxRunSite {
  ...
  return {
    ...
    async startTransport(): Promise<SandboxRunSiteTransport> {
      ...
      const pilotStart = options.measureBridgeStep("bridge.pilot", () =>
        options.startPilotBridge(stagedRootDir),
      );
      ...
      const finalizeLaunchEnv = (): Promise<Record<string, string>> =>
        (launchEnvPromise ??= (async () => {
          ...
          if (pilot) {
            ...
            await options.onPilotBridgeLog();
          }
