...
import type { AdapterExecutionContext, AdapterRuntimeMcpAccess } from "@pilotai/adapter-utils";
import {
...
vi.mock("@pilotai/adapter-utils/execution-target", async (importActual) => {
  const actual = await importActual<typeof import("@pilotai/adapter-utils/execution-target")>();
  return {
    ...actual,
...
async function makeTempRoot() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-acpx-skills-"));
  tempRoots.push(root);
  return root;
}
...
const ALLOWED_RUN_SPAN_ATTRIBUTE_KEYS = new Set<string>([
  "pilot.task.run.run_id",
  "pilot.task.run.wall_ms",
]);
...
describe("shared ACPX engine runtime behavior", () => {
  ...
  it("warns when runtime settings replace malformed user CODEX_CONFIG", async () => {
    ...
    expect(logs).toContainEqual({
      stream: "stderr",
      text: "[pilot] Ignoring invalid user CODEX_CONFIG while applying runtime Codex settings; expected a JSON object.\n",
    });
  });
  ...
  it("includes Pilot env and API access notes in the ACPX prompt without leaking the token", async () => {
    const { meta } = await runExecutor(
    ...
    expect(prompt).toContain("Pilot runtime note:");
    ...
    expect(prompt).toContain("Pilot API access note:");
    expect(prompt).toContain('PILOT_API_BASE="${PILOT_API_URL%/}"; PILOT_API_BASE="${PILOT_API_BASE%/api}"');
    expect(prompt).toContain("$PILOT_API_BASE/api/agents/me");
    expect(prompt).toContain("$PILOT_API_BASE/api/issues/$PILOT_TASK_ID");
    expect(prompt).toContain("x-pilot-Run-Id");
    expect(prompt).not.toContain("$PILOT_API_URL/api/");
  ...
  it("does not show a scoped issue API command when the task id is unavailable", async () => {
    ...
    expect(prompt).not.toContain("$PILOT_API_BASE/api/issues/$PILOT_TASK_ID");
  });
  ...
  it.skipIf(process.platform === "win32")("materializes ACPX Claude skills without symlinked descendants", async () => {
    ...
    const { meta } = await runExecutor({
      agent: "claude",
      stateDir,
      pilotRuntimeSkills: [skill],
      pilotSkillSync: { desiredSkills: [skill.key] },
    });
  ...
  it.skipIf(process.platform === "win32")("revokes removed ACPX Codex skills and skips symlinked descendants", async () => {
    ...
    const baseConfig = {
      ...
      pilotRuntimeSkills: [keep, remove],
    };

    await runExecutor({
      ...baseConfig,
      pilotSkillSync: { desiredSkills: [keep.key, remove.key] },
    });
    expect(await pathExists(path.join(codexHome, "skills", remove.runtimeName, "SKILL.md"))).toBe(true);

    await runExecutor({
      ...baseConfig,
      pilotSkillSync: { desiredSkills: [keep.key] },
    });
  ...
  it.skipIf(process.platform === "win32")("removes legacy ACPX Codex skill symlinks when a skill is no longer desired", async () => {
    ...
    await runExecutor({
      ...
      stateDir: path.join(root, "state"),
      env: { CODEX_HOME: codexHome },
      pilotRuntimeSkills: [legacy],
      pilotSkillSync: { desiredSkills: [] },
    });
    ...
    const { sessionInputs } = await runExecutor(
      {
        ...
        env: {
          ...
          // PILOT_API_KEY is never accepted from config.
          PILOT_API_KEY: "config-key",
          // A PILOT_*-named key the harness does not assign flows through.
  ...
  it("busts the session fingerprint when resolved adapter env changes but not across wakes", async () => {
    ...
    // per-wake PILOT_* churn does not needlessly reset the session.
    expect(fp(sameEnvNewWake)).toBe(fp(first));
  });

  it("busts the session fingerprint when a stable configured PILOT_* value rotates", async () => {
    const root = await makeTempRoot();
    const stateDir = path.join(root, "state");
    const baseConfig = { agentCommand: "node ./fake-acp.js", stateDir };

    // A configured PILOT_*-named value the harness does not assign (e.g. a
  ...
  it("busts the session fingerprint when the referenced-project set or pinned checkout changes", async () => {
    ...
    const withReferenced = (additional: unknown[]) => ({
      context: {
        taskId: "issue-1",
        wakeReason: "issue_assigned",
        pilotWorkspace: { cwd, realization: { additional } },
      },
    });
    const projectA = {
      path: "/host/project-a",
  ...
  it("busts the session fingerprint when referenced-project files change at the same host path", async () => {
    ...
    const withProject = () => ({
      context: {
        taskId: "issue-1",
        wakeReason: "issue_assigned",
        pilotWorkspace: { cwd, realization: { additional: [referencedProject] } },
      },
    });
    const fp = (r: { result: { sessionParams?: unknown } }) =>
      (r.result.sessionParams as { configFingerprint?: string } | undefined)?.configFingerprint;
  ...
  it("shapes ACPX session env for remote execution identities", async () => {
    ...
    const { sessionInputs, runtimeOptions } = await runExecutor(
      ...
      { context: { pilotWorkspace: { cwd: localCwd, workspaceWorktreePath: localCwd } }, executionTarget: { kind: "remote", transport: "ssh", remoteCwd } },
    );
    const env = (sessionInputs[0]!.sessionOptions as { env: Record<string, string> }).env;
    ...
    const runner = createLocalSandboxRunner(
      (input: { args?: string[]; env?: Record<string, string> }) => {
        if (input.env?.PILOT_SANDBOX_EXEC_CHANNEL === "bridge") {
          ...
          const match = script.match(/PILOT_PROCESS_SESSION_COMMAND_B64='([^']+)'/);
          if (match) {
  ...
  it("passes Pilot env through ACPX session options instead of process.env", async () => {
  ...
  it("writes a Pilot-managed .claude/settings.local.json for the claude agent so it can reach the Pilot API", async () => {
    const root = await makeTempRoot();
    const stateDir = path.join(root, "state");
    ...
    const { meta } = await runExecutor(
      ...
      { context: { pilotWorkspace: { cwd, agentHome: path.join(root, "agent-home") } } },
    );
    ...
    expect(allow).toContain(`Bash(${cwd}/scripts/pilot-issue-update.sh:*)`);
    ...
    const note = (meta[0]?.commandNotes as string[] | undefined)?.find((entry) =>
      entry.includes("Pilot-managed Claude settings"),
    );
    expect(note).toBeTruthy();
  });

  it("merges Pilot allowlist into an existing .claude/settings.local.json without losing user entries", async () => {
    const root = await makeTempRoot();
    const stateDir = path.join(root, "state");
    const cwd = path.join(root, "worktree");
  ...
  it("injects runtime MCP servers and fingerprints their identity without persisting bearer tokens", async () => {
    ...
    const server = {
      name: "github",
      url: "https://pilot.example/api/tool-gateway/gateways/github/mcp",
...
describe("findAncestorBin", () => {
  ...
  it("finds the binary in the start directory's own node_modules/.bin", async () => {
    const root = await makeTempRoot();
    const packageDir = path.join(root, "node_modules", "@pilotai", "adapter-utils");
...
describe("gemini ACP flag selection", () => {
  ...
  it("applies the 4h sandbox backstop when timeoutSec is unset on a sandbox execution target", async () => {
    ...
    expect(startLine!.text).toContain(
      `[pilot] Adapter execution timeout: timeoutSec=${DEFAULT_REMOTE_SANDBOX_ADAPTER_TIMEOUT_SEC} ` +
...
describe("ACPX engine remote sandbox staging seam (PR 1: workspace + cwd)", () => {
  ...
  it("test_remote_buildRuntime_crosses_staging_seam", async () => {
    ...
    expect(String(pilotArgs.runtimeRootDir)).toContain(".pilot-runtime");
  ...
  it("hands the merged pilot env to the process-session launch when the setups overlap", async () => {
    const { stateDir, localCwd, remoteCwd, executionTarget } = await setupRemoteSandbox();
  ...
  it("stops the process-session bridge when the pilot bridge fails under concurrency", async () => {
    ...
    vi.mocked(startAdapterExecutionTargetPilotBridge).mockImplementationOnce(async () => {
      throw new Error("pilot bridge boom");
    });
    ...
    ).rejects.toThrow("pilot bridge boom");
...
describe("ACPX engine remote managed-home seam (PR 2: per-adapter home seed)", () => {
  ...
  it("test_remote_seam_stages_assets_and_env_remap_reaches_process", async () => {
    ...
    expect(remoteAssetDir).toContain(".pilot-runtime");
  ...
  it("test_remote_seam_surfaces_referenced_project_staging_failure_reason", async () => {
    ...
    expect(failureLine?.text).toBe(
      "[pilot] Referenced project proj-x failed to stage; the run continues without it: extract failed: boom\n",
    );
  });
});
...
describe("ACPX engine sandbox-start spans (opt-in root + child parenting)", () => {
  ...
  it("test_task_run_span_opens_and_ends_once_for_remote", async () => {
    ...
    // the span.
    expect(runSpan.attributes["pilot.task.run.run_id"]).toMatch(/^[0-9a-f]{12}$/);
    expect(String(runSpan.attributes["pilot.task.run.run_id"])).not.toContain("run-");
  });
  ...
  it("parents both concurrent bridge spans to the root (neither orphans)", async () => {
    ...
    const rootSpan = spans.find((span) => span.name === "sandbox.startup");
    expect(rootSpan).toBeTruthy();
    const pilot = spans.find((span) => span.name === "bridge.pilot");
...
describe("ACPX engine per-step startup timing (run.startup.step events)", () => {
  ...
  it("does not emit startup-step events on a local (non-sandbox) run except workspace.resolve", async () => {
    ...
    expect(emitted.has("bridge.pilot")).toBe(false);
...
describe("ACPX engine run lifecycle corrections (F1: settle every failure after buildRuntime)", () => {
  ...
  it("test_prompt_build_failure_returns_error_result_with_phase_prepare_turn", async () => {
    ...
    Object.defineProperty(context, "pilotSessionHandoffMarkdown", {
      enumerable: false,
      get() {
...
describe("ACPX engine run lifecycle corrections (F3: one teardown error policy)", () => {
  ...
  function stubBridges() {
    ...
    return { pilotStops: pilotStops, processStops, anyStopped, stoppedCount };
  }
  ...
  it("test_teardown_continues_after_one_teardown_step_fails", async () => {
    const { stateDir, localCwd, executionTarget } = await setupRemoteSandbox();
    const { pilotStops: pilotStops, processStops, anyStopped } = stubBridges();
    const stagingLocks = new Map<string, Promise<unknown>>();
    const logs: Array<{ stream: string; text: string }> = [];
    const execute = createAcpxEngineExecutor({
      stagingLocks,
  ...
  it("test_result_mapping_throw_after_close_does_not_rerun_teardown", async () => {
    const { stateDir, localCwd, executionTarget } = await setupRemoteSandbox();
    const { pilotStops: pilotStops, processStops, stoppedCount } = stubBridges();
    let closeCount = 0;
    const execute = createAcpxEngineExecutor({
      warmHandles: new Map(),
      stagedRuntimes: new Map(),
  ...
  it("test_flush_child_stderr_runs_on_every_exit_path", async () => {
    ...
    try {
      ...
      for (const scenario of scenarios) {
        const root = await makeTempRoot();
        const marker = `pilot-flush-probe-${scenario.name}`;
        const execute = createAcpxEngineExecutor({
          createRuntime: (options) => {
