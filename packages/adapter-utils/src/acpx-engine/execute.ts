...
} from "@pilotai/adapter-utils/execution-target";
import {
...
import { shellQuote } from "@pilotai/adapter-utils/ssh";
import {
  createAcpRuntime,
  createAgentRegistry,
  createRuntimeStore,
...
const PILOT_MANAGED_CODEX_SKILLS_MANIFEST = ".pilot-managed-skills.json";
 ...
 * `@pilotai/adapter-utils`, a dependency of those packages — cannot import
...
interface AcpxPreparedRuntime {
  ...
  pilotBridge: AdapterExecutionTargetPilotBridgeHandle | null;
  ...
  pilotClaudeSettings: PilotClaudeSettingsResult | null;
...
export function buildSessionKey(identity: SessionKeyIdentity, fingerprint: string): string {
  return `pilot:${identity.companyId}:${identity.agentId}:${identity.taskKey}:${fingerprint}`;
}
...
function defaultPilotInstanceDir(): string {
  const home = process.env.PILOT_HOME?.trim() || path.join(os.homedir(), ".pilot");
...
async function prepareManagedCodexHome(input: {
  ...
  await onLog(
    "stdout",
    `[pilot] Using Pilot-managed ACPX Codex home "${targetHome}" (seeded from "${sourceHome}").\n`,
  );
  return targetHome;
}
...
async function buildSkillSetKey(input: {
  ...
  const hash = createHash("sha256");
  hash.update(`pilot-acpx-${input.label}-skills:v1\n`);
...
async function prepareClaudeSkillRuntime(input: {
  ...
  for (const entry of selectedSkills) {
    ...
    try {
      ...
      if (result.skippedSymlinks.length > 0) {
        await input.onLog(
          "stdout",
          `[pilot] Materialized ACPX Claude skill "${entry.runtimeName}" into ${skillsHome} and skipped ${result.skippedSymlinks.length} symlink(s).\n`,
        );
      }
    } catch (err) {
      await input.onLog(
        "stderr",
        `[pilot] Failed to materialize ACPX Claude skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
  ...
  const promptInstructions = selectedSkills.length > 0
    ? [
        "Pilot has materialized selected runtime skills for this ACPX Claude session.",
  ...
  return {
    ...
    commandNotes: selectedSkills.length > 0
      ? [`Materialized ${selectedSkills.length} Pilot skill(s) for ACPX Claude at ${skillsHome}.`]
      : [],
  };
}
...
async function reconcileManagedCodexSkills(input: {
  ...
  for (const name of managed) {
    ...
    if (await removeSkillTarget(path.join(input.skillsHome, name))) {
      await input.onLog("stdout", `[pilot] Revoked ACPX Codex skill "${name}" from ${input.skillsHome}\n`);
    }
  }
  ...
  for (const entry of input.allSkills) {
    ...
    if (await removeSkillTarget(target)) {
      await input.onLog("stdout", `[pilot] Revoked legacy ACPX Codex skill "${entry.runtimeName}" from ${input.skillsHome}\n`);
    }
  }
  ...
  for (const name of managed) {
    ...
    if (await removeSkillTarget(path.join(input.skillsHome, name))) {
      await input.onLog("stdout", `[pilot] Revoked unavailable ACPX Codex skill "${name}" from ${input.skillsHome}\n`);
    }
  }
}
  ...
  for (const entry of selectedSkills) {
    ...
    try {
      ...
      if (result.skippedSymlinks.length > 0) {
        await input.onLog(
          "stdout",
          `[pilot] Materialized ACPX Codex skill "${entry.runtimeName}" into ${skillsHome} and skipped ${result.skippedSymlinks.length} symlink(s).\n`,
        );
      }
    } catch (err) {
      await input.onLog(
        "stderr",
        `[pilot] Failed to inject ACPX Codex skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
...
async function prepareGeminiSkillRuntime(input: {
  ...
  for (const skillName of removedSkills) {
    await input.onLog("stdout", `[pilot] Removed maintainer-only ACPX Gemini skill "${skillName}" from ${skillsHome}\n`);
  }
  ...
  for (const entry of selectedSkills) {
    ...
    try {
      ...
      if (result === "created" || result === "repaired") {
        await input.onLog(
          "stdout",
          `[pilot] ${result === "repaired" ? "Repaired" : "Linked"} ACPX Gemini skill "${entry.runtimeName}" into ${skillsHome}\n`,
        );
      }
    } catch (err) {
      if (isErrnoException(err, "EPERM")) {
        ...
        await input.onLog(
          "stdout",
          `[pilot] Copied ACPX Gemini skill "${entry.runtimeName}" into ${skillsHome} because symlinks are unavailable.${result.skippedSymlinks.length > 0 ? ` Skipped ${result.skippedSymlinks.length} nested symlink(s).` : ""}\n`,
        );
        continue;
      }
      await input.onLog(
        "stderr",
        `[pilot] Failed to link ACPX Gemini skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
...
async function writePilotClaudeSettings(input: {
  ...
  const pilotAllow = uniqueSorted([
    ...
    "Bash(env:*)",
    "Bash(env)",
    `Bash(${input.cwd}/scripts/pilot-issue-update.sh:*)`,
    `Bash(${input.cwd}/scripts/pilot:*)`,
  ]);
...
async function stageAcpRemoteRuntime(input: {
  ...
  await input.onLog(
    "stdout",
    `[pilot] Syncing workspace to ${describeAdapterExecutionTarget(input.target)}.\n`,
  );
...
async function disposeFreshStagedRuntime(input: {
  ...
  try {
    ...
    await input.onLog(
      "stderr",
      `[pilot] Failed to dispose the fresh staged runtime after a managed-home seam error: ${
...
async function buildRuntime(input: {
  ...
  const workspaceContext = parseObject(context.pilotWorkspace);
  const secretsContext = parseObject(context.pilotSecrets);
  ...
  // Referenced-project workspace hints exposed to the agent through PILOT_WORKSPACES_JSON. The
  ...
  const workspaceHints = Array.isArray(context.pilotWorkspaces)
    ? context.pilotWorkspaces.filter(
  ...
  const linkedIssueIds = Array.isArray(context.issueIds)
    ...
    : [];
  const wakePayloadJson = stringifyPilotWakePayload(context.pilotWake);
  ...
  // user/adapter-configured env flows through this loop; per-wake PILOT_*
  // runtime vars (PILOT_RUN_ID, wake/approval ids, ...) were assigned to
  ...
  for (const [key, value] of Object.entries(shapedEnvConfig)) {
    ...
    // Runtime PILOT_* always wins over config: skip a PILOT_* key that
  ...
  if (acpxAgent === "codex") {
    ...
    if (codexStartupConfig.invalidExistingConfig) {
      await input.ctx.onLog(
        "stderr",
        "[pilot] Ignoring invalid user CODEX_CONFIG while applying runtime Codex settings; expected a JSON object.\n",
      );
    }
  ...
  if (acpxAgent === "claude") {
    ...
    skillCommandNotes.push(
      `Wrote Pilot-managed Claude settings to ${pilotClaudeSettings.filePath} (defaultMode=${pilotClaudeSettings.defaultMode}${
    ...
    if (desired.length > 0) {
      skillCommandNotes.push("Selected Pilot skills are tracked only; ACPX custom commands do not expose a runtime skill contract yet.");
    }
  }
  ...
  const fingerprintIdentity: SessionFingerprintIdentity = {
    ...
    pilotClaudeSettings: pilotClaudeSettings
      ? {
    ...
    // plain, secret_ref, and stable PILOT_* config such as an explicit
    // PILOT_API_KEY) into the fingerprint so a change to any forwarded value
  ...
  if (useRemoteProcessSession && executionTarget?.kind === "remote") {
    ...
    sandboxSite = createSandboxRunSite({
      ...
      onReuseLog: () =>
        input.ctx.onLog(
          "stdout",
          "[pilot] Reusing the staged in-sandbox runtime for this resumed session (no workspace re-ship / managed-home re-seed).\n",
        ),
      startPilotBridge: (runtimeRootDir) =>
        startAdapterExecutionTargetPilotBridge({
          runId,
      ...
      onPilotBridgeLog: () =>
        input.ctx.onLog("stdout", "[pilot] Sandbox ACP API callback bridge enabled for this run.\n"),
...
async function applySessionConfigOptions(input: {
  ...
  if (!input.runtime.setConfigOption) {
    ...
    await input.onLog("stderr", `[pilot] ${message}\n`);
  ...
  for (const option of options) {
    ...
    await input.onLog(
      "stdout",
      `[pilot] Applied ACPX ${input.prepared.acpxAgent} config ${option.key}=${option.value}\n`,
    );
  }
}
...
function renderPilotEnvNote(env: Record<string, string>): string {
  const pilotKeys = Object.keys(env)
    .filter((key) => key.startsWith("PILOT_"))
    .sort();
...
function renderApiAccessNote(env: Record<string, string>): string {
  ...
  const lines = [
    "Pilot API access note:",
    "Use terminal commands with curl to make Pilot API requests.",
    "Normalize the base URL before adding API paths:",
    `  PILOT_API_BASE="\${PILOT_API_URL%/}"; PILOT_API_BASE="\${PILOT_API_BASE%/api}"`,
    "GET example:",
    `  curl -s -H "Authorization: Bearer $PILOT_API_KEY" "$PILOT_API_BASE/api/agents/me"`,
  ];
  if (env.PILOT_TASK_ID) {
    lines.push(
      "Scoped issue comment example:",
      `  curl -s -X POST -H "Authorization: Bearer $PILOT_API_KEY" -H "Content-Type: application/json" -H "x-pilot-Run-Id: $PILOT_RUN_ID" -d '{"body":"Status update from agent."}' "$PILOT_API_BASE/api/issues/$PILOT_TASK_ID/comments"`,
    );
  } else {
...
async function buildPrompt(ctx: AdapterExecutionContext, resumedSession: boolean, env: Record<string, string>): Promise<{
  ...
  if (instructionsFilePath) {
    try {
      ...
      await onLog(
        "stderr",
        `[pilot] Warning: could not read agent instructions file "${instructionsFilePath}": ${reason}\n`,
      );
  ...
  const wakePrompt = renderPilotWakePrompt(context.pilotWake, {
    resumedSession,
  ...
  const sessionHandoffNote = asString(context.pilotSessionHandoffMarkdown, "").trim();
...
async function emitAcpxFailure(input: {
  ...
  if (childStderrTail) {
    await ctx.onLog(
      "stderr",
      `[pilot] ACPX child stderr tail (${phase}):\n${childStderrTail}\n`,
    );
  }
  await emitAcpxLog(ctx, {
    type: "acpx.error",
 ...
 * prefix. */
const RUN_ROOT_SPAN_ATTR_PREFIX = "pilot.task.run.";
...
const TURN_SPAN_ATTR_PREFIX = "pilot.agent.turn.";
  ...
  return async function executeAcpxEngine(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
    ...
    const hostSite = createHostRunSite({
      ...
      closeWarmEntry: async (entry) => {
        await entry.runtime
          .close({ handle: entry.handle, reason: "pilot idle cleanup", discardPersistentState: false })
          .catch(() => {});
    ...
    try {
      ...
      const recordTeardownError = async (step: string, teardownErr: unknown) => {
        ...
        await ctx
          .onLog("stderr", `[pilot] ACPX teardown step "${step}" failed: ${reason}\n`)
          .catch(() => {});
      };
      ...
      const startup = async (): Promise<StartupResult> => {
       try {
        ...
        for (const failure of referencedProjectStagingFailures) {
          await ctx.onLog(
            "stderr",
            `[pilot] Referenced project ${failure.projectId} failed to stage; the run continues without it: ${failure.error}\n`,
          );
        }
        ...
        await ctx.onLog(
          "stderr",
          `[pilot] ${formatAdapterExecutionTimeoutStartLogLine(prepared.timeoutResolution)}\n`,
        );
        ...
        if (!canResume && asString(previousParams.runtimeSessionName, "")) {
          await ctx.onLog(
            "stdout",
            `[pilot] ACPX session "${asString(previousParams.runtimeSessionName, "")}" does not match the current agent/cwd/mode/runtime identity; starting fresh in "${prepared.cwd}".\n`,
          );
        }
        ...
        try {
          if (!handle) {
            try {
              ...
              await ctx.onLog(
                "stdout",
                `[pilot] ACPX resume session "${resumeSessionId}" is unavailable; retrying with a fresh session.\n`,
              );
          ...
          runtimeSettlement = {
            ...
            reason: "pilot handshake cleanup",
        ...
        if (!handle) {
          ...
          runtimeSettlement = {
            ...
            reason: "pilot missing-handle cleanup",
            discardPersistentState: false,
            dropWarmEntry: false,
            recordCloseError: true,
            cancelTurnReason: null,
      ...
      const stepPromptBuild = async (_signal: AbortSignal): Promise<void> => {
        ...
        if (ctx.onMeta) {
          await ctx.onMeta({
            ...
            cwd: prepared.cwd,
            commandNotes: [
              `ACPX runtime embedded in Pilot with ${prepared.mode} session mode.`,
      ...
      const stepTurnFinalize = async (
        ...
        runtimeSettlement = {
          ...
          handle: sessionHandle,
          reason: timedOut
            ? "pilot timeout cleanup"
            : failedTurn
              ? `pilot turn ${terminal.status}`
              : "pilot completed turn cleanup",
        ...
        runtimeSettlement = {
          mode: "direct",
          handle: sessionHandle,
          reason: timedOut ? "pilot timeout cleanup" : "pilot error cleanup",
      ...
      const LIVE_RUN_SCOPED_API_KEY = "pilot-run-scoped-api-key";
