...
describe("ACPX engine startup characterization", () => {
  ...
  describe("session fingerprint and session key", () => {
    ...
    it("forms the session key as pilot:company:agent:taskKey:fingerprint and embeds the fingerprint", async () => {
      const root = await makeTempRoot();
      ...
      expect(sessionKey).toBe(`pilot:company-1:agent-1:default:${fp}`);
    });
    ...
    it("keeps the fingerprint stable across two identical runs and a same-config new wake", async () => {
      ...
      // Per-wake PILOT_* churn does not reset the session fingerprint.
