...
import type { PreparedAdapterExecutionTargetRuntime } from "@pilotai/adapter-utils/execution-target";

function makeStagedRuntime(id: string): PreparedAdapterExecutionTargetRuntime {
  return {
    runtimeRootDir: `/remote/${id}/.pilot-runtime/acpx`,
...
function makeSite(overrides: Partial<SandboxRunSiteOptions> = {}) {
  ...
  const options: SandboxRunSiteOptions = {
    ...
    onReuseLog: async () => {},
    startPilotBridge: async () => {
      bridgeCalls.push("pilot:start");
...
describe("sandbox run site", () => {
  it("test_sandbox_site_registers_staging_bridges_and_lease_in_ledger", async () => {
    ...
    await site.placeWorkspace(makeContext("pilot:c:a:t:fp"));
    await site.startTransport(makeContext("pilot:c:a:t:fp"));
  ...
  it("test_sandbox_site_place_workspace_carries_referenced_project_failure_reason", async () => {
    ...
    const failedStaged = {
      runtimeRootDir: "/remote/fail/.pilot-runtime/acpx",
      additionalSourceDirs: {},
  ...
  it("test_sandbox_site_preserves_bridge_overlap_and_callback_sequencing", async () => {
    ...
    const { site } = makeSite({
      startPilotBridge: async () => {
        events.push("pilot:start");
        await pilotGate;
        events.push("pilot:env-ready");
        return { env: { PILOT_API_KEY: "run-token" }, stop: async () => {} } as never;
      },
      startProcessSessionBridge: async ({ launchEnv }) => {
    ...
    await Promise.resolve();
    expect(events).toContain("pilot:start");
    ...
    expect(events.indexOf("pilot:env-ready")).toBeLessThan(events.indexOf("process-session:launch"));
  ...
  it("test_sandbox_site_borrow_yields_files_and_runtime_is_still_created", async () => {
    ...
    const borrowed = site.reuse().borrow("pilot:c:a:t:fp");
    expect(borrowed?.stagedRuntime).toBe(cachedRuntime);
    expect(stagedRuntimes.has("pilot:c:a:t:fp")).toBe(true);
