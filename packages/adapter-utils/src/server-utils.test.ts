...
describe("buildInvocationEnvForLogs", () => {
  it("redacts inline secrets from resolved command metadata", () => {
    const loggedEnv = buildInvocationEnvForLogs(
      ...
      {
        resolvedCommand:
          "env OPENAI_API_KEY=sk-live-example PILOT_API_KEY='pilot-quoted-secret' custom-acp --pilot-api-key=pilot-flag-secret --token ghp_example_secret",
      },
    );

    expect(loggedEnv.SAFE_VALUE).toBe("visible");
    expect(loggedEnv.PILOT_RESOLVED_COMMAND).toBe(
      "env OPENAI_API_KEY=***REDACTED*** PILOT_API_KEY='***REDACTED***' custom-acp --pilot-api-key=***REDACTED*** --token ***REDACTED***",
    );
  });
});
...
describe("materializePilotSkillCopy", () => {
  it("refuses to materialize into an ancestor of the source", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-skill-copy-"));
    try {
...
describe("adapter skill snapshots", () => {
  ...
  it("reports source-missing company runtime skills without orphan warnings", () => {
    const snapshot = buildRuntimeMountedSkillSnapshot({
      ...
      availableEntries: [{
        ...
        source: "/pilot/skills/example/__runtime__/reflection-coach--abc123",
  ...
  it("can surface read-only external skills for runtime-mounted adapters", () => {
    const snapshot = buildRuntimeMountedSkillSnapshot({
      ...
      externalDetail: "Installed outside Pilot management in the Claude skills home.",
    });
  ...
  it("reports persistent adapter installed, stale, external, and missing states", () => {
    const snapshot = buildPersistentSkillSnapshot({
      ...
      desiredSkills: [requiredEntry.key, "missing-skill"],
      installed: new Map([
        ["pilot", { targetPath: "/runtime/pilot", kind: "symlink" }],
      ...
      externalDetail: "Installed outside Pilot management.",
    });
    ...
    expect(snapshot.entries).toContainEqual(expect.objectContaining({
      ...
      state: "external",
      managed: false,
      detail: "Installed outside Pilot management.",
    }));
    expect(snapshot.entries).toContainEqual(expect.objectContaining({
      key: "missing-skill",
      state: "missing",
  ...
  it("reports stale managed persistent skills when Pilot owns an undesired available skill", () => {
    const snapshot = buildPersistentSkillSnapshot({
      adapterType: "cursor",
...
describe("renderPilotWakePrompt", () => {
  ...
  it("suppresses the issue description when the prompt already carries the task-context markdown", () => {
    ...
    expect(prompt).toContain("- issue: PIL-15271 Preserve the task brief");

    const promptJson = stringifyPilotWakePayload(payload, { omitIssueDescription: true });
    expect(JSON.parse(promptJson ?? "{}")).toMatchObject({
      issue: { description: null, descriptionTruncated: false, identifier: "PIL-15271" },
    });
    expect(JSON.parse(stringifyPilotWakePayload(payload) ?? "{}")).toMatchObject({
  ...
  it("leaves the execution contract to the heartbeat template on fresh scoped wake prompts", () => {
    ...
    expect(prompt).toContain("## Pilot Wake Payload");
  ...
  it("adds the execution contract to resume delta prompts and opted-in fresh prompts", () => {
    ...
    for (const prompt of [
      ...
      expect(prompt).toContain("Immediately before returning, verify that Pilot records one of those dispositions");
  ...
  it("asks process-loss retries to lead with the work instead of narrating recovery", () => {
    const prompt = renderPilotWakePrompt({
      reason: "source_scoped_recovery_action",
      issue: { id: "issue-1", identifier: "PIL-14092", title: "Recover work", status: "blocked" },
      recovery: {
        cause: "process_lost",
        failureSummary: "adapter stopped",
        originalAssignee: { id: "agent-1", name: "Coder" },
  ...
  it("asks restored source owners to lead with work instead of narrating recovery", () => {
    const prompt = renderPilotWakePrompt({
      ...
      issue: { id: "issue-1", identifier: "PIL-14092", title: "Continue work", status: "todo" },
      commentWindow: { requestedCount: 0, includedCount: 0, missingCount: 0 },
      comments: [],
      fallbackFetchNeeded: false,
    });
  ...
  it("renders the execution workspace branch guard only on non-resumed sessions", () => {
    const payload = {
      ...
      executionWorkspace: { branchName: "PIL-1582-ship-the-fix" },
      commentWindow: {
        requestedCount: 0,
        includedCount: 0,
        missingCount: 0,
    ...
    expect(firstPrompt).toContain(
      "- execution workspace branch: you are running in an execution workspace on branch `PIL-1582-ship-the-fix`. Do not switch, rename, or re-point this branch; keep all commits on it.",
    );

    const resumedPrompt = renderPilotWakePrompt(payload, { resumedSession: true });
    expect(resumedPrompt).toContain("## Pilot Resume Delta");
  ...
  it("keeps an execution-workspace-only wake payload alive", () => {
    const payload = { executionWorkspace: { branchName: "PIL-1584-branch-pin" } };

    expect(JSON.parse(stringifyPilotWakePayload(payload) ?? "{}")).toMatchObject({
      executionWorkspace: { branchName: "PIL-1584-branch-pin" },
    });

    const prompt = renderPilotWakePrompt(payload);
    expect(prompt).toContain(
      "- execution workspace branch: you are running in an execution workspace on branch `PIL-1584-branch-pin`.",
    );
  });
  ...
  it("renders a plugin session message as the user turn without granting it system authority", () => {
    ...
    expect(prompt).toContain("not a Pilot system or board instruction");
  ...
  it("preserves Chinese, Japanese, and Hindi issue and comment text in scoped wake prompts", () => {
    ...
    const prompt = renderPilotWakePrompt(payload);
    expect(prompt).toContain(`- issue: PIL-9452 ${title}`);
  ...
  it("renders grouped non-plan document annotations with editing scope", () => {
    const prompt = renderPilotWakePrompt({
      reason: "issue_commented",
      issue: { id: "issue-1", identifier: "PIL-522", title: "Document annotations", status: "in_progress" },
  ...
  it("renders dependency-blocked interaction guidance", () => {
    ...
    expect(prompt).toContain("PIL-1723 Finish blocker (todo)");
  });
  ...
  it("includes continuation and child issue summaries in structured wake context", () => {
    ...
    expect(prompt).toContain("PIL-101 Implement helper (done)");
...
describe("selectPilotTaskMarkdown", () => {
  const fullMarkdown = "Pilot task context:\n- Issue: \"PIL-1\"\n\nIssue description:\n```text\nThe brief.\n```";
  const compactMarkdown = "Pilot task context:\n- Issue: \"PIL-1\"";
  const wake = (reason: string) => ({
    reason,
    issue: { id: "issue-1", identifier: "PIL-1", title: "T", status: "in_progress" },
    commentWindow: { requestedCount: 0, includedCount: 0, missingCount: 0 },
    comments: [],
    fallbackFetchNeeded: false,
  });
  ...
  it("returns the full markdown for fresh sessions and assignment-shaped resumes", () => {
    const context = {
      pilotTaskMarkdown: fullMarkdown,
      pilotTaskMarkdownCompact: compactMarkdown,
      pilotWake: wake("issue_commented"),
    };
    expect(selectPilotTaskMarkdown(context)).toBe(fullMarkdown);
    expect(
      selectPilotTaskMarkdown(
        { ...context, pilotWake: wake("issue_assigned") },
        { resumedSession: true },
      ),
    ).toBe(fullMarkdown);
  });
      ...
      selectPilotTaskMarkdown(
        {
          pilotTaskMarkdown: fullMarkdown,
          pilotTaskMarkdownCompact: compactMarkdown,
          pilotWake: { ...wake("issue_monitor_recovery"), recovery: { cause: "process_lost" } },
        },
        { resumedSession: true },
      ),
    ).toBe(fullMarkdown);
...
describe("renderPilotWakePrompt - task watchdog", () => {
  const baseWatchdogPayload = {
    reason: "task_watchdog_subtree_stopped",
    issue: {
      id: "watchdog-issue-1",
      identifier: "PIL-9001",
      title: "Watchdog over PIL-8000",
      status: "in_progress",
      workMode: "standard",
    },
    commentWindow: { requestedCount: 0, includedCount: 0, missingCount: 0 },
  ...
  it("injects the watchdog mandate, watched-issue header, and stop fingerprint when taskWatchdog is present", () => {
    const prompt = renderPilotWakePrompt({
      ...
      taskWatchdog: {
        ...
        watchedIssueIdentifier: "PIL-8000",
    ...
    expect(prompt).toContain("Watched issue: PIL-8000 Ship onboarding flow");
    ...
    expect(prompt).toContain("Target scope: PIL-8000 plus non-watchdog descendants.");
    ...
    expect(prompt).toContain("- PIL-8004 QA screenshots (done) [qa]");
    expect(prompt).toContain("  QA marked done without attaching the required screenshot.");
    expect(prompt).toContain("- PIL-8007 Migrate config (blocked)");
  ...
  it("survives a JSON round-trip through stringifyPilotWakePayload", () => {
    const payload = {
      ...baseWatchdogPayload,
      taskWatchdog: {
        watchedIssueId: "watched-issue-1",
    ...
    expect(parsed.taskWatchdog).toMatchObject({
      ...
      terminalLeafSummaries: [
        expect.objectContaining({ identifier: "PIL-8004", role: "qa" }),
      ],
    });
...
describe("applyPilotWorkspaceEnv", () => {
  it("adds shared workspace env vars including AGENT_HOME", () => {
    const env = applyPilotWorkspaceEnv(
      {},
      {
        ...
        workspaceRepoUrl: "https://github.com/patty-io/pilot.git",
    ...
    expect(env).toEqual({
      ...
      PILOT_WORKSPACE_REPO_URL: "https://github.com/patty-io/pilot.git",
...
describe("shapePilotWorkspaceEnvForExecution", () => {
      ...
      workspaceHints: [
        {
          workspaceId: "workspace-1",
          cwd: "/tmp/workspace",
          repoUrl: "https://github.com/patty-io/pilot.git",
        },
        {
...
describe("refreshPilotWorkspaceEnvForExecution", () => {
  it("rewrites Pilot workspace env to the prepared remote runtime cwd", () => {
    ...
    const shaped = refreshPilotWorkspaceEnvForExecution({
      ...
      executionTargetIsRemote: true,
      executionCwd: "/remote/workspace/.pilot-runtime/runs/run-1/workspace",
    });

    expect(shaped).toEqual({
      workspaceCwd: "/remote/workspace/.pilot-runtime/runs/run-1/workspace",
      workspaceWorktreePath: null,
      workspaceHints: [
        {
          workspaceId: "workspace-1",
          cwd: "/remote/workspace/.pilot-runtime/runs/run-1/workspace",
        },
        {
          workspaceId: "workspace-2",
        },
    ...
    expect(env.PILOT_WORKSPACE_CWD).toBe("/remote/workspace/.pilot-runtime/runs/run-1/workspace");
    expect(env.PILOT_WORKSPACE_WORKTREE_PATH).toBeUndefined();
    expect(env.QA_PROJECT_WORKSPACE_CWD).toBe("/remote/workspace/.pilot-runtime/runs/run-1/workspace");
  ...
  it("forwards resolved adapter env but never overrides Pilot runtime env", () => {
    const env: Record<string, string> = {
      PILOT_RUN_ID: "run-1",
  ...
  it("applies a configured PILOT_* key only when Pilot has not set it", () => {
    const env: Record<string, string> = {};
  ...
  it("never accepts PILOT_API_KEY from config env", () => {
    const env: Record<string, string> = {};
    ...
    // The harness-minted run token is the only PILOT_API_KEY source;
...
describe("buildPilotEnv", () => {
  ...
  it("prefers an explicit PILOT_API_URL override over the derived runtime URL", () => {
    withEnv(
      {
  ...
  it("emits a legacy PILOT_ twin for every PILOT_ key (rename alias window)", () => {
    withEnv({}, () => {
      const env = buildPilotEnv({ id: "agent-1", companyId: "company-1" });
      ...
      // PILOT_ twin so pre-rename spawned CLIs keep working.
      for (const key of pilotKeys) {
        expect(env[`PILOT_${key.slice("PILOT_".length)}`]).toBe(env[key]);
      }
