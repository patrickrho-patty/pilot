...
describe("sandbox adapter execution targets", () => {
  ...
  function encodeTailTick(stdout: Buffer, stderr: Buffer): string {
    return [
      "__PILOT_RUN_LOG_STDOUT__",
      stdout.toString("base64"),
      "__PILOT_RUN_LOG_STDERR__",
      stderr.toString("base64"),
      "__PILOT_RUN_LOG_END__",
      "",
    ].join("\n");
  }
  ...
  it("preserves stdin when wrapping sandbox adapter commands for run-log streaming", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-run-log-stdin-"));
    ...
    const logsDir = path.posix.join(rootDir, ".pilot-runtime", "bridge", "logs");
  ...
  it("creates the process session directories only in the launch exec, not in upfront makeDir execs", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-process-session-makedir-"));
    cleanupDirs.push(rootDir);
    ...
    const bridge = await startAdapterExecutionTargetProcessSessionBridge({
      ...
      target,
      runtimeRootDir: path.posix.join(rootDir, ".pilot-runtime", "acpx"),
      adapterKey: "acpx",
  ...
  it("test_process_session_poll_exec_parents_to_run_context", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-process-session-poll-parent-"));
    cleanupDirs.push(rootDir);
    const childPath = path.join(rootDir, "noop-acp-child.mjs");
    await writeFile(childPath, "process.stdin.on('data', () => {});\n", "utf8");
  ...
  it("test_process_session_poll_exec_stays_unparented_without_getter", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-process-session-poll-nogetter-"));
    cleanupDirs.push(rootDir);
    const childPath = path.join(rootDir, "noop-acp-child.mjs");
    await writeFile(childPath, "process.stdin.on('data', () => {});\n", "utf8");
  ...
  it("test_process_session_stdin_exec_reads_send_time_run_parent", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-process-session-stdin-parent-"));
    cleanupDirs.push(rootDir);
    const childPath = path.join(rootDir, "noop-acp-child.mjs");
    await writeFile(childPath, "process.stdin.on('data', () => {});\n", "utf8");
    ...
    const runner = {
      execute: async (input: Parameters<typeof delegate.execute>[0]) => {
        ...
        // The `.pilot-upload` temp path under the `stdin` directory is unique
        // to the stdin-write path; the poll loop reads the `events` directory.
        const script = (input.args ?? []).join("\n");
        if (stdinWriteStep === "unset" && /\/stdin\/[^\s']*pilot-upload/.test(script)) {
  ...
  it("wraps a stdin write in a sandbox.agentSession.sendInput span", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-process-session-sendinput-span-"));
    cleanupDirs.push(rootDir);
    const childPath = path.join(rootDir, "noop-acp-child.mjs");
    await writeFile(childPath, "process.stdin.on('data', () => {});\n", "utf8");
  ...
  it("wraps each poll tick in a sandbox.agentSession.pollOutput span", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-process-session-poll-span-"));
    cleanupDirs.push(rootDir);
    const childPath = path.join(rootDir, "noop-acp-child.mjs");
    await writeFile(childPath, "process.stdin.on('data', () => {});\n", "utf8");
  ...
  it("bridges bidirectional sandbox process sessions through a local ACPX-spawnable proxy", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-process-session-"));
    cleanupDirs.push(rootDir);
    ...
    try {
      ...
      const report = await describeProxyRun(result, path.posix.join(rootDir, ".pilot-runtime", "acpx"));
  ...
  it("buffers sandbox process session output until the local proxy connects", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-process-session-buffer-"));
    cleanupDirs.push(rootDir);
  ...
  it("delivers full output when the sandbox child exits immediately after writing", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-process-session-fast-exit-"));
    cleanupDirs.push(rootDir);
  ...
  it("ignores unauthenticated connections to the process session bridge", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-process-session-auth-"));
    cleanupDirs.push(rootDir);
  ...
  it("streams sandbox process session output before the remote child exits", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-process-session-stream-"));
    cleanupDirs.push(rootDir);
  ...
  describe("streamed output (streamOutputViaSession)", () => {
    it("bridges bidirectional sessions when the wrapper streams output to stdout", async () => {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-process-session-stream-echo-"));
      cleanupDirs.push(rootDir);
    ...
    it("wraps the long-lived streamed launch in a sandbox.agentProcess span", async () => {
      ...
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-process-session-stream-span-"));
      cleanupDirs.push(rootDir);
      const childPath = path.join(rootDir, "echo-acp-child.mjs");
      await writeFile(
        childPath,
    ...
    it("parents the sandbox.agentProcess span to the live run root, not the bring-up step", async () => {
      ...
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-process-session-stream-parent-"));
      cleanupDirs.push(rootDir);
      const childPath = path.join(rootDir, "noop-acp-child.mjs");
      await writeFile(childPath, "process.stdin.on('data', () => {});\n", "utf8");
      const target: AdapterSandboxExecutionTarget = {
    ...
    it("ends the sandbox.agentProcess span at stop() even when the process lingers", async () => {
      ...
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-process-session-stream-linger-"));
      cleanupDirs.push(rootDir);
    ...
    it("buffers streamed output until the local proxy connects", async () => {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-process-session-stream-buffer-"));
      cleanupDirs.push(rootDir);
    ...
    it("delivers full streamed output when the sandbox child exits immediately", async () => {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-process-session-stream-fast-exit-"));
      cleanupDirs.push(rootDir);
    ...
    it("streams live output before the child exits and never writes output event files", async () => {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-process-session-stream-live-"));
      cleanupDirs.push(rootDir);
      ...
      try {
        ...
        const hasEventsDir = await readdir(
          path.posix.join(rootDir, ".pilot-runtime", "acpx", "process-sessions"),
    ...
    it("keeps the agent command on the persistent session and forces bridge control execs off it", async () => {
      ...
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-process-session-stream-isolation-"));
      cleanupDirs.push(rootDir);
      const childPath = path.join(rootDir, "echo-acp-child.mjs");
      await writeFile(
        childPath,
      ...
      try {
        ...
        expect(
          result.stdout,
          await describeProxyRun(result, path.posix.join(rootDir, ".pilot-runtime", "acpx")),
        ).toBe("out:hello\n");
  ...
  it("starts a localhost Pilot bridge for sandbox targets in bridge mode", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-execution-target-bridge-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".pilot-runtime", "codex");
    ...
    const apiServer = createServer((req, res) => {
      requests.push({
        ...
        runId: typeof req.headers["x-pilot-run-id"] === "string" ? req.headers["x-pilot-run-id"] : null,
      });
  ...
  it("creates a sandbox run log tail factory when bridge streaming is enabled", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-execution-target-bridge-stream-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".pilot-runtime", "codex");
    await mkdir(runtimeRootDir, { recursive: true });
  ...
  it("defaults sandbox run log streaming on and honors the explicit opt-out", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-execution-target-bridge-stream-default-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".pilot-runtime", "codex");
    await mkdir(runtimeRootDir, { recursive: true });
  ...
  it("tails sandbox run log chunks with byte offsets and dedupes the final batch", async () => {
    ...
    const tail = createSandboxRunLogTailFactory({
      runner,
      remoteCwd: "/workspace",
      logsDir: "/workspace/.pilot-runtime/codex/pilot-bridge/queue/logs",
      pollIntervalMs: 1,
  ...
  it("delivers the final batch and a warning when run log polling degrades", async () => {
    ...
    expect(combinedStream(events, "stderr")).toBe(
      "final err\n[pilot] Run log streaming degraded during the run; remaining output was delivered at completion.\n",
    );
  });

  it("exposes the Pilot bridge to the sandbox shell surface", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-execution-target-bridge-shell-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".pilot-runtime", "claude");
    await mkdir(runtimeRootDir, { recursive: true });
    ...
    try {
      expect(bridge).not.toBeNull();
      const shellProbe = [
        "const url = `${process.env.PILOT_API_URL ?? process.env.PILOT_API_URL}/api/agents/me`;",
        "fetch(url, { headers: { authorization: `Bearer ${process.env.PILOT_API_KEY ?? process.env.PILOT_API_KEY}`, accept: 'application/json' } })",
        "  .then(async (response) => {",
        "    const body = await response.json();",
        "    process.stdout.write(JSON.stringify({",
        "      status: response.status,",
        "      body,",
        "      bridgeMode: process.env.PILOT_API_BRIDGE_MODE ?? process.env.PILOT_API_BRIDGE_MODE,",
        "    }));",
        "  })",
        "  .catch((error) => {",
  ...
  it("uses the effective adapter timeout when starting the sandbox callback bridge", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-execution-target-bridge-timeout-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".pilot-runtime", "codex");
    await mkdir(runtimeRootDir, { recursive: true });
  ...
  it("fails oversized host responses with a 502 before returning them to the sandbox client", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-execution-target-bridge-limit-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".pilot-runtime", "codex");
    await mkdir(runtimeRootDir, { recursive: true });
  ...
  it("forwards the host indeterminate-outcome header so the sandbox server maps the 504 to a non-retryable 409", async () => {
    ...
    // `x-pilot-bridge-outcome: indeterminate` header. The forward must keep
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-execution-target-bridge-outcome-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".pilot-runtime", "codex");
    await mkdir(runtimeRootDir, { recursive: true });
    ...
    const apiServer = createServer((_req, res) => {
      res.writeHead(504, {
        "content-type": "application/json",
        "x-pilot-bridge-outcome": "indeterminate",
      });
    ...
    try {
      ...
      expect(response.headers.get("x-pilot-bridge-outcome")).toBe("indeterminate");
      await expect(response.json()).resolves.toEqual({
  ...
  it("forwards bridge traffic to the local listen origin even when public API URLs are configured", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-execution-target-bridge-local-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".pilot-runtime", "claude");
    await mkdir(runtimeRootDir, { recursive: true });
    ...
    // exports the public origin via PILOT_RUNTIME_API_URL / PILOT_API_URL
    // and the local listen host/port via PILOT_LISTEN_HOST / PILOT_LISTEN_PORT.
  ...
  it("lets an explicit hostApiUrl input override the bridge forward target", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-execution-target-bridge-override-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".pilot-runtime", "claude");
    await mkdir(runtimeRootDir, { recursive: true });
...
describe("sandbox duplex gateway", () => {
  ...
  async function startDuplexGateway(env: Record<string, string>): Promise<DuplexGatewayHandle> {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-duplex-gateway-"));
  ...
  it("answers outstanding requests 409 and new requests 503 on stdin EOF, then exits", async () => {
    ...
    expect(lossResponse.headers.get("x-pilot-bridge-outcome")).toBe("indeterminate");
