...
describe("sandbox managed runtime", () => {
  ...
  it("preserves excluded local workspace artifacts during restore mirroring", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-sandbox-restore-"));
    cleanupDirs.push(rootDir);
    const sourceDir = path.join(rootDir, "source");
    const targetDir = path.join(rootDir, "target");
    await mkdir(path.join(sourceDir, "src"), { recursive: true });
    await mkdir(path.join(targetDir, ".claude"), { recursive: true });
    await mkdir(path.join(targetDir, ".pilot-runtime"), { recursive: true });
    ...
    await writeFile(path.join(targetDir, ".pilot-runtime", "state.json"), "{}\n", "utf8");

    await mirrorDirectory(sourceDir, targetDir, {
      preserveAbsent: [".pilot-runtime", ".claude", ".claude.json"],
    });
    ...
    await expect(readFile(path.join(targetDir, ".pilot-runtime", "state.json"), "utf8")).resolves.toBe("{}\n");
  ...
  it("applies file mode on a staged sibling before renaming into place", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-sandbox-copy-mode-"));
    cleanupDirs.push(rootDir);
    const sourceDir = path.join(rootDir, "source");
    const targetDir = path.join(rootDir, "target");
    ...
    expect(chmodMock.mock.calls[0]?.[0]).toContain(".pilot-copy.");
  ...
  it("cleans up a staged sibling when chmod fails before rename", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-sandbox-copy-cleanup-"));
    cleanupDirs.push(rootDir);
    const sourceDir = path.join(rootDir, "source");
    const targetDir = path.join(rootDir, "target");
    const relativePath = path.join("nested", "script.sh");
    ...
    expect(stagedPath).toContain(".pilot-copy.");
  ...
  it("syncs workspace and assets through a provider-neutral sandbox client", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-sandbox-managed-"));
    cleanupDirs.push(rootDir);
    ...
    await mkdir(path.join(localWorkspaceDir, ".pilot-runtime"), { recursive: true });
    await writeFile(path.join(localWorkspaceDir, ".pilot-runtime", "state.json"), "{}\n", "utf8");
    ...
    await expect(readFile(path.join(localWorkspaceDir, ".pilot-runtime", "state.json"), "utf8")).resolves.toBe("{}\n");
  ...
  it.each(["workspace", "git-workspace"])(
    ...
    async (reservedKey) => {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-sandbox-asset-key-"));
      cleanupDirs.push(rootDir);
      const localWorkspaceDir = path.join(rootDir, "local-workspace");
      const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
      const localAssetsDir = path.join(rootDir, "local-assets");
  ...
  it("syncs git-backed workspaces through a shallow standalone clone and keeps .git out of archives", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-sandbox-git-"));
    cleanupDirs.push(rootDir);
    ...
    await git(sourceRepoDir, ["config", "user.name", "Pilot Test"]);
    await git(sourceRepoDir, ["config", "user.email", "test@pilot.dev"]);
    ...
    await git(remoteWorkspaceDir, ["config", "user.name", "Pilot Sandbox"]);
    await git(remoteWorkspaceDir, ["config", "user.email", "sandbox@pilot.dev"]);
  ...
  it("repairs stale host index deletions when the sandbox restores a clean git worktree", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-sandbox-clean-restore-"));
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-worktree");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
  ...
  it("does not fail clean restore checks when local working tree changes survive", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-sandbox-preserved-local-"));
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-worktree");
    ...
    try {
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        "[pilot] Workspace restore preserved local working tree changes after clean sandbox restore.",
      );
    } finally {
      warnSpy.mockRestore();
    }
  ...
  it("excludes unignored dependency trees from git-backed workspace overlay archives", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-sandbox-unignored-deps-"));
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-worktree");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
  ...
  it("builds workspace/asset tarballs without a './' self-entry (so untar does not chmod/utime an unowned target dir)", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-sandbox-tarself-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    const localAssetsDir = path.join(rootDir, "local-assets");
  ...
  it("excludes transient symlinked home dirs from the asset tar while keeping required content", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-sandbox-home-tmp-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
  ...
  it("emits throttled, labeled upload and restore progress with direction and percentages", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-sandbox-progress-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    const localAssetsDir = path.join(rootDir, "local-assets");
  ...
  it("creates valid empty workspace tarballs when the workspace is empty", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-sandbox-empty-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    await mkdir(localWorkspaceDir, { recursive: true });
  ...
  it("provisions a contribution-less asset via a plain tar extract and restores it as a no-op", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-sandbox-default-asset-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    const localAssetsDir = path.join(rootDir, "local-assets");
  ...
  it("round-trips a non-codex asset through generic provision + restore contributions", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-sandbox-seam-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    const localAssetsDir = path.join(rootDir, "local-assets");
  ...
  it("rejects a provision stageFile.name that is not a simple basename", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-sandbox-traversal-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    const localAssetsDir = path.join(rootDir, "local-assets");
  ...
  it("routes a custom-provisioned asset through a single syncIn operation with its post-upload command (native runner → 0 direct writeFile/run)", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-sandbox-native-asset-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    const localAssetsDir = path.join(rootDir, "local-assets");
  ...
  it("stages git and workspace via syncIn preserving .pilot-runtime (native runner → 0 direct writeFile/run)", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-sandbox-native-git-"));
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    ...
    await git(sourceRepoDir, ["commit", "-m", "base"]);
    await git(sourceRepoDir, ["worktree", "add", "-b", "work", localWorkspaceDir, "HEAD"]);
    // Pre-seed the sandbox with a `.pilot-runtime` dir that MUST survive.
    await mkdir(path.join(remoteWorkspaceDir, ".pilot-runtime"), { recursive: true });
    await writeFile(path.join(remoteWorkspaceDir, ".pilot-runtime", "keep.txt"), "keep\n", "utf8");
    ...
    // `.pilot-runtime` while replacing the rest of the tree (wipe-except-preserved).
    const gitCommand = op.postUploadCommands![0].command;
    expect(gitCommand).toContain(".pilot-runtime");
    expect(gitCommand).toContain("tar -xf");

    // The pre-seeded runtime dir survived the git+workspace staging.
    await expect(
      readFile(path.join(remoteWorkspaceDir, ".pilot-runtime", "keep.txt"), "utf8"),
    ).resolves.toBe("keep\n");
  ...
  it("the workspace wipe command preserves in-flight sync scratch tarballs (.pilot-upload-*)", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-sandbox-scratch-shape-"));
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    ...
    expect(wipeCommand).toContain("! -name '.pilot-upload-*'");
  });

  it("the workspace wipe keeps an in-flight scratch tarball at the root but removes a stale sibling", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-sandbox-scratch-race-"));
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    ...
    await git(sourceRepoDir, ["commit", "-m", "base"]);
    await git(sourceRepoDir, ["worktree", "add", "-b", "work", localWorkspaceDir, "HEAD"]);
    // Pre-seed the sandbox root. `.pilot-upload-test.tar` simulates a
    ...
    await writeFile(path.join(remoteWorkspaceDir, ".pilot-upload-test.tar"), "scratch\n", "utf8");
    ...
    await expect(
      readFile(path.join(remoteWorkspaceDir, ".pilot-upload-test.tar"), "utf8"),
  ...
  it("issues one merged syncIn operation for a git-backed workspace stage-sync with two ordered extract commands", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-sandbox-merged-git-"));
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    ...
    // Both tar targets live under `.pilot-runtime`, so the git extract's wipe
    // (which preserves `.pilot-runtime`) cannot delete the overlay tar before
    // the overlay extract runs.
    for (const mapping of op.files) {
      expect(mapping.targetPath).toContain("/.pilot-runtime/");
    }
    ...
    expect(commands[0].command).toContain(".pilot-runtime");
  ...
  it("the merged workspace confine guard covers both tar mappings (escape in either trips it)", () => {
    const runtimeRoot = "/home/daytona/pilot-workspace/.pilot-runtime/test-adapter";
    const tempRoot = "/tmp/pilot-sandbox-sync-abc";
    const gitMapping = {
  ...
  it("collapses a representative codex_local start to two syncIn round-trips: one merged workspace op plus the asset op", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-sandbox-codex-roundtrip-"));
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
  ...
  it("authors the advisory access intent rw on workspace, git, and asset inbound mappings", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-access-rw-"));
    cleanupDirs.push(rootDir);
    const sourceRepoDir = path.join(rootDir, "source-repo");
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    ...
    // asset tar.
    const remoteAssetDir = path.posix.join(remoteWorkspaceDir, ".pilot-runtime", "test-adapter", "home");
  ...
  it("authors the advisory access intent ro on referenced-project inbound mappings", async () => {
    ...
    try {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-access-ro-"));
      cleanupDirs.push(rootDir);
      const localWorkspaceDir = path.join(rootDir, "local-workspace");
      const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
  ...
  // `PILOT_MULTI_PROJECT_WORKSPACE_SYNC` gates whether run prep resolves any
  ...
  it("stages multiple referenced projects into isolated sandbox dirs end-to-end, skipping a failing source", async () => {
    ...
    process.env[flagKey] = "1";
    try {
      const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-e2e-additional-"));
      cleanupDirs.push(rootDir);
      ...
      const runtimeRootDir = path.posix.join(remoteWorkspaceDir, ".pilot-runtime", "test-adapter");
  ...
  it("builds the workspace tarball inside one host pack span for a usual workspace sync", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-pack-span-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    await mkdir(localWorkspaceDir, { recursive: true });
  ...
  it("nests the host pack span under the stage.workspace task span", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-pack-nest-"));
    cleanupDirs.push(rootDir);
    const localWorkspaceDir = path.join(rootDir, "local-workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    await mkdir(localWorkspaceDir, { recursive: true });
...
describe("sandbox managed runtime inbound coordinator", () => {
  ...
  async function makeInboundDirs(names: string[]): Promise<{
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-inbound-coordinator-"));
    cleanupDirs.push(rootDir);
...
describe("sandbox managed runtime outbound coordinator", () => {
  ...
  async function makeOutboundDirs(names: string[]): Promise<{
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-outbound-coordinator-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    const remoteWorkspaceDir = path.join(rootDir, "remote-workspace");
    await mkdir(workspaceDir, { recursive: true });
  ...
  it("with concurrency permitted, two concurrent restore tasks use separate temporary state", async () => {
    ...
    expect(tempA!).toContain("pilot-sandbox-restore-");
    expect(tempB!).toContain("pilot-sandbox-restore-");
...
describe("sandbox git-bundle export transport", () => {
  ...
  it("moves the bundle through one native syncOut file mapping, never through readFile", async () => {
    ...
    const { localWorkspaceDir, remoteWorkspaceDir } = await setupGitBackedWorkspace("pilot-bundle-native-");
  ...
  it("reads the bundle through readFile when the client has no native syncOut", async () => {
    const capture: TransportCapture = { syncOutOperations: [], readFilePaths: [] };
    const { localWorkspaceDir, remoteWorkspaceDir } = await setupGitBackedWorkspace("pilot-bundle-fallback-");
  ...
  it("retries the full bundle through the native branch when the delta misses its prerequisite", async () => {
    const capture: TransportCapture = { syncOutOperations: [], readFilePaths: [] };
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-bundle-retry-"));
    cleanupDirs.push(rootDir);
    ...
    await git(localWorkspaceDir, ["config", "user.name", "Pilot Test"]);
    await git(localWorkspaceDir, ["config", "user.email", "test@pilot.dev"]);
