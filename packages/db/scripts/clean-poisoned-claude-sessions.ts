 ...
 *   pnpm --filter @pilotai/db exec tsx scripts/clean-poisoned-claude-sessions.ts --dry-run
 *
...
Flags:
  --config <path>            Path to pilot config.json (defaults to
                             $PILOT_HOME/instances/default/config.json or
...
function defaultConfigPath(): string | null {
  ...
  candidates.push(path.join(home, ".pilot", "instances", "default", "config.json"));
...
async function main(argv: string[]): Promise<void> {
  ...
  if (!databaseUrl) {
    throw new Error(
      "Unable to resolve database URL. Pass --database-url or --config <pilot config.json>.",
    );
  }
