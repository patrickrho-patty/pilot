...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://pilot:pilot@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "pilot");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://pilot:pilot@127.0.0.1:${preferredPort}/pilot`,
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://pilot:pilot@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "pilot");
    return {
      connectionString: `postgres://pilot:pilot@127.0.0.1:${port}/pilot`,
  ...
  const adminConnectionString = `postgres://pilot:pilot@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "pilot");

  return {
    connectionString: `postgres://pilot:pilot@127.0.0.1:${selectedPort}/pilot`,
