...
const DEFAULT_PILOT_EMBEDDED_POSTGRES_PORT = 54329;
...
async function getAvailablePort(): Promise<number> {
  ...
  throw new Error(
    `Failed to allocate embedded Postgres test port outside reserved Pilot ports: ${[
      ...reservedPorts,
    ].join(", ")}`,
  );
}
...
async function probeEmbeddedPostgresSupport(): Promise<EmbeddedPostgresTestSupport> {
  ...
  try {
    started = await startEmbeddedPostgresWithRetry("pilot-embedded-postgres-probe-");
...
export async function startEmbeddedPostgresTestDatabase(
  ...
  try {
    ...
    const connectionString = `postgres://pilot:pilot@127.0.0.1:${port}/pilot`;
