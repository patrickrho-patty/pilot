...
function readInstanceId(configPath: string, label: "source" | "target"): string {
  ...
  if (!existsSync(envPath)) {
    ...
    throw new Error(`Registered ${label} Pilot config is missing its adjacent .env instance pointer.`);
  }
  const contents = readFileSync(envPath, "utf8");
  for (const rawLine of contents.split(/\r?\n/)) {
    const match = rawLine.match(
      /^\s*(?:export\s+)?PILOT_INSTANCE_ID\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s#]+))/,
    );
    const value = (match?.[1] ?? match?.[2] ?? match?.[3] ?? "").trim();
    if (value) return value;
  }
  throw new Error(`Registered ${label} Pilot config has no PILOT_INSTANCE_ID binding.`);
}
...
function inspectDeclaredEntry(entryPath: string, configPath: string, detail?: string): Stats | null {
  try {
    ...
    throw new Error(
      `Registered base project workspace Pilot config at ${configPath} cannot be inspected (${errorCode(error)}${detail ?? ""}).`,
    );
  }
}
 ...
 * at the config itself or at the `.pilot` directory holding it, still counts as a
...
export function baseWorkspaceDeclaresInstanceConfig(baseWorkspaceCwd: string): boolean {
  const configDir = path.join(baseWorkspaceCwd, ".pilot");
  const configPath = path.join(configDir, "config.json");
  if (inspectDeclaredEntry(configPath, configPath)) return true;

  // The probe above resolves `.pilot` before it reaches the config, so a broken link
  // there also reports ENOENT. Only an absent or traversable `.pilot` lets the caller
  // name another source; a link that hides whatever it points at is malformed, not empty.
  const configDirEntry = inspectDeclaredEntry(configDir, configPath, " on its .pilot entry");
  if (configDirEntry?.isSymbolicLink()) {
    try {
      statSync(configDir);
    } catch (error) {
      throw new Error(
        `Registered base project workspace Pilot config at ${configPath} cannot be inspected (${errorCode(error)} on its .pilot symlink target).`,
      );
    }
  }
  return false;
...
export function resolveRegisteredWorktreeSeedSource(
  ...
  if (registeredCwd) {
    ...
    registeredConfigPath = baseWorkspaceDeclaresInstanceConfig(canonicalBaseCwd)
      ? path.join(canonicalBaseCwd, ".pilot", "config.json")
      : null;
  }
  ...
  if (!selectedPath) {
    throw new Error(
      "Registered base project workspace has no Pilot config of its own and no explicit source was provided.",
    );
  }
  const canonicalSourceConfigPath = canonicalRegularFile(selectedPath, "Registered source Pilot config");
  if (registeredConfigPath && canonicalSourceConfigPath !== registeredConfigPath) {
    throw new Error("Registered source Pilot config escapes the base project workspace or uses a symlink alias.");
  }

  if (explicitSource) {
    const canonicalExplicitSource = canonicalRegularFile(explicitSource, "Explicit source Pilot config");
    if (canonicalExplicitSource !== canonicalSourceConfigPath) {
      throw new Error("Explicit source Pilot config does not match the registered base project workspace.");
    }
  }

  const canonicalTargetConfigPath = canonicalRegularFile(
    input.targetConfigPath,
    "Target worktree Pilot config",
  );
  if (canonicalSourceConfigPath === canonicalTargetConfigPath) {
    throw new Error("Source and target Pilot configs are the same canonical file.");
  }
  ...
  if (targetInstanceId !== input.expectedTargetInstanceId) {
    throw new Error("Target Pilot instance does not match the registered worktree instance.");
  }
  if (sourceInstanceId === targetInstanceId) {
    throw new Error("Source and target Pilot configs name the same instance.");
  }
 ...
 * carries its own `.pilot/config.json` stays authoritative, so an explicit path
