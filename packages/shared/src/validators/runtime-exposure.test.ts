...
describe("readRuntimeExposureIntent", () => {
  it("treats a legacy expose block with no exposure fields as unset", () => {
    ...
    expect(readRuntimeExposureIntent({ urlTemplate: "http://pilot-dev:{{port}}" })).toBe("unset");
...
describe("resolveDeclaredRuntimeExposureConfig", () => {
  it("returns null unless the block explicitly opts in", () => {
    ...
    expect(resolveDeclaredRuntimeExposureConfig({ urlTemplate: "http://pilot-dev:{{port}}" })).toBeNull();
  ...
  it("honors an explicit sub-field override", () => {
    expect(
      resolveDeclaredRuntimeExposureConfig({ type: "tailscale_https", includePilotViteHmr: false }),
    ).toEqual({ ...DEFAULT_TAILSCALE_HTTPS_EXPOSURE, includePilotViteHmr: false });
  });
  ...
  it("accepts a well-formed ready status", () => {
    expect(() =>
      runtimeExposureStatusSchema.parse({
        ...
        state: "ready",
        publicUrl: "https://pilot-dev.tail29c1aa.ts.net:42010",
        hostname: "pilot-dev.tail29c1aa.ts.net",
        listeners: [
