...
| --- | --- |
| First-party event names | `PilotEventName` in `generated/paperclip-telemetry.ts` |
| Per-event dimensions and optionality | `EventDimensionsMap` in `generated/paperclip-telemetry.ts` |
| Enum descriptions for telemetry dimensions | `PILOT_ENUM_DESCRIPTIONS` in `generated/paperclip-telemetry.ts` |
...
| `bridge.paperclip` | Pilot bridge start step. | `sandbox.startup` |
