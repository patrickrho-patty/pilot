...
This document explains how contributors should use Pilot's public telemetry
...
`packages/shared/src/telemetry/generated/pilot-telemetry.ts`.
...
| --- | --- |
| First-party event names | `PilotEventName` in `generated/pilot-telemetry.ts` |
| Per-event dimensions and optionality | `EventDimensionsMap` in `generated/pilot-telemetry.ts` |
| Enum descriptions for telemetry dimensions | `PILOT_ENUM_DESCRIPTIONS` in `generated/pilot-telemetry.ts` |
| Schema version and event envelope helpers | `SCHEMA_VERSION`, `makeEvent()`, and `makeBatch()` in `generated/pilot-telemetry.ts` |
...
Pilot telemetry uses named events with explicit dimension fields. Treat
...
Pilot opens OpenTelemetry spans on the sandbox start path. These spans are a
...
The spans are opt-in. Pilot exports them only when an OTLP endpoint is
configured. With no endpoint the whole span path is a no-op. Pilot opens the
spans only for a run that targets a remote sandbox. A local run and an SSH run
stay out of these spans.

Every span attribute uses the closed `pilot.sandbox.startup.` prefix and
...
Each numeric attribute is finite. Pilot omits an attribute when its value is
...
| `bridge.pilot` | Pilot bridge start step. | `sandbox.startup` |
...
| `sandbox.callbackBridge.relayRequest` | One Pilot-API callback request — read the request, write the response, remove it. | the active run span |
...
The `pilot.sandbox.startup.outcome` attribute uses a closed value set:
...
| --- | --- | --- | --- |
| `pilot.sandbox.startup.root.wall_ms` | number | no | The root-span wall time of the whole bring-up. |
| `pilot.sandbox.startup.root.work_ms` | number | no | The sum of the step wall times. |
| `pilot.sandbox.startup.root.diff_ms` | number | no | `work_ms − wall_ms`; the overlap the parallel steps saved. |
| `pilot.sandbox.startup.provider` | string | yes | The normalized provider family. |
| `pilot.sandbox.startup.cold_start` | boolean | yes | Whether the bring-up is a cold start. |
| `pilot.sandbox.startup.region` | string | yes | The clamped region label. |
| `pilot.sandbox.startup.image_id` | string | yes | The hashed image id. |
| `pilot.sandbox.startup.sandbox_id` | string | yes | The hashed sandbox id. |
| `pilot.sandbox.startup.lease_id` | string | yes | The hashed lease id. |
...
| Attribute | Type | Optional | Meaning |
| --- | --- | --- | --- |
| `pilot.sandbox.startup.step.wall_ms` | number | no | The wall time of the step. |
| `pilot.sandbox.startup.outcome` | string | no | The step outcome (`ok`, `skipped`, or `failed`). |
| `pilot.sandbox.startup.provider` | string | yes | The normalized provider family. |
| `pilot.sandbox.startup.batch` | string | yes | A shared tag that marks two parallel steps as one batch. |
| `pilot.sandbox.startup.handshake.create_runtime.wall_ms` | number | yes | The create-runtime sub-time of the `acp.handshake` step. |
| `pilot.sandbox.startup.handshake.ensure_session.wall_ms` | number | yes | The ensure-session sub-time of the `acp.handshake` step. |
...
The `sandbox.exec` span uses this closed attribute allowlist. Pilot omits a
numeric attribute when the provider does not report the value.

| Attribute | Type | Optional | Meaning |
| --- | --- | --- | --- |
| `pilot.sandbox.startup.provider` | string | no | The normalized provider family. |
| `pilot.sandbox.startup.exec.command` | string | no | The clamped `argv[0]` command label. |
| `pilot.sandbox.startup.exec.exit_code` | number | yes | The numeric process exit code. |
| `pilot.sandbox.startup.exec.wall_ms` | number | no | The host-measured wall time of the execution. |
| `pilot.sandbox.startup.exec.wait_before_ms` | number | yes | The provider handle-fetch wait before the execution ran. |
| `pilot.sandbox.startup.exec.sandbox_ms` | number | yes | The in-sandbox run time of the execution. |
| `pilot.sandbox.startup.exec.network_ms` | number | yes | The transport time the host adds; `wall_ms − wait_before_ms − sandbox_ms`. |
| `pilot.sandbox.startup.exec.critical_path` | boolean | no | Whether the execution sits on the startup critical path. |
| `pilot.sandbox.startup.exec.cache_hit` | boolean | yes | Whether the provider served the sandbox handle from its warm cache. |
| `pilot.sandbox.startup.outcome` | string | no | The execution outcome (`ok` or `failed`). |

The plugin decides the cache hit at the sandbox-handle lookup. The span no
longer infers a cache hit from `wait_before_ms == 0`. Pilot omits the
...
`pilot.sandbox.startup.` attribute prefix and obey the same opt-in and
...
| `sandbox.daytona.transfer` | The transfer step: an upload to the sandbox (inbound) or a download from the sandbox (outbound). The `pilot.sandbox.startup.transfer.direction` attribute records the direction. | the active sync task span (`stage.*` inbound, `restore.*` under `sandbox.syncBack` outbound) |
...
| --- | --- | --- | --- |
| `pilot.sandbox.startup.provider` | string | no | The normalized provider family. |
| `pilot.sandbox.startup.outcome` | string | yes | The step outcome (`ok`, `skipped`, or `failed`). |
| `pilot.sandbox.startup.pack.wall_ms` | number | yes | The host-local wall time of the pack step. It rides the `sandbox.daytona.pack` span. |
| `pilot.sandbox.startup.transfer.wall_ms` | number | yes | The wall time of the transfer step. It rides the `sandbox.daytona.transfer` span. |
| `pilot.sandbox.startup.transfer.guard.count` | number | yes | The number of serial guard round trips before one transfer. It rides the `sandbox.daytona.transfer` span. |
| `pilot.sandbox.startup.transfer.direction` | string | yes | The transfer direction (`inbound` or `outbound`). It rides the `sandbox.daytona.transfer` span. |
...
Pilot writes one `run.startup.step` event to the run log for each bring-up
...
Pilot writes one `run.phase.timing` event to the run log for each
...
1. Start from `generated/pilot-telemetry.ts`. The generated types are what
