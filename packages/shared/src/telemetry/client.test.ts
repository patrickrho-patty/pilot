...
describe("TelemetryClient runtime event gate", () => {
  ...
  it("swallows proposed first-party events before they touch state or the queue", async () => {
    ...
    client.track(
      // @ts-expect-error -- proposed-telemetry(PIL-2411): fixture proposal not in generated schema
  ...
  it("uses own-property membership so prototype event names are swallowed", async () => {
    ...
    // @ts-expect-error constructor is grammar-valid but not a registered Pilot event.
    client.track("constructor", {});
    // @ts-expect-error toString is grammar-valid but not a registered Pilot event.
