...
describe("forceLoopbackBindInCommand", () => {
  it("replaces the exact command the failing managed lanes ran", () => {
    // PIL-17256 reproduction: this is verbatim what `workspace_runtime_services`
...
describe("isPilotDevRunnerCommand", () => {
...
describe("rewriteUrlHostToLoopback", () => {
  it("redirects a MagicDNS probe target to loopback, keeping port and path", () => {
    expect(rewriteUrlHostToLoopback("http://pilot-dev:42003/api/health")).toBe(
      "http://127.0.0.1:42003/api/health",
    );
    expect(rewriteUrlHostToLoopback("http://pilot-dev:42003")).toBe("http://127.0.0.1:42003/");
  });
