/**
 * Forcing a managed runtime's listeners onto loopback through argv (PIL-17256).
 *
 ...
 * request that with env vars alone (`PILOT_BIND` / `PILOT_BIND_HOST`),
 ...
 * exposure overwrites `PILOT_BIND` from its own `--bind` argv and deletes
 * `PILOT_BIND_HOST` outright. A branch pinned at such a commit therefore
 ...
 * MagicDNS name like `http://pilot-dev:42003` — which only ever answered
