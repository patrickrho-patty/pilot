...
const EXPECTED_BUNDLED_KEYS = [
  ...
  "pilotai/bundled/pilot-operations/issue-triage",
  "pilotai/bundled/pilot-operations/reflection-coach",
  "pilotai/bundled/pilot-operations/status-card-query",
  "pilotai/bundled/pilot-operations/summarize-status",
  "pilotai/bundled/pilot-operations/task-planning",
  "pilotai/bundled/product/pilot-capsules",
...
const EXPECTED_OPTIONAL_KEYS = [
  "pilotai/optional/browser/agent-browser",
  "pilotai/optional/content/release-announcement",
  "pilotai/optional/content/simplified-english",
  "pilotai/optional/finance/ramp",
  "pilotai/optional/product/design-critique",
  "pilotai/optional/research/last30days",
  "pilotai/optional/software-development/prepare-mcp-integration",
];
...
describe("shipped skills catalog", () => {
  ...
  it("uses canonical pilotai keys derived from kind/category/slug", () => {
    const violations: string[] = [];
    for (const skill of catalogSkills) {
      const expectedKey = `pilotai/${skill.kind}/${skill.category}/${skill.slug}`;
      const expectedId = `pilotai:${skill.kind}:${skill.category}:${skill.slug}`;
  ...
  it("exposes a stable manifest header for downstream consumers", () => {
    ...
    expect(catalogManifest.packageName).toBe("@pilotai/skills-catalog");
  ...
  it("resolves shipped skills by id, key, and unique slug", () => {
    const sample = catalogSkills.find((skill) => skill.key === "pilotai/bundled/software-development/github-pr-workflow");
  ...
  it("keeps the Ramp wrapper fail-closed on mixed-provenance playbooks", () => {
    ...
    expect(rampSkill).toContain("do not execute them inside Pilot unless a Pilot approval explicitly names the playbook");
