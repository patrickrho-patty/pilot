...
const CATALOG_PACKAGE_NAME = "@pilotai/skills-catalog";
...
export async function validateCatalog(packageDir: string): Promise<BuildCatalogManifestResult> {
  ...
  if (generatedText !== null) {
    ...
    if (generatedText !== expectedText) {
      errors.push("generated/catalog.json is stale. Run pnpm --filter @pilotai/skills-catalog build:manifest.");
    }
  }
...
function skillIdForCandidate(candidate: BaseSkillCandidate) {
  return `pilotai:${candidate.kind}:${candidate.category}:${candidate.slug}`;
}
