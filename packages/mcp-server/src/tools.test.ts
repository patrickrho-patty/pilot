...
describe("pilot MCP tools", () => {
  ...
  it("adds auth headers and run id to mutating requests", async () => {
    ...
    const tool = getTool("pilotUpdateIssue");
    await tool.execute({
      issueId: "PIL-1135",
      status: "done",
    });
    ...
    expect(String(url)).toBe("http://localhost:3100/api/issues/PIL-1135");
    expect(init.method).toBe("PATCH");
    expect((init.headers as Record<string, string>)["Authorization"]).toBe("Bearer token-123");
    expect((init.headers as Record<string, string>)["x-pilot-Run-Id"]).toBe(
  ...
  it("uses default company id for company-scoped list tools", async () => {
    ...
    const tool = getTool("pilotListIssues");
  ...
  it("uses default agent id for checkout requests", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      mockJsonResponse({ id: "PIL-1135", status: "in_progress" }),
    );
    vi.stubGlobal("fetch", fetchMock);

    const tool = getTool("pilotCheckoutIssue");
    await tool.execute({
      issueId: "PIL-1135",
    });
  ...
  it("allows create issue requests to omit status so the API applies assignee defaults", async () => {
    ...
    const tool = getTool("pilotCreateIssue");
    await tool.execute({
  ...
  it("defaults issue document format to markdown", async () => {
    ...
    const tool = getTool("pilotUpsertIssueDocument");
    await tool.execute({
      issueId: "PIL-1135",
      key: "plan",
      body: "# Updated",
  ...
  it("controls issue workspace services through the current execution workspace", async () => {
    ...
    const tool = getTool("pilotControlIssueWorkspaceServices");
    await tool.execute({
      issueId: "PIL-1135",
      action: "restart",
    ...
    expect(String(lookupUrl)).toBe("http://localhost:3100/api/issues/PIL-1135/heartbeat-context");
  ...
  it("waits for an issue workspace runtime service URL", async () => {
    ...
    const tool = getTool("pilotWaitForIssueWorkspaceService");
    const response = await tool.execute({
      issueId: "PIL-1135",
      serviceName: "web",
      timeoutSeconds: 1,
  ...
  it("creates suggest_tasks interactions with the expected issue-scoped payload", async () => {
    ...
    const tool = getTool("pilotSuggestTasks");
    await tool.execute({
      issueId: "PIL-1135",
    ...
    const [url, init] = fetchMock.mock.calls[0] as [string, RequestInit];
    expect(String(url)).toBe("http://localhost:3100/api/issues/PIL-1135/interactions");
    expect(init.method).toBe("POST");
    expect(JSON.parse(String(init.body))).toEqual({
      kind: "suggest_tasks",
  ...
  it("creates request_confirmation interactions with plan target payloads", async () => {
    ...
    const tool = getTool("pilotRequestConfirmation");
    await tool.execute({
      issueId: "PIL-1135",
      idempotencyKey: "confirmation:PIL-1135:plan:33333333-3333-4333-8333-333333333333",
      title: "Plan approval",
      payload: {
        version: 1,
  ...
  it("creates request_checkbox_confirmation interactions with checkbox payloads", async () => {
    ...
    const tool = getTool("pilotRequestCheckboxConfirmation");
    await tool.execute({
      issueId: "PIL-1135",
      idempotencyKey: "confirmation:PIL-1135:files",
      title: "Choose files",
      payload: {
        version: 1,
  ...
  it("creates approvals with the expected company-scoped payload", async () => {
    ...
    const tool = getTool("pilotCreateApproval");
    await tool.execute({
      type: "hire_agent",
      payload: { branch: "pil-1167" },
  ...
  it("rejects invalid generic request paths", async () => {
    ...
    const tool = getTool("pilotApiRequest");
    const response = await tool.execute({
      method: "GET",
      path: "issues",
    });
