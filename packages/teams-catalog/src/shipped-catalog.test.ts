...
const EXPECTED_BUNDLED_KEYS = [
  "pilotai/bundled/company-defaults/core-exec-team",
  "pilotai/bundled/product/product-design",
  "pilotai/bundled/software-development/product-engineering",
];

const EXPECTED_OPTIONAL_KEYS = [
  "pilotai/optional/content/content-machine",
];
...
describe("shipped teams catalog", () => {
  ...
  it("uses canonical pilotai keys derived from kind/category/slug", () => {
    ...
    for (const team of catalogTeams) {
      const expectedKey = `pilotai/${team.kind}/${team.category}/${team.slug}`;
      const expectedId = `pilotai:${team.kind}:${team.category}:${team.slug}`;
  ...
  it("exposes a stable manifest header for downstream consumers", () => {
    ...
    expect(catalogManifest.packageName).toBe("@pilotai/teams-catalog");
  ...
  it("resolves shipped teams by id, key, and unique slug", () => {
    const sample = catalogTeams.find((team) => team.key === "pilotai/bundled/company-defaults/core-exec-team");
