...
This document records the migration state of the initial bundled and optional teams shipped in `@pilotai/teams-catalog`. It exists so future contributors know what was intentionally deferred, what is safe to delete from legacy sources, and which compatibility tests must land before the legacy onboarding assets can be removed.

The approved plan for this package lives at [PIL-10206 plan document](/PIL/issues/PIL-10206#document-plan).
...
| `skills/pilot-create-agent/references/agents/coder.md` | **Migrated content** into `bundled/software-development/product-engineering/agents/senior-coder/AGENTS.md` (collaboration/handoffs/safety sections collapsed for catalog brevity). Keep the template as a reference for ad-hoc hiring until onboarding switches. |
| `skills/pilot-create-agent/references/agents/qa.md` | **Migrated content** into both `bundled/company-defaults/core-exec-team/agents/qa/AGENTS.md` and `bundled/software-development/product-engineering/agents/qa/AGENTS.md`. Keep the template. |
| `skills/pilot-create-agent/references/agents/uxdesigner.md` | **Migrated content** into `bundled/product/product-design/agents/ux-designer/AGENTS.md`. Lens dictionary intentionally trimmed in the catalog copy — the template stays authoritative for ad-hoc hiring. |
| `skills/pilot-create-agent/references/agents/securityengineer.md` | **Not migrated.** No `SecurityEngineer` team ships in Phase H — see deferred entries below. |
...
The plan in [PIL-10206](/PIL/issues/PIL-10206#document-plan) lists additional recommended entry classes that are **not** part of the Phase H catalog. They wait on:

- `optional/ops/cloud-operations` — needs a `CloudOpsEngineer` role template under `skills/pilot-create-agent/references/agents/` first, plus an explicit security review of the deployment/secrets routines it implies. Defer until a follow-up curation pass.
...
Before removing `server/src/onboarding-assets/ceo/` or the `skills/pilot-create-agent/references/agents/*.md` templates, the following tests should be in place. None are written yet — they are tracked here so a future remove-legacy issue does not skip them:
...
3. **Skill resolution drift test** — fails if a bundled team's `requiredSkills` references a catalog skill key that no longer exists in the latest `@pilotai/skills-catalog` manifest.
...
- Packaging-level changes (adding new manifest fields, changing the validation rules, or extending the import service) belong to the coding owner of [PIL-10236](/PIL/issues/PIL-10236) and the integration service in [PIL-10238](/PIL/issues/PIL-10238).
- Content-only updates (new bundled or optional teams, copy edits, skill requirement tweaks) can land directly in this package after `pnpm --filter @pilotai/teams-catalog validate` and `pnpm --filter @pilotai/teams-catalog test` both pass.
