...
Use when:
- You want Pilot to run Cursor Agent CLI locally as the agent runtime
...
- Pilot auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$pilot" and related skills on local runs.
- Pilot auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
