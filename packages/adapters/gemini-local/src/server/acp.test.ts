...
describe("gemini_local ACP lane", () => {
  ...
  it("defaults to ACP when prerequisites pass and falls back to CLI only for auto resolution", async () => {
    const root = await makeTempRoot("pilot-gemini-acp-default-");
  ...
  it("executes Gemini through the shared ACP runtime", async () => {
    const root = await makeTempRoot("pilot-gemini-acp-run-");
  ...
  it("creates the ACP session on the in-sandbox workspace cwd for runner-backed remote runs", async () => {
    const root = await makeTempRoot("pilot-gemini-acp-remote-cwd-");
    process.env.HOME = path.join(root, "home");
  ...
  it("seeds the managed Gemini home into the sandbox, repoints HOME, and keeps the key file-only", async () => {
    const root = await makeTempRoot("pilot-gemini-acp-home-seed-");
    const localCwd = path.join(root, "worktree");
    const remoteCwd = path.join(root, "remote-workspace");
    ...
    const result = await execute(
      buildContext(localCwd, {
        config: {
          ...
          promptTemplate: "Do the assigned work.",
          pilotRuntimeSkills: [{ key: "company/review", runtimeName: "review", source: skillSource }],
          pilotSkillSync: { desiredSkills: ["company/review"] },
        },
        context: {
          issueId: "issue-1",
          pilotWorkspace: { cwd: localCwd, source: "project_workspace", workspaceId: "workspace-1" },
    ...
    expect(remappedHome).toContain(".pilot-runtime");
  ...
  it("test_gemini_acp_seam_registers_workspace_sync_back", async () => {
    const root = await makeTempRoot("pilot-gemini-acp-syncback-");
    const localCwd = path.join(root, "worktree");
    const remoteCwd = path.join(root, "remote-workspace");
    await fs.mkdir(localCwd, { recursive: true });
    await fs.mkdir(remoteCwd, { recursive: true });
  ...
  it("does not persist an api-key auth selector from a host-only credential", async () => {
    const root = await makeTempRoot("pilot-gemini-acp-hostkey-");
    const localCwd = path.join(root, "worktree");
    const remoteCwd = path.join(root, "remote-workspace");
    const hostHome = path.join(root, "home");
    await fs.mkdir(localCwd, { recursive: true });
  ...
  it("reports Gemini ACP environment readiness", async () => {
    const root = await makeTempRoot("pilot-gemini-acp-env-");
