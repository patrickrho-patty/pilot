...
export function formatGeminiAcpFallbackMessage(reason: string): string {
  return `[pilot] Gemini ACP default unavailable; falling back to Gemini CLI. ${reason} Set engine=acp to require ACP or engine=cli to silence this fallback.\n`;
}
...
async function prepareGeminiRemoteManagedHome(
  ...
  const registerWorkspaceSyncBack = (
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[pilot] Gemini ACP teardown workspace restore failed: ${
