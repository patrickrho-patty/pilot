...
function renderPilotEnvNote(env: Record<string, string>): string {
  const pilotKeys = [...new Set(
    Object.keys(env)
      ...
      .map((key) => (key.startsWith("PILOT_") ? "PILOT_" + key.slice("PILOT_".length) : key)),
  )].sort();
...
function renderApiAccessNote(env: Record<string, string>): string {
  ...
  return [
    ...
    "Use run_shell_command with curl to make Pilot API requests.",
    "GET example:",
    `  run_shell_command({ command: "curl -s -H \\"Authorization: Bearer $PILOT_API_KEY\\" \\"$PILOT_API_URL/api/agents/me\\"" })`,
    "POST/PATCH example:",
    `  run_shell_command({ command: "curl -s -X POST -H \\"Authorization: Bearer $PILOT_API_KEY\\" -H 'Content-Type: application/json' -H \\"x-pilot-Run-Id: $PILOT_RUN_ID\\" -d '{...}' \\"$PILOT_API_URL/api/issues/{id}/checkout\\"" })`,
    "",
    "",
  ].join("\n");
}
...
async function ensureGeminiSkillsInjected(
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[pilot] Failed to prepare Gemini skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[pilot] Removed maintainer-only Gemini skill "${skillName}" from ${skillsHome}\n`,
    );
  }
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[pilot] ${result === "repaired" ? "Repaired" : "Linked"} Gemini skill: ${entry.key}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[pilot] Failed to link Gemini skill "${entry.key}": ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function buildGeminiSkillsDir(
  ...
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-gemini-skills-"));
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (executionTargetIsRemote) {
    try {
      ...
      await onLog(
        "stdout",
        `[pilot] Syncing workspace and Gemini runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
      );
  ...
  if (executionTargetIsRemote && runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[pilot] Gemini session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`,
    );
  } else if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[pilot] Gemini session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
    );
  }
  ...
  try {
    ...
    if (
      ...
      await onLog(
        "stdout",
        `[pilot] Gemini resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
      );
