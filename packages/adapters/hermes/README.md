...
A [Pilot](https://pilot.test) adapter package that lets you run [Hermes Agent](https://github.com/NousResearch/hermes-agent) as a managed employee in a Pilot company.
...
manager, or add an entry to `~/.pilot/adapter-plugins.json` and restart
...
[
  {
    "packageName": "@pilotai/hermes-pilot-adapter",
    "localPath": "/absolute/path/to/pilot/packages/adapters/hermes",
    "type": "hermes_local",
...
`@pilotai/adapter-hermes-gateway` remains as a deprecated compatibility shim
for one release. It re-exports the gateway entrypoints from
`@pilotai/hermes-pilot-adapter/gateway` and preserves the legacy exports
for existing plugin installs. New installs and built-in Pilot registrations
should use `@pilotai/hermes-pilot-adapter`; the adapter type remains
`hermes_gateway`.
...
node ./pilot-task.mjs create-task --parent-id "<approved-parent-issue-id>" --title "Investigate checkout failures" --description "Capture failing request and root cause."
...
```bash
git clone https://github.com/patty-io/pilot
...
- [Pilot](https://github.com/patty-io/pilot) — The orchestration platform
- [Nous Research](https://nousresearch.com) — The team behind Hermes
- [Pilot Docs](https://docs.pilot.test) — Pilot documentation
