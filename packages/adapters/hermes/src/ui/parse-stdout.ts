...
/**
 * Parse Hermes Agent stdout into TranscriptEntry objects for the Pilot UI.
 *
 ...
 *   System:     "[hermes] ..."
 *
 * We emit structured tool_call/tool_result pairs so Pilot renders proper
 * tool cards (with status icons, expand/collapse) instead of raw stdout blocks.
 */

import type { TranscriptEntry } from "@pilotai/adapter-utils";
 ...
 * Pilot uses this to match them in normalizeTranscript.
 */
 ...
 * Pilot renders proper tool cards with status icons and expand/collapse.
 *
...
export function parseHermesStdoutLine(
  ...
  // Emit as stderr so Pilot groups them into the amber accordion.
