...
const helperPath = path.resolve(__dirname, "../../skills/pilot-task-bridge/pilot-task.mjs");
const apiKey = "pilot_test_secret_should_not_print";
...
describe("pilot-task-bridge helper", () => {
  ...
  beforeEach(async () => {
    ...
    server = http.createServer(async (req, res) => {
      ...
      if (req.method === "POST" && req.url === "/api/issues/PIL-123/comments") {
        res.statusCode = 201;
        res.end(JSON.stringify({
      ...
      if (req.method === "PATCH" && req.url === "/api/issues/PIL-123") {
        res.end(JSON.stringify({
          id: "33333333-3333-4333-8333-333333333333",
          identifier: "PIL-123",
          title: "Existing task",
  ...
  function env() {
    return {
      PILOT_API_URL: baseUrl,
      PILOT_BRIDGE_API_KEY: apiKey,
      PILOT_COMPANY_ID: "22222222-2222-4222-8222-222222222222",
      PILOT_AGENT_ID: "11111111-1111-4111-8111-111111111111",
      PILOT_RUN_ID: "66666666-6666-4666-8666-666666666666",
    };
  }
  ...
  it("lists assigned tasks without printing credentials", async () => {
    ...
    expect(result.stdout).toContain('"identifier": "PIL-123"');
  ...
  it("creates tasks assigned to the authenticated agent by default", async () => {
    ...
    expect(result.code).toBe(0);
    expect(result.stdout).toContain('"identifier": "PIL-124"');
    const createRequest = requests.find((request) => request.method === "POST" && request.url.includes("/companies/"));
    expect(createRequest?.headers["x-pilot-run-id"]).toBe("66666666-6666-4666-8666-666666666666");
  ...
  it("comments and updates status through direct issue identifier routes", async () => {
    const comment = await runHelper(["comment", "--issue", "PIL-123", "--body", "Progress from Hermes"], env());
    const update = await runHelper(["update-status", "--issue", "PIL-123", "--status", "in_review", "--comment", "Ready"], env());
