 ...
 *
 * @see https://github.com/patty-io/pilot/issues/8723
 */
...
vi.mock("@pilotai/adapter-utils/server-utils", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@pilotai/adapter-utils/server-utils")>();
  return {
    ...actual,
...
import * as serverUtils from "@pilotai/adapter-utils/server-utils";
...
describe("hermes-local adapter onSpawn forwarding", () => {
  ...
  it("does not inherit PILOT_API_KEY without a harness token", async () => {
    const previousApiKey = process.env.PILOT_API_KEY;
    process.env.PILOT_API_KEY = "parent-process-key";
    ...
    try {
      ...
      expect(opts.env.PILOT_API_KEY).toBeUndefined();
    } finally {
