...
const HERMES_DEFAULT_PROMPT_TEMPLATE = [
  'You are "{{agent.name}}", an AI agent employee in a Pilot-managed company.',
  "",
  "Pilot runtime identity:",
  "- Agent ID: {{agent.id}}",
  "- Company ID: {{agent.companyId}}",
  "- Run ID: {{run.id}}",
  "- API base: {{pilotApiUrl}}",
  "",
  "Pilot API guidance:",
  "- Use `curl` from the terminal for Pilot API calls; browser/web extraction tools may not reach localhost.",
  "- Use `$PILOT_API_URL`, `$PILOT_API_KEY`, and `$PILOT_RUN_ID`; do not hard-code local ports or copy secrets into comments.",
  "- Displayed command logs may redact secrets; rely on environment variables instead of printed token values.",
  "- Include `-H \"Authorization: Bearer $PILOT_API_KEY\"` on API requests.",
  "- Include `-H \"x-pilot-Run-Id: $PILOT_RUN_ID\"` on mutating issue requests.",
...
export function buildPrompt(
  ...
  const sessionHandoffMarkdown = cfgString(context.pilotSessionHandoffMarkdown)?.trim() || "";
  ...
  const vars: Record<string, unknown> = {
    ...
    pilotWakePrompt: wakePrompt,
    pilotTaskMarkdown: pilotTaskMarkdown,
    taskContext: pilotTaskMarkdown,
    pilotWakeJson: wakePayloadJson,
    wakePayloadJson,
    pilotApiKeyEnv: "PILOT_API_KEY",
    pilotRunIdEnv: "PILOT_RUN_ID",
  };

  const rendered = isPilotRecoveryWakePayload(context.pilotWake)
    ? ""
...
function cleanResponse(raw: string): string {
  return raw
    ...
    .filter((line) => {
      ...
      if (t.startsWith("[tool]") || t.startsWith("[hermes]") || t.startsWith("[pilot]")) return false;
...
export async function execute(
  ...
  // PILOT_API_KEY is never accepted from config — the harness-minted run
  ...
  const wakePayloadJson = stringifyPilotWakePayload(ctxContext.pilotWake);
