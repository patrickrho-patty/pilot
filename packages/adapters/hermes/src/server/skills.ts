...
async function buildHermesSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  ...
  const availableByKey = new Map(pilotEntries.map((e) => [e.key, e]));
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(
      `Desired skill "${desiredSkill}" is not available in Pilot or Hermes skills.`,
    );
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      ...
      targetPath: null,
      detail:
        "Cannot find this skill in Pilot or ~/.hermes/skills/.",
    });
  }
