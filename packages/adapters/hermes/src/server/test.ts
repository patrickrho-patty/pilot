...
function checkModel(
  ...
  if (!model) {
    return {
      ...
      hint: "Set a model explicitly in Pilot only if you want to override your local Hermes configuration.",
...
async function checkProviderConsistency(
  ...
  if (!explicitProvider && resolvedFrom.startsWith("hermesConfigUnsupported:")) {
    ...
    return {
      ...
      hint: "Pilot will avoid model-name provider inference here and let Hermes resolve the provider from ~/.hermes/config.yaml at runtime.",
