...
test("renders standard assignment wake with task authority and no backlog discovery guidance", () => {
  ...
  expect(prompt).toContain("- issue: PIL-11750 Add Hermes prompt rendering regression tests");
  expect(prompt).toContain("- issue work mode: standard");
  expect(prompt).toContain("Pilot task context:");
...
test("renders scoped planning wake authority before the Hermes default workflow", () => {
  const prompt = buildPrompt(baseContext(), {
    pilotApiUrl: "http://127.0.0.1:3101/api",
  });

  expect(prompt).toContain("## Pilot Wake Payload");
  expect(prompt).toContain("- issue: PIL-3404 Plan the Hermes prompt update");
...
test("renders resume deltas instead of full scoped-wake boilerplate when continuing a session", () => {
  ...
  expect(prompt).toContain("## Pilot Resume Delta");
  expect(prompt).toContain("You are resuming an existing Pilot session.");
...
test("keeps authoritative parent and ancestor context from task markdown", () => {
  const prompt = buildPrompt(baseContext({
    pilotTaskMarkdown: [
      ...
      "- Parent: PIL-11724 Optimize prompt traces (in_progress) [medium]",
      "- Ancestor 2: PIL-11721 Fetch raw traces (done) [medium]",
    ].join("\n"),
  }), {});

  expect(prompt).toContain("Authoritative parent / ancestor context:");
  expect(prompt).toContain("- Parent: PIL-11724 Optimize prompt traces (in_progress) [medium]");
  expect(prompt).not.toContain("check the issue body or comments for references");
});

test("renders safe Pilot API examples from environment variables with multiline update preservation", () => {
  const prompt = buildPrompt(baseContext(), {
    pilotApiUrl: "http://pilot.local/api",
  });

  expect(prompt).toContain("Use `$PILOT_API_URL`, `$PILOT_API_KEY`, and `$PILOT_RUN_ID`");
  expect(prompt).toContain("Displayed command logs may redact secrets");
  expect(prompt).toContain('-H "Authorization: Bearer $PILOT_API_KEY"');
  expect(prompt).toContain('-H "x-pilot-Run-Id: $PILOT_RUN_ID"');
...
test("preserves custom prompt templates while exposing runtime and wake variables", () => {
  const prompt = buildPrompt(baseContext(), {
    ...
    promptTemplate: [
      ...
      "agent={{agent.name}}",
      "api={{pilotApiUrl}}",
      "keyEnv={{pilotApiKeyEnv}}",
      "runEnv={{pilotRunIdEnv}}",
      "wakePrompt={{pilotWakePrompt}}",
      "task={{pilotTaskMarkdown}}",
      "wakeJson={{pilotWakeJson}}",
  ...
  expect(prompt).toContain("api=http://pilot.local/api");
  expect(prompt).toContain("keyEnv=PILOT_API_KEY");
  expect(prompt).toContain("runEnv=PILOT_RUN_ID");
  expect(prompt).toContain("wakePrompt=## Pilot Wake Payload");
  expect(prompt).toContain("task=Pilot task context:");
  expect(prompt).toContain("wakeJson={\"reason\":\"issue_assigned\"");
  expect(prompt).toContain('"reason":"issue_assigned"');
  expect(prompt).toContain("## Pilot Wake Payload");
  expect(prompt).toContain("Issue description:\n```text\nUse the wake payload as runtime authority.\n```");
  expect(prompt).not.toContain("Pilot runtime identity:");
});
