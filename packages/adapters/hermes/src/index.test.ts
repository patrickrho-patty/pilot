...
test("root package export exposes Pilot external adapter entrypoint", () => {
...
test("Hermes adapter exposes bundled Pilot task bridge skill", async () => {
  const adapter = createServerAdapter();
  ...
  expect(snapshot?.entries.some((entry) => entry.runtimeName === "pilot-task-bridge")).toBe(true);
});
