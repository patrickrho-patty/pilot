import type { AdapterSessionManagement, ServerAdapterModule } from "@pilotai/adapter-utils";
...
- Pilot should create Hermes runs through POST /v1/runs and observe them through SSE events.
- You need remote Hermes session continuity with X-Hermes-Session-Key.

Don't use when:
- Hermes should run as a local child process on the Pilot host; use hermes_local instead.
- The Hermes API server is not enabled or is only reachable over an unsafe public HTTP endpoint.
- You need Hermes-originated Pilot task creation only; that is a Pilot skill/API bridge, not this wake adapter.
...
- pilotApiUrl (string): Pilot API URL reachable from the Hermes host. This is not a credential.
...
- Sends Idempotency-Key equal to the Pilot run id for correlation only; Hermes v0.16.0 did not dedupe duplicate creates.
...
- Do not put Hermes apiKey or Pilot bearer tokens in prompts, comments, logs, or result JSON.
