...
const PILOT_SESSION_KEY_PATTERN =
  /\bpilot:(?:company:[A-Za-z0-9-]+:agent:[A-Za-z0-9-]+(?::(?:issue|run):[A-Za-z0-9-]+)?|run:[A-Za-z0-9-]+)\b/gi;
...
export function resolveSessionKey(input: {
  ...
  if (input.strategy === "agent") {
    return `pilot:company:${input.companyId}:agent:${input.agentId}`;
  }
  if (input.strategy === "run") {
    return `pilot:run:${input.runId}`;
  }
  const issuePart = input.issueId ? `issue:${input.issueId}` : `run:${input.runId}`;
  return `pilot:company:${input.companyId}:agent:${input.agentId}:${issuePart}`;
}
...
function buildInput(ctx: AdapterExecutionContext, pilotApiUrl: string | null): string {
  ...
  const wakePrompt = renderPilotWakePrompt(ctx.context.pilotWake, {
    // The task-context markdown is the authoritative brief on this lane; keep
    // the wake prompt's description copy out so the prompt carries it once.
    suppressIssueDescription: Boolean(taskMarkdown),
  });
  const wakePayloadJson = stringifyPilotWakePayload(ctx.context.pilotWake, {
    omitIssueDescription: Boolean(taskMarkdown),
  });
  const sessionHandoff = nonEmpty(ctx.context.pilotSessionHandoffMarkdown);
  const issueWorkMode = readPilotIssueWorkModeFromContext(ctx.context);
  const lines = [
    `You are ${ctx.agent.name}, an AI agent employee in a Pilot-managed company.`,
    "",
    "Pilot runtime identity:",
    `- Agent ID: ${ctx.agent.id}`,
    `- Company ID: ${ctx.agent.companyId}`,
    `- Run ID: ${ctx.runId}`,
    ...(pilotApiUrl ? [`- Pilot API URL: ${pilotApiUrl}`] : []),
    ...(issueWorkMode ? [`- Issue work mode: ${issueWorkMode}`] : []),
    "",
    ...(isPilotRecoveryWakePayload(ctx.context.pilotWake)
      ? []
      : [
          "Execution contract:",
          ...
          "- Use x-pilot-Run-Id on mutating Pilot API requests when a Pilot API key is available.",
          "",
        ]),
    wakePrompt,
...
function buildRunBody(ctx: AdapterExecutionContext, sessionKey: string | null): Record<string, unknown> {
  const pilotApiUrl = nonEmpty(ctx.config.pilotApiUrl);
  ...
  const instructions =
    ...
    "Follow the Pilot wake instructions exactly. Do not expose secrets in logs, comments, or final output.";
  return {
    ...payloadTemplate,
    input,
    instructions,
