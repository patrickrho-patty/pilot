...
const DEFAULT_CLIENT_VERSION = "pilot";
...
export function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "pilot";
  if (input.strategy === "run") {
    return prefixSessionKeyForAgent(`pilot:run:${input.runId}`, input.agentId);
  }
  if (input.strategy === "issue" && input.issueId) {
    return prefixSessionKeyForAgent(`pilot:issue:${input.issueId}`, input.agentId);
  }
...
const DEFAULT_CLAIMED_API_KEY_PATH = "~/.openclaw/workspace/pilot-claimed-api-key.json";
...
function buildPilotEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const pilotApiUrlOverride = resolvePilotApiUrlOverride(ctx.config.pilotApiUrl);
...
function buildWakeText(
  ...
  const lines = [
    "Pilot wake event for a cloud adapter.",
    "",
    ...
    "",
    `Load PILOT_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $PILOT_API_KEY on every API call.",
    "- Use x-pilot-Run-Id: $PILOT_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: PILOT_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$PILOT_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\",\"in_review\"]}",
    ...
    "   - GET /api/companies/$PILOT_COMPANY_ID/issues?assigneeAgentId=$PILOT_AGENT_ID&status=todo,in_progress,in_review,blocked",
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "pilot-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const structuredWakePrompt = renderPilotWakePrompt(ctx.context.pilotWake, {
    includeExecutionContract: true,
  });
  const structuredWakeJson = stringifyPilotWakePayload(ctx.context.pilotWake);
