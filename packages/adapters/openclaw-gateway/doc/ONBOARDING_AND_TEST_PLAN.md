...
## Scope
This plan is now **gateway-only**. Pilot supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Pilot skill.
...
```json
{
  "pilotApiUrl": "http://host.docker.internal:3100",
...
3. In `/new` OpenClaw session, OpenClaw can still create a Pilot task.
...
```bash
pnpm --filter @pilotai/server test:run -- openclaw-gateway
pnpm --filter @pilotai/server typecheck
pnpm --filter @pilotai/ui typecheck
pnpm --filter pilotai typecheck
```
