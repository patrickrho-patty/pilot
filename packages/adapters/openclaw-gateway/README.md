...
This document describes how `@pilotai/adapter-openclaw-gateway` invokes OpenClaw over the Gateway protocol.
...
5. stream `event agent` frames into Pilot logs/transcript parsing
...
Like every Pilot adapter, this one must treat the local execution-workspace
...
the OpenClaw side, use the round-trip helpers in `@pilotai/adapter-utils`
