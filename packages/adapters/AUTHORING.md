...
Why: Pilot resolves a local execution workspace (a worktree) for each
...
- Never `git push` from adapter runtime code. Never assume the local worktree
  ...
  read it from the local cwd Pilot handed you.
- If your adapter runs the agent on a different host (ssh, sandbox, remote
  container), use the round-trip helpers in `@pilotai/adapter-utils`:
...
`// pilot:allow-git-push: <reason>` comment on the line (or the line
