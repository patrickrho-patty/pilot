...
import { inferOpenAiCompatibleBiller } from "@pilotai/adapter-utils";
import {
...
export function formatCodexAcpFallbackMessage(reason: string): string {
  return `[pilot] Codex ACP default unavailable; falling back to Codex CLI. ${reason} Set engine=acp to require ACP or engine=cli to silence this fallback.\n`;
}
  ...
  return {
    ...
    teardown: async () => {
      try {
        await onLog(
          "stdout",
          "[pilot] Restoring workspace changes and Codex auth from the sandbox.\n",
        );
        ...
        await onLog(
          "stderr",
          `[pilot] Codex ACP teardown restore/copy-back failed: ${
  ...
  if (!targetIsRemote) {
    ...
    if (credentialReadiness.ready && credentialReadiness.authMode === "api") {
      ...
      checks.push({
        ...
        level: "warn",
        message: "No Codex ACP credentials visible to the Pilot server were detected.",
        hint: "Set OPENAI_API_KEY in the agent adapter env, set it in the Pilot server environment, or run `codex login` for the same OS user that runs the Pilot server before starting a Codex ACP agent. A `/login` in a separate Codex/chat session does not authenticate the server.",
      });
    }
