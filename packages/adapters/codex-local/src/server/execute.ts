...
import { inferOpenAiCompatibleBiller, type AdapterExecutionContext, type AdapterExecutionResult } from "@pilotai/adapter-utils";
...
} from "@pilotai/adapter-utils/local-process-sandbox";
import {
  parseCodexJsonl,
...
async function isLikelyPilotRepoRoot(candidate: string): Promise<boolean> {
...
async function isLikelyPilotRuntimeSkillPath(
  candidate: string,
  skillName: string,
  ...
  for (let depth = 0; depth < 6; depth += 1) {
    if (await isLikelyPilotRepoRoot(cursor)) return true;
...
async function pruneBrokenUnavailablePilotSkillSymlinks(
  skillsHome: string,
  ...
  for (const entry of entries) {
    ...
    if (
      !(await isLikelyPilotRuntimeSkillPath(resolvedLinkedPath, entry.name, {
    ...
    await onLog(
      "stdout",
      `[pilot] Removed stale Codex skill "${entry.name}" from ${skillsHome}\n`,
    );
  }
}
...
function managedMcpGatewaysFromContext(context: Record<string, unknown>): ManagedCodexMcpGateway[] {
  const managedMcp = parseObject(context.pilotManagedMcp);
...
export async function ensureCodexSkillsInjected(
  ...
  const allSkillsEntries = options.skillsEntries ?? await readPilotRuntimeSkillEntries({}, __moduleDir);
    ...
    try {
      ...
      if (existing?.isSymbolicLink()) {
        ...
        if (
          ...
          (await isLikelyPilotRuntimeSkillPath(resolvedLinkedPath, entry.runtimeName))
        ) {
          ...
          await onLog(
            "stdout",
            `[pilot] Repaired Codex skill "${entry.runtimeName}" into ${skillsHome}\n`,
          );
          continue;
        }
      }
      ...
      await onLog(
        "stdout",
        `[pilot] ${result === "repaired" ? "Repaired" : "Injected"} Codex skill "${entry.runtimeName}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[pilot] Failed to inject Codex skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }

  await pruneBrokenUnavailablePilotSkillSymlinks(
    skillsHome,
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const workspaceHints = Array.isArray(context.pilotWorkspaces)
    ...
    : [];
  const runtimeServiceIntents = Array.isArray(context.pilotRuntimeServiceIntents)
    ? context.pilotRuntimeServiceIntents.filter(
        (value): value is Record<string, unknown> => typeof value === "object" && value !== null,
      )
    : [];
  const runtimeServices = Array.isArray(context.pilotRuntimeServices)
    ? context.pilotRuntimeServices.filter(
        (value): value is Record<string, unknown> => typeof value === "object" && value !== null,
      )
    : [];
  const runtimePrimaryUrl = asString(context.pilotRuntimePrimaryUrl, "");
  ...
  const configuredCodexHome =
    typeof envConfig.CODEX_HOME === "string" && envConfig.CODEX_HOME.trim().length > 0
      ...
      : null;
  const codexSkillEntries = await readPilotRuntimeSkillEntries(config, __moduleDir);
  ...
  if (isCodexAuthCacheEnabled(process.env)) {
    ...
    await selectVendCredential(
      ...
      await onLog(
        "stderr",
        `[pilot] Codex auth cache: vend skipped after an error; using the shared credential as-is.\n`,
      );
      void error;
    });
  }
  ...
  // Merge custom model providers (PILOT_CODEX_PROVIDERS) into the managed
  ...
  try {
    for (const note of preparedRuntimeConfig.notes) {
      await onLog("stdout", `[pilot] ${note}\n`);
    }
    const pilotBaseEnv = buildPilotEnv(agent);
    ...
    if (managedMcpGateways.length > 0) {
      await onLog(
        "stdout",
        `[pilot] Wrote ${managedMcpGateways.length} managed MCP gateway(s) into Codex config "${managedMcp.configPath}".\n`,
      );
    }
    for (const warning of managedMcp.warnings) {
      await onLog("stderr", `[pilot] ${warning}\n`);
    }
    ...
    const preparedExecutionTargetRuntime = executionTargetIsRemote
      ? await (async () => {
          await onLog(
            "stdout",
            `[pilot] Syncing ${targetWorkspaceRealization?.mode === "in_place" ? "CODEX_HOME" : "workspace and CODEX_HOME"} to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
          );
    ...
    const remoteCodexHome = executionTargetIsRemote
      ? preparedExecutionTargetRuntime?.assetDirs.home ??
        path.posix.join(effectiveExecutionCwd, ".pilot-runtime", "codex", "home")
      : null;
    ...
    if (localProcessSandbox) {
      ...
      await onLog(
        "stdout",
        `[pilot] Confining Codex with ${scopes} scope.\n`,
      );
    }
    ...
    if (monitorResolution.mode === "disabled") {
      await onLog(
        "stdout",
        `[pilot] Codex output inactivity monitor is DISABLED via adapterConfig.outputInactivityTimeoutMs=null. Hung codex runs will only be detected by the platform-level silent-run safety net.\n`,
      );
    } else if (monitorResolution.mode === "default" && "reason" in monitorResolution) {
      await onLog(
        "stdout",
        `[pilot] Ignoring non-positive adapterConfig.outputInactivityTimeoutMs; falling back to default ${monitorResolution.timeoutMs}ms.\n`,
      );
    }
    ...
    if (executionTargetIsRemote && runtimeSessionId && !canResumeSession) {
      await onLog(
        "stdout",
        `[pilot] Codex session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`,
      );
    } else if (runtimeSessionId && !canResumeSession) {
      await onLog(
        "stdout",
        `[pilot] Codex session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
      );
    }
    ...
    const renderedBootstrapPrompt =
      !sessionId && bootstrapPromptTemplate.trim().length > 0
        ...
        : "";
    const wakePrompt = renderPilotWakePrompt(context.pilotWake, { resumedSession: Boolean(sessionId) });
    const shouldUseResumeDeltaPrompt = Boolean(sessionId) && wakePrompt.length > 0;
    const promptInstructionsPrefix = shouldUseResumeDeltaPrompt ? "" : instructionsPrefix;
    instructionsChars = promptInstructionsPrefix.length;
    const continuationSummary = parseObject(context.pilotContinuationSummary);
    ...
    const renderedPrompt = shouldUseResumeDeltaPrompt || isPilotRecoveryWakePayload(context.pilotWake)
      ? ""
    ...
    try {
      ...
      if (
        ...
        await onLog(
          "stdout",
          `[pilot] Codex resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
        );
      ...
      if (restoreRemoteWorkspace) {
        ...
        try {
          await onLog(
            "stdout",
            `[pilot] Restoring workspace changes from ${describeAdapterExecutionTarget(executionTarget)}.\n`,
          );
          ...
          await Promise.resolve(
            onLog(
              "stderr",
              `[pilot] Failed to restore workspace changes from ${describeAdapterExecutionTarget(
                executionTarget,
    ...
    if (stagedCodexHomeDir) {
      await fs.rm(stagedCodexHomeDir, { recursive: true, force: true }).catch(async (error) => {
        await onLog(
          "stderr",
          `[pilot] Failed to remove staged Codex home "${stagedCodexHomeDir}": ${
            error instanceof Error ? error.message : String(error)
          }\n`,
        );
      });
    }
    // Restore the managed config.toml so PILOT_CODEX_PROVIDERS changes
