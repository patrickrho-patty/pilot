...
import { withDirectoryMergeLock } from "@pilotai/adapter-utils/workspace-restore-merge";
...
export async function writeCodexAuthCacheEntry(input: {
  ...
  const outcome = await writeCredentialSeedOrNewer({
    ...
    log: input.log,
    writtenLine: "[pilot] Codex auth cache: wrote the per-identity cache slot at mode 0600.",
    keptLine:
      "[pilot] Codex auth cache: kept the cache slot (source is not a strictly-newer same-identity subscription credential).",
...
export async function selectVendCredential(
  ...
  return withDirectoryMergeLock(hostDir, async () => {
    ...
    if (!cacheBytes) {
      await log("[pilot] Codex auth cache: no cached credential for the host identity; host credential kept.");
      return "kept-host";
    }
    ...
    try {
      ...
      if (decision === USE_SOURCE_EXIT) {
        ...
        await log(
          "[pilot] Codex auth cache: refreshed the host credential with a strictly-newer cached copy of the same identity at mode 0600.",
        );
        return "vended";
      }
      await log(
        "[pilot] Codex auth cache: host credential kept (the cached copy is not strictly newer for the same identity).",
      );
      return "kept-host";
    } finally {
