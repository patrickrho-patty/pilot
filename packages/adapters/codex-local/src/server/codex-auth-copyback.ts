...
export async function copyBackCodexAuth(input: CopyBackCodexAuthInput): Promise<CopyBackCodexAuthOutcome> {
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException | null)?.code === "ENOENT") {
      await log(
        "[pilot] Codex auth copy-back: no sandbox credential to copy back (absent auth.json); host credential kept.",
      );
      return "kept-host";
    }
    throw error;
  ...
  const hostOutcome = await withDirectoryMergeLock(hostDir, async () => {
    ...
    try {
      ...
      if (decision === USE_SOURCE_EXIT) {
        ...
        await log(
          "[pilot] Codex auth copy-back: sandbox credential is strictly newer for the same subscription identity; installed to the host at mode 0600.",
        );
        return "copied";
      }

      await log(
        "[pilot] Codex auth copy-back: host credential kept (sandbox copy is not a strictly-newer same-identity subscription credential).",
      );
      return "kept-host";
    } finally {
  ...
  if (resolveCacheEntryPath && isCodexAuthCacheEnabled(env)) {
    try {
      ...
      await Promise.resolve(
        log(
          `[pilot] Codex auth cache: additive cache write failed (${code}); host copy-back result kept.`,
        ),
      ).catch(() => undefined);
    }
  }
