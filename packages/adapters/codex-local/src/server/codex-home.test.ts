...
describe("codex managed home", () => {
  ...
  it("treats a concurrently-created expected auth symlink as success", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-home-"));
  ...
  it("ensureSymlink: unlinks a stale regular file and recreates the symlink", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-ensure-symlink-"));
    try {
  ...
  it("ensureSymlink: leaves an unexpected directory in place instead of throwing", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-ensure-symlink-dir-"));
    try {
      const source = path.join(root, "live-source.json");
...
describe("isManagedCodexHomePath", () => {
  const env = {
    PILOT_HOME: "/srv/pilot",
    PILOT_INSTANCE_ID: "default",
  } satisfies NodeJS.ProcessEnv;
  const companyRoot = path.resolve(
    "/srv/pilot/instances/default/companies/company-1",
  );
  ...
  it("treats a path outside the company tree as an external override", () => {
    ...
    expect(
      isManagedCodexHomePath(
        env,
        "company-1",
        path.resolve("/srv/pilot/instances/default/companies/company-2/codex-home"),
      ),
    ).toBe(false);
  });
...
describe("codexHomeHasUsableAuth", () => {
  it("is true for credential-bearing auth.json and false when missing", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-auth-"));
    try {
  ...
  it("recognizes the Codex 0.143 AuthDotJson subscription shape", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-auth-modern-"));
    try {
      await fs.writeFile(
  ...
  it("treats subscription auth without account_id or token material as unusable", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-auth-modern-invalid-"));
    try {
      await fs.writeFile(
        path.join(root, "auth.json"),
        JSON.stringify({
  ...
  it("is false for a dangling auth.json symlink", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-auth-dangling-"));
    try {
...
describe("seedManagedCodexHome", () => {
  it("symlinks auth.json from the shared source into an explicit per-agent home", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-seed-"));
    try {
      const sharedCodexHome = path.join(root, "shared-codex-home");
  ...
  it("writes an API-key auth.json into the home when an apiKey is supplied", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-seed-apikey-"));
    try {
  ...
  it("keeps a promoted subscription auth.json when the shared source has no auth", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-seed-promoted-"));
    try {
  ...
  it("keeps a promoted subscription auth.json whose identity differs from the shared source", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-seed-foreign-"));
    try {
      const companyHome = path.join(root, "company-home");
      const sharedCodexHome = path.join(root, "shared-codex-home");
      const promoted = subscriptionAuth("acct-promoted", "promoted");
  ...
  it("still replaces a same-identity stale regular copy with the shared symlink (#5028)", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-seed-stale-"));
    try {
      const companyHome = path.join(root, "company-home");
      const sharedCodexHome = path.join(root, "shared-codex-home");
  ...
  it("keeps the usable target when the shared source exists but cannot be read", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-seed-src-err-"));
    try {
      const companyHome = path.join(root, "company-home");
      const sharedCodexHome = path.join(root, "shared-codex-home");
  ...
  it("still removes an apikey-mode auth.json so the chatgpt-mode symlink is restored", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-seed-apikey-residue-"));
    try {
      const companyHome = path.join(root, "company-home");
      const sharedCodexHome = path.join(root, "shared-codex-home");
...
describe("reconcileManagedCodexHome", () => {
  async function makeFixture() {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-reconcile-"));
    const sharedCodexHome = path.join(root, "shared-codex-home");
    const pilotHome = path.join(root, "pilot-home");
    const agentHome = path.join(
      pilotHome,
...
describe("evaluateCodexCredentialReadiness", () => {
  async function makeFixture() {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-readiness-"));
    const sharedCodexHome = path.join(root, "shared-codex-home");
    const pilotHome = path.join(root, "pilot-home");
  ...
  it("replaces the managed MCP block and clears stale servers for an empty runtime set", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-mcp-config-"));
    try {
      ...
      await writeManagedCodexMcpConfig({
        ...
        gateways: [{
          name: "alpha",
          endpointPath: "https://pilot.example/api/tool-gateway/gateways/alpha/mcp",
      ...
      expect(alpha).toContain("# BEGIN PILOT MANAGED MCP");
      expect(alpha).toContain("# END PILOT MANAGED MCP");
...
describe("stageCodexHomeForSync", () => {
  ...
  async function buildFakeHome(root: string): Promise<{ home: string; authBytes: string; skillBytes: string }> {
    ...
    await fs.writeFile(path.join(home, "config.toml"), "model_provider = \"pilot\"\n", "utf8");
  ...
  it("stages exactly the allowlist, derefs auth.json to bytes, and excludes decoys", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-stage-"));
  ...
  it("writes the staged auth.json with mode 0600", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-stage-mode-"));
    let staged: string | null = null;
    try {
  ...
  it("writes the staged config.toml (managed MCP bearer header) with mode 0600", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-stage-toml-mode-"));
    let staged: string | null = null;
    try {
      const home = path.join(root, "codex-home");
      await fs.mkdir(home, { recursive: true });
  ...
  it("writes every staged regular file with mode 0600", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-stage-all-mode-"));
    let staged: string | null = null;
    try {
      const { home } = await buildFakeHome(root);
  ...
  it("creates the staged dir with mode 0700", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-stage-dir-"));
    let staged: string | null = null;
    try {
      const { home } = await buildFakeHome(root);
  ...
  it("skips absent optional entries without throwing", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-stage-absent-"));
    let staged: string | null = null;
    try {
  ...
  it("treats a dangling auth.json symlink as absent (skips it, no throw)", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-stage-dangling-"));
    let staged: string | null = null;
    try {
      const home = path.join(root, "codex-home");
      await fs.mkdir(home, { recursive: true });
  ...
  it("fails closed and removes the temp dir on an unexpected I/O error", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-stage-fail-"));
    try {
      const { home } = await buildFakeHome(root);
  ...
  it("skips circular skill symlinks (ELOOP) without throwing", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-stage-circular-"));
    let staged: string | null = null;
    try {
      const home = path.join(root, "codex-home");
      await fs.mkdir(path.join(home, "skills"), { recursive: true });
  ...
  it("writes nested skill files with mode 0600 regardless of source mode", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-stage-skill-mode-"));
    let staged: string | null = null;
    try {
      const home = path.join(root, "codex-home");
  ...
  it("does not infinitely traverse an ancestor directory link (back -> .) inside a skill", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-stage-cycle-"));
    let staged: string | null = null;
    try {
      const home = path.join(root, "codex-home");
  ...
  it("does not stage a nested skill symlink that escapes the skill root", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-stage-escape-"));
    let staged: string | null = null;
    try {
      const home = path.join(root, "codex-home");
      const skillDir = path.join(home, "skills", "my-skill");
  ...
  it("skips a top-level skills entry that resolves to an ancestor of skills/", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-stage-ancestor-"));
    let staged: string | null = null;
    try {
      const home = path.join(root, "codex-home");
      await fs.mkdir(path.join(home, "skills"), { recursive: true });
