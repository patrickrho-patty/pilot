...
const {
  ...
  tempCodexHome: "/tmp/pilot-codex-stderr-error-test-home",
}));

vi.mock("./acp.js", () => ({
  createCodexAcpExecutor: () => vi.fn(),
  formatCodexAcpFallbackMessage: (reason: string) =>
    `[pilot] Codex ACP default unavailable; falling back to Codex CLI. ${reason} Set engine=acp to require ACP or engine=cli to silence this fallback.\n`,
...
describe("codex_local stderr fallback error derivation", () => {
  ...
  it("skips adapter-injected [pilot] diagnostic lines when picking the fallback error", async () => {
    mockFailedProcess(
      [
        "[pilot] Codex ACP default unavailable; falling back to Codex CLI. Set engine=acp to require ACP or engine=cli to silence this fallback.",
        YOLO_WARNING,
...
describe("firstMeaningfulStderrLine", () => {
  it("returns the first line that is not a known benign warning", () => {
    ...
    expect(firstMeaningfulStderrLine("[pilot] Confining Codex with workspace scope.\nError: boom")).toBe(
      "Error: boom",
    );
  });
