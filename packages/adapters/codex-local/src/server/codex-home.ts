...
import type { AdapterExecutionContext } from "@pilotai/adapter-utils";
import { resolvePilotInstanceRootForAdapter } from "@pilotai/adapter-utils/server-utils";
import { readSubscriptionAccountId } from "./codex-auth-cache.js";

const TRUTHY_ENV_RE = /^(1|true|yes|on)$/i;
const COPIED_SHARED_FILES = ["config.json", "config.toml", "instructions.md"] as const;
const SYMLINKED_SHARED_FILES = ["auth.json"] as const;
const MANAGED_MCP_BLOCK_START = "# BEGIN PILOT MANAGED MCP";
const MANAGED_MCP_BLOCK_END = "# END PILOT MANAGED MCP";
...
function buildManagedMcpBlock(input: {
  ...
  const lines = [
    ...
    "# Written by Pilot for governed MCP gateway access. Do not edit this block by hand.",
  ];
  input.gateways.forEach((gateway, index) => {
    ...
    let managedName = directOverlap ? `pilot-${baseName}` : baseName;
    let suffix = 2;
    while (usedNames.has(managedName) || input.existingNames.has(managedName)) {
      managedName = `pilot-${baseName}-${suffix}`;
      suffix += 1;
    }
    usedNames.add(managedName);
    if (directOverlap) {
      warnings.push(
        `Found unmanaged Codex MCP server "${gateway.name}" overlapping a Pilot-governed gateway; leaving the direct entry in place and adding managed gateway "${managedName}". Pilot cannot enforce policies for that direct entry.`,
      );
    }
...
export async function stageCodexHomeForSync(
  ...
  const stagedHome = await fs.mkdtemp(
    path.join(os.tmpdir(), `pilot-codex-home-sync-${runIdPart ? `${runIdPart}-` : ""}`),
  );
  try {
...
export async function seedManagedCodexHome(
    ...
    if (existing && !existing.isSymbolicLink()) {
      ...
      if (targetIdentity) {
        ...
        if (keepPromotedAuth && sourceReadErrorCode) {
          ...
          await onLog(
            "stdout",
            `[pilot] Keeping the existing subscription auth.json in Codex home "${targetHome}" (shared source read failed: ${sourceReadErrorCode}); the next seed with a readable source reconciles it.\n`,
          );
        }
      }
      if (keepPromotedAuth) {
        await onLog(
          "stdout",
          `[pilot] Keeping the promoted subscription auth.json in Codex home "${targetHome}".\n`,
        );
      } else {
  ...
  if (seedFromShared) {
    ...
    await onLog(
      "stdout",
      `[pilot] Using ${isWorktreeMode(env) ? "worktree-isolated" : "Pilot-managed"} Codex home "${targetHome}" (seeded from "${sourceHome}").\n`,
    );
  }
  ...
  if (apiKey) {
    ...
    await onLog(
      "stdout",
      `[pilot] Wrote API-key auth.json into Codex home "${targetHome}" from configured OPENAI_API_KEY.\n`,
    );
  }
}
