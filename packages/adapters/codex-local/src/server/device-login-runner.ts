...
export async function runDeviceLogin(
  ...
  try {
    if (signal?.aborted) {
      log("[pilot] Device login cancelled before start.");
    ...
    if (raced.kind === "timeout") {
      log("[pilot] Device login timed out; disposing the sandbox.");
      return { outcome: "timeout", exitCode: null, promptSurfaced };
    }
    if (raced.kind === "cancelled") {
      log("[pilot] Device login cancelled; disposing the sandbox.");
      return { outcome: "cancelled", exitCode: null, promptSurfaced };
    }

    const exitCode = raced.exitCode;
    if (exitCode !== 0) {
      log("[pilot] Device login command ended with a non-zero exit code.");
    ...
    log("[pilot] Device login command ended successfully.");
    ...
    try {
      ...
      log("[pilot] Device login: the sandbox dispose step errored.");
    }
  }
}
