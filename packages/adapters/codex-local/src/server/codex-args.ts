import { asBoolean, asString, asStringArray } from "@pilotai/adapter-utils/server-utils";
import {
  ...
  return {
    ...
    fastModeIgnoredReason:
      fastModeRequested && !fastModeApplied
        ? `Configured fast mode is currently only supported on ${formatFastModeSupportedModels()}; Pilot will ignore it for model ${model || "(default)"}.`
        : null,
  };
}
