...
import type { SandboxManagedRuntimeAsset } from "@pilotai/adapter-utils/sandbox-managed-runtime";
...
const REMOTE_RUNTIME_ROOT = "/remote/workspace/.pilot-runtime/codex";
...
describe("codex execute — outbound auth copy-back restore contribution", () => {
  ...
  async function runTeardown(input: {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "pilot-codex-copyback-e2e-"));
