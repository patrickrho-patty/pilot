...
const {
  ...
  prepareAdapterExecutionTargetRuntime: vi.fn(async () => ({
    ...
    runtimeRootDir: "/sandbox/.pilot-runtime",
    assetDirs: { home: "/sandbox/.pilot-runtime/codex/home" },
...
describe("codex sandbox auth precedence warning", () => {
  ...
  it("logs and emits a run event when sandbox login is shadowed by host auth", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-codex-auth-precedence-"));
    ...
    expect(homeAsset?.localDir).toContain("pilot-codex-home-sync");
