...
const {
  ...
  return {
    ...
    prepareAdapterExecutionTargetRuntime: vi.fn(async (input: { assets?: Array<{ key: string; localDir: string }> }) => {
      ...
      return {
        target: null,
        workspaceRemoteDir: "/remote/workspace/.pilot-runtime/runs/test/workspace",
        runtimeRootDir: "/remote/workspace/.pilot-runtime/runs/test/workspace/.pilot-runtime/codex",
        assetDirs: {
          home: "/remote/workspace/.pilot-runtime/runs/test/workspace/.pilot-runtime/codex/home",
        },
        restoreWorkspace,
      };
    }),
    prepareManagedCodexHome: vi.fn(async () => {
      ...
      const dir = await fs.mkdtemp(`${os.tmpdir()}/pilot-managed-codex-home-`);
...
describe("codex remote environment diagnostics", () => {
  ...
  it("stages managed CODEX_HOME in an isolated runtime dir and keeps the probe cwd on the original remote workspace", async () => {
    ...
    expect(homeAsset?.localDir).toContain(`${os.tmpdir()}/pilot-codex-probe-home-`);
    expect(capturedHomeAssetFiles.value).toEqual(["auth.json", "config.toml"]);
    expect(runtimeInput?.workspaceLocalDir).toContain(`${os.tmpdir()}/pilot-codex-envtest-`);
    ...
    expect(probeCall?.[4]).toMatchObject({
      ...
      env: expect.objectContaining({
        CODEX_HOME: "/remote/workspace/.pilot-runtime/runs/test/workspace/.pilot-runtime/codex/home",
      }),
    });
  ...
  it("avoids /tmp CODEX_HOME for remote API-key hello probes", async () => {
    ...
    const probeCall = runAdapterExecutionTargetProcess.mock.calls[0] as unknown as
      ...
      | undefined;
    expect(probeCall?.[4].env.CODEX_HOME).toContain("/remote/workspace/.pilot-runtime/codex/probe-home-codex-envtest-");
  ...
  it("emits the canonical adapter_auth_missing check when a sandbox hello probe reports missing auth", async () => {
    ...
    prepareManagedCodexHome.mockImplementationOnce(async () => {
      const dir = await fs.mkdtemp(`${os.tmpdir()}/pilot-managed-codex-home-noauth-`);
      await fs.writeFile(`${dir}/config.toml`, "model = \"gpt-5\"\n");
      return dir;
    });
