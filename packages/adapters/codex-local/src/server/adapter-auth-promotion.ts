...
export async function checkStagedCredentialReadiness(
  ...
  const scratchHome = await mkdtemp(path.join(os.tmpdir(), "pilot-login-readiness-"));
  try {
...
export async function promoteDeviceLoginCredential(
  ...
  if (!userInitiated) {
    await log("[pilot] Codex device-login promotion: skipped (an automatic background login never seeds a company slot).");
  ...
  if (!soleOwner) {
    await log("[pilot] Codex device-login promotion: skipped (the session no longer holds the sole active claim on the slot).");
  ...
  const homeOutcome = await writeCredentialSeedOrNewer({
    ...
    log,
    writtenLine: "[pilot] Codex device-login promotion: wrote the company credential home at mode 0600.",
    keptLine: "[pilot] Codex device-login promotion: kept the company credential home (the login is not a seed or a strictly-newer same-identity credential).",
  ...
  if (isCodexAuthCacheEnabled(env)) {
    try {
      ...
      await log(
        "[pilot] Codex device-login promotion: the per-identity cache write failed; the company credential home is durable, so the login stays successful.",
      );
    }
  }
