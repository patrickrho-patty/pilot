...
import type { AcpxEngineExecutorOptions } from "@pilotai/adapter-utils/acpx-engine/execute";
import {
  asNumber,
  asString,
  parseObject,
...
export function formatKimiAcpFallbackMessage(reason: string): string {
  return `[pilot] Kimi ACP default unavailable; falling back to Kimi CLI. ${reason} Set engine=acp to require ACP or engine=cli to silence this fallback.\n`;
}
