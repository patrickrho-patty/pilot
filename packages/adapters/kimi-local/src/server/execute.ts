...
function renderApiAccessNote(env: Record<string, string>): string {
  ...
  return [
    ...
    "Use shell commands with curl to make Pilot API requests when needed.",
    "Include x-pilot-Run-Id on mutating requests.",
    "",
    "",
  ].join("\n");
}
...
async function buildKimiSkillsDir(
  ...
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-kimi-skills-"));
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const kimiSkillEntries = await readPilotRuntimeSkillEntries(config, __moduleDir);
  const desiredKimiSkillNames = resolvePilotDesiredSkillNames(config, kimiSkillEntries);
  const envConfig = parseObject(config.env);

  const hasExplicitApiKey =
    typeof envConfig.PILOT_API_KEY === "string" && envConfig.PILOT_API_KEY.trim().length > 0;
  ...
  if (executionTargetIsRemote) {
    try {
      ...
      await onLog(
        "stdout",
        `[pilot] Syncing workspace and Kimi runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
      );
  ...
  if (!executionTargetIsRemote && desiredKimiSkillNames.length > 0) {
    ...
    await onLog(
      "stderr",
      `[pilot] Prepared ${desiredKimiSkillNames.length} Kimi skill(s) for --skills-dir delivery.\n`,
    );
  }
  ...
  if (executionTargetIsRemote && runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[pilot] Kimi session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`,
    );
  } else if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[pilot] Kimi session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
    );
  }
  ...
  try {
    ...
    if (
      ...
      await onLog(
        "stdout",
        `[pilot] Kimi resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
      );
