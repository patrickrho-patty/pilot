...
// retries fail with "model not found". PILOT_OPENCODE_CHEAP_MODEL takes priority;
// PILOT_OPENCODE_SMALL_MODEL (the auxiliary/title model) is reused as a sensible
...
Use when:
- You want Pilot to run OpenCode locally as the agent runtime
...
- dangerouslySkipPermissions (boolean, optional): inject a runtime OpenCode config that allows \`external_directory\` access without interactive prompts; defaults to true for unattended Pilot runs
...
- Pilot requires an explicit \`model\` value for \`opencode_local\` agents.
...
- When \`dangerouslySkipPermissions\` is enabled, Pilot injects a temporary \
