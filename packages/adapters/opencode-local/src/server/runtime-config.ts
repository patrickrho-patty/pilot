...
import { asBoolean } from "@pilotai/adapter-utils/server-utils";
...
function parseProviderConfig(
  ...
  try {
    ...
    notes.push("PILOT_OPENCODE_PROVIDERS contains invalid JSON; custom providers ignored.");
    return null;
  }
  ...
  if (skipped.length > 0) {
    notes.push(
      `PILOT_OPENCODE_PROVIDERS: skipped provider(s) with non-object values: ${skipped.join(", ")}.`,
    );
  }
...
export async function prepareOpenCodeRuntimeConfig(input: {
  ...
  const runtimeConfigHome = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-opencode-config-"));
  ...
  // Merge gateway/custom provider definitions supplied via PILOT_OPENCODE_PROVIDERS
  ...
  if (gatewayProviders) {
    notes.push(
      `Injected ${Object.keys(gatewayProviders).length} custom OpenCode provider(s) from PILOT_OPENCODE_PROVIDERS: ${Object.keys(gatewayProviders).join(", ")}.`,
    );
  }
  ...
  // user config or PILOT_OPENCODE_PROVIDERS.
  ...
  // other helper tasks) via PILOT_OPENCODE_SMALL_MODEL. OpenCode otherwise
