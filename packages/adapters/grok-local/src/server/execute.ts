...
async function stageGrokProjectAssets(input: {
  ...
  if (input.instructionsFilePath) {
    if (!await pathExists(instructionsTarget)) {
      ...
      await input.onLog(
        "stdout",
        `[pilot] Grok workspace already contains ${instructionsTarget}; using --rules @${input.instructionsFilePath} instead of overwriting it.\n`,
      );
    }
  } else {
  ...
  if (selectedSkills.length > 0) {
    ...
    for (const skill of selectedSkills) {
      ...
      if (await pathExists(target)) {
        await input.onLog(
          "stdout",
          `[pilot] Grok skill target already exists at ${target}; leaving it unchanged.\n`,
        );
        continue;
      }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  try {
    ...
    if (executionTargetIsRemote) {
      await onLog(
        "stdout",
        `[pilot] Syncing Grok workspace to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
      );
    ...
    if (executionTargetIsRemote && runtimeSessionId && !canResumeSession) {
      await onLog(
        "stdout",
        `[pilot] Grok session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`,
      );
    } else if (runtimeSessionId && !canResumeSession) {
      await onLog(
        "stdout",
        `[pilot] Grok session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
      );
    }
    ...
    const commandNotes = (() => {
      ...
      if (stagedAssets.stagedSkillsCount > 0) {
        notes.push(`Staged ${stagedAssets.stagedSkillsCount} Pilot skill(s) into .claude/skills for native Grok discovery.`);
      }
      return notes;
    })();
    ...
    if (
      ...
      await onLog(
        "stdout",
        `[pilot] Grok resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
      );
