...
Use when:
- You want Pilot to run Cursor Cloud Agents through the official Cursor SDK
- You want durable remote Cursor agent sessions across Pilot heartbeats
- You want Pilot to keep task state while Cursor handles remote code execution
...
Notes:
- Pilot reuses the durable Cursor agent across heartbeats when the repo/runtime identity still matches.
- Each Pilot heartbeat maps to a Cursor run on that durable agent.
- Pilot injects PILOT_* runtime env vars into the cloud agent shell through Cursor SDK cloud envVars.
- Pilot remains the source of truth for issue/task state; Cursor provides the remote execution surface.
`;
