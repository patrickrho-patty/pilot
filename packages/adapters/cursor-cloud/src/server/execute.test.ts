...
describe("cursor_cloud execute", () => {
  ...
  it("creates a fresh Cursor agent and injects Pilot env without CURSOR_API_KEY", async () => {
    ...
    expect(createMock.mock.calls[0]?.[0]).toMatchObject({
      ...
      name: "Pilot Cursor Cloud Agent",
      model: { id: "gpt-5.4" },
      cloud: {
        env: { type: "cloud" },
        repos: [{ url: "https://github.com/patty-io/pilot.git", startingRef: "main" }],
      },
    });
  ...
  it("omits the Pilot API callback when no run JWT is issued (remote worker cannot call home)", async () => {
  ...
  it("maps non-finished Cursor results to failing Pilot runs", async () => {
