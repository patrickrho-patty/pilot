...
import type { AdapterExecutionContext, AdapterExecutionResult, AdapterInvocationMeta } from "@pilotai/adapter-utils";
import {
...
function buildWakeEnv(ctx: AdapterExecutionContext, configEnv: Record<string, string>): Record<string, string> {
  ...
  // buildPilotEnv always sets PILOT_API_URL, defaulting to the local
  ...
  const workspace = parseObject(context.pilotWorkspace);
...
function renderPilotEnvNote(env: Record<string, string>): string {
  ...
  return [
    "Pilot runtime note:",
    `The following PILOT_* environment variables are available in the cloud agent shell: ${keys.join(", ")}`,
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const wakePrompt = renderPilotWakePrompt(context.pilotWake, { resumedSession: canReuseSession });
  const renderedBootstrapPrompt =
    !canReuseSession && bootstrapPromptTemplate.trim().length > 0
      ? renderTemplate(bootstrapPromptTemplate, templateData).trim()
      : "";
  const renderedPrompt =
    (canReuseSession && wakePrompt.length > 0) || isPilotRecoveryWakePayload(context.pilotWake)
      ? ""
