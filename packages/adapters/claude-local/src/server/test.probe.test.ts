...
const {
  ...
  return {
    ...
    resolveAdapterExecutionTargetCwd: vi.fn(() => "/home/daytona/pilot-workspace"),
  };
});
...
const initLine =
  '{"type":"system","subtype":"init","cwd":"/home/daytona/pilot-workspace","session_id":"abc","tools":["Bash","Read"]}';
...
describe("claude CLI local hello probe hardening", () => {
  ...
  beforeEach(async () => {
    tempDir = await mkdtemp(path.join(os.tmpdir(), "pilot-cli-localprobe-"));
    ...
    resolveAdapterExecutionTargetCwd.mockReturnValue("/home/daytona/pilot-workspace");
  });
  ...
  it("names the local host target on every result", async () => {
    ...
    expect(targetCheck?.message).toContain("Pilot host");
  });
});
