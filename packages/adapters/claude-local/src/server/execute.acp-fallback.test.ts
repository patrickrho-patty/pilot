...
vi.mock("./acp.js", () => ({
  ...
  formatClaudeAcpFallbackMessage: (reason: string) =>
    `[pilot] Claude ACP default unavailable; falling back to Claude CLI. ${reason} Set engine=acp to require ACP or engine=cli to silence this fallback.\n`,
...
describe("claude_local ACP startup fallback", () => {
  ...
  it("trusts the Pilot API URL when network access is allowlisted", async () => {
    const pilotApiUrl = "http://127.0.0.1:4310";
    vi.stubEnv("PILOT_API_URL", pilotApiUrl);
    ...
    expect(runAdapterExecutionTargetProcess).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        localProcessSandbox: expect.objectContaining({
          ...
          networkTrustedUrls: [pilotApiUrl],
        }),
      }),
    );
  });
