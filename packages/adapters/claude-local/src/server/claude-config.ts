...
export async function prepareClaudeConfigSeed(
  ...
  if (copiedFiles.length > 0) {
    await onLog(
      "stdout",
      `[pilot] Prepared Claude config seed "${targetDir}" from "${sourceDir}" (${copiedFiles.map((file) => file.name).join(", ")}).\n`,
    );
  } else {
    await onLog(
      "stdout",
      `[pilot] No local Claude config seed files were found in "${sourceDir}". Remote Claude auth may still require login.\n`,
    );
  }
  ...
  if (
    ...
    try {
      ...
      tempWorkspaceDir = await fs.mkdtemp(
        path.join(os.tmpdir(), "pilot-claude-envtest-workspace-"),
      );
      ...
      const runtimeRootDir =
        ...
        path.posix.join(managedRemoteCwd, ".pilot-runtime", "claude");
      ...
      checks.push({
        ...
        level: "info",
        message: "The environment probe is using Pilot-managed Claude config materialization.",
      ...
      logSandboxProbeDiagnostic(
        "Could not materialize Pilot-managed Claude config for the environment probe",
        "spawn_error",
      ...
      checks.push({
        ...
        level: "error",
        message: "Could not materialize Pilot-managed Claude config for the environment probe.",
