  ...
  if (canRunProbe) {
    if (!claudeCommandLooksLike(command, "claude")) {
      ...
      checks.push({
        ...
        level: "warn",
        message: "Skipped the Claude hello probe because `claude` is not installed on the Pilot host.",
        hint: "Install the `claude` CLI on the Pilot host, then retry the Test.",
      });
    } else {
