...
import type { AdapterExecutionTarget } from "@pilotai/adapter-utils/execution-target";
import {
  DEFAULT_ACP_ENGINE_MODE,
  DEFAULT_ACP_ENGINE_NON_INTERACTIVE_PERMISSIONS,
  DEFAULT_ACP_ENGINE_PERMISSION_MODE,
  DEFAULT_ACP_ENGINE_WARM_HANDLE_IDLE_MS,
} from "@pilotai/adapter-utils/acpx-engine/constants";
import type {
  AcpxEngineExecutorOptions,
  AcpxRemoteManagedHomeContext,
  AcpxRemoteManagedHomeResult,
} from "@pilotai/adapter-utils/acpx-engine/execute";
import {
  asBoolean,
  asNumber,
  asString,
...
export function formatClaudeAcpFallbackMessage(reason: string): string {
  return `[pilot] Claude ACP default unavailable; falling back to Claude CLI. ${reason} Set engine=acp to require ACP or engine=cli to silence this fallback.\n`;
}
...
async function prepareClaudeRemoteManagedHome(
  ...
  const registerWorkspaceSyncBack = (
    ...
    try {
      await onLog("stdout", "[pilot] Restoring workspace changes from the sandbox.\n");
      await stagedRuntime.restoreWorkspace((line) => onLog("stdout", line));
    } catch (err) {
      await onLog(
        "stderr",
        `[pilot] Claude ACP teardown workspace restore failed: ${
  ...
  if (explicitClaudeConfigDir) {
    ...
    if (isUnderWorkspace) {
      ...
      await onLog(
        "stdout",
        `[pilot] Remapped operator CLAUDE_CONFIG_DIR from host path ${explicitClaudeConfigDir} onto the in-sandbox workspace path ${remappedConfigDir} for the remote ACP run.\n`,
      );
      return { stagedRuntime, teardown: registerWorkspaceSyncBack(stagedRuntime) };
    }
    await onLog(
      "stderr",
      `[pilot] operator-provided CLAUDE_CONFIG_DIR=${explicitClaudeConfigDir} is outside the staged workspace and cannot reach the remote sandbox; ignoring the host-only path and seeding the managed Claude config instead.\n`,
    );
  }
  ...
  const remoteClaudeRuntimeRoot =
    ...
    path.posix.join(stagedRuntime.workspaceRemoteDir ?? input.workspaceLocalDir, ".pilot-runtime", "claude");
  ...
  await onLog("stdout", `[pilot] Materializing Claude auth/config into ${remoteClaudeConfigDir}.\n`);
...
export function createClaudeAcpExecutor(options: ClaudeAcpExecutorOptions = {}): ClaudeAcpExecutor {
  ...
  return async (ctx) => {
    ...
    if (!currentExecutor) {
      const { createAcpxEngineExecutor } = await import("@pilotai/adapter-utils/acpx-engine/execute");
...
export async function probeClaudeAcpSandboxLogin(input: {
  ...
  if (targetIsRemote && target) {
    ...
    if (!built.command) {
      return [buildAcpLoginProbeUnavailableCheck("Claude is not installed on the Pilot host.")];
    }
...
export async function testClaudeAcpEnvironment(
  ...
  if (targetIsRemote) {
    checks.push({
      ...
      hint: "Remote ACP requires a bidirectional process target such as SSH or Pilot's sandbox process-session bridge.",
    });
  }
