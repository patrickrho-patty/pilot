...
import type { AdapterExecutionContext, AdapterExecutionResult } from "@pilotai/adapter-utils";
import type { RunProcessResult } from "@pilotai/adapter-utils/server-utils";
import {
...
async function buildClaudeRuntimeConfig(input: ClaudeExecutionInput): Promise<ClaudeRuntimeConfig> {
  ...
  for (const [key, value] of Object.entries(shapedEnvConfig)) {
    ...
    // Runtime PILOT_* always wins over config, and PILOT_API_KEY is
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (localProcessSandbox) {
    ...
    await onLog(
      "stdout",
      `[pilot] Confining Claude with ${scopes} scope.\n`,
    );
  }
  ...
  const preparedExecutionTargetRuntime = executionTargetIsRemote
    ? await (async () => {
        await onLog(
          "stdout",
          `[pilot] Syncing workspace and Claude runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
        );
  ...
  const effectivePromptBundleAddDir = executionTargetIsRemote
    ? preparedExecutionTargetRuntime?.assetDirs.skills ??
      path.posix.join(effectiveExecutionCwd, ".pilot-runtime", "claude", "skills")
    : promptBundle.addDir;
  ...
  const effectiveMcpConfigPath = executionTargetIsRemote
    ? path.posix.join(
        preparedExecutionTargetRuntime?.assetDirs["mcp-config"] ??
          path.posix.join(effectiveExecutionCwd, ".pilot-runtime", "claude", "mcp-config"),
        path.basename(localMcpConfigPath),
      )
    : localMcpConfigPath;
  const remoteClaudeRuntimeRoot = executionTargetIsRemote
    ? preparedExecutionTargetRuntime?.runtimeRootDir ??
      path.posix.join(effectiveExecutionCwd, ".pilot-runtime", "claude")
    : null;
  ...
  if (remoteClaudeConfigDir && remoteClaudeConfigSeedDir) {
    ...
    await onLog(
      "stdout",
      `[pilot] Materializing Claude auth/config into ${remoteClaudeConfigDir}.\n`,
    );
  ...
  if (executionTargetIsSandbox && effort) {
    ...
    if (supportsEffort === false) {
      ...
      await onLog(
        "stderr",
        `[pilot] Claude CLI in the environment does not advertise --effort; omitting configured effort "${effort}". Upgrade the environment CLI/image to restore reasoning-effort control.\n`,
      );
    }
  }
  ...
  if (runtimeSessionId && !isValidUuid) {
    await onLog(
      "stdout",
      `[pilot] Claude session "${runtimeSessionId}" is not a valid UUID and will not be passed to --resume.\n`,
    );
  }
  if (
    executionTargetIsRemote &&
    ...
    await onLog(
      "stdout",
      `[pilot] Claude session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`,
    );
  } else if (
    runtimeSessionId &&
    isValidUuid &&
    ...
    await onLog(
      "stdout",
      `[pilot] Claude session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
    );
  }
  if (runtimeSessionId && runtimePromptBundleKey.length > 0 && runtimePromptBundleKey !== promptBundle.bundleKey) {
    await onLog(
      "stdout",
      `[pilot] Claude session "${runtimeSessionId}" was saved for prompt bundle "${runtimePromptBundleKey}" and will not be resumed with "${promptBundle.bundleKey}".\n`,
    );
  }
  if (runtimeSessionId && !hasMatchingMcpServers) {
    await onLog(
      "stdout",
      `[pilot] Claude session "${runtimeSessionId}" was saved with a different runtime MCP server set and will not be resumed.\n`,
    );
  }
  ...
  const runAttempt = async (resumeSessionId: string | null) => {
    ...
    if (runtimeMcpServers.length > 0) {
      commandNotes.push(
        `Using ${runtimeMcpServers.length} Pilot-managed MCP server(s) from strict config ${effectiveMcpConfigPath}.`,
      );
    }
    if (onMeta) {
      await onMeta({
  ...
  try {
    ...
    if (sessionErrorKind !== null) {
      ...
      await onLog(
        "stdout",
        `[pilot] Claude resume session "${sessionId}" ${reason}; retrying with a fresh session.\n`,
      );
      if (sessionErrorKind === "poisoned" && !executionTargetIsRemote) {
        ...
        if (unlinked) {
          try {
            await onLog("stdout", `[pilot] Removed poisoned session file: ${poisonedJsonlPath}\n`);
          } catch {
