...
describe("preparePiRuntimeConfig", () => {
  it("is a no-op when PILOT_PI_PROVIDERS is unset", async () => {
  ...
  it("reads PILOT_PI_PROVIDERS from process.env when absent from the run env", async () => {
  ...
  it("expands {env:VAR} placeholders from process.env when absent from the run env", async () => {
    const providers = {
      tensorix: { baseUrl: "http://gw/anthropic", apiKey: "{env:PILOT_PI_TEST_KEY}", api: "anthropic-messages", models: [] },
    };
  ...
  it("ignores malformed PILOT_PI_PROVIDERS without writing a config", async () => {
    const prepared = await preparePiRuntimeConfig({
      env: { PILOT_PI_PROVIDERS: "not json" },
    });
    expect(prepared.env.PI_CODING_AGENT_DIR).toBeUndefined();
    expect(prepared.notes).toEqual([
      "PILOT_PI_PROVIDERS contains invalid JSON; custom providers ignored.",
    ]);
    await prepared.cleanup();
  });
  ...
  it("ignores provider entries that are not objects and names them in the note", async () => {
    ...
    expect(prepared.notes).toEqual([
      "PILOT_PI_PROVIDERS: skipped provider(s) with non-object values: tensorix.",
    ]);
    await prepared.cleanup();
  });
  ...
  it("surfaces skipped non-object entries while keeping the usable ones", async () => {
    ...
    expect(prepared.notes).toEqual([
      "PILOT_PI_PROVIDERS: skipped provider(s) with non-object values: bad.",
      "Injected 1 custom Pi provider(s) from PILOT_PI_PROVIDERS into a managed models.json: tensorix.",
    ]);
    await prepared.cleanup();
  });

  it("surfaces a note when PILOT_PI_PROVIDERS contains invalid JSON", async () => {
    const prepared = await preparePiRuntimeConfig({
      env: { PILOT_PI_PROVIDERS: "{not json" },
    });
    expect(prepared.env.PI_CODING_AGENT_DIR).toBeUndefined();
  ...
  it("surfaces a note when PILOT_PI_PROVIDERS is not a JSON object", async () => {
    const prepared = await preparePiRuntimeConfig({
      env: { PILOT_PI_PROVIDERS: "[1,2]" },
    });
    expect(prepared.notes).toEqual([
      "PILOT_PI_PROVIDERS is set but is not a JSON object; custom providers ignored.",
    ]);
    await prepared.cleanup();
  });
  ...
  it("surfaces the skipped entries when no provider objects remain", async () => {
    ...
    expect(prepared.env.PI_CODING_AGENT_DIR).toBeUndefined();
    expect(prepared.notes).toEqual([
      "PILOT_PI_PROVIDERS: skipped provider(s) with non-object values: a.",
    ]);
    await prepared.cleanup();
  });

  it("stays silent when PILOT_PI_PROVIDERS is an empty object", async () => {
    const prepared = await preparePiRuntimeConfig({
