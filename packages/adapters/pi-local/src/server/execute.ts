...
const PILOT_SESSIONS_DIR = path.join(os.homedir(), ".pi", "pilots");
...
async function ensurePiSkillsInjected(
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[pilot] Removed maintainer-only Pi skill "${skillName}" from ${PI_AGENT_SKILLS_DIR}\n`,
    );
  }
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[pilot] ${result === "repaired" ? "Repaired" : "Injected"} Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[pilot] Failed to inject Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function buildPiSkillsDir(config: Record<string, unknown>): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "pilot-pi-skills-"));
...
async function ensureSessionsDir(): Promise<string> {
  await fs.mkdir(PILOT_SESSIONS_DIR, { recursive: true });
  return PILOT_SESSIONS_DIR;
}

function buildSessionPath(agentId: string, timestamp: string): string {
  const safeTimestamp = timestamp.replace(/[:.]/g, "-");
  return path.join(PILOT_SESSIONS_DIR, `${safeTimestamp}-${agentId}.jsonl`);
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const piSkillEntries = await readPilotRuntimeSkillEntries(config, __moduleDir);
  const desiredPiSkillNames = resolvePilotDesiredSkillNames(config, piSkillEntries);
  if (!executionTargetIsRemote) {
  ...
  // Materialize custom Pi providers (PILOT_PI_PROVIDERS) into a managed
  ...
  try {
    ...
    if (executionTargetIsRemote) {
      try {
        ...
        await onLog(
          "stdout",
          `[pilot] Syncing workspace and Pi runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
        );
    ...
    if (runtimeSessionId && !canResumeSession) {
      const staleSessionCwdNote =
        savedSessionCwd !== null && !sessionHeaderCwdMatches
          ? ` Pi stored cwd "${savedSessionCwd}" in the session header, so Pilot will start a fresh session for "${effectiveExecutionCwd}".`
          : "";
      await onLog(
        "stdout",
        executionTargetIsRemote
          ? `[pilot] Pi session "${runtimeSessionId}" does not match the current remote execution state and will not be resumed in "${effectiveExecutionCwd}".${staleSessionCwdNote} Starting a fresh remote session.\n`
          : `[pilot] Pi session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".${staleSessionCwdNote}\n`,
      );
    }
    ...
    if (resolvedInstructionsFilePath) {
      try {
        ...
        await onLog(
          "stdout",
          `[pilot] Warning: could not read agent instructions file "${resolvedInstructionsFilePath}": ${reason}\n`,
        );
    ...
    const renderedBootstrapPrompt =
      !canResumeSession && bootstrapPromptTemplate.trim().length > 0
        ...
        : "";
    const wakePrompt = renderPilotWakePrompt(context.pilotWake, { resumedSession: canResumeSession });
    const shouldUseResumeDeltaPrompt = canResumeSession && wakePrompt.length > 0;
    const renderedHeartbeatPrompt = shouldUseResumeDeltaPrompt || isPilotRecoveryWakePayload(context.pilotWake)
      ? ""
    ...
    try {
      ...
      if (
        ...
        await onLog(
          "stdout",
          `[pilot] Pi session "${runtimeSessionId}" is unavailable; retrying with a fresh session.\n`,
        );
