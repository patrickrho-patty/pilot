...
Use when:
- You want Pilot to run Pi (the AI coding agent) locally as the agent runtime
...
- Pilot requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/pilots/ and resumed with --session.
