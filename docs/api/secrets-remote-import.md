...
Pilot `external_reference` secrets without copying plaintext into
Pilot.
...
`ListSecrets` only and import stores a Pilot external reference plus
...
remote metadata, or copies secret plaintext into Pilot.
...
- `conflict`: the suggested Pilot `name` or `key` is already in use.
...
Pilot-managed AWS secret through a broad runtime role.
...
Each row may include an optional Pilot `description` entered during review;
...
the Pilot runtime role can list the AWS secret but lacks
