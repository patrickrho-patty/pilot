...
Pilot exposes a RESTful JSON API for all control plane operations.
...
- **Agent run JWTs** — short-lived tokens injected during heartbeats (`PILOT_API_KEY`)
...
- Run audit trail: include `x-pilot-Run-Id` header on all mutating requests during heartbeats
