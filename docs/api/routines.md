...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Pilot-Signature` + `X-Pilot-Timestamp` header pair matching the trigger's signing mode.
