...
8. Run the focused tests from the repo root with `pnpm --filter @pilotai/server exec vitest run src/__tests__/built-in-agents.test.ts src/__tests__/built-in-agent-routes.test.ts`.
...
```sh
pnpm --filter @pilotai/server exec vitest run src/__tests__/built-in-agents.test.ts src/__tests__/built-in-agent-routes.test.ts
```
