# ClipHub: Marketplace for Pilot Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Pilot configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Pilot-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Pilot company (agents, projects, governance) |
...
A complete Pilot company configuration:
...
Single-agent configurations designed to plug into a Pilot org:
...
Portable skill files that any Pilot agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Pilot's skill loading system
...
interface Listing {
  ...
  pilotVersionMin: string;     // Minimum Pilot version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Pilot company
...
| `POST` | `/api/listings/:id/install` | Install to Pilot company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Pilot company (or create new)
  → ClipHub API calls Pilot API to:
  ...
  → Redirect to Pilot dashboard with new team running
```
...
### 5.3 Creator: Export from Pilot → Publish

```
Running Pilot company → "Export as Blueprint" (CLI or UI)
  → Pilot exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Pilot brand blue/purple)
...
| Install flow | One-click deploy to Pilot company |
...
{
  "targetCompanyId": "uuid",         // Existing Pilot company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `pilot-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Pilot
- **Backend**: Node.js API (or extend Pilot server)
- **Database**: Postgres (can share Pilot's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Pilot auth (or OAuth2)

### 9.2 Integration with Pilot

ClipHub can be:
- **Option A**: A separate app that calls Pilot's API to install blueprints
- **Option B**: A built-in section of the Pilot UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Pilot UI and API.
...
- [ ] Install flow: blueprint → Pilot company
