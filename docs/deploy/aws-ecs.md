...
summary: Deploy Pilot to AWS using ECS Fargate, RDS Postgres, and EFS
---

Deploy Pilot to AWS with ECS Fargate (compute), RDS Postgres 17 (database), and EFS (persistent storage). This guide uses the AWS CLI and produces a single-task ECS service behind an ALB with HTTPS.
...
export PILOT_DOMAIN=pilot.example.com   # your domain
...
# Build
docker build -t pilot-server .

# Tag and push
docker tag pilot-server:latest \
  $AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/pilot-server:latest
...
ALB_SG=$(aws ec2 create-security-group \
  --group-name pilot-alb \
  --description "Pilot ALB" \
  --vpc-id $VPC_ID \
...
ECS_SG=$(aws ec2 create-security-group \
  --group-name pilot-ecs \
  --description "Pilot ECS tasks" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
RDS_SG=$(aws ec2 create-security-group \
  --group-name pilot-rds \
  --description "Pilot RDS" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
EFS_SG=$(aws ec2 create-security-group \
  --group-name pilot-efs \
  --description "Pilot EFS" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
aws rds create-db-subnet-group \
  --db-subnet-group-name pilot-db-subnet \
  --db-subnet-group-description "Pilot RDS subnets" \
  --subnet-ids $SUBNET_1 $SUBNET_2

aws rds create-db-instance \
  --db-instance-identifier pilot-db \
  --db-instance-class db.t4g.micro \
  --engine postgres \
  --engine-version 17 \
  --master-username pilot \
...
DATABASE_URL="postgresql://pilot:${DB_PASSWORD}@${RDS_ENDPOINT}:5432/pilot"
```
...
EFS_ID=$(aws efs create-file-system \
  ...
  --encrypted \
  --tags Key=Name,Value=pilot-data \
...
aws secretsmanager create-secret \
  --name pilot/better-auth-secret \
  --secret-string "$AUTH_SECRET"

aws secretsmanager create-secret \
  --name pilot/openai-api-key \
...
aws iam put-role-policy \
  ...
  --policy-document '{
    ...
    "Statement": [{
      ...
      "Resource": "arn:aws:secretsmanager:'$AWS_REGION':'$AWS_ACCOUNT_ID':secret:pilot/*"
    }]
  }'

# Task role (application — add permissions as needed)
aws iam create-role \
  --role-name pilot-ecs-task \
  --assume-role-policy-document '{
    "Version": "2012-10-17",
    "Statement": [{
      "Effect": "Allow",
...
```bash
aws ecs create-cluster --cluster-name pilot

aws logs create-log-group --log-group-name /ecs/pilot
```
...
sed -e "s|<ACCOUNT_ID>|$AWS_ACCOUNT_ID|g" \
    ...
    -e "s|<EFS_ID>|$EFS_ID|g" \
    -e "s|<DOMAIN>|$PILOT_DOMAIN|g" \
    docker/ecs-task-definition.json > /tmp/pilot-task-def.json

aws ecs register-task-definition \
  --cli-input-json file:///tmp/pilot-task-def.json
```
...
- Create a CNAME or ALIAS record for `$PILOT_DOMAIN` -> `$ALB_DNS`
...
# Check task health
aws ecs list-tasks --cluster pilot --service-name pilot-server
TASK_ARN=$(aws ecs list-tasks --cluster pilot --service-name pilot-server --query 'taskArns[0]' --output text)
aws ecs describe-tasks --cluster pilot --tasks $TASK_ARN \
  --query 'tasks[0].{status:lastStatus,health:healthStatus}'

# Check logs
aws logs tail /ecs/pilot --since 10m --follow

# Hit the health endpoint
curl -sf https://$PILOT_DOMAIN/api/health
```
...
#   { "name": "PILOT_AUTH_DISABLE_SIGN_UP", "value": "true" }
...
aws ecs update-service \
  --cluster pilot \
  --service pilot-server \
  --task-definition pilot-server:<PREVIOUS_REVISION>
```
...
```bash
aws rds stop-db-instance --db-instance-identifier pilot-db
aws rds start-db-instance --db-instance-identifier pilot-db
```
...
aws ecs update-service --cluster pilot --service pilot-server --desired-count 0
aws ecs delete-service --cluster pilot --service pilot-server --force
aws ecs delete-cluster --cluster pilot
...
aws rds delete-db-instance \
  --db-instance-identifier pilot-db \
  --final-db-snapshot-identifier pilot-db-final
aws rds wait db-instance-deleted --db-instance-identifier pilot-db
aws rds delete-db-subnet-group --db-subnet-group-name pilot-db-subnet
...
for s in database-url anthropic-api-key better-auth-secret openai-api-key github-token; do
  aws secretsmanager delete-secret --secret-id pilot/$s --force-delete-without-recovery
done
...
# 7. ECR
aws ecr delete-repository --repository-name pilot-server --force

# 8. IAM roles
aws iam delete-role-policy --role-name pilot-ecs-execution --policy-name SecretsAccess
aws iam detach-role-policy --role-name pilot-ecs-execution \
  --policy-arn arn:aws:iam::aws:policy/service-role/AmazonECSTaskExecutionRolePolicy
aws iam delete-role --role-name pilot-ecs-execution
aws iam delete-role --role-name pilot-ecs-task

# 9. Log group
aws logs delete-log-group --log-group-name /ecs/pilot
```
