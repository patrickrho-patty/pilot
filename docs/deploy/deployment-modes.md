...
Pilot supports two runtime modes with different security profiles. Reachability is configured separately with `bind`.
...
When migrating from `local_trusted` to `authenticated`, Pilot emits a one-time claim URL at startup:
...
```sh
PILOT_DEPLOYMENT_MODE=authenticated PILOT_BIND=lan pnpm pilotai run
```
