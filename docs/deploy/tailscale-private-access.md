...
summary: Run Pilot with Tailscale-friendly bind presets and connect from other devices
---

Use this when you want to access Pilot over Tailscale (or a private LAN/VPN) instead of only `localhost`.

## 1. Start Pilot in private authenticated mode
...
From the machine running Pilot:
...
## 3. Open Pilot from another device

Use the Tailscale IP or MagicDNS host with the Pilot port:
...
- Login or redirect errors on a private hostname: add it with `pilotai allowed-hostname`.
