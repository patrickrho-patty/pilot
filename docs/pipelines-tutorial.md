...
Run this against a dev Pilot instance with a board token or an agent token that can manage pipelines, routines, and issues.

```sh
export PILOT_API_URL=http://localhost:3100
export PILOT_COMPANY_ID=<company-id>
export PILOT_API_KEY=<token>
...
pilotai pipelines set-transitions \
  -C "$PILOT_COMPANY_ID" \
  "$RELEASE_PIPELINE" \
...
pilotai pipelines guidance put \
  -C "$PILOT_COMPANY_ID" \
  "$CONTENT_PIPELINE" \
...
export DRAFTING_ROUTINE_ID="$(
  pilotai routine create \
    -C "$PILOT_COMPANY_ID" \
    --payload-json "$(jq -c . /tmp/drafting-routine.json)" \
    --json | jq -r '.id'
)"

pilotai pipelines set-automation \
  -C "$PILOT_COMPANY_ID" \
  "$CONTENT_PIPELINE" \
  --stage drafting \
...
pilotai pipelines ingest-batch \
  -C "$PILOT_COMPANY_ID" \
  "$FEATURE_PIPELINE" \
...
export SUGGESTION_ID="$(
  pilotai pipelines case suggest \
    -C "$PILOT_COMPANY_ID" \
    "$BLOG_CASE" \
    --to assets \
...
pilotai pipelines case resolve-suggestion \
  -C "$PILOT_COMPANY_ID" \
  "$BLOG_CASE" \
...
curl -sS -X POST \
  ...
  "$PILOT_API_URL/api/cases/$TWEET_CASE/issue-links" >/dev/null

pilotai pipelines case edit \
  -C "$PILOT_COMPANY_ID" \
  "$BLOG_CASE" \
  --expected-version 2 \
...
# Recovery:
pilotai pipelines case get -C "$PILOT_COMPANY_ID" "$BLOG_CASE" --json
```
...
```sh
export BLOG_VERSION="$(pilotai pipelines case get -C "$PILOT_COMPANY_ID" "$BLOG_CASE" --json | jq -r '.case.version')"
...
pilotai pipelines case transition -C "$PILOT_COMPANY_ID" "$HERO_CASE" --to published --expected-version 1 --reason "Hero image done."
pilotai pipelines case transition -C "$PILOT_COMPANY_ID" "$CARD_CASE" --to dropped --expected-version 1 --reason "Social card not needed."
```

When both asset cases are terminal, move the blog case to `assembly`.

```sh
export BLOG_ASSETS_VERSION="$(pilotai pipelines case get -C "$PILOT_COMPANY_ID" "$BLOG_CASE" --json | jq -r '.case.version')"

pilotai pipelines case transition \
  -C "$PILOT_COMPANY_ID" \
  "$BLOG_CASE" \
  --to assembly \
...
pilotai pipelines case transition -C "$PILOT_COMPANY_ID" "$ASSEMBLY_CASE" --to published --expected-version 1 --reason "Assembly complete."
```
...
```sh
export BLOG_REVIEW_VERSION="$(pilotai pipelines case get -C "$PILOT_COMPANY_ID" "$BLOG_CASE" --json | jq -r '.case.version')"

pilotai pipelines case review \
  -C "$PILOT_COMPANY_ID" \
  "$BLOG_CASE" \
  --approve \
...
```sh
pilotai pipelines case transition -C "$PILOT_COMPANY_ID" "$CHANGELOG_CASE" --to final_review --expected-version 1 --reason "Draft ready for final review."
...
pilotai pipelines case transition -C "$PILOT_COMPANY_ID" "$CHANGELOG_CASE" --to final_review --expected-version 4 --reason "Revised draft ready."
pilotai pipelines case review -C "$PILOT_COMPANY_ID" "$CHANGELOG_CASE" --approve --expected-version 5
pilotai pipelines case transition -C "$PILOT_COMPANY_ID" "$CHANGELOG_CASE" --to published --expected-version 6 --reason "Published after request-changes loop."
```
...
```sh
pilotai pipelines case transition -C "$PILOT_COMPANY_ID" "$TWEET_CASE" --to final_review --expected-version 1 --reason "Blog blocker is now done."
