...
Add first-class **documents** to Pilot as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Pilot workflow/docs.
...
Update the Pilot agent workflow so planning no longer edits the issue description.
