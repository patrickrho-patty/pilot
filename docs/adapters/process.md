...
- Running a Python script that calls the Pilot API
...
1. Pilot spawns the configured command as a child process
2. Standard Pilot environment variables are injected (`PILOT_AGENT_ID`, `PILOT_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the Pilot API and perform work.
