...
- `hermes_local` starts the local Hermes CLI from
  `@pilotai/hermes-pilot-adapter`.
- `hermes_gateway` calls an already-running Hermes API server through
  `@pilotai/hermes-pilot-adapter/gateway`.

The legacy `@pilotai/adapter-hermes-gateway` package is a deprecated
...
depend on or link `@pilotai/hermes-pilot-adapter` and declare the type
...
```json
{
  ...
  "dependencies": {
    "@pilotai/adapter-utils": "^2026.325.0",
    "picocolors": "^1.1.0"
  },
  "devDependencies": {
    "@types/node": "^24.0.0",
...
```ts
import type { ServerAdapterModule } from "@pilotai/adapter-utils";
...
#### Available Helpers from `@pilotai/adapter-utils`
...
For development, you can also edit `~/.pilot/adapter-plugins.json` directly:
...
```ts
import type { AdapterSessionCodec } from "@pilotai/adapter-utils";
