...
Build a custom adapter to connect Pilot to any agent runtime.
...
|---|---|---|
| Source | Inside `pilot-fork` | Separate npm package |
| Distribution | Ships with Pilot | Independent npm publish |
...
For most cases, **build an external adapter plugin**. It's cleaner, independently versioned, and doesn't require modifying Pilot's source. See [External Adapters](/adapters/external-adapters) for the full guide.
...
1. Read config using safe helpers (`asString`, `asNumber`, etc.) from `@pilotai/adapter-utils/server-utils`
2. Build environment with `buildPilotEnv(agent)` plus context vars
...
|--------|--------|---------|
| `runChildProcess(cmd, opts)` | `@pilotai/adapter-utils/server-utils` | Spawn with timeout, grace, streaming |
| `buildPilotEnv(agent)` | `@pilotai/adapter-utils/server-utils` | Inject `PILOT_*` env vars |
| `renderTemplate(tpl, data)` | `@pilotai/adapter-utils/server-utils` | `{{variable}}` substitution |
| `asString(v)` | `@pilotai/adapter-utils` | Safe config value extraction |
| `asNumber(v)` | `@pilotai/adapter-utils` | Safe number extraction |
...
For built-in adapters registered in Pilot's source:
...
`format-event.ts` — pretty-prints stdout for `pilotai run --watch` using `picocolors`.
...
With these flags set, the Pilot UI will automatically show the instructions bundle editor, skills management tab, and working directory field for agents using this adapter — no Pilot source changes required.
...
Make Pilot skills discoverable to your agent runtime without writing to the agent's working directory:
