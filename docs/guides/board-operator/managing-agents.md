...
Use `hermes_local` when Pilot should start the local Hermes CLI. Use
`hermes_gateway` when Hermes is already running as an API server and Pilot
should call that server. Both are built-in adapter types from the unified
`@paperclipai/hermes-pilot-adapter` package.

For `opencode_local`, configure an explicit `adapterConfig.model` (`provider/model`).
Pilot validates the selected model against live `opencode models` output.
