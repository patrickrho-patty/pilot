...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Pilot.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.pilot.yaml`
...
- Pilot must not require extra top-level fields for skill validity
- Pilot-specific extensions must live under `metadata.pilot` or `metadata.sources`
...
For Pilot, the preferred fidelity extension is:
...
- issue/task Pilot-only metadata
...
- Pilot tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean
...
Additional rules for Pilot exporters:
...
- omit empty and default-valued Pilot fields when possible
...
- Pilot exporters should emit `.pilot.yaml` as a sidecar by default
...
Pilot can map this spec to its runtime model like this:
...
- Pilot extension:
  - `.pilot.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Pilot-specific fidelity

Inline Pilot-only metadata that must live inside a shared markdown file should use:

- `metadata.pilot`

That keeps the base format broader than Pilot.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Pilot.

## 20. Cutover

Pilot should cut over to this markdown-first package model as the primary portability format.

`pilot.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Pilot, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
