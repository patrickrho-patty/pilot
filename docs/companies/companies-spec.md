...
- Pilot-specific extensions must live under `metadata.paperclip` or `metadata.sources`
...
- Pilot exporters should emit `.paperclip.yaml` as a sidecar by default
...
- Pilot extension:
  - `.paperclip.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Pilot-specific fidelity
