...
import type { HeartbeatRun, Issue } from "@pilotai/shared";
...
function PackageFileTreeDemo({ empty = false }: { empty?: boolean }) {
  ...
  const [selectedFile, setSelectedFile] = useState<string | null>(empty ? null : "tasks/PIL-1677.md");

  useEffect(() => {
    setExpandedDirs(collectAllPaths(nodes, "dir"));
    setCheckedFiles(allFilePaths);
    setSelectedFile(empty ? null : "tasks/PIL-1677.md");
        ...
        <div className="overflow-hidden rounded-lg border border-border bg-background/70">
          {rows.map((row) => (
            <EntityRow
              ...
              to={row.id === "issue" ? "/PIL/issues/PIL-1677" : undefined}
            />
          ))}
          {rows.length === 0 && (
...
function CompanyPatternIconMatrix() {
  const companies = [
    { name: "Pilot Storybook", color: "#0f766e" },
...
function AsciiArtAnimationDemo({ loading = false }: { loading?: boolean }) {
  return (
    <StoryShell>
      <Section eyebrow="AsciiArtAnimation" title={loading ? "Loading art surface" : "Animated ASCII pilot field"}>
