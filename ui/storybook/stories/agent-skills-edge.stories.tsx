...
import type { Agent, AgentSkillSnapshot, CompanySkillListItem } from "@pilotai/shared";
import { AgentSkillsTab } from "@/pages/agent-skills/AgentSkillsTab";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { queryKeys } from "@/lib/queryKeys";

// QA-only fixtures (PIL-13195) to exercise AC5 (stale desired-skill warning) and
...
const library: CompanySkillListItem[] = [
  libSkill("pilot", "Pilot", "Coordination skill: heartbeats, checkout, comments, and routine API patterns."),
  libSkill("design-guide", "Design guide", "Pilot UI design system reference: tokens, typography, status colors."),
];
...
function staleSnapshot(): AgentSkillSnapshot {
  return {
    ...
    supported: true,
    mode: "ephemeral",
    desiredSkills: ["pilot", "legacy-retired-skill"],
    warnings: [],
    entries: [
      {
        key: "pilot",
        ...
        originLabel: "Managed by Pilot",
        readOnly: false,
        sourcePath: "skills/pilot",
        targetPath: null,
  ...
  return {
    ...
    entries: [
      ...
      },
      {
        ...
        sourcePath: null,
        targetPath: null,
        detail: "Detected in the adapter's skills directory; managed outside Pilot.",
      },
...
export const StaleWarning: StoryObj = {
  ...
  render: () => (
    ...
    >
      <EdgeState agent={buildAgent("agent-stale", ["pilot", "legacy-retired-skill"])} snapshot={staleSnapshot()} />
    </StoryFrame>
  ),
};
...
export const DetectedAdapter: StoryObj = {
  ...
  render: () => (
    <StoryFrame
      ...
      subtitle="A user-installed skill outside Pilot's management shows in a collapsed, read-only section with a lock icon."
    >
      <EdgeState agent={buildAgent("agent-detected", ["pilot"])} snapshot={detectedSnapshot()} />
    </StoryFrame>
  ),
};
