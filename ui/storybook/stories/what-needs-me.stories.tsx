...
import type { AttentionItem, AttentionSourceKind, AttentionSeverity, InboxDismissalKind } from "@pilotai/shared";
...
const POPULATED: AttentionItem[] = [
  item(
    ...
    { subject: { kind: "recovery_action", id: "r1", companyId, title: "Run watchdog escalated — agent stalled 40m", identifier: null, status: "escalated", href: "/PIL/issues/PIL-1000", metadata: {} } },
  ),
  item(
    "appr-1",
    "approval",
    ...
    "Approval is pending a board decision.",
    {
      ...
      subject: { kind: "approval", id: "approval-1", companyId, title: "Hire agent: Research Analyst", identifier: null, status: "pending", href: "/PIL/approvals/approval-1", metadata: { type: "hire_agent" } },
      relatedIssue: null,
      decisionVerbs: [
        { id: "approve", label: "Approve", description: null },
        { id: "reject", label: "Reject", description: null },
  ...
  item(
    ...
    {
      inlineResolvable: true,
      subject: { kind: "interaction", id: "interaction-1", companyId, title: "Which rollout order should we use?", identifier: null, status: "pending", href: "/PIL/issues/PIL-1000#interaction-1", metadata: { kind: "ask_user_questions", issueId: "issue-1000" } },
  ...
  item(
    ...
    {
      inlineResolvable: true,
      subject: { kind: "join_request", id: "join-1", companyId, title: "alex@acme.dev wants to join", identifier: null, status: "pending_approval", href: "/PIL/settings/access", metadata: {} },
      relatedIssue: null,
    },
  ),
  item(
...
const SHOWCASE: AttentionItem[] = [
  {
    ...item("plan-1", "issue_thread_interaction", "high", "Approve plan: Attention queue redesign", "A plan is awaiting your approval.", {
      inlineResolvable: true,
      subject: { kind: "interaction", id: "intx-plan", companyId, title: "Approve plan: Attention queue redesign", identifier: null, status: "pending", href: "/PIL/issues/PIL-1000#plan", metadata: { kind: "request_confirmation", issueId: "issue-1000" } },
      decisionVerbs: [
  ...
  },
  {
    ...item("conf-1", "approval", "medium", "Confirm: publish release notes", "A confirmation is pending.", {
      inlineResolvable: true,
      subject: { kind: "approval", id: "appr-conf", companyId, title: "Confirm: publish release notes", identifier: null, status: "pending", href: "/PIL/approvals/appr-conf", metadata: {} },
      relatedIssue: null,
      decisionVerbs: [
        { id: "approve", label: "Approve", description: null },
        { id: "reject", label: "Reject", description: null },
  ...
  },
  {
    ...item("qs-1", "issue_thread_interaction", "medium", "Answer 2 questions on rollout", "Questions need answers.", {
      inlineResolvable: true,
      subject: { kind: "interaction", id: "intx-qs", companyId, title: "Answer 2 questions on rollout", identifier: null, status: "pending", href: "/PIL/issues/PIL-1000#qs", metadata: { kind: "ask_user_questions", issueId: "issue-1000" } },
  ...
  },
  {
    ...item("join-2", "join_request", "medium", "alex@acme.dev wants to join", "Join request pending.", {
      inlineResolvable: true,
      subject: { kind: "join_request", id: "join-2", companyId, title: "alex@acme.dev wants to join", identifier: null, status: "pending_approval", href: "/PIL/settings/access", metadata: {} },
      relatedIssue: null,
    }),
...
// Image variations (PIL-13544): a row with images can be expanded by clicking
...
const IMAGE_ROWS: AttentionItem[] = [
  ...
  },
  {
    ...item("img-questions", "issue_thread_interaction", "medium", "Answer 2 questions on rollout", "Questions need answers.", {
      inlineResolvable: true,
      subject: { kind: "interaction", id: "intx-img", companyId, title: "Answer 2 questions on rollout", identifier: null, status: "pending", href: "/PIL/issues/PIL-1000#qs", metadata: { kind: "ask_user_questions", issueId: "issue-1000" } },
      decisionVerbs: [{ id: "respond", label: "Answer", description: null }],
      project: { id: "proj-alpha", name: "Alpha", urlKey: "alpha", color: "#0f766e", icon: "rocket" },
    }),
    activityAt: new Date(NOW - 90 * 60 * 1000).toISOString(),
...
/**
 * Collapsed image rows (PIL-13544). Each row shows up to three small
...
/**
 * An expanded image row (PIL-13544). The 5-image review row is pre-expanded so
