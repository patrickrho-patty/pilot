...
import { storybookAgentMap } from "../fixtures/pilotData";
...
// ---------------------------------------------------------------------------
// MCP tool-approval card (PIL-13745). A `request_confirmation` carrying a
...
/**
 * PIL-17280: the audience row every pending card now carries. `Anyone` is the
...
export const ResolverAudienceStates: Story = {
  render: () => (
    <StoryFrame>
      <Section eyebrow="Resolver audience" title="Who can respond (PIL-17280)">
        <div className="grid gap-6 xl:grid-cols-2">
          <ScenarioCard
...
export const ToolActionAllStates: Story = {
  render: () => (
    <StoryFrame>
      <Section eyebrow="MCP Tool Approval" title="All six lifecycle states (PIL-13745)">
        <div className="grid gap-6 xl:grid-cols-2">
...
export const ReviewSurface: Story = {
  render: () => (
    <StoryFrame>
      <section className="pilot-story__frame p-6">
        <div className="pilot-story__label">Thread interactions</div>
  ...
  parameters: {
    docs: {
      description: {
        story:
          "Covers the prototype states called out in [PIL-1709](/PIL/issues/PIL-1709): suggested-task previews, collapsed descendants, rejection reasons, request confirmations, multi-question answers, and a mixed issue thread.",
      },
    },
  },
};
