...
import type { Agent, CompanySecret, EnvBinding, Project, RoutineVariable } from "@pilotai/shared";
...
import { createIssue, storybookAgents } from "../fixtures/pilotData";
...
function StoryShell({ children }: { children: ReactNode }) {
  return (
    <div className="pilot-story">
      <main className="pilot-story__inner space-y-6">{children}</main>
    </div>
  );
}
...
See [the implementation notes](https://github.com/pilotai/pilot).`;
...
const adapterSchema: JsonSchemaNode = {
  ...
  properties: {
    ...
    apiKey: {
      ...
      title: "API key",
      format: "secret-ref",
      description: "Stored with the active Pilot secret provider.",
    },
    concurrency: {
      type: "integer",
...
const validAdapterValues = {
  ...
  allowedCommands: ["pnpm --filter @pilotai/ui typecheck", "pnpm build-storybook"],
...
const storybookProject: Project = {
  ...
  goals: [{ id: "goal-company", title: "We're building Pilot" }],
  name: "Board UI",
  ...
  codebase: {
    ...
    defaultRef: "master",
    repoName: "pilot",
    localFolder: "/Users/dotta/pilot",
    managedFolder: "pilot",
    effectiveLocalFolder: "/Users/dotta/pilot",
    origin: "local_folder",
  },
  workspaces: [],
...
const entityOptions: InlineEntityOption[] = [
  { id: "issue-1672", label: "Storybook forms and editors", searchText: "PIL-1672 ui story coverage" },
    ...
    <StoryShell>
      <section className="pilot-story__frame p-6">
        <div className="flex flex-wrap items-start justify-between gap-5">
          <div>
            <div className="pilot-story__label">Forms and editors</div>
            <h1 className="mt-2 text-3xl font-semibold tracking-tight">Pilot form controls under realistic state</h1>
...
const meta = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          "Fixture-backed stories for Pilot form controls, markdown editors, inline editors, schedule controls, runtime-variable dialogs, and selection pickers.",
      },
    },
  },
} satisfies Meta;
