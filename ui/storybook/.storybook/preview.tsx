...
function installStorybookApiFixtures() {
  ...
  const currentWindow = window as typeof window & {
    __pilotStorybookFetchInstalled?: boolean;
  };
  if (currentWindow.__pilotStorybookFetchInstalled) return;

  const originalFetch = window.fetch.bind(window);
  currentWindow.__pilotStorybookFetchInstalled = true;
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (adapterSchemaMatch) {
      ...
      const schemas = (window as typeof window & {
        __pilotStorybookAdapterSchemas?: Record<string, unknown>;
      }).__pilotStorybookAdapterSchemas;
...
function StorybookProviders({
  ...
  return (
    <QueryClientProvider client={queryClient}>
      <ThemeProvider>
        <MemoryRouter initialEntries={["/PIL/storybook"]}>
          <CompanyProvider>
