...
export function HermesGatewayConfigFields({
  ...
  const pilotApiUrl = String(readValue("pilotApiUrl", "") ?? "");
  ...
  return (
    <>
      <Field
        label="API base URL"
        hint="Hermes API server base URL that Pilot can reach, such as http://127.0.0.1:8642 or a private HTTPS URL. Default dashboard root/chat URLs such as http://127.0.0.1:9119/chat are accepted and map to /api."
      >
        <DraftInput
          value={apiBaseUrl}
      ...
      <SecretField
        ...
        placeholder="Hermes API_SERVER_KEY, not PILOT_API_KEY"
        stored={!isCreate && hasStoredApiKey && !editApiKeyValue}
      />

      <Field
        label="Pilot API URL"
        hint="Optional Pilot API URL reachable by the Hermes host. This is not a credential."
      >
        <DraftInput
          value={pilotApiUrl}
          onCommit={(v) => writeValue("pilotApiUrl", v || undefined)}
          immediate
          className={inputClass}
