...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
        ...
        <Field label="Session key">
          <DraftInput
            value={
              isCreate
                ? values!.sessionKey ?? ""
                : eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "pilot"))
            }
            onCommit={(v) =>
              isCreate
                ? set!({ sessionKey: v })
      ...
      <Field label="Pilot API URL override">
        <DraftInput
          value={
            isCreate
              ? values!.pilotApiUrl ?? ""
              : eff("adapterConfig", "pilotApiUrl", String(config.pilotApiUrl ?? ""))
          }
          onCommit={(v) =>
            isCreate
              ? set!({ pilotApiUrl: v })
              : mark("adapterConfig", "pilotApiUrl", v || undefined)
          }
          immediate
          className={inputClass}
          placeholder="https://pilot.example"
        />
      </Field>
      ...
      {!isCreate && (
        <Field label="Claimed API key path">
          <DraftInput
            ...
            immediate
            className={inputClass}
            placeholder="~/.openclaw/workspace/pilot-claimed-api-key.json"
          />
        </Field>
      )}
      ...
      <Field label="Device auth">
        <div className="text-xs text-muted-foreground leading-relaxed">
          When enabled, Pilot persists a device key during onboarding so pairing approvals
