...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@pilotai/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@pilotai/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@pilotai/adapter-utils").TranscriptEntry[];
