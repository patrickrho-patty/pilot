...
describe("BreadcrumbContext", () => {
  ...
  it("does not rerender consumers when breadcrumbs are set to the same values", () => {
    ...
    act(() => {
      updateBreadcrumbs?.([{ label: "Issues", href: "/issues" }, { label: "PIL-1488" }]);
    });
  ...
  it("rerenders consumers when only the crumb identifier changes", () => {
    ...
    act(() => {
      updateBreadcrumbs?.([{ label: "First task prompt", identifier: "PIL-1204" }]);
    });
    ...
    act(() => {
      updateBreadcrumbs?.([{ label: "First task prompt", identifier: "PIL-1205" }]);
    });
  ...
  it("builds page titles with the selected company name before Pilot", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • Pilot",
    );
    expect(
      buildDocumentTitle(
        [{ label: "Issues", href: "/issues" }, { label: "PIL-3515" }],
        "Anachronist Wiki",
      ),
    ).toBe("PIL-3515 • Issues • Anachronist Wiki • Pilot");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • Pilot");
    expect(buildDocumentTitle([], null)).toBe("Pilot");
  });
});
