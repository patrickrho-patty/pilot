...
describe("LiveUpdatesProvider issue invalidation", () => {
  ...
  it("refreshes issue document caches when a document activity event arrives", () => {
    ...
    __liveUpdatesTestUtils.invalidateActivityQueries(
      ...
      { pathname: "/PIL/issues/PIL-9403", isForegrounded: true },
    );
    ...
    expect(invalidations).toContainEqual({
      queryKey: queryKeys.issues.documents("PIL-9403"),
    });
    expect(invalidations).toContainEqual({
      queryKey: queryKeys.issues.document("PIL-9403", "plan"),
    });
    expect(invalidations).toContainEqual({
      queryKey: queryKeys.issues.documentRevisions("PIL-9403", "plan"),
    });
    expect(invalidations).toContainEqual({
      queryKey: ["issues", "document-annotations", "PIL-9403", "plan"],
    });
    expect(invalidations).not.toContainEqual({
      queryKey: queryKeys.issues.documents("issue-1"),
  ...
  it("keeps visible issue detail refetches inactive for downstream agent updates", () => {
    ...
    const queryClient = {
      ...
      getQueryData: (key: unknown) => {
        if (JSON.stringify(key) === JSON.stringify(queryKeys.issues.detail("PIL-759"))) {
          return {
            id: "issue-1",
            identifier: "PIL-759",
            assigneeAgentId: "agent-1",
    ...
    __liveUpdatesTestUtils.invalidateActivityQueries(
      ...
      { userId: null, agentId: null },
      { pathname: "/PIL/issues/PIL-759", isForegrounded: true },
    );
  ...
  it("refreshes visible issue run queries when the displayed run changes status", () => {
    const invalidations: unknown[] = [];
    const cache = new Map<string, unknown>([
      [JSON.stringify(queryKeys.issues.detail("PIL-759")), {
        id: "issue-1",
        identifier: "PIL-759",
        assigneeAgentId: "agent-1",
      ...
      [JSON.stringify(queryKeys.issues.activeRun("PIL-759")), {
        id: "run-1",
      }],
      [JSON.stringify(queryKeys.issues.activeRun("issue-1")), {
        id: "run-1",
      }],
      [JSON.stringify(queryKeys.issues.liveRuns("PIL-759")), [{ id: "run-1" }]],
      [JSON.stringify(queryKeys.issues.liveRuns("issue-1")), [{ id: "run-1" }]],
      [JSON.stringify(queryKeys.issues.runs("PIL-759")), [{ runId: "run-1" }]],
    ...
    expect(invalidations).toContainEqual({
      queryKey: queryKeys.issues.detail("PIL-759"),
    });
    expect(invalidations).toContainEqual({
      queryKey: queryKeys.issues.activity("PIL-759"),
    });
    expect(invalidations).toContainEqual({
      queryKey: queryKeys.issues.runs("PIL-759"),
    });
    expect(invalidations).toContainEqual({
      queryKey: queryKeys.issues.liveRuns("PIL-759"),
    });
    expect(invalidations).toContainEqual({
      queryKey: queryKeys.issues.activeRun("PIL-759"),
    });
    expect(invalidations).toContainEqual({
      queryKey: queryKeys.issues.activeRun("issue-1"),
    });
    expect(cache.get(JSON.stringify(queryKeys.issues.activeRun("PIL-759")))).toBeNull();
    expect(cache.get(JSON.stringify(queryKeys.issues.liveRuns("PIL-759")))).toEqual([]);
    expect(cache.get(JSON.stringify(queryKeys.issues.detail("PIL-759")))).toMatchObject({
      executionRunId: null,
  ...
  it("ignores run status events for other issues", () => {
    ...
    const queryClient = {
      ...
      getQueryData: (key: unknown) => {
        ...
        if (JSON.stringify(key) === JSON.stringify(queryKeys.issues.activeRun("PIL-759"))) {
          return {
            id: "run-1",
          };
        }
        if (JSON.stringify(key) === JSON.stringify(queryKeys.issues.liveRuns("PIL-759"))) {
          return [{ id: "run-1" }];
        }
        if (JSON.stringify(key) === JSON.stringify(queryKeys.issues.runs("PIL-759"))) {
          return [{ runId: "run-1" }];
        }
        return undefined;
      },
  ...
  it("hydrates the visible issue comments cache with only the new comment", async () => {
    ...
    const queryClient = {
      getQueryData: (key: unknown) => {
        ...
        if (JSON.stringify(key) === JSON.stringify(queryKeys.issues.comments("PIL-759"))) {
          return {
            pages: [[{
              id: "comment-1",
              companyId: "company-1",
    ...
    expect(getCommentMock).toHaveBeenCalledWith("PIL-759", "comment-2");
    expect(setCalls).toHaveLength(1);
    expect(setCalls[0]?.key).toEqual(queryKeys.issues.comments("PIL-759"));
