  ...
  return (
    <>
      ...
      <Route element={<AppsExperimentalGate />}>
        ...
        {/* Needs attention folded into Connections (PIL-13254); keep legacy links working. */}
      ...
      <Route
        ...
        element={<StatusCardsExperimentalGate><StatusCards /></StatusCardsExperimentalGate>}
      />
      {/* Back-compat: the board lived at /status-cards before PIL-15223. */}
      ...
      {/* `/audit` merged into the single Activity page (PIL-16302). Existing deep
          links keep working, preset to the agent-actions scope. */}
      <Route path="audit" element={<Navigate to="/activity?mode=agents" replace />} />
      {/* Conference Room Chat surfaces (PIL-136/PIL-137): routes stay
...
// (PIL-10862). `/tools` and `/tools/:tab` redirect to their new home.
...
export function OnboardingRoutePage() {
  ...
  // can tab/click through to the form behind the modal (PIL-52). The launcher
