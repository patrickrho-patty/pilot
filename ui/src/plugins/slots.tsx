 ...
 *
 * Plugin bundles are built with `external: ["@pilotai/plugin-sdk/ui", "react", "react-dom"]`,
...
function createReactShimSource(reactModule: object): string {
  ...
  return `
        const R = globalThis.__pilotPluginBridge__?.react;
        if (!R) {
          throw new Error("Pilot plugin React runtime is not initialized.");
        }
        export default R;
${namedExports}
      `;
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    ...
    case "react/jsx-runtime":
      ...
      source = `
        const BRIDGE = globalThis.__pilotPluginBridge__;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__pilotPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__pilotPluginBridge__?.sdkUi ?? {};
        function missing(name) {
          return function MissingPilotSdkUiComponent() {
            throw new Error('Pilot plugin UI runtime is not initialized for "' + name + '". Ensure the host loaded the plugin bridge before rendering this UI module.');
          };
        }
 ...
 * - `import { ... } from "@pilotai/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@pilotai/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@pilotai/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@pilotai/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@pilotai/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__pilotPluginBridge__) {
 ...
 * self-register via `window.pilotPlugins.registerReactComponent()`.
 ...
 * of `@pilotai/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
