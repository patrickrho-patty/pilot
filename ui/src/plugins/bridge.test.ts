...
afterEach(() => {
  delete globalThis.__pilotPluginBridge__;
});
...
describe("plugin host navigation", () => {
  it("resolves plugin page routes into the active company prefix", () => {
    expect(resolveHostNavigationHref("/wiki", "PIL")).toBe("/PIL/wiki");
    expect(resolveHostNavigationHref("/wiki?tab=browse#page", "pil")).toBe(
      "/PIL/wiki?tab=browse#page",
    );
  });

  it("does not double-prefix active company paths or global host paths", () => {
    expect(resolveHostNavigationHref("/PIL/wiki", "PIL")).toBe("/PIL/wiki");
    expect(resolveHostNavigationHref("/pil/wiki", "PIL")).toBe("/pil/wiki");
  });

  it("rewrites legacy instance settings paths into the active company settings scope", () => {
    expect(resolveHostNavigationHref("/instance/settings/plugins", "PIL")).toBe(
      "/PIL/company/settings/instance/plugins",
    );
    expect(resolveHostNavigationHref("/settings/experimental?x=1#auto", "pil")).toBe(
      "/PIL/company/settings/instance/experimental?x=1#auto",
    );
  });

  it("intercepts only same-origin plain left-click navigation", () => {
    expect(shouldHandleHostNavigationClick(clickEvent(), "/PIL/wiki")).toBe(true);
    expect(
      shouldHandleHostNavigationClick(clickEvent({ ctrlKey: true }), "/PIL/wiki"),
    ).toBe(false);
    expect(
      shouldHandleHostNavigationClick(clickEvent(), "/PIL/wiki", "_blank"),
    ).toBe(false);
    expect(
...
describe("plugin SDK FileTree bridge", () => {
  ...
  it("throws a clear error when the host FileTree implementation is missing", () => {
    globalThis.__pilotPluginBridge__ = {
      react: React,
    ...
    ).toThrow('Pilot plugin UI runtime is not initialized for "FileTree"');
  });
});
...
describe("plugin SDK markdown component bridge", () => {
  it("injects markdown display and editor components through the bridge runtime", () => {
    ...
    const registry = globalThis.__pilotPluginBridge__?.sdkUi ?? {};
...
describe("plugin jsx runtime bridge", () => {
  it("exposes the host jsx runtime on the bridge registry", () => {
    ...
    const runtime = globalThis.__pilotPluginBridge__?.reactJsxRuntime as typeof ReactJsxRuntime;
