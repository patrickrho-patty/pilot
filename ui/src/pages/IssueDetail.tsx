...
import { Activity as ActivityIcon, AlertTriangle, Archive, ArrowLeft, Check, ChevronRight, Copy, Eye, EyeOff, ScanEye, Flag, FileCode2, ListTree, MessageSquare, MoreHorizontal, MoreVertical, PauseCircle, FileText, PlayCircle, Plus, Repeat, SlidersHorizontal, XCircle } from "lucide-react";
...
const FEEDBACK_TERMS_URL = import.meta.env.VITE_FEEDBACK_TERMS_URL?.trim() || "https://pilot.test/tos";
...
type IssueDetailChatTabProps = {
   ...
   * chat-style thread (PIL-375). Ignored by the classic thread.
   */
...
function IssueDetailActivityTab({
  ...
  return (
    <>
      ...
      {/* Waiting-monitor state now lives in the pinned top banner (IssueMonitorBanner) — PIL-14557 decision 1. */}
    </>
  );
}
...
export function IssueDetail() {
  ...
  // Experimental Cases: linkify `PIL-C7` chips in this issue's comment bodies.
  ...
  // PIL-496: the chat shell keeps the full sub-task tree directly below the
    ...
    () =>
      taskChatShellEnabled && issue && showRichSubIssuesSection ? (
        <IssuesList
          ...
          viewStateKey={`pilot:issue-detail:${issue.id}:subissues-view`}
  ...
  // company Artifacts page (PIL-10359) can deep-link to a specific artifact in
  ...
  useEffect(() => {
    ...
    window.addEventListener("pilot:open-file-viewer", handleOpenFileViewer as EventListener);
    return () => {
      window.removeEventListener(
        "pilot:open-file-viewer",
  ...
  // via the S4 (PIL-1586) op. `forward` is the ancestry-proven safe path (server re-verifies);
        ...
        taskChatShellEnabled
          ? isMobile
            ? // Mobile shell scrolls the DOCUMENT (main is overflow-visible,
              // auto height) — the thread renders in normal flow (PIL-360).
        ...
        >
          {resolvedDetailTab === "chat" ? (
            <IssueDetailChatTab
              ...
              userLabelMap={userLabelMap}
              userProfileMap={userProfileMap}
              draftKey={`pilot:issue-comment-draft:${issue.id}`}
