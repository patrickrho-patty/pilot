...
import type { ExportFidelityReport } from "@pilotai/shared/portability-fidelity";
...
vi.mock("@/lib/router", () => ({
  useNavigate: () => vi.fn(),
  useLocation: () => ({ pathname: "/PIL/company/export", search: "" }),
}));
...
function buildExportPreviewResult() {
  return {
    ...
    manifest: {
      ...
      company: { name: "Pilot", description: null },
      schemaVersion: 1,
...
function buildFidelityReport(warnings: ExportFidelityReport["warnings"]): ExportFidelityReport {
  return {
    schema: "pilot-export-fidelity-v1",
    companyId: "company-1",
    counts: {
      labelDefinitions: 0,
...
describe("CompanyExport", () => {
  ...
  beforeEach(() => {
    ...
    mockCompaniesApi.exportBundle.mockResolvedValue({ rootPath: "pilot", files: {} });
  ...
  it("shows a retryable error instead of a false loading state", async () => {
    ...
    expect(container.textContent).toContain("Pilot export");
  });
