...
import { Download, LoaderCircle, Package, RotateCcw, Search, X } from "lucide-react";
import {
  type FileTreeNode,
  type FileTreeTone,
...
/**
 * Filter .pilot.yaml content so it only includes entries whose
 ...
 * content (regenerated README.md, filtered .pilot.yaml) when present.
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://pilot.test) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("npx pilotai company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [Pilot](https://pilot.test) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [Pilot](https://pilot.test) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .pilot.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .pilot.yaml
    const yamlPath = exportData.pilotExtensionPath;
