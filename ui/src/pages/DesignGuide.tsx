...
import { BookOpen, Bot, Check, ChevronDown, CircleDot, Command as CommandIcon, DollarSign, Hexagon, History, Inbox, LayoutDashboard, ListTodo, Mail, Plus, Search, Settings, Target, Trash2, Upload, User, Zap } from "lucide-react";
...
import type { CompanySecret, EnvBinding } from "@pilotai/shared";
import {
  EnvInputsList,
  ExternalSourcesList,
  RequiredSkillsList,
...
import type { IssueWorkProduct } from "@pilotai/shared";
...
export function DesignGuide() {
  ...
  const [filters, setFilters] = useState<FilterValue[]>([
    ...
    // PIL-411: priority filter demo row suppressed while SHOW_TASK_PRIORITY_UI is off.
    ...
    <div className="space-y-10 max-w-4xl">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Every component, style, and pattern used across Pilot.
        </p>
      </div>
      ...
      <Section title="Status System">
        ...
        <SubSection title="IssueStatusBadge (brand chip + glyph — PIL-75)">
          <div className="flex items-center gap-2 flex-wrap">
        ...
        {/* PIL-411: PriorityIcon showcase gated behind SHOW_TASK_PRIORITY_UI per board decision. */}
        {SHOW_TASK_PRIORITY_UI && (
        ...
        <SubSection title="IssueReferencePill">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <IssueReferencePill issue={{ id: "demo-1", identifier: "PIL-123", title: "Identifier only — no status yet" }} />
            <IssueReferencePill issue={{ id: "demo-2", identifier: "PIL-456", title: "With in_progress status", status: "in_progress" }} />
            <IssueReferencePill issue={{ id: "demo-3", identifier: "PIL-789", title: "Done status", status: "done" }} />
            <IssueReferencePill issue={{ id: "demo-4", identifier: "PIL-101", title: "Blocked status", status: "blocked" }} />
            <IssueReferencePill strikethrough issue={{ id: "demo-5", identifier: "PIL-202", title: "Removed (strikethrough)", status: "todo" }} />
          </div>
        </SubSection>
      </Section>
      ...
      <Section title="Breadcrumb">
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbLink href="#">Pilot App</BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator />
            <BreadcrumbItem>
          ...
          <EntityRow
            leading={
              <>
                ...
                {/* PIL-411: PriorityIcon hidden behind SHOW_TASK_PRIORITY_UI. */}
      ...
      <Section title="Log Viewer">
        <div className="bg-neutral-950 rounded-lg p-3 font-mono text-xs max-h-80 overflow-y-auto">
          ...
          <div className="text-foreground">[12:00:02] INFO  Processing task PIL-001</div>
          <div className="text-yellow-400">[12:00:05] WARN  Rate limit approaching (80%)</div>
          <div className="text-foreground">[12:00:08] INFO  Task PIL-001 completed</div>
      ...
      <Section title="Property Row Pattern">
        <div className="border border-border rounded-md p-4 space-y-1 max-w-sm">
          ...
          {/* PIL-411: priority metadata row hidden behind SHOW_TASK_PRIORITY_UI. */}
          {SHOW_TASK_PRIORITY_UI && (
            <div className="flex items-center justify-between py-1.5">
      ...
      <Section title="Grouped List (Issues pattern)">
        <div>
          ...
          <div className="border border-border rounded-b-md">
            {/* PIL-411: leading PriorityIcon hidden behind SHOW_TASK_PRIORITY_UI. */}
            <EntityRow
      ...
      <Section title="Team Catalog">
        ...
        <SubSection title="TeamRow (browse list)">
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            ...
            shows the amber <code className="font-mono">↑</code> badge (PIL-10256).
          </p>
        </SubSection>
      ...
      <Section title="Tools & Access">
        ...
        <SubSection title="EnforcementBanner — presentational tones (info / warning / error)">
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            Static governance copy with a tone. Used for the PIL-10400 trust-tier banner on Runtime and the
        ...
        <SubSection title="Action approval card — pending / stale (surfaces 11/12)">
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            Signed payload sha256 + expiry surface on every variant (PIL-10400). The{" "}
      ...
      <Section title="Inline Banner">
        ...
        <div className="space-y-3">
          ...
          >
            Ships with Pilot and powers <strong>Briefs</strong>. It can be paused but not deleted.
          </InlineBanner>
          <InlineBanner
            tone="warning"
