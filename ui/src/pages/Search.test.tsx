...
describe("buildSearchUrl", () => {
  ...
  it("preserves the existing pathname and hash", () => {
    expect(buildSearchUrl("http://x/PIL/search?q=x#anchor", "y", "issues")).toBe(
      "/PIL/search?q=y&scope=issues#anchor",
    );
  });
});
...
describe("Search page", () => {
  ...
  it("issues a search request when ?q is in the URL and renders the result", async () => {
      ...
      results: [
        {
          ...
          type: "issue",
          score: 100,
          title: "PIL-3142 Auth middleware flakes",
          href: "/PIL/issues/PIL-3142",
    ...
    await waitForAssertion(() => {
      expect(container.textContent).toContain("PIL-3142");
  ...
  it("renders artifact search results in the company search surface", async () => {
      ...
      results: [
        {
          ...
          href: "/PIL/issues/PIL-42#document-brief",
    ...
    await waitForAssertion(() => {
      ...
      expect(container.textContent).toContain("PIL-42");
  ...
  it("renders comment and document result rows with exact anchors, source chips, and highlights", async () => {
      ...
      results: [
        {
          ...
          type: "issue",
          score: 180,
          title: "PIL-77 Comment source",
          href: "/PIL/issues/PIL-77#comment-comment-77",
        ...
        },
        {
          ...
          type: "issue",
          score: 170,
          title: "PIL-78 Document source",
          href: "/PIL/issues/PIL-78#document-plan",
    ...
    await waitForAssertion(() => {
      expect(container.querySelector('a[href="/PIL/issues/PIL-77#comment-comment-77"]')).not.toBeNull();
      expect(container.querySelector('a[href="/PIL/issues/PIL-78#document-plan"]')).not.toBeNull();
  ...
  it("auto-redirects an exact identifier match to the issue root, dropping any deep-link suffix", async () => {
      ...
      results: [
        {
          ...
          type: "issue",
          score: 1300,
          title: "PIL-3366 Continuation summary",
          href: "/PIL/issues/PIL-3366#document-continuation-summary",
    ...
    const { root } = renderSearch("/search?q=PIL-3366", container);

    await waitForAssertion(() => {
      expect(navigateMock).toHaveBeenCalledWith("/PIL/issues/PIL-3366", { replace: true });
    });
