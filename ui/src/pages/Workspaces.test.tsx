...
import type { WorkspaceOverviewItem, WorkspaceOverviewResponse } from "@pilotai/shared";
...
function overviewItem(overrides: Partial<WorkspaceOverviewItem> = {}): WorkspaceOverviewItem {
  return {
    ...
    projectUrlKey: overrides.projectUrlKey ?? "pilot-app",
    projectName: overrides.projectName ?? "Pilot App",
    ...
    branchName: overrides.branchName ?? "PIL-11916-workspaces",
...
describe("Workspaces", () => {
  ...
  it("uses the bounded overview endpoint and renders grouped workspace cards with linked task summaries", async () => {
    ...
    expect(container.textContent).toContain("Pilot App");
    expect(container.textContent).toContain("Workspace Alpha");
    expect(container.textContent).toContain("PIL-11916");
    expect(container.textContent).toContain("+1 more");
    expect(container.textContent).toContain("Showing 1 of 2 workspaces.");
    expect(container.querySelector('a[href="/projects/pilot-app/workspaces"]')).not.toBeNull();
    ...
    expect(container.textContent).toContain("PIL-11917");
  });
