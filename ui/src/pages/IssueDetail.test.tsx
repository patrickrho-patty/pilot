...
import type { Agent, Issue, IssueAttachment, IssueComment, IssueTreeControlPreview, IssueTreeHold, IssueWorkProduct } from "@pilotai/shared";
import { ONBOARDING_FIRST_TASK_ORIGIN_KIND } from "@pilotai/shared";
...
vi.mock("@/lib/router", () => ({
  ...
  useParams: () => ({ issueId: "PIL-1" }),
}));

vi.mock("../context/CompanyContext", () => ({
  useCompany: () => ({
    companies: [{ id: "company-1", name: "Pilot", issuePrefix: "PIL", status: "active" }],
    selectedCompanyId: "company-1",
    selectedCompany: { id: "company-1", name: "Pilot", issuePrefix: "PIL", status: "active" },
...
function createCancelPreview(issueCount = 8): IssueTreeControlPreview {
  const issues = Array.from({ length: issueCount }, (_, index) => ({
    ...
    identifier: index === 0 ? "PIL-1" : `PIL-${index + 1}`,
...
describe("IssueDetail", () => {
  ...
  beforeEach(() => {
    ...
    mockLocation.pathname = "/issues/PIL-1";
    mockLocation.search = "";
    mockLocation.hash = "";
  ...
  it("does not loop openPanel when the sub-task list query is still loading (PIL-508)", async () => {
  ...
  it("updates status from the task header control and hides the priority control (PIL-411)", async () => {
    ...
    // PIL-411: priority UI is hidden behind SHOW_TASK_PRIORITY_UI (off), so the header
  ...
  it("removes an inbox-origin archived issue and restores it when the toast Undo action is pressed", async () => {
    const issue = createIssue({ id: "issue-1", identifier: "PIL-1", title: "Archive me from detail" });
    const otherIssue = createIssue({ id: "issue-2", identifier: "PIL-2", title: "Keep me in inbox" });
  ...
  it("does not optimistically queue a fresh comment from an unlocked stale active-run cache", async () => {
    ...
    mockIssuesApi.addComment.mockReturnValue(postedComment.promise);
    queryClient.setQueryData(queryKeys.issues.activeRun("PIL-1"), {
      id: "run-stale",
      status: "running",
      invocationSource: "issue",
      triggerDetail: null,
  ...
  it("refreshes subtree pause state after resuming a hold", async () => {
    ...
    expect(mockIssuesApi.releaseTreeHold).toHaveBeenCalledWith("PIL-1", "hold-1", {
      reason: null,
  ...
  it("uses simplified full-subtree pause controls", async () => {
    ...
    expect(mockIssuesApi.previewTreeControl).toHaveBeenCalledWith("PIL-1", {
      mode: "pause",
      releasePolicy: { strategy: "manual" },
    });
    ...
    expect(mockIssuesApi.createTreeHold).toHaveBeenCalledWith("PIL-1", {
      mode: "pause",
      reason: null,
      releasePolicy: { strategy: "manual", note: "full_pause" },
    });
  ...
  it("routes live-run finalization actions through run cancellation before issue status update", async () => {
    ...
    expect(mockIssuesApi.update).toHaveBeenCalledWith("PIL-1", { status: "done" });
    ...
    expect(mockIssuesApi.update).toHaveBeenLastCalledWith("PIL-1", { status: "cancelled" });
  ...
  it("passes @task mention options to the thread by default", async () => {
    ...
    await waitForAssertion(() => {
      expect(mockIssueChatThreadRender.mock.calls.at(-1)?.[0].mentions).toEqual(
        expect.arrayContaining([expect.objectContaining({ kind: "issue", issueIdentifier: "PIL-9" })]),
      );
    });
  ...
  it("forwards composer work mode changes to the issues API", async () => {
    ...
    localStorage.setItem("pilot:issue-comment-draft:issue-1", "Draft follow-up message");
    ...
    expect(localStorage.getItem("pilot:issue-comment-draft:issue-1")).toBe("Draft follow-up message");
    localStorage.removeItem("pilot:issue-comment-draft:issue-1");
  });
...
describe("shouldScrollIssueDetailToTopOnNavigation", () => {
  it("does not scroll when only URL search params changed for the same issue", () => {
    expect(shouldScrollIssueDetailToTopOnNavigation({
      previousIssueId: "PIL-10306",
      nextIssueId: "PIL-10306",
