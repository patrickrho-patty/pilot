...
vi.mock("@/lib/router", () => ({
  ...
  useCaseHref: () => (...segments: string[]) =>
    `/PIL/${["cases", ...segments].filter(Boolean).join("/")}`,
}));
...
function createCase(overrides: Partial<CaseSummary>): CaseSummary {
  return {
    ...
    projectId: null,
    caseNumber: 1,
    identifier: overrides.identifier ?? "PIL-C1",
...
describe("Cases list", () => {
  ...
  it("loads cases by default and hides terminal cases client-side", async () => {
    mockCasesApi.list.mockResolvedValue([
      createCase({ id: "a", identifier: "PIL-C1", title: "Active post", status: "in_progress" }),
      createCase({ id: "b", identifier: "PIL-C2", title: "Done post", status: "done" }),
    ]);
  ...
  it("shows default columns in id, title, status, updated order grouped by type without keys", async () => {
    mockCasesApi.list.mockResolvedValue([
      createCase({ id: "a", identifier: "PIL-C1", key: "launch/post-one", title: "Post one", caseType: "blog_post" }),
      createCase({ id: "b", identifier: "PIL-C2", title: "Storm one", caseType: "tweet_storm" }),
    ]);
  ...
  it("copies case list identifiers with feedback without following the row link", async () => {
    ...
    await waitForAssertion(() => {
      ...
      expect(mockCopyTextToClipboard).toHaveBeenCalledWith("PIL-C1");
  ...
  it("supports inbox-style keyboard navigation, group folding, and opening on grouped case rows", async () => {
    ...
    expect(mockNavigate).toHaveBeenCalledWith("/PIL/cases/PIL-C1");
  ...
  it("applies multi-select type and status filters from persisted state", async () => {
    ...
    mockCasesApi.list.mockResolvedValue([
      createCase({ id: "a", identifier: "PIL-C1", title: "Blog active", status: "in_progress", caseType: "blog_post" }),
      createCase({ id: "b", identifier: "PIL-C2", title: "Docs done", status: "done", caseType: "docs_page" }),
      createCase({ id: "c", identifier: "PIL-C3", title: "Tweet active", status: "in_progress", caseType: "tweet_storm" }),
      createCase({ id: "d", identifier: "PIL-C4", title: "Blog cancelled", status: "cancelled", caseType: "blog_post" }),
    ]);
