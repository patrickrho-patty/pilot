...
import { AlertTriangle, ArrowUpCircle, Boxes, Check, ChevronDown, ChevronLeft, ChevronRight, Code2, Download, Eye, Filter, FileCode2, FileText, Folder, FolderInput, FolderOpen, FolderSearch, GitFork, Github, Globe, HelpCircle, LayoutGrid, Link2, Lock, ExternalLink, FlaskConical, MoreHorizontal, Pause, Pencil, Pin, Plus, Copy, RefreshCw, Save, Search, Settings, ShieldCheck, Star, Trash2, Users, Hash, History, X, XOctagon } from "lucide-react";
import type { FolderListItem, FolderListResult } from "@pilotai/shared";
...
function sourceMeta(sourceBadge: CompanySkillSourceBadge, sourceLabel: string | null) {
  ...
  switch (sourceBadge) {
    ...
    case "pilot":
      return { icon: FileText, label: sourceLabel ?? "Pilot", managedLabel: "Pilot managed" };
    default:
...
function classifySource(skill: {
  ...
  if (skill.sourceBadge === "pilot") return "company";
...
function CompatChip({ compatibility }: { compatibility: CompanySkillCompatibility }) {
  ...
  const map = {
    unknown: {
      ...
      tooltip: "Pilot could not validate this skill as Agent Skills markdown. Install at your own risk.",
...
// ---------------------------------------------------------------------------
// Skills Store discovery grid (PIL-10879)
// ---------------------------------------------------------------------------
...
function buildDiscoveryCards(
  ...
  for (const entry of catalog) {
    ...
    cards.push({
      ...
      name: entry.name,
      slug: entry.slug,
      author: entry.packageName ?? "Pilot",
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        ...
        {/* Where the skill came from (PIL-10907 E); native title gives a hover hint. */}
        {(() => {
...
export function DiscoveryGrid({
  ...
  // narrows whatever the parent already filtered by tab/category/search (PIL-10907 E).
  ...
  return (
    ...
    // and the results pane each scroll independently (PIL-10907). Mobile keeps
...
export function SkillDetailPage({
  ...
  // unloaded with a dirty draft (PIL-10907 J).
  ...
  // unhelpful filesystem path (PIL-10907).
  ...
  function renderAgentsBody() {
    // Only the agents actually using this skill are listed (PIL-10907); the
  ...
  return (
    <div className="min-h-(--sz-calc-30)">
      <div className="border-b border-border px-4 py-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              <div className="min-w-0">
                <div className="flex min-w-0 items-center gap-2">
                  ...
                  {/* Source icon sits right after the title; the tooltip names
                      where the skill was installed from (PIL-10907). */}
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <span
          ...
          {/* GitHub-style social proof, top-right: installs · stars · fork.
              ...
              (PIL-10907); stars and fork are interactive. */}
      ...
      <div className="grid gap-6 px-4 py-4 xl:grid-cols-(--gtc-29)">
        <main className="min-w-0">
          <Tabs value={activeTab} onValueChange={(value) => onTabChange(value as SkillDetailTab)}>
            {/* Underlined tab strip: the bottom padding keeps the active-tab
                underline inside the horizontal-scroll clip box (PIL-10907). */}
        ...
        <aside className="min-w-0 space-y-6 border-t border-border pt-4 xl:border-l xl:border-t-0 xl:pl-5 xl:pt-0">
          ...
          <section>
            ...
            <div className="space-y-3">
              {/* Big primary action opens the agent multi-selector (PIL-10907). */}
              <AttachAgentsPopover
                agents={attachAgents}
          ...
          {/* Revision / update controls sit under Agents, above the config gear
              (PIL-10907 F). Only GitHub-sourced skills can pull updates. */}
      ...
      {/* Floating save bar: stays visible while a file edit is dirty so the
          unsaved state is obvious (PIL-10907 J). */}
      {isDirty ? (
...
export function CompanySkills() {
  ...
  // for real restrictions" behavior from §9.10 (PIL-13865).
  ...
  // param so stale `?view=catalog` deep links land on the new surface (PIL-10907).
  useEffect(() => {
  ...
  const studioDraft = useMemo(() => {
    ...
    // destination folder to that folder (PIL-14038).
  ...
  const createSkill = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        ...
        body: `${skill.name} is now editable in the Pilot workspace.`,
      });
    },
    onError: (error) => {
  ...
  // --- Discovery grid derived data (PIL-10879) ---
  ...
  // category / source filters the user arrived with (PIL-10907).
  ...
  // skill detail, matched by canonical key (PIL-10907).
  ...
  const studioDescription = studioForkFromId
    ...
    : "Create an editable company skill in the Pilot workspace.";
  return (
    <>
      {policyDenial.denial ? (
      ...
      {isStudioNew ? (
        ...
        <DiscoveryGrid
          ...
          folderNudgeStorageKey={showInstalledFolders ? `pilot:skills-folder-nudge:${selectedCompanyId ?? "none"}` : undefined}
        />
        ...
        // new store — no modal, no legacy split view (PIL-10907).
        <div className="min-h-(--sz-calc-30)">
