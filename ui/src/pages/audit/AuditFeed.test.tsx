...
function record(overrides: Record<string, unknown> = {}) {
  return {
    ...
    entity: {
      issue: { id: "issue-1", identifier: "PIL-1", title: "Ship the audit UI" },
...
describe("AuditFeed", () => {
  ...
  it("renders the humanized sentence, the task link, the excerpt, and the on-behalf chip", async () => {
    ...
    const taskLink = container.querySelector('a[href="/issues/PIL-1"]');
    expect(taskLink?.textContent).toContain("PIL-1");
    ...
    expect(container.textContent).toContain("Recorded by Pilot");
  });
  ...
  it("shows the permission-denied upsell when the feed 403s", async () => {
    ...
    expect(container.textContent).toContain("Pilot Enterprise view");
    ...
    expect(container.textContent).not.toContain("Recorded by Pilot");
  });
  ...
  it("clears privileged filters and recovers the basic feed after an access downgrade", async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(container.textContent).toContain("commented on");
      expect(container.textContent).not.toContain("Pilot Enterprise view");
      expect(container.textContent).not.toContain("All agents");
      expect(container.textContent).not.toContain("Export CSV");
    });
  });
  ...
  it("loads more when a cursor is returned", async () => {
    listAgentActionsMock.mockImplementation((_companyId: string, filters: { cursor?: string }) => {
      if (filters.cursor === "cursor-2") {
        return Promise.resolve({ items: [record({ id: "evt-2", entity: { issue: { id: "i2", identifier: "PIL-2", title: "Second" }, comment: null, document: null } })], nextCursor: null, accessTier: "full" });
      }
      return Promise.resolve({ items: [record()], nextCursor: "cursor-2", accessTier: "full" });
    });
    await render();

    expect(container.querySelector('a[href="/issues/PIL-2"]')).toBeFalsy();
    await clickButton("Load more");
    await flushReact();
    expect(container.querySelector('a[href="/issues/PIL-2"]')).toBeTruthy();
  });
