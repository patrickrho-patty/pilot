...
import type { Agent, Environment, EnvironmentCapabilities } from "@pilotai/shared";
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import { ToastProvider } from "../context/ToastContext";
import type { BuiltInAgentState } from "../api/builtInAgents";
import { Agents } from "./Agents";
import type { AgentOrgChainHealth } from "@pilotai/shared";
...
function makeBuiltInAgentState(overrides: Partial<BuiltInAgentState> = {}): BuiltInAgentState {
  return {
    definition: {
      ...
      defaultInstructions: "You are Pilot's built-in Briefs agent.",
...
describe("Agents", () => {
  ...
  it("shows the configured model beside the adapter on the all agents page", async () => {
    ...
    // "Apr 30, 2026" never wrap (PIL-85 defect #2).
  ...
  it("gives mobile agent names the full row width after the leading status indicator", async () => {
    ...
    mockAgentsApi.list.mockResolvedValue([
      makeAgent({
        id: "agent-mobile",
        name: "Pilot Engineer With A Much Longer Display Name",
        title: "Software Engineer With A Much Longer Specialty Title",
        urlKey: "pilot-engineer-long",
      }),
    ]);
    ...
    const row = findAgentRow(container, "Pilot Engineer With A Much Longer Display Name");
    ...
    const name = row?.querySelector("span[title='Pilot Engineer With A Much Longer Display Name']");
  ...
  it("uses configured names for local-driver environments", async () => {
    ...
    expect(container.textContent).toContain("Pilot host");
  });
  ...
  it("shows and filters built-in agents when the experimental flag is enabled", async () => {
    ...
    expect(container.textContent).toContain("Briefs Agent");
    expect(container.textContent).not.toContain("Regular Agent");
    expect(container.querySelector('[title="Ships with Pilot"]')).toBeNull();
    expect(mockRouterState.navigate).not.toHaveBeenCalledWith("/agents/all", { replace: true });
  });

  it("gives list-view rows a fixed-width title so meta columns align (PIL-86)", async () => {
    root = createRoot(container);
    await act(async () => {
      root!.render(
        <QueryClientProvider client={queryClient}>
