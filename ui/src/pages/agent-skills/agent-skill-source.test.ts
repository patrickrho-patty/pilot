...
describe("buildAgentSkillSourceMeta", () => {
  ...
  it("does not surface long filesystem labels for local skills", () => {
    expect(source({
      sourceBadge: "local",
      sourceLabel: "/Users/dev/work/pilot/skills/private-review",
      sourceLocator: null,
  ...
  it("keeps human-readable project scan labels for local skills", () => {
    expect(source({
      sourceBadge: "local",
      sourceLabel: "Pilot App / Engineering workspace",
      sourceLocator: null,
      sourceType: "local_path",
    })).toBe("Pilot App / Engineering workspace");
  });

  it("does not surface long filesystem labels for catalog skills", () => {
    expect(source({
      sourceBadge: "catalog",
      sourceLabel: "/srv/pilot/home/.pilot/instances/default/skills/company-id/__catalog__/briefs-discover-cards--68f7e3ad47",
      sourceLocator: null,
      sourceType: "catalog",
    })).toBe("Catalog");
  });
