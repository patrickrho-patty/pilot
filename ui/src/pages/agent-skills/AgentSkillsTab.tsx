...
import type { Agent, AgentDesiredSkillEntry } from "@pilotai/shared";
...
const MATERIALIZATION_NOTE =
  "Enabled skills are materialized into the stable Pilot-managed prompt bundle on the agent's next run.";

/** Company skill key of the Pilot core skill that carries beta releases. */
const PILOT_CORE_SKILL_KEY = "pilotai/pilot/pilot";
...
export function AgentSkillsTab({ agent, companyId }: { agent: Agent; companyId?: string }) {
  ...
  const unsupportedMessage = useMemo(() => {
    ...
    if (
      ...
      return "Pilot cannot manage skills for custom ACP commands yet.";
    }
    if (agent.adapterType === "openclaw_gateway") {
      return "Pilot cannot manage OpenClaw skills here. Visit your OpenClaw instance to manage this agent's skills.";
    }
    return "Pilot cannot manage skills for this adapter yet. Manage them in the adapter directly.";
