import { Boxes, Folder, Github, Link2, FileText, type LucideIcon } from "lucide-react";
import type { CompanySkillListItem } from "@pilotai/shared";
...
export function buildAgentSkillSourceMeta(skill: SourceSkill): AgentSkillSourceMeta {
  ...
  if (skill.sourceBadge === "pilot") {
    return { icon: FileText, label: skill.sourceLabel?.trim() || "Pilot managed" };
  }
