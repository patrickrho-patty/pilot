...
import type { Approval, HeartbeatRun, Issue } from "@pilotai/shared";
...
describe("Inbox toolbar", () => {
  ...
  it("keeps archive hover actions and swipe targets on every unread non-task Mine row", async () => {
    routerMock.location.pathname = "/inbox/mine";
    localStorage.setItem("pilot:inbox:group-by", "none");
  ...
  it("restores folded and unfolded sub-tasks across remounts", async () => {
    routerMock.location.pathname = "/inbox/mine";
    const storageKey = "pilot:inbox:collapsed-parents:company-1";
  ...
  it("paints row hover via CSS only, without moving React selection state", async () => {
    routerMock.location.pathname = "/inbox/mine";
    const issueA = createIssue({ id: "issue-a", identifier: "PIL-1001", title: "First inbox row" });
    const issueB = createIssue({ id: "issue-b", identifier: "PIL-1002", title: "Second inbox row" });
  ...
  it("keeps hover→j/k selection in sync after the list reshapes (PIL-9679)", async () => {
    routerMock.location.pathname = "/inbox/mine";
    generalSettingsMock.keyboardShortcutsEnabled = true;
    const issueA = createIssue({ id: "issue-a", identifier: "PIL-2001", title: "Sync row A" });
    const issueB = createIssue({ id: "issue-b", identifier: "PIL-2002", title: "Sync row B" });
    const issueC = createIssue({ id: "issue-c", identifier: "PIL-2003", title: "Sync row C" });
  ...
  it("holds the inbox order across a reordering poll, then re-sorts at an attention boundary (PIL-16015)", async () => {
    routerMock.location.pathname = "/inbox/mine";
