...
import type { ExecutionWorkspace, Issue, Project, ProjectWorkspace, RoutineListItem, WorkspaceOperation } from "@pilotai/shared";
...
function LegacyWorkspaceTabRedirect({ workspaceId }: { workspaceId: string }) {
  useEffect(() => {
    try {
      localStorage.removeItem(`pilot:execution-workspace-tab:${workspaceId}`);
    } catch {}
  }, [workspaceId]);
...
function ExecutionWorkspaceIssuesList({
  ...
  return (
    <IssuesList
      ...
      viewStateKey="pilot:execution-workspace-issues-view"
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async (_, { id }) => {
      ...
      pushToast({
        ...
        body: "Pilot created a run using this execution workspace.",
        tone: "success",
      });
    },
    onSettled: () => {
...
export function ExecutionWorkspaceDetail() {
  ...
  /**
   * Password-independent workspace entry (PIL-17572).
   *
          ...
          <div className="space-y-4 sm:space-y-6">
            <Card className="rounded-none">
              <CardHeader>
                ...
                <CardDescription>
                  Edit the concrete path, repo, branch, provisioning, teardown, and runtime overrides attached to this execution workspace. Saved changes affect future runs; Pilot may refresh or replace a reused workspace when config changes.
                </CardDescription>
                <CardAction>
                  <Button
                    variant="destructive"
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <Field label="Provision command" hint="Runs when Pilot prepares this execution workspace">
                    <Textarea
                ...
                <div className="space-y-4">
                  ...
                  {configuredRuntimeServicePorts.length > 0 ? (
                    <div className="space-y-3 rounded-md border border-border bg-muted/20 p-4">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Pilot checks fixed ports again when a service starts and rejects cross-workspace conflicts.
                      </p>
                    </div>
                  ) : null}
                </div>
