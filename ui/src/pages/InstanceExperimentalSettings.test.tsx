...
function setWorktreeRuntimeMeta(enabled: boolean) {
  const name = "pilot-worktree-enabled";
...
describe("InstanceExperimentalSettings — Conference Room Chat card (PIL-11233)", () => {
  ...
  it("no longer renders the Streamlined Left Navigation toggle (opt-out retired, PIL-12472)", async () => {
    await renderPage();
  ...
  it("renders and patches the Built-in Agents experimental toggle", async () => {
    ...
    expect(container.textContent).toContain("Show Pilot-managed built-in agent surfaces");
  ...
  it("renders and patches the Beta skills experimental toggle", async () => {
    ...
    expect(container.textContent).toContain("pin beta releases of the Pilot core skill");
...
describe("InstanceExperimentalSettings — cloud-managed keys", () => {
  const MANAGED_BADGE_TEXT = "Managed by Pilot Cloud";
  ...
  it("renders a managed key locked with the badge while unmanaged keys stay editable", async () => {
    await renderPage({
      ...
      enableApps: true,
      managedKeys: {
        enableApps: { managed: true, managedBy: "pilot-cloud" },
      },
    });
  ...
  it("locks Status Cards when managed Summaries is disabled", async () => {
    await renderPage({
      ...defaultExperimentalSettings(),
      managedKeys: {
        enableSummaries: { managed: true, managedBy: "pilot-cloud" },
      },
    });
  ...
  it("locks Summaries on when managed Status Cards is enabled", async () => {
    await renderPage({
      ...
      enableStatusCards: true,
      managedKeys: {
        enableStatusCards: { managed: true, managedBy: "pilot-cloud" },
      },
    });
  ...
  it("locks the managed auto-recovery toggle without opening the preview dialog", async () => {
    await renderPage({
      ...defaultExperimentalSettings(),
      managedKeys: {
        enableIssueGraphLivenessAutoRecovery: { managed: true, managedBy: "pilot-cloud" },
      },
    });
...
describe("InstanceExperimentalSettings — card ordering and headings (PIL-393)", () => {
  let container: HTMLDivElement;
  let root: Root | null = null;
