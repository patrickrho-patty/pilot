...
import type { Project } from "@pilotai/shared";
...
const mockCompanyContext = vi.hoisted(() => ({
  companies: [{ id: "company-1", issuePrefix: "PIL" }] as Array<{ id: string; issuePrefix: string }>,
...
describe("ProjectDetail", () => {
  ...
  beforeEach(() => {
    ...
    mockLocation.search = "";
    mockCompanyContext.companies = [{ id: "company-1", issuePrefix: "PIL" }];
  ...
  it("shows managed plugin affordances and filters the operations tab by plugin origin", async () => {
    ...
    expect(mockIssuesApi.list).toHaveBeenCalledWith("company-1", {
      projectId: "project-1",
      originKindPrefix: "plugin:pilot.missions",
    });
  });

  describe("plugin detail-tab deep links", () => {
    const PLUGIN_TAB = "plugin:pilotai.plugin-llm-wiki:project-knowledge";
    const knowledgeSlot = {
    ...
    it("renders the registered plugin tab for a direct deep link", async () => {
      ...
      expect(mockPluginSlotMount).toHaveBeenCalledWith(expect.objectContaining({
        slot: expect.objectContaining({ id: "project-knowledge", pluginKey: "pilotai.plugin-llm-wiki" }),
      }));
