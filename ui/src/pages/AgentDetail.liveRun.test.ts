...
import type { HeartbeatRun } from "@pilotai/shared";
...
describe("resolveLatestRunNavigation", () => {
  ...
  it("resolves the task from contextSnapshot.issueId and points the row at the task detail page", () => {
    const issue: LatestRunIssue = { id: "issue-7", title: "Fix the thing", status: "in_progress", identifier: "PIL-7" };
    ...
    expect(nav.rowHref).toBe("/issues/PIL-7");
  });

  it("falls back to contextSnapshot.taskId when issueId is absent (older snapshots)", () => {
    const issue: LatestRunIssue = { id: "issue-9", title: "Legacy", status: "todo", identifier: "PIL-9" };
    const run = makeRun({ contextSnapshot: { taskId: "issue-9" } as HeartbeatRun["contextSnapshot"] });

    const nav = resolveLatestRunNavigation(run, AGENT_ID, issuesMap(issue));

    expect(nav.task).toBe(issue);
    expect(nav.rowHref).toBe("/issues/PIL-9");
  });

  it("prefers issueId over taskId when both are present", () => {
    const wanted: LatestRunIssue = { id: "issue-a", title: "A", status: "in_progress", identifier: "PIL-A" };
    const other: LatestRunIssue = { id: "issue-b", title: "B", status: "in_progress", identifier: "PIL-B" };
    ...
    expect(nav.rowHref).toBe("/issues/PIL-A");
  });
  ...
  it("resolves an issue fetched directly (outside the bounded assigned-issues page) once folded into the lookup", () => {
    ...
    const fetched: LatestRunIssue = { id: "issue-far", title: "Far task", status: "in_progress", identifier: "PIL-999" };
    ...
    expect(nav.rowHref).toBe("/issues/PIL-999");
  });
});
