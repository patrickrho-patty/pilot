...
const mockParams = vi.hoisted(() => ({
  companyPrefix: "PIL" as string | undefined,
...
vi.mock("@/context/CompanyContext", () => ({
  useCompany: () => ({
    companies: [{ id: "company-1", name: "Pilot", issuePrefix: "PIL" }],
...
describe("PluginPage", () => {
  ...
  it("renders the breadcrumb and Back button on a legacy plugin route (no routeSidebar)", async () => {
    ...
    expect(container.querySelector('a[href="/PIL/dashboard"]')).not.toBeNull();
  ...
  it("uses a route title and hides the Back button when a routeSidebar matches the active route", async () => {
    ...
    expect(container.querySelector('a[href="/PIL/dashboard"]')).toBeNull();
