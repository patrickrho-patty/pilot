...
import type { ToolMcpGatewayToken, ToolMcpGatewayWithTokens } from "@pilotai/shared";
...
function token(overrides: Partial<ToolMcpGatewayToken> = {}): ToolMcpGatewayToken {
  return {
    ...
    gatewayId: "gateway-1",
    name: "cto-cursor",
    tokenPrefix: "pilotgw_live_8x4Pa",
...
describe("TokensPanel", () => {
  ...
  it("masks existing tokens and never renders the full secret at rest", async () => {
    ...
    expect(container.textContent).toContain("pilotgw_live_8x4Pa•••");
    expect(container.textContent).toContain("Active");
  });

  it("mints a token and reveals it once, then copies the full value", async () => {
    const created = { ...token({ id: "new-token" }), token: "pilotgw_live_FULLSECRETVALUE" };
    ...
    expect(container.textContent).toContain("pilotgw_live_FULLSECRETVALUE");

    await clickButton("Copy");
    expect(writeText).toHaveBeenCalledWith("pilotgw_live_FULLSECRETVALUE");
  });
