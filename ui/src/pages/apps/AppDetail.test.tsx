...
describe("AppDetail", () => {
      ...
      apps: [
        {
          ...
          authKind: "none",
          transportTemplate: { transport: "local_stdio", templateKey: "pilot.google-sheets" },
          credentialFields: [],
          recommendedDefaults: {},
          urlPatterns: ["https://docs.google.com/spreadsheets/*"],
          availability: { available: true, robotEmail: "robot@pilot.iam.gserviceaccount.com" },
        },
      ],
    });
  ...
  it("renders activity attribution with issue context and human resolver names", async () => {
    ...
    listConnectionActivityMock.mockResolvedValue({
      ...
      issues: {
        "issue-1": { identifier: "PIL-10912", title: "Fix app connection copy" },
      },
      actionRequests: {
        "request-1": {
          status: "approved",
    ...
    expect(container.textContent).toContain("while working on PIL-10912");
    expect(container.textContent).toContain("Dotta approved Mark done");
    expect(container.querySelector('a[href="/issues/PIL-10912"]')).toBeTruthy();
