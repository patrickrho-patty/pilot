...
import { CONNECTABLE_APP_DEFINITIONS } from "@pilotai/shared";
...
vi.mock("@/context/CompanyContext", () => ({
  useCompany: () => ({
    ...
    selectedCompany: { id: "company-1", name: "Pilot" },
  }),
}));
...
describe("AppsConnect — Connect with a link (M4 frame)", () => {
  ...
  it("shows an in-flight state while Pilot prepares Notion sign-in", async () => {
    mockSearch.value = "source=notion";
    listGalleryMock.mockResolvedValueOnce({ apps: [NOTION] });
  ...
  // PIL-10922: "Run your own" / "Paste a config" moved from the sidebar to rows
  ...
  // PIL-11091: discoverability copy for remote MCP URLs — the link field
  ...
  it("shows the Google Sheets robot email and keeps empty sheet links from continuing", async () => {
    listGalleryMock.mockResolvedValueOnce({
      apps: [
        { ...GOOGLE_SHEETS, availability: { available: true, robotEmail: "robot@pilot.iam.gserviceaccount.com" } },
      ],
    });
    await render();
    ...
    expect(container.textContent).toContain("robot@pilot.iam.gserviceaccount.com");
    expect(container.textContent).toContain(
  ...
  // PIL-11283: the gallery step exposes a Name field (default = app name) so a
