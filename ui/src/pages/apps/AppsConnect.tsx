...
import { ArrowUpRight, Check, ChevronRight, Copy, ClipboardPaste, Link2, Loader2, Lock, Search, TerminalSquare } from "lucide-react";
...
import { credentialConfigPath, getAppDefinitionForUrl, getAvailableConnectionMethod } from "@pilotai/shared";
...
export function AppsConnect() {
      ...
      setOAuthError(
        details?.code === "invalid_grant"
          ...
          : error instanceof Error
            ? error.message
            : "Pilot couldn’t start secure sign-in. Try again.",
      );
    },
  });
  ...
  useEffect(() => {
    ...
    if (directOAuth && (applicationsQuery.isError || connectionsQuery.isError)) {
      setOAuthPhase("error");
      setOAuthError("Pilot couldn’t check for an existing connection. Try again.");
      return;
    }
...
export function OAuthConnectStateScreen({
  ...
  const status = phase === "entry"
    ? {
        title: `Connect ${entry.name} to Pilot`,
        body: `Pilot will open ${entry.name} so you can choose a workspace and approve access.`,
      }
    : phase === "starting"
      ? {
          title: "Preparing secure sign-in",
          body: `Pilot is creating a secure ${entry.name} connection.`,
        }
      : phase === "redirecting"
        ? {
        ...
        : {
            ...
            body: error ?? "Pilot couldn’t start secure sign-in. Try again.",
          };
    ...
    <div className="max-w-5xl">
      ...
      <div className="mx-auto max-w-xl rounded-2xl border border-border bg-card p-8">
        ...
        <p className="mt-5 flex items-center gap-1.5 text-xs text-muted-foreground">
          ...
          Your authorization stays in Pilot’s encrypted secret store.
        </p>
      </div>
    </div>
  );
    ...
    <div className="mx-auto max-w-xl rounded-2xl border border-border bg-card p-8">
      ...
      <div className="mt-8">
        ...
        <p className="mt-2 text-xs text-muted-foreground">
          The token is part of the URL. Pilot stores it securely and checks the connection before enabling actions.
        </p>
...
function GalleryStep({
  ...
  loading: boolean;
  apps: AppDefinition[];
  /** Entered via the "Connect your own MCP server" card (PIL-12371, Finding C): focus the link path. */
  byo?: boolean;
  source?: string | null;
        ...
        {filtered.map((app) => {
          ...
          return (
            <button
              ...
              title={
                unavailable
                  ? `${app.name} isn't configured on this instance yet. Ask your Pilot admin.`
                  : undefined
              }
              onClick={() => onPick(app)}
              className={cn(
      ...
      <div className="border-t border-border pt-5">
        ...
        <div className="mt-3 flex flex-col gap-2">
          <ConnectMethodRow
            icon={TerminalSquare}
            title="Run your own"
            description="Register a command Pilot runs in your workspace for a tool that isn’t listed."
        ...
        <div className="mt-6 space-y-3">
          ...
          >
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Anyone you’ve added to Pilot can use {appName} in their tasks. This is what most teams want.
              </p>
            </div>
          </button>
