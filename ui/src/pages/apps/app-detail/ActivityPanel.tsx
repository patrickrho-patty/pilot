...
type TimelineRow = {
  ...
  primary: string;
  dotClass: string;
  /** Secondary "while working on PIL-…" issue link, tool-call rows only. */
 ...
 * the gateway tagged the audit event `metadata.source === "test"` (PIL-11349).
 */
...
export function humanizeEvent(
  ...
  // distinguishable from real heartbeat agent activity in the audit trail (PIL-11415).
  ...
  // humanize it to "Kv Set" to match the cross-app Activity view (PIL-11105).
...
/** Humanize a connection lifecycle event into a prosumer sentence (PIL-11284). */
