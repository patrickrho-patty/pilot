...
// Regression guard for PIL-16302: `/audit` was merged into the single rich
// Activity page. Both the company-prefixed `/:company/audit` and the bare
// `/audit` (PIL-16300's unprefixed redirect) must keep resolving — as redirects
  ...
  if (!sheetProto.__papActivityRoutingPatched) {
    ...
    sheetProto.insertRule = function patched(this: CSSStyleSheet, rule: string, index?: number) {
      try {
        ...
        try {
          return original.call(this, ".pil16302-noop{}", index);
        } catch {
...
let companyState = {
  companies: [PIL_COMPANY] as Array<typeof PIL_COMPANY>,
  selected: PIL_COMPANY as typeof PIL_COMPANY | null,
};
...
describe("App Activity routing (PIL-16302)", () => {
  let container: HTMLDivElement;

  beforeEach(() => {
    container = document.createElement("div");
    document.body.appendChild(container);
    companyState = { companies: [PIL_COMPANY], selected: PIL_COMPANY };
  });
  ...
  it("serves the merged Activity page at /:company/activity", async () => {
    const root = renderAppAt(container, "/PIL/activity");
    await waitForRoute(container, "ACTIVITY_PAGE@/PIL/activity");
  ...
  it("redirects /:company/audit to Activity with the agent-actions mode preset", async () => {
    const root = renderAppAt(container, "/PIL/audit");
    await waitForRoute(container, "ACTIVITY_PAGE@/PIL/activity?mode=agents");
    flushSync(() => root.unmount());
  });

  it("keeps the company from the URL when /:company/audit is not the selected company", async () => {
    // A shared /ACME/audit link opened by someone whose selected company is PIL
    ...
    companyState = { companies: [PIL_COMPANY, ACME_COMPANY], selected: PIL_COMPANY };
    const root = renderAppAt(container, "/ACME/audit");
    await waitForRoute(container, "ACTIVITY_PAGE@/ACME/activity?mode=agents");
    expect(container.textContent).not.toContain("ACTIVITY_PAGE@/PIL/activity");
    flushSync(() => root.unmount());
  });
