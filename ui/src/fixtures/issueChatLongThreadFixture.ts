...
function markdownBody(index: number) {
  return [
    `## Baseline note ${index}`,
    "",
    `This assistant update captures a deterministic markdown-heavy row for long-thread rendering. It references [PIL-${2600 + index}](/PIL/issues/PIL-${2600 + index}) and includes enough structure to exercise markdown parsing.`,
    "",
    "- Parsed checklist item one with inline `code`",
    "- Parsed checklist item two with **bold** and _italic_ text",
    "- Parsed checklist item three with a link to [Pilot](/PIL/dashboard)",
    "",
    "| Metric | Value |",
    "| --- | ---: |",
    `| Fixture row | ${index} |`,
