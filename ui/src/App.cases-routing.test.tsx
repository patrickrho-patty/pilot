...
// Regression guard for PIL-13002: the experimental Cases UI emits *unprefixed*
...
vi.hoisted(() => {
  const sheetProto = window.CSSStyleSheet.prototype as unknown as {
    ...
    __pil13002Patched?: boolean;
  };
  if (!sheetProto.__pil13002Patched) {
    ...
    sheetProto.insertRule = function patched(this: CSSStyleSheet, rule: string, index?: number) {
      try {
        ...
        try {
          return original.call(this, ".pil13002-noop{}", index);
        } catch {
          return this.cssRules?.length ?? 0;
        }
      }
    };
    sheetProto.__pil13002Patched = true;
  }
});
...
vi.mock("./context/CompanyContext", () => ({
  useCompany: () => ({
    companies: [PIL_COMPANY],
    selectedCompanyId: PIL_COMPANY.id,
    selectedCompany: PIL_COMPANY,
    loading: false,
  }),
...
describe("App Cases routing (PIL-13002)", () => {
  ...
  it("redirects unprefixed /cases/:id to the company-prefixed detail page", async () => {
    const root = await renderAppAt(container, "/cases/PIL-C5");
