...
describe("AgentsUsingSkillDialog", () => {
  ...
  it("removes via GET-then-sync and preserves the agent's other skills", async () => {
    mockAgentsApi.skills.mockResolvedValue({
      ...
      supported: true,
      mode: "managed",
      desiredSkills: ["pilot/demo", "pilot/other"],
      desiredSkillEntries: [
        { key: "pilot/demo", versionId: null },
        { key: "pilot/other", versionId: null },
    ...
    expect(sentEntries).toEqual([{ key: "pilot/other", versionId: null }]);
  });
  ...
  it("pins a version by sending the full set with the target repinned", async () => {
    ...
    expect(mockAgentsApi.syncSkills).toHaveBeenCalledTimes(1);
    const [, sentEntries] = mockAgentsApi.syncSkills.mock.calls[0];
    expect(sentEntries).toContainEqual({ key: "pilot/other", versionId: null });
    expect(sentEntries).toContainEqual({ key: "pilot/demo", versionId: "ver-2" });
  });
