...
describe("IssueRow", () => {
  ...
  it("preserves the issue detail breadcrumb source and href in the link target", () => {
    ...
    const issue = createIssue();
    const state = {
      issueDetailBreadcrumb: { label: "Inbox", href: "/PIL/inbox/mine" },
    ...
    const link = container.querySelector("[data-inbox-issue-link]") as HTMLAnchorElement | null;
    expect(link).not.toBeNull();
    expect(link?.getAttribute("to") ?? link?.getAttribute("href")).toBe("/issues/PIL-1");
  ...
  it("renders checklist step numbers beside the issue identifier", () => {
    ...
    act(() => {
      root.render(
        <IssueRow
          issue={createIssue({ identifier: "PIL-42" })}
    ...
    const metaRow = Array.from(container.querySelectorAll("span.flex.items-center.gap-2"))
      .find((element) => element.textContent?.includes("PIL-42"));

    expect(metaRow).not.toBeUndefined();
    expect(metaRow?.textContent?.replace(/\s+/g, "")).toContain("2.1.PIL-42");
