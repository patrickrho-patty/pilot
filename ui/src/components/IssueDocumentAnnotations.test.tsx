...
describe("IssueDocumentAnnotations", () => {
  ...
  it("renders the open count chip and opens the panel on click", async () => {
    ...
    // longer floats over the viewport with position: fixed (PIL-504).
  ...
  it("docks the desktop annotation panel into an in-flow gutter column beside the document", async () => {
    ...
    await waitFor(() => {
      ...
      const host = container.querySelector(".pilot-doc-annotation-host") as HTMLElement | null;
  ...
  it("renders the mobile annotation panel through the sheet path", async () => {
    ...
    try {
      ...
      expect(sheet?.className).toContain("pilot-doc-annotation-sheet");
