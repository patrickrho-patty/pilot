...
describe("ChatComposer", () => {
  ...
  it("default (multiline) mode soft-wraps and auto-grows instead of clipping (PIL-116)", () => {
  ...
  it('defaults to the opaque "card" surface', () => {
    // PIL-131: surface is opt-in — existing adopters keep the bg-card box.
    const root = createRoot(container);
    act(() => {
      root.render(<Harness />);
    });
  ...
  it('surface="translucent" applies the task glass recipe and keeps shared chrome (PIL-131)', () => {
    const root = createRoot(container);
    act(() => {
