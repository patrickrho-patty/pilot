...
import type { AgentPermissions } from "@pilotai/shared";
    ...
    createdRoot.render(
      <TooltipProvider>
        <TrustPresetSection
          ...
          onChange={onChange}
          companyId="company-1"
          projectCandidates={[{ id: "project-1", label: "Pilot App" }]}
          issueCandidates={[{ id: "issue-1", label: "PIL-1 · Review PR" }]}
        />
      </TooltipProvider>,
    );
  });
...
describe("TrustPresetSection", () => {
  it("hides the CE boundary editor under Standard", () => {
    ...
    expect(view.text()).not.toContain("Get Pilot EE.");
  });
  ...
  it("shows a selectable CE boundary editor for low-trust review", () => {
    ...
    expect(view.text()).toContain("Pilot App");
    expect(view.text()).toContain("Get Pilot EE.");
