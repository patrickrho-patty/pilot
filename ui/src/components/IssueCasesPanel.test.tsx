...
const links: IssueCaseLink[] = [
  {
    ...
    case: { id: "c1", identifier: "PIL-C7", title: "Launch post", caseType: "blog_post", status: "in_review" },
  },
];
...
describe("IssueCasesPanel", () => {
  ...
  it("renders linked cases with role + status when enabled", async () => {
    ...
    expect(text).toContain("PIL-C7");
    expect(text).toContain("Launch post");
    expect(text).toContain("work");
    expect(text).toContain("in review");
    expect(container.querySelector('a[href="/PIL/cases/PIL-C7"]')).not.toBeNull();
    act(() => root.unmount());
  });
