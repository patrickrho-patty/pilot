...
describe("CaseChildrenTree", () => {
  ...
  it("renders each child with identifier, type and status chips linking to detail without keys", () => {
    const root = render([
      child({ identifier: "PIL-C8", key: "launch/post", caseType: "blog_post", status: "in_review", title: "Post" }),
      child({ identifier: "PIL-C9", caseType: "image", status: "done", title: "Hero image" }),
    ]);
    const text = container.textContent ?? "";
    expect(text).toContain("PIL-C8");
    ...
    expect(container.querySelector('a[href="/PIL/cases/PIL-C8"]')).not.toBeNull();
    expect(container.querySelector('a[href="/PIL/cases/PIL-C9"]')).not.toBeNull();
    expect(container.querySelector('a[href="/PIL/cases/PIL-C8"]')?.className).not.toContain("border");
    act(() => root.unmount());
  });
  ...
  it("caps long child lists until show more is clicked", () => {
    ...
    const children = Array.from({ length: 7 }, (_, index) =>
      child({ id: `child-${index + 1}`, identifier: `PIL-C${index + 1}`, title: `Child ${index + 1}` })
    );
