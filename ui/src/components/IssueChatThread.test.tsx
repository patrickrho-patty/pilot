...
import type { Agent } from "@pilotai/shared";
import {
  IssueChatThread,
...
vi.mock("../hooks/usePilotIssueRuntime", () => ({
  usePilotIssueRuntime: () => ({}),
}));
...
describe("IssueChatThread", () => {
  ...
  it("uses accent-safe markdown color in the current user's blue message bubble", () => {
    ...
    expect(markdownBodyRenderMock).toHaveBeenCalledWith(expect.objectContaining({
      ...
      className: expect.stringContaining("pilot-markdown-on-accent"),
    }));
  ...
  it("scrolls loaded hash targets through the virtualized message index when initial hash scrolling is enabled", () => {
    ...
    act(() => {
      root.render(
        <MemoryRouter initialEntries={[`/issues/PIL-1#comment-${targetComment!.id}`]}>
          <IssueChatThread
            comments={issueChatLongThreadComments}
            linkedRuns={issueChatLongThreadLinkedRuns}
            timelineEvents={issueChatLongThreadEvents}
  ...
  // Regression for PIL-2660: on the real issue page the chat thread is wrapped
  ...
  // PIL-12003: initial page load must not jump to the latest comment. If a
  ...
  it("can keep the page at the top on initial load even when the URL has a comment hash", () => {
    ...
    act(() => {
      root.render(
        <MemoryRouter initialEntries={["/PIL/issues/PIL-12003#comment-comment-target"]}>
          <IssueChatThread
            comments={[{
              id: "comment-target",
              companyId: "company-1",
  ...
  // Regression for PIL-2672: when the merged feed ends with a non-comment row
  ...
  it("targets the latest comment row when trailing rows are non-comments (PIL-2672)", () => {
  ...
  // Regression for PIL-2672 follow-up: clicking Jump to latest must refresh
  ...
  it("findLatestCommentMessageIndex prefers the last comment-anchored row (PIL-2672)", () => {
    const messages = [
  ...
  it("renders a genuine agent comment in the conference-room neutral bubble (PIL-97 rev 7)", () => {
    const root = createRoot(container);
    act(() => {
      root.render(
        <MemoryRouter>
  ...
  it("clears a deleted comment deep-link hash instead of highlighting it", () => {
    ...
    flushAct(() => {
      root.render(
        <MemoryRouter initialEntries={["/issues/PIL-1#comment-comment-deleted"]}>
          <IssueChatThread
            comments={[{
              id: "comment-deleted",
              companyId: "company-1",
    ...
    expect(replaceStateSpy).toHaveBeenCalledWith(null, "", "/issues/PIL-1");
  ...
  it("shows unresolved blocker context above the composer", () => {
    ...
    expect(container.textContent).toContain("PIL-1723");
    expect(container.textContent).toContain("QA the install flow");
    expect(container.querySelector('[data-issue-path-id="PIL-1723"]')).not.toBeNull();
  ...
  it("renders the blue 'Waiting on live work' variant when the blocker chain is covered", () => {
        ...
        <MemoryRouter>
          <IssueChatThread
            ...
            blockerAttention={{
              ...
              sampleBlockerIdentifier: "PIL-2002",
    ...
    expect(container.querySelector('[data-issue-path-id="PIL-2001"]')).not.toBeNull();
    expect(container.querySelector('[data-issue-path-id="PIL-2002"]')).not.toBeNull();
    expect(container.querySelector('[data-issue-path-id="PIL-2003"]')).not.toBeNull();
  ...
  it("shows a 'Now running' row replacing 'Ultimately waiting on' when the terminal leaf is live", () => {
        ...
        <MemoryRouter>
          <IssueChatThread
            ...
            blockerAttention={{
              ...
              stalledBlockerCount: 0,
              attentionBlockerCount: 0,
              sampleBlockerIdentifier: "PIL-3001",
              sampleStalledBlockerIdentifier: null,
            }}
            blockedBy={[
              {
    ...
    expect(nowRunning?.textContent).toContain("PIL-3002");
    expect(container.textContent).not.toContain("Ultimately waiting on");
        ...
        <MemoryRouter>
          <IssueChatThread
            ...
            blockerAttention={{
              ...
              stalledBlockerCount: 0,
              attentionBlockerCount: 0,
              sampleBlockerIdentifier: "PIL-4001",
              sampleStalledBlockerIdentifier: null,
            }}
            blockedBy={[
              {
          ...
          <MemoryRouter>
            <IssueChatThread
              ...
              blockerAttention={{
                ...
                sampleBlockerIdentifier: "PIL-5001",
                sampleStalledBlockerIdentifier: state === "stalled" ? "PIL-5001" : null,
              }}
              blockedBy={[
                {
                  id: "blocker-1",
  ...
  it("renders the transcript directly from stable Pilot messages", () => {
    const root = createRoot(container);
