...
describe("IssueThreadInteractionCard", () => {
  ...
  // PIL-17287: a denial is persistent, so the inline error keeps the server's
  ...
  it("keeps the server denial reason in an aria-live region when a confirmation is refused", async () => {
    ...
    // PIL-17289: one live region, not two. `role="alert"` is itself an
  ...
  it("renders a plan confirmation as a distinct state-coloured plan card", () => {
    ...
    expect(resumeFailed.textContent).toContain("Pilot needs attention before the agent can resume this approved work.");
...
describe("IssueThreadInteractionCard tool-action card", () => {
  ...
  it("renders the addressee chip without the removed policy badge", () => {
    ...
    // PIL-440: the "Agents may resolve" policy badge was pure noise — never rendered.
 ...
 * has to guess whether an open card is waiting on them (PIL-17280).
 */
