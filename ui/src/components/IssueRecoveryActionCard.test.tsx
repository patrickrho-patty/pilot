...
import type { Agent, IssueRecoveryAction } from "@pilotai/shared";
...
describe("IssueRecoveryActionCard", () => {
  ...
  it("explains issue_graph_liveness in plain language", () => {
    ...
    expect(node.textContent).toContain(
      "Pilot could not find a clear next step for this open task. Choose whether to continue work, send it for review, mark it done, or record what is blocking it.",
    );
  });
  ...
  it("renders workspace_validation with its kind attribute and the recorded next action", () => {
    ...
    expect(node.textContent).toContain(
      "Pilot stopped this run because the task's git workspace could not be validated.",
    );
...
function buildWorkspaceValidationAction(
  ...
  const provenance = {
    ...
    plainLanguageReason:
      'The recorded branch "PIL-522-recorded" is not an ancestor of the checked-out branch "nleach/PIL-1405-live", so Pilot cannot prove a forward-only reconciliation.',
  ...
  return buildAction({
    ...
    evidence: {
      workspaceValidation: {
        ...
        expectedBranch: "PIL-522-recorded",
        actualBranch: "nleach/PIL-1405-live",
        cleanliness: "clean",
        provenance,
...
describe("IssueRecoveryActionCard workspace_validation divergence", () => {
  it("renders the divergence diagnosis with branches, shas, verdict and plain-language reason", () => {
    ...
    expect(text).toContain("PIL-522-recorded");
    expect(text).toContain("nleach/PIL-1405-live");
  ...
  it("offers the re-issue action and passes the live branch as the base ref", () => {
    ...
    expect(onReissueIsolated).toHaveBeenCalledWith({
      baseRef: "nleach/PIL-1405-live",
      liveBranch: "nleach/PIL-1405-live",
...
describe("IssueRecoveryActionCard W7 reconcile actions", () => {
  ...
  it("break-glass restates the divergence and gates the override behind a required reason", () => {
    ...
    expect(restatedText).toContain("PIL-522-recorded");
    expect(restatedText).toContain("nleach/PIL-1405-live");
...
describe("IssueRecoveryActionCard repair workspace (quarantine_restore)", () => {
  ...
  it("confirm popover restates the dirty count, live branch, rescue branch and recorded branch, then fires the handler", () => {
    ...
    ).toContain("pilot/rescue/PIL-1405/");
  ...
  it("in the contended case disables repair, explains the claimant, and recommends re-issue", () => {
        ...
        action={buildDirtyDivergenceAction({
          workspaceValidation: {
            contention: {
              ...
              claimedByIssueIdentifier: "PIL-9001",
              activeRun: { id: "run-9001", status: "running", issueId: "issue-99", issueIdentifier: "PIL-9001" },
            },
          },
        })}
        onQuarantineRestore={onQuarantineRestore}
    ...
    expect(notice?.textContent).toContain("PIL-9001");
    ...
    expect(disabled?.textContent).toContain(
      "Held by PIL-9001 — re-issue on an isolated workspace instead.",
    );
...
describe("IssueRecoveryActionCard owner-sticky retry lineage", () => {
  ...
  it("stays quiet and names the retry lane while the original owner is being retried", () => {
    ...
    expect(node.textContent).toContain("Pilot is retrying the original owner");
  });
  ...
  it("warns strongly only once the automatic path is exhausted", () => {
    ...
    expect(node.textContent).not.toContain("Pilot is retrying the original owner");
    expect(node.textContent).not.toContain("Times out");
  });

  it("warns strongly when the stored retry came due and never ran", () => {
    // PIL-17561: this exact shape rendered "Recovery in progress · Attempt 1 of 5 · Next try
