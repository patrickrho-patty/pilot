...
describe("NewIssueDialog", () => {
  ...
  it("shows sub-issue context only when opened from a sub-issue action", async () => {
    ...
    expect(container.textContent).toContain("PIL-1");
  ...
  it("applies project and execution workspace defaults for normal new issues", async () => {
    ...
    await waitForAssertion(() => {
      expect(container.textContent).toContain("Reusing PIL-100");
    });
  ...
  it("keeps the reusable workspace search popover inside the modal", async () => {
    ...
    mockExecutionWorkspacesApi.listSummaries.mockResolvedValue([
      {
        id: "workspace-1",
        name: "PIL-11446-on-mobile-the-agent-chat",
        mode: "isolated_workspace",
        status: "active",
        branchName: "PIL-11446-on-mobile-the-agent-chat",
        cwd: "/tmp/workspace-1",
  ...
  it("hides the priority chip and mobile priority option (PIL-411)", async () => {
    const { root } = renderDialog(container);
    await flush();

    // PIL-411: priority UI is hidden behind SHOW_TASK_PRIORITY_UI (off). Neither the
  ...
  it("still submits the default priority when the priority UI is hidden (PIL-411)", async () => {
    dialogState.newIssueDefaults = {
    ...
    // PIL-411: the priority control is hidden, but the data-model default must survive.
    expect(mockIssuesApi.create).toHaveBeenCalledWith(
      "company-1",
      expect.objectContaining({
        title: "Priority default persists",
  ...
  it("allows editor autocomplete portal pointer events inside the modal", async () => {
    ...
    menu.setAttribute("data-pilot-floating-ui", "");
  ...
  describe("PIL-8501: company badge shows issuePrefix", () => {
