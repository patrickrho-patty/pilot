...
import { usePilotIssueRuntime, type PilotIssueRuntimeReassignment } from "../hooks/usePilotIssueRuntime";
...
import { buildAgentMentionHref } from "@pilotai/shared";
...
import { AlertTriangle, ArrowRight, Brain, Check, ChevronDown, ClipboardList, Copy, Hammer, Loader2, MoreHorizontal, FileText, PauseCircle, Search, Square, ThumbsDown, ThumbsUp, Trash2 } from "lucide-react";
...
interface IssueChatMessageContext {
  ...
  /** Linkify `PIL-C7` case chips in comment bodies (experimental Cases flag). */
...
interface IssueChatThreadProps {
   ...
   * (PIL-375). Only the chat-style TaskChatThread consumes it; this thread
 ...
 * timestamp" instead of throwing mid-render (PIL-16607).
 */
...
const IssueChatTextPart = memo(function IssueChatTextPart({ text, recessed, onAccent }: { text: string; recessed?: boolean; onAccent?: boolean }) {
  ...
  return (
    <WorkspaceFileMarkdownBody
      className={cn("text-sm leading-6", onAccent && "pilot-markdown-on-accent")}
        ...
        className={cn(
          ...
          queued
            ...
            : isCurrentUser
              ...
              // in DECISION-SHEET.md A6) for the human's own messages (PIL-95 rev 5).
...
function IssueChatAssistantMessage({
  ...
  // keeps the existing flat / metadata treatment (PIL-95 rev 6).
  ...
  // blue bubble in IssueChatUserMessage). See PIL-95 rev 6.
  if (isGenuineComment) {
    return (
      <div id={anchorId}>
        <div className="group flex flex-col items-start py-1.5">
          {/* Icon + name together in a header ABOVE the bubble (PIL-95 rev 7). */}
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Choose whether voted AI outputs can be shared with Pilot Labs. This
...
function SystemNoticeCommentRow({
  ...
  const source = (() => {
    ...
    if (authorType === "system") {
      const label = runAgentName ?? "Pilot";
      if (runAgentId && runId) return { label, href: `/agents/${runAgentId}/runs/${runId}` };
      return { label };
    }
    if (runAgentId && runId) {
      return { label: authorName ?? runAgentName ?? "Pilot", href: `/agents/${runAgentId}/runs/${runId}` };
    }
      ...
      <div className="py-1">
        ...
        <div className="mt-1 flex items-center justify-end gap-1.5 px-1 opacity-0 transition-opacity group-hover:opacity-100">
          ...
          {anchorId ? (
            ...
            >
              {copiedLink ? <Check className="h-3.5 w-3.5" /> : <FileText className="h-3.5 w-3.5" />}
            </button>
          ) : null}
          <button
            type="button"
...
// segment; stacked rows read as one continuous rail. See PIL-95 mockup rev 5.
...
// chat area renders blank past the first viewport (PIL-2660).
...
const IssueChatComposer = forwardRef<IssueChatComposerHandle, IssueChatComposerProps>(function IssueChatComposer({
  ...
  // Interrupt-handoff clarity (PIL-10669): preview what this comment will durably
  ...
  return (
    ...
    >
        ...
        <div className="mr-auto flex items-center gap-2">
          ...
          {canToggleWorkMode ? (
            <Popover open={workModeMenuOpen} onOpenChange={setWorkModeMenuOpen}>
              <PopoverTrigger asChild>
                {/* Single persistent mode chip (PIL-95b mockup rev 5): yellow in
      ...
      {/* No-assignee warning modal (PIL-128 C): replaces the old press-Send-again toast. */}
...
export function IssueChatThread({
  ...
  // with tall markdown comments (PIL-2536 et al.), totalSize is hugely
  ...
  function handleJumpToLatest() {
    if (onRefreshLatestComments) {
      ...
      // *loaded* comment but not the absolute newest. (PIL-2672 follow-up.)
