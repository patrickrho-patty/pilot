...
export interface MentionOption {
  ...
  userId?: string;
  /** Issue/task references (PIL-95f). `name` carries the searchable identifier + title. */
  issueId?: string;
...
function mentionMarkdown(option: MentionOption): string {
  if (option.kind === "issue" && option.issueIdentifier) {
    // Insert a compact issue link (e.g. `[PIL-123](/issues/PIL-123)`). The chip
  ...
  if (richEditorError) {
    return (
      <div
        ref={containerRef}
        className={cn(
          "relative pilot-mdxeditor-scope",
    ...
    >
      <MarkdownEditorRichErrorBoundary onError={handleRichEditorError}>
        <MDXEditor
          ...
          className={cn("pilot-mdxeditor", !bordered && "pilot-mdxeditor--borderless")}
          contentEditableClassName={cn(
            "pilot-mdxeditor-content focus:outline-none [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5 [&_li]:list-item",
            contentClassName,
          )}
