...
const mockCompanyState = vi.hoisted(() => ({
  companies: [{ id: "company-1", issuePrefix: "PIL", name: "Pilot" }],
  selectedCompany: { id: "company-1", issuePrefix: "PIL", name: "Pilot" },
...
let currentPathname = "/PIL/dashboard";
...
vi.mock("@/lib/router", () => ({
  ...
  useParams: () => {
    ...
    return {
      companyPrefix: firstSegment ?? "PIL",
...
describe("Layout", () => {
  ...
  beforeEach(() => {
    ...
    currentPathname = "/PIL/dashboard";
    mockCompanyState.companies = [{ id: "company-1", issuePrefix: "PIL", name: "Pilot" }];
    mockCompanyState.selectedCompany = { id: "company-1", issuePrefix: "PIL", name: "Pilot" };
  ...
  it("collapses atomically when the pointer is still over the sidebar (no re-peek) — PIL-10676", async () => {
    const root = createRoot(container);
  ...
  it("keeps the app sidebar and shows the settings sidebar in the secondary pane on settings routes", async () => {
    currentPathname = "/PIL/company/settings/access";
    mockPluginSlots.slots = [
      {
        type: "page",
        id: "company-page",
    ...
    // Takeover model (PIL-10695): the app sidebar is kept (collapsed to its
  ...
  it("renders a mobile company settings selector on company settings routes", async () => {
    currentPathname = "/PIL/company/settings/secrets";
  ...
  it("renders the company settings sidebar on instance settings routes", async () => {
    currentPathname = "/PIL/company/settings/instance/general";
    const root = createRoot(container);
    const queryClient = new QueryClient({
      defaultOptions: { queries: { retry: false } },
    });
  ...
  it.each(["/PIL/company/export", "/PIL/company/import"])(
  ...
  it("keeps the app sidebar and shows the Apps sidebar in the secondary pane on legacy tools routes", async () => {
    currentPathname = "/PIL/tools/runtime";
    const root = createRoot(container);
    const queryClient = new QueryClient({
      defaultOptions: { queries: { retry: false } },
    });
  ...
  it("keeps the Apps sidebar on the M8 advanced-setup tabs", async () => {
    currentPathname = "/PIL/apps/advanced/run-your-own";
    const root = createRoot(container);
    const queryClient = new QueryClient({
      defaultOptions: { queries: { retry: false } },
    });
  ...
  it("keeps the Apps sidebar on legacy developer tabs behind the Advanced door", async () => {
    currentPathname = "/PIL/apps/advanced/runtime";
    const root = createRoot(container);
    const queryClient = new QueryClient({
      defaultOptions: { queries: { retry: false } },
    });
  ...
  it.each(["browse", "connections", "review"])("keeps the Apps sidebar on the %s surface", async (route) => {
    currentPathname = `/PIL/apps/${route}`;
    const root = createRoot(container);
    const queryClient = new QueryClient({
      defaultOptions: { queries: { retry: false } },
    });
  ...
  it("keeps the Apps sidebar on the gateways list and detail routes", async () => {
    for (const pathname of ["/PIL/apps/gateways", "/PIL/apps/gateways/gw-1/overview"]) {
      currentPathname = pathname;
      const root = createRoot(container);
      const queryClient = new QueryClient({
        defaultOptions: { queries: { retry: false } },
  ...
  it("uses the app connection sidebar on app detail routes", async () => {
    currentPathname = "/PIL/apps/conn-1/permissions";
    const root = createRoot(container);
    const queryClient = new QueryClient({
      defaultOptions: { queries: { retry: false } },
    });
  ...
  it("uses the app detail sidebar on not-connected app routes", async () => {
    currentPathname = "/PIL/apps/app/app-1/permissions";
    const root = createRoot(container);
    const queryClient = new QueryClient({
      defaultOptions: { queries: { retry: false } },
    });
  ...
  it("forces the app sidebar rail only for the Skills Store route", async () => {
    ...
    let root = await renderAt("/PIL/skills/studio");
    expect(mockSetForceCollapsed).toHaveBeenCalledWith(true);
    await act(async () => {
      root.unmount();
    });
    ...
    root = await renderAt("/PIL/agents/briefing-analyst/skills");
  ...
  it("renders a route-scoped plugin sidebar for a matching plugin page route", async () => {
    ...
    // Takeover model (PIL-10695): the app sidebar coexists with the plugin's
  ...
  it("keeps the route-scoped plugin sidebar on nested plugin page routes", async () => {
    currentPathname = "/PIL/wiki/page/templates";
    mockPluginSlots.slots = [
      {
        type: "page",
        id: "wiki-page",
  ...
  it("uses the route company context for plugin route sidebars on the first render", async () => {
    ...
    mockCompanyState.companies = [
      { id: "company-1", issuePrefix: "PIL", name: "Pilot" },
