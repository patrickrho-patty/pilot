...
describe("Sidebar", () => {
  ...
  it("streamlined is now standard: a stale enableStreamlinedLeftNavigation=false opt-out is ignored", async () => {
    // PIL-12472 retired the experimental opt-out; the streamlined sidebar is the
  ...
  it("shows the Conference Room nav item when conference room chat is enabled (PIL-137)", async () => {
    mockInstanceSettingsApi.getExperimental.mockResolvedValue({
      enableIsolatedWorkspaces: false,
  ...
  it("hides the Conference Room nav item when conference room chat is off (PIL-137)", async () => {
    mockInstanceSettingsApi.getExperimental.mockResolvedValue({
      enableIsolatedWorkspaces: false,
  ...
  it("does not flash the Conference Room item while experimental settings are loading (PIL-137)", async () => {
    mockInstanceSettingsApi.getExperimental.mockImplementation(() => new Promise(() => {}));
    const root = await renderSidebar();
  ...
  it("hides the expand/collapse toggle while a secondary sidebar locks the rail", async () => {
    ...
    // the primary while it is shown (PIL-10694).
    mockInstanceSettingsApi.getExperimental.mockResolvedValue({ enableIsolatedWorkspaces: false });
  ...
  it("keeps the collapsed rail top bar to just the company logo (no clipped toggle)", async () => {
    ...
    // would overflow/clip, shoving the logo out of the icon column (PIL-10676), so
