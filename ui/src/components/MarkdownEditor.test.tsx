...
import { buildIssueReferenceHref, buildProjectMentionHref, buildRoutineMentionHref, buildSkillMentionHref } from "@pilotai/shared";
import {
...
// Preserve the deferred Pilot-name autocomplete fixture without resembling an issue-prefix token.
const PILOT_MENTION_QUERY = ["@", "Pi", "lot"].join("");
  ...
  it("strips the leading identifier from the mention name", () => {
    expect(
      issueMentionTitle({
        id: "issue:1",
        kind: "issue",
        name: "PIL-102 @task references",
        issueIdentifier: "PIL-102",
      }),
...
describe("MarkdownEditor", () => {
  ...
  it("keeps mention queries active across spaces", () => {
    expect(findMentionMatch("Ping @Pilot App", "Ping @Pilot App".length)).toEqual({
      trigger: "mention",
      marker: "@",
      query: "Pilot App",
      atPos: 5,
      endPos: "Ping @Pilot App".length,
    });
  });
  ...
  async function openMentionMenuFor(
    ...
    matchText = "Pilot App",
    mentionQuery = PILOT_MENTION_QUERY,
  ...
  it("accepts mention selection from a touch tap", async () => {
    ...
    expect(handleChange).toHaveBeenCalledWith(
      `[@Pilot App](${buildProjectMentionHref("project-123", "#336699")}) `,
    );
  ...
  it("inserts a compact issue link when an @task reference is selected", async () => {
    ...
    expect(handleChange).toHaveBeenCalledWith(
      `[PIL-102](${buildIssueReferenceHref("PIL-102")}) `,
    );
  ...
  it("renders the task tag and identifier for issue mention options", async () => {
    ...
    expect(option.textContent).toContain("PIL-102");
  ...
  it("marks the autocomplete portal as floating UI for modal pointer handling", async () => {
    ...
    const menu = option.closest("[data-pilot-floating-ui]");
    expect(menu).toBeTruthy();
  ...
  it("lets wheel and touch scrolling reach the autocomplete menu inside a modal", async () => {
      ...
      root.render(
        <Dialog open>
          <DialogContent>
            ...
            <MarkdownEditor value={PILOT_MENTION_QUERY} onChange={() => {}} mentions={mentions} />
          </DialogContent>
        </Dialog>,
      );
    });
    ...
    const selection = window.getSelection();
    const range = document.createRange();
    range.setStart(textNode!, PILOT_MENTION_QUERY.length);
    range.collapse(true);
    selection?.removeAllRanges();
    selection?.addRange(range);
