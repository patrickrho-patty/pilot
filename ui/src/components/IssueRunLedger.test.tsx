...
import type { ActivityEvent, Issue, RunLivenessState } from "@pilotai/shared";
...
function createActiveRun(overrides: Partial<ActiveRunForIssue> = {}): ActiveRunForIssue {
  return {
    ...
    outputSilence: {
      ...
      evaluationIssueIdentifier: "PIL-404",
...
describe("IssueRunLedger", () => {
  ...
  it("surfaces active and completed child issue summaries", () => {
    renderLedger({
      childIssues: [
        createIssue({ id: "child-1", identifier: "PIL-2", title: "Implement worker handoff", status: "in_progress" }),
        createIssue({ id: "child-2", identifier: "PIL-3", title: "Verify final report", status: "done" }),
        createIssue({ id: "child-3", identifier: "PIL-4", title: "Cancelled experiment", status: "cancelled" }),
      ],
    });

    expect(container.textContent).toContain("Child work");
    expect(container.textContent).toContain("1 active, 1 done, 1 cancelled");
    expect(container.textContent).toContain("PIL-2");
  ...
  it("renders stale-run banner, watchdog actions, and silence badge for live runs", () => {
    ...
    expect(container.textContent).toContain("PIL-404");
