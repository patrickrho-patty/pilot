...
/** Recover the case identifier from a `/cases/PIL-C7` href produced by the plugin. */
...
interface MarkdownBodyProps {
  ...
  /**
   * Linkify bare case identifiers (`PIL-C7`) to the case detail page. Off by
   * default; enabled on surfaces behind the experimental Cases flag (PIL-12969).
   */
...
function MarkdownIssueLink({
  ...
  return (
    <Link
      ...
      className={cn("pilot-markdown-issue-ref", "font-normal underline")}
      title={title}
...
function MarkdownCaseLink({
  ...
  return (
    <Link
      ...
      className={cn("pilot-markdown-case-ref", "font-normal underline")}
...
function MarkdownExternalLink({
  ...
  return (
    <a
      ...
      className="pilot-markdown-external-ref"
    >
...
function createWikiLinkNode(href: string, wikiLink: ParsedWikiLink): MarkdownAstNode {
  return {
    ...
    data: {
      hProperties: {
        "data-pilot-wiki-link": "true",
        "data-pilot-wiki-target": wikiLink.target,
      },
    },
...
function CodeBlock({
  ...
  return (
    <div className="pilot-markdown-codeblock" data-wrap-lines={wrapLines || undefined}>
      <pre
        {...preProps}
        ref={preRef}
        style={{
      ...
      <div
        className="pilot-markdown-codeblock-actions"
      ...
      >
        <button
          ...
          title={wrapLabel}
          className="pilot-markdown-codeblock-action pilot-markdown-codeblock-wrap"
          style={wrapLines
            ? {
        ...
        <button
          ...
          title={copyLabel}
          className="pilot-markdown-codeblock-action pilot-markdown-codeblock-copy"
        ...
        >
          ...
          <span className="pilot-markdown-codeblock-action-label">{copyLabel}</span>
        </button>
      </div>
    </div>
  );
...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`pilot-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="pilot-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("pilot-mermaid-status", error && "pilot-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="pilot-mermaid-source">
...
function MarkdownBodyImpl({
  ...
  // parent re-renders frequently (see PIL-10767). Memoize both so re-renders
  ...
  const components = useMemo<Components>(() => {
    ...
    table: ({ node: _node, style: tableStyle, children: tableChildren, ...tableProps }) => (
      <div className="pilot-markdown-table-scroll" role="region" aria-label="Scrollable table" tabIndex={0}>
    ...
    a: ({ node: _node, href, style: linkStyle, children: linkChildren, ...anchorProps }) => {
      ...
      const isWikiLink = dataProps["data-pilot-wiki-link"] === "true";
      ...
      if (parsed) {
        ...
        return (
          <a
            ...
            className={cn(
              "pilot-mention-chip",
              `pilot-mention-chip--${parsed.kind}`,
              parsed.kind === "project" && "pilot-project-mention-chip",
            )}
  ...
  return (
    <div
      className={cn(
        "pilot-markdown prose prose-sm min-w-0 max-w-full break-words overflow-hidden",
