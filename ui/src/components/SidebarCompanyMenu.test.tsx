...
const mockLocation = vi.hoisted(() => ({ pathname: "/PIL/dashboard" }));
...
describe("SidebarCompanyMenu", () => {
  ...
  beforeEach(() => {
    ...
    mockLocation.pathname = "/PIL/dashboard";
  ...
  it("toggles company order editing without selecting a company", async () => {
    ...
    expect(document.body.textContent).not.toContain("PIL");
  ...
  it("navigates to the selected company dashboard from company-prefixed routes", async () => {
    mockLocation.pathname = "/PIL/issues";
    const root = createRoot(container);
    const queryClient = new QueryClient({
      defaultOptions: { queries: { retry: false } },
    });
