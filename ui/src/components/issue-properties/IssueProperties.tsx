...
export function IssueProperties({
  ...
  const approverTrigger = approverValues.length > 0
    ...
    : <span className="text-sm text-muted-foreground">None</span>;
  // PIL-16506 P4: who may give the `in_review` verdict. Only an agent sets this,
  ...
  const propertiesBody = (
    <div>
      <PropertySection title="Triage" first>
        ...
        {/* PIL-411: priority UI is hidden behind SHOW_TASK_PRIORITY_UI. Revive by flipping the flag. */}
      ...
      <PropertySection title="Relationships">
        ...
        {/* Chat shell promotes sub-tasks to their own pane tab (the full tree),
            so the slim pill row here would duplicate that home (PIL-496). Keep
      ...
      {/* Experimental Cases rail (PIL-12969) — self-gates on the flag and
