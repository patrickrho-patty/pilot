...
import type { AttentionItem, AttentionSourceKind } from "@pilotai/shared";
...
describe("AttentionQueueRow", () => {
  ...
  it("inlines a stalled review with the three review verbs (PIL-16080 §4.4)", () => {
    const el = render(
      <AttentionQueueRow
        item={buildItem({
  ...
  // PIL-16506 P4: an opt-in policy warns before the card offers Approve, because
      ...
      <AttentionQueueRow
        item={buildItem({
          ...
          subject: {
            ...
            id: "i1",
            companyId: "c1",
            title: "Update primary pilot instance",
            identifier: "PIL-23",
            status: "blocked",
            href: "/PIL/issues/PIL-23",
          },
          relatedIssue: null,
        })}
        companyId="c1"
  ...
  it("shows the task key when the subject is the task itself", () => {
    ...
    const link = Array.from(container?.querySelectorAll("a") ?? []).find((a) => a.textContent === "PIL-23");
    expect(link).toBeTruthy();
    expect(link?.getAttribute("href")).toBe("/PIL/issues/PIL-23");
  });
  ...
  it("shows no task key on a row that is not attached to a task", () => {
    ...
    expect(container?.textContent).not.toMatch(/PIL-\d+/);
  });
      ...
      <AttentionQueueRow
        item={buildItem({
          ...
          subject: {
            ...
            identifier: null,
            status: "pending",
            href: "/PIL/issues/issue-1#interaction-interaction-1",
  ...
  it("shows a larger gallery with an n-more link to the issue when expanded", () => {
    ...
    expect(moreLink?.getAttribute("href")).toBe("/PIL/issues/PIL-42");
  });
  ...
  // PIL-17287: the collapsed row must say who may resolve an interaction before
  ...
  // PIL-17289: the row's shell is `overflow-hidden`, so a clause that cannot
