  ...
  it("renders actor name and run→issue attribution", () => {
    const root = render([
      event({
        ...
        runId: "run-1",
        issue: { id: "i1", identifier: "PIL-42", title: "Source task", status: "in_progress" },
      }),
    ]);
    ...
    const issueLink = container.querySelector('a[href="/issues/PIL-42"]');
    expect(issueLink?.textContent).toContain("PIL-42");
  ...
  it("renders an auto-link event as a system actor with a linked issue", () => {
    const root = render([
      event({
        kind: "issue_linked",
        actorType: "system",
        issue: { id: "i2", identifier: "PIL-9", title: "Auto", status: "todo" },
      }),
    ]);
    const text = container.textContent ?? "";
    ...
    expect(container.querySelector('a[href="/issues/PIL-9"]')).not.toBeNull();
    act(() => root.unmount());
  });
