...
interface TaskChatBubbleProps {
  ...
  /**
   * copy · 👍 · 👎 controls for an agent bubble's footer line (PIL-413).
...
export function TaskChatBubble({ item, queuedAction, attachedTurn, actions }: TaskChatBubbleProps) {
  ...
  if (item.interstitial) {
    // Interstitial updates are ephemeral (PIL-361): while streaming the text
  ...
  if (item.author === "system") {
    // Collapsed humanized one-liner, expandable to the full detail (PIL-443).
    ...
    <div className={cn("tc-enter-bubble flex w-full flex-col gap-1", isHuman ? "items-end" : "items-start")}>
      ...
      {bodyText.length > 0 ? (
        <div
          ...
          // (PIL-501) can scope background/border overrides to the agent
      ...
      {item.optimistic ? (
        ...
        // copy/👍/👎 actions (PIL-413) ride the turn's summary row via its
        ...
        // with the always-visible timestamp trailing (PIL-413).
