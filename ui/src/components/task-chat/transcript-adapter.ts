...
/**
 * The live parent row's nesting rule (PIL-354, narrowed by PIL-361): only tool
...
/** One settled run's raw summary inputs, kept so back-to-back runs can coalesce (PIL-362). */
...
/**
 * Summary for a turn coalesced from several back-to-back runs (PIL-362).
...
/**
 * Assemble the thread body (PIL-367): backbone entries in order, each followed
...
/** Stable id of the description-as-first-bubble item (PIL-375). */
export const ISSUE_BRIEF_ITEM_ID = "issue-brief";

/**
 * Prepend the issue-brief placeholder (PIL-375) to the fully assembled thread.
...
/**
 * Final assembly pass (PIL-362): merge runs of settled turns from the SAME
 ...
 * row's line while it streams (PIL-361).
 */
