 ...
 *
 * The `.tc-line-scroll` slide plays ONLY at swap moments (PIL-368): on mount —
 ...
 * When `leaving` (held past the dwell with no successor, PIL-376), the
...
interface HeldSelfTalk {
  ...
  swapKey: number;
  /** Sliding out after sitting held for the full dwell (PIL-376). */
  leaving?: boolean;
}

/**
 * Held interstitial state (PIL-368, amended by PIL-376): a finished update
...
interface TaskChatStatusPillProps {
   ...
   * (PIL-376): its text vanishes on expand, so hovering it as part of the
...
export function TaskChatStatusPill({
  ...
  if (live) {
    ...
    // held after its stream ends (PIL-368) and slides away once it has sat
    // for the full --motion-interstitial-dwell without a successor (PIL-376);
