...
export interface TaskChatMessageItem {
   ...
   * calls). Ephemeral in the redesigned view (PIL-361): while streaming it
  ...
  /**
   * Structured system-notice fields (PIL-443), carried only for
...
export interface TaskChatStatusItem {
   ...
   * (PIL-361, round 9): it renders in a dedicated single-line row directly
   ...
   * ends (PIL-368) — nothing persists in the settled thread.
   */
  selfTalk?: string;
  elapsedMs?: number;
 ...
 * (PIL-375). A placeholder kind only — the host supplies the render
...
export interface TaskChatTurnItem {
   ...
   * row while collapsed (PIL-354 parent-row model). Absent once settled.
   */
