...
interface TaskChatThreadViewProps {
  ...
  /**
   * Renders the description-as-first-bubble placeholder (PIL-375). The live
   ...
   * line (PIL-413). The live thread binds it to the feedback-vote API; harness
...
function renderItem(
  ...
  switch (item.kind) {
    case "message": {
      ...
      // put when the tool history expands (PIL-413). The two paths are mutually
    ...
    case "thinking":
      // Thinking never renders as a row (PIL-361): its live signal is the
