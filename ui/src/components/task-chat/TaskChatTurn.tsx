...
interface TaskChatTurnProps {
  ...
  /**
   * Content rendered on the header row, leading the summary line (PIL-413: the
 ...
 * by default, owning all activity (PIL-354). Expanding nests the chronological
  ...
  const header = item.settled ? (
    ...
    >
      ...
      {turnSummaryMetrics(item.summary) ? (
        // Time/tools/tokens is demoted, not deleted (PIL-502): it stays in the
    ...
    // hover/click target (PIL-376). Without activity there is no
    // chevron/button.
    <TaskChatStatusPill
  ...
  return (
    <div data-testid="task-chat-turn" data-settled={item.settled ? "true" : "false"}>
      {leading ? (
        ...
        // tool history never moves the actions off the summary line (PIL-413).
