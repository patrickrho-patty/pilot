...
function authorKind(comment: IssueChatComment): TaskChatAuthorKind {
  // System authorship wins over any derivable run→agent linkage (PIL-443):
...
/** Shared bubble-footer time format ("2:34 PM") — also used by the description bubble (PIL-375). */
...
export function commentsToTaskChatItems(
  ...
  for (const comment of comments) {
    ...
    items.push({
      ...
      // layer (PIL-443); other authors keep the item lean.
