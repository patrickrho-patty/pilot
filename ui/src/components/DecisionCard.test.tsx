...
const ISSUES: Record<string, DecisionIssueRef> = {
  "issue-origin": { id: "issue-origin", identifier: "PIL-123", title: "Gardener sweep", href: "/PIL/issues/PIL-123", status: "in_progress" },
  "issue-target": { id: "issue-target", identifier: "PIL-456", title: "Stale epic", href: "/PIL/issues/PIL-456", status: "backlog" },
  "issue-parent": { id: "issue-parent", identifier: "PIL-789", title: "Parent task", href: "/PIL/issues/PIL-789", status: "todo" },
  "issue-blocker": { id: "issue-blocker", identifier: "PIL-790", title: "Blocking task", href: "/PIL/issues/PIL-790", status: "todo" },
  "issue-new": { id: "issue-new", identifier: "PIL-999", title: "Follow-up", href: "/PIL/issues/PIL-999", status: "todo" },
};
...
describe("DecisionCard", () => {
  it("renders a pending decision with provenance, effect summary and dismiss", () => {
    ...
    expect(el.textContent).toContain("PIL-123");
    expect(el.textContent).toContain("Comment on PIL-456");
  ...
  it("links the target issue the decision applies to, not just the origin", () => {
    ...
    expect(provenance?.textContent).toContain("PIL-456");
    expect(
      [...(provenance?.querySelectorAll("a") ?? [])].some((a) => a.getAttribute("href") === "/PIL/issues/PIL-456"),
    ).toBe(true);
  });
  ...
  it("renders decided result rows with entity links", () => {
    ...
    expect(el.textContent).toContain("Created PIL-999");
    expect(el.textContent).toContain("Set PIL-456 to done");
    expect([...el.querySelectorAll("a")].some((a) => a.getAttribute("href") === "/PIL/issues/PIL-999")).toBe(true);
