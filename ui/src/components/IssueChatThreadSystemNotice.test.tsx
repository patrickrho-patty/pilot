...
import type { Agent, SuccessfulRunHandoffState } from "@pilotai/shared";
...
describe("IssueChatThread system notice routing", () => {
  it("renders authorType=system comments as a SystemNotice rather than a user bubble", () => {
        ...
        sections: [
          {
            ...
            rows: [
              { type: "issue_link", label: "Source issue", issueId: "i1", identifier: "PIL-3440", title: "Recovery" },
    ...
    expect(container.textContent).toContain("Pilot needs a disposition");
    // collapsed by default — metadata identifier should not be visible
    expect(container.textContent).not.toContain("PIL-3440");
  ...
  it("renders a system notice with a malformed createdAt without tripping the error boundary (PIL-16607)", () => {
    const comment: IssueChatComment = {
  ...
  it("labels system notice source as Pilot when no run agent can be resolved", () => {
    const comment: IssueChatComment = {
    ...
    const status = container.querySelector('[role="status"]');
    expect(status).not.toBeNull();
    expect(status?.textContent).toContain("Pilot");
    expect(status?.textContent).not.toContain("You");
  });

  it("falls back to Pilot in the system notice header when run agent is unknown to agentMap", () => {
    const comment: IssueChatComment = {
    ...
    expect(sourceLink?.textContent).toBe("Pilot");
  });
  ...
  it("folds stale successful-run disposition warnings into the activity log disclosure style", () => {
    ...
    expect(row?.textContent).not.toContain("Pilot needs a disposition before this issue can continue.");
  ...
  it("keeps the required disposition warning loud when no live continuation exists", () => {
    ...
    expect(container.textContent).toContain("Pilot needs a disposition before this issue can continue.");
  });
