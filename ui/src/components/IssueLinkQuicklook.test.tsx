...
describe("IssueLinkQuicklook", () => {
        ...
        <QueryClientProvider client={queryClient}>
          <MemoryRouter>
            <IssueLinkQuicklook
              issuePathId="PIL-1"
              issuePrefetch={issue}
              to="/companies/company-1/issues/PIL-1"
            >
              PIL-1
            </IssueLinkQuicklook>
          </MemoryRouter>
  ...
  it("states identity, then title, then summary", () => {
    const card = openCard(
      renderQuicklook({
        ...
        project: { id: "project-1", name: "Pilot App" } as unknown as Issue["project"],
      }),
    );
    const text = card?.textContent ?? "";

    expect(text).toContain("PIL-1");
    expect(text).toContain("Pilot App");
    expect(text).toContain("Quicklook title");
    expect(text).toContain("Quicklook description");
    expect(text.indexOf("PIL-1")).toBeLessThan(text.indexOf("Quicklook title"));
  ...
  it("truncates a long project name but never the task key or the timestamp", () => {
    ...
    const key = Array.from(card?.querySelectorAll("span") ?? []).find((s) => s.textContent === "PIL-1");
