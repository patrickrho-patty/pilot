...
import type { ExecutionWorkspace, Issue } from "@pilotai/shared";
...
function createIssue(overrides: Partial<Issue> = {}): Issue {
  return {
    ...
    identifier: overrides.identifier ?? "PIL-1",
...
function createSummary(overrides: Partial<ProjectWorkspaceSummary> = {}): ProjectWorkspaceSummary {
  const issues = overrides.issues ?? [
    createIssue({ id: "issue-1", identifier: "PIL-1364" }),
    createIssue({ id: "issue-2", identifier: "PIL-1367" }),
    createIssue({ id: "issue-3", identifier: "PIL-1362" }),
    createIssue({ id: "issue-4", identifier: "PIL-1363" }),
    createIssue({ id: "issue-5", identifier: "PIL-1340" }),
  ];
  return {
    key: overrides.key ?? "execution:workspace-1",
    kind: overrides.kind ?? "execution_workspace",
    workspaceId: overrides.workspaceId ?? "workspace-1",
    workspaceName: overrides.workspaceName ?? "PIL-989-multi-user-implementation",
    cwd: overrides.cwd ?? "/worktrees/PIL-989-multi-user-implementation",
    branchName: overrides.branchName ?? "PIL-989-multi-user-implementation",
...
describe("ProjectWorkspaceSummaryCard", () => {
  ...
  it("uses project workspace routes and omits close controls for project workspaces", () => {
      ...
      root.render(
        <ProjectWorkspaceSummaryCard
          ...
          summary={createSummary({
            ...
            issues: [createIssue({ id: "issue-6", identifier: "PIL-1400" })],
          })}
          runtimeActionKey={null}
          runtimeActionPending={false}
    ...
    const titleLink = container.querySelector("a[href='/projects/pilot-app/workspaces/workspace-1']");
  ...
  it("copies branch and path from both text and icon controls with feedback", async () => {
    ...
    const summary = createSummary({
      branchName: "PIL-1552-workspace-polish",
      cwd: "/Users/dotta/pilot/.worktrees/PIL-1552-workspace-polish",
    });
