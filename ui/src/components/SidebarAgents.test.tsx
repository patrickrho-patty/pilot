...
import type { Agent, ResourceMemberships } from "@pilotai/shared";
...
vi.mock("@/lib/router", () => ({
  ...
  useLocation: () => ({ pathname: "/PIL/dashboard", search: "", hash: "", state: null }),
}));
...
describe("SidebarAgents", () => {
  ...
  it("renders icon-only agent rows with tooltips and no row actions in the rail", async () => {
    ...
    // clipped) so the row stays 1:1 tall with the expanded state (PIL-10676); the
  ...
  it("keeps top mode in stored org-aware order", async () => {
    localStorage.setItem("pilot.agentOrder:company-1:user-1", JSON.stringify(["agent-b", "agent-a", "agent-c"]));
    mockAgentsApi.list.mockResolvedValue([
      makeAgent({ id: "agent-a", name: "Alpha", urlKey: "alpha" }),
      makeAgent({ id: "agent-b", name: "Bravo", urlKey: "bravo" }),
  ...
  it("sorts alphabetically and persists the selected mode per company and user", async () => {
    ...
    expect(localStorage.getItem("pilot.agentSortMode:company-1:user-1")).toBe("alphabetical");
  });
  ...
  it("shows only active agents when any agent has a live run", async () => {
    ...
    // PIL-76: the full-list entry point stays visible even when only active
    // agents are shown.
