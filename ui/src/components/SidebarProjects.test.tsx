...
import type { Project, ResourceMemberships } from "@pilotai/shared";
...
vi.mock("@/lib/router", () => ({
  ...
  useLocation: () => ({ pathname: "/PIL/projects/bravo/issues", search: "", hash: "", state: null }),
}));

vi.mock("../context/CompanyContext", () => ({
  useCompany: () => ({
    selectedCompanyId: "company-1",
    selectedCompany: { id: "company-1", issuePrefix: "PIL" },
  }),
}));
...
describe("SidebarProjects", () => {
  ...
  it("renders icon-only project rows with tooltips and no row actions in the rail", async () => {
    ...
    // so rows stay 1:1 tall with the expanded state (PIL-10676); rows become
  ...
  it("sorts alphabetically and persists the selected mode per company and user", async () => {
    ...
    expect(localStorage.getItem("pilot.projectSortMode:company-1:user-1")).toBe("alphabetical");
  });
