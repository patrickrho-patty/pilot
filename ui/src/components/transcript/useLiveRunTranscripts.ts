...
import type { LiveEvent } from "@pilotai/shared";
...
// and silently drops already-rendered scrollback (PIL-462 B3). A run that is
...
export function useLiveRunTranscripts({
  ...
  // PIL-462 B3: buffered runs that dropped out of the `runs` list, mapped to the
  ...
  useEffect(() => {
    ...
    // PIL-462 B3: a run that just disappeared from the list starts its grace
