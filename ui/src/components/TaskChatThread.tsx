...
import type { FeedbackVoteValue } from "@pilotai/shared";
...
// PIL-462 B4: backstop for how long the just-settled run's transcript stays
 ...
 *     transcriptToTaskChatItems converter the settled turns use (PIL-463).
 *
...
export function TaskChatThread(props: TaskChatThreadProps) {
  ...
  // backbone chronologically at its start time (PIL-367).
  ...
  const orderedEntries = useMemo(() => {
    ...
    for (const interaction of interactions ?? []) {
      ...
      // newer question (PIL-437) — is filtered here so it leaves no empty slot
      // or gap in the ordered backbone (PIL-424, plan from PIL-420).
      ...
      entries.push({
        ...
        // above a message written before the user answered (PIL-416); pending
  ...
  const { items, settledRunIds } = useMemo<{ items: TaskChatItem[]; settledRunIds: Set<string> }>(() => {
    ...
    // hands off to this as its "the settled turn has rendered" signal (PIL-462
    ...
    // and thinking are excluded entirely (PIL-361): the final reply already
    ...
    // coalesce into one "Worked" row in the final pass (PIL-362).
    ...
    for (const source of runs) {
      ...
      settledTurns.push({
        ...
        // (PIL-367): the run's start time. A run with no linked meta yet
    ...
    // PIL-362: two runs replying back-to-back (same agent, nothing but the
    ...
    // PIL-375: the description-as-first-bubble placeholder prepends LAST, after
  ...
  // PIL-462 B4: the moment a run settles, `liveRun` flips to null — but its
  ...
  // now-static transcript until its settled form renders (PIL-462 B4).
  ...
  // Status-pill inputs for the tail (PIL-461, A1): the run's start, its finish
  ...
  // The tail's clean rows (PIL-463 C1): the streaming transcript parsed through
  ...
  // 👍/👎 state to each agent bubble by its comment id (PIL-413).
  ...
  // copy · 👍 · 👎 cluster for an agent bubble's footer line (PIL-413). Human
  ...
  // Mobile (PIL-360): the app shell scrolls the DOCUMENT (Layout's main is
      ...
      {showComposer ? (
        <div
          ...
          className={cn(
            ...
            // when the nav auto-hides (PIL-495). transition-[bottom] rides the
