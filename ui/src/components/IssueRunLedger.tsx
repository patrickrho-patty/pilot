...
import type { ActivityEvent, Issue, Agent } from "@pilotai/shared";
import { isResponsibleUserDenialCode, responsibleUserLabel } from "@pilotai/shared";
...
const RETRY_PENDING_LIVENESS_COPY: LivenessCopy = {
  ...
  description: "Pilot queued an automatic retry that has not started yet.",
};
...
function watchdogDecisionErrorMessage(error: unknown) {
  ...
  return error instanceof Error && error.message.trim().length > 0
    ? error.message
    : "Pilot could not record the watchdog decision.";
}
