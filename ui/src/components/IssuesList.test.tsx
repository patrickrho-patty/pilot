...
import type { Issue, Project } from "@pilotai/shared";
...
describe("IssuesList", () => {
  ...
  it("forwards external-object summaries into issue rows", async () => {
    ...
    const { root } = renderWithQueryClient(
      <IssuesList
        ...
        agents={[]}
        projects={[]}
        viewStateKey="pilot:test-issues"
  ...
  it("filters issue rows by external-object status summaries", async () => {
    ...
    const failedIssue = createIssue({ id: "issue-failed", identifier: "PIL-10", title: "Failed external object" });
    const freshIssue = createIssue({ id: "issue-fresh", identifier: "PIL-11", title: "Fresh external object" });
    const noObjectIssue = createIssue({ id: "issue-none", identifier: "PIL-12", title: "No external object" });
    localStorage.setItem("pilot:test-issues:company-1", JSON.stringify({ externalObjectStatuses: ["failed"] }));
    mockExternalObjectsApi.getIssueSummaries.mockResolvedValue({
      summaries: {
        "issue-failed": {
          total: 1,
  ...
  it("renders server search results instead of filtering the full issue list locally", async () => {
    const localIssue = createIssue({ id: "issue-local", identifier: "PIL-1", title: "Local issue" });
    const serverIssue = createIssue({ id: "issue-server", identifier: "PIL-2", title: "Server result" });
  ...
  it("renders the opt-in sub-issue progress summary with workflow next-up linking", async () => {
    ...
    await waitForAssertion(() => {
      ...
      const link = container.querySelector('a[href="/issues/PIL-2"]');
  ...
  it("adds checklist affordances for workflow-sorted sub-issue lists", async () => {
    ...
    const issueBlocked = createIssue({
      ...
      title: "Blocked issue",
      status: "blocked",
      blockedBy: [{ id: "issue-active", identifier: "PIL-3", title: "Active blocker", status: "todo", priority: "medium", assigneeAgentId: null, assigneeUserId: null }],
      createdAt: new Date("2026-04-02T00:00:00.000Z"),
    });
    ...
    await waitForAssertion(() => {
      ...
      expect(container.textContent?.replace(/\s+/g, "")).toContain("1.PIL-1");
      expect(container.textContent?.replace(/\s+/g, "")).toContain("2.PIL-3");
      ...
      expect(container.textContent).toContain("blocked by PIL-3 · step 2");
    });
  ...
  it("hides the Priority option from the Sort and Group menus while priority UI is off (PIL-411)", async () => {
    const { root } = renderWithQueryClient(
      <IssuesList
        issues={[createIssue({ id: "issue-1", identifier: "PIL-1", title: "Task one" })]}
        agents={[]}
        projects={[]}
        viewStateKey="pilot:test-issues"
        onUpdateIssue={() => undefined}
    ...
    await waitForAssertion(() => {
      ...
      // Status sort option renders, but the Priority option is gated off (PIL-411).
  ...
  it("hides the workflow blocker chip when a sub-issue is blocked only by its previous sibling", async () => {
    ...
    await waitForAssertion(() => {
      ...
      expect(container.textContent).not.toContain("blocked by PIL-1");
    });
  ...
  it("collapses multiple workflow blocker chips to the first blocker and a count", async () => {
    ...
    await waitForAssertion(() => {
      expect(container.textContent).toContain("blocked by PIL-2");
      expect(container.textContent).toContain("... and 2 more");
      expect(container.textContent).not.toContain("blocked by PIL-3");
      expect(container.textContent).not.toContain("blocked by PIL-4");
      ...
      expect(blockerButtons[0]?.textContent).toBe("blocked by PIL-2 · step 2 ... and 2 more");
    });
  ...
  it("lets board users choose the per-column page size", async () => {
    ...
    expect(localStorage.getItem("pilot:test-issues:company-1")).toContain("\"boardColumnPageSize\":25");
  ...
  it("uses context-scoped persisted column visibility", async () => {
    localStorage.setItem("pilot:test-issues:company-1:issue-columns", JSON.stringify(["id", "assignee"]));
    ...
    await waitForAssertion(() => {
      ...
      expect(container.textContent).toContain("PIL-9");
  ...
  it("filters the list to a single workspace when a workspace name is clicked", async () => {
    localStorage.setItem("pilot:test-issues:company-1:issue-columns", JSON.stringify(["id", "workspace"]));
    mockInstanceSettingsApi.getExperimental.mockResolvedValue({ enableIsolatedWorkspaces: true });
    mockExecutionWorkspacesApi.listSummaries.mockResolvedValue([
      {
        id: "workspace-alpha",
  ...
  // Run 3 review (Jul 8) reversed PIL-243's lg enlargement: task rows in the
  // list and inbox standardize on md (16px). The live list always supplies its
  // own `statusSlot` (the PIL-246 slot-override gotcha), so assert the real
  // slot size here.
  ...
  it("draws day and week separators between recency-sorted rows", async () => {
    ...
    const { root } = renderWithQueryClient(
      <IssuesList
        issues={[
          createIssue({ id: "issue-recent", identifier: "PIL-1", title: "Just updated", updatedAt: hourAgo }),
          createIssue({ id: "issue-mid", identifier: "PIL-2", title: "A few days old", updatedAt: threeDaysAgo }),
          createIssue({ id: "issue-old", identifier: "PIL-3", title: "Over a week old", updatedAt: tenDaysAgo }),
        ]}
        agents={[]}
        projects={[]}
        viewStateKey="pilot:test-issues"
  ...
  it("draws both separators when adjacent rows skip the middle age bucket", async () => {
    ...
    const { root } = renderWithQueryClient(
      <IssuesList
        issues={[
          createIssue({ id: "issue-recent", identifier: "PIL-1", title: "Just updated", updatedAt: new Date(now - 60 * 60 * 1000) }),
          createIssue({ id: "issue-old", identifier: "PIL-2", title: "Over a week old", updatedAt: new Date(now - 10 * 24 * 60 * 60 * 1000) }),
        ]}
        agents={[]}
        projects={[]}
        viewStateKey="pilot:test-issues"
  ...
  it("places separators around expanded nested rows in visible order", async () => {
    ...
    const { root } = renderWithQueryClient(
      <IssuesList
        issues={[
          createIssue({ id: "issue-parent", identifier: "PIL-1", title: "Recent parent", updatedAt: new Date(now - 60 * 60 * 1000) }),
          createIssue({ id: "issue-child", identifier: "PIL-2", parentId: "issue-parent", title: "Older child", updatedAt: new Date(now - 3 * 24 * 60 * 60 * 1000) }),
          createIssue({ id: "issue-old", identifier: "PIL-3", title: "Old root", updatedAt: new Date(now - 10 * 24 * 60 * 60 * 1000) }),
        ]}
        agents={[]}
        projects={[]}
        viewStateKey="pilot:test-issues"
  ...
  it("omits date separators when all rows share a recency bucket", async () => {
    ...
    const { root } = renderWithQueryClient(
      <IssuesList
        issues={[
          createIssue({ id: "issue-a", identifier: "PIL-1", title: "One", updatedAt: new Date(now - 60 * 60 * 1000) }),
          createIssue({ id: "issue-b", identifier: "PIL-2", title: "Two", updatedAt: new Date(now - 2 * 60 * 60 * 1000) }),
        ]}
        agents={[]}
        projects={[]}
        viewStateKey="pilot:test-issues"
