...
describe("SummarySlotCard", () => {
  ...
  it("shows an active generating state with the linked task", async () => {
    ...
    expect(container.textContent).toContain("PIL-14000: Summarize project");
    expect(container.querySelector('a[href="/issues/PIL-14000"]')).not.toBeNull();
    expect(container.querySelector('a[href="/issues/PIL-14000"]')?.closest("div")?.parentElement?.className)
  ...
  it("shows stopped generation as a failed retryable state", async () => {
    mockSummarySlotsApi.get.mockResolvedValue({
      slot: slot({
        status: "failed",
        failureReason: "Summary generation task PIL-14000 finished without writing a summary.",
    ...
    expect(container.textContent).toContain("PIL-14000 finished without writing a summary");
