...
import type { Agent, AttentionDetailImage, AttentionItem } from "@pilotai/shared";
...
/**
 * Memoized (PIL-13784): the queue renders every feed row in one flat list, so
...
export const AttentionQueueRow = memo(function AttentionQueueRow({
  ...
  // images expand to reveal a larger gallery (PIL-13544); triage-enabled rows
  // expand to reveal the per-card triage strip (PIL-16032 §4.5). Any of these
  ...
  // feed built before the metadata existed (PIL-17287).
...
/**
 * Larger image gallery shown when a row is expanded (PIL-13544). Shows the
