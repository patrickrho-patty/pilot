...
import type { Issue, IssueDocument } from "@pilotai/shared";
...
function createExecutionWorkspace(overrides: Partial<ExecutionWorkspace> = {}): ExecutionWorkspace {
  return {
    ...
    name: "PIL-1 workspace",
    status: "active",
    deliveryState: "unknown",
    cwd: "/tmp/pilot/PIL-1",
    repoUrl: null,
    baseRef: "master",
    branchName: "pil-1-workspace",
    providerType: "git_worktree",
    providerRef: "/tmp/pilot/PIL-1",
...
function createProject(overrides: Partial<Project> = {}): Project {
  ...
  return {
    ...
    codebase: {
      ...
      defaultRef: "master",
      repoName: null,
      localFolder: "/tmp/pilot",
      managedFolder: "/tmp/pilot",
      effectiveLocalFolder: "/tmp/pilot",
      origin: "local_folder",
    },
...
describe("IssueProperties", () => {
  ...
  it("hides the Priority property row while priority UI is off (PIL-411)", async () => {
    const root = renderProperties(container, {
  ...
  it("exposes the classic-layout add sub-issue pill action", async () => {
    ...
    // + its Add sub-task button only render in the classic layout (PIL-496).
    mockInstanceSettingsApi.getExperimental.mockResolvedValue({
      enableTaskWatchdogs: false,
  ...
  it("renders blocked-by issues as direct chips and edits them from an add action", async () => {
    const onUpdate = vi.fn();
    mockIssuesApi.list.mockResolvedValue([
      createIssue({ id: "issue-3", identifier: "PIL-3", title: "New blocker", status: "todo" }),
    ]);
    ...
    const blockerLink = container.querySelector('a[href="/issues/PIL-2"]');
    expect(blockerLink).not.toBeNull();
    expect(blockerLink?.textContent).toContain("PIL-2");
    ...
    const removeButton = container.querySelector('button[aria-label="Remove PIL-2 as blocker"]');
    ...
    const candidateButton = Array.from(container.querySelectorAll("button"))
      .find((button) => button.textContent?.includes("PIL-3 New blocker"));
  ...
  it("searches all company issues when adding a blocker", async () => {
    const onUpdate = vi.fn();
    const loadedIssue = createIssue({ id: "issue-3", identifier: "PIL-3", title: "Loaded issue", status: "todo" });
    const remoteIssue = createIssue({ id: "issue-99", identifier: "PIL-99", title: "Remote blocker", status: "in_progress" });
    ...
    await waitForAssertion(() => {
      ...
      expect(container.textContent).toContain("PIL-99 Remote blocker");
      expect(container.textContent).not.toContain("PIL-3 Loaded issue");
    });

    const candidateButton = Array.from(container.querySelectorAll("button"))
      .find((button) => button.textContent?.includes("PIL-99 Remote blocker"));
    expect(candidateButton).not.toBeUndefined();
  ...
  it("removes a blocked-by issue from the chip remove action after confirmation", async () => {
    ...
    expect(document.body.textContent).toContain("Remove PIL-2: Existing blocker as a blocker for this task.");
  ...
  it("opens visit and remove actions when a blocked-by chip is tapped on mobile", async () => {
    ...
    expect(container.querySelector('a[href="/issues/PIL-2"]')).toBeNull();
    expect(container.querySelector('button[aria-label="Remove PIL-2 as blocker"]')).toBeNull();
    const blockerActions = container.querySelector('button[aria-label="Actions for blocker PIL-2"]');
    ...
    const visitLink = Array.from(document.body.querySelectorAll('a[href="/issues/PIL-2"]'))
  ...
  it("collapses long blocked-by and sub-task lists until the more button is clicked", async () => {
    ...
    // now — the chat shell promotes sub-tasks to their own pane tab (PIL-496).
    mockInstanceSettingsApi.getExperimental.mockResolvedValue({
      enableTaskWatchdogs: false,
      enableClassicTaskInterface: true,
    });
  ...
  it("copies branch and folder values with visible feedback", async () => {
    ...
    const branchCopyButton = container.querySelector<HTMLButtonElement>(
      'button[aria-label="Copy pil-1-workspace to clipboard"]',
    );
    ...
    expect(writeText).toHaveBeenCalledWith("pil-1-workspace");
  ...
  it("shows related task references below sub-issues", async () => {
          ...
          outbound: [
            {
              ...
              mentionCount: 1,
              sources: [{ kind: "description", sourceRecordId: null, label: "description", matchedText: "PIL-22" }],
            },
          ],
          inbound: [],
        },
    ...
    expect(container.textContent).toContain("PIL-22");
        ...
        relatedWork: {
          outbound: [
            ...
            },
            {
              ...
              mentionCount: 1,
              sources: [{ kind: "description", sourceRecordId: null, label: "description", matchedText: "PIL-33" }],
            },
            {
              issue: {
                id: "child-44",
              ...
              mentionCount: 1,
              sources: [{ kind: "description", sourceRecordId: null, label: "description", matchedText: "PIL-44" }],
            },
          ],
          inbound: [],
        },
  ...
  it("allows setting and clearing a parent issue from the properties pane", async () => {
    const onUpdate = vi.fn();
    mockIssuesApi.list.mockResolvedValue([
      createIssue({ id: "issue-2", identifier: "PIL-2", title: "Candidate parent", status: "in_progress" }),
    ]);
    ...
    const candidateButton = Array.from(container.querySelectorAll("button"))
      .find((button) => button.textContent?.includes("PIL-2 Candidate parent"));
    expect(candidateButton).not.toBeUndefined();
    ...
    const parentLink = container.querySelector('a[href="/issues/PIL-2"]');
  ...
  it("searches the server for parent candidates so a lower-priority match past the default page stays selectable", async () => {
    ...
    await waitForAssertion(() => {
      expect(container.textContent).toContain("PIL-2 High priority candidate");
    });
    expect(container.textContent).not.toContain("PIL-900 Low priority needle");
    ...
    await waitForAssertion(() => {
      ...
      expect(container.textContent).toContain("PIL-900 Low priority needle");
      expect(container.textContent).not.toContain("PIL-2 High priority candidate");
    });

    const candidateButton = Array.from(container.querySelectorAll("button"))
      .find((button) => button.textContent?.includes("PIL-900 Low priority needle"));
    expect(candidateButton).not.toBeUndefined();
  ...
  it("renders each external object as its own properties row using display metadata", async () => {
    ...
    expect(pullRequestLink?.className).not.toContain("pilot-mention-chip");
  ...
  // PIL-16506 P4: only an agent sets `reviewPolicy`, so the panel shows it and
