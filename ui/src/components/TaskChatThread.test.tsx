...
describe("TaskChatThread blocker links", () => {
  it("shows the direct and server-selected terminal blocker at the top and bottom", () => {
    ...
    render(
      <TaskChatThread
        ...
        blockerAttention={{
          ...
          sampleBlockerIdentifier: "PIL-777",
    ...
    for (const notice of notices) {
      expect(notice.textContent).toContain("Blocked byPIL-600Waiting in review");
      expect(notice.textContent).toContain("Ultimately blocked byPIL-777Actual work");
      expect(notice.querySelector('a[href="/issues/PIL-600"]')).not.toBeNull();
      expect(notice.querySelector('a[href="/issues/PIL-777"]')).not.toBeNull();
    }
  ...
  it("shows the ordered live-work queue at the top and bottom", () => {
    ...
    render(
      <TaskChatThread
        ...
        blockerAttention={{
          ...
          sampleBlockerIdentifier: "PIL-17426",
          sampleStalledBlockerIdentifier: null,
    ...
    for (const notice of notices) {
      ...
      expect(orderedLinks).toEqual([
        "PIL-17424Run the guarded cutover",
        "PIL-17425Verify the live projection",
        "PIL-17427Verify the completed projection",
      ]);
      expect(notice.textContent).toContain("Now runningPIL-17426Restore live alias projection");
      expect(notice.querySelector('a[href="/issues/PIL-17426"]')).not.toBeNull();
    }
    ...
    render(
      <TaskChatThread
        ...
        blockerAttention={{
          ...
          stalledBlockerCount: 0,
          attentionBlockerCount: 0,
          sampleBlockerIdentifier: "PIL-500",
          sampleStalledBlockerIdentifier: null,
  ...
  it("shows only the direct row when the blocker has no deeper unresolved leaf", () => {
    ...
    expect(container.querySelectorAll('[data-testid="task-chat-blocker-links"]')).toHaveLength(2);
    expect(container.textContent).toContain("Blocked byPIL-500Direct dependency");
  ...
  it("keeps a server-selected intermediate blocker on its direct chain", () => {
    ...
    render(
      <TaskChatThread
        ...
        blockerAttention={{
          ...
          attentionBlockerCount: 1,
          sampleBlockerIdentifier: "PIL-650",
          sampleStalledBlockerIdentifier: "PIL-650",
    ...
    for (const notice of container.querySelectorAll('[data-testid="task-chat-blocker-links"]')) {
      expect(notice.textContent).toContain("Blocked byPIL-600Selected dependency");
      expect(notice.textContent).toContain("Ultimately blocked byPIL-650Stalled intermediate review");
    }
...
describe("TaskChatThread mobile composer dock (PIL-495)", () => {
  it("pins the composer to the nav-aware bottom offset so its action row clears the auto-hiding bottom nav", () => {
    ...
    // what let the nav occlude the action row before PIL-495.
...
describe("TaskChatThread live transcript", () => {
  ...
  it("renders in-flight output through TaskChatLiveTail, dropping the debug plumbing (PIL-463 C1)", () => {
  ...
  it("keeps the transcript mounted through run settle until the settled turn renders (PIL-462 B4)", () => {
    transcriptState.transcriptByRun.set("run-1", [
      {
        kind: "assistant",
        ts: "2026-08-07T00:00:00.000Z",
