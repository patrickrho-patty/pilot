...
import type { CompanySecret, EnvBinding, UserSecretDefinition } from "@pilotai/shared";
...
const DEFAULT_RESERVED_PREFIXES = ["PILOT_"];

const DEFAULT_HINT =
  "Set the KEY to the env var name the process expects, for example GH_TOKEN. Choose a secret to resolve a stored value at run start. PILOT_* variables are injected automatically.";
...
export interface EnvironmentVariablesEditorProps {
  ...
  /** Read-only rendering. */
  disabled?: boolean;
  /** Prefixes flagged as reserved/auto-provided. Default `["PILOT_"]`. */
