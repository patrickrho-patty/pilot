...
// closing menu via `window.setTimeout(…, 0)` (PIL-12476/12477/12478). Under real
...
describe("EnvironmentVariablesEditor", () => {
  ...
  afterEach(async () => {
    // Restore real timers first (the PIL-12478 test swaps in fake timers and may
  ...
  it("keeps long secret names clear of the latest version control", () => {
    const longSecret = makeSecret("long", {
      name: "/pilot-cloud/prod/provider/resend/api-key-with-a-very-long-name",
      latestVersion: 4,
    });
  ...
  it("renders name warnings as a row spanning the name and value columns", () => {
    render(
      <EnvironmentVariablesEditor
        value={{ PILOT_PAGE_BASE_URL: { type: "plain", value: "https://pages.pilot.test" } }}
        secrets={secrets}
        onChange={() => {}}
        onCreateSecret={async () => secrets[0]}
      />,
  ...
  it("opens the create-secret popover from the picker's + Create item (§6.4, PIL-12476)", async () => {
  ...
  it("keeps the create-secret popover open when focus returns to a control inside the value cell (§6.4, PIL-12492)", async () => {
  ...
  it("opens the store-as-secret popover from the ⋯ overflow menu (PIL-12477)", async () => {
  ...
  it("opens the store popover from the source dropdown Text→Company secret (with a value) and keeps it open (PIL-12478)", () => {
    ...
    // (PIL-12476/12477): opening the popover synchronously inside the menu-item's
  ...
  it("lets slash-delimited secrets be browsed by folder and exposes full paths on hover", async () => {
    const cloudProviderKey = "/pilot-cloud/prod/provider/aws-access-key-id";
    const cloudMigrationKey = "/pilot-cloud/prod/migration/postgres-url";
    ...
    render(
      <SecretPicker
        ...
        secrets={[
          ...
          makeSecret("plain-key", { name: "pilot-page-aws-access-key-id", key: "pilot-page-aws-access-key-id" }),
        ]}
        onSelect={onSelect}
        onCreateNew={() => {}}
        disablePortal
    ...
    expect(rowTitle(itemByText("pilot-cloud"))).toBe("/pilot-cloud");
    expect(rowTitle(itemByText("pilot-page-aws-access-key-id"))).toBe(
      "pilot-page-aws-access-key-id",
    );

    itemByText("pilot-cloud")!.click();
    await flush();
    expect(rowTitle(itemByText("prod"))).toBe("/pilot-cloud/prod");

    itemByText("prod")!.click();
    await flush();
    expect(rowTitle(itemByText("provider"))).toBe("/pilot-cloud/prod/provider");
    expect(rowTitle(itemByText("migration"))).toBe("/pilot-cloud/prod/migration");
  ...
  it("keeps secret search global while browsing starts at slash folders", async () => {
      ...
      <SecretPicker
        ...
        secrets={[
          makeSecret("provider-key", {
            name: "/pilot-cloud/prod/provider/aws-access-key-id",
            key: "/pilot-cloud/prod/provider/aws-access-key-id",
          }),
          makeSecret("migration-key", { name: cloudMigrationKey, key: cloudMigrationKey }),
        ]}
        onSelect={() => {}}
    ...
    combobox.focus();
    await flush();
    expect(document.body.textContent).toContain("pilot-cloud");
