...
import type { Approval } from "@pilotai/shared";
...
describe("CommentThread external object decoration (integration)", () => {
  ...
  it("decorates a resolved URL and leaves an unknown URL unchanged when rendered through the real MarkdownBody", () => {
    ...
    expect(resolvedLink?.classList.contains("pilot-markdown-external-ref")).toBe(true);
    ...
    expect(unknownLink?.classList.contains("pilot-markdown-external-ref")).toBe(false);
