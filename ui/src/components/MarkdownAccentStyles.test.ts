...
describe("accent markdown styles", () => {
  it("inherits the current message bubble foreground for prose text, links, and list counters", () => {
    const block = cssBlock(".pilot-markdown.pilot-markdown-on-accent");
  ...
  it("keeps ordered-list markers and rendered link variants readable on accent bubbles", () => {
    expect(cssBlock(".pilot-markdown.pilot-markdown-on-accent li::marker")).toContain(
      "color: currentColor",
    );
    expect(cssBlock(".pilot-markdown.pilot-markdown-on-accent :where(a, a:visited)")).toContain(
      "color: currentColor",
    );
    expect(cssBlock(".pilot-markdown.pilot-markdown-on-accent .pilot-workspace-file-link")).toContain(
      "color: currentColor",
    );
    expect(
      cssBlock(
        ".pilot-markdown.pilot-markdown-on-accent :where(a.pilot-mention-chip, a.pilot-project-mention-chip)",
      ),
