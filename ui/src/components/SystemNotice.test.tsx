...
describe("SystemNotice", () => {
  ...
  it("collapses metadata details by default and toggles aria-expanded on click", () => {
    ...
    expect(node.textContent).not.toContain("PIL-3440");
    ...
    expect(node.textContent).toContain("PIL-3440");
  });
  ...
  it("renders typed metadata rows with hrefs when present", () => {
        ...
        metadata={[
          {
            rows: [
              ...
              },
              {
                ...
                href: "/PIL/agents/codexcoder/runs/9cdba892",
                status: "succeeded",
              },
            ],
          },
    ...
    expect(links).toContain("/PIL/issues/PIL-3440");
    expect(links).toContain("/PIL/agents/cto");
    expect(links).toContain("/PIL/agents/codexcoder/runs/9cdba892");
    expect(node.textContent).toContain("PIL-3440");
  ...
  it("renders metadata link rows as plain text when href is missing", () => {
        ...
        metadata={[
          {
            rows: [
              ...
              { kind: "issue", label: "Issue", identifier: "PIL-1" },
            ],
          },
        ]}
      />,
    ...
    expect(node.textContent).toContain("PIL-1");
  });
});
