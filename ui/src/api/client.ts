...
/**
 * Non-authoritative observability hints (PIL-12556 / Phase 1). The server treats
 * these as scheduling/telemetry only and never as security signals.
 */
function applyObservabilityHeaders(headers: Headers) {
  if (headers.has("x-pilot-Tab-Visible")) return; // caller override wins
  const visibility = getPageVisibility();
  headers.set("x-pilot-Tab-Visible", getVisibilityHeaderValue(visibility));
  if (typeof window !== "undefined" && window.location) {
    headers.set("x-pilot-Route", window.location.pathname);
  }
}
