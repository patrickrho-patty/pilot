/**
 * Cross-tab shared polling coordination (PIL-12557 / Phase 2 of PIL-12542).
 *
 * Goal: when a user restores many Pilot tabs at once, we do not want every
...
const TAB_ID_STORAGE_KEY = "pilot:shared-poll:tab-id";
...
export function sharedPollingLeaseKey(companyId: string): string {
  return `pilot:shared-poll:${sanitizeCompanyId(companyId)}:leader`;
}

export function sharedPollingChannelName(companyId: string): string {
  return `pilot:shared-poll:${sanitizeCompanyId(companyId)}`;
}
...
function createBrowserLeaseStore(companyId: string): LeaseStore | null {
  ...
  try {
    const probeKey = `pilot:shared-poll:${sanitizeCompanyId(companyId)}:probe`;
