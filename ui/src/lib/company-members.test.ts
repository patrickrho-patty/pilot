...
describe("company-members helpers", () => {
  ...
  it("includes human users in markdown mention options", () => {
    const options = buildMarkdownMentionOptions({
      ...
      projects: [{ id: "project-1", name: "Pilot App", color: "#336699" }],
    });
    ...
    expect(options).toEqual([
      ...
      { id: "project:project-1", name: "Pilot App", kind: "project", projectId: "project-1", projectColor: "#336699" },
    ]);
  });

  it("builds issue mention options with identifier + title search text", () => {
    const options = buildIssueMentionOptions([
      { id: "issue-1", identifier: "PIL-102", title: "@task references" },
      { id: "issue-2", identifier: "PIL-7", title: "" },
    ]);
  ...
  it("appends issue mention options after agents and projects, preserving order", () => {
    const options = buildMarkdownMentionOptions({
      ...
      projects: [{ id: "project-1", name: "Pilot App", color: "#336699" }],
      issues: [
        { id: "issue-2", identifier: "PIL-50", title: "Newer" },
        { id: "issue-1", identifier: "PIL-3", title: "Older" },
      ],
    });
