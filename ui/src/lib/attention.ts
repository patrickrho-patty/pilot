 ...
 * surface.
 *
 * `review` is inline *only when stalled* (PIL-16080 §4.4): a stalled review has
...
// ---------------------------------------------------------------------------
// Decision kind → borrowed task status (supersedes the PIL-13409 §4 tone map)
//
...
// ---------------------------------------------------------------------------
// Richer detail line (PIL-13409 §7) — render T1's structured `detail` block into
...
// ---------------------------------------------------------------------------
// Aging shelf (PIL-16032 §4.4) — items idle past the threshold leave the desk.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Date-range chips (PIL-16032 §4.2) — resolve to server-side activity bounds.
...
// ---------------------------------------------------------------------------
// Grouping / sorting / filtering (PIL-13408 — Inbox-style toolbar)
//
...
export const ATTENTION_GROUP_BY_KEY = "pilot:attention:group-by";
export const ATTENTION_SORT_KEY = "pilot:attention:sort";
export const ATTENTION_FILTERS_KEY_PREFIX = "pilot:attention:filters";
export const ATTENTION_COLLAPSED_GROUPS_KEY_PREFIX = "pilot:attention:collapsed-groups";
 ...
 * groups first, then the open curtains (PIL-13784). The feed is uncapped, so
