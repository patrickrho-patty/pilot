import type { PipelineCaseLiveness } from "@pilotai/shared";
...
const AUTO_RETRY_NOTE =
  "Pilot retries automatically once the blocker clears — you don't need to move the item by hand.";
 ...
 * the PIL-11245 voice rule forbids, so we translate it here. See PIL-11259.
 */
const NO_ACTION_PATH_BODY =
  "Pilot can't see anything to work on next here — no automation, retry, blocker, or review. " +
  ...
  switch (liveness.reason) {
    ...
    case "permission_preflight_failed":
      return {
        ...
        retryLabel: "",
        helperNote:
          "Grant the access above to the configured responsible, then Pilot retries automatically.",
      };
...
/** True when the PIL-11238 "Re-run stage automation" menu item must be disabled. */
