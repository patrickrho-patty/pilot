...
describe("issueDetailBreadcrumb", () => {
    ...
    return {
      ...
      project: {
        ...
        codebase: {
          ...
          repoName: null,
          localFolder: null,
          managedFolder: "/tmp/pilot-app",
          effectiveLocalFolder: "/tmp/pilot-app",
          origin: "local_folder",
        },
        workspaces: [],
  ...
  it("returns clean issue detail paths", () => {
    expect(createIssueDetailPath("PIL-465")).toBe("/issues/PIL-465");
  });

  it("prefers the full breadcrumb from route state", () => {
    const state = createIssueDetailLocationState("Inbox", "/inbox/mine", "inbox");

    expect(readIssueDetailBreadcrumb("PIL-465", state, "?from=issues")).toEqual({
      label: "Inbox",
      href: "/inbox/mine",
    });
  });
  ...
  it("falls back to the source query param when route state is unavailable", () => {
    expect(readIssueDetailBreadcrumb("PIL-465", null, "?from=inbox")).toEqual({
      label: "Inbox",
      href: "/inbox",
    });
  });
  ...
  it("restores the exact breadcrumb href from the query fallback", () => {
    expect(
      readIssueDetailBreadcrumb("PIL-465", null, "?from=inbox&fromHref=%2Finbox%2Funread"),
    ).toEqual({
      label: "Inbox",
      href: "/inbox/unread",
    });
  ...
  it("reads hidden breadcrumb context from session storage when route state is unavailable", () => {
    ...
    rememberIssueDetailLocationState("PIL-465", state);

    expect(
      readIssueDetailLocationState("PIL-465", null),
    ).toEqual({
  ...
  it("persists issue header seed data when breadcrumb state is remembered", () => {
    ...
    sessionStorageMock.clear();
    rememberIssueDetailLocationState("PIL-42", seededState);

    const restoredState = readIssueDetailLocationState("PIL-42", null);
