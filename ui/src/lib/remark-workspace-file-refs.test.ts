...
describe("remarkWorkspaceFileRefs", () => {
  ...
  it("round-trips workspace folder hrefs", () => {
    const targetPath = "content-os/cases/active/2026-06-06-pil-10199-bundled-skills/";
  ...
  it("converts trailing-slash inline code into workspace folder links", () => {
    const tree = paragraph([
      textNode("Open "),
      inlineCode("content-os/cases/active/2026-06-06-pil-10199-bundled-skills/"),
      textNode("."),
    ]);
    runPlugin(tree);
    const link = tree.children![1];
    ...
    expect(parseWorkspaceFileHref(link.url)).toMatchObject({
      path: "content-os/cases/active/2026-06-06-pil-10199-bundled-skills/",
      resourceKind: "directory",
    });
  });

  it("round-trips explicit project workspace identity", () => {
    const targetPath = "content-os/cases/active/2026-06-06-pil-10199-bundled-skills/README.md";
    const href = buildWorkspaceFileHref({
      path: targetPath,
      line: 5,
      column: null,
  ...
  describe("availability gating", () => {
    ...
    it("leaves an ordinary markdown link untouched when the ref is not openable", () => {
      ...
      runPlugin(tree, resolveNoneOpenable);
      expect(tree.children![0].url).toBe("/PIL/issues/PIL-10306");
