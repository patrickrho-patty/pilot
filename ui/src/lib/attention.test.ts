...
import type { AttentionFeed, AttentionItem, AttentionSourceKind } from "@pilotai/shared";
import {
...
describe("isInlineResolvable", () => {
  ...
  it("inlines a stalled review the server flagged (PIL-16080 §4.4)", () => {
...
describe("attentionTaskRef", () => {
  it("reads the task off the subject when the subject IS the task", () => {
    ...
    expect(attentionTaskRef(item)).toEqual({ identifier: "PIL-23", href: "/PIL/issues/PIL-23" });
  });
  ...
  it("reads the task off relatedIssue when the subject merely hangs off one", () => {
    const item = buildItem({
      ...
      subject: {
        ...
        identifier: null,
        status: "pending",
        href: "/PIL/issues/PIL-20#interaction-x1",
      },
      relatedIssue: {
        kind: "issue",
        id: "i2",
        ...
        identifier: "PIL-20",
        status: "in_review",
        href: "/PIL/issues/PIL-20",
      },
    });
    expect(attentionTaskRef(item)).toEqual({ identifier: "PIL-20", href: "/PIL/issues/PIL-20" });
  });
  ...
  it("prefers relatedIssue when both are present — it is the record the subject can't describe", () => {
    const item = buildItem({
      ...
      relatedIssue: {
        ...
        identifier: "PIL-2",
        status: "todo",
        href: "/PIL/issues/PIL-2",
      },
    });
    expect(attentionTaskRef(item)?.identifier).toBe("PIL-2");
  });
...
describe("planAttentionRenderRows (PIL-13784 incremental rendering)", () => {
