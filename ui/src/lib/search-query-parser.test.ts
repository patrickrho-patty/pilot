...
const context = {
  ...
  projects: [
    { id: "11111111-1111-4111-8111-111111111111", name: "Pilot App", urlKey: "pilot-app" },
  ],
  labels: [
...
describe("parseSearchQuery", () => {
  ...
  it("parses project names", () => {
    expect(parseSearchQuery("project:pilot-app", context).filters).toEqual({
  ...
  it("parses mixed free text and multiple operators", () => {
    expect(parseSearchQuery("auth status:in_progress priority:critical project:pilot-app", context)).toMatchObject({
      query: "auth",
      filters: {
        status: ["in_progress"],
        priority: ["critical"],
