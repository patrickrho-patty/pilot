...
describe("buildAgentOnboardingPrompt", () => {
  it("includes Hermes Gateway setup guidance for external agents", () => {
    ...
    expect(prompt).toContain("not the Pilot API key");
    ...
    expect(prompt).toContain("agentDefaultsPayload.pilotApiUrl");
    ...
    expect(prompt).toContain("Pilot maps it to `/api`");
    ...
    expect(prompt).toContain("`hermes_local` runs Hermes on the Pilot host");
    expect(prompt).toContain("Hermes-originated Pilot API calls");
    expect(prompt).toContain("Do not rotate or invent a Pilot key manually");
