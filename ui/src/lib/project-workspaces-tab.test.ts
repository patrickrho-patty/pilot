...
import type { ExecutionWorkspace, Issue, Project, ProjectWorkspace, WorkspaceRuntimeService } from "@pilotai/shared";
...
function createExecutionWorkspace(overrides: Partial<ExecutionWorkspace>): ExecutionWorkspace {
  return {
    ...
    name: overrides.name ?? "PIL-893",
    status: overrides.status ?? "active",
    deliveryState: overrides.deliveryState ?? "unknown",
    cwd: overrides.cwd ?? "/repo/.worktrees/PIL-893",
    repoUrl: overrides.repoUrl ?? null,
    baseRef: overrides.baseRef ?? "public-gh/master",
    branchName: overrides.branchName ?? "PIL-893-workspaces-tab",
    ...
    const summaries = buildProjectWorkspaceSummaries({
      ...
      executionWorkspaces: [
        createExecutionWorkspace({
          id: "exec-1",
          name: "PIL-893",
          branchName: "PIL-893-workspaces-tab",
