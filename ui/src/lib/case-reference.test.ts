...
describe("parseCaseReferenceFromHref", () => {
  it("linkifies a bare case identifier to the case detail path", () => {
    expect(parseCaseReferenceFromHref("PIL-C7")).toEqual({
      identifier: "PIL-C7",
      href: "/cases/PIL-C7",
    });
  });
  ...
  it("normalizes case to upper", () => {
    expect(parseCaseReferenceFromHref("pil-c12")?.identifier).toBe("PIL-C12");
  });

  it("ignores plain issue identifiers (no -C infix)", () => {
    expect(parseCaseReferenceFromHref("PIL-123")).toBeNull();
    expect(parseCaseReferenceFromHref("PIL-7")).toBeNull();
  });

  it("respects the known-prefix allowlist when provided", () => {
    expect(parseCaseReferenceFromHref("FOO-C1", new Set(["PIL"]))).toBeNull();
    expect(parseCaseReferenceFromHref("PIL-C1", new Set(["PIL"]))?.identifier).toBe("PIL-C1");
  });
...
describe("remarkLinkCaseReferences", () => {
  it("rewrites a bare token inside a text node into a link node", () => {
    const tree = {
      ...
      children: [
        {
          type: "paragraph",
          children: [{ type: "text", value: "see PIL-C7 for details" }],
        },
      ],
    };
    ...
    expect(link?.url).toBe("/cases/PIL-C7");
  ...
  it("rewrites inline code that is exactly a case identifier", () => {
    const tree = {
      type: "root",
      children: [
        { type: "paragraph", children: [{ type: "inlineCode", value: "PIL-C42" }] },
      ],
    };
    remarkLinkCaseReferences()(tree as never);
    const paragraph = tree.children[0] as { children: Array<{ type: string; url?: string }> };
    expect(paragraph.children[0]).toMatchObject({ type: "link", url: "/cases/PIL-C42" });
  });
      ...
      children: [
        {
          ...
          children: [{ type: "text", value: "PIL-C7" }],
        },
      ],
    };
    remarkLinkCaseReferences()(tree as never);
