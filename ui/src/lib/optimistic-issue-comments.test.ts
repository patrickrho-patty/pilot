...
describe("optimistic issue comments", () => {
  ...
  it("matches issues by either uuid or identifier reference", () => {
    expect(matchesIssueRef({ id: "issue-1", identifier: "PIL-1" } as const, ["issue-1"])).toBe(true);
    expect(matchesIssueRef({ id: "issue-1", identifier: "PIL-1" } as const, ["PIL-1"])).toBe(true);
    expect(matchesIssueRef({ id: "issue-1", identifier: "PIL-1" } as const, ["issue-2", "PIL-2"])).toBe(false);
  });
  ...
  it("applies optimistic field updates across cached issue collections", () => {
    ...
    const next = applyOptimisticIssueFieldUpdateToCollection(issues, ["PIL-1"], { assigneeAgentId: "agent-9" });
