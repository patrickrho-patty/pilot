...
describe("createZipArchive", () => {
  it("writes a zip archive with the export root path prefixed into each entry", () => {
    ...
    expect(readString(archive, 30, firstNameLength)).toBe("pilot-demo/agents/ceo/AGENTS.md");
    ...
    expect(readString(archive, secondOffset + 30, secondNameLength)).toBe("pilot-demo/COMPANY.md");
  ...
  it("reads a Pilot zip archive back into rootPath and file contents", async () => {
    const archive = createZipArchive(
      {
        "COMPANY.md": "# Company\n",
        "agents/ceo/AGENTS.md": "# CEO\n",
        ".pilot.yaml": "schema: pilot/v1\n",
      },
      "pilot-demo",
    );
  ...
  it("falls back to base64 for invalid UTF-8 bytes instead of mangling them", async () => {
    ...
    expect(result.files["tasks/pil-1/raw-notes"]).toEqual({
      encoding: "base64",
      data: Buffer.from(invalidUtf8).toString("base64"),
      contentType: "application/octet-stream",
    });
  ...
  it("reads standard DEFLATE zip archives created outside Pilot", async () => {
  ...
  it("ignores directory entries from standard zip archives", async () => {
    const archive = createZipArchiveWithDirectoryEntries("pilot-demo");
...
describe("estimateZipArchiveSize", () => {
  it("matches the real archive byte length for mixed text and binary entries", () => {
    ...
    expect(estimateZipArchiveSize(files, "pilot-demo")).toBe(
      createZipArchive(files, "pilot-demo").byteLength,
    );
  });
  ...
  it("returns the 22-byte end-of-central-directory record for an empty map", () => {
    expect(estimateZipArchiveSize({}, "pilot-demo")).toBe(22);
    expect(estimateZipArchiveSize({}, "pilot-demo")).toBe(
      createZipArchive({}, "pilot-demo").byteLength,
    );
  });
});
