...
// `resolverPolicy` (PIL-17277 contract).
...
describe("issue thread interaction helpers", () => {
  ...
  it("allows only safe confirmation target hrefs for rendering", () => {
    for (const href of [
      ...
      "/PIL/issues/PIL-123#document-plan",
      "#document-plan",
    ]) {
...
describe("isDegenerateAskUserQuestions", () => {
  ...
  it("keeps a question whose only choice is a first-class free-text option (PIL-419)", () => {
    expect(isDegenerateAskUserQuestions(askInteraction([
      {
        id: "q1",
...
describe("isSupersededByNewerSiblingInteraction", () => {
  ...
  it("never hides a still-pending card (would strand the assignee)", () => {
    // Guards the PIL-424 / 00b136f45 invariant: a pending question must always
