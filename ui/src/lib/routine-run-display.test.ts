...
import type { RoutineVariable } from "@pilotai/shared";
...
describe("runRowSubtitle", () => {
    ...
    const subtitle = runRowSubtitle(
      {
        status: "succeeded",
        failureReason: null,
        triggerPayload: { customer: "Acme", retries: 3, PILOT_RUN_ID: "ignored" },
      },
      variables,
    );
  ...
  it("only includes declared routine variables, not builtin payload keys", () => {
    const subtitle = runRowSubtitle(
      { status: "succeeded", failureReason: null, triggerPayload: { PILOT_RUN_ID: "x" } },
      variables,
    );
