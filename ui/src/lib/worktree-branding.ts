 ...
 * (`PILOT_IN_WORKTREE=true` on the server). Unlike {@link getWorktreeUiBranding}
...
export function isWorktreeRuntime(): boolean {
  return readMetaContent("pilot-worktree-enabled") === "true";
}
...
export function getWorktreeInstanceId(): string | null {
  return readMetaContent("pilot-instance-id");
}

export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("pilot-worktree-enabled") !== "true") return null;

  const name = readMetaContent("pilot-worktree-name");
  const color = normalizeHexColor(readMetaContent("pilot-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("pilot-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
