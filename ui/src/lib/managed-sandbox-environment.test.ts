...
describe("managed sandbox environment helpers", () => {
  it("identifies platform-managed rows by the provisioner marker", () => {
    expect(isPlatformManagedEnvironment({ metadata: { managedByPilot: true } })).toBe(true);
    expect(isPlatformManagedEnvironment({ metadata: { managedByPilot: false } })).toBe(false);
