...
// PIL-75 brand mapping ("blue = liveness"): todo → amber (queued), in_progress
...
// ---------------------------------------------------------------------------
// Brand `.task-chip` status palette (PIL-75 / status-reference.html)
//
// Colour-named, 1px border, light + dark — values straight from pilot.test
// `brand.css`. Shared by the agents section (PIL-80) and the All Projects page
// (PIL-91); PIL-99 brings it to issue/task status chips, adding `violet` for
// `in_review`.
// ---------------------------------------------------------------------------
...
export const statusBadge: Record<string, string> = {
  ...
  // Skill Studio test-run "queued" aligns with pending (yellow). PIL-12962 D6.
  ...
  // Case statuses (PIL-12968 E3) — `draft` is the neutral pre-work state,
  ...
  // Issue statuses — consistent hues with issueStatusIcon above (PIL-75 brand
  ...
  // Tool access — policy decisions, catalog, and runtime health (Tools & Access, PIL-10389)
...
// ---------------------------------------------------------------------------
// Agent status — brand state system (PIL-75)
// ---------------------------------------------------------------------------
...
/**
 * Issue/task status → brand colour name (PIL-75). `in_progress` is blue
...
export const brandBanner: Record<BannerTone, string> = {
  ...
  // PIL-14031: aligned to the proven `failed`/`error` chip recipe (bg-red-100 /
...
/**
 * Task/issue status → AA-tuned ICON-hue CSS var (PIL-238). Drives the standalone
...
// ---------------------------------------------------------------------------
//
// Categories come from `EXTERNAL_OBJECT_STATUS_CATEGORIES` in @pilotai/shared.
