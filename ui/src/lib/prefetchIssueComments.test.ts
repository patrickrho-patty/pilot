...
import type { Issue, IssueComment } from "@pilotai/shared";
...
describe("prefetchIssueComments", () => {
  ...
  it("warms the first comments page under the infinite query key IssueDetail mounts", async () => {
    ...
    await prefetchIssueComments(queryClient, "PIL-1");

    expect(issuesApi.listComments).toHaveBeenCalledWith("PIL-1", { order: "desc", limit: 50 });
