...
describe("activity formatting", () => {
  ...
  it("formats blocker activity using linked issue identifiers", () => {
    const details = {
      addedBlockedByIssues: [
        { id: "issue-2", identifier: "PIL-22", title: "Blocked task" },
      ],
      removedBlockedByIssues: [],
    };

    expect(formatActivityVerb("issue.blockers_updated", details)).toBe("added blocker PIL-22 to");
    expect(formatIssueActivityAction("issue.blockers_updated", details)).toBe("added blocker PIL-22");
  });
  ...
  // PIL-16506 P4: agents can now resolve an interaction, including a review of
