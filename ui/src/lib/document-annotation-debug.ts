export const SELECTION_DEBUG_STORAGE_KEY = "pilotDebugSelection";
...
declare global {
  interface Window {
    __pilotSelectionDebug?: SelectionDebugState;
  }
}
...
export function initializeSelectionDebug(): SelectionDebugState {
  if (!window.__pilotSelectionDebug) {
    window.__pilotSelectionDebug = createState();
    console.info(`[pilot selection debug] enabled; set localStorage.${SELECTION_DEBUG_STORAGE_KEY} = "0" and reload to disable`);
  }
  return window.__pilotSelectionDebug;
}
...
function record(type: string, details: SelectionDebugEvent["details"]): void {
  ...
  console.debug("[pilot selection debug]", event);
}
