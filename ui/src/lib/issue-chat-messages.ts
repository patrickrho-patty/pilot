...
import type { Agent, IssueComment } from "@pilotai/shared";
...
function authorNameForComment(
  ...
  if (authorAgentId) {
    return agentMap?.get(authorAgentId)?.name ?? (options?.isSystemNotice ? "Pilot" : authorAgentId.slice(0, 8));
  }
  const authorUserId = comment.authorUserId ?? null;
  if (!authorUserId) return options?.isSystemNotice ? "Pilot" : "You";
...
export function buildIssueChatMessages(args: {
  ...
  for (const interaction of sortByCreated(interactions)) {
    ...
    // stale sibling superseded by a newer question (PIL-437) — is skipped here so
    // it leaves no empty message slot in the thread (PIL-424, plan from PIL-420).
