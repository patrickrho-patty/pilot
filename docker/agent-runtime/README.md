...
Container images for running coding-agent harnesses in sandboxed environments (for example the kubernetes sandbox provider, stage 1 of the k8s contribution). Images are named `agent-runtime-{harness}:{version}` and published to `ghcr.io/patty-io/` by the `agent-runtime-images` workflow. The registry is overridable: every reference flows through the `REGISTRY` bake variable.
...
**Pilot Binaries:**
- `/usr/local/bin/pilot-agent-shim`: Go binary compiled from `tools/agent-shim/`. Reads `/run/pilot/runtime-command.json` and `syscall.Exec`s the harness CLI.
...
- `CMD`: `/usr/local/bin/pilot-agent-shim`
...
docker run --rm ghcr.io/patty-io/agent-runtime-claude:dev claude-code --version
```

## Agent Container (pilot-agent-shim)

The main agent process runs as the shim (PID 1 under tini). The shim:

1. Reads `/run/pilot/runtime-command.json` (path overridable via `-spec`), a JSON file mounted by whatever schedules the run
