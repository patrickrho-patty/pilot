...
2. **No issue exists** — describe the problem directly in your PR body, following one of our [issue templates](.github/ISSUE_TEMPLATE/) so a reviewer has the same fields they'd get from a filed issue:
   - **Bug fix:** what happened, expected behavior, steps to reproduce, Pilot version/commit, deployment mode. See [`bug_report.yml`](.github/ISSUE_TEMPLATE/bug_report.yml).
...
Many contributors run their own Pilot instance to manage their work. Issue ids and links from *your* instance are private — reviewers and other contributors cannot open them, so they show up as clutter or broken links.
...
Do **not** include references to internal/instance-local Pilot work, such as:
...
Tooling (including Pilot) often names a working branch after an internal issue and task — e.g. `PAPA-42-why-did-this-break`. That name leaks instance-local context, isn't meaningful to reviewers, and ends up as the public branch on your PR.
...
- Do **not** include internal Pilot ticket ids (`PAPA-123`, `PAP-224`), instance task slugs, or other instance-derived details in the branch name.
...
### Pilot Gates Must Pass

All Pilot CI gates (lint, typecheck, tests, build, and any other required checks) must be satisfied before a PR can be merged. Don't ask for a merge while gates are red — fix them first.
...
We actively manage the core Pilot feature roadmap.
...
> - Pilot is the open source app people use to manage AI agents for work
...
> - This pull request adds per-adapter configuration of compaction, either auto or pilot managed
> - That way we can get optimal performance from any adapter/provider in Pilot
