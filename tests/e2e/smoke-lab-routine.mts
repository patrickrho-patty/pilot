/**
 * Smoke Lab recurring-routine wrapper (PIL-13351 / plan §D5).
 *
...
const SHOT_DIR = process.env.SMOKE_SHOT_DIR ?? path.join(os.tmpdir(), `pil13351-routine-${Date.now()}`);
const BUDGET_MS = Number(process.env.SMOKE_BUDGET_MS ?? 600_000);
const ONLY = process.env.SMOKE_ONLY ?? "";
const DRY = process.env.SMOKE_DRY_RUN === "1";

const CP_BASE = (() => {
  const b = (process.env.PILOT_API_URL ?? "").replace(/\/$/, "");
  return b.replace(/\/api$/, "");
})();
const CP_KEY = process.env.PILOT_API_KEY ?? "";
const CP_COMPANY = process.env.PILOT_COMPANY_ID ?? "";
const CP_RUN = process.env.PILOT_RUN_ID ?? "";
const ROUTINE_ISSUE_ID = process.env.ROUTINE_ISSUE_ID ?? process.env.PILOT_TASK_ID ?? "";
...
function issueLink(issue: Json): string {
  ...
  const prefix = String(identifier).split("-")[0] || "PIL";
...
async function recordFailure(result: Json): Promise<"filed" | "suppressed_duplicate"> {
  ...
  const bodyLines = [
    ...
    ``,
    `Auto-filed by the daily Smoke Lab routine (PIL-13351 / plan §D5). A real product failure was observed while driving the P1–P7 lifecycle in a real browser against a \`local_trusted\` instance.`,
    ``,
    `### Failing step(s)`,
    "```",
    ...failed.map((f) => `- ${f}`),
