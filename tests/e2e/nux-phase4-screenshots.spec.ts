 ...
 *   - Team-lead hire step (capsule wizard, PIL-125)
...
test.describe("NUX Phase 4 visual QA", () => {
  test("captures every integrated surface", async ({ page }) => {
    // New-NUX surfaces are flag-gated default-OFF (PIL-136/137/138): turn the
    ...
    if (!(await page.getByRole("heading", { name: "Welcome to Pilot" }).count())) {
      await page.getByRole("button", { name: /Back to start/ }).click();
    }
    await expect(
      page.getByRole("heading", { name: "Welcome to Pilot" }),
    ...
    // Rules-of-Hooks crash that previously blanked this page — see PIL-50.)
    await expect(
