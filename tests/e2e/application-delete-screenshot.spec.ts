...
// One-off visual capture for PIL-10817. The retired Tools -> Applications
...
test("captures the current app removal confirmations", async ({ page }) => {
  ...
  const companyRes = await page.request.post("/api/companies", {
    data: { name: `PIL-10817 remove app ${Date.now()}` },
  });
  ...
  await page.screenshot({ path: "test-results/pil-10817-delete-dialog.png", fullPage: true });
  ...
  await page.getByRole("button", { name: "Remove app", exact: true }).click();
  await expect(page.getByRole("button", { name: "Yes, remove it" })).toBeVisible();
  await page.screenshot({ path: "test-results/pil-10817-delete-dialog-guarded.png", fullPage: true });
