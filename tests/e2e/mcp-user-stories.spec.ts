...
function buildGatewayCallScript(connectionId: string, toolName: string, parameters: Json = {}) {
  return `
const required = ["PILOT_API_URL", "PILOT_API_KEY", "PILOT_RUN_ID"];
for (const key of required) {
  if (!process.env[key]) throw new Error(\`Missing \${key}\`);
}
const headers = {
  "authorization": \`Bearer \${process.env.PILOT_API_KEY ?? process.env.PILOT_API_KEY}\`,
  "content-type": "application/json"
};
const sessionRes = await fetch(\`\${process.env.PILOT_API_URL ?? process.env.PILOT_API_URL}/api/tool-gateway/sessions\`, {
  method: "POST",
  headers,
  body: JSON.stringify({ runId: process.env.PILOT_RUN_ID ?? process.env.PILOT_RUN_ID, ttlMs: 60000 })
});
if (!sessionRes.ok) throw new Error(\`session \${sessionRes.status}: \${await sessionRes.text()}\`);
const session = await sessionRes.json();
const toolsRes = await fetch(\`\${process.env.PILOT_API_URL ?? process.env.PILOT_API_URL}/api/tool-gateway/tools\`, {
  headers: { "x-pilot-tool-gateway-token": session.token }
});
...
const callRes = await fetch(\`\${process.env.PILOT_API_URL ?? process.env.PILOT_API_URL}/api/tool-gateway/tools/call\`, {
  method: "POST",
  headers: {
    "content-type": "application/json",
    "x-pilot-tool-gateway-token": session.token
  },
  body: JSON.stringify({
    tool: tool.name,
