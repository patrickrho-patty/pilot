...
async function createPipelineWriterAgentKey(board: APIRequestContext, companyId: string) {
  ...
  if (!token) throw new Error("PILOT_AGENT_JWT_SECRET is required for pipeline writer JWT setup");
...
async function dragCardToColumn(page: Page, itemTitle: string, fromColumn: string, toColumn: string) {
  ...
  try {
    // The move-confirmation dialog can render slowly when the suite runs under
    // load; a short window here makes the fallback path double-trigger a move.
  ...
  test("walks setup, intake, board moves, item detail, review queue, and learnings", async ({ page }) => {
    ...
    try {
      ...
      await page.evaluate((companyId) => {
        window.localStorage.setItem("pilot.selectedCompanyId", companyId);
      }, company.id);
    ...
    if (tweetDragged) {
      ...
      // The dialog can re-render between hit-testing and commit; retry the
      // click until the dialog actually closes and the move lands.
      await expect(
        async () => {
